...
class TradeCloser:
    ...
    async def should_close_trade(self, trade_id, trade_info):
        try:
            ...
            raw_candles = await self.client.get_candles(instrument, CONFIG.CANDLE_GRANULARITY, CONFIG.CANDLE_COUNT)
            ...
            current_price = float(candles[-1]["mid"]["c"]) if "mid" in candles[-1] else float(candles[-1]["close"])
