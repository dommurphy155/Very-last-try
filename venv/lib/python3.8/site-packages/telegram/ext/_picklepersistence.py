...
def _all_subclasses(cls: Type[TelegramObj]) -> Set[Type[TelegramObj]]:
    ...
    return set(subclasses).union([s for c in subclasses for s in
    _all_subclasses(c)])


def _reconstruct_to(cls: Type[TelegramObj], kwargs: dict) -> TelegramObj:
    """
    This method is used for unpickling. The data, which is in the form a
    dictionary, is
    converted back into a class. Works mostly the same as:meth:`TelegramObject.__setstate__`.
    This function should be kept in place for backwards compatibility even if
    the pickling logic
    is changed, since `_custom_reduction` places references to this function
    into the pickled data.
    """
    obj = cls.__new__(cls)
...
def _custom_reduction(cls: TelegramObj) -> Tuple[Callable,
    Tuple[Type[TelegramObj], dict]]:
    """
    This method is used for pickling. The bot attribute is preserved so
    _BotPickler().persistent_id
    works as intended.
    """
    data = cls._get_attrs(include_private=True)  # pylint:
...
class _BotPickler(pickle.Pickler):
    ...
    def __init__(self, bot: Bot, *args: Any, **kwargs: Any):
        ...
        if py_ver < (3, 8):  # self.reducer_override is used above this version
            # Here we define a private dispatch_table, because we want to
    preserve the bot
            # attribute of objects so persistent_id works as intended.
    Otherwise, the bot attribute
            # is deleted in __getstate__, which is used during regular
    pickling (via pickle.dumps)
    ...
    https://docs.python.org/3/library/pickle.html#pickle.Pickler.persistent_id for more info
        ...
        if isinstance(obj, Bot):
            warn(
                "Unknown bot instance found. Will be replaced by `None` during
    unpickling",
                stacklevel=2,
            )
...
class _BotUnpickler(pickle.Unpickler):
    ...
    def persistent_load(self, pid: str) -> Optional[Bot]:
        """Replaces the bot with the current bot if known, else it is replaced
    by:obj:`None`."""
        if pid == _REPLACED_KNOWN_BOT:
            return self._bot
        if pid == _REPLACED_UNKNOWN_BOT:
            return None
        raise pickle.UnpicklingError("Found unknown persistent id when
    unpickling!")
...
class PicklePersistence(BasePersistence[UD, CD, BD]):
    ...
    Note:
        This implementation of:class:`BasePersistence` uses the functionality
    of the pickle module
        to support serialization of bot instances. Specifically any reference
    to:attr:`~BasePersistence.bot` will be replaced by a placeholder before
    pickling and:attr:`~BasePersistence.bot` will be inserted back when loading the
    data.

    Examples::any:`Persistent Conversation Bot <examples.persistentconversationbot>`
    ...
    Args:
        filepath (:obj:`str` |:obj:`pathlib.Path`): The filepath for storing
    the pickle files.
            When:attr:`single_file` is:obj:`False` this will be used as a
    prefix.
    ...
    available kinds of
            data will be saved.
        single_file (:obj:`bool`, optional): When:obj:`False` will store 5
    separate files of
        ...
        on_flush (:obj:`bool`, optional): When:obj:`True` will only save to
    file when:meth:`flush` is called and keep data in memory until that
    happens. When:obj:`False` will store data on any transaction *and* on call to:meth:`flush`.
            Default is:obj:`False`.
        context_types (:class:`telegram.ext.ContextTypes`, optional): Pass an
    instance
            of:class:`telegram.ext.ContextTypes` to customize the types used
    in the
            ``context`` interface. If not passed, the defaults documented in:class:`telegram.ext.ContextTypes` will be used.
    ...
    happens. When:obj:`False` will store data on any transaction *and* on call to:meth:`flush`.
            Default is:obj:`False`.
        context_types (:class:`telegram.ext.ContextTypes`): Container for the
    types used
    ...
    @overload
    def __init__(
        self: "PicklePersistence[Dict[Any, Any], Dict[Any, Any], Dict[Any,
    Any]]",
    ...
    update_interval=update_interval)
        ...
        self.conversations: Optional[Dict[str, Dict[Tuple[Union[int, str],
    ...], object]]] = None
    ...
    def _load_singlefile(self) -> None:
        try:
            ...
            raise TypeError(f"File {filename} does not contain valid pickle
    data") from exc
        except Exception as exc:
            raise TypeError(f"Something went wrong unpickling
    {self.filepath.name}") from exc
    ...
    def _load_file(self, filepath: Path) -> Any:
        try:
            ...
            raise TypeError(f"File {filepath.name} does not contain valid
    pickle data") from exc
        except Exception as exc:
            raise TypeError(f"Something went wrong unpickling
    {filepath.name}") from exc
    ...
    object of type:obj:`dict` |:attr:`telegram.ext.ContextTypes.bot_data`.

        Returns::obj:`dict` |:attr:`telegram.ext.ContextTypes.bot_data`: The
    restored bot data.
        """
        if self.bot_data:
            pass
    ...
    async def get_conversations(self, name: str) -> ConversationDict:
        """Returns the conversations from the pickle file if it exists or an
    empty dict.
        ...
        if self.conversations:
            ...
            self._load_singlefile()
        return self.conversations.get(name, {}).copy()  # type:
    ignore[union-attr]
    ...
    on:attr:`on_flush`
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(Path(f"{self.filepath}_conversations"),
    self.conversations)
            else:
    ...
    for.
            data (:obj:`dict`): The:attr:`telegram.ext.Application.user_data`
    ``[user_id]``.
        """
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(Path(f"{self.filepath}_user_data"),
    self.user_data)
            else:
                self._dump_singlefile()
    ...
    ``[chat_id]``.
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(Path(f"{self.filepath}_chat_data"),
    self.chat_data)
            else:
                self._dump_singlefile()
    ...
    The:attr:`telegram.ext.Application.bot_data`.
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(Path(f"{self.filepath}_bot_data"),
    self.bot_data)
            else:
                self._dump_singlefile()
    ...
    async def update_callback_data(self, data: CDCData) -> None:
        ...
        if not self.on_flush:
            if not self.single_file:
                self._dump_file(Path(f"{self.filepath}_callback_data"),
    self.callback_data)
            else:
                self._dump_singlefile()
    ...
    on:attr:`on_flush` save the pickle file.
