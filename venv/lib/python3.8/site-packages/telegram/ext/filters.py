...
"""
This module contains filters for use with:class:`telegram.ext.MessageHandler`,:class:`telegram.ext.CommandHandler`, or:class:`telegram.ext.PrefixHandler`.

.. versionchanged:: 20.0

    #. Filters are no longer callable, if you're using a custom filter and are
    calling an existing
       filter, then switch to the new syntax:
    ``filters.{filter}.check_update(update)``.
    #. Removed the ``Filters`` class. The filters are now directly
    attributes/classes of the:mod:`~telegram.ext.filters` module.
...
class BaseFilter:
    ...
    Note:
        Filters use the same short circuiting logic as python's:keyword:`and`,:keyword:`or` and:keyword:`not`. This means that for example::

            filters.Regex(r'(a?x)') | filters.Regex(r'(b?x)')

        With ``message.text == 'x'``, will only ever return the matches for
    the first filter,
        since the second one is never evaluated.

    If you want to create your own filters create a class inheriting from
    either:class:`MessageFilter` or:class:`UpdateFilter` and implement a
    ``filter()``
    method that returns a boolean::obj:`True` if the message should be
    handled,:obj:`False` otherwise.
    Note that the filters work only as class instances, not actual class
    objects (so remember to
    initialize your filter classes).

    By default, the filters name (what will get printed when converted to a
    string for display)
    will be the class name. If you want to overwrite this assign a better name
    to the:attr:`name`
    class variable.
    ...
    Args:
        name (:obj:`str`): Name for this filter. Defaults to the type of
    filter.
        data_filter (:obj:`bool`): Whether this filter is a data filter. A
    data filter should
            return a dict with lists. The dict will be merged with:class:`telegram.ext.CallbackContext`'s internal dict in most cases
    ...
    def check_update(  # skipcq: PYL-R0201
        ...
        Returns::obj:`bool`::obj:`True` if the update contains one of:attr:`~telegram.Update.channel_post`,:attr:`~telegram.Update.message`,:attr:`~telegram.Update.edited_channel_post` or:attr:`~telegram.Update.edited_message`,:obj:`False` otherwise.
        """
...
class MessageFilter(BaseFilter):
    ...
    def check_update(self, update: Update) -> Optional[Union[bool,
    FilterDataDict]]:
        """Checks if the specified update should be handled by this filter by
    passing:attr:`~telegram.Update.effective_message` to:meth:`filter`.

        Args:
            update (:class:`telegram.Update`): The update to check.

        Returns::obj:`bool` | Dict[:obj:`str`,:obj:`list`] |:obj:`None`: If the
    update should be
            handled by this filter, returns:obj:`True` or a dict with lists,
    in case the filter
            is a data filter. If the update should not be handled by this
    filter,:obj:`False` or:obj:`None`.
        """
        if super().check_update(update):
            return self.filter(update.effective_message)  # type:
    ignore[arg-type]
        return False
...
class UpdateFilter(BaseFilter):
    ...
    passed to:meth:`filter` is an instance of:class:`telegram.Update`, which
    allows to create
    filters like:attr:`telegram.ext.filters.UpdateType.EDITED_MESSAGE`.

    Please see:class:`telegram.ext.filters.BaseFilter` for details on how to
    create custom
    filters.
...
class _MergedFilter(UpdateFilter):
    ...
    Args:
        ...
        and_filter: Optional filter to "and" with base_filter. Mutually
    exclusive with or_filter.
        or_filter: Optional filter to "or" with base_filter. Mutually
    exclusive with and_filter.
    ...
    or_filter: BaseFilter = None
        ...
        if self.or_filter and not isinstance(self.and_filter, bool) and
    self.or_filter.data_filter:
            self.data_filter = True

    @staticmethod
    def _merge(base_output: Union[bool, Dict], comp_output: Union[bool, Dict])
    -> FilterDataDict:
    ...
    def filter(self, update: Update) -> Union[bool, FilterDataDict]:
        ...
        # We need to check if the filters are data filters and if so return
    the merged data.
...
class _XORFilter(UpdateFilter):
    """Convenience filter acting as wrapper for:class:`MergedFilter`
    representing the an XOR gate
    for two filters.
    ...
    def __init__(self, base_filter: BaseFilter, xor_filter: BaseFilter):
        ...
        self.merged_filter = (base_filter & ~xor_filter) | (~base_filter &
    xor_filter)
...
class Caption(MessageFilter):
    """Messages with a caption. If a list of strings is passed, it filters
    messages to only
    allow those whose caption is appearing in the given list.

    Examples:
        ``MessageHandler(filters.Caption(['PTB rocks!', 'PTB']),
    callback_method_2)``

    .. seealso:::attr:`telegram.ext.filters.CAPTION`

    Args:
        strings (List[:obj:`str`] | Tuple[:obj:`str`], optional): Which
    captions to allow. Only
            exact matches are allowed. If not specified, will allow any
    ...
    def __init__(self, strings: Union[List[str], Tuple[str, ...]] = None):
        ...
        super().__init__(name=f"filters.Caption({strings})" if strings else
    "filters.CAPTION")
...
Examples:
    To allow any caption, simply use ``MessageHandler(filters.CAPTION,
    callback_method)``.
"""
...
class CaptionEntity(MessageFilter):
    ...
    Args:
        entity_type (:obj:`str`): Caption Entity type to check for. All types
    can be found as
    ...
    def filter(self, message: Message) -> bool:
        return any(entity.type == self.entity_type for entity in
...
class CaptionRegex(MessageFilter):
    ...
    This filter works similarly to:class:`Regex`, with the only exception
    being that
    it applies to the message caption instead of the text.

    Examples:
        Use ``MessageHandler(filters.PHOTO & filters.CaptionRegex(r'help'),
    callback)``
        to capture all photos with caption containing the word 'help'.

    Note:
        This filter will not work on simple text messages, but only on media
    with caption.

    Args:
        pattern (:obj:`str` |:func:`re.Pattern <re.compile>`): The regex
    pattern.
    """
    ...
    def __init__(self, pattern: Union[str, Pattern[str]]):
        ...
        super().__init__(name=f"filters.CaptionRegex({self.pattern})",
    data_filter=True)

    def filter(self, message: Message) -> Optional[Dict[str,
    List[Match[str]]]]:
        if message.caption:
...
class _ChatUserBaseFilter(MessageFilter, ABC):
    ...
    @abstractmethod
    def _get_chat_or_user(self, message: Message) -> Union[TGChat, TGUser,
    None]:
        ...
    ...
    def _parse_username(username: Optional[SCT[str]]) -> Set[str]:
        ...
        return {chat[1:] if chat.startswith("@") else chat for chat in
    username}

    def _set_chat_ids(self, chat_id: Optional[SCT[int]]) -> None:
        if chat_id and self._usernames:
            raise RuntimeError(
                f"Can't set {self._chat_id_name} in conjunction with (already
    set) "
    ...
    def _set_usernames(self, username: Optional[SCT[str]]) -> None:
        if username and self._chat_ids:
            raise RuntimeError(
                f"Can't set {self._username_name} in conjunction with (already
    set) "
    ...
    def usernames(self) -> FrozenSet[str]:
        ...
        Warning::attr:`usernames` will give a *copy* of the saved usernames as:obj:`frozenset`. This
            ...
            ``filter.usernames = new_set``, if you are entirely sure that it
    is not causing race
            conditions, as this will complete replace the current set of
    allowed users.
    ...
    def add_usernames(self, username: SCT[str]) -> None:
        ...
        Args:
            username(:obj:`str` | Collection[:obj:`str`]): Which username(s) to
                allow through. Leading ``'@'`` s in usernames will be
    discarded.
        """
        if self._chat_ids:
            raise RuntimeError(
    ...
    def remove_usernames(self, username: SCT[str]) -> None:
        ...
        Args:
            username(:obj:`str` | Collection[:obj:`str`]): Which username(s) to
                disallow through. Leading ``'@'`` s in usernames will be
    discarded.
        """
        if self._chat_ids:
            raise RuntimeError(
    ...
    def filter(self, message: Message) -> bool:
        ...
        if chat_or_user:
            ...
            if self.usernames:
                return bool(chat_or_user.username and chat_or_user.username in
    self.usernames)
    ...
    @name.setter
    def name(self, name: str) -> NoReturn:
        raise RuntimeError(f"Cannot set name for
    filters.{self.__class__.__name__}")


class Chat(_ChatUserBaseFilter):
    """Filters messages to allow only those which are from a specified chat ID
    or username.

    Examples:
        ``MessageHandler(filters.Chat(-1234), callback_method)``

    Warning::attr:`chat_ids` will give a *copy* of the saved chat ids as:class:`frozenset`. This
        is to ensure thread safety. To add/remove a chat, you should use:meth:`add_chat_ids`, and:meth:`remove_chat_ids`. Only update the entire set by
    ``filter.chat_ids = new_set``,
        if you are entirely sure that it is not causing race conditions, as
    this will complete
    ...
    Args:
        ...
        allow_empty(:obj:`bool`, optional): Whether updates should be
    processed, if no chat
            is specified in:attr:`chat_ids` and:attr:`usernames`. Defaults
    to:obj:`False`.

    Attributes:
        chat_ids (set(:obj:`int`)): Which chat ID(s) to allow through.
        allow_empty (:obj:`bool`): Whether updates should be processed, if no
    chat
...
class ChatType:  # A convenience namespace for Chat types.
    ...
    Caution:
        ``filters.ChatType`` itself is *not* a filter, but just a convenience
    namespace.
    """
...
class Command(MessageFilter):
    """
    Messages with a:attr:`telegram.MessageEntity.BOT_COMMAND`. By default,
    only allows
    ...
    .. seealso:::attr:`telegram.ext.filters.COMMAND`.

    Note::attr:`telegram.ext.filters.TEXT` also accepts messages containing a
    command.

    Args:
        only_start (:obj:`bool`, optional): Whether to only allow messages
    that `start` with a bot
    ...
    def __init__(self, only_start: bool = True):
        ...
        super().__init__(f"filters.Command({only_start})" if not only_start
    else "filters.COMMAND")
    ...
    def filter(self, message: Message) -> bool:
        ...
        if self.only_start:
            return bool(first.type == MessageEntity.BOT_COMMAND and
    first.offset == 0)
        return bool(any(e.type == MessageEntity.BOT_COMMAND for e in
    message.entities))
...
class _Dice(MessageFilter):
    ...
    def __init__(self, values: SCT[int] = None, emoji: DiceEmojiEnum = None):
        ...
        self.values: Optional[Collection[int]] = [values] if
    isinstance(values, int) else values

        if emoji:  # for filters.Dice.BASKETBALL
            self.name = f"filters.Dice.{emoji.name}"
            if self.values and emoji:  # for filters.Dice.Dice(4) 
    SLOT_MACHINE -> SlotMachine
                self.name = f"filters.Dice.{emoji.name.title().replace('_',
    '')}({self.values})"
    ...
    def filter(self, message: Message) -> bool:
        ...
        if self.emoji:
            ...
            if self.values:
                return message.dice.value in self.values and emoji_match  #
    emoji and value
            return emoji_match  # emoji, no value
        return message.dice.value in self.values if self.values else True  #
    no emoji, only value


class Dice(_Dice):
    """Dice Messages. If an integer or a list of integers is passed, it
    filters messages to only
    ...
    Examples:
        ...
        To allow only dice messages with the emoji ⚽ and with value 5 `or` 6,
    use
        ``MessageHandler(filters.Dice.Football([5, 6]), callback_method)``.

    Note:
        Dice messages don't have text. If you want to filter either text or
    dice messages, use
        ``filters.TEXT | filters.Dice.ALL``.

    Args:
        values (:obj:`int` | Collection[:obj:`int`], optional):
            Which values to allow. If not specified, will allow the specified
    dice message.
    """
    ...
    class Basketball(_Dice):
        ...
        Args:
            values (:obj:`int` | Collection[:obj:`int`]): Which values to
    allow.
        """
...
class Document:
    ...
    Examples:
        ...
        ``filters.Document.MimeType("text/plain")`` etc. Or just use
    ``filters.Document.ALL`` for
        all document messages.

    Caution:
        ``filters.Document`` itself is *not* a filter, but just a convenience
    namespace.
    """
    ...
    class Category(MessageFilter):
        ...
        Example:
            ``filters.Document.Category('audio/')`` returns:obj:`True` for
    all types
            of audio sent as a file, for example ``'audio/mpeg'`` or
    ``'audio/x-wav'``.

        Note:
            This Filter only filters by the mime_type of the document, it
    doesn't check the
            validity of the document. The user can manipulate the mime-type of
    a message and
    ...
    class FileExtension(MessageFilter):
        ...
        Args:
            file_extension (:obj:`str` |:obj:`None`): Media file extension
    you want to filter.
            case_sensitive (:obj:`bool`, optional): Pass:obj:`True` to make
    the filter case
        ...
        Example:
            ...
            * ``filters.Document.FileExtension(".jpg")``
              filters files with extension ``"..jpg"``.
            * ``filters.Document.FileExtension("Dockerfile",
    case_sensitive=True)``
        ...
        Note:
            * This Filter only filters by the file ending/extension of the
    document,
        ...
        def __init__(self, file_extension: Optional[str], case_sensitive: bool
    = False):
            super().__init__()
            ...
            if file_extension is None:
                ...
                self.name = (
                    f"filters.Document.FileExtension({file_extension!r},
    case_sensitive=True)"
                )
            else:
    ...
    class MimeType(MessageFilter):
        ...
        Example:
            ``filters.Document.MimeType('audio/mpeg')`` filters all audio in
    `.mp3` format.

        Note:
            This Filter only filters by the mime_type of the document, it
    doesn't check the
            validity of document. The user can manipulate the mime-type of a
    message and
            send media with wrong types that don't fit to this handler.
        """
...
class Entity(MessageFilter):
    ...
    Args:
        entity_type (:obj:`str`): Entity type to check for. All types can be
    found as constants
...
class ForwardedFrom(_ChatUserBaseFilter):
    """Filters messages to allow only those which are forwarded from the
    specified chat ID(s)
    or username(s) based on:attr:`telegram.Message.forward_from` and:attr:`telegram.Message.forward_from_chat`.

    .. versionadded:: 13.5

    Examples:
        ``MessageHandler(filters.ForwardedFrom(chat_id=1234),
    callback_method)``

    Note:
        When a user has disallowed adding a link to their account while
    forwarding their
        messages, this filter will *not* work since both:attr:`telegram.Message.forward_from` and:attr:`telegram.Message.forward_from_chat` are:obj:`None`. However,
    this behaviour
    ...
    ``filter.chat_ids = new_set``, if
        you are entirely sure that it is not causing race conditions, as this
    will complete replace
    ...
    Args:
        ...
        username(:obj:`str` | Collection[:obj:`str`], optional):
            Which username(s) to allow through. Leading ``'@'`` s in usernames
    will be
            discarded.
        allow_empty(:obj:`bool`, optional): Whether updates should be
    processed, if no chat
    ...
    def _get_chat_or_user(self, message: Message) -> Union[TGUser, TGChat,
    None]:
    ...
    def add_chat_ids(self, chat_id: SCT[int]) -> None:
        ...
        Args:
            chat_id(:obj:`int` | Collection[:obj:`int`]): Which chat/user
    ID(s) to
                allow through.
        """
        return super()._add_chat_ids(chat_id)
...
HAS_PROTECTED_CONTENT =
    _HasProtectedContent(name="filters.HAS_PROTECTED_CONTENT")
...
class Language(MessageFilter):
    """Filters messages to only allow those which are from users with a
    certain language code.

    Note:
        According to official Telegram Bot API documentation, not every single
    user has the
        `language_code` attribute. Do not count on this filter working on all
    users.
    ...
    def filter(self, message: Message) -> bool:
        return bool(
            ...
            and any(message.from_user.language_code.startswith(x) for x in
    self.lang)
        )
...
class Regex(MessageFilter):
    ...
    text.
    The:func:`re.search` function is used to determine whether an update
    should be filtered.
    ...
    all messages that
        ...
        ``MessageHandler(filters.Regex(re.compile(r'help', re.IGNORECASE)),
    callback)`` if
        you want your pattern to be case insensitive. This approach is
    recommended
        if you need to specify flags on your pattern.

    Note:
        Filters use the same short circuiting logic as python's:keyword:`and`,:keyword:`or` and:keyword:`not`.
        This means that for example:

            >>> filters.Regex(r'(a?x)') | filters.Regex(r'(b?x)')

        With a:attr:`telegram.Message.text` of `x`, will only ever return the
    matches for the
    ...
    def __init__(self, pattern: Union[str, Pattern[str]]):
        if isinstance(pattern, str):
            pattern = re.compile(pattern)
        self.pattern: Pattern[str] = pattern
        super().__init__(name=f"filters.Regex({self.pattern})",
    data_filter=True)
...
class SenderChat(_ChatUserBaseFilter):
    """Filters messages to allow only those which are from a specified sender
    chat's chat ID or
    username.

    Examples:
        * To filter for messages sent to a group by a channel with ID
          ``-1234``, use ``MessageHandler(filters.SenderChat(-1234),
    callback_method)``.
        * To filter for messages of anonymous admins in a super group with
    username
          ``@anonymous``, use
          ``MessageHandler(filters.SenderChat(username='anonymous'),
    callback_method)``.
        ...
        * To filter for messages forwarded to a discussion group from *any*
    channel or of anonymous
          admins in *any* super group, use
    ``MessageHandler(filters.SenderChat.ALL, callback)``

    Note:
        Remember, ``sender_chat`` is also set for messages in a channel as the
    channel itself,
        so when your bot is an admin in a channel and the linked discussion
    group, you would
        receive the message twice (once from inside the channel, once inside
    the discussion
        group). Since v13.9, the field:attr:`telegram.Message.is_automatic_forward` will be:obj:`True` for the discussion group message.

    .. seealso:::attr:`telegram.ext.filters.IS_AUTOMATIC_FORWARD`

    Warning::attr:`chat_ids` will return a *copy* of the saved chat ids as:obj:`frozenset`. This
        is to ensure thread safety. To add/remove a chat, you should use:meth:`add_chat_ids`, and:meth:`remove_chat_ids`. Only update the entire set by
    ``filter.chat_ids = new_set``, if
        you are entirely sure that it is not causing race conditions, as this
    will complete replace
    ...
    Args:
        ...
        username(:obj:`str` | Collection[:obj:`str`], optional):
            ...
            Leading ``'@'`` s in usernames will be discarded.
        allow_empty(:obj:`bool`, optional): Whether updates should be
    processed, if no sender
            chat is specified in:attr:`chat_ids` and:attr:`usernames`.
    Defaults to:obj:`False`.

    Attributes:
        chat_ids (set(:obj:`int`)): Which sender chat chat ID(s) to allow
    through.
        allow_empty (:obj:`bool`): Whether updates should be processed, if no
    sender chat is
    ...
    def add_chat_ids(self, chat_id: SCT[int]) -> None:
        ...
        Args:
            chat_id(:obj:`int` | Collection[:obj:`int`]): Which sender chat
    ID(s) to
                allow through.
        """
        return super()._add_chat_ids(chat_id)
...
class StatusUpdate:
    ...
    Examples:
        Use these filters like: ``filters.StatusUpdate.NEW_CHAT_MEMBERS`` etc.
    Or use just
        ``filters.StatusUpdate.ALL`` for all status update messages.

    Caution:
        ``filters.StatusUpdate`` itself is *not* a filter, but just a
    ...
    class _All(UpdateFilter):
        ...
        def filter(self, update: Update) -> bool:
            return bool(
                ...
                or
    StatusUpdate.MESSAGE_AUTO_DELETE_TIMER_CHANGED.check_update(update)
    ...
    """Messages that contain:attr:`telegram.Message.group_chat_created`,:attr:`telegram.Message.supergroup_chat_created` or:attr:`telegram.Message.channel_chat_created`."""
    ...
    CONNECTED_WEBSITE =
    _ConnectedWebsite(name="filters.StatusUpdate.CONNECTED_WEBSITE")
    ...
    DELETE_CHAT_PHOTO =
    _DeleteChatPhoto(name="filters.StatusUpdate.DELETE_CHAT_PHOTO")
    ...
    FORUM_TOPIC_CLOSED =
    _ForumTopicClosed(name="filters.StatusUpdate.FORUM_TOPIC_CLOSED")
    ...
    FORUM_TOPIC_CREATED =
    _ForumTopicCreated(name="filters.StatusUpdate.FORUM_TOPIC_CREATED")
    ...
    FORUM_TOPIC_EDITED =
    _ForumTopicEdited(name="filters.StatusUpdate.FORUM_TOPIC_EDITED")
    ...
    _ForumTopicReopened(name="filters.StatusUpdate.FORUM_TOPIC_REOPENED")
    ...
    LEFT_CHAT_MEMBER =
    _LeftChatMember(name="filters.StatusUpdate.LEFT_CHAT_MEMBER")
    ...
    class _Migrate(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.migrate_from_chat_id or
    message.migrate_to_chat_id)

    MIGRATE = _Migrate(name="filters.StatusUpdate.MIGRATE")
    """Messages that contain:attr:`telegram.Message.migrate_from_chat_id` or:attr:`telegram.Message.migrate_to_chat_id`."""
    ...
    NEW_CHAT_MEMBERS =
    _NewChatMembers(name="filters.StatusUpdate.NEW_CHAT_MEMBERS")
    ...
    VIDEO_CHAT_ENDED =
    _VideoChatEnded(name="filters.StatusUpdate.VIDEO_CHAT_ENDED")
    ...
    _VideoChatScheduled(name="filters.StatusUpdate.VIDEO_CHAT_SCHEDULED")
    ...
    VIDEO_CHAT_STARTED =
    _VideoChatStarted(name="filters.StatusUpdate.VIDEO_CHAT_STARTED")
    ...
    _WriteAccessAllowed(name="filters.StatusUpdate.WRITE_ACCESS_ALLOWED")
...
class Sticker:
    ...
    Examples:
        Use this filter like: ``filters.Sticker.VIDEO``. Or, just use
    ``filters.Sticker.ALL`` for
        any type of sticker.

    Caution:
        ``filters.Sticker`` itself is *not* a filter, but just a convenience
    namespace.
    """
    ...
    class _Animated(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and bool(message.sticker.is_animated)
    # type: ignore

    ANIMATED = _Animated(name="filters.Sticker.ANIMATED")
    """Messages that contain:attr:`telegram.Message.sticker` and:attr:`is animated <telegram.Sticker.is_animated>`.
    ...
    class _Static(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and (
                not bool(message.sticker.is_animated)  # type:
    ignore[union-attr]
                and not bool(message.sticker.is_video)  # type:
    ignore[union-attr]
            )

    STATIC = _Static(name="filters.Sticker.STATIC")
    """Messages that contain:attr:`telegram.Message.sticker` and is a static
    sticker, i.e. does
    ...
    class _Video(MessageFilter):
        ...
        def filter(self, message: Message) -> bool:
            return bool(message.sticker) and bool(message.sticker.is_video)  #
    type: ignore

    VIDEO = _Video(name="filters.Sticker.VIDEO")
    """Messages that contain:attr:`telegram.Message.sticker` and is a:attr:`video sticker <telegram.Sticker.is_video>`.
    ...
    """Messages that contain:attr:`telegram.Message.sticker` and have a:attr:`premium animation <telegram.Sticker.premium_animation>`.
...
class Text(MessageFilter):
    """Text Messages. If a list of strings is passed, it filters messages to
    only allow those
    whose text is appearing in the given list.

    Examples:
        A simple use case for passing a list is to allow only messages that
    were sent by a
    ...
    .. seealso:::attr:`telegram.ext.filters.TEXT`
    ...
    dice messages, use
        ...
        * Messages containing a command are accepted by this filter. Use
          ``filters.TEXT & (~filters.COMMAND)``, if you want to filter only
    text messages without
          commands.

    Args:
        strings (List[:obj:`str`] | Tuple[:obj:`str`], optional): Which
    messages to allow. Only
            exact matches are allowed. If not specified, will allow any text
    message.
    """
    ...
    def __init__(self, strings: Union[List[str], Tuple[str, ...]] = None):
        self.strings: Optional[Sequence[str]] = strings
        super().__init__(name=f"filters.Text({strings})" if strings else
    "filters.TEXT")
...
class UpdateType:
    ...
    Caution:
        ``filters.UpdateType`` itself is *not* a filter, but just a
    convenience namespace.
    """
    ...
    class _ChannelPosts(UpdateFilter):
        ...
        def filter(self, update: Update) -> bool:
            return update.channel_post is not None or
    update.edited_channel_post is not None

    CHANNEL_POSTS = _ChannelPosts(name="filters.UpdateType.CHANNEL_POSTS")
    """Updates with either:attr:`telegram.Update.channel_post` or:attr:`telegram.Update.edited_channel_post`."""

    class _Edited(UpdateFilter):
        __slots__ = ()

        def filter(self, update: Update) -> bool:
            return update.edited_message is not None or
    update.edited_channel_post is not None

    EDITED = _Edited(name="filters.UpdateType.EDITED")
    """Updates with either:attr:`telegram.Update.edited_message` or:attr:`telegram.Update.edited_channel_post`.
    ...
    EDITED_CHANNEL_POST =
    _EditedChannelPost(name="filters.UpdateType.EDITED_CHANNEL_POST")
    ...
    class _Messages(UpdateFilter):
        ...
        def filter(self, update: Update) -> bool:
            return update.message is not None or update.edited_message is not
    None

    MESSAGES = _Messages(name="filters.UpdateType.MESSAGES")
    """Updates with either:attr:`telegram.Update.message` or:attr:`telegram.Update.edited_message`."""


class User(_ChatUserBaseFilter):
    """Filters messages to allow only those which are from specified user
    ID(s) or
    username(s).
    ...
    Args:
        user_id(:obj:`int` | Collection[:obj:`int`], optional): Which user
    ID(s) to
            allow through.
        username(:obj:`str` | Collection[:obj:`str`], optional):
            Which username(s) to allow through. Leading ``'@'`` s in usernames
    ...
    Attributes:
        allow_empty (:obj:`bool`): Whether updates should be processed, if no
    user is specified in:attr:`user_ids` and:attr:`usernames`.
    """
    ...
    def __init__(
        ...
        super().__init__(chat_id=user_id, username=username,
    ...
    def user_ids(self) -> FrozenSet[int]:
        ...
        Warning::attr:`user_ids` will give a *copy* of the saved user ids as:obj:`frozenset`. This
            ...
            ``filter.user_ids = new_set``, if you are entirely sure that it is
    not causing race
            conditions, as this will complete replace the current set of
    allowed users.
...
"""This filter filters *any* message that have a user who added the bot to
    their:attr:`attachment menu <telegram.User.added_to_attachment_menu>` as:attr:`telegram.Update.effective_user`.
...
"""This filter filters *any* message from a:attr:`Telegram Premium user <telegram.User.is_premium>` as:attr:`telegram.Update.effective_user`.
...
class ViaBot(_ChatUserBaseFilter):
    """Filters messages to allow only those which are from specified via_bot
    ID(s) or username(s).
    ...
    Args:
        bot_id(:obj:`int` | Collection[:obj:`int`], optional): Which bot ID(s)
    to
            allow through.
        username(:obj:`str` | Collection[:obj:`str`], optional):
            Which username(s) to allow through. Leading ``'@'`` s in usernames
    ...
    Attributes:
        allow_empty (:obj:`bool`): Whether updates should be processed, if no
    bot is specified in:attr:`bot_ids` and:attr:`usernames`.
    """
    ...
    def __init__(
        ...
        super().__init__(chat_id=bot_id, username=username,
    allow_empty=allow_empty)
    ...
    def bot_ids(self) -> FrozenSet[int]:
        ...
        Warning::attr:`bot_ids` will give a *copy* of the saved bot ids as:obj:`frozenset`. This
            is to ensure thread safety. To add/remove a bot, you should use:meth:`add_bot_ids`,
            and:meth:`remove_bot_ids`. Only update the entire set by
    ``filter.bot_ids = new_set``,
            if you are entirely sure that it is not causing race conditions,
    ...
    def add_bot_ids(self, bot_id: SCT[int]) -> None:
        ...
        Args:
            bot_id(:obj:`int` | Collection[:obj:`int`]): Which bot ID(s) to
    allow
                through.
        """
    ...
    def remove_bot_ids(self, bot_id: SCT[int]) -> None:
        ...
        Args:
            bot_id(:obj:`int` | Collection[:obj:`int`], optional): Which bot
    ID(s) to
                disallow through.
        """
