...
"""This module contains helper functions related to inspecting the program
    stack.
...
def was_called_by(frame: Optional[FrameType], caller: Path) -> bool:
    ...
    Arguments:
        frame (:obj:`FrameType`): The frame - usually the return value of
            ``inspect.currentframe()``. If:obj:`None` is passed, the return
    value will be:obj:`False`.
        caller (:obj:`pathlib.Path`): File that should be the caller.

    Returns::obj:`bool`: Whether the frame was called by the specified file.
    """
    if frame is None:
        return False
    ...
    try:
        ...
        _LOGGER.debug(
            "Failed to check if frame was called by `caller`. Assuming that it
    was not.",
            exc_info=exc,
        )
        return False
