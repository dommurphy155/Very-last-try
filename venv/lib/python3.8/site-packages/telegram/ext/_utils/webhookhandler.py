...
# Instead of checking for ImportError here, we do that in `updater.py`, where
    we import from
# this module. Doing it here would be tricky, as the classes below subclass
    tornado classes
import tornado.web
...
class WebhookServer:
    ...
    def __init__(
        self, listen: str, port: int, webhook_app: "WebhookAppClass", ssl_ctx:
    Optional[SSLContext]
    ):
...
class WebhookAppClass(tornado.web.Application):
    ...
    def __init__(
        self, webhook_path: str, bot: "Bot", update_queue: asyncio.Queue,
    secret_token: str = None
        ...
        handlers = [(rf"{webhook_path}/?", TelegramHandler,
    self.shared_objects)]
        tornado.web.Application.__init__(self, handlers)  # type: ignore

    def log_request(self, handler: tornado.web.RequestHandler) -> None:
        """Overrides the default implementation since we have our own logging
    setup."""
...
class TelegramHandler(tornado.web.RequestHandler):
    ...
    secret_token: str) -> None:
        """Initialize for each request - that's the interface provided by
    tornado"""
        ...
        if secret_token:
            _LOGGER.debug(
                "The webhook server has a secret token, expecting it in
    incoming requests now"
            )
    ...
    def _validate_post(self) -> None:
        ...
        # verifying that the secret token is the one the user set when the
    user set one
    ...
    secret token"
            ...
            if token != self.secret_token:
                ...
                raise tornado.web.HTTPError(
                    HTTPStatus.FORBIDDEN, reason="Request had the wrong secret
    token"
                )
