...
class DictPersistence(BasePersistence[Dict[Any, Any], Dict[Any, Any],
    Dict[Any, Any]]):
    """Using Python's:obj:`dict` and:mod:`json` for making your bot
    persistent.
    ...
    Note:
        * Data managed by:class:`DictPersistence` is in-memory only and will
    be lost when the bot
          shuts down. This is, because:class:`DictPersistence` is mainly
    intended as starting
          point for custom persistence classes that need to JSON-serialize the
    stored data before
          writing them to file/database.

        * This implementation of:class:`BasePersistence` does not handle data
    that cannot be
    ...
    available kinds of
            data will be saved.
        user_data_json (:obj:`str`, optional): JSON string that will be used
    to reconstruct
            user_data on creating this persistence. Default is ``""``.
        chat_data_json (:obj:`str`, optional): JSON string that will be used
    to reconstruct
            chat_data on creating this persistence. Default is ``""``.
        bot_data_json (:obj:`str`, optional): JSON string that will be used to
    reconstruct
            bot_data on creating this persistence. Default is ``""``.
        conversations_json (:obj:`str`, optional): JSON string that will be
    used to reconstruct
            conversation on creating this persistence. Default is ``""``.
        callback_data_json (:obj:`str`, optional): JSON string that will be
    used to reconstruct
    ...
    update_interval=update_interval)
        ...
        if user_data_json:
            try:
                self._user_data =
    self._decode_user_chat_data_from_json(user_data_json)
                self._user_data_json = user_data_json
            except (ValueError, AttributeError) as exc:
                raise TypeError("Unable to deserialize user_data_json. Not
    valid JSON") from exc
        if chat_data_json:
            try:
                self._chat_data =
    self._decode_user_chat_data_from_json(chat_data_json)
                self._chat_data_json = chat_data_json
            except (ValueError, AttributeError) as exc:
                raise TypeError("Unable to deserialize chat_data_json. Not
    valid JSON") from exc
        if bot_data_json:
            try:
                ...
                raise TypeError("Unable to deserialize bot_data_json. Not
    valid JSON") from exc
        ...
        if callback_data_json:
            ...
            # We are a bit more thorough with the checking of the format here,
    because it's
            ...
            try:
                if data is None:
                    ...
                    self._callback_data = cast(
                        CDCData,
                        ([(one, float(two), three) for one, two, three in
    data[0]], data[1]),
                    )
                self._callback_data_json = callback_data_json
            except (ValueError, IndexError) as exc:
                raise TypeError("callback_data_json is not in the required
    format") from exc
    ...
    format")
        ...
        if conversations_json:
            try:
                self._conversations =
    self._decode_conversations_from_json(conversations_json)
    ...
    def callback_data(self) -> Optional[CDCData]:
        ...
        Dict[:obj:`str`,:obj:`str`]]: The metadata on the stored callback
    data.
    ...
    def callback_data_json(self) -> str:
        """:obj:`str`: The metadata on the stored callback data as a
    JSON-string.
    ...
    async def get_user_data(self) -> Dict[int, Dict[object, object]]:
        """Returns the user_data created from the ``user_data_json`` or an
    empty:obj:`dict`.

        Returns::obj:`dict`: The restored user data.
        """
    ...
    async def get_chat_data(self) -> Dict[int, Dict[object, object]]:
        """Returns the chat_data created from the ``chat_data_json`` or an
    empty:obj:`dict`.

        Returns::obj:`dict`: The restored chat data.
        """
    ...
    async def get_bot_data(self) -> Dict[object, object]:
        ...
        Returns::obj:`dict`: The restored bot data.
        """
    ...
    async def get_callback_data(self) -> Optional[CDCData]:
        """Returns the callback_data created from the ``callback_data_json``
    or:obj:`None`.
    ...
    async def get_conversations(self, name: str) -> ConversationDict:
        """Returns the conversations created from the ``conversations_json``
    or an empty:obj:`dict`.

        Returns::obj:`dict`: The restored conversations data.
        """
    ...
    async def update_conversation(
        ...
        Args:
            ...
            new_state (:obj:`tuple` |:class:`object`): The new state for the
    given key.
        """
    ...
    async def update_user_data(self, user_id: int, data: Dict[Any, Any]) ->
    None:
    ...
    async def update_chat_data(self, chat_id: int, data: Dict[Any, Any]) ->
    None:
    ...
    async def update_callback_data(self, data: CDCData) -> None:
        ...
        Args:
            data (Tuple[List[Tuple[:obj:`str`,:obj:`float`, Dict[:obj:`str`,:class:`object`]]], \
                Dict[:obj:`str`,:obj:`str`]]): The relevant data to restore:class:`telegram.ext.CallbackDataCache`.
        """
    ...
    async def refresh_user_data(self, user_id: int, user_data: Dict[Any, Any])
    -> None:
        """Does nothing.
    ...
    async def refresh_chat_data(self, chat_id: int, chat_data: Dict[Any, Any])
    -> None:
        """Does nothing.
    ...
    @staticmethod
    def _encode_conversations_to_json(conversations: Dict[str,
    ConversationDict]) -> str:
        """Helper method to encode a conversations dict (that uses tuples as
    keys) to a
        JSON-serializable way. Use:meth:`self._decode_conversations_from_json` to decode.

        Args:
            conversations (:obj:`dict`): The conversations dict to transform
    to JSON.

        Returns::obj:`str`: The JSON-serialized conversations dict
        """
    ...
    @staticmethod
    def _decode_conversations_from_json(json_string: str) -> Dict[str,
    ConversationDict]:
        """Helper method to decode a conversations dict (that uses tuples as
    keys) from a
        ...
        Returns::obj:`dict`: The conversations dict after decoding
        """
    ...
    @staticmethod
    def _decode_user_chat_data_from_json(data: str) -> Dict[int, Dict[object,
    object]]:
        """Helper method to decode chat or user data (that uses ints as keys)
    from a
        JSON-string.
        ...
        Returns::obj:`dict`: The user/chat_data defaultdict after decoding
        """
