...
_UpdaterType = TypeVar("_UpdaterType", bound="Updater")  # pylint:
    disable=invalid-name
_LOGGER = get_logger(__name__)


class Updater(AsyncContextManager["Updater"]):
    """This class fetches updates for the bot either via long polling or by
    starting a webhook
    server. Received updates are enqueued into the:attr:`update_queue` and
    may be fetched from
    ...
    .. seealso:::wiki:`Architecture Overview <Architecture>`,:wiki:`Builder Pattern <Builder-Pattern>`
    ...
    .. versionchanged:: 20.0
        ...
        * The only arguments and attributes are now:attr:`bot` and:attr:`update_queue` as now
          the sole purpose of this class is to fetch updates. The entry point
    to a PTB application
    ...
    async def initialize(self) -> None:
        """Initializes the Updater & the associated:attr:`bot` by calling:meth:`telegram.Bot.initialize`.
    ...
    async def shutdown(self) -> None:
        ...
        Raises::exc:`RuntimeError`: If the updater is still running.
        """
        if self.running:
    ...
    polling updates from
            ...
            timeout (:obj:`float`, optional): Passed to:paramref:`telegram.Bot.get_updates.timeout`. Defaults to
    ``10`` seconds.
    ...
    pending updates on
            ...
            error_callback (Callable[[:exc:`telegram.error.TelegramError`],:obj:`None`], \
                ...
                while calling:meth:`telegram.Bot.get_updates` during polling.
    Defaults to:obj:`None`, in which case errors will be logged. Callback
    signature::
                ...
                Note:
                    ...
                    asynchronous behavior of the callback is wanted, please
    schedule a task from
                    within the callback.

        Returns::class:`asyncio.Queue`: The update queue that can be filled from
    the main thread.

        Raises::exc:`RuntimeError`: If the updater is already running or was not
    initialized.

        """
        if error_callback and asyncio.iscoroutinefunction(error_callback):
            raise TypeError(
                "The `error_callback` must not be a coroutine function! Use an
    ordinary function "
                "instead. "
            )
        ...
        async with self.__lock:
            ...
            if not self._initialized:
                raise RuntimeError("This Updater was not initialized via
    `Updater.initialize`!")
    ...
    async def _start_polling(
        ...
        async def polling_action_cb() -> bool:
            try:
                ...
                # TODO: in py3.8+, CancelledError is a subclass of
    BaseException, so we can drop
                ...
                _LOGGER.critical(
                    "Something went wrong processing the data received from
    Telegram. "
    ...
    ignored and pulled "
                    ...
                    self._last_update_id = updates[-1].update_id + 1  # Add
    one to 'confirm' it

            return True  # Keep fetching updates & don't quit. Polls with
    poll_interval.

        def default_error_callback(exc: TelegramError) -> None:
            _LOGGER.exception("Exception happened while polling for updates.",
    exc_info=exc)
    ...
    optional requirement
        ...
        .. versionchanged:: 13.4:meth:`start_webhook` now *always* calls:meth:`telegram.Bot.set_webhook`, so pass
            ``webhook_url`` instead of calling
    ...
    url. Useful behind
            ...
            ip_address (:obj:`str`, optional): Passed to:meth:`telegram.Bot.set_webhook`.
                ...
                .. versionadded:: 13.4
            allowed_updates (List[:obj:`str`], optional): Passed to:meth:`telegram.Bot.set_webhook`. Defaults to:obj:`None`.
    ...
    error if either the
                ...
                .. versionadded:: 20.0
        Returns::class:`queue.Queue`: The update queue that can be filled from the
    main thread.
    ...
    initialized.
        ...
        if not WEBHOOKS_AVAILABLE:
            raise RuntimeError(
                "To use `start_webhook`, PTB must be installed via `pip
    install "
    ...
    certificate
        # Note that we only use the SSL certificate for the WebhookServer, if
    the key is also
        # present. This is because the WebhookServer may not actually be in
    charge of performing
        ...
        await self._bootstrap(
            # Passing a Path or string only works if the bot is running
    against a local bot API
    ...
    @staticmethod
    def _gen_webhook_url(protocol: str, listen: str, port: int, url_path: str)
    -> str:
        # TODO: double check if this should be https in any case - the docs of
    start_webhook
        # say differently!
    ...
    async def _network_loop_retry(
        ...
        Stop condition for loop: `self.running` evaluates:obj:`False` or
    return value of
    ...
    function to call.
            on_err_cb (:obj:`callable`): Callback to call when TelegramError
    is caught. Receives
                the exception object as a parameter.
            description (:obj:`str`): Description text to use for logs and
    exception raised.
            interval (:obj:`float` |:obj:`int`): Interval to sleep between
    each call to
                `action_cb`.
        ...
        while self.running:
            try:
                try:
                    ...
                    _LOGGER.error("Error while %s: %s", description,
    telegram_exc)
                    on_err_cb(telegram_exc)

                    # increase waiting times on subsequent errors up to 30secs
                    cur_interval = 1 if cur_interval == 0 else min(30, 1.5 *
    cur_interval)
                else:
                    cur_interval = interval
                ...
                _LOGGER.debug("Network loop retry %s was cancelled",
    description)
                break
    ...
    async def _bootstrap(
        ...
        """Prepares the setup for fetching updates: delete or set the webhook
    and drop pending
        updates if appropriate. If there are unsuccessful attempts, this will
    retry as specified by:paramref:`max_retries`.
        """
        retries = 0
    ...
    self.bot.delete_webhook(drop_pending_updates=drop_pending_updates)
        ...
        def bootstrap_on_err_cb(exc: Exception) -> None:
            # We need this since retries is an immutable object otherwise and
    the changes
            # wouldn't propagate outside of thi function
            nonlocal retries

            if not isinstance(exc, InvalidToken) and (max_retries < 0 or
    retries < max_retries):
                retries += 1
                _LOGGER.warning(
                    "Failed bootstrap phase; try=%s max_retries=%s", retries,
    max_retries
                )
            else:
                _LOGGER.error("Failed bootstrap phase after %s retries (%s)",
    retries, exc)
                raise exc

        # Dropping pending updates from TG can be efficiently done with the
    drop_pending_updates
        # parameter of delete/start_webhook, even in the case of polling.
    Also, we want to make
        # sure that no webhook is configured in case of polling, so we just
    always call
        ...
        # Restore/set webhook settings, if needed. Again, we don't know ahead
    if a webhook is set,
        # so we set it anyhow.
        if webhook_url:
            await self._network_loop_retry(
    ...
    async def stop(self) -> None:
        ...
        .. seealso:::meth:`start_polling`,:meth:`start_webhook`

        Raises::exc:`RuntimeError`: If the updater is not running.
        """
        async with self.__lock:
            if not self.running:
    ...
    async def _stop_httpd(self) -> None:
        ...
        if self._httpd:
            _LOGGER.debug("Waiting for current webhook connection to be
    closed.")
    ...
    async def _stop_polling(self) -> None:
        ...
        if self.__polling_task:
            ...
            with contextlib.suppress(asyncio.CancelledError):
                ...
                # It only fails in rare edge-cases, e.g. when `stop()` is
    called directly
