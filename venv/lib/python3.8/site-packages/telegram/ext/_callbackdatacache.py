...
class InvalidCallbackData(TelegramError):
    """
    Raised when the received callback data has been tampered with or deleted
    from cache.

    Examples::any:`Arbitrary Callback Data Bot <examples.arbitrarycallbackdatabot>`
    ...
    Args:
        callback_data (:obj:`int`, optional): The button data of which the
    callback data could not
            be found.
    ...
    def __init__(self, callback_data: str = None) -> None:
        super().__init__(
            "The object belonging to this callback_data was deleted or the
    callback_data was "
            "manipulated."
        )
        self.callback_data: Optional[str] = callback_data

    def __reduce__(self) -> Tuple[type, Tuple[Optional[str]]]:  # type:
    ignore[override]
...
class _KeyboardData:
    ...
    def __init__(
        self, keyboard_uuid: str, access_time: float = None, button_data:
    ...
    def to_tuple(self) -> Tuple[str, float, Dict[str, object]]:
        """Gives a tuple representation consisting of the keyboard uuid, the
    access time and the
        button data.
        """
...
class CallbackDataCache:
    ...
    * One for mapping the data received in callback queries to the cached
    objects
    * One for mapping the IDs of received callback queries to the cached
    objects

    The second mapping allows to manually drop data that has been cached for
    keyboards of messages
    ...
    .. seealso:::wiki:`Architecture Overview <Architecture>`,:wiki:`Arbitrary callback_data <Arbitrary-callback_data>`
    ...
    Args:
        ...
        maxsize (:obj:`int`, optional): Maximum number of items in each of the
    internal mappings.
            Defaults to ``1024``.
    ...
    optional): \
        Data to initialize the cache with, as returned by \:meth:`telegram.ext.BasePersistence.get_callback_data`.
    ...
    def __init__(
        ...
        if not CACHE_TOOLS_AVAILABLE:
            raise RuntimeError(
                "To use `CallbackDataCache`, PTB must be installed via `pip
    install "
        ...
        self._keyboard_data: MutableMapping[str, _KeyboardData] =
    LRUCache(maxsize=maxsize)
        self._callback_queries: MutableMapping[str, str] =
    LRUCache(maxsize=maxsize)
    ...
    def load_persistence_data(self, persistent_data: CDCData) -> None:
        ...
        Args:
            ...
            Data to load, as returned by \:meth:`telegram.ext.BasePersistence.get_callback_data`.
        """
    ...
    def persistence_data(self) -> CDCData:
        ...
        Dict[:obj:`str`,:obj:`str`]]: The data that needs to be persisted to
    allow
        caching callback data across bot reboots.
        """
        # While building a list/dict from the LRUCaches has linear runtime (in
    the number of
        # entries), the runtime is bounded by maxsize and it has the big
    upside of not throwing a
        # highly customized data structure at users trying to implement a
    custom persistence class
        return [data.to_tuple() for data in self._keyboard_data.values()],
    dict(
    ...
    InlineKeyboardMarkup:
        """Registers the reply markup to the cache. If any of the buttons have:attr:`~telegram.InlineKeyboardButton.callback_data`, stores that data
    and builds a new
        keyboard with the correspondingly replaced buttons. Otherwise, does
    nothing and returns
        the original reply markup.

        Args:
            reply_markup (:class:`telegram.InlineKeyboardMarkup`): The
    keyboard.

        Returns::class:`telegram.InlineKeyboardMarkup`: The keyboard to be passed
    to Telegram.
    ...
    needed
        buttons = [
            [
                # We create a new button instead of replacing callback_data in
    case the
                # same object is used elsewhere
                InlineKeyboardButton(
                    btn.text,
                    callback_data=self.__put_button(btn.callback_data,
    keyboard_data),
                )
    ...
    @staticmethod
    def __put_button(callback_data: object, keyboard_data: _KeyboardData) ->
    str:
        """Stores the data for a single button in:attr:`keyboard_data`.
        Returns the string that should be passed instead of the callback_data,
    which is
    ...
    def __get_keyboard_uuid_and_button_data(
        ...
        try:
            # we get the values before calling update() in case KeyErrors are
    raised
    ...
    def extract_uuids(callback_data: str) -> Tuple[str, str]:
        ...
        Args:
            callback_data (:obj:`str`): The:paramref:`~telegram.InlineKeyboardButton.callback_data` as
    ...
    def process_message(self, message: Message) -> None:
        """Replaces the data in the inline keyboard attached to the message
    with the cached
        objects, if necessary. If the data could not be found,:class:`telegram.ext.InvalidCallbackData` will be inserted.

        Note:
            Checks:attr:`telegram.Message.via_bot` and:attr:`telegram.Message.from_user` to check
            if the reply markup (if any) was actually sent by this cache's
    bot. If it was not, the
            message will be returned unchanged.

            Note that this will fail for channel posts, as:attr:`telegram.Message.from_user` is:obj:`None` for those! In the corresponding reply markups the
    callback data will be
            replaced by:class:`telegram.ext.InvalidCallbackData`.

        Warning:
            * Does *not* consider:attr:`telegram.Message.reply_to_message` and:attr:`telegram.Message.pinned_message`. Pass them to this
    method separately.
            * *In place*, i.e. the passed:class:`telegram.Message` will be
    changed!
    ...
    def __process_message(self, message: Message) -> Optional[str]:
        """As documented in process_message, but returns the uuid of the
    ...
    self.__get_keyboard_uuid_and_button_data(
                        button_data
                    )
    ...
    updated
                    ...
                    if not keyboard_uuid and not isinstance(callback_data,
    InvalidCallbackData):
    ...
    def process_callback_query(self, callback_query: CallbackQuery) -> None:
        """Replaces the data in the callback query and the attached messages
    keyboard with the
        cached objects, if necessary. If the data could not be found,:class:`telegram.ext.InvalidCallbackData` will be inserted.
        If:attr:`telegram.CallbackQuery.data` or:attr:`telegram.CallbackQuery.message` is
        present, this also saves the callback queries ID in order to be able
    to resolve it to the
        stored data.

        Note:
            Also considers inserts data into the buttons of:attr:`telegram.Message.reply_to_message` and:attr:`telegram.Message.pinned_message`
            if necessary.

        Warning:
            *In place*, i.e. the passed:class:`telegram.CallbackQuery` will
    be changed!
    ...
    query.
        ...
        if callback_query.data:
            ...
            keyboard_uuid, button_data =
    self.__get_keyboard_uuid_and_button_data(data)
    ...
    def drop_data(self, callback_query: CallbackQuery) -> None:
        ...
        Note:
            Will *not* raise exceptions in case the callback data is not found
    in the cache.
    ...
    def clear_callback_data(self, time_cutoff: Union[float, datetime] = None)
    -> None:
        """Clears the stored callback data.

        Args:
            time_cutoff (:obj:`float` |:obj:`datetime.datetime`, optional):
    Pass a UNIX timestamp
                or a:obj:`datetime.datetime` to clear only entries which are
    older.
    ...
    def __clear(self, mapping: MutableMapping, time_cutoff: Union[float,
    datetime] = None) -> None:
        if not time_cutoff:
            mapping.clear()
            return
        ...
        if isinstance(time_cutoff, datetime):
            effective_cutoff = to_float_timestamp(
                time_cutoff, tzinfo=self.bot.defaults.tzinfo if
    self.bot.defaults else None
            )
        else:
            effective_cutoff = time_cutoff

        # We need a list instead of a generator here, as the list doesn't
    change it's size
        # during the iteration
        to_drop = [key for key, data in mapping.items() if data.access_time <
    effective_cutoff]
        for key in to_drop:
            mapping.pop(key)
