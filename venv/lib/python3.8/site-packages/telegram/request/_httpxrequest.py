...
"""This module contains methods to make POST and GET requests using the httpx
    library."""
...
# Note to future devs:
# Proxies are currently only tested manually. The httpx development docs have
    a nice guide on that:
# https://www.python-httpx.org/contributing/#development-proxy-setup (also
...
class HTTPXRequest(BaseRequest):
    """Implementation of:class:`~telegram.request.BaseRequest` using the
    library
    ...
    Args:
        connection_pool_size (:obj:`int`, optional): Number of connections to
    keep in the
            connection pool. Defaults to ``1``.

            Note:
                Independent of the value, one additional connection will be
    reserved for:meth:`telegram.Bot.get_updates`.
        proxy_url (:obj:`str`, optional): The URL to the proxy server. For
    example
            ``'http://127.0.0.1:3128'`` or ``'socks5://127.0.0.1:3128'``.
    ...
    ``HTTPS_PROXY`` or
                ...
                * For Socks5 support, additional dependencies are required.
    Make sure to install
                  PTB via:command:`pip install python-telegram-bot[socks]` in
    this case.
    ...
    specifies the maximum
            amount of time (in seconds) to wait for a response from Telegram's
    server.
    ...
    attempt to a server
            to succeed. This value is used unless a different value is passed
    to:meth:`do_request`. Defaults to ``5``.
    ...
    available.
            ...
            Warning:
                ...
                exceptions to be thrown when more requests are made
    simultaneously than there are
                connections in the connection pool!
        http_version (:obj:`str`, optional): If ``"2"``, HTTP/2 will be used
    instead of HTTP/1.1.
            Defaults to ``"1.1"``.
    ...
    def __init__(
        ...
        # See
    https://github.com/python-telegram-bot/python-telegram-bot/pull/3542
    ...
    noqa: C408
        ...
        try:
            ...
            if "httpx[http2]" not in str(exc) and "httpx[socks]" not in
    str(exc):
                raise exc

            if "httpx[socks]" in str(exc):
                raise RuntimeError(
                    "To use Socks5 proxies, PTB must be installed via `pip
    install "
    ...
    def _build_client(self) -> httpx.AsyncClient:
        return httpx.AsyncClient(**self._client_kwargs)  # type:
    ignore[arg-type]
    ...
    async def do_request(
        ...
        # If user did not specify timeouts (for e.g. in a bot method), use the
    default ones when we
        ...
        # TODO p0: On Linux, use setsockopt to properly set socket level
    keepalive.
        ...
        try:
            ...
            if isinstance(err, httpx.PoolTimeout):
                raise TimedOut(
                    message=(
                        "Pool timeout: All connections in the connection pool
    are occupied. "
                        "Request was *not* sent to Telegram. Consider
    adjusting the connection "
            ...
            # We include the class name for easier debugging. Especially
    useful if the error
            # message of `err` is empty.
            raise NetworkError(f"httpx.{err.__class__.__name__}: {err}") from
    err
