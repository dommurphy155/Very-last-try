...
class InlineQueryResultVideo(InlineQueryResult):
    ...
    Alternatively, you can use:attr:`input_message_content` to send a message with the specified content
    instead of
    the video.
    ...
    Args:
        id (:obj:`str`): Unique identifier for this result,:tg-const:`telegram.InlineQueryResult.MIN_ID_LENGTH`-:tg-const:`telegram.InlineQueryResult.MAX_ID_LENGTH` Bytes.
        video_url (:obj:`str`): A valid URL for the embedded video player or
    video file.
        mime_type (:obj:`str`): Mime type of the content of video url,
    "text/html" or "video/mp4".
        thumbnail_url (:obj:`str`, optional): URL of the thumbnail (JPEG only)
    for the video.

            Warning:
                The Bot API does **not** define this as an optional argument.
    It is formally
                optional for backwards compatibility with the deprecated:paramref:`thumb_url`.
                If you pass neither:paramref:`thumbnail_url` nor:paramref:`thumb_url`,:class:`ValueError` will be raised.
    ...
    It is formally
                ...
                become optional.:class:`TypeError` will be raised if no
    ``title`` is passed.
    ...
    optional): Content of the
            ...
            (e.g., a YouTube video).
        thumb_url (:obj:`str`, optional): URL of the thumbnail (JPEG only) for
    the video.

            .. deprecated:: 20.2
               |thumbargumentdeprecation|:paramref:`thumbnail_url`.

    Raises::class:`ValueError`: If neither:paramref:`thumbnail_url` nor:paramref:`thumb_url` is
            supplied or if both are supplied and are not equal.:class:`TypeError`: If no:paramref:`title` is passed.
    ...
    video file.
        mime_type (:obj:`str`): Mime type of the content of video url,
    "text/html" or "video/mp4".
        thumbnail_url (:obj:`str`): URL of the thumbnail (JPEG only) for the
    video.
    ...
    def __init__(
        ...
        video_url: str,
        mime_type: str,
        # thumbnail_url and title are not optional in Telegram API, but we
    want to support
        # thumb_url as well, so thumbnail_url may not be passed if thumb_url
    is passed.
        # We will raise ValueError manually if neither thumbnail_url nor
    thumb_url are passed.
        thumbnail_url: str = None,
        # title had to be made optional because of thumbnail_url. This is
    compensated by raising
        ...
        # thumb_url is not optional in Telegram API, but it is here, along
    with thumbnail_url.
        thumb_url: str = None,
        *,
        ...
        if not (thumbnail_url or thumb_url):
            raise ValueError(
                "You must pass either 'thumbnail_url' or 'thumb_url'. Note
    that 'thumb_url' is "
                "deprecated."
            )
        ...
        if title is None:
            raise TypeError(
                "InlineQueryResultVideo.__init__() missing a required
    argument: you forgot to pass"
