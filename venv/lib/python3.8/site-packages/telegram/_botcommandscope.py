...
class BotCommandScope(TelegramObject):
    """Base class for objects that represent the scope to which bot commands
    are applied.
    ...
    *:class:`telegram.BotCommandScopeChatMember`
    ...
    this class are
    considered equal, if their:attr:`type` is equal. For subclasses with
    additional attributes,
    the notion of equality is overridden.

    Note:
        Please see the `official docs`_ on how Telegram determines which
    commands to display.
    ...
    ALL_PRIVATE_CHATS: ClassVar[str] =
    constants.BotCommandScopeType.ALL_PRIVATE_CHATS
    """:const:`telegram.constants.BotCommandScopeType.ALL_PRIVATE_CHATS`"""
    ALL_GROUP_CHATS: ClassVar[str] =
    constants.BotCommandScopeType.ALL_GROUP_CHATS
    """:const:`telegram.constants.BotCommandScopeType.ALL_GROUP_CHATS`"""
    ALL_CHAT_ADMINISTRATORS: ClassVar[str] =
    ...
    CHAT_ADMINISTRATORS: ClassVar[str] =
    constants.BotCommandScopeType.CHAT_ADMINISTRATORS
...
class BotCommandScopeChat(BotCommandScope):
    ...
    def __init__(self, chat_id: Union[str, int], *, api_kwargs: JSONDict =
    None):
        super().__init__(type=BotCommandScope.CHAT, api_kwargs=api_kwargs)
        with self._unfrozen():
            self.chat_id: Union[str, int] = (
                chat_id if isinstance(chat_id, str) and
    chat_id.startswith("@") else int(chat_id)
            )
...
class BotCommandScopeChatMember(BotCommandScope):
    ...
    def __init__(self, chat_id: Union[str, int], user_id: int, *, api_kwargs:
    JSONDict = None):
