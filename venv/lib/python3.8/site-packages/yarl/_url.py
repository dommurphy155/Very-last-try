...
# are not allowed to have an empty host
...
# this pattern matches anything that is *not* in those classes. and is only
    used
...
Query = Union[
    None, str, "Mapping[str, QueryVariable]", "Sequence[Tuple[str,
    QueryVariable]]"
]
_T = TypeVar("_T")
...
class URL:
        ...
        *,
        ...
        if not encoded:
            ...
            if not netloc:  # netloc
                ...
                raw_user = None if username is None else
    cls._REQUOTER(username)
                raw_password = None if password is None else
    cls._REQUOTER(password)
                netloc = cls._make_netloc(raw_user, raw_password, host, port)
                # Remove brackets as host encoder adds back brackets for IPv6
    addresses
            ...
            fragment = cls._FRAGMENT_REQUOTER(fragment) if fragment else
    fragment
            cache["scheme"] = scheme
    ...
    def build(
        ...
        *,
        ...
        if authority and (user or password or host or port):
            raise ValueError(
                'Can\'t mix "authority" with "user", "password", "host" or
    "port".'
            )
        if port is not None and not isinstance(port, int):
            raise TypeError("The port is required to be int.")
        if port and not host:
            raise ValueError('Can\'t build URL with "port" but without
    "host".')
        if query and query_string:
            raise ValueError('Only one of "query" or "query_string" should be
    passed')
        if (
            scheme is None
    ...
    instead.'
        ...
        if encoded:
            ...
            if authority:
                ...
                _host = cls._encode_host(_host, validate_host=False) if _host
    else ""
            elif host:
            ...
            query_string = (
                cls._QUERY_QUOTER(query_string) if query_string else
    query_string
            )
            fragment = cls._FRAGMENT_QUOTER(fragment) if fragment else fragment

        url = cls._from_val(SplitResult(scheme, netloc, path, query_string,
    fragment))
        if query:
    ...
    def __str__(self) -> str:
        ...
        if not val.path and val.netloc and (val.query or val.fragment):
            path = "/"
        if (port:= self.explicit_port) is not None and port ==
    self._default_port:
            # port normalization - using None for default ports to remove from
    rendering
            # https://datatracker.ietf.org/doc/html/rfc3986.html#section-6.2.3
            host = self.host_subcomponent
            netloc = self._make_netloc(self.raw_user, self.raw_password, host,
    None)
    ...
    def authority(self) -> str:
        ...
        """
        return self._make_netloc(self.user, self.password, self.host,
    self.port)
    ...
    def raw_host(self) -> Union[str, None]:
        ...
        When working with IPv6 addresses, use the `host_subcomponent` property
    instead
    ...
    fragment=""))
            return self
        parts = path.split("/")
        val = self._val._replace(path="/".join(parts[:-1]), query="",
    fragment="")
        return self._from_val(val)
    ...
    def _make_child(self, paths: "Sequence[str]", encoded: bool = False) ->
    "URL":
        """
        ...
        if self._val.netloc:
            parsed = _normalize_path_segments(parsed) if needs_normalize else
    parsed
        ...
        return self._from_val(self._val._replace(path=new_path, query="",
    fragment=""))
    ...
    def _encode_host(
        ...
        looks_like_ip, lower_host, is_ascii, raw_ip, sep, zone =
    cls._parse_host(host)
        if looks_like_ip:
            ...
            #                             have been removed before it gets
    here)
    ...
    def with_user(self, user: Union[str, None]) -> "URL":
        ...
        if not val.netloc:
            raise ValueError("user replacement is not allowed for relative
    URLs")
        encoded_host = self.host_subcomponent or ""
        netloc = self._make_netloc(user, password, encoded_host,
    self.explicit_port)
    ...
    def with_password(self, password: Union[str, None]) -> "URL":
        ...
        if not self._val.netloc:
            raise ValueError("password replacement is not allowed for relative
    URLs")
        encoded_host = self.host_subcomponent or ""
    ...
    def with_host(self, host: str) -> "URL":
        ...
        val = self._val
        if not val.netloc:
            raise ValueError("host replacement is not allowed for relative
    URLs")
        if not host:
            raise ValueError("host removing is not allowed")
        encoded_host = self._encode_host(host) if host else ""
        port = self.explicit_port
        netloc = self._make_netloc(self.raw_user, self.raw_password,
    encoded_host, port)
        return self._from_val(val._replace(netloc=netloc))
    ...
    def with_path(self, path: str, *, encoded: bool = False) -> "URL":
        ...
        if len(path) > 0 and path[0] != "/":
            path = "/" + path
        return self._from_val(self._val._replace(path=path, query="",
    fragment=""))
    ...
    def _get_str_query_from_sequence_iterable(
        ...
        pairs = [
            f"{quoter(k)}={quoter(v if type(v) is str else
    self._query_var(v))}"
            for k, val in items
            for v in (
                val
    ...
    def with_suffix(self, suffix: str) -> "URL":
        ...
        name = name + suffix if not old_suffix else name[: -len(old_suffix)] +
    suffix
    ...
    def join(self, url: "URL") -> "URL":
        ...
        # scheme is in uses_authority as uses_authority is a superset of
    uses_relative
        ...
        if other_val.path[0] == "/":
            ...
            # …
            # and relativizing ".."
            # parts[0] is / for absolute urls, this join will add a double
    slash there
    ...
    def human_repr(self) -> str:
        ...
        return self._unsplit_result(scheme, netloc, path, query_string,
    fragment)
