...
class _Catcher:
    def __init__(self, handler_map: Mapping[tuple[type[BaseException], ...],
    _Handler]):
            ...
            if unhandled is exc:
                ...
                if isinstance(exc, BaseExceptionGroup):
                    try:
                        ...
                        # Change __context__ to __cause__ because Python 3.11
    does this
                        # too
            ...
            if matched:
                try:
                    ...
                    if inspect.iscoroutine(result):
                        raise TypeError(
                            f"Error trying to handle {matched!r} with
    {handler!r}. "
        ...
        if new_exceptions:
            ...
            excgroup and len(excgroup.exceptions) == 1 and
    excgroup.exceptions[0] is exc
        ):
            return exc
        else:
            return excgroup
...
def catch(
    __handlers: Mapping[type[BaseException] | Iterable[type[BaseException]],
    _Handler],
    ...
    thereof"
        ...
        for exc_type in iterable:
            if not isinstance(exc_type, type) or not issubclass(
                ...
                raise TypeError(
                    "each key must be either an exception classes or an
    iterable "
                    "thereof"
                )
