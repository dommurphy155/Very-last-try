...
def asarray(
    ...
    *,
    ...
    if isinstance(obj, Array):
        ...
        return obj
    if dtype is None and isinstance(obj, int) and (obj > 2 ** 64 or obj < -(2
    ** 63)):
...
def linspace(
    ...
    *,
    ...
    if device not in ["cpu", None]:
        raise ValueError(f"Unsupported device {device!r}")
    return Array._new(np.linspace(start, stop, num, dtype=dtype,
    endpoint=endpoint))
...
def meshgrid(*arrays: Array, indexing: str = "xy") -> List[Array]:
    ...
    return [
        Array._new(array)
        for array in np.meshgrid(*[a._array for a in arrays],
    indexing=indexing)
    ]
