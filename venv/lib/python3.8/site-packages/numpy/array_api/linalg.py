...
# literals.
def matrix_norm(x: Array, /, *, keepdims: bool = False, ord:
    ...
    # np.linalg.norm.
    if x.dtype not in _floating_dtypes:
        raise TypeError('Only floating-point dtypes are allowed in
    matrix_norm')

    return Array._new(np.linalg.norm(x._array, axis=(-2, -1),
...
def matrix_power(x: Array, n: int, /) -> Array:
    ...
    if x.dtype not in _floating_dtypes:
        raise TypeError('Only floating-point dtypes are allowed for the first
...
def matrix_rank(x: Array, /, *, rtol: Optional[Union[float, Array]] = None) ->
    Array:
    """
    ...
    # dimensional arrays.
    if x.ndim < 2:
        raise np.linalg.LinAlgError("1-dimensional array given. Array must be
    ...
    np.finfo(S.dtype).eps
        ...
        if isinstance(rtol, Array):
            rtol = rtol._array
        # Note: this is different from np.linalg.matrix_rank, which does not
    multiply
...
def svd(x: Array, /, *, full_matrices: bool = True) -> SVDResult:
    ...
    return SVDResult(*map(Array._new, np.linalg.svd(x._array,
...
# Note: axes must be a tuple, unlike np.tensordot where it can be an array or
    array-like.
def tensordot(x1: Array, x2: Array, /, *, axes: Union[int,
    Tuple[Sequence[int], Sequence[int]]] = 2) -> Array:
    # Note: the restriction to numeric dtypes only is different from
    # np.tensordot.
    if x1.dtype not in _numeric_dtypes or x2.dtype not in _numeric_dtypes:
...
def trace(x: Array, /, *, offset: int = 0) -> Array:
    ...
    # operates on the first two axes by default
    return Array._new(np.asarray(np.trace(x._array, offset=offset, axis1=-2,
    axis2=-1)))
...
def vecdot(x1: Array, x2: Array, /, *, axis: int = -1) -> Array:
    ...
    if x1_shape[axis] != x2_shape[axis]:
        raise ValueError("x1 and x2 must have the same size along the given
    axis")
...
def vector_norm(x: Array, /, *, axis: Optional[Union[int, Tuple[int, ...]]] =
    ...
    if axis is None:
        ...
        a = np.transpose(a, newshape).reshape(
            (np.prod([a.shape[i] for i in axis], dtype=int), *[a.shape[i] for
    i in rest]))
        _axis = 0
    else:
        _axis = axis
    ...
    if keepdims:
        ...
        shape = list(x.shape)
        _axis = normalize_axis_tuple(range(x.ndim) if axis is None else axis,
    x.ndim)
        for i in _axis:
            shape[i] = 1
...
__all__ = ['cholesky', 'cross', 'det', 'diagonal', 'eigh', 'eigvalsh', 'inv',
    'matmul', 'matrix_norm', 'matrix_power', 'matrix_rank', 'matrix_transpose', 'outer', 'pinv', 'qr', 'slogdet', 'solve', 'svd', 'svdvals', 'tensordot', 'trace', 'vecdot', 'vector_norm']
