...
class Array:
    ...
    def _new(cls, x, /):
        ...
        if x.dtype not in _all_dtypes:
            raise TypeError(
                f"The array_api namespace does not support the dtype
    '{x.dtype}'"
            )
        obj._array = x
        return obj
    ...
    def __repr__(self: Array, /) -> str:
        ...
        if 0 in self.shape:
            ...
            prefix = "Array("
            mid = np.array2string(self._array, separator=', ', prefix=prefix,
    suffix=suffix)
    ...
    def _check_allowed_dtypes(self, other: bool | int | float | Array,
    dtype_category: str, op: str) -> Array:
        """
        ...
        if self.dtype not in _dtype_categories[dtype_category]:
            raise TypeError(f"Only {dtype_category} dtypes are allowed in
    {op}")
    ...
    {op}")
        ...
        if op.startswith("__i"):
            ...
            if res_dtype != self.dtype:
                raise TypeError(
                    f"Cannot perform {op} with dtypes {self.dtype} and
    {other.dtype}"
                )
    ...
    def _promote_scalar(self, scalar):
        ...
        if isinstance(scalar, bool):
            ...
            if self.dtype not in _floating_dtypes:
                raise TypeError(
                    "Python float scalars can only be promoted with
    ...
    def _normalize_two_args(x1, x2) -> Tuple[Array, Array]:
        ...
        To fix this, we add a dimension to the 0-dimension array before
    passing it
        through. This works because a dimension would be added anyway from
        broadcasting, so the resulting shape is the same, but this prevents
    NumPy
    ...
    def _validate_index(self, key):
        ...
        The following cases are allowed by NumPy, but not specified by the
    array
        API specification:
    ...
    def __float__(self: Array, /) -> float:
        ...
        # Note: This is an error here.
        if self._array.ndim != 0:
            raise TypeError("float is only allowed on arrays with 0
    dimensions")
        if self.dtype not in _floating_dtypes:
    ...
    def __getitem__(
        self: Array,
        key: Union[
            int, slice, ellipsis, Tuple[Union[int, slice, ellipsis], ...],
    Array
        ],
        /,
    ) -> Array:
    ...
    def __truediv__(self: Array, other: Union[float, Array], /) -> Array:
        ...
        """
        other = self._check_allowed_dtypes(other, "floating-point",
    "__truediv__")
        if other is NotImplemented:
            return other
        self, other = self._normalize_two_args(self, other)
    ...
    def __ifloordiv__(self: Array, other: Union[int, float, Array], /) ->
    Array:
        """
    ...
    def __rfloordiv__(self: Array, other: Union[int, float, Array], /) ->
    Array:
        """
    ...
    def to_device(self: Array, device: Device, /, stream: None = None) ->
    Array:
        if stream is not None:
            raise ValueError("The stream argument to to_device() is not
    supported")
        if device == 'cpu':
            return self
    ...
    https://data-apis.org/array-api/latest/API_specification/array_object.html#t
        if self.ndim != 2:
            raise ValueError("x.T requires x to have 2 dimensions. Use x.mT to
