...
represented by a complex exponential:math:`a_m = \\exp\\{2\\pi i\\,f m\\Delta t\\}`, where:math:`\\Delta t`
...
.. math::
   a_m = \\frac{1}{n}\\sum_{k=0}^{n-1}A_k\\exp\\left\\{2\\pi i{mk\\over
    n}\\right\\}
   \\qquad m = 0,\\ldots,n-1.
...
to ``"ortho"`` so that both direct and inverse transforms are scaled by:math:`1/\\sqrt{n}`. Finally, setting the keyword argument ``norm`` to
...
.. math::
   ...
   a_{mn}\\exp\\left\\{-2\\pi i \\left({mk\\over M}+{nl\\over
    N}\\right)\\right\\}
