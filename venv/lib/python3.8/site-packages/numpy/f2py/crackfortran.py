...
Note: 'implicit integer (z) static (z)' is 'implicit static (z)' (this is
    minor bug).
Note: code after 'contains' will be ignored until its scope ends.
Note: 'common' statement is extended: dimensions are moved to variable
    definitions
...
Usage:
  ...
  `postlist` contains declaration information read from the list of files
    `files`.
 ...
 *** it is a list of dictionaries containing `blocks':
    ...
    {'block','body','vars','parent_block'[,'name','prefix','args','result',
     ...
     B['block'] = 'interface' | 'function' | 'subroutine' | 'module' |
                  ...
                  'abstract interface'
     B['body'] --- list containing `subblocks' with the same structure as
    `blocks'
    ...
    (independent first)
     ...
     B['args'] --- list of argument names if B['block']== 'function' |
    'subroutine'
     ...
     B['commonvars'] --- list of variables used in common blocks (dimensions
    ...
    {<modulename>:{['only':<0|1>],['map':{<local_name1>:<use_name1>,...}]}}
     ...
     B['saved_interface'] --- a string of scanned routine signature, defines
    explicit interface
 *** Variable definition is a dictionary
     D = B['vars'][<variable name>] =
     {'typespec'[,'attrspec','kindselector','charselector','=','typename']}
     D['typespec'] = 'byte' | 'character' | 'complex' | 'double complex' |
                     'double precision' | 'integer' | 'logical' | 'real' |
    'type'
    ...
    on
     D['check'] --- list of C-expressions; if C-expr returns zero, exception
    is raised
...
TODO:
    * Apply 'parameter' attribute (e.g. 'integer parameter:: i=2' 'real x(i)'
                                   -> 'real x(2)')
    The above may be solved by creating appropriate preprocessor program, for
    example.
...
badnames = {}
invbadnames = {}
for n in ['int', 'double', 'float', 'char', 'short', 'long', 'void', 'case',
    'while',
          'return', 'signed', 'unsigned', 'if', 'for', 'typedef', 'sizeof',
    'union',
          'struct', 'static', 'register', 'new', 'break', 'do', 'goto',
    'switch',
          'continue', 'else', 'inline', 'extern', 'delete', 'const', 'auto',
          'len', 'rank', 'shape', 'index', 'slen', 'size', '_i',
          'max', 'min',
          'flen', 'fshape',
          'string', 'complex_double', 'float_double', 'stdin', 'stderr',
    'stdout',
          'type', 'default']:
    badnames[n] = n + '_bn'
    invbadnames[n + '_bn'] = n
    ...
    with openhook(file, 'r') as f:
        ...
        while n > 0 and line:
            if line[0] != '!' and line.strip():
                n -= 1
                if (line[0] != '\t' and _free_f90_start(line[:5])) or
    line[-2:-1] == '&':
                    result = 1
                    break
            line = f.readline()
...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    """
    global gotnextfile, filepositiontext, currentfilename, sourcecodeform,
    strictf77
    global beginpattern, quiet, verbose, dolowercase, include_paths

    if not istop:
        saveglobals = gotnextfile, filepositiontext, currentfilename,
            ...
            if os.path.isfile(fn):
                ...
                for inc_dir in include_dirs:
                    ...
                    if os.path.isfile(fn1):
                        ...
                        break
                if not foundfile:
                    outmess('readfortrancode: could not find include file %s
    in %s. Ignoring.\n' % (
        ...
        if os.path.isfile(fn):
            ...
            for inc_dir in include_dirs:
                ...
                if os.path.isfile(fn1):
                    ...
                    break
            if not foundfile:
                outmess('readfortrancode: could not find include file %s in
    %s. Ignoring.\n' % (
                    repr(fn), os.pathsep.join(include_dirs)))
    else:
        dowithline(finalline)
    ...
    if istop:
        ...
        gotnextfile, filepositiontext, currentfilename, sourcecodeform,
    strictf77,\
...
intentpattern = re.compile(beforethisafter % (
    '', 'intent|depend|note|check', 'intent|depend|note|check',
    r'\s*\(.*?\).*'), re.I), 'intent'
...
def split_by_unquoted(line, characters):
    ...
    """
    assert not (set('"\'') & set(characters)), "cannot split by unquoted
    quotes"
    r = re.compile(
...
def crackline(line, reset=0):
    ...
    if not m:
        ...
        if 'externals' in groupcache[groupcounter]:
            for name in groupcache[groupcounter]['externals']:
                if name in invbadnames:
                    name = invbadnames[name]
                if 'interfaced' in groupcache[groupcounter] and name in
    ...
    name, markouterparen(line), re.I)
                if m1:
                    ...
                    if not m:
                        outmess(
                            'crackline: could not resolve function call for
    line=%s.\n' % repr(line))
                        return
                    analyzeline(m, 'callfun', line)
                    return
        if verbose > 1 or (verbose == 1 and
...
def _resolvenameargspattern(line):
    ...
    if m1:
        return m1.group('name'), m1.group('args'), m1.group('result'),
    m1.group('bind')
...
def analyzeline(m, case, line):
    ...
    if expectbegin and case not in ['begin', 'call', 'callfun', 'type'] \
       and not skipemptyends and groupcounter < 1:
        ...
        outmess(
            'analyzeline: no group yet. Creating program group with name
    "%s".\n' % newname)
        gotnextfile = 0
        groupcounter = groupcounter + 1
    ...
    (groupcounter=%s).\n' % (
            ...
            groupcache[groupcounter]['from'] = '%s:%s' % (
                groupcache[groupcounter - 1]['from'], groupcache[groupcounter
    - 1]['name'])
            groupcache[groupcounter]['body'] = []
            groupcache[groupcounter]['externals'] = []
            groupcache[groupcounter]['interfaced'] = []
    ...
    elif case in ['dimension', 'intent', 'optional', 'required', 'external',
            ...
            if case == 'intent':
                ...
                if _intentcallbackpattern.match(ap):
                    if k not in groupcache[groupcounter]['args']:
                        if groupcounter > 1:
                            if '__user__' not in groupcache[groupcounter -
    2]['name']:
                                outmess(
                                    'analyzeline: missing __user__ module
    (could be nothing)\n')
                            # fixes ticket 1693
                            if k != groupcache[groupcounter]['name']:
                                outmess('analyzeline: appending
    intent(callback) %s'
                                        ' to %s arguments\n' % (k,
    groupcache[groupcounter]['name']))
    ...
    ignored\n' % (k))
                        ...
                        errmess('analyzeline: intent(callback) %s is already'
                                ' in argument list\n' % (k))
            if case in ['optional', 'required', 'public', 'external',
    'private', 'intrinsic']:
                ap = case
                ...
                if t.get('typespec') == 'real':
                    ...
                    for m in real16pattern.finditer(initexpr):
                        tt[m.start():m.end()] = list(
                            initexpr[m.start():m.end()].lower().replace('d',
    'e'))
                    initexpr = ''.join(tt)
    ...
    statement.\n')
            ...
            for e in markoutercomma(m.group('after')).split('@,@'):
                decl = {}
                m1 = re.match(
                    r'\s*(?P<this>.*?)\s*(\(\s*(?P<after>[a-z-,
    ]+)\s*\)\s*|)\Z', e, re.I)
                if not m1:
                    outmess(
                        'analyzeline: could not extract info of implicit
    statement part "%s"\n' % (e))
                    continue
                m2 = typespattern4implicit.match(m1.group('this'))
                if not m2:
                    outmess(
                        'analyzeline: could not extract types pattern of
                ...
                for r in markoutercomma(m1.group('after')).split('@,@'):
                    if '-' in r:
                        try:
                            ...
                            outmess(
                                'analyzeline: expected "<char>-<char>" instead
    of "%s" in range list of implicit statement\n' % r)
                            continue
                    else:
                        begc = endc = r.strip()
                    if not len(begc) == len(endc) == 1:
                        outmess(
                            'analyzeline: expected "<char>-<char>" instead of
    ...
    Skipping.\n' % l[0])
            ...
            j = 0
            llen = len(l[1])
            for v in rmbadname([x.strip() for x in
    ...
    Skipping.\n' % v)
                ...
                if '=' in vars[v] and not vars[v]['='] == l[1][j:i - 1]:
                    outmess('analyzeline: changing init expression of "%s"
    ("%s") to "%s"\n' % (
                        v, vars[v]['='], l[1][j:i - 1]))
                vars[v]['='] = l[1][j:i - 1]
                j = i
...
def cracktypespec0(typespec, ll):
    ...
    if typespec in ['complex', 'integer', 'logical', 'real', 'character',
    'type']:
        selector = d['this']
        ll = d['after']
    i = ll.find('::')
...
kindselector = re.compile(
    r'\s*(\(\s*(kind\s*=)?\s*(?P<kind>.*)\s*\)|\*\s*(?P<kind2>.*?))\s*\Z',
    re.I)
...
def updatevars(typespec, selector, attrspec, entitydecl):
    ...
    to "%s". Ignoring.\n' % (
            ...
            if 'kindselector' not in edecl:
                ...
                for k in list(kindselect.keys()):
                    if k in edecl['kindselector'] and (not kindselect[k] ==
    edecl['kindselector'][k]):
                        outmess('updatevars: attempt to change the
    kindselector "%s" of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if 'charselector' not in edecl and charselect:
                if not_has_typespec:
                    ...
                    errmess('updatevars:%s: attempt to change empty
    ...
    charselector "%s" of "%s" ("%s") to "%s". Ignoring.\n' % (
            ...
            if 'typename' not in edecl:
                ...
                outmess('updatevars: attempt to change the typename of "%s"
    ("%s") to "%s". Ignoring.\n' % (
            ...
            if m1:
                ...
                if 'len' in d1:
                    if typespec in ['complex', 'integer', 'logical', 'real']:
                        if ('kindselector' not in edecl) or (not
    edecl['kindselector']):
                            edecl['kindselector'] = {}
    ...
    edecl['charselector']):
                ...
                if 'init' in d1:
                    if '=' in edecl and (not edecl['='] == d1['init']):
                        outmess('updatevars: attempt to change the init
    expression of "%s" ("%s") to "%s". Ignoring.\n' % (
                ...
                if 'len' in d1 and 'array' in d1:
                    if d1['len'] == '':
                        ...
                        del d1['len']
                        errmess('updatevars: "%s %s" is mapped to "%s
    %s(%s)"\n' % (
                    ...
                    if 'attrspec' not in edecl or (not edecl['attrspec']):
                        ...
                        for dm1 in edecl['attrspec']:
                            if dm1[:9] == 'dimension' and dm1 != dm:
                                ...
                                errmess('updatevars:%s: attempt to change %r
    to %r. Ignoring.\n'
                                        % (ename, dm1, dm))
                                break

            else:
                outmess('updatevars: could not crack entity declaration "%s".
    Ignoring.\n' % (
                    ename + m.group('after')))
    ...
    if 'externals' in block and block['externals']:
        ...
        if interface['vars'] or interface['body']:
            ...
            mblock = {'block': 'python module', 'body': [
                interface], 'vars': {}, 'name': mname, 'interfaced':
    block['externals']}
            useblock[mname] = {}
...
def analyzecommon(block):
    ...
    markoutercomma(m.group('dims')).split('@,@')]
                ...
                n = e
                errmess(
                    'analyzecommon: failed to extract "<name>[(<dims>)]" from
    "%s" in common /%s/.\n' % (e, k))
            comvars.append(n)
...
def buildimplicitrules(block):
    ...
    % repr(block['name']))
            ...
            for k in list(block['implicit'].keys()):
                if block['implicit'][k].get('typespec') not in ['static',
    'automatic']:
...
def _selected_real_kind_func(p, r=0, radix=0):
    ...
    if machine.startswith(('aarch64', 'power', 'ppc', 'riscv', 's390x',
    'sparc')):
        if p <= 20:
            return 16
    else:
...
def analyzevars(block):
    ...
    for n in svars:
        ...
        if 'typespec' not in vars[n]:
            if not('attrspec' in vars[n] and 'external' in
    vars[n]['attrspec']):
                if implicitrules:
                    ln0 = n[0].lower()
                    for k in list(implicitrules[ln0].keys()):
                        if k == 'typespec' and implicitrules[ln0][k] ==
    'undefined':
                            continue
                        if k not in vars[n]:
                    ...
                    outmess('analyzevars: typespec of variable %s is not
    ...
    markoutercomma(intent).split('@,@')]:
                ...
                if depend is not None:
                    if 'depend' not in vars[n]:
                        vars[n]['depend'] = []
                    for c in rmbadname([x.strip() for x in
    ...
    markoutercomma(check).split('@,@')]:
            ...
            if dim and 'dimension' not in vars[n]:
                vars[n]['dimension'] = []
                for d in rmbadname([x.strip() for x in
    markoutercomma(dim).split('@,@')]):
                    star = ':' if d == ':' else '*'
                    # Evaluate `d` with respect to params
                    if d in params:
                        d = str(params[d])
                    for p in params:
                        re_1 = re.compile(r'(?P<before>.*?)\b' + p +
    r'\b(?P<after>.*)', re.I)
                        m = re_1.match(d)
                        while m:
                            d = m.group('before') + \
    ...
    repr(block['prefix']))
    if not block['block'] in ['module', 'pythonmodule', 'python module',
    'block data']:
...
def determineexprtype(expr, vars, rules={}):
    ...
    % repr(expr))
        return {'typespec': 'real'}
    for op in ['+', '-', '*', '/']:
        for e in [x.strip() for x in markoutercomma(expr, comma=op).split('@'
    + op + '@')]:
            if e in vars:
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if 'f2pyenhancements' in block:
        for k in list(block['f2pyenhancements'].keys()):
            f2pyenhancements = '%s%s%s %s' % (
                f2pyenhancements, tab + tabchar, k,
    ...
    ret = '%s%s%s %s%s%s %s%s%s%s%s%s%send %s %s' % (
        tab, prefix, blocktype, name, args, result, mess, f2pyenhancements,
    use, vars, common, body, tab, blocktype, name)
    return ret
    ...
    for a in nout:
        if 'depend' in vars[a]:
            for d in vars[a]['depend']:
                if d in vars and 'depend' in vars[d] and a in
    vars[d]['depend']:
                    errmess(
                        'vars2fortran: Warning: cross-dependence between
    variables "%s" and "%s"\n' % (a, d))
...
if __name__ == "__main__":
    ...
    for l in sys.argv[1:]:
        if l == '':
            ...
            if strictf77:
                outmess(
                    'Use option -f90 before -fix if Fortran 90 code is in fix
    form.\n', 0)
            skipemptyends = 1
            sourcecodeform = 'fix'
