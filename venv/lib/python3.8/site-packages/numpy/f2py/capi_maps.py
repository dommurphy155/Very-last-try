...
def load_f2cmap_file(f2cmap_file):
    ...
    {'%s':{'%s':'%s'->'%s'}}\n" % (k, k1, f2cmap_all[k][k1], d[k][k1]))
                    ...
                    errmess("\tIgnoring map {'%s':{'%s':'%s'}}: '%s' must be
    in %s\n" % (
            ...
            if '*' in var['kindselector']:
                ...
                try:
                    ...
                    try:
                        ...
                        errmess('getctype: "%s(kind=%s)" is mapped to C "%s"
...
def getstrlength(var):
    ...
    if re.match(r'\(\s*(\*|:)\s*\)', len) or re.match(r'(\*|:)', len):
        if isintent_hide(var):
            errmess('getstrlength:intent(hide): expected a string with defined
    length but got: %s\n' % (
                repr(var)))
        len = '-1'
    return len
...
def getarrdims(a, var, verbose=0):
    ...
    instead of %r\n'
                ...
                errmess(
                    'getarrdims: If in call-back function: array argument %s
...
def getpydocsign(a, var):
    ...
    if isscalar(var):
        if isintent_inout(var):
            sig = '%s: %s rank-0 array(%s,\'%s\')%s' % (a, opt,
    c2py_map[ctype],
                                                         c2pycode_map[ctype],
    init)
        else:
    ...
    storage'\
        ...
        ua = ''
        if a in lcb_map and lcb_map[a] in lcb2_map and 'argname' in
    lcb2_map[lcb_map[a]]:
    ...
    if hasinitvalue(var):
        ...
        if iscomplex(var) or iscomplexarray(var):
            ...
            try:
                ...
                raise ValueError(
                    'getinit: expected complex number `(r,i)\' but got `%s\'
...
def routsign2map(rout):
    ...
    if 'use' in rout:
        ...
        errmess('routsign2map: Confused: function %s has externals %s but no
    "use" statement.\n' % (
    ...
    if isfunction(rout):
        ...
        if debugcapi(rout):
            ...
            if isstringfunction(rout):
                ret['routdebugshowvalue'] = 'debug-capi:slen(%s)=%%d
    %s=\\"%%s\\"' % (
                    a, a)
        if isstringfunction(rout):
            ret['rlength'] = getstrlength(rout['vars'][a])
            if ret['rlength'] == '-1':
                errmess('routsign2map: expected explicit specification of the
