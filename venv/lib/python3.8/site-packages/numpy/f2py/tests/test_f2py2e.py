...
def get_io_paths(fname_inp, mname="untitled"):
    """Takes in a temporary file for testing and returns the expected output
    and input paths
...
def test_f2py_skip(capfd, retreal_f77, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        for skey in toskip.split():
            assert (
                f'buildmodule: Could not found the body of interfaced routine
    "{skey}". Skipping.'
                in err)
...
def test_f2py_only(capfd, retreal_f77, monkeypatch):
    ...
    with util.switchdir(ipath.parent):
        ...
        for skey in toskip.split():
            assert (
                f'buildmodule: Could not find the body of interfaced routine
    "{skey}". Skipping.'
                in err)
...
def test_f2cmap(capfd, f2cmap_f90, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", f'f2py -m blah {ipath} --f2cmap
    mapfile'.split())
