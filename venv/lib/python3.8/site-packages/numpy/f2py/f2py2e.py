...
2) To compile fortran files and build extension modules:
      ...
      f2py -c [<options>, <build_flib options>, <extra options>] <fortran
    files>
...
Options:
  ...
  only:            Use only fortran functions that follow until `:'.:                Get back to <fortran files> mode.
...
def scaninputline(inputline):
    ...
    for l in inputline:
        if l == '':
            ...
            outmess(
                'f2py option --include_paths is deprecated, use
    ...
    if signsfile and os.path.isfile(signsfile) and 'h-overwrite' not in
    options:
        errmess(
            'Signature file "%s" exists!!! Use --overwrite-signature to
    overwrite.\n' % (signsfile))
        sys.exit()
...
def run_main(comline_list):
    ...
    .. literalinclude:: ../../source/f2py/code/results/run_main_session.dat:language: python
    ...
    if 'signsfile' in options:
        if options['verbose'] > 1:
            outmess(
                'Stopping. Edit the signature file and then run f2py on the
    signature file: ')
            outmess('%s %s\n' %
    ...
    for plist in postlist:
        if plist['block'] != 'python module':
            if 'python module' not in options:
                errmess(
                    'Tip: If your original code is Fortran source then you
    must use -m option.\n')
