...
Test the header writing.
    ...
    ...     f = BytesIO()
    ...     format.write_array_header_1_0(f, arr)   # XXX: arr is not a dict,
...
def test_version_2_0_memmap(tmpdir):
    ...
    # 1.0 requested but data cannot be saved this way
    assert_raises(ValueError, format.open_memmap, tf1, mode='w+',
    dtype=d.dtype,
...
@pytest.mark.parametrize('dt, fail', [
    ...
    (np.dtype({'names': ['a', 'b'], 'formats': [
        float, np.dtype({'names': ['c'], 'formats': [np.dtype(int,
    metadata={})]})
    ]}), False)
    ])
