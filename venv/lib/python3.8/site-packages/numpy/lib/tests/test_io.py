    ...
    def test_large_zip(self):
        def check_large_zip(memoryerror_raised):
            ...
            try:
                ...
                raise
        # run in a subprocess to ensure memory is released on PyPy, see
    gh-15775
...
class TestLoadTxt(LoadTxtBase):
    ...
    def test_binary_load(self):
        ...
        with temppath() as path:
            ...
            x = [b'5,6,7,\xc3\x95scarscar', b'15,2,3,hello',
    b'20,2,3,\xc3\x95scar']
...
class Testfromregex:
    ...
    def test_record_unicode(self, path_type):
        ...
        with temppath() as str_path:
            ...
            x = np.fromregex(path, r"(?u)([0-9.]+)\s+(\w+)", dt,
    encoding='UTF-8')
            a = np.array([(1.312, 'foo' + utf8.decode('UTF-8')), (1.534,
    'bar'),
                           (4.444, 'qux')], dtype=dt)
            assert_array_equal(x, a)
...
M   33  21.99
        ...
        ctrl = np.array([('M', 21, 72.1), ('F', 35, 58.33), ('M', 33, 21.99)],
                        dtype=[('gender', '|S1'), ('age', int), ('weight',
    float)])
        assert_equal(test, ctrl)
...
M   33  21.99
    ...
    def test_names_and_comments_none(self):
        ...
        control = np.array([(1, 2), (3, 4)], dtype=[('col1', int), ('col2',
    int)])
        assert_equal(test, control)
    ...
    def test_invalid_converter(self):
        strip_rand = lambda x: float((b'r' in x.lower() and x.split()[-1]) or
                                     (b'r' not in x.lower() and x.strip() or
    0.0))
        strip_per = lambda x: float((b'%' in x.lower() and x.split()[0]) or
                                    (b'%' not in x.lower() and x.strip() or
    0.0))
    ...
    def test_easy_structured_dtype(self):
        ...
        ctrl = np.array([(0, 1., 2.3), (4, 5., 6.7)],
                        dtype=[("f_00", int), ("f_01", float), ("f_02",
    float)])
        assert_equal(mtest, ctrl)
    ...
    def test_unpack_auto_dtype(self):
        ...
        txt = TextIO("M 21 72.\nF 35 58.")
        expected = (np.array(["M", "F"]), np.array([21, 35]), np.array([72.,
    58.]))
...
class TestPathUsage:
    ...
    def test_recfromcsv(self):
        with temppath(suffix='.txt') as path:
            ...
            kwargs = dict(missing_values="N/A", names=True,
    case_sensitive=True)
