...
def poly(seq_of_zeros):
    ...
    .. note::
       ...
       A summary of the differences can be found in the:doc:`transition guide </reference/routines.polynomials>`.
    ...
    matrix **A** is given by:math:`p_a(t) = \\mathrm{det}(t\\, \\mathbf{I} - \\mathbf{A})`,
...
def polyint(p, m=1, k=None):
    ...
    The returned order `m` antiderivative `P` of polynomial `p` satisfies:math:`\\frac{d^m}{dx^m}P(x) = p(x)` and is defined up to `m - 1`
...
class poly1d:
    ...
    c_or_r: array_like
        ...
        ``poly1d([1, 2, 3])`` returns an object that represents:math:`x^2 + 2x + 3`, whereas ``poly1d([1, 2, 3], True)`` returns
