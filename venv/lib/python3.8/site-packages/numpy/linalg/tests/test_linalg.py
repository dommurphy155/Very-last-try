...
CASES += apply_tag('square', [
    ...
    LinalgCase("cdouble_2",
               array([[1. + 2j, 2 + 3j], [3 + 4j, 4 + 5j]], dtype=cdouble),
               array([[2. + 1j, 1. + 2j, 1 + 3j], [1 - 2j, 1 - 3j, 1 - 6j]],
    dtype=cdouble)),
    LinalgCase("0x0",
    ...
    for case in CASES:
        for a, a_label in _stride_comb_iter(case.a):
            for b, b_label in _stride_comb_iter(case.b):
                new_case = LinalgCase(case.name + "_" + a_label + "_" +
    b_label, a, b,
...
class SVDCases(LinalgSquareTestCase, LinalgGeneralizedSquareTestCase):
    ...
    def do(self, a, b, tags):
        ...
        assert_allclose(a, dot_generalized(np.asarray(u) * np.asarray(s)[...,
    None,:],
                                           np.asarray(vt)),
...
class _TestNormGeneral(_TestNormBase):
            ...
            for axis in itertools.combinations(range(-nd, nd), 2):
                ...
                if row_axis == col_axis:
                    ...
                    if row_axis < col_axis:
                        ...
                        expected = [norm(B[:].take(k, axis=k_index).T,
    ord=order)
                                    for k in range(B.shape[k_index])]
    ...
    def test_keepdims(self):
        ...
        shape_err = 'Shape mismatch found {0}, expected {1}, order={2},
    axis={3}'
        ...
        for order in [None, -1, 0, 1, 2, 3, np.Inf, -np.Inf]:
            for k in range(A.ndim):
                ...
                assert_(found.shape == expected_shape,
                        shape_err.format(found.shape, expected_shape, order,
    k))
