...
if os.name == 'nt':
    ...
    def GetPerformanceAttributes(object, counter, instance=None,
        ...
        # See
    http://msdn.microsoft.com/library/en-us/dnperfmo/html/perfmonpt2.asp (dead link)
...
def assert_approx_equal(actual,desired,significant=7,err_msg='',verbose=True):
    ...
    # scale =
    float(pow(10,math.floor(math.log10(0.5*(abs(desired)+abs(actual))))))
...
def assert_raises_regex(exception_class, expected_regexp, *args, **kwargs):
    ...
    """
    __tracebackhide__ = True  # Hide traceback for py.test
    return _d.assertRaisesRegex(exception_class, expected_regexp, *args,
    **kwargs)
...
def assert_array_almost_equal_nulp(x, y, nulp=1):
    ...
    if not np.all(np.abs(x-y) <= ref):
        if np.iscomplexobj(x) or np.iscomplexobj(y):
            ...
            msg = "X and Y are not equal to %d ULP (max is %g)" % (nulp,
    max_nulp)
        raise AssertionError(msg)
