...
def print_coercion_table(ntypes, inputfirstvalue, inputsecondvalue,
...
def print_new_cast_table(*, can_cast=True, legacy=False, flags=False):
    ...
    for cast in casts:
        ...
        to_dict[cast["to"]] = cast_info(can_cast=can_cast, legacy=legacy,
    flags=flags)
    ...
    if can_cast:
        ...
        print()
        print("Casting: # is equivalent, = is safe, ~ is same-kind, and . is
    unsafe")
        print()
    ...
    if flags:
        ...
        print(f"{flags_table[0]}: no flags, {flags_table[1]}: PyAPI, "
              f"{flags_table[2]}: supports unaligned, {flags_table[4]}:
    no-float-errors")
        print()
        print_table("flags")
