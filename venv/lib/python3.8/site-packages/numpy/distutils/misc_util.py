...
def njoin(*path):
    ...
    of n names as in njoin(['a','b']) is handled, or a mixture of such
    arguments.
    """
    paths = []
    for p in path:
...
class Configuration:
    ...
    def __init__(self,
        ...
        package_path -- directory of package. Will be computed by magic from
    the
    ...
    def _get_configuration_from_setup_py(self, setup_py,
        ...
        try:
            ...
            if not hasattr(setup_module, 'configuration'):
                ...
                pn = dot_join(*([parent_name] +
    subpackage_name.split('.')[:-1]))
                args = (pn,)
            ...
            if config.name!=dot_join(parent_name, subpackage_name):
                self.warn('Subpackage %r configuration returned as %r' % \
                          (dot_join(parent_name, subpackage_name),
    config.name))
        finally:
            del sys.path[0]
        return config
    ...
    def get_subpackage(self,subpackage_name,
        ...
        if subpackage_name is None:
            if subpackage_path is None:
                raise ValueError(
                    "either subpackage_name or subpackage_path must be
    specified")
        ...
        if subpackage_path is None and '*' in subpackage_name:
            return self._wildcard_get_subpackage(subpackage_name,
                                                 parent_name,
                                                 caller_level = caller_level+1)
        assert '*' not in subpackage_name, repr((subpackage_name,
    subpackage_path, parent_name))
        if subpackage_path is None:
            subpackage_path = njoin([self.local_path] + l)
        else:
    ...
    %r' \
                            ...
                            assert s==path_list[i], repr((s, path_list[i],
    data_path, d, path, rpath))
                            target_list.append(s)
                        i += 1
                    if path_list[i:]:
                        self.warn('mismatch of pattern_list=%s and
    path_list=%s'\
    ...
    def _optimize_data_files(self):
        ...
        for p, files in self.data_files:
            ...
            for f in files:
                data_dict[p].add(f)
        self.data_files[:] = [(p, list(files)) for p, files in
    data_dict.items()]
    ...
    build_info=None):
        ...
        Notes
        -----
        The best way to encode the options required to link against the
    specified
        ...
        self.installed_libraries.append(InstallableLib(name, build_info,
    install_dir))
    ...
    def add_npy_pkg_config(self, template, install_dir, subst_dict=None):
        ...
        subst_dict: dict, optional
            ...
            ``subst_dict[key]`` in the template file when installed. The
    install
            prefix is always available through the variable ``@prefix@``,
    since the
    ...
    def _get_svn_revision(self, path):
        ...
        if sys.platform=='win32' and os.environ.get('SVN_ASP_DOT_NET_HACK',
    None):
...
def generate_config_py(target):
    ...
    Usage:
        config['py_modules'].append((packagename,
    '__config__',generate_config_py))
    """
    ...
    (os.path.basename(sys.argv[0])))
        f.write('# It contains system_info results at the time of building
    this package.\n')
        ...
        f.write(textwrap.dedent(r'''
            ...
            def show():
                ...
                1. Classes specifying the information to be printed are defined
                   ...
                   * ``include_dirs``: directories containing library header
    files
    ...
    algebra
                   APIs. In this case, ``library_dirs`` reports the original
    build
    ...
    time
                   ...
                   (``pip install numpy --no-binary numpy``) searches for BLAS
    and
                   ...
                   In NumPy 1.21+ on macOS, 'accelerate' (Apple's Accelerate
    BLAS
