...
class build_src(build_ext.build_ext):
    ...
    user_options = [
        ...
        ('swig-cpp', None, "make SWIG create C++ files (default is
    autodetected from sources)"),
        ('f2pyflags=', None, "additional flags to f2py (use --f2py-opts=
    instead)"), # obsolete
        ('swigflags=', None, "additional flags to swig (use --swig-opts=
    instead)"), # obsolete
    ...
    *sys.version_info[:2])
    ...
    def build_npy_pkg_config(self):
        ...
        if build_npkg:
            for pkg, infos in self.distribution.installed_pkg_config.items():
                ...
                d = {'prefix': prefix}
                for info in infos:
                    install_dir, generated = self._build_npy_pkg_config(info,
    d)
    ...
    os.path.dirname(base))
                ...
                if (self.force or newer_group([source] + depends,
    target_file)):
    ...
    names: %s '
                                                  'provides %r but expected
    %r' % (
                            source, name, ext_name))
                    target_file = os.path.join(target_dir, name+'module.c')
                else:
                    log.debug('  source %s does not exist: skipping
    f2py\'ing.' \
                              % (source))
                    name = ext_name
                    skip_f2py = 1
        ...
        if f2py_sources:
            if len(f2py_sources) != 1:
                raise DistutilsSetupError(
                    'only one .pyf file is allowed per extension module but
    got'\
                    ' more: %r' % (f2py_sources,))
    ...
    (source))
            ...
            if (self.force or newer_group(depends, target_file, 'newer')) \
                ...
                log.debug("  skipping f2py fortran files for '%s'
    (up-to-date)"\
                          % (target_file))
    ...
    os.path.dirname(base))
                ...
                if os.path.isfile(source):
                    ...
                    if typ is None:
                        ...
                        if typ2 is None:
                            log.warn('source %r does not define swig target,
    assuming %s swig target' \
                                     % (source, typ))
                        elif typ!=typ2:
    ...
    swig target' \
                                     % (typ, source, typ2))
                            if typ2=='c++':
                                log.warn('resetting swig target to c++ (some
    targets may have .c extension)')
                                is_cpp = True
                            else:
                                log.warn('assuming that %r has c++ swig
    target' % (source))
                    if is_cpp:
                        target_ext = '.cpp'
...
_swig_module_name_match =
    re.compile(r'\s*%module\s*(.*\(\s*package\s*=\s*"(?P<package>[\w_]+)".*\)|)\s*(?P<name>[\w_]+)',
                                     re.I).match
...
_f2py_user_module_name_match =
    re.compile(r'\s*python\s*module\s*(?P<name>[\w_]*?'
