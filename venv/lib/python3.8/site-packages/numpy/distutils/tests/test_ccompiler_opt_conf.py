...
-   before it called by the abstract class '_Feature'
    ...
    uppercase")
        ...
        self.test_extra_checks(error_msg, search_in, feature_name,
    feature_dict)
    ...
    def test_duplicates(self, error_msg, option, val):
        ...
        if len(val) != len(set(val)):
            raise AssertionError(error_msg + "duplicated values in option
    '%s'" % option)
        ...
        for impl in implies:
            ...
            if impl_dict is not None:
                if "disable" in impl_dict:
                    raise AssertionError(error_msg + "implies disabled feature
    '%s'" % impl)
                continue
            raise AssertionError(error_msg + "implies non-exist feature '%s'"
    % impl)
    ...
    def test_extra_checks(self, error_msg, search_in, feature_name,
    feature_dict):
        if feature_dict.get("disabled") is not None:
            return
        ...
        for f in extra_checks:
            ...
            if not impl_dict or "disable" in impl_dict:
                continue
            raise AssertionError(error_msg +
                "in option 'extra_checks', extra test case '%s' already exists
    as a feature name" % f
            )
