...
class _Test_CCompilerOpt:
        ...
        with tempdir() as tmpdir:
            ...
            for source, flags in fake_objects:
                ...
                if glen == 0:
                    ...
                    # converting multi-target into parentheses str format to
    be equivalent
        ...
        if len(targets) != len(gtargets) or not all(t in gtargets for t in
    targets):
            raise AssertionError(
                "'sources_status' returns different targets than the compiled
    targets\n"
    ...
    match_tar
            ...
            if not re.match(match_flags, flags):
                raise AssertionError(
                    '"%s" flags "%s" not match "%s"' % (match_tar, flags,
    match_flags)
                )
    ...
    def test_args_validation(self):
        ...
        for baseline, dispatch in (
            ("unkown_feature - max +min", "unknown max min"), # unknowing
    features
            ("#avx2", "$vsx") # groups and polices aren't acceptable
        ):
            try:
                self.nopt(cpu_baseline=baseline, cpu_dispatch=dispatch)
                raise AssertionError("excepted an exception for invalid
    arguments")
    ...
    def test_args_options(self):
        ...
        for o in ("max", "native"):
            ...
            self.expect(o,
                x86_gcc=".* xop fma4 .* avx512f .* avx512_knl avx512_knm
    avx512_skx .*",
    ...
    def test_flags(self):
        ...
        self.expect_flags(
            "asimd",
            aarch64="",
            armhf_gcc=r"-mfp16-format=ieee -mfpu=neon-fp-armv8
    -march=armv8-a\+simd"
        )
        self.expect_flags(
            "asimdhp",
