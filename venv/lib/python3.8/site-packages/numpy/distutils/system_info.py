...
Currently, the following classes are available, along with their section names:
    ...
    blas64__opt_info:ALL               # usage recommended (general ILP64
    BLAS, 64_ symbol suffix)
    blas_ilp64_opt_info:ALL            # usage recommended (general ILP64 BLAS)
    blas_ilp64_plain_opt_info:ALL      # usage recommended (general ILP64
    BLAS, no symbol suffix)
    blas_info:blas
    blas_mkl_info:mkl
    ...
    lapack64__opt_info:ALL             # usage recommended (general ILP64
    LAPACK, 64_ symbol suffix)
    lapack_atlas_3_10_info:atlas
    lapack_atlas_3_10_threads_info:atlas
    lapack_atlas_info:atlas
    lapack_atlas_threads_info:atlas
    lapack_ilp64_opt_info:ALL          # usage recommended (general ILP64
    LAPACK)
    lapack_ilp64_plain_opt_info:ALL    # usage recommended (general ILP64
    LAPACK, no symbol suffix)
    lapack_info:lapack
    lapack_mkl_info:mkl
...
def _parse_env_order(base_order, env):
    """ Parse an environment variable `env` by splitting with "," and only
    returning elements from `base_order`

    This method will sequence the environment variable and check for their
    individual elements in `base_order`.

    The items in the environment variable may be negated via '^item' or
    '!itema,itemb'.
    ...
    if neg:
        if sum_neg > 1:
            raise ValueError(f"Environment variable '{env}' may only contain a
    single (prefixed) negation: {order_str}")
        # remove prefix
        order_str = order_str[1:]
    elif sum_neg > 0:
        raise ValueError(f"Environment variable '{env}' may not mix negated an
    non-negated items: {order_str}")
...
class AliasedOptionError(DistutilsError):
    ...
    In section '{section}' we found multiple appearances of options
    {options}."""
...
class system_info:
    ...
    os.pathsep.join(default_runtime_dirs),
                    'rpath': '',
...
class atlas_info(system_info):
    ...
    *********************************************************************
                        ...
                        Follow the instructions in the KNOWN PROBLEMS section
    of the file
                        numpy/INSTALL.txt.
...
class lapack_opt_info(system_info):
    ...
    def _get_info_lapack(self):
        ...
        if not info:
            ...
            if not info_src:
                warnings.warn(LapackSrcNotFoundError.__doc__ or '',
    stacklevel=3)
                return {}
    ...
    def calc_info(self):
        lapack_order, unknown_order = _parse_env_order(self.lapack_order,
    self.order_env_var_name)
        ...
        if 'lapack' not in lapack_order:
            # Since the user may request *not* to use any library, we still
    need
...
class blas_opt_info(system_info):
    ...
    def calc_info(self):
        blas_order, unknown_order = _parse_env_order(self.blas_order,
    self.order_env_var_name)
        if len(unknown_order) > 0:
            raise ValueError("blas_opt_info user defined BLAS order has
    unacceptable values: {}".format(unknown_order))
...
class blas_info(system_info):
    ...
    def calc_info(self):
        ...
        if platform.system() == 'Windows':
            ...
            cblas_opt = cblas_info_obj.get_option_single('cblas_libs',
    'libraries')
...
class flame_info(system_info):
    ...
    Be aware that libflame 5.1.0 has some missing names in the shared library,
    so
    ...
    def check_embedded_lapack(self, info):
        """ libflame does not necessarily have a wrapper for fortran LAPACK,
    we need to check """
        c = customized_ccompiler()
