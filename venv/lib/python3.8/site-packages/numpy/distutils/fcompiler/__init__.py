...
class FCompiler(CCompiler):
    ...
    src_extensions = ['.for', '.ftn', '.f77', '.f', '.f90', '.f95', '.F',
    '.F90', '.FOR']
    obj_extension = ".o"
    ...
    def _compile(self, obj, src, ext, cc_args, extra_postargs, pp_opts):
        ...
        if is_f_file(src) and not has_f90_header(src):
            ...
            if compiler is None:
                raise DistutilsExecError('f90 not supported by %s needed for
    %s'\
            ...
            if compiler is None:
                raise DistutilsExecError('f90 (fixed) not supported by %s
    needed for %s'\
                      % (self.__class__.__name__, src))
            extra_compile_args = self.extra_f90_compile_args or []
    ...
    def module_options(self, module_dirs, module_build_dir):
        ...
        if self.module_dir_switch is not None:
            if self.module_dir_switch[-1]==' ':
                options.extend([self.module_dir_switch.strip(),
    module_build_dir])
            else:
...
_default_compilers = (
    ...
    ('cygwin.*', ('gnu', 'intelv', 'absoft', 'compaqv', 'intelev', 'gnu95',
    'g95')),
...
def _find_existing_fcompiler(compiler_types,
    ...
    for compiler_type in compiler_types:
        ...
        try:
            ...
            log.debug("_find_existing_fcompiler: compiler_type='%s' raised
    DistutilsModuleError", compiler_type)
        except CompilerNotFound:
            log.debug("_find_existing_fcompiler: compiler_type='%s' not
    found", compiler_type)
        if v is not None:
...
_f77flags_re =
    re.compile(r'(c|)f77flags\s*\(\s*(?P<fcname>\w+)\s*\)\s*=\s*(?P<fflags>.*)', re.I)
