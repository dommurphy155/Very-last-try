...
class _Config:
    ...
    conf_cache_factors: list
        ...
        are the arguments of `CCompilerOpt` and `CCompiler`'s properties(type,
    flags, etc).
    ...
    conf_target_groups: dict
        ...
        **Notes**:
            ...
            - sign '#' must stick in the begin of group name and only within
    ``@targets``
    ...
    conf_features: dict
        ...
        Default value is covers almost all CPU features for *X86*,
    *IBM/Power64*
        and *ARM 7/8*.
        ...
        "extra_checks": str or list, optional
            ...
            It should contain at least one intrinsic or function related to
    the test case.

            If the compiler able to successfully compile the C file then
    `CCompilerOpt`
            will add a C ``#define`` for it into the main dispatch header, e.g.
            ``#define {conf_c_prefix}_XXXX`` where ``XXXX`` is the case name
    in upper case.

        **NOTES**:
            * space can be used as separator with options that supports "str
    or list"
            * case-sensitive for all values and feature name must be in
    upper-case.
            * if flags aren't applicable, its will skipped rather than disable
    the
              CPU feature
    ...
    conf_features = dict(
        ...
        AVX512_SKX = dict(
            interest=42, implies="AVX512CD", group="AVX512VL AVX512BW
    AVX512DQ",
...
class _Distutils:
    ...
    ----------
    ccompiler: `CCompiler`
        The generate instance that returned from
    ...
    def dist_info(self):
        ...
        cc_info = getattr(self._ccompiler, "compiler",
...
class _Cache:
    ...
    *factors:
    ...
    cache_infile: bool
        Utilized during initializing this class, to determine if the cache was
    able
...
class _CCompiler:
    ...
    def __init__(self):
        ...
        for detect, searchin in ((detect_arch, platform), (detect_compiler,
    compiler_info)):
    ...
    stderr=True)
        ...
        if self.cc_is_nocc:
            """
            mingw can be treated as a gcc, and also xlc even if it based on
    clang,
            but still has the same gcc optimization flags.
            """
            self.dist_log(
                "unable to detect compiler type which leads to treating it as
    GCC. "
                "this is a normal behavior if you're using gcc-like compiler
    such as MinGW or IBM/XLC."
                f"check dist_info:<<\n{dist_info}\n>>",
                stderr=True
            )
            self.cc_is_gcc = True
...
class _Feature:
    ...
    **C** compiler.
    ...
    def feature_is_supported(self, name, force_flags=None, macros=[]):
        """
        Check if a certain CPU feature is supported by the platform and
    compiler.
    ...
    def feature_extra_checks(self, name):
        ...
        self.dist_log("Testing extra checks for feature '%s'" % name,
    extra_checks)
    ...
    def feature_c_preprocessor(self, feature_name, tabs=0):
        """
        Generate C preprocessor definitions and include headers of a CPU
    feature.
        ...
        'feature_name': str
            CPU feature name in uppercase.
        'tabs': int
            if > 0, align the generated strings to the right depend on number
    of tabs.
...
class _Parse:
    ...
    Special options can be:
        - **MIN**: Enables the minimum CPU features that utilized via
    ...
    machine.
        ...
        - **Operand +/-**: remove or add features, useful with options
    **MAX**, **MIN** and **NATIVE**.
    ...
    groups
        ...
        if cpu_baseline is not None:
            cpu_baseline = self._parse_arg_features("cpu_baseline",
    cpu_baseline)
        ...
        if cpu_dispatch is not None:
            cpu_dispatch_ = self._parse_arg_features("cpu_dispatch",
    cpu_dispatch)
            cpu_dispatch = {
            ...
            if len(conflict_baseline) > 0:
                self.dist_log(
                    "skip features", conflict_baseline, "since its part of
    baseline"
                )
    ...
    def _parse_arg_features(self, arg_name, req_features):
        ...
        tokens = list(filter(None, re.split(self._parse_regex_arg,
    req_features)))
    ...
    def _parse_target_tokens(self, tokens):
        ...
        for tok in tokens:
            ...
            if ch in ('+', '-'):
                ...
                self._parse_token_group(TOK, has_baseline, final_targets,
    extra_flags)
            elif ch == '(':
                if multi_target is not None:
    ...
    def _parse_token_group(self, token, has_baseline, final_targets,
    extra_flags):
    ...
    def _parse_policy_not_keepbase(self, has_baseline, final_targets,
    extra_flags):
    ...
    def _parse_policy_not_keepsort(self, has_baseline, final_targets,
    extra_flags):
    ...
    skipped.")
            ...
            self.dist_log("optimization is disabled, policy 'maxopt' is
    skipped.")
        else:
...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    """
    def __init__(self, ccompiler, cpu_baseline="min", cpu_dispatch="max",
    cache_path=None):
        _Config.__init__(self)
        _Distutils.__init__(self, ccompiler)
        _Cache.__init__(self, cache_path, self.dist_info(), cpu_baseline,
    cpu_dispatch)
    ...
    def try_dispatch(self, sources, src_dir=None, ccompiler=None, **kwargs):
        ...
        src_dir: str
            Path of parent directory for the generated headers and wrapped
    sources.
        ...
        for src in sources:
            ...
            if src_dir:
                ...
                if output_dir not in include_dirs:
                    # To allow including the generated config
    header(*.dispatch.h)
    ...
    has_baseline)
            for tar in targets:
                tar_src = self._wrap_target(output_dir, src, tar,
    nochange=nochange)
        ...
        #   before the compilation in case if there are dependency
    relationships
        #   among them.
        objects = []
    ...
    def generate_dispatch_header(self, header_path):
        """
        Generate the dispatch header which contains the #definitions and
    headers
        ...
        if not os.path.exists(header_dir):
            self.dist_log(
                f"dispatch header dir {header_dir} does not exist, creating
    it",
                stderr=True
            )
    ...
    '\n'
            ...
            for name in dispatch_names:
                ...
                """).format(
                    pfx=self.conf_c_prefix_, name=name,
    ...
    "none")
        ...
        if not full or not target_sources:
            ...
            dispatch_rows.append(("Generated", generated[:-1] if generated
    else "none"))
        else:
    ...
    '.join(tar)
                ...
                implies = '
    '.join(self.feature_sorted(self.feature_implies(tar)))
                detect = ' '.join(self.feature_detect(tar))
                extra_checks = []
                for name in ((tar,) if isinstance(tar, str) else tar):
                    extra_checks += self.feature_extra_checks(name)
                extra_checks = (' '.join(extra_checks) if extra_checks else
    "none")
    ...
    def _wrap_target(self, output_dir, dispatch_src, target, nochange=False):
        ...
        wrap_path = "{0}.{2}{1}".format(*os.path.splitext(wrap_path),
    ext_name.lower())
    ...
    def _generate_config(self, output_dir, dispatch_src, targets,
    has_baseline=False):
...
def new_ccompiler_opt(compiler, dispatch_hpath, **kwargs):
    ...
    which contains the #definitions and headers of platform-specific
