...
from numpy.typing.mypy_plugin import _PRECISION_DICT, _EXTENDED_PRECISION_LIST, _C_INTP
...
def test_fail(path: str) -> None:
    ...
    for error_line in output_mypy[path]:
        ...
        match = re.match(r"(?P<lineno>\d+): (error|note): .+$", error_line)
        if match is None:
...
def test_reveal(path: str) -> None:
    ...
    for error_line in output_mypy[path]:
        ...
        match = re.match(r"(?P<lineno>\d+): note: .+$", error_line)
        if match is None:
...
def _test_reveal(
    path: str, expression: str, reveal: str, expected_reveal: str, lineno: int
) -> None:
