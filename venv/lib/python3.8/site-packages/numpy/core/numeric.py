...
def correlate(a, v, mode="valid"):
    ...
    with a and v sequences being zero-padded where necessary and:math:`\overline x` denoting complex conjugation.
    ...
    because it does
    not use the FFT to compute the convolution; in that case,
...
def isscalar(element):
    ...
    functions in the style of the ``dx`` arguments to `gradient` and the
    ``bins``
...
def binary_repr(num, width=None):
    ...
    system can represent every integer in the range:math:`-2^{N-1}` to:math:`+2^{N-1}-1`.
