...
NPY_RELAXED_STRIDES_CHECKING = (os.environ.get('NPY_RELAXED_STRIDES_CHECKING',
    "1") != "0")
...
NPY_RELAXED_STRIDES_DEBUG = (os.environ.get('NPY_RELAXED_STRIDES_DEBUG', "0")
    != "0")
NPY_RELAXED_STRIDES_DEBUG = NPY_RELAXED_STRIDES_DEBUG and
...
def check_types(config_cmd, ext, build_dir):
    ...
    for type in ('short', 'int', 'long'):
        res = config_cmd.check_decl("SIZEOF_%s" % sym2def(type),
    headers=["Python.h"])
        if res:
            public_defines.append(('NPY_SIZEOF_%s' % sym2def(type),
    "SIZEOF_%s" % sym2def(type)))
        else:
            res = config_cmd.check_type_size(type, expected=expected[type])
            if res >= 0:
                public_defines.append(('NPY_SIZEOF_%s' % sym2def(type), '%d' %
    res))
            else:
    ...
    res))
            if not already_declared and not type == 'long double':
                private_defines.append(('SIZEOF_%s' % sym2def(type), '%d' %
    res))
        else:
            raise SystemError("Checking sizeof (%s) failed !" % type)
        ...
        res = config_cmd.check_type_size(complex_def,
                                         expected=[2 * x for x in
    expected[type]])
        if res >= 0:
            public_defines.append(('NPY_SIZEOF_COMPLEX_%s' % sym2def(type),
    '%d' % res))
        else:
    ...
    '%d' % res))
        ...
        if res >= 0:
            #private_defines.append(('SIZEOF_%s' % sym2def('long long'), '%d'
    % res))
...
def configuration(parent_package='',top_path=None):
    ...
    def generate_config_h(ext, build_dir):
        ...
        if newer(__file__, target):
            ...
            with open(target, 'w') as target_f:
                ...
                # add the guard to make sure config.h is never included
    directly,
                # but always through npy_config.h
                target_f.write(textwrap.dedent("""
                    #ifndef NUMPY_CORE_SRC_COMMON_NPY_CONFIG_H_
                    #error config.h should never be included directly, include
    npy_config.h instead
                    #endif
                    """))
    ...
    config.add_define_macros([("NPY_INTERNAL_BUILD", "1")]) # this macro
    indicates that Numpy build is in process
    ...
    have_blas = blas_info and ('HAVE_CBLAS', None) in
