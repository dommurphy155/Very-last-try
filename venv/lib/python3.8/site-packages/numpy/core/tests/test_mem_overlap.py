        ...
        while min(feasible_count, infeasible_count) < min_count:
            ...
            if X is None:
                ...
                for r in ranges:
                    size *= len(r)
                if size < 100000:
                    assert_(not any(sum(w) == b for w in
...
def check_may_share_memory_exact(a, b):
    ...
    if got != exact:
        err_msg = "    " + "\n    ".join([
            "base_a - base_b = %r" % (a.__array_interface__['data'][0] -
...
def test_may_share_memory_manual():
    ...
    xs = []
    for x in xs0:
        for ss in itertools.product(*(([slice(None), slice(None, None,
    -1)],)*4)):
            xp = x[ss]
            xs.append(xp)
...
def test_may_share_memory_harder_fuzz():
    ...
    check_may_share_memory_easy_fuzz(get_max_work=lambda a, b: max(a.size,
    b.size)//2,
                                     same_steps=False,
                                     min_count=2000)
...
def test_internal_overlap_diophantine():
    def check(A, U, exists=None):
        ...
        if X is not None:
            assert_(sum(a*x for a, x in zip(A, X)) == sum(a*u//2 for a, u in
    zip(A, U)))
            ...
            while overlapping < min_count:
                ...
                if get_out_axis_size is None:
                    ...
                    for axis in itertools.chain(range(ndim), [None]):
                        ...
                        # Check result
                        assert_copy_equivalent(operation, [a], out=b_out,
    axis=axis)
    ...
    def test_unary_gufunc_fuzz(self):
        ...
        for ndim in range(2, 6):
            ...
            while overlapping < min_count:
                ...
                if min(a.shape[-2:]) < 2 or min(b.shape[-2:]) < 2 or
    a.shape[-1] < 2:
                    continue
