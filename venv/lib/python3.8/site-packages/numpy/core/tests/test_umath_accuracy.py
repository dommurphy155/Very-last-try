...
IS_AVX = __cpu_features__.get('AVX512F', False) or \
        (__cpu_features__.get('FMA3', False) and __cpu_features__.get('AVX2',
    False))
...
def convert(s, datatype="np.float32"):
    ...
    integer
        fp = cast(cp, POINTER(c_double))  # cast the int pointer to a double
    pointer
    else:
...
class TestAccuracy:
    ...
    names=('type','input','output','ulperr'),
                    ...
                    for datatype in np.unique(data['type']):
                        ...
                        inval  =
    np.array(str_to_float(data_subset['input'].astype(str), data_subset['type'].astype(str)), dtype=eval(datatype))
                        outval =
    np.array(str_to_float(data_subset['output'].astype(str), data_subset['type'].astype(str)), dtype=eval(datatype))
