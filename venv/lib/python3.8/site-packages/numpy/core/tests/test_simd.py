...
class _Test_Utility:
    ...
    def _data(self, start=None, count=None, reverse=False):
        """
        Create list of consecutive numbers according to number of vector's
    lanes.
        """
        if start is None:
            start = 1
...
class _SIMD_BOOL(_Test_Utility):
    ...
    def test_tobits(self):
        data2bits = lambda data: sum([int(x != 0) << i for i, x in
    enumerate(data, 0)])
...
class _SIMD_FP32(_Test_Utility):
    ...
    def test_conversions(self):
        """
        Round to nearest even integer, assume CPU control register is set to
    rounding.
        Test intrinsics:
            npyv_round_s32_##SFX
        """
...
class _SIMD_FP(_Test_Utility):
    ...
    def test_arithmetic_fused(self):
        ...
        data_fma = self.load([a * b + c for a, b, c in zip(vdata_a, vdata_b,
    vdata_c)])
    ...
    def test_reciprocal(self):
        ...
        recip_cases = ((nan, nan), (pinf, 0.0), (ninf, -0.0), (0.0, pinf),
    (-0.0, ninf))
        ...
        data_recip = self.load([1/x for x in data]) # load to truncate
    precision
...
class _SIMD_ALL(_Test_Utility):
    ...
    def test_misc(self):
        ...
        # py level of npyv_set_* don't support ignoring the extra specified
    lanes or
        # fill non-specified lanes with zero.
        vset = self.set(*data_a)
        assert vset == data_a
        # py level of npyv_setf_* don't support ignoring the extra specified
    lanes or
        ...
        select_a = self.select(self.cmpeq(self.zero(), self.zero()), vdata_a,
    vdata_b)
    ...
    def test_arithmetic_subadd(self):
        ...
        # non-saturated
        data_add = self.load([a + b for a, b in zip(data_a, data_b)]) # load
    to cast
...
for target_name, npyv in targets.items():
    ...
    target_name=target_name)
            tcls =
    type(f"Test{cls.__name__}_{simd_width}_{target_name}_{sfx}", inhr, attr)
            if skip_m:
