...
class TestEinsum:
    ...
    optimize=do_opt)
            ...
            assert_raises(ValueError, np.einsum, "i", np.arange(6).reshape(2,
    3),
                          optimize=do_opt)
            ...
            for n in range(1, 17):
                if n < 8 or dtype != 'f2':
                    ...
                    assert_equal(np.einsum("ij,jk", a, b, optimize=do_opt),
                                 np.dot(a, b))
                    assert_equal(np.einsum(a, [0, 1], b, [1, 2],
    optimize=do_opt),
                                 np.dot(a, b))
    ...
    def test_einsum_misc(self):
        ...
        assert_equal(np.einsum('ij...,j...->i...', a, b, optimize=True),
    [[[2], [2]]])
    ...
    def test_subscript_range(self):
        # Issue #7741, make sure that all letters of Latin alphabet (both
    uppercase & lowercase) can be used
        ...
        assert_raises(ValueError, lambda: np.einsum(a, [0, 52], b, [52, 2],
    [0, 2], optimize=False))
        assert_raises(ValueError, lambda: np.einsum(a, [-1, 5], b, [5, 2],
    [-1, 2], optimize=False))
    ...
    # used to raise error
        ...
        for opt in [True, False]:
            assert_equal(np.einsum('ik...,k...->i...', A, B, optimize=opt),
    ref)
            assert_equal(np.einsum('ik...,...kj->i...j', A, B, optimize=opt),
    ref)
...
class TestEinsumPath:
    ...
    def test_long_paths(self):
        ...
        # Long test 1
        long_test1 =
    self.build_operands('acdf,jbje,gihb,hfac,gfac,gifabc,hfac')
    ...
    def test_edge_paths(self):
        ...
        edge_test4 = self.build_operands('a,ac,ab,ad,cd,bd,bc->',
                                         size_dict={"a": 20, "b": 20, "c": 20,
    "d": 20})
        path, path_str = np.einsum_path(*edge_test4, optimize='greedy')
        self.assert_path_equal(path, ['einsum_path', (0, 1), (0, 1, 2, 3, 4,
    5)])
