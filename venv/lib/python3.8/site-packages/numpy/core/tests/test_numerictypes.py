...
class TestMaximumSctype:
    ...
    @pytest.mark.parametrize('t', [np.byte, np.short, np.intc, np.int_,
    np.longlong])
    def test_int(self, t):
        assert_equal(np.maximum_sctype(t), np.sctypes['int'][-1])

    @pytest.mark.parametrize('t', [np.ubyte, np.ushort, np.uintc, np.uint,
    np.ulonglong])
    ...
    @pytest.mark.parametrize('t', [np.bool_, np.object_, np.unicode_,
    np.bytes_, np.void])
