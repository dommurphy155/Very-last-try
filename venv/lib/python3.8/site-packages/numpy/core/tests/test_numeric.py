...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        u8, u16, u32, u64 = np.uint8(0), np.uint16(0), np.uint32(0),
    np.uint64(0)
...
class TestNonzero:
    ...
    def test_nonzero_onedim(self):
        ...
        #              dtype=[('a', 'i4'), ('b', 'i2')])
        x = np.array([(1, 2, -5, -3), (0, 0, 2, 7), (1, 1, 0, 1), (-1, 3, 1,
    0), (0, 7, 0, 4)],
                     dtype=[('a', 'i4'), ('b', 'i2'), ('c', 'i1'), ('d',
    'i8')])
    ...
    def test_sparse(self):
        ...
        for i in range(20):
            ...
            assert_equal(np.nonzero(c)[0],
                         np.concatenate((np.arange(10 + i, 20 + i), [20 +
    i*2])))
...
class TestIndex:
    def test_boolean(self):
        ...
        V[g1, g2] = -V[g1, g2]
        assert_((np.array([a[0][V > 0], a[1][V > 0], a[2][V > 0]]) == a[:, V >
    0]).all())
...
class TestArrayComparisons:
    def test_array_equal(self):
        ...
        assert_(not res)
        assert_(type(res) is bool)
        res = np.array_equal(np.array(['a'], dtype='S1'), np.array(['a'],
    dtype='S1'))
        assert_(res)
        assert_(type(res) is bool)
...
class TestClip:
    ...
    def test_clip_property(self, data, arr):
        ...
        on previous values.  An alternative would be to define a custom
    strategy
        ...
        # and with the base shape.  Below, we might decide to use scalar
    bounds,
...
class TestIsclose:
    ...
    def tst_isclose_allclose(self, x, y):
        ...
        if np.isscalar(x) and np.isscalar(y):
            ...
            assert_array_equal(np.isclose(x, y).all(), np.allclose(x, y), msg
    % (x, y))
    ...
    def test_timedelta(self):
        ...
        assert np.isclose(a, a, atol=np.timedelta64(1, "ns"),
    equal_nan=True).all()
...
class TestTensordot:
    ...
    def test_zero_dimensional(self):
        # gh-12130
        arr_0d = np.array(1)
        ret = np.tensordot(arr_0d, arr_0d, ([], []))  # contracting no axes is
    well defined
