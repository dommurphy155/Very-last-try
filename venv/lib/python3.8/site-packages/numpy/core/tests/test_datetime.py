...
class TestDateTime:
    ...
    def test_pydatetime_creation(self):
        ...
        a = np.array(['1999-12-31', datetime.date(1999, 12, 31)],
    dtype='M8[D]')
        assert_equal(a[0], a[1])
    ...
    def test_cast_overflow(self):
        # gh-4486
        def cast():
            numpy.datetime64("1971-01-01
    00:00:00.000000000000000").astype("<M8[D]")
        ...
        for unit1 in ['D', '12h', 'h', 'm', 's', '4s', 'ms', 'us']:
            ...
            for unit2 in ['D', 'h', 'm', 's', 'ms', 'us']:
                ...
                assert_(np.equal(np.array('1932-02-17', dtype='M').astype(dt1),
                     ...
                     casting='unsafe'))
                assert_(np.equal(np.array('10000-04-27',
    dtype='M').astype(dt1),
    ...
    def test_datetime_busday_offset(self):
        ...
        assert_equal(np.busday_offset(np.datetime64('NaT'), 1, roll='nat'),
                     np.datetime64('NaT'))
        assert_equal(np.busday_offset(np.datetime64('NaT'), 1,
    roll='following'),
                     np.datetime64('NaT'))
        assert_equal(np.busday_offset(np.datetime64('NaT'), 1,
    roll='preceding'),
    ...
    def test_datetime_busdaycalendar(self):
        ...
        assert_raises(ValueError, np.busdaycalendar, weekmask=[0, 0, 0, 0, 0,
    0, 0])
    ...
    @pytest.mark.parametrize("time_unit", [
        ...
        "D", "h", "m",
        "s", "ms", "us", "ns", "ps", "fs", "as",
        pytest.param("10D", marks=pytest.mark.xfail(reason="similar to
    gh-13197")),
    ])
