...
class TestComparisonDeprecations(_DeprecationTestCase):
    ...
    This used to mean that when an error occurred during element-wise
    comparison
...
class BuiltInRoundComplexDType(_DeprecationTestCase):
    ...
    def test_deprecated(self):
        for scalar_type in self.deprecated_types:
            ...
            self.assert_deprecated(round, args=(scalar,), kwargs={'ndigits':
    0})
...
class TestNoseDecoratorsDeprecated(_DeprecationTestCase):
    ...
    def test_skip_generators_hardcoded(self):
        def _test_skip_generators_hardcoded():
            ...
            @np.testing.dec.knownfailureif(False, "This test is NOT known to
    fail")
            def g2(x):
                yield from range(x)
    ...
    def test_skip_generators_callable(self):
        def _test_skip_generators_callable():
            ...
            @np.testing.dec.knownfailureif(skip_tester, "This test is known to
    fail")
            def g1(x):
                yield from range(x)
            ...
            @np.testing.dec.knownfailureif(skip_tester, "This test is NOT
    known to fail")
            def g2(x):
                yield from range(x)
                raise self.DidntSkipException('FAIL')
    ...
    stacklevel=1)
            ...
            with warnings.catch_warnings(record=True):
                warnings.simplefilter("always")  # do not propagate unrelated
    warnings
