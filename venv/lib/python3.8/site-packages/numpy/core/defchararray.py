...
def decode(a, encoding=None, errors=None):
    ...
    and may be extended at runtime.  For more information, see the:mod:`codecs` module.
...
def strip(a, chars=None):
    ...
    >>> np.char.strip(c)
    array(['aAaAaA', 'aA', 'abBABba'], dtype='<U7')
    >>> np.char.strip(c, 'a') # 'a' unstripped from c[1] because whitespace
    leads
    array(['AaAaA', '  aA  ', 'bBABb'], dtype='<U7')
    >>> np.char.strip(c, 'A') # 'A' unstripped from c[1] because (unprinted)
    ws trails
...
def isnumeric(a):
    ...
    if _use_unicode(a) != unicode_:
        raise TypeError("isnumeric is only available for Unicode strings and
    arrays")
