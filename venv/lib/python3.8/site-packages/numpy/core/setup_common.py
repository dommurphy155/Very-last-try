...
OPTIONAL_INTRINSICS = [("__builtin_isnan", '5.'),
                       ...
                       ("__asm__ volatile", '"vfpclasspd $0x40, %zmm15,
    %k6\\n"\
                                             "vmovdqu8 %xmm0, %xmm1\\n"\
...
# tested via "#include<%s> int %s %s(void *){code; return 0;};" % (header,
    attribute, name, code)
# function name will be converted to HAVE_<upper-case-name> preprocessor macro
...
def check_long_double_representation(cmd):
    ...
    # Disable whole program optimization (the default on vs2015, with python
    3.5+)
    ...
    # Disable multi-file interprocedural optimization in the Intel compiler on
    Linux
    # which generates intermediary object files and prevents checking the
    # float representation.
...
_IBM_DOUBLE_DOUBLE_BE = (['301', '235', '157', '064', '124', '000', '000',
    '000'] +
                     ['000'] * 8)
_IBM_DOUBLE_DOUBLE_LE = (['000', '000', '000', '124', '064', '157', '235',
    '301'] +
                     ['000'] * 8)
...
def long_double_representation(lines):
    ...
    # the form before_seq + content + after_sequence, where content is the
    long double
    # representation:
    #  - content is 12 bytes: 80 bits Intel representation
    #  - content is 16 bytes: 80 bits Intel representation (64 bits) or quad
    precision
    ...
    for line in lines:
        ...
        for w in line.split()[1:]:
            ...
            if read[-8:] == _AFTER_SEQ:
                ...
                # "before" bytes. In other words the first 4 "before" bytes
    went
...
def check_for_right_shift_internal_compiler_error(cmd):
    ...
    The failure looks like the following, and can be reproduced on ARM64 GCC
    5.4:

        <source>: In function 'right_shift':
        <source>:4:20: internal compiler error: in expand_shift_1, at
    expmed.c:2349
               ip1[i] = ip1[i] >> in2;
                      ^
    ...
    no_err = cmd.try_compile(textwrap.dedent("""\
        ...
        __attribute__((optimize("O3"))) void right_shift(the_type in2,
    the_type *ip1, int n) {
            for (int i = 0; i < n; i++) {
