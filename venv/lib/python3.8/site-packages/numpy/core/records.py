...
class format_parser:
    ...
    def _parseFormats(self, formats, aligned=False):
        ...
        if isinstance(formats, list):
            dtype = sb.dtype(
                [('f{}'.format(i), format_) for i, format_ in
    enumerate(formats)],
                aligned,
            )
        else:
...
class recarray(ndarray):
    ...
    def __setattr__(self, attr, val):
        ...
        if attr == 'dtype' and issubclass(val.type, nt.void) and val.names is
    not None:
...
def fromfile(fd, dtype=None, shape=None, offset=0, formats=None,
    ...
    byteorder).dtype
        ...
        if nbytes > size:
            raise ValueError(
                    "Not enough bytes left in file for specified shape and
    type")
...
def array(obj, dtype=None, shape=None, offset=0, strides=None, formats=None,
    ...
    >>> b = [(1, 1), (2, 4), (3, 9)]
    >>> c = np.core.records.array(b, formats = ['i2', 'f2'], names = ('x',
    'y'))
    >>> c
