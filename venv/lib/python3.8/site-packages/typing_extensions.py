...
if sys.version_info >= (3, 10, 1):
    ...
    def _flatten_literal_params(parameters):
        """An internal helper for Literal creation: flatten Literals among
    parameters"""
        params = []
        for p in parameters:
    ...
    Literal = _LiteralForm(doc="""\
                           ...
                           that the corresponding value has a value literally
    equivalent
    ...
    runtime
                           checking verifying that the parameter is actually a
    value
                           instead of a type.""")
...
if sys.version_info >= (3, 13, 0, "beta"):
    ...
    defaults=()):
            if _special_generic_alias_base is typing._GenericAlias:
                ...
                super().__init__(origin, nparams, special=True, inst=inst,
    name=name)
            else:
                # Python >= 3.9
...
if sys.version_info >= (3, 13):
    ...
    def _allow_reckless_class_checks(depth=3):
        ...
        issubclass() on the whole MRO of a user class, which may contain
    protocols.
        """
    ...
    class _ProtocolMeta(type(typing.Protocol)):
        ...
        # That would call the methods on typing._ProtocolMeta on Python
    3.8-3.11
        # and those are slow
    ...
    typing.Protocol}
                        or base.__name__ in
    _PROTO_ALLOWLIST.get(base.__module__, [])
                        or is_protocol(base)
                    ):
                        raise TypeError(
        ...
        def __subclasscheck__(cls, other):
            ...
            if (
                ...
                if (
                    ...
                    _type_check_issubclass_arg_1(other)
                    non_method_attrs =
    sorted(cls.__non_callable_proto_members__)
                    raise TypeError(
    ...
    with"
            ...
            for attr in cls.__protocol_attrs__:
                try:
                    ...
                    break
                # this attribute is set by @runtime_checkable:
                if val is None and attr not in
    cls.__non_callable_proto_members__:
                    break
            else:
                return True
...
if sys.version_info >= (3, 13):
    ...
    def runtime_checkable(cls):
        ...
        """
        if not issubclass(cls, typing.Generic) or not getattr(cls,
    '_is_protocol', False):
            raise TypeError(f'@runtime_checkable can be only applied to
    protocol classes,'
                            f' got {cls!r}')
        ...
        # `__protocol_attrs__` attribute. Meanwhile, typing.Protocol classes
    on 3.12.2+
        # break if we *don't* execute this block, because *they* assume that
    all
...
if _PEP_728_IMPLEMENTED:
    ...
    class _TypedDictMeta(type):
        ...
        def __new__(cls, name, bases, ns, *, total=True, closed=None,
            ...
            TypedDict supports all three syntax forms described in its
    docstring.
            Subclasses and instances of TypedDict return actual dictionaries.
            """
            for base in bases:
                if type(base) is not _TypedDictMeta and base is not
    typing.Generic:
                    raise TypeError('cannot inherit from both a TypedDict type
    '
                                    'and a non-TypedDict base class')
            if closed is not None and extra_items is not NoExtraItems:
                raise TypeError(f"Cannot combine closed={closed!r} and
    extra_items")
            ...
            # typing.py generally doesn't let you inherit from plain Generic,
    unless
            # the name of the class happens to be "Protocol"
            tp_dict = type.__new__(_TypedDictMeta, "Protocol", (*generic_base,
    dict), ns)
    ...
    type"
        ...
        def __subclasscheck__(cls, other):
            ...
            raise TypeError('TypedDict does not support instance and class
    checks')
    ...
    dict.
        ...
        TypedDict creates a dictionary type such that a type checker will
    expect all
    ...
    A type
        checker is only expected to support a literal False or True as the
    value of
        the total argument. True is the default, and makes all items defined
    in the
    ...
    parameter"
            ...
            if closed is not False and closed is not True and closed is not
    None:
    ...
    arguments,"
        ...
        if kwargs:
            ...
            warnings.warn(
                "The kwargs-based syntax for TypedDict definitions is
    deprecated "
                "in Python 3.11, will be removed in Python 3.13, and may not
    be "
...
if hasattr(typing, "ReadOnly"):  # 3.13+
    ...
    def _clean_optional(obj, hints, globalns=None, localns=None):
        ...
        if not hints or isinstance(obj, type):
            return
        defaults = typing._get_defaults(obj)  # avoid accessing
    __annotations___
        if not defaults:
            return
    ...
    safety
            ...
            original_evaluated = typing._eval_type(original_value, globalns,
    localns)
            if sys.version_info < (3, 9) and get_origin(original_evaluated) is
    Union:
...
# 3.8
    ...
    class _AnnotatedAlias(typing._GenericAlias, _root=True):
        ...
        At its core 'Annotated[t, dec1, dec2, ...]' is an alias for the type
    't'
...
if _PEP_696_IMPLEMENTED:
    ...
    infer_variance.")
        ...
        def __init_subclass__(cls) -> None:
            raise TypeError(f"type '{__name__}.TypeVar' is not an acceptable
    base type")
...
if not hasattr(typing, 'Concatenate'):
    ...
    class _ConcatenateGenericAlias(list):
        ...
        def __parameters__(self):
            return tuple(
                tp for tp in self.__args__ if isinstance(tp, (typing.TypeVar,
    ParamSpec))
            )
        ...
        def copy_with(self, params):
            ...
            elif (not (params[-1] is ... or isinstance(params[-1],
    ParamSpec))):
        ...
        def __getitem__(self, args):
            ...
            for param in params:
                ...
                # 3.8 - 3.9 & typing.ParamSpec
                    ...
                    if (
                        i == len(args)
                        and getattr(param, '__default__', NoDefault) is not
    NoDefault
                    ):
            ...
            if alen != plen:
                raise TypeError(
                    f"Too {'many' if alen > plen else 'few'} arguments for
    {self};"
            ...
            for arg in self.__args__:
                ...
                if isinstance(arg, TypeVar):
                    arg = subst[arg]
                    if (
                        (isinstance(arg, typing._GenericAlias) and
    _is_unpack(arg))
                        or (
...
# 3.10+
    ...
    if sys.version_info < (3, 11):
        ...
        class _ConcatenateGenericAlias(typing._ConcatenateGenericAlias,
    _root=True):
    ...
    ParamSpec)):
                    raise TypeError("The last parameter to Concatenate should
    be a "
                            "ParamSpec variable or ellipsis.")
                return super(typing._ConcatenateGenericAlias,
    self).copy_with(params)

            def __getitem__(self, args):
                value = super().__getitem__(args)
                if isinstance(value, tuple) and any(_is_unpack(t) for t in
    value):
...
# 3.9
    ...
    see
        ...
        """
        item = typing._type_check(parameters, f'{self} accepts only a single
    type.')
...
# 3.9
    ...
    def TypeIs(self, parameters):
        """Special typing form used to annotate the return type of a
    user-defined
        type narrower function.  ``TypeIs`` only accepts a single type
    argument.
        At runtime, functions marked this way should return a boolean.
...
# 3.9
    ...
    def TypeForm(self, parameters):
        """A special form representing the value that results from the
    evaluation
        of a type expression. This value encodes the information supplied in
    the
    ...
    objects.
        It accepts a single type argument, which must be a valid type
    expression.
        ``TypeForm[T]`` describes the set of all type form objects that
    represent
...
# 3.8
    ...
    TypeForm = _TypeFormForm(
        'TypeForm',
        doc="""A special form representing the value that results from the
    evaluation
        of a type expression. This value encodes the information supplied in
    the
        type expression, and it represents the type described by that type
        ...
        def __typing_unpacked_tuple_args__(self):
            ...
            if isinstance(arg, (typing._GenericAlias, _types.GenericAlias)):
                if arg.__origin__ is not tuple:
                    raise TypeError("Unpack[...] must be used with a tuple
    type")
                return arg.__args__
            return None
...
if _PEP_696_IMPLEMENTED:
        ...
        def __new__(cls, name, *, default=NoDefault):
            ...
            def _typevartuple_prepare_subst(alias, args):
                ...
                for k, arg in enumerate(args):
                    if not isinstance(arg, type):
                        subargs = getattr(arg,
    '__typing_unpacked_tuple_args__', None)
    ...
    ...:
                ...
                if var_tuple_index is not None:
                    ...
                    raise TypeError(f"Too few arguments for {alias};"
                                    f" actual {alen}, expected at least {plen
    - 1}")
                ...
                return (
                    ...
                    *([fillarg] * (typevartuple_index - left)),
                    replacement,
                    *([fillarg] * (plen - right - left - typevartuple_index -
    1)),
                    *args[alen - right:],
                )
...
def _has_generic_or_protocol_as_origin() -> bool:
    ...
    # - Catch AttributeError: not all Python implementations have
    sys._getframe()
    ...
    #   and the call stack isn't deep enough
        ...
        # Cannot use "in" because origin may be an object with a buggy __eq__
    that
        # throws an error.
        return origin is typing.Generic or origin is Protocol or origin is
    typing.Protocol
...
if hasattr(typing, '_collect_type_vars'):
    def _collect_type_vars(types, typevar_types=None):
        ...
        # A required TypeVarLike cannot appear after a TypeVarLike with a
    default
        ...
        for t in types:
            if _is_unpacked_typevartuple(t):
                ...
                isinstance(t, typevar_types) and not isinstance(t,
    _UnpackAlias)
                and t not in tvars
            ):
                if enforce_default_ordering:
                    has_default = getattr(t, '__default__', NoDefault) is not
    NoDefault
                    if has_default:
    ...
    class.
                ...
                if t not in parameters:
                    if enforce_default_ordering:
                        has_default = (
                            getattr(t, '__default__', NoDefault) is not
    NoDefault
                        )
            ...
            class Dummy:
                def __init_subclass__(cls):
                    ...
                    raise TypeError(
                        ...
                        f"`{subcls_name} = NewType({subcls_name!r},
    {supercls_name})`"
                    )
...
# 3.8-3.13
    ...
    class TypeAliasType:
        ...
        is equivalent to:
            ...
            T = TypeVar("T")
            ListOrSet = TypeAliasType("ListOrSet", list[T] | set[T],
    type_params=(T,))

        The name ListOrSet can then be used as an alias for the type it refers
    to.
    ...
    invalid,
        ...
        def __init__(self, name: str, value, *, type_params=()):
            ...
            for type_param in type_params:
                if (
                    not isinstance(type_param, (TypeVar, TypeVarTuple,
    ParamSpec))
                    # 3.8-3.11
                    # Unpack Backport passes isinstance(type_param, TypeVar)
                    or _is_unpack(type_param)
                ):
                    raise TypeError(f"Expected a type param, got
    {type_param!r}")
                has_default = (
                    getattr(type_param, '__default__', NoDefault) is not
    NoDefault
                )
                if default_value_encountered and not has_default:
                    raise TypeError(f"non-default type parameter
    '{type_param!r}'"
    ...
    extracted.
        ...
        def __init_subclass__(cls, *args, **kwargs):
            raise TypeError(
                "type 'typing_extensions.TypeAliasType' is not an acceptable
    base type"
            )
...
if hasattr(typing, "Doc"):
    ...
    class Doc:
        """Define the documentation of a type annotation using ``Annotated``,
    to be
         used in class attributes, function and method parameters, return
    values,
         and variables.

        The value should be a positional-only string literal to allow static
    tools
...
if _PEP_649_OR_749_IMPLEMENTED:
    ...
    https://docs.python.org/3/library/inspect.html#inspect.get_annotations
        ...
        * FORWARDREF: return annotations as-is if possible, but replace any
          undefined names with ForwardRef objects. The implementation proposed
    by
          PEP 649 relies on language changes that cannot be backported; the
          typing-extensions implementation simply returns the same result as
    VALUE.
        * STRING: return annotations as strings, in a format close to the
    original
          source. Again, this behavior cannot be replicated directly in a
    backport.
          As an approximation, typing-extensions retrieves the annotations
    under
          VALUE semantics and then stringifies them.

        The purpose of this backport is to allow users who would like to use
        FORWARDREF or STRING semantics once PEP 649 is implemented, but who
    also
    ...
    functools.singledispatch,
        ...
        if not isinstance(ann, dict):
            raise ValueError(f"{obj!r}.__annotations__ is neither a dict nor
    None")
    ...
    typing._type_repr(value)
        ...
        return_value = {key:
            value if not isinstance(value, str) else eval(value, globals,
    locals)
            for key, value in ann.items() }
        return return_value
...
if hasattr(typing, "evaluate_forward_ref"):
    ...
    namespace),
        ...
        if type_params is not None:
            ...
            for param in type_params:
                ...
                if (
                    _FORWARD_REF_HAS_CLASS and not
    ...
    def evaluate_forward_ref(
        ...
        *forward_ref* must be an instance of ForwardRef. *owner*, if given,
        should be the object that holds the annotations that the forward
    reference
        derived from, such as a module, class object, or function. It is used
    to
        infer the namespaces to use for looking up names. *globals* and
    *locals*
        can also be explicitly given to provide the global and local
    namespaces.
        *type_params* is a tuple of type parameters that are in scope when
        evaluating the forward reference. This parameter must be provided
    (though
        it may be an empty tuple) if *owner* is not given and the forward
    reference
        does not already have an owner set. *format* specifies the format of
    the
        ...
        if sys.version_info < (3, 12, 5) and type_params:
            ...
            for tvar in type_params:
                if tvar.__name__ not in locals:  # lets not overwrite
    something present
...
# Explicitly assign these (rather than using `from typing import *` at the
    top),
