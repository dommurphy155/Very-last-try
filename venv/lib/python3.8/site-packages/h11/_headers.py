import re
from typing import AnyStr, cast, List, overload, Sequence, Tuple,
    TYPE_CHECKING, Union
...
class Headers(Sequence[Tuple[bytes, bytes]]):
    ...
    def __getitem__(self, idx: int) -> Tuple[bytes, bytes]:  # type:
    ignore[override]
...
@overload
def normalize_and_validate(headers: Headers, _parsed: Literal[True]) ->
    Headers:
    ...


@overload
def normalize_and_validate(headers: HeaderTypes, _parsed: Literal[False]) ->
    Headers:
    ...
...
def normalize_and_validate(
    ...
    for name, value in headers:
        ...
        if not _parsed:
            ...
            validate(_field_value_re, value, "Illegal header value {!r}",
    value)
...
def set_comma_header(headers: Headers, name: bytes, new_values: List[bytes])
    -> Headers:
    # The header name `name` is expected to be lower-case bytes.
    #
