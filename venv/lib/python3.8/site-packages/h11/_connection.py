...
def _body_framing(
    ...
    if type(event) is Response:
        if (
            ...
            or (request_method == b"CONNECT" and 200 <= event.status_code <
    300)
        ):
...
class Connection:
    ...
    Args:
        ...
        max_incomplete_event_size (int):
            ...
            exceeded, then:meth:`next_event` will raise:exc:`RemoteProtocolError`.
    ...
    def __init__(
        ...
        if our_role not in (CLIENT, SERVER):
            raise ValueError("expected CLIENT or SERVER, not
    {!r}".format(our_role))
    ...
    def their_state(self) -> Type[Sentinel]:
        """The current state of whichever role we are NOT playing. See:ref:`state-machine` for details.
        """
        return self._cstate.states[self.their_role]

    @property
    def they_are_waiting_for_100_continue(self) -> bool:
        return self.their_role is CLIENT and
    ...
    def start_next_cycle(self) -> None:
        ...
        cycle on this same connection. Otherwise, raises a:exc:`LocalProtocolError`.
    ...
    def _process_event(self, role: Type[Sentinel], event: Event) -> None:
        ...
        if role is self.their_role and type(event) in (
            ...
            event = cast(Union[Request, Response, InformationalResponse],
    event)
    ...
    def _get_io_object(
        ...
        if state is SEND_BODY:
            ...
            framing_type, args = _body_framing(
                cast(bytes, self._request_method), cast(Union[Request,
    Response], event)
            )
            return io_dict[SEND_BODY][framing_type](*args)  # type:
    ignore[index]
        else:
    ...
    def receive_data(self, data: bytes) -> None:
        ...
        Args:
            data (:term:`bytes-like object`):
                ...
                standard Python APIs like:meth:`file.read` or:meth:`socket.recv` use ``b""`` to indicate end-of-file, while
                ...
                make sure to check for such strings and avoid passing them to:meth:`receive_data`.
        ...
        if event is None:
            if not self._receive_buffer and self._receive_buffer_closed:
                ...
                if hasattr(self._reader, "read_eof"):
                    event = self._reader.read_eof()  # type:
    ignore[attr-defined]
                else:
    ...
    def next_event(self) -> Union[Event, Type[NEED_DATA], Type[PAUSED]]:
        ...
        Returns:: One of three things:
            ...
            2) The special constant:data:`NEED_DATA`, which indicates that
               you need to read more data from your socket and pass it to:meth:`receive_data` before this method will be able to return
               any more events.

            3) The special constant:data:`PAUSED`, which indicates that we
               are not in a state where we can process incoming data (usually
               because the peer has finished their part of the current
               request/response cycle, and you have not yet called:meth:`start_next_cycle`). See:ref:`flow-control` for details.
        ...
        If this method raises any exception then it also sets:attr:`Connection.their_state` to:data:`ERROR` -- see:ref:`error-handling` for discussion.

        """

        if self.their_state is ERROR:
            raise RemoteProtocolError("Can't receive data when peer state is
    ERROR")
        try:
            ...
            if event is NEED_DATA:
                ...
                if self._receive_buffer_closed:
                    ...
                    raise RemoteProtocolError("peer unexpectedly closed
    connection")
            return event
    ...
    def send(self, event: Event) -> Optional[bytes]:
        ...
        If this method raises any exception then it also sets:attr:`Connection.our_state` to:data:`ERROR` -- see:ref:`error-handling` for discussion.
    ...
    def send_with_data_passthrough(self, event: Event) ->
    Optional[List[bytes]]:
        """Identical to:meth:`send`, except that in situations where:meth:`send` returns a single:term:`bytes-like object`, this instead
        returns a list of them -- and when sending a:class:`Data` event, this
        list is guaranteed to contain the exact object you passed in as:attr:`Data.data`. See:ref:`sendfile` for discussion.
    ...
    def send_failed(self) -> None:
        ...
        This causes:attr:`Connection.our_state` to immediately become:data:`ERROR` -- see:ref:`error-handling` for discussion.
    ...
    # side channels.
    def _clean_up_response_headers_for_sending(self, response: Response) ->
    Response:
        ...
        if framing_type in ("chunked", "http/1.0"):
            ...
            if self.their_http_version is None or self.their_http_version <
    b"1.1":
    ...
    [b"chunked"])
        ...
        if not self._cstate.keep_alive or need_close:
            ...
            headers = set_comma_header(headers, b"connection",
    sorted(connection))
