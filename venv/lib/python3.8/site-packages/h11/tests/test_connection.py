...
from .._connection import _body_framing, _keep_alive, Connection, NEED_DATA,
    PAUSED
from .._events import (
    ConnectionClosed,
    Data,
...
def test__keep_alive() -> None:
    ...
    assert not _keep_alive(
        Request(
            method="GET",
            target="/",
            headers=[("Host", "Example.com"), ("Connection", "a, b, cLOse,
    foo")],
        )
    )
    assert not _keep_alive(
    ...
    assert _keep_alive(Response(status_code=200, headers=[]))  # type:
    ignore[arg-type]
    assert not _keep_alive(Response(status_code=200, headers=[("Connection",
    "close")]))
    assert not _keep_alive(
        Response(status_code=200, headers=[("Connection", "a, b, cLOse, foo")])
    )
    assert not _keep_alive(Response(status_code=200, headers=[],
...
def test__body_framing() -> None:
    ...
    # Content-Length
    for meth, r in [(None, req(cl=100)), (b"GET", resp(cl=100))]:  # type:
    ignore
        assert _body_framing(meth, r) == ("content-length", (100,))

    # No headers
    assert _body_framing(None, req()) == ("content-length", (0,))  # type:
    ignore
...
def test_Connection_basics_and_content_length() -> None:
    ...
    data = p.send(SERVER, Response(status_code=200,
    ...
    assert data == b"12345"
    data = p.send(
        CLIENT, Data(data=b"67890"), expect=[Data(data=b"67890"),
    EndOfMessage()]
    )
    assert data == b"67890"
    ...
    data = p.send(SERVER, Data(data=b"1"), expect=[Data(data=b"1"),
    EndOfMessage()])
    assert data == b"1"
...
def test_chunked() -> None:
    ...
    p.send(
        ...
        Request(
            method="GET",
            target="/",
            headers=[("Host", "example.com"), ("Transfer-Encoding",
    "chunked")],
        ),
    )
    data = p.send(CLIENT, Data(data=b"1234567890", chunk_start=True,
    chunk_end=True))
    assert data == b"a\r\n1234567890\r\n"
    data = p.send(CLIENT, Data(data=b"abcde", chunk_start=True,
    chunk_end=True))
    assert data == b"5\r\nabcde\r\n"
    ...
    p.send(
        SERVER, Response(status_code=200, headers=[("Transfer-Encoding",
    "chunked")])
    )
...
def test_chunk_boundaries() -> None:
    ...
    assert conn.next_event() == Data(data=b"hello", chunk_start=True,
    chunk_end=True)

    conn.receive_data(b"5\r\nhel")
    assert conn.next_event() == Data(data=b"hel", chunk_start=True,
    chunk_end=False)

    conn.receive_data(b"l")
    assert conn.next_event() == Data(data=b"l", chunk_start=False,
    chunk_end=False)

    conn.receive_data(b"o\r\n")
    assert conn.next_event() == Data(data=b"o", chunk_start=False,
    chunk_end=True)
...
def test_client_talking_to_http10_server() -> None:
    c = Connection(CLIENT)
    c.send(Request(method="GET", target="/", headers=[("Host",
    "example.com")]))
    c.send(EndOfMessage())
...
def test_server_talking_to_http10_client() -> None:
    ...
    assert receive_and_get(c, b"234567890") == [Data(data=b"234567890"),
    EndOfMessage()]
    assert c.their_state is MUST_CLOSE
...
def test_automatic_transfer_encoding_in_response() -> None:
    ...
    for user_headers in [
        ...
        p.send(
            CLIENT,
            [
                Request(method="GET", target="/", headers=[("Host",
    "example.com")]),
                EndOfMessage(),
            ],
        )
...
def test_automagic_connection_close_handling() -> None:
    ...
    p.send(
        SERVER,
        # no header here...
        [Response(status_code=204, headers=[]), EndOfMessage()],  # type:
    ignore[arg-type]
...
def test_max_incomplete_event_size_countermeasure() -> None:
    ...
    assert get_all_events(c) == [
        Request(
            method="GET", target="/", http_version="1.0", headers=[("big", "a"
    * 4000)]
        ),
        EndOfMessage(),
    ]
...
def test_reuse_simple() -> None:
    ...
    p.send(
        SERVER,
        [
            Response(status_code=200, headers=[(b"transfer-encoding",
    b"chunked")]),
            EndOfMessage(),
        ],
    )
    ...
    p.send(
        SERVER,
        [
            Response(status_code=404, headers=[(b"transfer-encoding",
    b"chunked")]),
            EndOfMessage(),
        ],
    )
...
def test_protocol_switch() -> None:
    ...
    b"chunked")]),
        ...
        (
            Request(
                method="GET",
                target="/",
                headers=[("Host", "foo"), ("Content-Length", "1"), ("Upgrade",
    "a, b")],
            ),
            Response(status_code=200, headers=[(b"transfer-encoding",
    b"chunked")]),
    ...
    b"chunked")]),
        ...
        for conn in p.conns:
            assert conn.states == {CLIENT: SWITCHED_PROTOCOL, SERVER:
    SWITCHED_PROTOCOL}
...
def test_close_simple() -> None:
    ...
    # happened yet.
    for (who_shot_first, who_shot_second) in [(CLIENT, SERVER), (SERVER,
    CLIENT)]:
...
def test_close_different_states() -> None:
    ...
    p.send(
        CLIENT,
        Request(
            method="GET", target="/", headers=[("Host", "a"),
    ("Content-Length", "10")]
        ),
    )
    with pytest.raises(LocalProtocolError):
...
def test_errors() -> None:
    ...
    for role in [CLIENT, SERVER]:
        ...
        if role is SERVER:
            assert (
                c.send(Response(status_code=400, headers=[]))  # type:
    ignore[arg-type]
                == b"HTTP/1.1 400 \r\nConnection: close\r\n\r\n"
            )
    ...
    for role in [CLIENT, SERVER]:
        if role is CLIENT:
            ...
            good = Request(method="GET", target="/", headers=[("Host",
    "example.com")])
            bad = Request(
                method="GET",
                target="/",
            ...
            good = Response(status_code=200, headers=[])  # type:
...
def test_408_request_timeout() -> None:
    ...
    p = ConnectionPair()
    p.send(SERVER, Response(status_code=408, headers=[(b"connection",
    b"close")]))
