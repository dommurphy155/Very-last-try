...
SIMPLE_CASES = [
    ...
    (
        (SERVER, SEND_RESPONSE),
        Response(status_code=200, headers=[], reason=b"OK"),  # type:
    ignore[arg-type]
        b"HTTP/1.1 200 OK\r\n\r\n",
    ),
    (
...
def test_readers_unusual() -> None:
    ...
    tr(
        ...
        b"HTTP/1.0 200 OK\r\n" b"Foo:\r\n\r\n",
        Response(
            status_code=200, headers=[("Foo", "")], http_version="1.0",
    reason=b"OK"
        ),
    )
    ...
    tr(
        ...
        b"HTTP/1.0 200\r\n" b"Foo: bar\r\n\r\n",
        Response(
            status_code=200, headers=[("Foo", "bar")], http_version="1.0",
    reason=b""
        ),
    )
...
def t_body_reader(thunk: Any, data: bytes, expected: Any, do_eof: bool =
    False) -> None:
    # Simple: consume whole thing
    print("Test 1")
    buf = makebuf(data)
...
def test_Http10Reader() -> None:
    ...
    t_body_reader(
        Http10Reader, b"asdf", [Data(data=b"asdf"), EndOfMessage()],
    do_eof=True
    )
...
def test_allow_some_garbage_in_cookies() -> None:
    tr(
        ...
        Request(
            ...
            headers=[
                ("Host", "foo"),
                ("Set-Cookie", "___utmvafIumyLc=kUd\x01UpAt; path=/;
    Max-Age=900"),
            ],
        ),
    )
