...
class CancelScope(BaseCancelScope):
    ...
    def __init__(self, original: trio.CancelScope | None = None, **kwargs:
    Any) -> None:
...
class TaskGroup(abc.TaskGroup):
    ...
    async def __aexit__(
        ...
        try:
            return await self._nursery_manager.__aexit__(exc_type, exc_val,
    exc_tb)
...
class UNIXSocketStream(SocketStream, abc.UNIXSocketStream):
    ...
    list[int]]:
        ...
        with self._receive_guard:
            while True:
                try:
                    message, ancdata, flags, addr = await
    self._trio_socket.recvmsg(
...
class Lock(BaseLock):
    ...
    def statistics(self) -> LockStatistics:
        orig_statistics = self.__original.statistics()
        owner = TrioTaskInfo(orig_statistics.owner) if orig_statistics.owner
    else None
...
_capacity_limiter_wrapper: trio.lowlevel.RunVar =
    RunVar("_capacity_limiter_wrapper")
...
class TestRunner(abc.TestRunner):
    ...
    def run_asyncgen_fixture(
        ...
        fixturevalue: T_Retval = self._call_in_runner_task(asyncgen.asend,
    None)
...
class TrioTaskInfo(TaskInfo):
    ...
    def has_pending_cancellation(self) -> bool:
        try:
            ...
            # If the task is no longer around, it surely doesn't have a
    cancellation
            # pending
            return False
...
class TrioBackend(AsyncBackend):
    ...
    async def open_process(
        ...
        **kwargs: Any,
        ...
        stdin_stream = SendStreamWrapper(process.stdin) if process.stdin else
    None
        stdout_stream = ReceiveStreamWrapper(process.stdout) if process.stdout
    else None
        stderr_stream = ReceiveStreamWrapper(process.stderr) if process.stderr
    else None
    ...
    async def wait_socket_readable(cls, sock: socket.socket) -> None:
        try:
            ...
            raise ClosedResourceError().with_traceback(exc.__traceback__) from
    None
