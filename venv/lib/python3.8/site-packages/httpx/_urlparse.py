...
* It properly differentiates between an empty querystring and an absent
    querystring,
  to distinguish URLs with a trailing '?'.
* It handles scheme, hostname, port, and path normalization.
* It supports IDNA hostnames, normalizing them to their encoded form.
* The API supports passing individual components, as well as the complete URL
    string.

Previously we relied on the excellent `rfc3986` package to handle URL parsing
    and
validation, but this module provides a simpler alternative, with less
    indirection
required.
"""
import ipaddress
...
AUTHORITY_REGEX = re.compile(
    (
        r"(?:(?P<userinfo>{userinfo})@)?" r"(?P<host>{host})"
    r":?(?P<port>{port})?"
    ).format(
        userinfo="[^@]*",  # Any character sequence not including '@'.
        host="(\\[.*\\]|[^:]*)",  # Either any character sequence not
    including ':',
...
def urlparse(url: str = "", **kwargs: typing.Optional[str]) -> ParseResult:
    ...
    for key, value in kwargs.items():
        if value is not None:
            ...
            # If a component includes any ASCII control characters including
    \t, \r, \n,
            # then treat it as invalid.
            if any(char.isascii() and not char.isprintable() for char in
    value):
                raise InvalidURL(
...
def validate_path(path: str, has_scheme: bool, has_authority: bool) -> None:
    """
    Path validation rules that depend on if the URL contains a scheme or
    authority component.
    ...
    if has_authority:
        ...
        if path and not path.startswith("/"):
            raise InvalidURL("For absolute URLs, path must be empty or begin
    with '/'")
    else:
    ...
    cannot begin
        ...
        if path.startswith("//"):
            raise InvalidURL(
                "URLs with no authority component cannot have a path starting
    with '//'"
            )
    ...
    reference, in which
        ...
        if path.startswith(":") and not has_scheme:
            raise InvalidURL(
                "URLs with no scheme component cannot have a path starting
    with ':'"
            )
...
def urlencode(items: typing.List[typing.Tuple[str, str]]) -> str:
    ...
    # we don't need to handle a bunch of different typing cases, such as bytes
    vs str.
    #
    #
