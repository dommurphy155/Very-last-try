...
class Headers(typing.MutableMapping[str, str]):
    ...
    def keys(self) -> typing.KeysView[str]:
        return {key.decode(self.encoding): None for _, key, value in
    self._list}.keys()
    ...
    def get_list(self, key: str, split_commas: bool = False) ->
    typing.List[str]:
        """
...
class Request:
    ...
    explicitly.
            if key.lower() == "transfer-encoding" and "Content-Length" in
    self.headers:
                continue
        ...
        has_content_length = (
            "Content-Length" in self.headers or "Transfer-Encoding" in
    self.headers
        )
    ...
    def read(self) -> bytes:
        ...
        if not hasattr(self, "_content"):
            ...
            if not isinstance(self.stream, ByteStream):
                # If a streaming request has been read entirely into memory,
    then
...
class Response:
    ...
    def text(self) -> str:
        if not hasattr(self, "_text"):
            ...
            if not content:
                ...
                self._text = "".join([decoder.decode(self.content),
    decoder.flush()])
        return self._text
    ...
    def encoding(self) -> typing.Optional[str]:
        ...
        * The encoding as specified by the charset parameter in the
    Content-Type header.
        * The encoding as determined by `default_encoding`, which may either be
          a string like "utf-8" indicating the encoding to use, or may be a
    callable
    ...
    def _get_content_decoder(self) -> ContentDecoder:
        ...
        if not hasattr(self, "_decoder"):
            ...
            values = self.headers.get_list("content-encoding",
    split_commas=True)
            for value in values:
    ...
    def is_redirect(self) -> bool:
        ...
        Note that not all responses with a 3xx status code indicate a URL
    redirect.

        Use `response.has_redirect_location` to determine responses with a
    properly
    ...
    def is_error(self) -> bool:
        """
        A property which is `True` for 4xx and 5xx status codes, `False`
    otherwise.
        """
    ...
    def json(self, **kwargs: typing.Any) -> typing.Any:
        if self.charset_encoding is None and self.content and
    len(self.content) > 3:
    ...
    @property
    def links(self) -> typing.Dict[typing.Optional[str], typing.Dict[str,
    str]]:
        """
    ...
    def iter_bytes(
        ...
        if hasattr(self, "_content"):
            chunk_size = len(self._content) if chunk_size is None else
    chunk_size
    ...
    def iter_raw(
        ...
        if not isinstance(self.stream, SyncByteStream):
            raise RuntimeError("Attempted to call a sync iterator on an async
    stream.")
    ...
    def close(self) -> None:
        ...
        """
        if not isinstance(self.stream, SyncByteStream):
            raise RuntimeError("Attempted to call an sync close on an async
    stream.")
    ...
    async def aiter_raw(
        ...
        if not isinstance(self.stream, AsyncByteStream):
            raise RuntimeError("Attempted to call an async iterator on an sync
    stream.")
    ...
    async def aclose(self) -> None:
        ...
        """
        if not isinstance(self.stream, AsyncByteStream):
            raise RuntimeError("Attempted to call an async close on an sync
    stream.")
...
class Cookies(typing.MutableMapping[str, str]):
                ...
                if domain is None or cookie.domain == domain:
                    if path is None or cookie.path == path:
                        if value is not None:
                            message = f"Multiple cookies exist with
    name={name}"
    ...
    def clear(
        self, domain: typing.Optional[str] = None, path: typing.Optional[str]
    = None
    ) -> None:
        """
