...
def _skip_leading_empty_chunks(body: typing.Iterable[_T]) ->
    typing.Iterable[_T]:
    body = iter(body)
    for chunk in body:
        if chunk:
...
class WSGITransport(BaseTransport):
    ...
    * `script_name` - The root path on which the WSGI application should be
    mounted.
    * `remote_addr` - A string indicating the client IP of incoming requests.
    ...
    def handle_request(self, request: Request) -> Response:
        ...
        port = request.url.port or {"http": 80, "https":
    443}[request.url.scheme]
        environ = {
            "wsgi.version": (1, 0),
