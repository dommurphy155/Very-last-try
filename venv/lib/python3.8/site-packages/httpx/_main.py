...
def print_help() -> None:
    ...
    table.add_row(
        ...
        "Request method, such as GET, POST, PUT, PATCH, DELETE, OPTIONS,
    HEAD.\n"
    ...
    table.add_row(
        "-c, --content [cyan]TEXT", "Byte content to include in the request
    body."
    )
    table.add_row(
        "-d, --data [cyan]<NAME VALUE> ...", "Form data to include in the
    request body."
    )
    table.add_row(
    ...
    table.add_row("-j, --json [cyan]TEXT", "JSON data to include in the
    request body.")
    table.add_row(
    ...
    table.add_row(
        "--cookies [cyan]<NAME VALUE> ...", "Cookies to include in the
    request."
    )
    table.add_row(
        "--auth [cyan]<USER PASS>",
        "Username and password to include in the request. Specify '-' for the
    password to use "
        "a password prompt. Note that using --verbose/-v will expose the
    Authorization "
        "header, including the password encoding in a trivially reversible
    format.",
    )

    table.add_row(
        "--proxies [cyan]URL",
        "Send the request via a proxy. Should be the URL giving the proxy
    address.",
    )
    ...
    table.add_row(
        "--http2", "Send the request using HTTP/2, if the remote server
    supports it."
    )
    ...
    table.add_row("-v, --verbose", "Verbose output. Show request as well as
    response.")
...
def format_request_headers(request: httpcore.Request, http2: bool = False) ->
    str:
    version = "HTTP/2" if http2 else "HTTP/1.1"
    headers = [
        (name.lower() if http2 else name, value) for name, value in
    request.headers
    ]
...
def print_request_headers(request: httpcore.Request, http2: bool = False) ->
    None:
    console = rich.console.Console()
...
def trace(
    ...
    if name == "connection.connect_tcp.started" and verbose:
        ...
        console.print(f"* Connected to {server_addr[0]!r} on port
    {server_addr[1]}")
    elif name == "connection.start_tls.complete" and verbose:  # pragma: no
    cover
        stream = info["return_value"]
    ...
    elif name == "http2.send_request_headers.started" and verbose:  # pragma:
    no cover
        request = info["request"]
...
@click.option(
    ...
    type=str,
    default=None,
    help="Send the request via a proxy. Should be the URL giving the proxy
    address.",
)
@click.option(
    "--timeout",
    ...
    try:
        with Client(
            ...
            with client.stream(
                ...
                extensions={"trace": functools.partial(trace,
    verbose=verbose)},
            ) as response:
