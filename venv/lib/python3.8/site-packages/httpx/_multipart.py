...
class DataField:
    ...
    def __init__(
        ...
        if not isinstance(name, str):
            raise TypeError(
                f"Invalid type for name. Expected str, got {type(name)}:
    {name!r}"
            )
        if value is not None and not isinstance(value, (str, bytes, int,
    float)):
            raise TypeError(
                f"Invalid type for value. Expected primitive type, got
    {type(value)}: {value!r}"
            )
        self.name = name
...
class FileField:
    ...
    (headers) was included
                ...
                filename, fileobj, content_type, headers = value  # type:
    ignore
        else:
        ...
        has_content_type_header = any("content-type" in key.lower() for key in
    headers)
        if content_type is not None and not has_content_type_header:
            # note that unlike requests, we ignore the content_type
            # provided in the 3rd tuple element if it is also included in the
    headers
...
class MultipartStream(SyncByteStream, AsyncByteStream):
    ...
    def __init__(
        ...
        self.content_type = "multipart/form-data; boundary=%s" %
    boundary.decode(
            "ascii"
        )
    ...
    def _iter_fields(
        ...
        file_items = files.items() if isinstance(files, typing.Mapping) else
    files
    ...
    def get_headers(self) -> typing.Dict[str, str]:
        ...
        if content_length is None:
            return {"Transfer-Encoding": "chunked", "Content-Type":
    content_type}
        return {"Content-Length": str(content_length), "Content-Type":
    content_type}
