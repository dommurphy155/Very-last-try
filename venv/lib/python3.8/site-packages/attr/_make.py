...
def _transform_attrs(
    ...
    for a in (a for a in attrs if a.init is not False and a.kw_only is False):
        if had_default is True and a.default is NOTHING:
            msg = f"No mandatory attributes allowed after an attribute with a
...
def _make_cached_property_getattr(cached_properties, original_getattr, cls):
    ...
    construction).
        "def wrapper(_cls):",
        "    __class__ = _cls",
        "    def __getattr__(self, item, cached_properties=cached_properties,
    original_getattr=original_getattr, _cached_setattr_get=_cached_setattr_get):",
    ...
    if original_getattr is not None:
        ...
        lines.extend(
            [
                ...
                "         original_error = f\"'{self.__class__.__name__}'
...
class _ClassBuilder:
    ...
    def _add_method_dunders_safe(self, method: Callable) -> Callable:
        ...
        with contextlib.suppress(AttributeError):
            method.__doc__ = f"Method generated by attrs for class
    {self._cls.__qualname__}."
...
def attrs(
    ...
    *never* go away, though).
    ...
    .. caution::
        Refer to `attrs.define` for the rest of the parameters, but note that
    they
    ...
    if repr_ns is not None:
        ...
        warnings.warn(
            DeprecationWarning(
                "The `repr_ns` argument is deprecated and will be removed in
    or after August 2025."
            ),
            stacklevel=2,
        )
    ...
    def wrap(cls):
        ...
        if hash is False or (hash is None and eq is False) or is_exc:
            ...
            # which is by id.
            if cache_hash:
                msg = "Invalid value for cache_hash.  To use hash caching,
    hashing must be either explicitly or implicitly enabled."
                raise TypeError(msg)
    ...
    if attrs:
        ...
        for a in attrs:
            if a.eq_key:
                ...
                globs[cmp_name] = a.eq_key
                lines.append(
                    f"        {cmp_name}(self.{a.name}) ==
    {cmp_name}(other.{a.name})"
                )
            else:
...
def _setattr_with_converter(
    ...
    its converter first.
    """
    return f"_setattr('{attr_name}', {converter._fmt_converter_call(attr_name,
    value_var)})"
...
def _assign_with_converter(
    ...
    return f"self.{attr_name} = {converter._fmt_converter_call(attr_name,
    value_var)}"
...
class Attribute:
    ...
    *Read-only* representation of an attribute.
    ...
    - ``eq_key`` and ``order_key`` (`typing.Callable` or `None`): The
      ...
      `attr.ib`'s ``eq``, ``order``, or ``cmp`` arguments. See also:ref:`comparison customization <custom-comparison>`.
    ...
    def from_counting_attr(cls, name: str, ca: _CountingAttr, type=None):
        ...
        if type is None:
            ...
            msg = f"Type annotation and type argument cannot both be present
    for '{name}'."
            raise ValueError(msg)
        return cls(
            name,
