...
def merge_headers(
    ...
    override_headers = [] if override_headers is None else
    list(override_headers)
...
class AsyncHTTPProxy(AsyncConnectionPool):
    ...
    by
            ...
            local_address: Local address to connect from. Can also be used to
                ...
                `local_address="0.0.0.0"` will connect using an `AF_INET`
    address
...
class AsyncTunnelHTTPConnection(AsyncConnectionInterface):
    ...
    async def handle_async_request(self, request: Request) -> Response:
        ...
        async with self._connect_lock:
            if not self._connected:
                target = b"%b:%d" % (self._remote_origin.host,
    ...
    self._proxy_headers
                ...
                if connect_response.status < 200 or connect_response.status >
    299:
                    reason_bytes =
    connect_response.extensions.get("reason_phrase", b"")
    ...
    self._remote_origin.host.decode("ascii"),
                ...
                async with Trace("start_tls", logger, request, kwargs) as
    trace:
