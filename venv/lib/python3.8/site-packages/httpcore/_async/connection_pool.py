...
class AsyncConnectionPool(AsyncRequestInterface):
    ...
    available.
            max_keepalive_connections: The maximum number of idle HTTP
    connections
                that will be maintained in the pool.
            keepalive_expiry: The duration in seconds that an idle HTTP
    connection
                may be maintained for before being expired from the pool.
            http1: A boolean indicating if HTTP/1.1 requests should be
    supported
                by the connection pool. Defaults to True.
            http2: A boolean indicating if HTTP/2 requests should be supported
    by
    ...
    (IPv6).
            ...
            network_backend: A backend instance to use for handling network
    I/O.
    ...
    async def _attempt_to_acquire_connection(self, status: RequestStatus) ->
    bool:
        """
        ...
        for idx, connection in enumerate(self._pool):
            if connection.can_handle_request(origin) and
    ...
    async def _close_expired_connections(self) -> None:
        """
        Clean up the connection pool by closing off any connections that have
    expired.
        """
        ...
        # If the pool size exceeds the maximum number of allowed keep-alive
    connections,
        # then close off idle connections as required.
        pool_size = len(self._pool)
        for idx, connection in reversed(list(enumerate(self._pool))):
            if connection.is_idle() and pool_size >
    self._max_keepalive_connections:
                await connection.aclose()
                self._pool.pop(idx)
                pool_size -= 1
    ...
    async def handle_async_request(self, request: Request) -> Response:
        ...
        This is the core implementation that is called into by `.request()` or
    `.stream()`.
        """
    ...
    async def response_closed(self, status: RequestStatus) -> None:
        """
        This method acts as a callback once the request/response cycle is
    complete.
        ...
        async with self._pool_lock:
            ...
            # Since we've had a response closed, it's possible we'll now be
    able
