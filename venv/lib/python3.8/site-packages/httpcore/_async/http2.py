...
class AsyncHTTP2Connection(AsyncConnectionInterface):
    ...
    async def handle_async_request(self, request: Request) -> Response:
        if not self.can_handle_request(request.url.origin):
            ...
            raise RuntimeError(
                f"Attempted to send request to {request.url.origin} on
    connection "
                f"to {self._origin}"
            )
        ...
        async with self._init_lock:
            if not self._sent_connection_init:
                try:
                    ...
                    async with Trace("send_connection_init", logger, request,
    kwargs):
    ...
    AsyncSemaphore(local_settings_max_streams)
        ...
        try:
            ...
            async with Trace("send_request_headers", logger, request, kwargs):
                await self._send_request_headers(request=request,
    stream_id=stream_id)
            async with Trace("send_request_body", logger, request, kwargs):
                await self._send_request_body(request=request,
    stream_id=stream_id)
            async with Trace(
            ...
            return Response(
                status=status,
                headers=headers,
                content=HTTP2ConnectionByteStream(self, request,
    stream_id=stream_id),
                extensions={
            ...
            if isinstance(exc, h2.exceptions.ProtocolError):
                ...
                # a protocol error at any point they interact with the
    'h2_state'.
                #
    ...
    async def _send_connection_init(self, request: Request) -> None:
        ...
        self._h2_state.local_settings = h2.settings.Settings(
            client=True,
            initial_values={
                # Disable PUSH_PROMISE frames from the server since we don't
    do anything
        ...
        # present in the initial handshake since it's not defined in the
    original
    ...
    async def _send_request_headers(self, request: Request, stream_id: int) ->
    None:
        """
    ...
    async def _send_end_stream(self, request: Request, stream_id: int) -> None:
        """
        Send an empty data frame on on a given stream ID with the END_STREAM
    flag set.
        """
    ...
    async def _receive_response_body(
        ...
        """
        Iterator that returns the bytes of the response body for a given
    stream ID.
        """
        while True:
            event = await self._receive_stream_event(request, stream_id)
    ...
    async def _receive_events(
        ...
        async with self._read_lock:
            ...
            # This conditional is a bit icky. We don't want to block reading
    if we've
            # actually got an event to return for a given stream. We need to
    do that
            # check *within* the atomic read lock. Though it also need to be
    optional,
            # because when we call it from `_wait_for_outgoing_flow` we *do*
    want to
            # block until we've available flow control, event when we have
    events
    ...
    async def _response_closed(self, stream_id: int) -> None:
        ...
        async with self._state_lock:
            ...
            elif self._state == HTTPConnectionState.ACTIVE and not
    self._events:
                self._state = HTTPConnectionState.IDLE
    ...
    future reads.
            #    (For example, this means that a single read timeout or
    disconnect will
            #    immediately close all pending streams. Without requiring
    multiple
            #    sequential timeouts.)
            # 2. Mark the connection as errored, so that we don't accept any
    other
            #    incoming requests.
        ...
        events: typing.List[h2.events.Event] =
    self._h2_state.receive_data(data)
    ...
    async def _write_outgoing_data(self, request: Request) -> None:
        ...
        async with self._write_lock:
            ...
            try:
                ...
                # 1. Save the exception and just raise it immediately on any
    future write.
                #    (For example, this means that a single write timeout or
    disconnect will
                #    immediately close all pending streams. Without requiring
    multiple
                #    sequential timeouts.)
                # 2. Mark the connection as errored, so that we don't accept
    any other
                #    incoming requests.
...
class HTTP2ConnectionByteStream:
    def __init__(
        self, connection: AsyncHTTP2Connection, request: Request, stream_id:
    int
    ) -> None:
    ...
    async def __aiter__(self) -> typing.AsyncIterator[bytes]:
        ...
        try:
            async with Trace("receive_response_body", logger, self._request,
    kwargs):
