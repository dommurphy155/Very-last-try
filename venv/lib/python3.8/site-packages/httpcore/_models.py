...
def enforce_bytes(value: Union[bytes, str], *, name: str) -> bytes:
    ...
    However we enforce that any string arguments must only contain characters
    in
    ...
    if isinstance(value, str):
        try:
            ...
            raise TypeError(f"{name} strings may not include unicode
    characters.")
...
def enforce_url(value: Union["URL", bytes, str], *, name: str) -> "URL":
    ...
    seen_type = type(value).__name__
    raise TypeError(f"{name} must be a URL, bytes, or str, but got
    {seen_type}.")


def enforce_headers(
    value: Union[HeadersAsMapping, HeadersAsSequence, None] = None, *, name:
    str
    ...
    seen_type = type(value).__name__
    raise TypeError(
        f"{name} must be a mapping or sequence of two-tuples, but got
    {seen_type}."
    )


def enforce_stream(
    value: Union[bytes, Iterable[bytes], AsyncIterable[bytes], None], *, name:
    str
...
class ByteStream:
    """
    A container for non-streaming content, and that supports both sync and
    async
    stream iteration.
    """
...
class URL:
    ...
    ```python
    url = httpcore.URL(scheme=b'https', host=b'www.example.com', port=None,
    target=b'/')
    ```

    Using this second more explicit style allows integrations that are using
    `httpcore` to pass through URLs that have already been parsed in order to
    use
    libraries such as `rfc-3986` rather than relying on the stdlib. It also
    ensures
    that URL parsing is treated identically at both the networking level and
    at any
    higher layers of abstraction.

    The four components are important here, as they allow the URL to be
    precisely
    specified in a pre-parsed format. They also allow certain types of request
    to
    be created that could not otherwise be expressed.

    For example, an HTTP request to `http://www.example.com/` forwarded via a
    proxy
    ...
    For convenience, string-like arguments may be specified either as strings
    or
    ...
    In order to avoid any ambiguity over character encodings, when strings are
    used
    as arguments, they must be strictly limited to the ASCII range
    `chr(0)`-`chr(127)`.
    If you require a bytewise representation that is outside this range you
    must
    ...
    `"www.example.com"`.
            ...
            target: The target of the HTTP request. Such as
    `"/items?search=red"`.
        """
        if url:
    ...
    def __bytes__(self) -> bytes:
        ...
        return b"%b://%b:%d%b" % (self.scheme, self.host, self.port,
    self.target)
...
class Request:
    ...
    or bytes.
        ...
        self.stream: Union[Iterable[bytes], AsyncIterable[bytes]] =
    enforce_stream(
...
class Response:
    ...
    def content(self) -> bytes:
        if not hasattr(self, "_content"):
            if isinstance(self.stream, Iterable):
                raise RuntimeError(
                    "Attempted to access 'response.content' on a streaming
    response. "
    ...
    def iter_stream(self) -> Iterator[bytes]:
        if not isinstance(self.stream, Iterable):  # pragma: nocover
            raise RuntimeError(
                "Attempted to stream an asynchronous response using 'for ...
    in "
                "response.iter_stream()'. "
                "You should use 'async for ... in response.aiter_stream()'
    instead."
            )
        if self._stream_consumed:
            raise RuntimeError(
                "Attempted to call 'for ... in response.iter_stream()' more
    than once."
            )
    ...
    def close(self) -> None:
        if not isinstance(self.stream, Iterable):  # pragma: nocover
            raise RuntimeError(
                "Attempted to close an asynchronous response using
    'response.close()'. "
    ...
    async def aread(self) -> bytes:
        ...
        if not hasattr(self, "_content"):
            self._content = b"".join([part async for part in
    self.aiter_stream()])
        return self._content
    ...
    async def aiter_stream(self) -> AsyncIterator[bytes]:
        if not isinstance(self.stream, AsyncIterable):  # pragma: nocover
            raise RuntimeError(
                "Attempted to stream an synchronous response using 'async for
    ... in "
