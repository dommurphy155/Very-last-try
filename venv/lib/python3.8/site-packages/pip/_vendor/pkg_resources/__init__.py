...
# For now we'd simply use implicit Any/Unknown which would add redundant
    annotations
...
This module is deprecated. Users are directed to:mod:`importlib.resources`,:mod:`importlib.metadata` and:pypi:`packaging` instead.
"""
...
_AdapterT = TypeVar(
    "_AdapterT", _DistFinderType[Any], _ProviderFactoryType,
    _NSHandlerType[Any]
)
...
def get_provider(moduleOrReq: str | Requirement) -> IResourceProvider |
    Distribution:
...
class WorkingSet:
    ...
    def resolve(
        ...
        requirements: Iterable[Requirement],
        env: Environment | None = None,
        installer: _InstallerType | None | _InstallerTypeT[_DistributionT] =
    None,
        replace_conflicting: bool = False,
        extras: tuple[str, ...] | None = None,
...
class _ReqExtras(Dict["Requirement", Tuple[str, ...]]):
    ...
    def markers_pass(self, req: Requirement, extras: tuple[str, ...] | None =
    None):
        """
...
class NullProvider:
    ...
    def metadata_isdir(self, name: str) -> bool:
        return bool(self.egg_info and self._isdir(self._fn(self.egg_info,
    name)))
    ...
    method or check the parameter first."
            )
...
is not allowed.
        ...
        if (path.startswith("\\") or ntpath.isabs(path)) and not
...
class ZipManifests(Dict[str, "MemoizedZipManifests.manifest_mod"]):
    ...
    # `path` could be `StrPath | IO[bytes]` but that violates the LSP for
...
class MemoizedZipManifests(ZipManifests):
    ...
    def load(self, path: str) -> dict[str, zipfile.ZipInfo]:  # type:
    ignore[override] # ZipManifests.load is a classmethod
        """
...
class ZipProvider(EggProvider):
    ...
    noqa: C901
        if zip_path in self._index():
            for name in self._index()[zip_path]:
                last = self._extract_resource(manager, os.path.join(zip_path,
    name))
    ...
    platform'
        ...
        try:
            if not self.egg_name:
                raise OSError(
                    '"egg_name" is empty. This likely means no egg could be
    found from the "module_path".'
                )
...
def register_finder(importer_type: type[_T], distribution_finder:
    _DistFinderType[_T]):
...
class Distribution:
    ...
    def from_location(
        ...
        basename: StrPath,
        metadata: _MetadataType = None,
        **kw: int,  # We could set `precedence` explicitly, but keeping this
    as `**kw` for full backwards and subclassing compatibility
    ) -> Distribution:
        ...
        if not hasattr(self, "_parsed_version"):
            try:
                ...
                if hasattr(ex, "add_note"):
                    ...
                    raise
                raise _packaging_version.InvalidVersion(f"{str(ex)} {info}")
    from None
    ...
    **kw
    ...
    @overload
    def get_entry_map(self, group: None = None) -> dict[str, dict[str,
    EntryPoint]]: ...
    @overload
...
def _read_utf8_with_fallback(file: str, fallback_encoding=_LOCALE_ENCODING) ->
    str:
    ...
    ********************************************************************************
        `encoding="utf-8"` fails with {file!r}, trying
    `encoding={fallback_encoding!r}`.

        This fallback behaviour is considered **deprecated** and future
    versions of
        `setuptools/pkg_resources` may not implement it.

        Please encode {file!r} with "utf-8" to ensure future builds will
    succeed.

        If this file was produced by `setuptools` itself, cleaning up the
    cached files
        and re-building/re-installing the package with a newer version of
    `setuptools`
