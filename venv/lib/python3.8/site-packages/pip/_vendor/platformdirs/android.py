...
class Android(PlatformDirsABC):
    ...
    `version
    <platformdirs.api.PlatformDirsABC.version>`, `ensure_exists
    ...
    e.g.,``/data/user/<userid>/<packagename>/cache/<AppName>``"""
        return self._append_app_name_and_version(cast(str, _android_folder()),
    "cache")

    @property
    def site_cache_dir(self) -> str:
        """:return: cache directory shared by users, same as
    `user_cache_dir`"""
    ...
    def user_log_dir(self) -> str:
        """:return: log directory tied to the user, same as `user_cache_dir` if
    not opinionated else ``log`` in it,
...
def _android_folder() -> str | None:  # noqa: C901
    ...
    # type checker isn't happy with our "import android", just don't do this
    when type checking see
    ...
    mActivity.getApplicationContext())  # noqa: F821
    ...
    available or doesn't deliver any useful
            # result...
            ...
            result = None
    if result is None:
        # and if that fails, too, find an android folder looking at path on
    the sys.path
    ...
    taking adopted storage paths into
        # account
        pattern =
    re.compile(r"/mnt/expand/[a-fA-F0-9-]{36}/(data|user/\d+)/(.+)/files")
        for path in sys.path:
            if pattern.match(path):
                result = path.split("/files")[0]
                break
