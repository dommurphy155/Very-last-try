...
class Color(NamedTuple):
    ...
    to use default. Defaults to None.
            foreground (bool, optional): True for a foreground color, or False
    for background. Defaults to True.
        ...
        if self.type == ColorType.TRUECOLOR:
            ...
            return theme.foreground_color if foreground else
    ...
    def from_triplet(cls, triplet: "ColorTriplet") -> "Color":
        ...
        Args:
            triplet (ColorTriplet): A color triplet containing red, green and
    blue components.
    ...
    def parse(cls, color: str) -> "Color":
        ...
        if color_number is not None:
            return cls(
                color,
                type=(ColorType.STANDARD if color_number < 16 else
    ColorType.EIGHT_BIT),
                number=color_number,
            )
    ...
    int(color_24[4:6], 16)
            ...
            number = int(color_8)
            if number > 255:
                raise ColorParseError(f"color number must be <= 255 in
    {color!r}")
            return cls(
                color,
                type=(ColorType.STANDARD if number < 16 else
    ColorType.EIGHT_BIT),
                number=number,
            )
    ...
    def downgrade(self, system: ColorSystem) -> "Color":
        ...
        if system == ColorSystem.EIGHT_BIT and self.system ==
    ColorSystem.TRUECOLOR:
            assert self.triplet is not None
    ...
    number=color_number)
            ...
            color_number = (
                16 + 36 * round(six_red) + 6 * round(six_green) +
    round(six_blue)
            )
