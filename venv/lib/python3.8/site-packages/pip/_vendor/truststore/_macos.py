...
if _mac_version_info < (10, 8):
    ...
    {_mac_version_info[0]}.{_mac_version_info[1]}"
    )
...
try:
    ...
    Security.SecTrustResultType = SecTrustResultType  # type:
    ignore[attr-defined]
    ...
    CoreFoundation.kCFAllocatorDefault = CFAllocatorRef.in_dll(  # type:
    ignore[attr-defined]
        CoreFoundation, "kCFAllocatorDefault"
    )
    CoreFoundation.kCFTypeArrayCallBacks = c_void_p.in_dll(  # type:
    ignore[attr-defined]
...
def _handle_osstatus(result: OSStatus, _: typing.Any, args: typing.Any) ->
    typing.Any:
    """
    ...
    try:
        error_message_cfstring = Security.SecCopyErrorMessageString(result,
    None)
...
Security.SecTrustCreateWithCertificates.errcheck = _handle_osstatus  # type:
    ignore[assignment]
Security.SecTrustSetAnchorCertificates.errcheck = _handle_osstatus  # type:
    ignore[assignment]
Security.SecTrustSetAnchorCertificatesOnly.errcheck = _handle_osstatus  #
    type: ignore[assignment]
Security.SecTrustGetTrustResult.errcheck = _handle_osstatus  # type:
    ignore[assignment]
Security.SecTrustEvaluate.errcheck = _handle_osstatus  # type:
    ignore[assignment]
...
def _bytes_to_cf_data_ref(value: bytes) -> CFDataRef:  # type:
    ignore[valid-type]
...
def _cf_string_ref_to_str(cf_string_ref: CFStringRef) -> str | None:  # type:
    ignore[valid-type]
    """
...
def _der_certs_to_cf_cert_array(certs: list[bytes]) -> CFMutableArrayRef:  #
    ...
    """Builds a CFArray of SecCertificateRefs from a list of DER-encoded
    certificates.
    Responsibility of the caller to call CoreFoundation.CFRelease on the
    CFArray.
    """
...
def _verify_peercerts_impl(
    ...
    try:
        ...
        if server_hostname is not None and ssl_context.check_hostname:
            ...
            try:
                cf_str_hostname =
    _bytes_to_cf_string(server_hostname.encode("ascii"))
        ...
        if ssl_context.verify_flags & ssl.VERIFY_CRL_CHECK_CHAIN:
            ...
            raise NotImplementedError("VERIFY_CRL_CHECK_LEAF not implemented
    for macOS")
...
def _verify_peercerts_impl_macos_10_13(
    ...
    Security.SecTrustEvaluate(sec_trust_ref,
...
def _verify_peercerts_impl_macos_10_14(
    ...
    if not is_trusted:
        ...
        try:
            cf_error_string_ref =
    CoreFoundation.CFErrorCopyDescription(cf_error)
