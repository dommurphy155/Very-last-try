...
class Wheel(object):
    ...
    def build(self, paths, tags=None, wheel_version=None):
        ...
        # be. Use the number of path segments to keep directory entries
    together,
            ...
            try:
                for zinfo in zf.infolist():
                    ...
                    if lib_only and u_arcname.startswith((info_pfx, data_pfx)):
                        ...
                        continue
                    is_script = (u_arcname.startswith(script_pfx) and not
    u_arcname.endswith('.exe'))
                    ...
                    if not is_script:
                        ...
                        # zf.extract(zinfo, libdir) should have worked, but
    didn't,
                        # see
    ...
    zinfo
                        if os.name == 'posix':
                            ...
                            os.chmod(outfile, (zinfo.external_attr >> 16) &
    0x1FF)
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = os.path.join(get_cache_base(), str('dylib-cache'), '%s.%s'
    % sys.version_info[:2])
            cache = Cache(base)
        return cache
        ...
        with ZipFile(pathname, 'r') as zf:
            try:
                with zf.open(arcname) as bf:
                    ...
                    prefix = cache.prefix_to_dir(self.filename,
    use_abspath=False)
        ...
        def update_version(version, path):
            ...
            try:
                ...
                if i < 0:
                    ...
                    parts[-1] += 1
                    updated = '%s+%s' % (version[:i], '.'.join(str(i) for i in
    parts))
        ...
        for abi in add_abis:
            for arch in arches:
                ...
                if abi != 'none' and sys.platform.startswith('linux'):
                    ...
                    if len(parts) == 2:
                        if parts >= (2, 5):
                            result.append((''.join((IMP_PREFIX, version)),
    abi, 'manylinux1_%s' % arch))
                        if parts >= (2, 12):
                            result.append((''.join((IMP_PREFIX, version)),
    abi, 'manylinux2010_%s' % arch))
                        if parts >= (2, 17):
                            result.append((''.join((IMP_PREFIX, version)),
    abi, 'manylinux2014_%s' % arch))
                        result.append((''.join(
