"""
    ...
    Formatter for HTML output.:copyright: Copyright 2006-2024 by the Pygments team, see AUTHORS.:license: BSD, see LICENSE for details.
"""
...
class HtmlFormatter(Formatter):
    r"""
    Format tokens as HTML 4 ``<span>`` tags. By default, the content is
    enclosed
    in a ``<pre>`` tag, itself wrapped in a ``<div>`` tag (but see the
    `nowrap` option).
    ...
    .. sourcecode:: html
        ...
        <table><tr>
          ...
          <td class="code">
            <pre><span class="Ke">def </span><span
    class="NaFu">foo</span>(bar):
    ...
    If you have Pygments 0.6 or higher, you can also pass a list or tuple to
    the
    ...
    `nowrap`
        ...
        around the tokens. This disables most other options (default:
    ``False``).
    ...
    `classprefix`
        ...
        `classprefix` option to give a string to prepend to all
    Pygments-generated
    ...
    `linenos`
        ...
        **Note:** with the default ("table") line number mechanism, the line
        ...
        unless you give the enclosing ``<pre>`` tags an explicit
    ``line-height``
    ...
    `lineanchors`
        ...
        output line in an anchor tag with an ``id`` (and `name`) of
    ``foo-linenumber``.
    ...
    `tagurlformat`
        A string formatting pattern used to generate links to ctags
    definitions.
    ...
    **Subclassing the HTML formatter**
    ...
    `self._format_lines()` which returns a generator that yields tuples of
    ``(1,
    ...
    source lines and other code makes it possible to wrap the generator
    multiple
    times.
    ...
    This results in wrapping the formatted lines with a ``<code>`` tag, where
    the
    source lines are broken using ``<br>`` tags.

    After calling `wrap()`, the `format()` method also adds the "line numbers"
    and/or "full document" wrappers if the respective options are set. Then,
    all
    ...
    def __init__(self, **options):
        ...
        self.cssclass = self._decodeifneeded(options.get('cssclass',
    'highlight'))
    ...
    def _get_css_classes(self, ttype):
        """Return the CSS classes of this token type prefixed with the
    classprefix option."""
    ...
    '.format(webify(ndef['bgcolor']))
            if ndef['border']:
                style += 'border: 1px solid {};
    '.format(webify(ndef['border']))
            if style:
                t2c[ttype] = name
    ...
    def get_background_style_defs(self, arg=None):
        ...
        if hl_color is not None:
            lines.insert(
                0, '{} {{ background-color: {} }}'.format(prefix('hll'),
    hl_color)
            )
            ...
            if os.path.isabs(self.cssfile):
                ...
                try:
                    ...
                    print('Note: Cannot determine output file name, '
                          'using current directory as base for the CSS file
    name',
                          file=sys.stderr)
                    cssfilename = self.cssfile
    ...
    option.
            try:
                if not os.path.exists(cssfilename) or not
    self.noclobber_cssfile:
    ...
    def _wrap_tablelinenos(self, inner):
        ...
        # If a filename was specified, we can't put it into the code table as
    it
    ...
    def _wrap_inlinelinenos(self, inner):
        ...
        for _, inner_line in inner_lines:
            ...
            if aln:
                yield 1, ('<a href="#%s-%d">%s</a>' % (anchor_name, num,
    linenos) +
                          inner_line)
            else:
    ...
    def _format_lines(self, tokensource):
        ...
        for ttype, value in tokensource:
            try:
                ...
                title = ' title="{}"'.format('.'.join(ttype)) if
    self.debug_token_types else ''
                if nocls:
