...
_RAW_TO_EMAIL_MAPPING = {raw: email for email, raw in
    _EMAIL_TO_RAW_MAPPING.items()}
...
def parse_email(data: bytes | str) -> tuple[RawMetadata, dict[str, list[str]]]:
    ...
    try:
        ...
        unparsed.setdefault("description", []).append(
            parsed.get_payload(decode=isinstance(data, bytes))  # type:
...
class _Validator(Generic[T]):
    ...
    in its "enriched" form (e.g. ``version.Version`` for the ``Version``
    field).
    ...
    def __get__(self, instance: Metadata, _owner: type[Metadata]) -> T:
        # With Python 3.8, the caching can be replaced with
    ...
    def _process_metadata_version(self, value: str) -> _MetadataVersion:
        ...
        if value not in _VALID_METADATA_VERSIONS:
            raise self._invalid_metadata(f"{value!r} is not a valid metadata
    version")
    ...
    def _process_description_content_type(self, value: str) -> str:
        ...
        # Check if content-type is valid or defaulted to `text/plain` and thus
    was
        # not parseable.
        if content_type not in content_types or content_type not in
    value.lower():
            raise self._invalid_metadata(
                f"{{field}} must be one of {list(content_types)}, not
    {value!r}"
            )
        ...
        if charset != "UTF-8":
            raise self._invalid_metadata(
                f"{{field}} can only specify the UTF-8 charset, not
    {list(charset)}"
            )

        markdown_variants = {"GFM", "CommonMark"}
        variant = parameters.get("variant", "GFM")  # Use an acceptable
    default.
        if content_type == "text/markdown" and variant not in
    markdown_variants:
            raise self._invalid_metadata(
    ...
    def _process_provides_extra(
        ...
        try:
            for name in value:
                normalized_names.append(utils.canonicalize_name(name,
    validate=True))
        except utils.InvalidName as exc:
            raise self._invalid_metadata(
    ...
    def _process_license_files(self, value: list[str]) -> list[str]:
        ...
        for path in value:
            ...
            if "*" in path:
                raise self._invalid_metadata(
                    f"{path!r} is invalid for {{field}}, paths must be
    resolved"
                )
            if (
...
class Metadata:
    ...
    will cause:exc:`InvalidMetadata` to be raised (with a:py:attr:`~BaseException.__cause__` attribute as appropriate).
    """
    ...
    values.
        ...
        if validate:
            ...
            for key in fields_to_check:
                try:
                    if metadata_version:
                        # Can't use getattr() as that triggers descriptor
    protocol which
                        # will fail due to no value for the instance argument.
                        try:
                            field_metadata_version = cls.__dict__[key].added
                        except KeyError:
                            exc = InvalidMetadata(key, f"unrecognized field:
    {key!r}")
    ...
    @classmethod
    def from_email(cls, data: bytes | str, *, validate: bool = True) ->
    Metadata:
    ...
    # `name` is not normalized/typed to NormalizedName so as to provide access
    to
    # the original/raw name.
    name: _Validator[str] = _Validator()
    """:external:ref:`core-metadata-name`
    (required; validated using:func:`~packaging.utils.canonicalize_name` and
    its
    *validate* parameter)"""
    ...
    newlines)"""
    description: _Validator[str | None] = _Validator()  # TODO 2.1: can be in
    body
