...
from pip._internal.utils.temp_dir import TempDirectoryTypeRegistry as
    TempDirRegistry
...
class Command(CommandContextMixIn):
    ...
    def _main(self, args: List[str]) -> int:
        ...
        if options.python and "_PIP_RUNNING_IN_SUBPROCESS" not in os.environ:
            logger.critical(
                "The --python option must be placed before the pip subcommand
    name"
            )
            sys.exit(ERROR)
        ...
        if options.require_venv and not self.ignore_require_venv:
            ...
            if not running_under_virtualenv():
                logger.critical("Could not find an activated virtualenv
    (required).")
