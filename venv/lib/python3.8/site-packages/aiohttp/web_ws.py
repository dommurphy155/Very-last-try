...
class WebSocketResponse(StreamResponse):
    ...
    def _send_heartbeat(self) -> None:
        if self._heartbeat is not None and not self._closed:
            ...
            # task in the class.
            self._loop.create_task(self._writer.ping())  # type:
    ignore[union-attr]
        ...
        if self._compress:
            ...
            if compress:
                enabledext = ws_ext_gen(
                    compress=compress, isserver=True,
    ...
    def _pre_start(self, request: BaseRequest) -> Tuple[str, WebSocketWriter]:
        ...
        writer = WebSocketWriter(
            request._protocol, transport, compress=compress,
    ...
    def _post_start(
        ...
        self._reader = FlowControlDataQueue(request._protocol, 2**16,
    loop=loop)
        request.protocol.set_parser(
            WebSocketReader(self._reader, self._max_msg_size,
    ...
    async def receive(self, timeout: Optional[float] = None) -> WSMessage:
        ...
        while True:
            if self._waiting is not None:
                raise RuntimeError("Concurrent call to receive() is not
    allowed")
            ...
            try:
                ...
                try:
                    async with async_timeout.timeout(timeout or
    ...
    async def receive_bytes(self, *, timeout: Optional[float] = None) -> bytes:
        ...
        if msg.type != WSMsgType.BINARY:
            raise TypeError(f"Received message {msg.type}:{msg.data!r} is not
    bytes")
        return cast(bytes, msg.data)

    async def receive_json(
        self, *, loads: JSONDecoder = json.loads, timeout: Optional[float] =
    None
    ) -> Any:
