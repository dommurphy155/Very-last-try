...
_WSMessageBase = collections.namedtuple("_WSMessageBase", ["type", "data",
    "extra"])
...
_XOR_TABLE: Final[List[bytes]] = [bytes(a ^ b for a in range(256)) for b in
    range(256)]
...
_WS_EXT_RE_SPLIT: Final[Pattern[str]] =
    re.compile(r"permessage-deflate([^,]+)?")


def ws_ext_parse(extstr: Optional[str], isserver: bool = False) -> Tuple[int,
    bool]:
    if not extstr:
        return 0, False
...
class WebSocketReader:
    def __init__(
        self, queue: DataQueue[WSMessage], max_msg_size: int, compress: bool =
    True
    ) -> None:
        self.queue = queue
        ...
        for fin, opcode, payload, compressed in self.parse_frame(data):
            ...
            if opcode == WSMsgType.CLOSE:
                if len(payload) >= 2:
                    ...
                    if close_code < 3000 and close_code not in
    ALLOWED_CLOSE_CODES:
                    ...
                    try:
                        ...
                        raise WebSocketError(
                            WSCloseCode.INVALID_TEXT, "Invalid UTF-8 text
    message"
                        ) from exc
                ...
                if not fin:
                    ...
                    if self._max_msg_size and len(self._partial) >=
    self._max_msg_size:
                        raise WebSocketError(
            ...
            if self._state == WSParserState.READ_HEADER:
                if buf_length - start_pos >= 2:
                    ...
                    if opcode > 0x7 and length > 125:
                        raise WebSocketError(
                            WSCloseCode.PROTOCOL_ERROR,
                            "Control frame payload cannot be " "larger than
    125 bytes",
                        )
    ...
    async def _send_frame(
        ...
        if (compress or self.compress) and opcode < 8:
            if compress:
                ...
                compressobj = zlib.compressobj(level=zlib.Z_BEST_SPEED,
    wbits=-compress)
            else:  # self.compress
