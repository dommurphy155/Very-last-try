...
class NamedPipeSite(BaseSite):
    ...
    def __init__(
        self, runner: "BaseRunner", path: str, *, shutdown_timeout: float =
    60.0
    ) -> None:
        loop = asyncio.get_event_loop()
        if not isinstance(
...
class SockSite(BaseSite):
    ...
    async def start(self) -> None:
        ...
        assert server is not None
        self._server = await loop.create_server(
            server, sock=self._sock, ssl=self._ssl_context,
    backlog=self._backlog
        )
...
class BaseRunner(ABC):
    ...
    def _reg_site(self, site: BaseSite) -> None:
        if site in self._sites:
            raise RuntimeError(f"Site {site} is already registered in runner
    {self}")
        self._sites.append(site)

    def _check_site(self, site: BaseSite) -> None:
        if site not in self._sites:
            raise RuntimeError(f"Site {site} is not registered in runner
    {self}")
...
class ServerRunner(BaseRunner):
    ...
    def __init__(
        self, web_server: Server, *, handle_signals: bool = False, **kwargs:
    Any
    ) -> None:
