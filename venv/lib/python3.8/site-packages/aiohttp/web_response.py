...
class StreamResponse(BaseClass, HeadersMixin):
    ...
    def set_status(
        ...
        assert not self.prepared, (
            "Cannot change the response status code after " "the headers have
    been sent"
        )
    ...
    None:
        ...
        if hdrs.CONTENT_LENGTH in self._headers:
            raise RuntimeError(
                "You can't enable chunked encoding when " "a content length is
    set"
            )
    ...
    def content_length(self, value: Optional[int]) -> None:
        if value is not None:
            ...
            if self._chunked:
                raise RuntimeError(
                    "You can't set content length when " "chunked encoding is
    enable"
                )
    ...
    async def drain(self) -> None:
        ...
        assert self._payload_writer is not None, "Response has not been
    started"
        warnings.warn(
...
class Response(StreamResponse):
    ...
    type(text))
                ...
                if charset is None:
                    charset = "utf-8"
                real_headers[hdrs.CONTENT_TYPE] = content_type + "; charset="
    + charset
    ...
    async def write_eof(self, data: bytes = b"") -> None:
        ...
        assert self._payload_writer is not None
        if body is not None:
            if self._req._method == hdrs.METH_HEAD or self._status in [204,
    304]:
    ...
    def _compress_body(self, zlib_mode: int) -> None:
        ...
        self._compressed_body = compressobj.compress(body_in) +
    compressobj.flush()
    ...
    async def _do_start_compression(self, coding: ContentCoding) -> None:
        ...
        if coding != ContentCoding.identity:
            ...
            zlib_mode = (
                16 + zlib.MAX_WBITS if coding == ContentCoding.gzip else
    zlib.MAX_WBITS
            )
            body_in = self._body
            assert body_in is not None
...
def json_response(
    ...
    *,
    ...
    if data is not sentinel:
        if text or body:
            raise ValueError("only one of data, text, or body should be
    specified")
        else:
            text = dumps(data)
    return Response(
