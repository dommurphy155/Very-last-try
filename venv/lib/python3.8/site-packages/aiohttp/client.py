...
def _default_fallback_charset_resolver(response: ClientResponse, body: bytes)
    -> str:
...
class ClientSession:
    ...
    ClientWebSocketResponse,
        ...
        if connector._loop is not loop:
            raise RuntimeError("Session and connector has to use same event
    loop")
        ...
        if timeout is sentinel:
            ...
            if read_timeout is not sentinel:
                warnings.warn(
                    "read_timeout is deprecated, " "use timeout argument
    instead",
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    connect=conn_timeout)
                warnings.warn(
                    "conn_timeout is deprecated, " "use timeout argument
    instead",
                    DeprecationWarning,
                    stacklevel=2,
                )
    ...
    def __del__(self, _warnings: Any = warnings) -> None:
        if not self.closed:
            ...
            context = {"client_session": self, "message": "Unclosed client
    session"}
        ...
        *,
        ...
        if timeout is sentinel:
            ...
            if not isinstance(timeout, ClientTimeout):
                real_timeout = ClientTimeout(total=timeout)  # type:
    ignore[arg-type]
            else:
            ...
            with timer:
                while True:
                    ...
                    try:
                        ...
                        if exc.errno is None and isinstance(exc,
    ...
    allow_redirects:
                        ...
                        if (resp.status == 303 and resp.method !=
    hdrs.METH_HEAD) or (
                            resp.status in (301, 302) and resp.method ==
    hdrs.METH_POST
                        ):
    ...
    resp.headers.get(
                        ...
                        if scheme not in ("http", "https", ""):
                            resp.close()
                            raise ValueError("Can redirect only to http or
    https")
                        elif not scheme:
    ...
    def get(
        ...
        return _RequestContextManager(
            self._request(hdrs.METH_GET, url, allow_redirects=allow_redirects,
    **kwargs)
        )
    ...
    def options(
        ...
        return _RequestContextManager(
            self._request(
                hdrs.METH_OPTIONS, url, allow_redirects=allow_redirects,
    **kwargs
            )
        )
    ...
    def delete(self, url: StrOrURL, **kwargs: Any) -> "_RequestContextManager":
        ...
        return _RequestContextManager(self._request(hdrs.METH_DELETE, url,
    **kwargs))
    ...
    def raise_for_status(
        ...
        """Should `ClientResponse.raise_for_status()` be called for each
    response."""
...
class _BaseRequestContextManager(Coroutine[Any, Any, _RetType],
    ...
    def throw(self, arg: BaseException) -> None:  # type:
    ignore[arg-type,override]
