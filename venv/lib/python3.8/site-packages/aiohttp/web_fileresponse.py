...
class FileResponse(StreamResponse):
    ...
    async def _sendfile_fallback(
        self, writer: AbstractStreamWriter, fobj: IO[Any], offset: int, count:
    int
    ...
    def _strong_etag_match(etag_value: str, etags: Tuple[ETag, ...]) -> bool:
        if len(etags) == 1 and etags[0].value == ETAG_ANY:
            return True
        return any(etag.value == etag_value for etag in etags if not
    etag.is_weak)
    ...
    Optional[AbstractStreamWriter]:
        ...
        if ifmatch is not None and not self._strong_etag_match(etag_value,
    ifmatch):
        ...
        if ifnonematch is not None and self._strong_etag_match(etag_value,
    ifnonematch):
