...
async def _check_request_resolves(request: Request, path: str) -> Tuple[bool,
    Request]:
...
def normalize_path_middleware(
    *,
    ...
    """Factory for producing a middleware that normalizes the path of a
    request.
    ...
    async def impl(request: Request, handler: Handler) -> StreamResponse:
        if isinstance(request.match_info.route, SystemRoute):
            ...
            for path in paths_to_check:
                path = re.sub("^//+", "/", path)  # SECURITY:
    GHSA-v6wp-4m6f-gcjg
                resolves, request = await _check_request_resolves(request,
    path)
                if resolves:
