...
class Application(MutableMapping[str, Any]):
    ...
    def __init__(
        ...
        *,
        ...
        if router is None:
            ...
            warnings.warn(
                "router argument is deprecated", DeprecationWarning,
    stacklevel=2
            )
        ...
        if debug is not ...:
            warnings.warn(
                "debug argument is deprecated", DeprecationWarning,
    stacklevel=2
            )
        self._debug = debug
    ...
    def _check_frozen(self) -> None:
        if self._frozen:
            warnings.warn(
                "Changing state of started or joined " "application is
    deprecated",
                DeprecationWarning,
                stacklevel=3,
            )
    ...
    def loop(self) -> asyncio.AbstractEventLoop:
        ...
        warnings.warn("loop property is deprecated", DeprecationWarning,
    stacklevel=2)
    ...
    @property
    def debug(self) -> bool:
        warnings.warn("debug property is deprecated", DeprecationWarning,
    stacklevel=2)
    ...
    def _add_subapp(
        self, resource_factory: Callable[[], AbstractResource], subapp:
    "Application"
    ) -> AbstractResource:
        if self.frozen:
            raise RuntimeError("Cannot add sub application to frozen
    application")
        if subapp.frozen:
        ...
        **kwargs: Any,
        ...
        if not issubclass(access_log_class, AbstractAccessLogger):
            raise TypeError(
                ...
                "aiohttp.abc.AbstractAccessLogger, got
    {}".format(access_log_class)
            )
    ...
    def make_handler(
        ...
        **kwargs: Any,
        ...
        warnings.warn(
            "Application.make_handler(...) is deprecated, " "use AppRunner API
    instead",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def _prepare_middleware(self) -> Iterator[Tuple[_Middleware, bool]]:
        for m in reversed(self._middlewares):
            if getattr(m, "__middleware_version__", None) == 1:
                ...
                warnings.warn(
                    'old-style middleware "{!r}" deprecated, ' "see
    #2252".format(m),
                    DeprecationWarning,
                    stacklevel=2,
                )
        ...
        if resp is None:
            ...
            if self._run_middlewares:
                for app in match_info.apps[::-1]:
                    for m, new_style in app._middlewares_handlers:  # type:
    ignore[union-attr] # noqa
                        if new_style:
                            handler = update_wrapper(
                                partial(m, handler=handler), handler
                            )
                        else:
                            handler = await m(app, handler)  # type:
    ignore[arg-type]
    ...
    async def _on_cleanup(self, app: Application) -> None:
        ...
        for it in reversed(self._exits):
            try:
                ...
                errors.append(RuntimeError(f"{it!r} has more than one
    'yield'"))
        if errors:
            if len(errors) == 1:
                raise errors[0]
