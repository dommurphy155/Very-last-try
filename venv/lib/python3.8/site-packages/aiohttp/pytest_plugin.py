...
AiohttpClient = Callable[[Union[Application, BaseTestServer]],
...
@contextlib.contextmanager
def _passthrough_loop_context(loop, fast=False):  # type:
...
def pytest_pycollect_makeitem(collector, name, obj):  # type:
    ignore[no-untyped-def]
...
def proactor_loop():  # type: ignore[no-untyped-def]
    if not PY_37:
        ...
        policy._loop_factory = asyncio.ProactorEventLoop  # type:
    ignore[attr-defined]
    else:
        policy = asyncio.WindowsProactorEventLoopPolicy()  # type:
    ignore[attr-defined]
        asyncio.set_event_loop_policy(policy)
...
@pytest.fixture
def unused_port(aiohttp_unused_port):  # type: ignore[no-untyped-def] #
    pragma: no cover
    warnings.warn(
...
@pytest.fixture
def test_server(aiohttp_server):  # type: ignore[no-untyped-def]  # pragma: no
    cover
    warnings.warn(
...
def aiohttp_raw_server(loop):  # type: ignore[no-untyped-def]
    ...
    async def go(handler, *, port=None, **kwargs):  # type:
    ignore[no-untyped-def]
...
@pytest.fixture
def test_client(aiohttp_client):  # type: ignore[no-untyped-def]  # pragma: no
    cover
    warnings.warn(
