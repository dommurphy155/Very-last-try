...
def _merge_ssl_params(
    ...
    if fingerprint is not None:
        warnings.warn(
            "fingerprint is deprecated, " "use ssl=Fingerprint(fingerprint)
    instead",
            DeprecationWarning,
            stacklevel=3,
        )
...
class ClientRequest:
    ...
    def connection_key(self) -> ConnectionKey:
        ...
        if proxy_headers:
            h: Optional[int] = hash(tuple((k, v) for k, v in
    proxy_headers.items()))
        else:
            h = None
    ...
    def update_content_encoding(self, data: Any) -> None:
        ...
        if enc:
            if self.compress:
                raise ValueError(
                    "compress can not be set " "if Content-Encoding header is
    set"
                )
        elif self.compress:
    ...
    async def _on_chunk_request_sent(self, method: str, url: URL, chunk:
    bytes) -> None:
...
class ClientResponse(HeadersMixin):
    ...
    _raw_headers: RawHeaders = None  # type: ignore[assignment] # Response raw
    headers
    ...
    def url_obj(self) -> URL:
        warnings.warn("Deprecated, use .url #1654", DeprecationWarning,
    stacklevel=2)
        return self._url
        ...
        if self._connection is not None:
            ...
            if self._loop.get_debug():
                if PY_36:
                    ...
                    kwargs = {}
                _warnings.warn(f"Unclosed response {self!r}", ResourceWarning,
    **kwargs)
                context = {"client_response": self, "message": "Unclosed
    response"}
        ...
        with self._timer:
            while True:
                ...
                try:
                    ...
                    message, payload = await protocol.read()  # type:
    ignore[union-attr]
                ...
                if message.code < 100 or message.code > 199 or message.code ==
    101:
                    break
    ...
    @property
    def ok(self) -> bool:
        """Returns ``True`` if ``status`` is less than ``400``, ``False`` if
    not.
    ...
    async def text(self, encoding: Optional[str] = None, errors: str =
    "strict") -> str:
