...
class BasicAuth(namedtuple("BasicAuth", ["login", "password", "encoding"])):
    ...
    def __new__(
        ...
        if ":" in login:
            raise ValueError('A ":" is not allowed in login (RFC
    1945#section-11.1)')
    ...
    @classmethod
    def from_url(cls, url: URL, *, encoding: str = "latin1") ->
    Optional["BasicAuth"]:
...
def parse_mimetype(mimetype: str) -> MimeType:
    ...
    stype, suffix = (
        cast(Tuple[str, str], stype.split("+", 1)) if "+" in stype else
    (stype, "")
    )

    return MimeType(
        type=mtype, subtype=stype, suffix=suffix,
...
def content_disposition_header(
    disptype: str, quote_fields: bool = True, _charset: str = "utf-8",
    **params: str
) -> str:
...
class reify(Generic[_T]):
    ...
    def __get__(self, inst: _TSelf[_T], owner: Optional[Type[Any]] = None) ->
    _T:
        try:
            try:
...
def _is_ip_address(
    regex: Pattern[str], regexb: Pattern[bytes], host: Optional[Union[str,
    bytes]]
) -> bool:
    if host is None:
        return False
...
class HeadersMixin:
    ATTRS = frozenset(["_content_type", "_content_dict",
    ...
    def _parse_content_type(self, raw: str) -> None:
        ...
        if raw is None:
            ...
            self._content_dict = dict(params[1:])  # First element is content
    type again

    @property
    def content_type(self) -> str:
        """The value of content part for Content-Type HTTP header."""
        raw = self._headers.get(hdrs.CONTENT_TYPE)  # type:
    ignore[attr-defined]
