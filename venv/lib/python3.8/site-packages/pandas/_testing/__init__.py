...
UNSIGNED_INT_NUMPY_DTYPES: list[Dtype] = ["uint8", "uint16", "uint32",
    "uint64"]
...
if not pa_version_under1p01:
    ...
    UNSIGNED_INT_PYARROW_DTYPES = [pa.uint8(), pa.uint16(), pa.uint32(),
    pa.uint64()]
    SIGNED_INT_PYARROW_DTYPES = [pa.int8(), pa.int16(), pa.int32(), pa.int64()]
    ALL_INT_PYARROW_DTYPES = UNSIGNED_INT_PYARROW_DTYPES +
    ...
    TIMEDELTA_PYARROW_DTYPES = [pa.duration(unit) for unit in ["s", "ms",
    "us", "ns"]]
...
def makeCategoricalIndex(k=10, n=3, name=None, **kwargs) -> CategoricalIndex:
    ...
    return CategoricalIndex(
        Categorical.from_codes(np.arange(k) % n, categories=x), name=name,
    **kwargs
    )
...
def makeTimedeltaIndex(k: int = 10, freq="D", name=None, **kwargs) ->
    TimedeltaIndex:
    return pd.timedelta_range(start="1 day", periods=k, freq=freq, name=name,
    **kwargs)
...
def makePeriodSeries(nper=None, name=None) -> Series:
    if nper is None:
        nper = _N
    return Series(np.random.randn(nper), index=makePeriodIndex(nper),
    name=name)
...
def makeCustomIndex(
    ...
    assert names is None or names is False or names is True or len(names) is
    nlevels
...
def makeCustomDataframe(
    ...
    c_idx_names, idx_names  - False/True/list of strings,  yields No names ,
            ...
            c_idx_nlevels ==1.
    c_idx_nlevels - number of levels in columns index. > 1 will yield
    MultiIndex
    r_idx_nlevels - number of levels in rows index. > 1 will yield MultiIndex
    data_gen_f - a function f(row,col) which return the data value
            at that position, the default generator used yields values of the
    form
    ...
    length
            N < idx_nlevels, for just the first N levels. If ndupe doesn't
    divide
...
def external_error_raised(expected_exception: type[Exception]) ->
    ContextManager:
    """
...
def shares_memory(left, right) -> bool:
    ...
    if isinstance(left, pd.core.arrays.IntervalArray):
        return shares_memory(left._left, right) or shares_memory(left._right,
    right)

    if isinstance(left, ExtensionArray) and left.dtype == "string[pyarrow]":
        # https://github.com/pandas-dev/pandas/pull/43930#discussion_r736862669
        if isinstance(right, ExtensionArray) and right.dtype ==
    "string[pyarrow]":
    ...
    if isinstance(left, BaseMaskedArray) and isinstance(right,
    BaseMaskedArray):
