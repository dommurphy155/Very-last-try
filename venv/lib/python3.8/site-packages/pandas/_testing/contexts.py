...
def decompress_file(path, compression) -> Iterator[IO[bytes]]:
    ...
    file object
    """
    with get_handle(path, "rb", compression=compression, is_text=False) as
    handle:
        yield handle.handle
...
def ensure_clean(filename=None, return_filelike: bool = False, **kwargs: Any):
    ...
    This implementation does not use tempfile.mkstemp to avoid having a file
    handle.
    If the code using the returned path wants to delete the file itself,
    windows
