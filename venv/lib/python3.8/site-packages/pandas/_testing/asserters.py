    ...
    **kwargs,
    ...
    if check_less_precise is not no_default:
        warnings.warn(
            ...
            "You can stop passing 'check_less_precise' to silence this
    warning.",
            FutureWarning,
    ...
    if isinstance(left, Index):
        ...
        if check_dtype:
            if is_number(left) and is_number(right):
                ...
                if isinstance(left, np.ndarray) or isinstance(right,
    np.ndarray):
                    obj = "numpy array"
                else:
                    obj = "Input"
        ...
        _testing.assert_almost_equal(
            left, right, check_dtype=bool(check_dtype), rtol=rtol, atol=atol,
    **kwargs
        )
...
def assert_index_equal(
    ...
    def _check_types(left, right, obj="Index") -> None:
        ...
        if is_categorical_dtype(left.dtype) and
    is_categorical_dtype(right.dtype):
    ...
    if check_categorical:
        if is_categorical_dtype(left.dtype) or
    is_categorical_dtype(right.dtype):
            assert_categorical_equal(left._values, right._values, obj=f"{obj}
    category")
...
def assert_categorical_equal(
    ...
    exact=exact
        ...
        assert_numpy_array_equal(
            left.codes, right.codes, check_dtype=check_dtype,
    obj=f"{obj}.codes"
        )
    else:
        try:
...
def assert_numpy_array_equal(
    ...
    if check_same == "same":
        if left_base is not right_base:
            raise AssertionError(f"{repr(left_base)} is not
    {repr(right_base)}")
...
def assert_extension_array_equal(
    ...
    if (
        ...
        assert_numpy_array_equal(
            np.asarray(left.asi8), np.asarray(right.asi8),
    index_values=index_values
        )
        return
    ...
    assert_numpy_array_equal(
        left_na, right_na, obj="ExtensionArray NA mask",
    index_values=index_values
    )
    ...
    if check_exact:
        assert_numpy_array_equal(
            left_valid, right_valid, obj="ExtensionArray",
    index_values=index_values
        )
    else:
        _testing.assert_almost_equal(
...
def assert_series_equal(
    ...
    *,
    ...
    if check_flags:
        assert left.flags == right.flags, f"{repr(left.flags)} !=
    {repr(right.flags)}"
    ...
    if check_exact and is_numeric_dtype(left.dtype) and
    ...
    ) or is_extension_array_dtype_and_needs_i8_conversion(right.dtype,
    left.dtype):
        assert_extension_array_equal(
            left._values,
            right._values,
...
def assert_frame_equal(
    ...
    check_column_type: bool or {'equiv'}, default 'equiv'
        ...
        are identical. Is passed as the ``exact`` argument of:func:`assert_index_equal`.
    ...
    if left.shape != right.shape:
        raise_assert_detail(
            obj, f"{obj} shape mismatch", f"{repr(left.shape)}",
    f"{repr(right.shape)}"
        )
    ...
    if by_blocks:
        ...
        for dtype in list(set(list(lblocks.keys()) + list(rblocks.keys()))):
            ...
            assert_frame_equal(
                lblocks[dtype], rblocks[dtype], check_dtype=check_dtype,
    obj=obj
            )
...
def assert_equal(left, right, **kwargs) -> None:
    ...
    **kwargs
        All keyword arguments are passed through to the underlying assert
    method.
    """
    __tracebackhide__ = True
...
def assert_copy(iter1, iter2, **eql_kwargs) -> None:
    ...
    for elem1, elem2 in zip(iter1, iter2):
        ...
        msg = (
            f"Expected object {repr(type(elem1))} and object
    {repr(type(elem2))} to be "
...
def is_extension_array_dtype_and_needs_i8_conversion(left_dtype, right_dtype)
    ...
    """
    return is_extension_array_dtype(left_dtype) and
    needs_i8_conversion(right_dtype)
