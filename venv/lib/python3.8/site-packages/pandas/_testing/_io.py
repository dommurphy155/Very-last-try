    ...
    * ``raise_on_error`` supersedes ``check_before_test``
    ...
    def wrapper(*args, **kwargs):
        if (
            ...
            pytest.skip(
                f"May not have network connectivity because cannot connect to
    {url}"
            )
        try:
            ...
            if not errno and hasattr(errno, "reason"):
                ...
                errno = getattr(err.reason, "errno", None)  # type:
    ignore[attr-defined]

            if errno in skip_errnos:
                pytest.skip(f"Skipping test due to known errno and error
    {err}")
            ...
            if any(m.lower() in e_str.lower() for m in _skip_on_messages):
                pytest.skip(
                    f"Skipping test because exception message is known and
    error {err}"
                )
            ...
            if not isinstance(err, error_classes) or raise_on_error:
                ...
                pytest.skip(
                    f"Skipping test due to lack of connectivity and error
    {err}"
                )
