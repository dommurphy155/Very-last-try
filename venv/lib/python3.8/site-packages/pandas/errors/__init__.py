...
class IntCastingNaNError(ValueError):
    """
    Exception raised when converting (``astype``) an array with NaN to an
    integer type.
    """
...
class DtypeWarning(Warning):
    ...
    >>> df2 = pd.read_csv('test.csv', sep=',', dtype={'a': str})  # doctest:
    +SKIP
...
class EmptyDataError(ValueError):
    """
    Exception raised in ``pd.read_csv`` when empty data or header is
    encountered.
    """
...
class AbstractMethodError(NotImplementedError):
    ...
    def __str__(self) -> str:
        ...
        return f"This {self.methodtype} must be defined in the concrete class
    {name}"
...
class NumExprClobberingError(NameError):
    """
    Exception raised when trying to use a built-in numexpr name as a variable
    name.

    ``eval`` or ``query`` will throw the error if the engine is set
    to 'numexpr'. 'numexpr' is the default engine value for these methods if
    the
    ...
    ... # NumExprClobberingError: Variables in expression "(abs) > (2)"
    overlap...
    >>> sin, a = 1, 2
    >>> pd.eval("sin + a", engine='numexpr') # doctest: +SKIP
    ... # NumExprClobberingError: Variables in expression "(sin) + (a)"
    overlap...
    """
...
class UndefinedVariableError(NameError):
    """
    Exception raised by ``query`` or ``eval`` when using an undefined variable
    name.
...
class IndexingError(Exception):
    """
    Exception is raised when trying to index and there is a mismatch in
    dimensions.
...
class CSSWarning(UserWarning):
    ...
    This can be due to the styling not having an equivalent value or because
    the
...
class PossibleDataLossError(Exception):
    ...
    ... # PossibleDataLossError: Re-opening the file [my-store] with mode
    [a]...
    """
...
class ClosedFileError(Exception):
    """
    Exception is raised when trying to perform an operation on a closed
    HDFStore file.
...
class IncompatibilityWarning(Warning):
    """
    Warning raised when trying to use where criteria on an incompatible HDF5
    file.
    """
...
class AttributeConflictWarning(Warning):
    ...
    name than the existing index on an HDFStore or attempting to append an
    index with a
...
class DatabaseError(OSError):
    """
    Error is raised when executing sql with bad syntax or sql that throws an
    error.
    ...
    ... # DatabaseError: Execution failed on sql 'test': near "test": syntax
    error
    """
...
class PossiblePrecisionLoss(Warning):
    """
    Warning raised by to_stata on a column with a value outside or equal to
    int64.
...
class ValueLabelTypeMismatch(Warning):
    """
    Warning raised by to_stata on a category column that contains non-string
    values.

    Examples
    --------
    >>> df = pd.DataFrame({"categories": pd.Series(["a", 2],
    dtype="category")})
    >>> df.to_stata('test') # doctest: +SKIP
    ... # ValueLabelTypeMismatch: Stata value labels (pandas categories) must
    be str...
    """
...
class InvalidColumnName(Warning):
    ...
    >>> df.to_stata('test') # doctest: +SKIP
    ... # InvalidColumnName: Not all pandas column names were valid Stata
    variable...
    """
...
class CategoricalConversionWarning(Warning):
    """
    Warning is raised when reading a partial labeled Stata file using a
    iterator.
