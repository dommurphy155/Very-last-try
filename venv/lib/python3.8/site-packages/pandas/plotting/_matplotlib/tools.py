...
def table(
    ...
    table = matplotlib.table.table(
        ax, cellText=cellText, rowLabels=rowLabels, colLabels=colLabels,
    **kwargs
    )
    return table
...
def _get_layout(
    ...
    if layout is not None:
        ...
        if nrows * ncols < nplots:
            raise ValueError(
                f"Layout of {nrows}x{ncols} must be larger than required size
    {nplots}"
            )
        ...
        if is_list_like(ax):
            ...
            if sharex or sharey:
                warnings.warn(
                    ...
                    "are ignored. These settings must be specified when
    creating axes.",
                    UserWarning,
                    stacklevel=find_stack_level(),
                )
...
def _has_externally_shared_axis(ax1: Axes, compare_axis: str) -> bool:
    ...
    if compare_axis == "x":
        ...
        raise ValueError(
            "_has_externally_shared_axis() needs 'x' or 'y' as a second
    parameter"
        )
