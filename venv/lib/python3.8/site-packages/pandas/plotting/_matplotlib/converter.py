...
def register() -> None:
    ...
    for type_, cls in pairs:
        ...
        if type_ in units.registry and not isinstance(units.registry[type_],
    cls):
    ...
    def axisinfo(unit: tzinfo | None, axis) -> units.AxisInfo:
        ...
        *unit* is a tzinfo instance or None.
        ...
        return units.AxisInfo(
            majloc=majloc, majfmt=majfmt, label="", default_limits=(datemin,
    datemax)
        )
    ...
    def __call__(self):
        ...
        if estimate > self.MAXTICKS * 2:
            raise RuntimeError(
                ...
                f"{estimate:d} ticks from {dmin} to {dmax}: exceeds
    Locator.MAXTICKS"
...
def _daily_finder(vmin, vmax, freq: BaseOffset):
    ...
    info = np.zeros(
        span, dtype=[("val", np.int64), ("maj", bool), ("min", bool), ("fmt",
    "|S20")]
    )
    ...
    def first_label(label_flags):
        if (label_flags[0] == 0) and (label_flags.size > 1) and ((vmin_orig %
    1) > 0.0):
    ...
    if span <= periodspermonth:
        ...
        def _second_finder(label_interval):
            ...
            year_start = period_break(dates_, "year")
            info_fmt = info["fmt"]
            info_fmt[second_start & (_second % label_interval == 0)] =
    "%H:%M:%S"
...
def _monthly_finder(vmin, vmax, freq):
    ...
    # Initialize the output
    info = np.zeros(
        span, dtype=[("val", int), ("maj", bool), ("min", bool), ("fmt",
    "|S8")]
    )
