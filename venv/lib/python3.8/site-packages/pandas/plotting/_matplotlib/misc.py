    ...
    for i, a in enumerate(df.columns):
        for j, b in enumerate(df.columns):
            ...
            if i == j:
                ...
                ax.scatter(
                    df[b][common], df[a][common], marker=marker, alpha=alpha,
    **kwds
                )
...
def radviz(
    ...
    **kwds,
    ...
    colors = get_standard_colors(
        num_colors=len(classes), colormap=colormap, color_type="random",
    color=color
    )
    ...
    s = np.array(
        [(np.cos(t), np.sin(t)) for t in [2 * np.pi * (i / m) for i in
    range(m)]]
    )
    ...
    for xy, name in zip(s, df.columns):
        ...
        if xy[0] < 0.0 and xy[1] < 0.0:
            ax.text(
                xy[0] - 0.025, xy[1] - 0.025, name, ha="right", va="top",
    size="small"
            )
            ...
            ax.text(
                xy[0] + 0.025, xy[1] - 0.025, name, ha="left", va="top",
    size="small"
            )
        elif xy[0] >= 0.0 and xy[1] >= 0.0:
            ax.text(
                xy[0] + 0.025, xy[1] + 0.025, name, ha="left", va="bottom",
    size="small"
            )
...
def lag_plot(series: Series, lag: int = 1, ax: Axes | None = None, **kwds) ->
    Axes:
...
def autocorrelation_plot(series: Series, ax: Axes | None = None, **kwds) ->
    Axes:
    import matplotlib.pyplot as plt
