...
def create_iter_data_given_by(
    ...
    kind: str, plot kind. This function is only used for `hist` and `box`
    plots.
...
def reconstruct_data_with_by(
    ...
    """
    Internal function to group data, and reassign multiindex column names onto
    the
    ...
    Returns
    -------
    Output is the reconstructed DataFrame with MultiIndex columns. The first
    level
    ...
    for key, group in grouped:
        ...
        columns = MultiIndex.from_product([[key], cols])  # type:
    ignore[list-item]
...
def reformat_hist_y_given_by(
    ...
    """Internal function to reformat y given `by` is applied or not for hist
    plot.

    If by is None, input y is 1-d with NaN removed; and if by is not None,
    groupby
