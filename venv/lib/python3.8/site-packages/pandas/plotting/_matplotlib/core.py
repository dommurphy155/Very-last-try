...
class MPLPlot(ABC):
    ...
    def __init__(
        ...
        **kwds,
        ...
        # Assign the rest of columns into self.columns if by is explicitly
    defined
        # while column is not, only need `columns` in hist/box plot when it's
    DF
    ...
    is_numeric_dtype(data[col])
        ...
        # For `hist` plot, need to get grouped original data before
    `self.data` is
        # updated later
        ...
        self.ax = ax
        self.fig = fig
        self.axes = np.array([], dtype=object)  # "real" version get set in
    `generate`
    ...
    def _validate_subplots_kwarg(
        ...
        Returns
        -------
        validated subplots: a bool or a list of tuples of column indices.
    Columns
    ...
    "
        ...
        for group in subplots:
            ...
            if (idx_locs == -1).any():
                ...
                raise ValueError(
                    f"Column label(s) {list(bad_labels)} not found in the
    DataFrame."
                )
            else:
                ...
                if duplicates:
                    raise ValueError(
                        ...
                        f"Columns {duplicates} were found in multiple
    subplots."
                    )
    ...
    def _validate_color_args(self):
        ...
        if (
            ...
            warnings.warn(
                "'color' and 'colormap' cannot be used simultaneously. Using
    'color'",
    ...
    def nseries(self) -> int:
        ...
        # When `by` is explicitly assigned, grouped data size will be defined,
    and
    ...
    def _has_plotted_object(self, ax: Axes) -> bool:
        ...
        return len(ax.lines) != 0 or len(ax.artists) != 0 or
    len(ax.containers) != 0
    ...
    len(self.subplots)
        ...
        if input_log - valid_log:
            ...
            raise ValueError(
                f"Boolean, None and 'sym' are valid options, '{invalid_log}'
    is given."
            )
    ...
    def result(self):
        ...
        if self.subplots:
            ...
            all_sec = (
                ...
                and len(self.secondary_y) == self.nseries  # type:
    ignore[arg-type]
            )
    ...
    def _compute_plot_data(self):
        ...
        if self.by is not None:
            self.subplots = True
            data = reconstruct_data_with_by(self.data, by=self.by,
    cols=self.columns)
    ...
    def _adorn_subplots(self):
        ...
        for ax in self.axes:
            ...
            # GH9093, currently Pandas does not show ylabel, so if users
    provide
    ...
    def _mark_right_label(self, label: str, index: int) -> str:
        """
        Append ``(right)`` to the label of a line if it's plotted on the right
    axis.
    ...
    def _append_legend_handles_labels(self, handle: Artist, label: str) ->
    None:
        """
        Append current handle and label to ``legend_handles`` and
    ``legend_labels``.
    ...
    def _get_ax_legend(self, ax: Axes):
        ...
        other_ax = getattr(ax, "left_ax", None) or getattr(ax, "right_ax",
    None)
        other_leg = None
    ...
    @register_pandas_matplotlib_converters
    def _plot(
        cls, ax: Axes, x, y: np.ndarray, style=None, is_errorbar: bool =
    False, **kwds
    ):
        mask = isna(y)
        if mask.any():
    ...
    def _get_custom_index_name(self):
        """Specify whether xlabel/ylabel should be used to override index
    name"""
        return self.xlabel
    ...
    def _col_idx_to_axis_idx(self, col_idx: int) -> int:
        """Return the index of the axis where the column at col_idx should be
    plotted"""
    ...
    def _parse_errorbars(self, label, err):
        ...
        and right) errors. For a ``MxN``:class:`DataFrame`, asymmetrical
    errors
    ...
    def _get_subplots(self):
        ...
        return [
            ax for ax in self.axes[0].get_figure().get_axes() if
    isinstance(ax, Subplot)
        ]
    ...
    def __init__(self, data, x, y, **kwargs) -> None:
        ...
        if self._kind == "hexbin":
            if len(self.data[x]._get_numeric_data()) == 0:
                raise ValueError(self._kind + " requires x column to be
    numeric")
            if len(self.data[y]._get_numeric_data()) == 0:
                raise ValueError(self._kind + " requires y column to be
    numeric")
...
class ScatterPlot(PlanePlot):
    ...
    def _make_plot(self):
        ...
        color_by_categorical = c_is_column and
    is_categorical_dtype(self.data[c])
        ...
        cb = self.kwds.pop("colorbar", is_numeric_dtype(c_values) and
    plot_colorbar)
...
class LinePlot(MPLPlot):
    ...
    def _make_plot(self):
        if self._is_ts_plot():
            ...
            # "Callable[[Any, Any, Any, Any, Any, Any, KwArg(Any)], Any]",
    variable has
    ...
    def _plot(  # type: ignore[override]
        cls, ax: Axes, x, y, style=None, column_num=None, stacking_id=None,
    **kwds
    ):
...
class BarPlot(MPLPlot):
    ...
    def _decorate_ticks(self, ax: Axes, name, ticklabels, start_edge,
    end_edge):
...
class PiePlot(MPLPlot):
    ...
    def __init__(self, data, kind=None, **kwargs) -> None:
        data = data.fillna(value=0)
        if (data < 0).any().any():
            raise ValueError(f"{self._kind} plot doesn't allow negative
    values")
    ...
    def _make_plot(self):
        colors = self._get_colors(num_colors=len(self.data),
    color_kwds="colors")
