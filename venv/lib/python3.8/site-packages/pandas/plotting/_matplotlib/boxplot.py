...
class BoxPlot(LinePlot):
    ...
    def __init__(self, data, return_type: str = "axes", **kwargs) -> None:
        ...
        if return_type not in self._valid_return_types:
            raise ValueError("return_type must be {None, 'axes', 'dict',
    'both'}")
    ...
    def _validate_color_args(self):
        ...
        colors = get_standard_colors(num_colors=3, colormap=self.colormap,
    color=None)
    ...
    def _make_plot(self):
        if self.subplots:
            ...
            for i, (label, y) in enumerate(self._iter_data(data=data)):
                ...
                # When by is applied, show title for subplots to know which
    group it is
    ...
    right input
                if self.by is not None:
                    ...
                    # When `by` is assigned, the ticklabels will become unique
    grouped
                    # values, instead of label which is used as subtitle in
    this case.
                    ticklabels = [
                        pprint_thing(col) for col in
    self.data.columns.levels[0]
                    ]
                else:
    ...
    **kwds,
    ...
    if subplots is True:
        ...
        for (key, group), ax in zip(grouped, axes):
            d = group.boxplot(
                ax=ax, column=column, fontsize=fontsize, rot=rot, grid=grid,
    **kwds
            )
    ...
    wspace=0.2)
        ...
        # GH 16748, DataFrameGroupby fails when subplots=False and `column`
    argument
        # is assigned, and in this case, since `df` here becomes MI after
    groupby,
        # so we need to couple the keys (grouped values) and column (original
    df
