...
def pytest_addoption(parser) -> None:
    parser.addoption("--skip-slow", action="store_true", help="skip slow
    tests")
    parser.addoption("--skip-network", action="store_true", help="skip network
    tests")
    ...
    parser.addoption("--only-slow", action="store_true", help="run only slow
    tests")
    parser.addoption(
        "--strict-data-files",
        action="store_true",
...
def ignore_doctest_warning(item: pytest.Item, path: str, message: str) -> None:
    ...
    item: pytest.Item
        pytest test item.
    path: str
        Module path to Python object, e.g.
    "pandas.core.frame.DataFrame.append". A
        warning will be filtered when item.name ends with in given path. So it
    is
...
def pytest_collection_modifyitems(items, config):
    ...
    # Warnings from doctests that can be ignored; place reason in comment
    above.
    ...
    ignored_doctest_warnings = [
        ...
        ("dtypes.common.is_extension_type", "'is_extension_type' is
    deprecated"),
    ...
    for item in items:
        if config.getoption("--doctest-modules") or config.getoption(
            ...
            # autouse=True for the add_doctest_imports can lead to expensive
    teardowns
    ...
    "arraymanager"
        ...
        for (mark, kwd, skip_if_found, arg_name) in marks:
            if kwd in item.keywords:
                # If we're skipping, no need to actually add the marker or
    look for
                # other markers
                if skip_if_found:
                    item.add_marker(pytest.mark.skip(f"skipping due to
    {arg_name}"))
                    break
...
# Common arguments
# ----------------------------------------------------------------
@pytest.fixture(params=[0, 1, "index", "columns"], ids=lambda x:
    f"axis={repr(x)}")
def axis(request):
    """
...
def index_or_series(request):
    ...
    List item 0 has incompatible type "Type[Series]"; expected
    "Type[PandasObject]"
...
@pytest.fixture(params=[Index, Series, pd.array], ids=["index", "series",
    "array"])
...
@pytest.fixture(params=[Index, Series, DataFrame, pd.array], ids=lambda x:
    x.__name__)
...
def multiindex_year_month_day_dataframe_random_data():
    ...
    ymd = tdf.groupby([lambda x: x.year, lambda x: x.month, lambda x:
    x.day]).sum()
    # use Int64Index, to make sure things work
    ymd.index = ymd.index.set_levels([lev.astype("i8") for lev in
    ymd.index.levels])
...
def multiindex_dataframe_random_data(
    ...
    return DataFrame(
        np.random.randn(10, 3), index=index, columns=Index(["A", "B", "C"],
    name="exp")
    )
...
def _create_mi_with_dt64tz_level():
    ...
    return MultiIndex.from_product(
        [[1, 2], ["a", "b"], pd.date_range("20130101", periods=3,
    tz="US/Eastern")],
        names=["one", "two", "three"],
    )
...
indices_dict = {
    ...
    "string-python": Index(pd.array(tm.makeStringIndex(100),
    dtype="string[python]")),
}
if has_pyarrow:
...
@pytest.fixture(
    params=[
        key for key in indices_dict if not isinstance(indices_dict[key],
    MultiIndex)
    ]
)
    ...
    def deco(*args):
        ...
        if not os.path.exists(path):
            if strict_data_files:
                raise ValueError(
                    f"Could not find file {path} and --strict-data-files is
    set."
                )
            else:
...
if zoneinfo is not None:
    TIMEZONES.extend([zoneinfo.ZoneInfo("US/Pacific"),
...
@pytest.fixture(
    params=[
        "string[python]",
        pytest.param(
            "string[pyarrow]", marks=td.skip_if_no("pyarrow",
    min_version="1.0.0")
        ),
    ]
)
...
_any_skipna_inferred_dtype = [
    ("string", ["a", np.nan, "c"]),
    ("string", ["a", pd.NA, "c"]),
    ("mixed", ["a", pd.NaT, "c"]),  # pd.NaT not considered valid by
    is_string_array
...
def using_copy_on_write() -> bool:
    ...
    """
    return pd.options.mode.copy_on_write and pd.options.mode.data_manager ==
    "block"
