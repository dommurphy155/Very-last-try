...
def _preparse(
    source: str,
    f=_compose(
        _replace_locals, _replace_booleans, _rewrite_assign,
    clean_backtick_quoted_toks
    ),
) -> str:
    """
...
def _op_maker(op_class, op_symbol):
    ...
    def f(self, node, *args, **kwargs):
        """
        Return a partial function with an Op subclass with an operator already
    passed.
...
class BaseExprVisitor(ast.NodeVisitor):
        ...
        if isinstance(node, str):
            ...
            try:
                ...
                if any(iskeyword(x) for x in clean.split()):
                    e.msg = "Python keyword not valid identifier in numexpr
    query"
                raise e
    ...
    def visit_Subscript(self, node, **kwargs):
        ...
        try:
            ...
            # an Op instance
            lhs = pd_eval(
                value, local_dict=self.env, engine=self.engine,
    parser=self.parser
            )
            v = lhs[result]
    ...
    "__call__":
        ...
        if res is None:
            ...
            raise ValueError(
                f"Invalid function call {node.func.id}"  # type:
    ignore[attr-defined]
            )
        ...
        if isinstance(res, FuncNode):
            ...
            for key in node.keywords:
                if not isinstance(key, ast.keyword):
                    # error: "expr" has no attribute "id"
                    raise ValueError(
                        "keyword error in function call "  # type:
    ignore[attr-defined]
                        f"'{node.func.id}'"
                    )
    ...
    def visit_Compare(self, node, **kwargs):
        ...
        for op, comp in zip(ops, comps):
            new_node = self.visit(
                ast.Compare(comparators=[comp], left=left,
    ...
    def visit_BoolOp(self, node, **kwargs):
        def visitor(x, y):
            ...
            op, op_class, lhs, rhs = self._maybe_transform_eq_ne(node, lhs,
    rhs)
