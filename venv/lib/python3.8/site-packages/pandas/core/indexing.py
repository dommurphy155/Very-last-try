...
class IndexingMixin:
    ...
    the
        ...
        - A tuple of row and column indexes. The tuple elements consist of one
    of the
    ...
    def at(self) -> _AtIndexer:
        ...
        Notes
        -----
        See:ref:`Fast scalar value getting and setting
    <indexing.basics.get_value>`
        for more details.
...
class _LocationIndexer(NDFrameIndexerBase):
    ...
    def _get_setitem_indexer(self, key):
        ...
        if isinstance(ax, MultiIndex) and self.name != "iloc" and
    is_hashable(key):
    ...
    def _maybe_mask_setitem_value(self, indexer, value):
        ...
        same length as obj, we treat this as obj.iloc[mask] =
    series_or_frame[mask],
        ...
        if (
            ...
            if com.is_bool_indexer(pi) and len(value) == len(pi):
                ...
                if is_scalar_indexer(icols, self.ndim - 1) and ndim == 1:
                    ...
                    if len(newkey) == 0:
                        # FIXME: kludge for
    test_loc_setitem_boolean_mask_allfalse
                        # TODO(GH#45333): may be fixed when deprecation is
    enforced
    ...
    test_setitem_loc_only_false_indexer_dtype_changed
                            #  TODO(GH#45333): may be fixed when deprecation
    is enforced
                            value = value.iloc[:0]
                        else:
                            # test_loc_setitem_ndframe_values_alignment
    ...
    def _ensure_listlike_indexer(self, key, axis=None, value=None):
        """
        Ensure that a list-like of column labels are all present by adding
    them if
    ...
    def _convert_tuple(self, key: tuple) -> tuple:
        ...
        keyidx = [self._convert_to_indexer(k, axis=i) for i, k in
    enumerate(key)]
        return tuple(keyidx)
    ...
    def _getitem_lowerdim(self, tup: tuple):
        ...
        if (
            ...
            # Note: in all extant test cases, replacing the slice condition
    with
            #  `all(is_hashable(x) or com.is_null_slice(x) for x in tup)`
            #  is equivalent.
            #  (see the other place where we call
        ...
        for i, key in enumerate(tup):
            if is_label_like(key):
                ...
                if section.ndim == self.ndim:
                    ...
                    #  rules out having DataFrame here, so we dont need to
    worry
                    #  about transposing.
    ...
    def _getitem_nested_tuple(self, tup: tuple):
        ...
        if len(tup) > self.ndim:
            ...
            if all(is_hashable(x) or com.is_null_slice(x) for x in tup):
                # GH#10521 Series should reduce MultiIndex dimensions instead
    of
                #  DataFrame, IndexingError is not raised when
    ...
    self)._handle_lowerdim_multi_index_axis0(
        ...
        # this is iterative
        obj = self.obj
        # GH#41369 Loop in reverse order ensures indexing along columns before
    rows
...
class _LocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: int):
        ...
        if isinstance(key, slice) and (
            ...
            raise TypeError(f"{key}: boolean values can not be used in a
    slice")
    ...
    def _multi_take_opportunity(self, tup: tuple) -> bool:
        ...
        Currently the limit is that all axes being indexed, must be indexed
    with
        list-likes.
    ...
    def _convert_to_indexer(self, key, axis: int):
        ...
        if is_scalar(key) or (isinstance(labels, MultiIndex) and
    is_hashable(key)):
...
class _iLocIndexer(_LocationIndexer):
    ...
    def _validate_key(self, key, axis: int):
        ...
        if isinstance(key, slice):
            ...
            raise ValueError(f"Can only index by location with a
    [{self._valid_types}]")
    ...
    def _has_valid_setitem_indexer(self, indexer) -> bool:
        ...
        if isinstance(indexer, ABCDataFrame):
            warnings.warn(
                "DataFrame indexer for .iloc is deprecated and will be removed
    in "
                "a future version.\n"
                "consider using .loc with a DataFrame indexer for automatic
    alignment.",
                FutureWarning,
                    ...
                    if self.ndim > 1 and i == info_axis:
                        ...
                        if com.is_null_slice(indexer[0]):
                            ...
                            if not isinstance(value, ABCSeries):
                                # if not Series (in which case we need to
    align),
    ...
    isinstance(value, dict):
        ...
        if is_list_like_indexer(value) and getattr(value, "ndim", 1) > 0:
            ...
            elif len(ilocs) == 1 and lplane_indexer == len(value) and not
    is_scalar(pi):
    ...
    def _setitem_single_column(self, loc: int, value, plane_indexer):
        ...
        if com.is_null_slice(pi) or com.is_full_slice(pi, len(self.obj)):
            ...
            # set value into the column (first attempting to operate inplace,
    then
        ...
        if can_hold_element(orig_values, new_values) and not len(new_values)
    == 0:
            # Don't issue the warning yet, as we can still trim a few cases
    where
    ...
    def _setitem_single_block(self, indexer, value, name: str):
        ...
        if isinstance(indexer, tuple):
            ...
            indexer = maybe_convert_ix(*indexer)  # e.g.
            ...
            if isinstance(value, ABCSeries):
                ...
                if is_list_like_indexer(value):
                    ...
                    if len(value) != len(self.obj.columns):
                        raise ValueError("cannot set a row with mismatched
    columns")
    ...
    def _align_series(self, indexer, ser: Series, multiindex_indexer: bool =
    False):
        """
        Parameters
        ----------
    ...
    GH#45778
                ...
                ser_values = ser.reindex(obj.axes[0][indexer[0]],
    copy=True)._values
                ...
                if len(indexer) > 1 and not multiindex_indexer:
                    ...
                    ser_values = (
                        np.tile(ser_values, len_indexer).reshape(len_indexer,
    -1).T
                    )
...
class _AtIndexer(_ScalarAccessIndexer):
    ...
    def __getitem__(self, key):
        ...
        if self.ndim == 2 and not self._axes_are_unique:
            ...
            if not isinstance(key, tuple) or not all(is_scalar(x) for x in
    key):
                raise ValueError("Invalid call for scalar access (getting)!")
    ...
    if isinstance(key, slice):
        ...
        if idx._supports_partial_string_indexing:
            try:
                ...
                warnings.warn(
                    "Indexing a DataFrame with a datetimelike index using a
    single "
                    "string to slice the rows, like `frame[string]`, is
    deprecated "
