...
class Resampler(BaseGroupBy, PandasObject):
    ...
    def transform(self, arg, *args, **kwargs):
        ...
        """
        return self._selected_obj.groupby(self.groupby).transform(arg, *args,
    **kwargs)
    ...
    def _gotitem(self, key, ndim: int, subset=None):
        ...
        if subset is None:
            subset = self.obj
        grouped = get_groupby(
            subset, by=None, grouper=grouper, axis=self.axis,
    ...
    def _groupby_and_aggregate(self, how, *args, **kwargs):
        ...
        grouped = get_groupby(
            obj, by=None, grouper=grouper, axis=self.axis,
    group_keys=self.group_keys
        )
        ...
        try:
            ...
            # test_apply_to_one_column_of_df the function being applied
    references
            #  a DataFrame column, but aggregate_item_by_item operates
    column-wise
            #  on Series, raising AttributeError or KeyError
            #  (depending on whether the column lookup uses
    getattr/__getitem__)
            result = grouped.apply(how, *args, **kwargs)
    ...
    def _apply_loffset(self, result):
        ...
        if needs_offset:
            # error: Cannot determine type of 'loffset'
            result.index = result.index + self.loffset  # type:
    ignore[has-type]
    ...
    def quantile(self, q=0.5, **kwargs):
        ...
        --------
        Series.quantile
            Return a series, where the index is q and the values are the
    quantiles.
        DataFrame.quantile
...
def _add_downsample_kernel(
        ...
        def f(
            ...
            **kwargs,
            ...
            if numeric_only is lib.no_default and name != "sum":
                # For DataFrameGroupBy, set it to be False for methods other
    than `sum`.
                numeric_only = False
    ...
    signatures
        def f(  # type: ignore[misc]
            self, numeric_only: bool | lib.NoDefault = lib.no_default, *args,
    **kwargs
        ):
            nv.validate_resampler_func(name, args, kwargs)
...
class _GroupByMixin(PandasObject):
    ...
    def __init__(self, obj, parent=None, groupby=None, key=None, **kwargs) ->
    None:
...
class DatetimeIndexResampler(Resampler):
    ...
    def _downsample(self, how, **kwargs):
        ...
        **kwargs: kw args passed to how function
        ...
        result = obj.groupby(self.grouper, axis=self.axis).aggregate(how,
    **kwargs)
...
def get_resampler_for_grouping(
    groupby, rule, how=None, fill_method=None, limit=None, kind=None, on=None,
    **kwargs
):
    """
...
class TimeGrouper(Grouper):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if convention not in {None, "start", "end", "e", "s"}:
            raise ValueError(f"Unsupported value {convention} for
    `convention`")
        ...
        rule = freq.rule_code
        if rule in end_types or ("-" in rule and rule[: rule.find("-")] in
    end_types):
            if closed is None:
                closed = "right"
            if label is None:
        ...
        if origin in ("epoch", "start", "start_day", "end", "end_day"):
            ...
            try:
                ...
                raise ValueError(
                    "'origin' should be equal to 'epoch', 'start',
    'start_day', "
                    "'end', 'end_day' or "
                    f"should be a Timestamp convertible type. Got '{origin}'
    instead."
                ) from err
    ...
    #31809)
        if base is not None and offset is not None:
            raise ValueError("'offset' and 'base' cannot be present at the
    same time")
    ...
    name=ax.name)
        ...
        # GH 25758: If DST lands at midnight (e.g. 'America/Havana'), user
    feedback
    ...
    def _get_time_delta_bins(self, ax: TimedeltaIndex):
        ...
        if not len(ax):
            binner = labels = TimedeltaIndex(data=[], freq=self.freq,
    name=ax.name)
            return binner, [], labels
    ...
    len(ax))
        ...
        if isinstance(self.freq, Tick):
            ...
            start_offset = Period(start, self.freq) - Period(p_start,
    self.freq)
...
def _take_new_index(
    obj: NDFrameT, indexer: npt.NDArray[np.intp], new_index: Index, axis: int
    = 0
) -> NDFrameT:
    ...
    "NDFrameT")
        ...
        new_mgr = obj._mgr.reindex_indexer(new_axis=new_index,
    indexer=indexer, axis=1)
...
def _get_timestamp_range_edges(
    ...
    is None):
            raise ValueError("The origin must have the same timezone as the
    index.")
        elif origin == "epoch":
...
def _asfreq_compat(index: DatetimeIndex | PeriodIndex | TimedeltaIndex, freq):
    ...
    if len(index) != 0:
        ...
        raise ValueError(
            "Can only set arbitrary freq for empty DatetimeIndex or
    TimedeltaIndex"
        )
    new_index: Index
    if isinstance(index, PeriodIndex):
        ...
        new_index = DatetimeIndex([], dtype=index.dtype, freq=freq,
    name=index.name)
    elif isinstance(index, TimedeltaIndex):
        new_index = TimedeltaIndex([], dtype=index.dtype, freq=freq,
    name=index.name)
