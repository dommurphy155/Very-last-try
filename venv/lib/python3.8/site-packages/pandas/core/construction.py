...
def array(
    ...
    dtype: str, np.dtype, or ExtensionDtype, optional
        ...
        dtype or an extension type registered with pandas using:meth:`pandas.api.extensions.register_extension_dtype`.

        If not specified, there are two possibilities:

        1. When `data` is a:class:`Series`,:class:`Index`, or:class:`ExtensionArray`, the `dtype` will be taken
           from the data.
        ...
        *not* used for inferring the array type. This is because
        ...
        ============================== =======================================:class:`pandas.Interval`:class:`pandas.arrays.IntervalArray`:class:`pandas.Period`:class:`pandas.arrays.PeriodArray`:class:`datetime.datetime`:class:`pandas.arrays.DatetimeArray`:class:`datetime.timedelta`:class:`pandas.arrays.TimedeltaArray`:class:`int`:class:`pandas.arrays.IntegerArray`:class:`float`:class:`pandas.arrays.FloatingArray`:class:`str`:class:`pandas.arrays.StringArray` or:class:`pandas.arrays.ArrowStringArray`:class:`bool`:class:`pandas.arrays.BooleanArray`
        ============================== =======================================

        The ExtensionArray created when the scalar type is:class:`str` is
    determined by
        ``pd.options.mode.string_storage`` if the dtype is not explicitly
    given.
    ...
    Examples
    --------
    If a dtype is not specified, pandas will infer the best dtype from the
    values.
    ...
    If pandas does not infer a dedicated extension type a:class:`arrays.PandasArray` is returned.
    ...
    if dtype is None and isinstance(data, (ABCSeries, ABCIndex,
    ExtensionArray)):
    ...
    if dtype is None:
        ...
        if inferred_dtype == "period":
            period_data = cast(Union[Sequence[Optional[Period]],
    AnyArrayLike], data)
    ...
    pd.options.mode.string_storage
            return StringDtype().construct_array_type()._from_sequence(data,
    copy=copy)
...
def sanitize_array(
    ...
    *,
    ...
    DataFrame constructor, as the dtype keyword there may be interpreted as
    only
    ...
    if not is_list_like(data):
        if index is None:
            raise ValueError("index must be specified when data is not
    list-like")
    ...
    if isinstance(data, np.ndarray):
        ...
        if dtype is not None and is_float_dtype(data.dtype) and
    ...
    Series(arr) or "
                ...
                if not raise_cast_failure:
                    ...
                    warnings.warn(
                        "In a future version, passing float-dtype values and
    an "
                        "integer dtype to DataFrame will retain floating dtype
    "
                        "if they cannot be cast losslessly (matching Series
    behavior). "
    ...
    DataFrame(data).astype(dtype)",
                    ...
                    #  ignore the dtype for DataFrame, and _do_ cast even
    though
                    #  it is lossy.
                ...
                # TODO: where is the discussion that documents the reason for
    this?
                subarr = np.array(data, copy=copy)
        else:
...
def range_to_ndarray(rng: range) -> np.ndarray:
    ...
    try:
        ...
        if (rng.start >= 0 and rng.step > 0) or (rng.stop >= 0 and rng.step <
    0):
            try:
    ...
    *,
    ...
    if getattr(result, "ndim", 0) == 0:
        ...
        if is_object_dtype(dtype) and isinstance(dtype, ExtensionDtype):
            ...
            # error: Argument "dtype" to "asarray_tuplesafe" has incompatible
    type
            # "Union[dtype[Any], ExtensionDtype, None]"; expected "Union[str,
            # dtype[Any], None]"
            result = com.asarray_tuplesafe(data, dtype=dtype)  # type:
    ignore[arg-type]
    return result
...
def _try_cast(
    ...
    if dtype is None:
        ...
        if is_ndarray:
            ...
            shape = (len(arr),)
        return lib.ensure_string_array(arr, convert_na_value=False,
    copy=copy).reshape(
            shape
        )
    ...
    try:
        ...
        if raise_cast_failure:
            ...
            warnings.warn(
                ...
                "passed to 'DataFrame', either all columns will be cast to
    that "
...
def is_empty_data(data: Any) -> bool:
    ...
    is_list_like_without_dtype = is_list_like(data) and not hasattr(data,
    "dtype")
