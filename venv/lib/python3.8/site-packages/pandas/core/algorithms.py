...
def _ensure_data(values: ArrayLike) -> np.ndarray:
    ...
    if is_object_dtype(values.dtype):
        ...
        if not values._hasna:
            # No pd.NAs -> We can avoid an object-dtype cast (and copy)
    GH#41816
        ...
        # Note: checking `values.dtype == "float128"` raises on Windows and
    32bit
...
def _ensure_arraylike(values) -> ArrayLike:
    ...
    if not is_array_like(values):
        ...
        if inferred in ["mixed", "string", "mixed-integer"]:
            # "mixed-integer" to ensure we do not cast ["ss", 42] to str
    GH#22160
...
def isin(comps: AnyArrayLike, values: AnyArrayLike) -> npt.NDArray[np.bool_]:
    ...
    if not isinstance(values, (ABCIndex, ABCSeries, ABCExtensionArray,
    np.ndarray)):
        orig_values = values
    ...
    elif needs_i8_conversion(values.dtype) and not
    is_object_dtype(comps_array.dtype):
...
def factorize_array(
    ...
    if values.dtype.kind in ["m", "M"]:
        ...
        #  need to do the same to na_value. We are assuming here that the
    passed
...
def factorize(
    ...
    na_sentinel: int or None, default -1
        ...
        .. deprecated:: 1.5.0
            ...
            will be removed in a future version of pandas. Specify
    use_na_sentinel as
    ...
    use_na_sentinel: bool, default True
        ...
        NaN values will be encoded as non-negative integers and will not drop
    the
    ...
    ``pd.factorize(values)``. The results are identical for methods like:meth:`Series.factorize`.
    ...
    # Other na_sentinel values will not be supported when the deprecation is
    enforced.
    ...
    if (
        ...
        ) and "use_na_sentinel" in
    inspect.signature(values.factorize).parameters:
            ...
            with warnings.catch_warnings():
                ...
                warnings.filterwarnings("ignore", ".*use_na_sentinel.*",
    FutureWarning)
                codes, uniques = values.factorize(na_sentinel=na_sentinel_arg)

    else:
        values = np.asarray(values)  # convert DTA/TDA/MultiIndex
        # TODO: pass na_sentinel=na_sentinel to factorize_array. When sort is
    True and
        #       na_sentinel is None we append NA on the end because safe_sort
    does not
        ...
        if not dropna and not sort and is_object_dtype(values):
            # factorize can now handle differentiating various types of null
    values.
    ...
    if sort and len(uniques) > 0:
        ...
        uniques, codes = safe_sort(
            uniques, codes, na_sentinel=na_sentinel, assume_unique=True,
    verify=False
        )

    if not dropna and sort:
        # TODO: Can remove entire block when na_sentinel=na_sentinel as in
    TODO above
        if na_sentinel is None:
            na_sentinel_arg = -1
        else:
        ...
        if code_is_na.any():
            # na_value is set based on the dtype of uniques, and compat set to
    False is
...
def resolve_na_sentinel(
    ...
    """
    if na_sentinel is not lib.no_default and use_na_sentinel is not
    lib.no_default:
        raise ValueError(
    ...
    if na_sentinel is lib.no_default:
        result = -1 if use_na_sentinel is lib.no_default or use_na_sentinel
    else None
    else:
        if na_sentinel is None:
            msg = (
...
def mode(
    values: ArrayLike, dropna: bool = True, mask: npt.NDArray[np.bool_] | None
    = None
) -> ArrayLike:
    """
...
def checked_add_with_arr(
    ...
    if not mask1.any():
        ...
        to_raise = ((i8max - b2[mask1] < arr[mask1]) & not_nan[mask1]).any()
    or (
...
def searchsorted(
    ...
    # "Union[NumpyValueArrayLike, ExtensionArray]"; expected
    "NumpyValueArrayLike"
    return arr.searchsorted(value, side=side, sorter=sorter)  # type:
    ignore[arg-type]
...
def diff(arr, n: int, axis: int = 0):
    ...
    if not isinstance(dtype, np.dtype):
        ...
        if hasattr(arr, f"__{op.__name__}__"):
            if axis != 0:
                raise ValueError(f"cannot diff {type(arr).__name__} on
    axis={axis}")
            return op(arr, arr.shift(n))
        else:
            warnings.warn(
                "dtype lost in 'diff()'. In the future this will raise a "
                "TypeError. Convert to a suitable dtype prior to calling
    'diff'.",
                FutureWarning,
                stacklevel=find_stack_level(),
            )
...
def safe_sort(
    ...
    if not is_list_like(values):
        raise TypeError(
            "Only list-like objects are allowed to be passed to safe_sort as
    values"
        )
    ...
    "Union[dtype[Any],
        # ExtensionDtype]"; expected "Union[dtype[Any], None, type,
    _SupportsDType, str,
    ...
    if (
        ...
        try:
            ...
            if is_mi:
                # Operate on original object instead of casted array
    (MultiIndex)
...
@overload
def _sort_tuples(values: np.ndarray, original_values: np.ndarray) ->
    np.ndarray:
    ...


@overload
def _sort_tuples(values: np.ndarray, original_values: MultiIndex) ->
    MultiIndex:
    ...
...
def union_with_duplicates(lvals: ArrayLike, rvals: ArrayLike) -> ArrayLike:
    """
    Extracts the union from lvals and rvals with respect to duplicates and
    nans in
    both arrays.
