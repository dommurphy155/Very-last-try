...
_cpython_optimized_decoders = _cpython_optimized_encoders + ("utf-16",
    "utf-32")
...
class StringMethods(NoNewAttributesMixin):
    ...
    def __iter__(self):
        warnings.warn(
            "Columnar iteration over characters will be deprecated in future
    releases.",
            FutureWarning,
    ...
    def _wrap_result(
        ...
        if expand is None:
            ...
            if result and not self._is_string:
                ...
                result = [
                    x * max_len if len(x) == 0 or x[0] is np.nan else x for x
    in result
                ]
    ...
    def _get_series_list(self, others):
        ...
        idx = self._orig if isinstance(self._orig, ABCIndex) else
    self._orig.index
    ...
    def cat(
        ...
        except ValueError as err:  # do not catch TypeError raised by
    _get_series_list
            raise ValueError(
    ...
    sep)
            ...
            all_cols = [
                np.where(nm, na_rep, col) for nm, col in zip(na_masks,
    all_cols)
            ]
    ...
    @Appender(
        ...
        % {
            "side": "first",
            "return": "3 elements containing the string itself, followed by
    two "
            "empty strings",
    ...
    @Appender(
        ...
        % {
            "side": "last",
            "return": "3 elements containing two empty strings, followed by
    the "
            "string itself",
    ...
    specified key.
        ...
        Extract element from lists, tuples, dict, or strings in each element
    in the
        Series/Index.
    ...
    regex is
        ...
        na: scalar, optional
            ...
            array. For object-dtype, ``numpy.nan`` is used. For
    ``StringDtype``,
            ``pandas.NA`` is used.
    ...
    re.search.
        Series.str.startswith: Test if the start of each string element
    matches a
            pattern.
    ...
    np.NaN])
        ...
        Note in the following example one might expect only `s2[1]` and
    `s2[3]` to
    ...
    ``StringDtype``,
        ...
        """
        result = self._data.array._str_match(pat, case=case, flags=flags,
    na=na)
        return self._wrap_result(result, fill_value=na, returns_string=False)
    ...
    string
        ...
        extract: Extract matched groups.
        """
        result = self._data.array._str_fullmatch(pat, case=case, flags=flags,
    na=na)
        return self._wrap_result(result, fill_value=na, returns_string=False)
    ...
    def replace(
        ...
        flags: int, default 0 (no flags)
            Regex module flags, e.g. re.IGNORECASE. Cannot be set if `pat` is
    a compiled
            regex.
        regex: bool, default True
            Determines if the passed-in pattern is a regular expression:
    ...
    compiled
        ...
        compiled regex. Use of `case`, `flags`, or `regex=False` with a
    compiled
        regex will raise an error.

        Examples
        --------
        When `pat` is a string and `regex` is True (the default), the given
    `pat`
    ...
    regex=False)
        ...
        When `repl` is a callable, it is called on every `pat` using:func:`re.sub`. The callable should expect one positional argument
    ...
    "
                ...
                if len(pat) == 1:
                    msg += (
                        " In addition, single character regular expressions
    will "
    ...
    regex=False")
        ...
        # The current behavior is to treat single character patterns as
    literal strings,
    ...
    See Also
    --------
    Series.str.remove%(other_side)s: Remove a %(other_side)s from an object
    series.
    ...
    @Appender(
        _shared_docs["str_removefix"] % {"side": "prefix", "other_side":
    "suffix"}
    )
    @forbid_nonstring_types(["bytes"])
    ...
    @Appender(
        _shared_docs["str_removefix"] % {"side": "suffix", "other_side":
    "prefix"}
    )
    @forbid_nonstring_types(["bytes"])
    ...
    string.whitespace)
        ...
        break_long_words: bool, optional
            ...
            that no lines are longer than width. If it is false, long words
    will
    ...
    def count(self, pat, flags=0):
        ...
        pattern is repeated in each of the string elements of the:class:`~pandas.Series`.
    ...
    def endswith(
        ...
        Series.str.startswith: Same as endswith, but tests the start of
    string.
        Series.str.contains: Tests if string element contains a pattern.
    ...
    matches
        ...
        Regular expressions are supported too. For instance, the search for
    all the
    ...
    def extract(
        ...
        if not expand and regex.groups > 1 and isinstance(self._data,
    ABCIndex):
        ...
        if returns_df:
            ...
            if obj.array.size == 0:
                ...
                result = DataFrame(
                    result_list, columns=columns, index=result_index,
    dtype=result_dtype
                )

        else:
            name = _get_single_group_name(regex)
            result = self._data.array._str_extract(pat, flags=flags,
    expand=returns_df)
    ...
    def extractall(self, pat, flags=0):
        ...
        -------
        DataFrame
            A ``DataFrame`` with one row for each match, and one column for
    each
            group. Its rows have a ``MultiIndex`` with first levels that come
    from
            the subject ``Series``. The last level is named 'match' and
    indexes the
    ...
    def normalize(self, form):
        ...
        For more information on the forms, see the:func:`unicodedata.normalize`.
    ...
    This is equivalent to running the Python string method:meth:`str.%(method)s` for each element of the Series/Index. If a string
    ...
    **Checks for Character Case**
    ...
    isnumeric = _map_and_wrap(
        "isnumeric", docstring=_shared_docs["ismethods"] %
    _doc_args["isnumeric"]
    )
    isdecimal = _map_and_wrap(
        "isdecimal", docstring=_shared_docs["ismethods"] %
    _doc_args["isdecimal"]
    )
