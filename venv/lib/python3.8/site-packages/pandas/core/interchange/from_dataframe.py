...
def _from_dataframe(df: DataFrameXchg, allow_copy=True):
    ...
    if not allow_copy and len(pandas_dfs) > 1:
        raise RuntimeError(
            "To join chunks a copy is required which is forbidden by
    allow_copy=False"
        )
    if len(pandas_dfs) == 1:
...
def categorical_column_to_series(col: Column) -> tuple[pd.Series, Any]:
    ...
    if not categorical["is_dictionary"]:
        raise NotImplementedError("Non-dictionary categoricals not supported
    yet")

    cat_column = categorical["categories"]
    # for mypy/pyright
    assert isinstance(cat_column, PandasColumn), "categories must be a
    PandasColumn"
...
def string_column_to_ndarray(col: Column) -> tuple[np.ndarray, Any]:
    ...
    data = buffer_to_ndarray(data_buff, data_dtype, offset=0,
    length=col.size())

    # Retrieve the offsets buffer containing the index offsets demarcating
    # the beginning and the ending of each string
    offset_buff, offset_dtype = buffers["offsets"]
    # Offsets buffer contains start-stop positions of strings in the data
    buffer,
    # meaning that it has more elements than in the data buffer, do
    `col.size() + 1`
    ...
    if null_kind in (ColumnNullType.USE_BITMASK, ColumnNullType.USE_BYTEMASK):
        assert buffers["validity"], "Validity buffers cannot be empty for
    masks"
        valid_buff, valid_dtype = buffers["validity"]
        null_pos = buffer_to_ndarray(valid_buff, valid_dtype, col.offset,
    col.size())
        if sentinel_val == 0:
            null_pos = ~null_pos
...
def parse_datetime_format_str(format_str, data):
    ...
    if date_meta:
        ...
        if unit == "D":
            ...
            data = (data.astype(np.uint64) * (24 * 60 *
    60)).astype("datetime64[s]")
        elif unit == "m":
...
def buffer_to_ndarray(
    ...
    if column_dtype is None:
        raise NotImplementedError(f"Conversion for {dtype} is not yet
    supported.")

    # TODO: No DLPack yet, so need to construct a new ndarray from the data
    pointer
    # and size in the buffer plus the dtype on the column. Use DLPack as NumPy
    supports
    ...
    if bit_width == 1:
        assert length is not None, "`length` must be specified for a bit-mask
    buffer."
        arr = np.ctypeslib.as_array(data_pointer, shape=(buffer.bufsize,))
        return bitmask_to_bool_ndarray(arr, length, first_byte_offset=offset %
    8)
    else:
...
def set_nulls(
    ...
    allow_modify_inplace: bool, default: True
        Whether to modify the `data` inplace when zero-copy is possible (True)
    or always
    ...
    elif null_kind in (ColumnNullType.USE_BITMASK,
    ColumnNullType.USE_BYTEMASK):
    ...
    col.size())
        ...
        raise NotImplementedError(f"Null kind {null_kind} is not yet
    supported.")
