...
class PandasColumn(Column):
    ...
    def __init__(self, column: pd.Series, allow_copy: bool = True) -> None:
        ...
        if not isinstance(column, pd.Series):
            raise NotImplementedError(f"Columns of type {type(column)} not
    handled yet")
    ...
    def dtype(self) -> tuple[DtypeKind, int, str, str]:
        ...
        if is_categorical_dtype(dtype):
            ...
            raise NotImplementedError("Non-string object dtypes are not
    supported yet")
        else:
            return self._dtype_from_pandasdtype(dtype)

    def _dtype_from_pandasdtype(self, dtype) -> tuple[DtypeKind, int, str,
    str]:
        """
    ...
    handled
        #       datetime and timedelta both map to datetime (is timedelta
    handled?)

        kind = _NP_KINDS.get(dtype.kind, None)
        if kind is None:
            # Not a NumPy dtype. Check if it's a categorical maybe
            raise ValueError(f"Data type {dtype} not supported by interchange
    protocol")

        return kind, dtype.itemsize * 8, dtype_to_arrow_c_fmt(dtype),
    dtype.byteorder
    ...
    ...).
        ...
        if not self.dtype[0] == DtypeKind.CATEGORICAL:
            raise TypeError(
                "describe_categorical only works on a column with categorical
    dtype!"
            )
    ...
    def _get_data_buffer(
        ...
        """
        Return the buffer containing the data and the buffer's associated
    dtype.
        """
        if self.dtype[0] in (
            DtypeKind.INT,
    ...
    later
            ...
            raise NotImplementedError(f"Data type {self._col.dtype} not
    handled yet")
    ...
    mask.
        ...
        try:
            msg = _NO_VALIDITY_BUFFER[null] + " so does not have a separate
    mask"
        except KeyError:
