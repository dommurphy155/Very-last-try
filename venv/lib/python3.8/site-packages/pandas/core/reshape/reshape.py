...
class _Unstacker:
    ...
    def __init__(self, index: MultiIndex, level=-1, constructor=None) -> None:
        ...
        num_rows = np.max([index_level.size for index_level in
    ...
    def _make_selectors(self):
        ...
        if mask.sum() < len(self.index):
            raise ValueError("Index contains duplicate entries, cannot
    reshape")
    ...
    @cache_readonly
    def arange_result(self) -> tuple[npt.NDArray[np.intp],
    ...
    def new_index(self) -> MultiIndex:
        ...
        result_codes = [lab.take(self.compressor) for lab in
    self.sorted_labels[:-1]]
...
def unstack(obj: Series | DataFrame, level, fill_value=None):
    ...
    if isinstance(obj, DataFrame):
        ...
        raise ValueError(
            f"index must be a MultiIndex to unstack, {type(obj.index)} was
    passed"
        )
    else:
...
def _stack_multi_column_index(columns: MultiIndex) -> MultiIndex:
    ...
    # The dtype of each level must be explicitly set to avoid inferring the
    wrong type.
    # See GH-36991.
