...
class _Concatenator:
    ...
    "{type(objs).__name__}"'
        ...
        if join == "outer":
            ...
            raise ValueError(
                "Only can inner (intersect) or outer (union) join the other
    axis"
            )
        ...
        if len(ndims) > 1:
            ...
            non_empties = [
                obj for obj in objs if sum(obj.shape) > 0 or isinstance(obj,
    ABCSeries)
            ]

            if len(non_empties) and (
                keys is None and names is None and levels is None and not
    self.intersect
            ):
                objs = non_empties
                sample = objs[0]
    ...
    def get_result(self):
        ...
        if self._is_series:
            ...
            if self.bm_axis == 0:
                ...
                result = cons(res, index=self.new_axes[0], name=name,
    dtype=res.dtype)
        ...
        # combine block managers
            ...
            new_data = concatenate_managers(
                mgrs_indexers, self.new_axes, concat_axis=self.bm_axis,
    copy=self.copy
            )
            if not self.copy:
    ...
    def _get_new_axes(self) -> list[Index]:
        ...
        return [
            self._get_concat_axis if i == self.bm_axis else
    self._get_comb_axis(i)
            for i in range(ndim)
        ]
...
def _make_concat_multiindex(indexes, keys, levels=None, names=None) ->
    ...
    if not all_indexes_same(indexes) or not all(level.is_unique for level in
    levels):
        codes_list = []
        ...
        if len(names) == len(levels):
            ...
            if not len({idx.nlevels for idx in indexes}) == 1:
                raise AssertionError(
                    "Cannot concat indices that do not have the same number of
    levels"
                )
        ...
        return MultiIndex(
            levels=levels, codes=codes_list, names=names,
    ...
    for hlevel, level in zip(zipped, levels):
        ...
        mask = mapped == -1
        if mask.any():
            raise ValueError(f"Values not found in passed level:
    {hlevel[mask]!s}")
