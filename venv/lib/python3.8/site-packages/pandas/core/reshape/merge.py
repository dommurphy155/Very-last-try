    ...
    for key, lhs in lby.grouper.get_iterator(lby._selected_obj, axis=lby.axis):
        ...
        if rby is None:
            ...
            try:
                ...
                cols = lcols + [r for r in right.columns if r not in
    set(lcols)]
...
def merge_asof(
    ...
    >>> right = pd.DataFrame({"right_val": [1, 2, 3, 6, 7]}, index=[1, 2, 3,
    6, 7])
    >>> right
       right_val
    1          1
...
class _MergeOperation:
    ...
    removed "
                f"in a future version. ({_left.columns.nlevels} levels on the
    left, "
    ...
    def _maybe_restore_index_levels(self, result: DataFrame) -> None:
        ...
        **Note:** This method has side effects. It modifies `result` in-place
        ...
        for name, left_key, right_key in zip(
            ...
            if (
                # Argument 1 to "_is_level_reference" of "NDFrame" has
    incompatible
                # type "Union[Hashable, ExtensionArray, Index, Series]";
    expected
                # "Hashable"
                self.orig_left._is_level_reference(left_key)  # type:
    ignore[arg-type]
                # Argument 1 to "_is_level_reference" of "NDFrame" has
    incompatible
                # type "Union[Hashable, ExtensionArray, Index, Series]";
    ...
    def _maybe_add_join_keys(
        ...
        for i, (name, lname, rname) in enumerate(keys):
            ...
            elif left_indexer is not None and
    ...
    fill_value=lfill)
                ...
                if take_right is None:
                    ...
                    rvals = algos.take_nd(take_right, right_indexer,
    fill_value=rfill)
    ...
    def _get_join_indexers(self) -> tuple[npt.NDArray[np.intp],
    npt.NDArray[np.intp]]:
            ...
            for lk, rk in zip(self.left_on, self.right_on):
                if is_lkey(lk):
                    ...
                    if is_rkey(rk):
                        ...
                        # Then we're either Hashable or a wrong-length
    arraylike,
    ...
    right_keys.append(right._get_label_or_level_values(rk))
                            ...
                            # work-around for merge_asof(right_index=True)
                            right_keys.append(right.index)
                        if lk is not None and lk == rk:  # FIXME: what about
    other NAs?
    ...
    def _maybe_coerce_merge_keys(self) -> None:
        ...
        for lk, rk, name in zip(
            ...
            msg = (
                ...
                f"{rk.dtype} columns. If you wish to proceed you should use
    pd.concat"
            )
    ...
    -> int
                    with np.errstate(invalid="ignore"):
                        if not (lk ==
    lk.astype(rk.dtype))[~np.isnan(lk)].all():
                            warnings.warn(
    ...
    -> int
                    with np.errstate(invalid="ignore"):
                        if not (rk ==
    rk.astype(lk.dtype))[~np.isnan(rk)].all():
                            warnings.warn(
                                "You are merging on int and float "
                                "columns where the float values "
                                "are not equal to their int representation.",
    ...
    bool_types:
                ...
                # unless we are merging non-string-like with string-like
                    ...
                    inferred_left in string_types and inferred_right not in
    string_types
                ) or (
                    inferred_right in string_types and inferred_left not in
    string_types
                ):
                    raise ValueError(msg)

            # datetimelikes must match exactly
            elif needs_i8_conversion(lk.dtype) and not
    needs_i8_conversion(rk.dtype):
                raise ValueError(msg)
            elif not needs_i8_conversion(lk.dtype) and
    needs_i8_conversion(rk.dtype):
                raise ValueError(msg)
    ...
    def _create_cross_configuration(
        ...
        """
        Creates the configuration to dispatch the cross operation to inner
    join,
        e.g. adding a join column and resetting parameters. Join column is
    added
        ...
        -------
            ...
            DataFrames with cross_col, the merge operation set to inner and
    the column
            to join over.
        """
    ...
    def _validate_left_right_on(self, left_on, right_on):
        ...
        if self.how == "cross":
            if (
                ...
                raise MergeError(
                    "Can not pass on, right_on, left_on or set
    right_index=True or "
                    "left_index=True"
                )
        # Hm, any way to make this logic less complicated??
            ...
            if self.left_index and self.right_index:
                ...
                if (
                    ...
                    raise MergeError(f"Data columns not unique:
    {repr(common_cols)}")
            ...
            if self.left_index:
                raise MergeError(
                    'Can only pass argument "left_on" OR "left_index" not
    both.'
                )
            ...
            if self.right_index:
                raise MergeError(
                    'Can only pass argument "right_on" OR "right_index" not
    both.'
                )
    ...
    def _validate(self, validate: str) -> None:
        ...
        if self.right_index:
            ...
            right_unique =
    MultiIndex.from_arrays(self.right_join_keys).is_unique
...
class _AsOfMerge(_OrderedMerge):
    ...
    def _get_merge_keys(self):
        ...
        # note this function has side effects
        (left_join_keys, right_join_keys, join_names) =
    ...
    is_categorical_dtype(rk.dtype):
                    ...
                    msg = (
                        ...
                        f"{repr(rk.dtype)}, both sides category, but not equal
    ones"
                    )
                else:
                    msg = (
    ...
    npt.NDArray[np.intp]]:
        ...
        # values to compare
        left_values = (
            self.left.index._values if self.left_index else
    self.left_join_keys[-1]
        )
        right_values = (
            self.right.index._values if self.right_index else
    self.right_join_keys[-1]
        ...
        if not Index(left_values).is_monotonic_increasing:
            ...
            if isna(left_values).any():
                raise ValueError(f"Merge keys contain null values on {side}
    side")
            else:
    ...
    test_merge_groupby_multiple_column_with_categorical_column
            ...
            # error: Cannot call function of unknown type
            left_by_values = by_type_caster(left_by_values)  # type:
    ignore[operator]
            # error: Cannot call function of unknown type
            right_by_values = by_type_caster(right_by_values)  # type:
    ignore[operator]
...
def _get_single_indexer(
    ...
    left_key, right_key, count = _factorize_keys(join_key, index._values,
    sort=sort)
...
def _left_join_on_index(
    ...
    if len(join_keys) > 1:
        if not (
            isinstance(right_ax, MultiIndex) and len(join_keys) ==
    right_ax.nlevels
        ):
        ...
        left_indexer, right_indexer = _get_single_indexer(jkey, right_ax,
    sort=sort)
...
def _factorize_keys(
    ...
    -------
    np.ndarray[np.intp]
        Left (resp. right if called with `key='right'`) labels, as enumerated
    type.
    np.ndarray[np.intp]
        Right (resp. left if called with `key='right'`) labels, as enumerated
    type.
    int
...
def _sort_labels(
    uniques: np.ndarray, left: npt.NDArray[np.intp], right:
    npt.NDArray[np.intp]
) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.intp]]:
...
def _validate_operand(obj: DataFrame | Series) -> DataFrame:
    if isinstance(obj, ABCDataFrame):
        ...
        raise TypeError(
            f"Can only merge Series or DataFrame objects, a {type(obj)} was
    passed"
        )
...
def _items_overlap_with_suffix(
    ...
    if not is_list_like(suffixes, allow_sets=False):
        warnings.warn(
            f"Passing 'suffixes' as a {type(suffixes)}, is not supported and
    may give "
    ...
    if dups:
        warnings.warn(
            f"Passing 'suffixes' which cause duplicate columns {set(dups)} in
    the "
            f"result is deprecated and will raise a MergeError in a future
    version.",
            FutureWarning,
            stacklevel=find_stack_level(),
        )
