...
@Appender(_shared_docs["melt"] % {"caller": "pd.melt(df, ", "other":
    "DataFrame.melt"})
def melt(
    frame: DataFrame,
    id_vars=None,
    ...
    if value_name in frame.columns:
        warnings.warn(
            "This dataframe has a column name that matches the 'value_name'
    column "
            "name of the resulting Dataframe. "
            "In the future this will raise an error, please set the
    'value_name' "
    ...
    if id_vars is not None:
        if not is_list_like(id_vars):
            id_vars = [id_vars]
        elif isinstance(frame.columns, MultiIndex) and not isinstance(id_vars,
    list):
            raise ValueError(
                "id_vars must be a list of tuples when columns are a
    MultiIndex"
            )
        else:
    ...
    if value_vars is not None:
        ...
        elif isinstance(frame.columns, MultiIndex) and not
    isinstance(value_vars, list):
            raise ValueError(
                "value_vars must be a list of tuples when columns are a
    MultiIndex"
            )
        else:
        ...
        if col_level is not None:
            ...
            idx = algos.unique(frame.columns.get_indexer_for(id_vars +
    value_vars))
    ...
    if var_name is None:
        if isinstance(frame.columns, MultiIndex):
            if len(frame.columns.names) == len(set(frame.columns.names)):
                ...
                var_name = [f"variable_{i}" for i in
    range(len(frame.columns.names))]
        else:
            var_name = [
                frame.columns.name if frame.columns.name is not None else
    "variable"
            ]
    ...
    for col in id_vars:
        ...
        if is_extension_array_dtype(id_data):
            if K > 0:
                ...
                id_data = type(id_data)([], name=id_data.name,
    dtype=id_data.dtype)
        else:
...
def lreshape(data: DataFrame, groups, dropna: bool = True, label=None) ->
    DataFrame:
    """
...
def wide_to_long(
    ...
    _melted = [melt_stub(df, s, i, j, v, sep) for s, v in zip(stubnames,
    value_vars)]
