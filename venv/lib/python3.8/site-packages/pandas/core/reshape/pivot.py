...
def __internal_pivot_table(
    ...
    msg = (
        "pivot_table dropped a column because it failed to aggregate. This
    behavior "
        "is deprecated and will raise in a future version of pandas. Select
    only the "
...
def _compute_grand_margin(data: DataFrame, values, aggfunc, margins_name: str
    = "All"):
...
def _generate_marginal_results(
    table, data, values, rows, cols, aggfunc, observed, margins_name: str =
    "All"
):
    if len(cols) > 0:
    ...
    observed=observed):
                ...
                table_pieces.append(piece)
                # GH31016 this is to calculate margin for each group, and
    assign
...
def _generate_marginal_results_without_values(
    table: DataFrame, data, rows, cols, aggfunc, observed, margins_name: str =
    "All"
):
    if len(cols) > 0:
        # need to "interleave" the margins
        ...
        if len(rows) > 0:
            ...
            margin = data.groupby(level=0, axis=0,
...
def pivot(
    ...
    if values is None:
        ...
        # error: Unsupported operand types for + ("List[Any]" and
    "ExtensionArray")
        ...
        if index is None:
            if isinstance(data.index, MultiIndex):
                # GH 23955
                index_list = [
                    data.index.get_level_values(i) for i in
    range(data.index.nlevels)
                ]
            else:
    ...
    index=multiindex)
    # error: Argument 1 to "unstack" of "DataFrame" has incompatible type
    "Union
...
def crosstab(
    ...
    ABCDataFrame))]
    if pass_objs:
        common_idx = get_objs_combined_axis(pass_objs, intersect=True,
    sort=False)
    ...
    if normalize is not False:
        table = _normalize(
            table, normalize=normalize, margins=margins,
    margins_name=margins_name
        )
...
def _build_names_mapper(
    ...
    """
    Given the names of a DataFrame's rows and columns, returns a set of unique
    row
    and column names and mappers that convert to original names.

    A row or column name is replaced if it is duplicate among the rows of the
    inputs,
    ...
    colnames_mapper: dict[str, str]
        a dictionary with new column names as keys and original column names
    as values
    ...
    dup_names = get_duplicates(rownames) | get_duplicates(colnames) |
    shared_names

    rownames_mapper = {
        f"row_{i}": name for i, name in enumerate(rownames) if name in
    dup_names
    }
    unique_rownames = [
        f"row_{i}" if name in dup_names else name for i, name in
    enumerate(rownames)
    ]

    colnames_mapper = {
        f"col_{i}": name for i, name in enumerate(colnames) if name in
    dup_names
    }
    unique_colnames = [
        f"col_{i}" if name in dup_names else name for i, name in
    enumerate(colnames)
    ]
