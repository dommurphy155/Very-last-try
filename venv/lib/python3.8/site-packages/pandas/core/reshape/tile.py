...
def cut(
    ...
    if not np.iterable(bins):
        ...
        if np.isinf(mn) or np.isinf(mx):
            # GH 24314
            raise ValueError(
                "cannot specify integer `bins` when input data contains
    infinity"
            )
...
def qcut(
    ...
    produce a Categorical object indicating quantile membership for each data
    point.
...
def _format_labels(
    bins, precision: int, right: bool = True, include_lowest: bool = False,
    dtype=None
):
...
def _postprocess_for_cut(fac, bins, retbins: bool, dtype, original):
    ...
    if isinstance(original, ABCSeries):
        fac = original._constructor(fac, index=original.index,
    name=original.name)
