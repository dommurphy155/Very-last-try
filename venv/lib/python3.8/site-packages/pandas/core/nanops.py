    ...
    def __call__(self, f: F) -> F:
        ...
        def _f(*args, **kwargs):
            ...
            if any(self.check(obj) for obj in obj_iter):
                ...
                raise TypeError(
                    f"reduction operation '{f_name}' not allowed for this
    dtype"
                )
            try:
...
def _bn_ok_dtype(dtype: DtypeObj, name: str) -> bool:
    ...
    if not is_object_dtype(dtype) and not needs_i8_conversion(dtype):
        ...
        # unlike numpy which implements pairwise summation, which has
    O(log(n)) loss
...
def _get_values(
    ...
    values = extract_array(values, extract_numpy=True)  # type:
    ignore[assignment]
...
def _na_for_min_count(values: np.ndarray, axis: int | None) -> Scalar |
    np.ndarray:
    """
...
def maybe_operate_rowwise(func: F) -> F:
    ...
    def newfunc(values: np.ndarray, *, axis: int | None = None, **kwargs):
        if (
            ...
            # only takes this path for wide arrays (long dataframes), for
    threshold see
            #
    https://github.com/pandas-dev/pandas/pull/43311#issuecomment-974891737
            ...
            if kwargs.get("mask") is not None:
                ...
                results = [
                    func(arrs[i], mask=mask[i], **kwargs) for i in
    range(len(arrs))
                ]
            else:
...
def nanmedian(values, *, axis=None, skipna=True, mask=None):
    ...
    encountered")
            ...
            # empty set so return nans of shape "everything but the passed
    axis"
            # since "axis" is where the reduction would occur if we had a
    nonempty
            # array
...
def nanargmax(
    ...
    *,
    ...
    array([2, 2, 1, 1])
    """
    values, mask, _, _, _ = _get_values(values, True, fill_value_typ="-inf",
    mask=mask)
...
def nanprod(
    ...
    *,
    ...
    # error: Incompatible return value type (got "Union[ndarray, float]",
    expected
    # "float")
...
@disallow("M8", "m8")
def nancorr(
    a: np.ndarray, b: np.ndarray, *, method="pearson", min_periods: int | None
    = None
) -> float:
    """
    a, b: ndarrays
        ...
        if is_integer_dtype(x) or is_bool_dtype(x):
            ...
            try:
                ...
                try:
                    ...
                    raise TypeError(f"Could not convert {x} to numeric") from
    err
            else:
...
def na_accum_func(values: ArrayLike, accum_func, *, skipna: bool) -> ArrayLike:
    ...
    accum_func: {np.cumprod, np.maximum.accumulate, np.cumsum,
    ...
    if values.dtype.kind in ["m", "M"]:
        ...
        if isinstance(values.dtype, np.dtype):
            ...
            npdtype = orig_dtype if isinstance(orig_dtype, np.dtype) else
    "M8[ns]"
            # Item "type" of "Union[Type[ExtensionArray], Type[ndarray[Any,
    Any]]]"
