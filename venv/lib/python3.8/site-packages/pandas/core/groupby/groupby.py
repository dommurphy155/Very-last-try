...
_apply_docs = {
    ...
    like ``agg`` or ``transform``. Pandas offers a wide range of method that
    will
    ...
    In the above, the groups are not part of the index. We can have them
    included
...
-----
See more `here
<https://pandas.pydata.org/pandas-docs/stable/user_guide/groupby.html#piping-fu
    nction-calls>`_
...
.. deprecated:: 1.5.0
    ...
    be performed in a future version of pandas. You can apply ``.to_numpy()``
    to the
...
class BaseGroupBy(PandasObject, SelectionMixin[NDFrameT],
    ...
    def _get_indices(self, names):
        ...
        if isinstance(index_sample, tuple):
            if not isinstance(name_sample, tuple):
                msg = "must supply a tuple to get_group with multiple grouping
    keys"
    ...
    @Substitution(
        ...
        To get the difference between each groups maximum and minimum value in
    one
        pass, you can do
...
class GroupBy(BaseGroupBy[NDFrameT]):
    ...
    **Attributes**
    ...
    def _make_wrapper(self, name: str) -> Callable:
        ...
        with self._group_selection_context():
            ...
            if not isinstance(f, types.MethodType):
                ...
                # (got "NDFrameT", expected "Callable[..., Any]") 
    [return-value]
                return cast(Callable, self.apply(lambda self: getattr(self,
    name)))
        ...
        def wrapper(*args, **kwargs):
            ...
            if self._selected_obj.ndim != 1 and self.axis != 1 and result.ndim
    != 1:
                missing =
    self._obj_with_exclusions.columns.difference(result.columns)
                if len(missing) > 0:
    ...
    def _set_group_selection(self) -> None:
        ...
        groupers = [g.name for g in grp.groupings if g.level is None and
    g.in_axis]
    ...
    def _set_result_index_ordered(
        ...
        result = result.set_axis(original_positions, axis=self.axis,
    copy=False)
        result = result.sort_index(axis=self.axis)
        if self.grouper.has_dropped_na:
            # Add back in any missing rows due to dropna - index here is
    integral
    ...
    def _wrap_aggregated_output(
        ...
        if isinstance(output, (Series, DataFrame)):
            # We get here (for DataFrameGroupBy) if we used
    Manager.grouped_reduce,
    ...
    def _resolve_numeric_only(
        ...
        For SeriesGroupBy we want the default to be False (to match Series
    behavior).
    ...
    dropped
                ...
                if len(obj.columns) and not len(check.columns) and not
    obj.empty:
                    numeric_only = False
        ...
        if numeric_only and self.obj.ndim == 1 and not
    is_numeric_dtype(self.obj.dtype):
            # GH#47500
            warnings.warn(
                f"{type(self).__name__}.{how} called with "
                f"numeric_only={numeric_only} and dtype {self.obj.dtype}. This
    will "
    ...
    def _maybe_warn_numeric_only_depr(
        self, how: str, result: DataFrame | Series, numeric_only: bool |
    lib.NoDefault
    ) -> None:
    ...
    def _numba_prep(self, data):
        ...
        if len(self.grouper.groupings) > 1:
            raise NotImplementedError(
                "More than 1 grouping labels are not supported with
    engine='numba'"
            )
        # GH 46867
    ...
    def _numba_agg_general(
        ...
        *aggregator_args,
        ...
        """
        Perform groupby with a standard numerical aggregation function (e.g.
    mean)
        with Numba.
        """
        if not self.as_index:
    ...
    @final
    def _transform_with_numba(self, data, func, *args, engine_kwargs=None,
    **kwargs):
        """
        Perform groupby transform routine with the numba engine.

        This routine mimics the data splitting routine of the DataSplitter
    class
        to generate the indices of each group in the sorted data and then
    passes the
        ...
        # result values needs to be resorted to their original positions since
    we
        # evaluated the data sorted by group
        return result.take(np.argsort(sorted_index), axis=0)

    @final
    def _aggregate_with_numba(self, data, func, *args, engine_kwargs=None,
    **kwargs):
        """
    ...
    def apply(self, func, *args, **kwargs) -> NDFrameT:
        # GH#50538
        is_np_func = func in com._cython_table and func not in
    com._builtin_table
        orig_func = func
        ...
        if isinstance(func, str):
            if hasattr(self, func):
                ...
                if callable(res):
                    ...
                    raise ValueError(f"Cannot pass arguments to property
    {func}")
                return res
        ...
        with option_context("mode.chained_assignment", None):
            try:
                ...
                new_msg = (
                    f"The operation {orig_func} failed on a column. If any
    error is "
    ...
    "
                ...
                with self._group_selection_context():
                    ...
                    new_msg = (
                        f"The operation {orig_func} failed on a column. If any
    error "
                        f"is raised, this will raise an exception in a future
    version "
                        f"of pandas. Drop these columns to avoid this warning."
                    )
                    with rewrite_warning(
    ...
    def _python_apply_general(
        ...
        not_indexed_same: bool, optional
            When specified, overrides the value of not_indexed_same. Apply
    behaves
        ...
        if (not not_indexed_same and self.group_keys is lib.no_default) and
    not (
            is_transform or is_empty_agg
        ):
            # We've detected value-dependent behavior: the result's index
    depends on
            ...
            # We want to behave as if `self.group_keys=False` when
    reconstructing
    ...
    @final
    def _python_agg_general(self, func, *args, raise_on_typeerror=False,
    **kwargs):
        func = com.is_builtin_func(func)
    ...
    def _agg_py_fallback(
        ...
        if isinstance(values, Categorical):
            ...
            res_values = type(values)._from_sequence(res_values,
    dtype=values.dtype)
    ...
    def _cython_agg_general(
        ...
        **kwargs,
        ...
        numeric_only_bool = self._resolve_numeric_only(how, numeric_only,
    axis=0)
        ...
        if numeric_only_bool:
            if is_ser and not is_numeric_dtype(self._selected_obj.dtype):
                ...
                if how in ["any", "all"]:
                    kwd_name = "bool_only"
                raise NotImplementedError(
                    f"{type(self).__name__}.{how} does not implement
    {kwd_name}."
                )
            elif not is_ser:
    ...
    @final
    def _transform(self, func, *args, engine=None, engine_kwargs=None,
    **kwargs):
        ...
        if not isinstance(func, str):
            ...
            raise ValueError(msg)
        elif func in base.cythonized_kernels or func in
    base.transformation_kernels:
            ...
            # GH#30918 Use _transform_fast only when we know func is an
    aggregation
    ...
    def _wrap_transform_fast_result(self, result: NDFrameT) -> NDFrameT:
        ...
        result = result.reindex(self.grouper.result_index, axis=self.axis,
    copy=False)
    ...
    def _bool_agg(self, val_test: Literal["any", "all"], skipna: bool):
        ...
        def objs_to_bool(vals: ArrayLike) -> tuple[np.ndarray, type]:
            if is_object_dtype(vals.dtype):
                ...
                if skipna:
                    func = np.vectorize(
                        lambda x: bool(x) if not isna(x) else True,
    otypes=[bool]
                    )
                    vals = func(vals)
                else:
        ...
        def result_to_bool(
            ...
            if nullable:
                return BooleanArray(result.astype(bool, copy=False), result ==
    -1)
            else:
    ...
    def any(self, skipna: bool = True):
        ...
        -------
        Series or DataFrame
            DataFrame or Series of boolean values, where a value is True if
    any element
    ...
    ``nogil``
              and ``parallel`` dictionary keys. The values must either be
    ``True`` or
              ``False``. The default ``engine_kwargs`` for the ``'numba'``
    engine is
        ...
        """
        numeric_only_bool = self._resolve_numeric_only("mean", numeric_only,
    axis=0)
    ...
    use
        ...
        """
        numeric_only_bool = self._resolve_numeric_only("median", numeric_only,
    axis=0)
    ...
    engine is
        ...
        if maybe_use_numba(engine):
            ...
            return np.sqrt(self._numba_agg_general(sliding_var, engine_kwargs,
    ddof))
        else:
            # Resolve numeric_only so that var doesn't warn
            numeric_only_bool = self._resolve_numeric_only("std",
    numeric_only, axis=0)
            if (
                numeric_only_bool
                and self.obj.ndim == 1
    ...
    @Substitution(name="groupby")
    @Appender(_common_see_also)
    def sem(self, ddof: int = 1, numeric_only: bool | lib.NoDefault =
    lib.no_default):
        """
        ...
        numeric_only_bool = self._resolve_numeric_only("sem", numeric_only,
    axis=0)
        if (
            numeric_only_bool
            and self.obj.ndim == 1
        ...
        if result.ndim == 1:
            ...
            with warnings.catch_warnings():
                ...
                result.iloc[:, result_ilocs] /= np.sqrt(counts.iloc[:,
    count_ilocs])
        return result
    ...
    def size(self) -> DataFrame | Series:
        ...
        if not self.as_index:
            ...
            result = result.rename("size").reset_index()  # type:
    ignore[assignment]
    ...
    def sum(
        ...
        if maybe_use_numba(engine):
            ...
            # If we are grouping on categoricals we want unobserved categories
    to
            # return zero, rather than the default of NaN which the reindexing
    in
    ...
    def prod(
        self, numeric_only: bool | lib.NoDefault = lib.no_default, min_count:
    int = 0
    ):
        return self._agg_general(
            numeric_only=numeric_only, min_count=min_count, alias="prod",
    npfunc=np.prod
        )
    ...
    def min(
        ...
        if maybe_use_numba(engine):
            ...
            return self._numba_agg_general(sliding_min_max, engine_kwargs,
    False)
        else:
            return self._agg_general(
                numeric_only=numeric_only,
    ...
    fewer
        ...
        DataFrame.groupby: Apply a function groupby to each row or column of a
            DataFrame.
        DataFrame.core.groupby.GroupBy.last: Compute the last non-null entry
    of each
            column.
    ...
    fewer
        ...
        DataFrame.groupby: Apply a function groupby to each row or column of a
            DataFrame.
        DataFrame.core.groupby.GroupBy.first: Compute the first non-null
    entry of each
            column.
        DataFrame.core.groupby.GroupBy.nth: Take the nth row from each group.
    ...
    def ohlc(self) -> DataFrame:
        """
        Compute open, high, low and close values of a group, excluding missing
    values.
    ...
    def _fill(self, direction: Literal["ffill", "bfill"], limit=None):
        ...
        ids, _, _ = self.grouper.group_info
        sorted_labels = np.argsort(ids, kind="mergesort").astype(np.intp,
    copy=False)
        if direction == "bfill":
    ...
    def ffill(self, limit=None):
        ...
        DataFrame.ffill: Object with missing values filled or None if
    inplace=True.
    ...
    def nth(self) -> GroupByNthSelector:
        """
        Take the nth row from each group if n is an int, otherwise a subset of
    rows.

        Can be either a call or an index. dropna is not available with index
    notation.
        ...
        if not dropna:
            with self._group_selection_context():
                ...
                if self.axis == 0:
                    ...
                    if not self.observed and isinstance(result_index,
    CategoricalIndex):
        ...
        # dropna is truthy
        if not is_integer(n):
            raise ValueError("dropna option only supported for an integer
    argument")
    ...
    def quantile(
        ...
        b    3.0
        """
        numeric_only_bool = self._resolve_numeric_only("quantile",
    numeric_only, axis=0)
        if (
            numeric_only_bool
            and self.obj.ndim == 1
        ...
        def pre_processor(vals: ArrayLike) -> tuple[np.ndarray, np.dtype |
    None]:
            ...
            elif is_bool_dtype(vals.dtype) and isinstance(vals,
    ExtensionArray):
                out = vals.to_numpy(dtype=float, na_value=np.nan)
        ...
        def post_processor(vals: np.ndarray, inference: np.dtype | None) ->
    np.ndarray:
            if inference:
                # Check for edge case
                if not (
        ...
        func = partial(
            libgroupby.group_quantile, labels=ids, qs=qs,
    ...
    "b"],
        ...
        ...     df[f'{method}_rank'] =
    df.groupby('group')['value'].rank(method)
        >>> df
          group  value  average_rank  min_rank  max_rank  dense_rank 
    first_rank
        0     a      2           1.5       1.0       2.0         1.0        
    1.0
        1     a      4           4.0       4.0       4.0         3.0        
    4.0
        2     a      2           1.5       1.0       2.0         1.0        
    2.0
        3     a      3           3.0       3.0       3.0         2.0        
    3.0
        4     a      5           5.0       5.0       5.0         4.0        
    5.0
        5     b      1           1.5       1.0       2.0         1.0        
    1.0
        6     b      2           3.0       3.0       3.0         2.0        
    3.0
        7     b      4           4.0       4.0       4.0         3.0        
    4.0
        8     b      1           1.5       1.0       2.0         1.0        
    2.0
        9     b      5           5.0       5.0       5.0         4.0        
    5.0
        """
    ...
    def cummin(self, axis=0, numeric_only=False, **kwargs) -> NDFrameT:
        ...
        if axis != 0:
            ...
            numeric_only_bool = self._resolve_numeric_only("cummax",
    numeric_only, axis)
            obj = self._selected_obj
            if numeric_only_bool:
    ...
    def _get_cythonized_result(
        ...
        **kwargs,
        ...
        post_processing: function, default None
            ...
            (ndarray, Type). If `needs_nullable=True`, a third argument should
    be
    ...
    values.
        ...
        # GH45562 - to retain existing behavior and match behavior of
    Series.diff(),
    ...
    @Substitution(name="groupby")
    @Appender(_common_see_also)
    def pct_change(self, periods=1, fill_method="ffill", limit=None,
    freq=None, axis=0):
        """
    ...
    def head(self, n: int = 5) -> NDFrameT:
        ...
        Similar to ``.apply(lambda x: x.head(n))``, but it returns a subset of
    rows
    ...
    def tail(self, n: int = 5) -> NDFrameT:
        ...
        Similar to ``.apply(lambda x: x.tail(n))``, but it returns a subset of
    rows
        from the original DataFrame with original index and order preserved
        (``as_index`` flag is ignored).
    ...
    def _reindex_output(
        ...
        if qs is not None:
            ...
            names = names + [None]
        index, _ = MultiIndex.from_product(levels_list,
    names=names).sortlevel()
        ...
        in_axis_grps = (
            (i, ping.name) for (i, ping) in enumerate(groupings) if
    ping.in_axis
        )
    ...
    range(6)}
        ...
        group_iterator = self.grouper.get_iterator(self._selected_obj,
    self.axis)
...
def _insert_quantile_level(idx: Index, qs: npt.NDArray[np.float64]) ->
    MultiIndex:
    """
    Insert the sequence 'qs' of quantiles as the inner-most level of a
    MultiIndex.
...
def warn_dropping_nuisance_columns_deprecated(cls, how: str, numeric_only) ->
    None:
    if numeric_only is not lib.no_default and not numeric_only:
        # numeric_only was specified and falsey but still dropped nuisance
    columns
        warnings.warn(
