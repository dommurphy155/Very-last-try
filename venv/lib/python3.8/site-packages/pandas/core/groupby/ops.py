...
class WrappedCythonOp:
    ...
    def _disallow_invalid_ops(self, dtype: DtypeObj, is_numeric: bool = False):
        ...
        if isinstance(dtype, CategoricalDtype):
            ...
            if how in ["sum", "prod", "cumsum", "cumprod"]:
                raise TypeError(f"{dtype} type does not support {how}
    operations")
            ...
            if how in ["sum", "prod", "cumsum", "cumprod"]:
                raise TypeError(f"datetime64 type does not support {how}
    operations")
        elif is_timedelta64_dtype(dtype):
            if how in ["prod", "cumprod"]:
                raise TypeError(f"timedelta64 type does not support {how}
    operations")
    ...
    def _ea_wrap_cython_operation(
        ...
        **kwargs,
        ...
        if isinstance(values, BaseMaskedArray) and self.uses_mask():
            ...
            #  `if self.how in self.cast_blocklist` like we do for other
    dtypes.
            return res_values
    ...
    def _ea_to_cython_values(self, values: ExtensionArray) -> np.ndarray:
        ...
        if isinstance(values, (DatetimeArray, PeriodArray, TimedeltaArray)):
            ...
            # FloatingArray
            npvalues = values.to_numpy(values.dtype.numpy_dtype,
    na_value=np.nan)
    ...
    def _masked_ea_wrap_cython_operation(
        ...
        **kwargs,
        ...
        """
        Equivalent of `_ea_wrap_cython_operation`, but optimized for masked
    EA's
    ...
    is_numeric)
        ...
        if self.kind == "aggregate":
            ...
            if is_integer_dtype(result.dtype) and not is_datetimelike:
                ...
                cutoff = max(0 if self.how in ["sum", "prod"] else 1,
    min_count)
        ...
        if self.how not in self.cast_blocklist:
            # e.g. if we are int64 and need to restore to
    ...
    limited to 2")
            ...
            # Note: it is *not* the case that axis is always 0 for 1-dim
    values,
...
class BaseGrouper:
    ...
    def indices(self) -> dict[Hashable, npt.NDArray[np.intp]]:
        ...
        if len(self.groupings) == 1 and isinstance(self.result_index,
    CategoricalIndex):
    ...
    def result_ilocs(self) -> npt.NDArray[np.intp]:
        ...
        # But when dropna is true, we need to remove null values while
    accounting for
    ...
    def groups(self) -> dict[Hashable, np.ndarray]:
        ...
        if len(self.groupings) == 1:
            ...
            to_groupby = zip(*(ping.grouping_vector for ping in
    self.groupings))
    ...
    @cache_readonly
    def group_info(self) -> tuple[npt.NDArray[np.intp], npt.NDArray[np.intp],
    int]:
    ...
    def _get_compressed_codes(
        ...
        if len(self.groupings) > 1:
            group_index = get_group_index(self.codes, self.shape, sort=True,
    xnull=True)
    ...
    def reconstructed_codes(self) -> list[npt.NDArray[np.intp]]:
        ...
        return decons_obs_group_ids(ids, obs_ids, self.shape, codes,
    xnull=True)
    ...
    _aggregate_series_fast
            ...
            #  with _from_sequence.  NB we are assuming here that
    _from_sequence
        ...
        if preserve_dtype:
            out = maybe_cast_pointwise_result(npvalues, obj.dtype,
    numeric_only=True)
        else:
            out = npvalues
        return out
...
class DataSplitter(Generic[NDFrameT]):
    def __init__(
        ...
        self.data = data
        self.labels = ensure_platform_int(labels)  # _should_ already be
    np.intp
