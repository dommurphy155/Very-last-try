...
class Grouper:
    ...
    def ax(self) -> Index:
        ...
        if index is None:
            raise ValueError("_set_grouper must be called before ax is
    accessed")
        return index
    ...
    "BaseGrouper",
        ...
        self.grouper, _, self.obj = get_grouper(  # type:
    ignore[type-var,assignment]
            self.obj,
            [self.key],
            axis=self.axis,
            level=self.level,
        ...
        return self.binner, self.grouper, self.obj  # type:
    ignore[return-value]
    ...
    def _set_grouper(self, obj: NDFrame, sort: bool = False) -> None:
        ...
        if self.key is not None and self.level is not None:
            raise ValueError("The Grouper cannot specify both a key and a
    level!")
        ...
        if self.key is not None:
            ...
            if getattr(self._gpr_index, "name", None) == key and isinstance(
                ...
                # call self._grouper.take(obj.index) so we need to undo the
    sorting
...
class Grouping:
    ...
    **Attributes**:
    ...
    sanitize_to_nanoseconds(self.grouping_vector)
    ...
    def result_index(self) -> Index:
        ...
        if self._all_grouper is not None:
            ...
            return recode_from_groupby(self._all_grouper, self._sort,
    group_idx)
    ...
    @cache_readonly
    def _codes_and_uniques(self) -> tuple[npt.NDArray[np.signedinteger],
    ArrayLike]:
    ...
    "Categorical")
            uniques = (
                self.grouping_vector.result_index._values  # type:
    ignore[assignment]
            )
        else:
    ...
    def groups(self) -> dict[Hashable, np.ndarray]:
        return self._index.groupby(Categorical.from_codes(self.codes,
    self.group_index))
...
def get_grouper(
    ...
    if len(groupings) == 0 and len(obj):
        ...
        groupings.append(Grouping(Index([], dtype="int"), np.array([],
    dtype=np.intp)))
...
def _is_label_like(val) -> bool:
    return isinstance(val, (str, tuple)) or (val is not None and
    is_scalar(val))
...
def _check_deprecated_resample_kwargs(kwargs, origin):
    """
    Check for use of deprecated parameters in ``resample`` and related
    functions.

    Raises the appropriate warnings if these parameters are detected.
    Only sets an approximate ``stacklevel`` for the warnings (see #37603,
    #36629).
    ...
    origin: object
        From where this function is being called; either Grouper or
    TimeGrouper. Used
