...
def is_list_like_indexer(key) -> bool:
    ...
    return is_list_like(key) and not (isinstance(key, tuple) and type(key) is
    not tuple)
...
def is_empty_indexer(indexer) -> bool:
    ...
    if not isinstance(indexer, tuple):
        indexer = (indexer,)
    return any(isinstance(idx, np.ndarray) and len(idx) == 0 for idx in
    indexer)
...
def check_setitem_lengths(indexer, value, values) -> bool:
    ...
    if isinstance(indexer, (np.ndarray, list)):
        ...
        if is_list_like(value):
            if len(value) != length_of_indexer(indexer, values) and
    values.ndim == 1:
...
def validate_indices(indices: np.ndarray, n: int) -> None:
    ...
    if len(indices):
        min_idx = indices.min()
        if min_idx < -1:
            msg = f"'indices' contains values less than allowed ({min_idx} <
    -1)"
...
def check_key_length(columns: Index, key, value: DataFrame) -> None:
    ...
    Raises
    ------
    ValueError: If the length of key is not equal to the number of columns in
    value
                or if the number of columns referenced by key is not equal to
    number
                of columns.
    """
...
def check_array_indexer(array: AnyArrayLike, indexer: Any) -> Any:
    ...
    if is_bool_dtype(dtype):
        ...
        raise IndexError("arrays used as indices must be of integer or boolean
    type")
