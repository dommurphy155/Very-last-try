...
def arithmetic_op(left: ArrayLike, right: Any, op):
    ...
    if (
        ...
        # Timedelta/Timestamp and other custom scalars are included in the
    check
        # because numexpr will fail on it, see GH#31457
        res_values = op(left, right)
    else:
        # TODO we should handle EAs consistently and move this check before
    the if/else
        ...
        # "Union[ExtensionArray, ndarray[Any, Any]]"; expected "ndarray[Any,
    Any]"
        res_values = _na_arithmetic_op(left, right, op)  # type:
    ignore[arg-type]
...
def comparison_op(left: ArrayLike, right: Any, op) -> ArrayLike:
    ...
    op: {operator.eq, operator.ne, operator.gt, operator.ge, operator.lt,
    operator.le}
    ...
    if should_extension_dispatch(lvalues, rvalues) or (
        (isinstance(rvalues, (Timedelta, BaseOffset, Timestamp)) or right is
    NaT)
    ...
    try:
        ...
        if isinstance(y, np.ndarray):
            ...
            try:
                ...
                typ = type(y).__name__
                raise TypeError(
                    f"Cannot perform '{op.__name__}' with a dtyped [{x.dtype}]
    array "
...
def logical_op(left: ArrayLike, right: Any, op) -> ArrayLike:
    ...
    if should_extension_dispatch(lvalues, rvalues):
        ...
        if isinstance(rvalues, np.ndarray):
            ...
            rvalues = rvalues if is_other_int_dtype else fill_bool(rvalues,
    lvalues)
