...
def align_method_FRAME(
    ...
    def to_series(right):
        msg = "Unable to coerce to Series, length must be {req_len}: given
    {given_len}"
    ...
    elif is_list_like(right) and not isinstance(right, (ABCSeries,
    ABCDataFrame)):
        # GH 36702. Raise when attempting arithmetic with list of array-like.
        if any(is_array_like(el) for el in right):
            raise ValueError(
                f"Unable to coerce list of {type(right[0])} to
    Series/DataFrame"
            )
        # GH17901
        right = to_series(right)
        ...
        if not flex:
            if not left.axes[axis].equals(right.index):
                warnings.warn(
                    ...
                    "is deprecated and will raise ValueError in a future
    version. "
...
def frame_arith_method_with_reindex(left: DataFrame, right: DataFrame, op) ->
    DataFrame:
    """
...
def _maybe_align_series_as_frame(frame: DataFrame, series: Series, axis: int):
    ...
    if not isinstance(rvalues, np.ndarray):
        ...
        if rvalues.dtype == "datetime64[ns]" or rvalues.dtype ==
    "timedelta64[ns]":
...
def flex_arith_method_FRAME(op):
    ...
    def f(self, other, axis=default_axis, level=None, fill_value=None):
        ...
        if isinstance(other, ABCSeries) and fill_value is not None:
            ...
            raise NotImplementedError(f"fill_value {fill_value} not
    supported.")

        axis = self._get_axis_number(axis) if axis is not None else 1

        other = maybe_prepare_scalar_for_op(other, self.shape)
        self, other = align_method_FRAME(self, other, axis, flex=True,
    level=level)
