...
def _guess_datetime_format_for_array(arr, dayfirst: bool | None = False):
    ...
    if len(non_nan_elements):
        return guess_datetime_format(arr[non_nan_elements[0]],
    dayfirst=dayfirst)


def should_cache(
    arg: ArrayConvertible, unique_share: float = 0.7, check_count: int | None
    = None
) -> bool:
    """
...
def _return_parsed_timezone_results(result: np.ndarray, timezones, tz, name)
    -> Index:
    """
    ...
    """
    tz_results = np.array(
        [Timestamp(res).tz_localize(zone) for res, zone in zip(result,
    timezones)]
    )
    if tz is not None:
        # Convert to the same tz
        tz_results = np.array([tz_result.tz_convert(tz) for tz_result in
    tz_results])
...
def _convert_listlike_datetimes(
    ...
    if format is not None:
        res = _to_datetime_with_format(
            arg, orig_arg, name, tz, format, exact, errors,
...
def _adjust_to_origin(arg, origin, unit):
    ...
    if origin == "julian":
        ...
        try:
            ...
            raise OutOfBoundsDatetime(f"origin {origin} is Out of Bounds")
    from err
        ...
        if is_list_like(arg) and not isinstance(arg, (ABCSeries, Index,
    np.ndarray)):
            arg = np.asarray(arg)
        arg = arg + ioffset
    return arg
...
def to_datetime(
    ...
    DataFrame/dict-like
        The object to convert to a datetime. If a:class:`DataFrame` is
    provided, the
        method expects minimally the following columns::const:`"year"`,:const:`"month"`,:const:`"day"`.
    ...
    utc: bool, default None
        ...
        - If:const:`True`, the function *always* returns a timezone-aware
          UTC-localized:class:`Timestamp`,:class:`Series` or:class:`DatetimeIndex`. To do this, timezone-naive inputs are
          *localized* as UTC, while timezone-aware inputs are *converted* to
    UTC.
    ...
    format: str, default None
        The strftime to parse time, e.g.:const:`"%d/%m/%Y"`. Note that:const:`"%f"` will parse all the way up to nanoseconds. See
    ...
    origin: scalar, default 'unix'
        ...
        - If:const:`'julian'`, unit must be:const:`'D'`, and origin is set to
          beginning of Julian Calendar. Julian day number:const:`0` is
    assigned
    ...
    cache: bool, default True
        ...
        duplicate date strings, especially ones with timezone offsets. The
    cache
    ...
    ValueError
        ...
        when a Timezone-aware:class:`datetime.datetime` is found in an
    array-like
    ...
    - **array-like** can contain int, float, str, datetime objects. They are
      ...
      converted to:class:`Index` with:class:`object` dtype, containing:class:`datetime.datetime`. None/NaN/null entries are converted to:const:`NaT` in both cases.

    - **Series** are converted to:class:`Series` with:class:`datetime64`
      dtype when possible, otherwise they are converted to:class:`Series` with:class:`object` dtype, containing:class:`datetime.datetime`.
    None/NaN/null
      entries are converted to:const:`NaT` in both cases.

    - **DataFrame/dict-like** are converted to:class:`Series` with:class:`datetime64` dtype. For each row a datetime is created from
    assembling
    ...
    being returned (possibly inside an:class:`Index` or a:class:`Series` with:class:`object` dtype) instead of a proper pandas designated type
    (:class:`Timestamp`,:class:`DatetimeIndex` or:class:`Series`
    with:class:`datetime64` dtype):

    - when any input element is before:const:`Timestamp.min` or after:const:`Timestamp.max`, see `timestamp limitations
      <https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html
      #timeseries-timestamp-limits>`_.

    - when ``utc=False`` (default) and the input is an array-like or:class:`Series` containing mixed naive/aware datetime, or aware with
    mixed
      time offsets. Note that this happens in the (quite frequent) situation
    when
    ...
    **Handling various input formats**
    ...
    Assembling a datetime from multiple columns of a:class:`DataFrame`. The
    keys
    ...
    >>> %timeit pd.to_datetime(s, infer_datetime_format=False)  # doctest:
    +SKIP
    ...
    **Timezones and time offsets**
    ...
    - A mix of timezone-aware and timezone-naive inputs is converted to
      a timezone-aware:class:`DatetimeIndex` if the offsets of the
    timezone-aware
      are constant:
    ...
    if isinstance(arg, Timestamp):
        ...
        try:
            ...
            # "Union[float, str, datetime, List[Any], Tuple[Any, ...],
    ExtensionArray,
            # ndarray[Any, Any], Series]"; expected "Union[List[Any],
    Tuple[Any, ...],
            # Union[Union[ExtensionArray, ndarray[Any, Any]], Index, Series],
    Series]"
            argc = cast(
                Union[list, tuple, ExtensionArray, np.ndarray, "Series",
    Index], arg
            )
...
def _assemble_from_unit_mappings(arg, errors: DateTimeErrorChoices, tz):
    ...
    if len(excess):
        _excess = ",".join(excess)
        raise ValueError(
            f"extra keys have been passed to the datetime assemblage:
    [{_excess}]"
        )
    ...
    for u in units:
        ...
        if value is not None and value in arg:
            try:
                values += to_timedelta(coerce(arg[value]), unit=u,
    errors=errors)
            except (TypeError, ValueError) as err:
                raise ValueError(
...
def _attempt_YYYYMMDD(arg: npt.NDArray[np.object_], errors: str) -> np.ndarray
    | None:
    """
    ...
    expected
        ...
        mask = ~algorithms.isin(arg, list(nat_strings))  # type:
    ignore[arg-type]
...
def to_time(arg, format=None, infer_time_format=False, errors="raise"):
    # GH#34145
    warnings.warn(
        "`to_time` has been moved, should be imported from
