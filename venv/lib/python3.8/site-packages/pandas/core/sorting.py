...
def get_indexer_indexer(
    ...
    if level is not None:
        ...
        indexer = lexsort_indexer(
            target._get_codes_for_sorting(), orders=ascending,
...
def decons_obs_group_ids(
    ...
    if not is_int64_overflow_possible(shape):
        ...
        return out if xnull or not lift.any() else [x - y for x, y in zip(out,
    lift)]

    indexer = unique_label_indices(comp_ids)
    return [lab[indexer].astype(np.intp, subok=False, copy=True) for lab in
    labels]
...
def lexsort_indexer(
    ...
    na_position: {'first', 'last'}, default 'last'
        Determines placement of NA elements in the sorted list ("last" or
    "first")
    key: Callable, optional
    ...
    for k, order in zip(keys, orders):
        with warnings.catch_warnings():
            ...
            warnings.filterwarnings(
                "ignore", ".*(SparseArray|SparseDtype).*",
...
def nargsort(
    ...
    if isinstance(items, ABCRangeIndex):
        return items.argsort(ascending=ascending)  # TODO: test coverage with
    key?
    elif not isinstance(items, ABCMultiIndex):
        items = extract_array(items)
    if mask is None:
        mask = np.asarray(isna(items))  # TODO: does this exclude MultiIndex
    too?
...
def _nanargminmax(values: np.ndarray, mask: npt.NDArray[np.bool_], func) ->
    int:
    """
...
def ensure_key_mapped(values, key: Callable | None, levels=None):
    ...
    if len(result) != len(values):
        raise ValueError(
            "User-provided `key` function must not change the shape of the
    array."
        )
    ...
    try:
        if isinstance(
            ...
            result = type_of_values(result)  # try to revert to original type
    otherwise
    except TypeError:
        raise TypeError(
            f"User-provided `key` function returned an invalid type
    {type(result)} \
...
def get_group_index_sorter(
    ...
    do_groupsort = count > 0 and ((alpha + beta * ngroups) < (count *
    np.log(count)))
    if do_groupsort:
