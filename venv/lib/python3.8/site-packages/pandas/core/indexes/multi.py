...
class MultiIndex(Index):
    ...
    def _validate_codes(self, level: list, code: list):
        ...
        if np.any(null_mask):
            ...
            code = np.where(null_mask[code], -1, code)  # type:
    ignore[assignment]
        return code

    def _verify_integrity(self, codes: list | None = None, levels: list | None
    = None):
        """
        Parameters
        ----------
        ...
        for i, (level, level_codes) in enumerate(zip(levels, codes)):
            ...
            if len(level_codes) and level_codes.max() >= len(level):
                raise ValueError(
                    f"On level {i}, code max ({level_codes.max()}) >= length
    of "
                    f"level ({len(level)}). NOTE: this index is in an "
                    "inconsistent state"
                )
            if len(level_codes) and level_codes.min() < -1:
                raise ValueError(f"On level {i}, code value
    ({level_codes.min()}) < -1")
        ...
        if self.sortorder is not None:
            if self.sortorder > _lexsort_depth(self.codes, self.nlevels):
                raise ValueError(
                    ...
                    f"with lexsort_depth {_lexsort_depth(self.codes,
    self.nlevels)}"
                )

        codes = [
            self._validate_codes(level, code) for level, code in zip(levels,
    codes)
        ]
    ...
    def from_tuples(
        ...
        if len(tuples) and all(isinstance(e, tuple) and not e for e in tuples):
            ...
            levels = [Index(com.asarray_tuplesafe(tuples,
    dtype=np.dtype("object")))]
        ...
        if len(tuples) == 0:
            if names is None:
                raise TypeError("Cannot infer number of levels from empty
    list")
    ...
    @classmethod
    def from_frame(cls, df: DataFrame, sortorder=None, names=None) ->
    MultiIndex:
        """
    ...
    def levels(self) -> FrozenList:
        ...
        result = [x._rename(name=name) for x, name in zip(self._levels,
    self._names)]
        for level in result:
        ...
        *,
        ...
        if validate:
            ...
            if level is None and len(levels) != self.nlevels:
                raise ValueError("Length of levels must match number of
    levels.")
            if level is not None and len(levels) != len(level):
                raise ValueError("Length of levels must match length of
    level.")
    ...
    def set_levels(
        ...
        MultiIndex([('a', 1),
            ...
            ('c', 2)],
           names=['foo', 'bar'])
        >>> idx.set_levels([['a', 'b', 'c'], [1, 2, 3, 4]], level=[0,
    1]).levels
        FrozenList([['a', 'b', 'c'], [1, 2, 3, 4]])
        """
        if inplace is not None:
            warnings.warn(
                "inplace is deprecated and will be removed in a future
    version.",
                FutureWarning,
        ...
        idx._reset_identity()
        idx._set_levels(
            levels, level=level, validate=True,
        ...
        *,
        ...
        if validate:
            ...
            if level is not None and len(codes) != len(level):
                raise ValueError("Length of codes must match length of
    levels.")
    ...
    @deprecate_nonkeyword_arguments(version=None, allowed_args=["self",
    "codes"])
    def set_codes(self, codes, level=None, inplace=None, verify_integrity:
    bool = True):
        """
    ...
    # Return type "Callable[..., MultiIndex]" of "_constructor" incompatible
    with return
    # type "Type[MultiIndex]" in supertype "Index"
    @property
    def _constructor(self) -> Callable[..., MultiIndex]:  # type:
    ignore[override]
        return type(self).from_tuples

    @doc(Index._shallow_copy)
    def _shallow_copy(self, values: np.ndarray, name=lib.no_default) ->
    MultiIndex:
    ...
    def copy(
        ...
        name: Label
            Kept for compatibility with 1-dimensional Index. Should not be
    used.
    ...
    def _format_native_types(
        ...
        if len(new_levels) == 1:
            ...
            return
    Index(new_levels[0].take(new_codes[0]))._format_native_types()
        else:
    ...
    def format(
        ...
        for lev, level_codes in zip(self.levels, self.codes):
            ...
            if len(lev) > 0:
                ...
                # weird all NA case
                formatted = [
                    pprint_thing(na if isna(x) else x, escape_chars=("\t",
    "\r", "\n"))
    ...
    def _set_names(self, names, *, level=None, validate: bool = True):
        ...
        if validate:
            ...
            if level is None and len(names) != self.nlevels:
                raise ValueError(
                    "Length of names must match number of levels in
    MultiIndex."
                )
    ...
    def _get_grouper_for_level(
        ...
        *,
        ...
        codes, uniques = algos.factorize(values, sort=True,
    ...
    GH27495
        ...
        values = [
            self._get_level_values(i)._values for i in
    reversed(range(len(self.levels)))
        ]
        try:
    ...
    def get_level_values(self, level):
        ...
        level_1    int64
        dtype: object
        >>> pd.MultiIndex.from_arrays([[1, None, 2], [3, 4,
    5]]).get_level_values(0)
    ...
    and in "
        ...
        if name is not lib.no_default:
            if not is_list_like(name):
                raise TypeError("'name' must be a list / sequence of column
    names.")

            if len(name) != len(self.levels):
                raise ValueError(
                    "'name' should have same length as number of levels on
    index."
                )
            idx_names = name
        else:
        ...
        if not allow_duplicates and len(set(idx_names)) != len(idx_names):
            raise ValueError(
                "Cannot create duplicate column labels if allow_duplicates is
    False"
            )
    ...
    order
        result = DataFrame(
            {level: self._get_level_values(level) for level in
    ...
    # error: Return type "Index" of "to_flat_index" incompatible with return
    type
    ...
    def _is_lexsorted(self) -> bool:
        ...
        Examples
        --------
        In the below examples, the first level of the MultiIndex is sorted
    because
        a<b<c, so there is no need to look at the next level.

        >>> pd.MultiIndex.from_arrays([['a', 'b', 'c'], ['d', 'e',
    'f']]).is_lexsorted()
        True
        >>> pd.MultiIndex.from_arrays([['a', 'b', 'c'], ['d', 'f',
    'e']]).is_lexsorted()
        True
        ...
        >>> pd.MultiIndex.from_arrays([[0, 1, 1], ['a', 'b',
    'c']]).is_lexsorted()
        True
        >>> pd.MultiIndex.from_arrays([[0, 1, 1], ['a', 'c',
    'b']]).is_lexsorted()
        False
    ...
    def take(
        ...
        **kwargs,
        ...
        return MultiIndex(
            levels=self.levels, codes=taken, names=self.names,
    verify_integrity=False
        )
    ...
    def append(self, other):
        ...
        if all(
            (isinstance(o, MultiIndex) and o.nlevels >= self.nlevels) for o in
    other
        ):
            arrays = []
            for i in range(self.nlevels):
    ...
    "ndarray",
        ...
        return MultiIndex(
            levels=self.levels,
            codes=[
                level_codes.view(np.ndarray).astype(np.intp,
    ...
    def _drop_from_level(self, codes, level, errors="raise") -> MultiIndex:
        ...
        # If nan should be dropped it will equal -1 here. We have to check
    which values
    ...
    def _get_codes_for_sorting(self) -> list[Categorical]:
        ...
        return [
            Categorical.from_codes(level_codes, cats(level_codes),
    ordered=True)
            for level_codes in self.codes
        ]
    ...
    def _get_values_for_loc(self, series: Series, loc, key):
        ...
        new_ser = series._constructor(new_values, index=new_index,
    name=series.name)
    ...
    `self.get_level_values(0).get_indexer_for(target)`.
        """
        lev = self.levels[0]
        codes = self._codes[0]
    ...
    "left"):
        if len(tup) > self._lexsort_depth:
            raise UnsortedIndexError(
                f"Key length ({len(tup)}) was greater than MultiIndex lexsort
    depth "
        ...
        for k, (lab, lev, level_codes) in enumerate(zipped):
            ...
            if isinstance(idx, slice) and k < n - 1:
                # Get start and end value from slice, necessary when a
    non-integer
    ...
    def _get_loc_single_level_index(self, level_index: Index, key: Hashable)
    -> int:
        """
    ...
    def get_loc(self, key, method=None):
        ...
        or a sequence of such. If you want to use those, use:meth:`MultiIndex.get_locs` instead.
    ...
    def _get_loc_level(self, key, level: int | list[int] = 0):
        ...
        if isinstance(level, (tuple, list)):
            if len(key) != len(level):
                raise AssertionError(
                    "Key for location must have same length as number of
    levels"
                )
            result = None
        ...
        if isinstance(key, tuple) and level == 0:
            ...
            if not any(isinstance(k, slice) for k in key):
                ...
                ilevels = [i for i in range(len(key)) if key[i] != slice(None,
    None)]
                if len(ilevels) == self.nlevels:
                    ...
                    ilevels = [
                        ...
                        if (
                            ...
                            or not
    self.levels[i]._supports_partial_string_indexing
                        )
                ...
                for i, k in enumerate(key):
                    if not isinstance(k, slice):
                        ...
                        if isinstance(loc_level, slice):
                            if com.is_null_slice(loc_level) or
    com.is_full_slice(
                                loc_level, len(self)
                            ):
                                # everything
                                ...
                                # e.g.
    test_xs_IndexSlice_argument_not_implemented
                        ...
                        raise TypeError(f"Expected label or tuple of labels,
    got {key}")
    ...
    key.step)
            ...
            if level > 0 or self._lexsort_depth == 0:
                ...
                if isinstance(idx, slice):
                    ...
                    locs = (level_codes >= idx.start) & (level_codes <
    idx.stop)
                    return locs
    ...
    def get_locs(self, seq):
        ...
        >>> mi.get_locs([[True, False, True], slice('e', 'f')])  # doctest:
    +SKIP
        array([2], dtype=int64)
        """
    ...
    or'd)
                ...
                try:
                    lvl_indexer = self._get_level_indexer(k, level=i,
    indexer=indexer)
                except (InvalidIndexError, TypeError, KeyError) as err:
                    # InvalidIndexError e.g. non-hashable, fall back to
    treating
                    #  this as a sequence of labels
                    # KeyError it can be ambiguous if this is a label or
    sequence
                    #  of labels
                    # 
    github.com/pandas-dev/pandas/issues/39424#issuecomment-871626708
                    for x in k:
                        ...
                        try:
                            ...
                            warnings.warn(
                                "The behavior of indexing on a MultiIndex with
    a "
                                "nested sequence of labels is deprecated and
    will "
                                "change in a future version. "
                                "`series.loc[label, sequence]` will raise if
    any "
                                "members of 'sequence' or not present in "
                                "the index's second level. To retain the old "
                                "behavior, use `series.index.isin(sequence,
    level=1)`",
    ...
    def _reorder_indexer(
        ...
        if self._is_lexsorted():
            ...
            for i, k in enumerate(seq):
                ...
                elif isinstance(k, slice) and k.step is not None and k.step <
    0:
                    need_sort = True
    ...
    list
            ...
            if com.is_bool_indexer(k):
                ...
                k = algos.unique(k)
                key_order_map = np.ones(len(self.levels[i]), dtype=np.uint64)
    * len(
                    self.levels[i]
                )
                # Set order as given in the indexer list
                level_indexer = self.levels[i].get_indexer(k)
                level_indexer = level_indexer[level_indexer >= 0]  # Filter
    absent keys
    ...
    def _union(self, other, sort) -> MultiIndex:
        ...
        return MultiIndex.from_arrays(zip(*result), sortorder=None,
    names=result_names)
    ...
    def _wrap_intersection_result(self, other, result) -> MultiIndex:
        ...
        if len(result) == 0:
            ...
            return MultiIndex.from_arrays(zip(*result), sortorder=0,
    names=result_names)
    ...
    def _wrap_difference_result(self, other, result) -> MultiIndex:
        ...
        if len(result) == 0:
            ...
            return MultiIndex.from_tuples(result, sortorder=0,
    names=result_names)
    ...
    def insert(self, loc: int, item) -> MultiIndex:
        ...
        for k, level, level_codes in zip(item, self.levels, self.codes):
            ...
            new_codes.append(np.insert(ensure_int64(level_codes), loc,
    lev_loc))

        return MultiIndex(
            levels=new_levels, codes=new_codes, names=self.names,
    verify_integrity=False
        )
...
def _lexsort_depth(codes: list[np.ndarray], nlevels: int) -> int:
    """Count depth (up to a maximum of `nlevels`) with which codes are
    lexsorted."""
...
def _coerce_indexer_frozen(array_like, categories, copy: bool = False) ->
    np.ndarray:
    """
    Coerce the array-like indexer to the smallest integer dtype that can
    encode all
