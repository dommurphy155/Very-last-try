...
def _maybe_return_indexers(meth: F) -> F:
    ...
    def join(
        ...
        join_index, lidx, ridx = meth(self, other, how=how, level=level,
    sort=sort)
...
class Index(IndexOpsMixin, PandasObject):
    ...
    @final
    def _left_indexer_unique(self: _IndexT, other: _IndexT) ->
    npt.NDArray[np.intp]:
    ...
    # UInt64Index or RangeIndex. Needed for backwards compat. Remove this
    attribute and
    ...
    def __new__(
        cls, data=None, dtype=None, copy=False, name=None, tupleize_cols=True,
    **kwargs
    ) -> Index:
        ...
        if kwargs:
            warnings.warn(
                ...
                "future version.  Use the specific Index subclass directly
    instead.",
                FutureWarning,
    ...
    **kwargs)
            ...
            if dtype is None:
                # with e.g. a list [1, 2, 3] casting to numeric is _not_
    deprecated
    ...
    def asi8(self):
        ...
        """
        warnings.warn(
            "Index.asi8 is deprecated and will be removed in a future
    version.",
            FutureWarning,
            stacklevel=find_stack_level(),
        )
    ...
    @classmethod
    def _simple_new(cls: type[_IndexT], values, name: Hashable = None) ->
    _IndexT:
        """
    ...
    type
            ...
            # "ndarray[Any, Any]"
            values = lib.maybe_convert_objects(result._values)  # type:
    ignore[arg-type]
    ...
    [attr-defined]
            out.index = type(self).from_tuples(out.index)  # type:
    ignore[attr-defined]
    ...
    def _shallow_copy(self: _IndexT, values, name: Hashable = no_default) ->
    _IndexT:
        """
    ...
    def _engine(
        ...
        # to avoid a reference cycle, bind `target_values` to a local
    variable, so
    ...
    **kwargs):
        if any(isinstance(other, (ABCSeries, ABCDataFrame)) for other in
    inputs):
    ...
    def __array_wrap__(self, result, context=None):
        ...
        if is_bool_dtype(result) or lib.is_scalar(result) or np.ndim(result) >
    1:
            return result
    ...
    def ravel(self, order="C"):
        ...
        if needs_i8_conversion(self.dtype):
            # Item "ndarray[Any, Any]" of "Union[ExtensionArray, ndarray[Any,
    Any]]"
    ...
    def view(self, cls=None):
        ...
        if cls is not None and not hasattr(cls, "_typ"):
            ...
            if isinstance(dtype, (np.dtype, ExtensionDtype)) and
    needs_i8_conversion(
                dtype
            ):
    ...
    def astype(self, dtype, copy: bool = True):
        ...
        if isinstance(values, ExtensionArray):
            if isinstance(dtype, np.dtype) and dtype.kind == "M" and
    is_unitless(dtype):
            ...
            # TODO: this differs from Series behavior; can/should we align
    them?
            raise TypeError(
            ...
            try:
                if dtype == str:
                    ...
                    # GH#13149 specifically use astype_nansafe instead of
    astype
        ...
        if self._is_backward_compat_public_numeric_index:
            # this block is needed so e.g. NumericIndex[int8].astype("int32")
    returns
            # NumericIndex[int32] and not Int64Index with dtype int64.
            # When Int64Index etc. are removed from the code base, removed
    this also.
    ...
    def take(
        self, indices, axis: int = 0, allow_fill: bool = True,
    fill_value=None, **kwargs
        ...
        if isinstance(values, np.ndarray):
            taken = algos.take(
                values, indices, allow_fill=allow_fill,
    ...
    @final
    def _maybe_disallow_fill(self, allow_fill: bool, fill_value, indices) ->
    bool:
        """
        ...
        if allow_fill and fill_value is not None:
            ...
            if self._can_hold_na:
                ...
                raise ValueError(
                    f"Unable to fill values because {cls_name} cannot contain
    NA"
                )
        else:
            allow_fill = False
    ...
    def _get_level_names(self) -> Hashable | Sequence[Hashable]:
        ...
        if self._is_multi:
            return [
                level if name is None else name for level, name in
    ...
    def _format_with_header(self, header: list[str_t], na_rep: str_t) ->
    list[str_t]:
    ...
    Index's name "
                "or uses a default name of 0. This behaviour is deprecated,
    and in "
    ...
    def _validate_names(
        ...
        if len(new_names) != len(self.names):
            raise ValueError(
                f"Length of new names must be {len(self.names)}, got
    {len(new_names)}"
            )
    ...
    def _set_names(self, values, *, level=None) -> None:
        ...
        if len(values) != 1:
            raise ValueError(f"Length of new names must be 1, got
    {len(values)}")
    ...
    def set_names(self, names, level=None, inplace: bool = False):
        ...
        level: int, label or list of int or label, optional
            If the index is a MultiIndex and names is not dict-like, level(s)
    to set
        ...
        elif level is not None and not is_list_like(level) and
    is_list_like(names):
            raise TypeError("Names must be a string when a single level is
    provided.")
            ...
            raise TypeError("Can only pass dict-like as `names` for
    MultiIndex.")

        elif is_dict_like(names) and level is not None:
            raise TypeError("Can not pass level for dictlike `names`.")

        if isinstance(self, ABCMultiIndex) and is_dict_like(names) and level
    is None:
    ...
    def sortlevel(self, level=None, ascending=True, sort_remaining=None):
        ...
        if isinstance(ascending, list):
            if len(ascending) != 1:
                raise TypeError("ascending must be a list of bool values of
    length 1")
        ...
        if len(new_levels) == 1:
            ...
            if len(lev) == 0:
                ...
                if len(new_codes[0]) == 0:
                    ...
                    res_values = algos.take(lev._values, new_codes[0],
    allow_fill=True)
    ...
    def is_floating(self) -> bool:
        ...
        -------
        bool
            Whether or not the Index only consists of only consists of floats,
    NaNs, or
    ...
    def is_mixed(self) -> bool:
        ...
        """
        warnings.warn(
            "Index.is_mixed is deprecated and will be removed in a future
    version. "
    ...
    @deprecate_nonkeyword_arguments(version=None, allowed_args=["self"])
    def drop_duplicates(self: _IndexT, keep: str_t | bool = "first") ->
    _IndexT:
        """
    ...
    def union(self, other, sort=None):
        ...
        if not is_dtype_equal(self.dtype, other.dtype):
            if (
                ...
                raise NotImplementedError(
                    "Can only union MultiIndex with MultiIndex or Index of
    tuples, "
            ...
            # NB: whether this (and the `if not len(self)` check below) come
    before
    ...
    def _union(self, other: Index, sort):
        ...
        if (
            ...
            # Both are monotonic and at least one is unique, so can use outer
    join
        ...
        if not self.is_monotonic_increasing or not
    ...
    sort=sort).rename(result_name)
        ...
        if self._is_multi:
            self = cast("MultiIndex", self)
            if len(result) == 0:
                # On equal symmetric_difference MultiIndexes the difference is
    empty.
    ...
    def get_indexer(
        ...
        if not self._should_compare(target) and not
    self._should_partial_index(target):
    ...
    unique=True)
        ...
        if is_categorical_dtype(self.dtype):
            ...
            if self.hasnans and target.hasnans:
                # After _maybe_cast_listlike_indexer, target elements which do
    not
                # belong to some category are changed to NaNs
                # Mask to track actual NaN values compared to inserted NaN
    values
                # GH#45361
        ...
        if is_categorical_dtype(target.dtype):
            # potential fastpath
            # get an indexer for unique categories then propagate to codes via
    take_nd
            # get_indexer instead of _get_indexer needed for MultiIndex cases
            #  e.g. test_append_different_columns_types
            categories_indexer = self.get_indexer(target.categories)

            indexer = algos.take_nd(categories_indexer, target.codes,
    fill_value=-1)

            if (not self._is_multi and self.hasnans) and target.hasnans:
                # Exclude MultiIndex because hasnans raises NotImplementedError
                # we should only get here if we are unique, so loc is an
    integer
                # GH#41934
                loc = self.get_loc(np.nan)
                mask = target.isna()
        ...
        if not is_dtype_equal(self.dtype, target.dtype) and not
    is_interval_dtype(
            self.dtype
        ):
    ...
    def _get_indexer(
        ...
        if method in ["pad", "backfill"]:
            ...
            if target._is_multi and self._is_multi:
                engine = self._engine
                # error: Item "IndexEngine" of "Union[IndexEngine,
    ExtensionEngine]"
                # has no attribute "_extract_level_codes"
                tgt_values = engine._extract_level_codes(  # type:
    ignore[union-attr]
                    target
                )
            else:
    ...
    def _check_indexing_method(
        ...
        """
        if method not in [None, "bfill", "backfill", "pad", "ffill",
    "nearest"]:
    ...
    np.ndarray:
        ...
        if target.size != tolerance.size and tolerance.size > 1:
            raise ValueError("list-like tolerance size must match target index
    size")
        return tolerance
    ...
    tolerance=None
        ...
        if self._is_multi:
            # TODO: get_indexer_with_fill docstring says values must be
    _sorted_
            ...
            return engine.get_indexer_with_fill(  # type: ignore[union-attr]
                target=target._values, values=self._values, method=method,
    limit=limit
            )
        ...
        if self.is_monotonic_increasing and target.is_monotonic_increasing:
            ...
            if method == "pad":
                ...
                # i.e. "backfill"
                indexer = libalgos.backfill(own_values, target_values,
    limit=limit)
        else:
    ...
    def _difference_compat(
        ...
        # Compatibility for PeriodArray, for which __sub__ returns an
    ndarray[object]
        #  of DateOffset objects, which do not support __abs__ (and would be
    slow
        #  if they did)
        ...
        if isinstance(self.dtype, PeriodDtype):
            ...
            diff = self._values[indexer] - target._values  # type:
    ignore[operator]
        return abs(diff)
    ...
    def _convert_slice_indexer(self, key: slice, kind: str_t):
        ...
        ints_are_positional = self._should_fallback_to_positional or
    is_interval_dtype(
            self.dtype
        )
    ...
    def reindex(
        ...
        See Also
        --------
        Series.reindex: Conform Series to new index with optional filling
    logic.
    ...
    filling logic.
        ...
        if level is not None and (
            isinstance(self, ABCMultiIndex) or isinstance(target,
    ABCMultiIndex)
        ):
            if method is not None:
    ...
    def join(
        ...
        *,
        ...
        sort: bool = ...,
    ) -> Index | tuple[Index, npt.NDArray[np.intp] | None,
    ...
    npt.NDArray[np.intp] | None]:
        """
        Compute join_index and indexers to conform data structures to the new
    index.
        ...
        if isinstance(self, ABCDatetimeIndex) and isinstance(other,
    ABCDatetimeIndex):
    ...
    DatetimeIndex")
        ...
        if not self._is_multi and not other._is_multi:
            ...
            if pself is not self or pother is not other:
                return pself.join(
                    pother, how=how, level=level, return_indexers=True,
    sort=sort
                )
        ...
        if not self.is_unique and not other.is_unique:
            ...
            if self.is_monotonic_increasing and other.is_monotonic_increasing:
                if not is_interval_dtype(self.dtype):
                    ...
                    # GH#39133
                    # go through object dtype for ea till engine is supported
    properly
            ...
            and (
                ...
                or not any(is_categorical_dtype(dtype) for dtype in
    self.dtypes)
            )
            and not is_categorical_dtype(self.dtype)
        ):
            # Categorical is monotonic if data are ordered as categories, but
    join can
    ...
    None]:
        ...
        def _get_leaf_sorter(labels: list[np.ndarray]) -> npt.NDArray[np.intp]:
            ...
            labels: list[np.ndarray]
                Each ndarray has signed integer dtype, not necessarily
    identical.
        ...
        if isinstance(self, MultiIndex) and isinstance(other, MultiIndex):
            raise TypeError("Join on level between two MultiIndex objects is
    ambiguous")
    ...
    len(old_level))
            ...
            if keep_order:  # just drop missing values. o.w. keep order
                ...
                if level == 0:  # outer most level, take the fast route
                    max_new_lev = 0 if len(new_lev_codes) == 0 else
    new_lev_codes.max()
        ...
        right_indexer = (
            None if right_indexer is None else
    ...
    def _wrap_joined_index(self: _IndexT, joined: ArrayLike, other: _IndexT)
    -> _IndexT:
        assert other.dtype == self.dtype
        ...
        if isinstance(self, ABCMultiIndex):
            ...
            # expected "_IndexT")
            return self._constructor(joined, name=name)  # type:
    ignore[return-value]
        else:
            name = get_op_result_name(self, other)
    ...
    def values(self) -> ArrayLike:
        ...
        .. warning::
           ...
           We recommend using:attr:`Index.array` or:meth:`Index.to_numpy`, depending on whether you need
    ...
    def _require_scalar(self, value):
        ...
        if not is_scalar(value):
            raise TypeError(f"'value' must be a scalar, passed:
    {type(value).__name__}")
        return value
    ...
    def _can_hold_identifiers_and_holds_name(self, name) -> bool:
        ...
        """
        if self.is_object() or is_string_dtype(self.dtype) or
    self.is_categorical():
            return name in self
        return False
    ...
    def identical(self, other) -> bool:
        """
        Similar to equals, but checks that object attributes and types are
    also equal.
    ...
    def sort_values(
        ...
        if not isinstance(self, ABCMultiIndex):
            _as = nargsort(
                items=idx, ascending=ascending, na_position=na_position,
    key=key
            )
        else:
            _as = idx.argsort()
    ...
    def sort(self, *args, **kwargs):
        ...
        """
        raise TypeError("cannot sort an Index object in-place, use sort_values
    instead")
    ...
    def get_indexer_non_unique(
        ...
        if not self._should_compare(target) and not
    is_interval_dtype(self.dtype):
            # IntervalIndex get special treatment bc numeric scalars can be
            #  matched to Interval scalars
            return self._get_indexer_non_comparable(target, method=None,
    unique=False)
        ...
        if self._is_multi and target._is_multi:
            ...
            tgt_values = engine._extract_level_codes(target)  # type:
    ignore[union-attr]
    ...
    def _get_indexer_strict(self, key, axis_name: str_t) -> tuple[Index,
    np.ndarray]:
        """
    ...
    def _raise_if_missing(self, key, indexer, axis_name: str_t) -> None:
        ...
        if nmissing:
            ...
            not_found =
    list(ensure_index(key)[missing_mask.nonzero()[0]].unique())
    ...
    npt.NDArray[np.intp]]:
        ...
        if method is not None:
            ...
            raise TypeError(f"Cannot compare dtypes {self.dtype} and
    {other.dtype}")
    ...
    _requires_unique_msg = "Reindexing only valid with uniquely valued Index
    objects"
    ...
    ABCDatetimeIndex):
        ...
        elif self.inferred_type == "date" and isinstance(other,
    ABCDatetimeIndex):
            try:
                return type(other)(self), other
            except OutOfBoundsDatetime:
                return self, other
        elif self.inferred_type == "timedelta" and isinstance(other,
    ABCTimedeltaIndex):
            ...
            try:
                ...
                other = type(self).from_tuples(other)  # type:
    ignore[attr-defined]
    ...
    def _find_common_type_compat(self, target) -> DtypeObj:
        ...
        # special case: if one dtype is uint64 and the other a signed int,
    return object
    ...
    def _should_compare(self, other: Index) -> bool:
        ...
        if (other.is_boolean() and self.is_numeric()) or (
            ...
            #  found. Without this fix False and True would be treated as 0
    and 1
            #  respectively.
            return False
    ...
    def map(self, mapper, na_action=None):
        ...
        same_dtype = lib.infer_dtype(new_values, skipna=False) ==
    self.inferred_type
        if same_dtype:
        ...
        return Index._with_infer(new_values, dtype=dtype, copy=False,
    name=self.name)
    ...
    def _transform_index(self, func, *, level=None) -> Index:
        ...
        Only apply function to one level of the MultiIndex if level is
    specified.
        """
        if isinstance(self, ABCMultiIndex):
            if level is not None:
                ...
                items = [
                    tuple(func(y) if i == level else y for i, y in
    enumerate(x))
                    for x in self
                ]
            else:
    ...
    def slice_locs(
        ...
        if isinstance(start, (str, datetime)) and isinstance(end, (str,
    datetime)):
            try:
                ...
                if not tz_compare(ts_start.tzinfo, ts_end.tzinfo):
                    raise ValueError("Both dates must have the same UTC
    offset")
    ...
    def delete(self: _IndexT, loc) -> _IndexT:
        ...
        loc: int or list of int
            ...
            Use a list of locations to delete more than one value at the same
    time.
    ...
    def insert(self, loc: int, item) -> Index:
        ...
        if arr.dtype != object or not isinstance(
            ...
            # with object-dtype we need to worry about numpy incorrectly
    casting
    ...
    https://github.com/numpy/numpy/issues/12550
            ...
            new_values = np.insert(arr, loc, None)  # type:
    ignore[call-overload]
            loc = loc if loc >= 0 else loc - 1
            new_values[loc] = item

        if self._typ == "numericindex":
            # Use self._constructor instead of Index to retain NumericIndex
    GH#43921
    ...
    and len(
        ...
        elif is_object_dtype(self.dtype) and not isinstance(self,
    ABCMultiIndex):
    ...
    def _arith_method(self, other, op):
        if (
            ...
            # We return NotImplemented for object-dtype index *subclasses* so
    they have
    ...
    def any(self, *args, **kwargs):
        ...
        **kwargs
        ...
        # error: Argument 1 to "any" has incompatible type "ArrayLike";
    expected
    ...
    def all(self, *args, **kwargs):
        ...
        **kwargs
        ...
        # error: Argument 1 to "all" has incompatible type "ArrayLike";
    expected
        # "Union[Union[int, float, complex, str, bytes, generic],
    Sequence[Union[int,
        # float, complex, str, bytes, generic]], Sequence[Sequence[Any]],
    ...
    def min(self, axis=None, skipna=True, *args, **kwargs):
        ...
        if not self._is_multi and not isinstance(self._values, np.ndarray):
            ...
            return self._values.min(skipna=skipna)  # type:
    ignore[attr-defined]
    ...
    def max(self, axis=None, skipna=True, *args, **kwargs):
        ...
        if not self._is_multi and not isinstance(self._values, np.ndarray):
            ...
            return self._values.max(skipna=skipna)  # type:
    ignore[attr-defined]
...
def ensure_index_from_sequences(sequences, names=None) -> Index:
    ...
    >>> ensure_index_from_sequences([["a", "a"], ["a", "b"]], names=["L1",
    "L2"])
    MultiIndex([('a', 'a'),
                ('a', 'b')],
               names=['L1', 'L2'])
...
def _maybe_cast_data_without_dtype(
    ...
    "
            "dtypes when passed object-dtype sequences (matching Series
    behavior)",
            FutureWarning,
            stacklevel=find_stack_level(),
        )
