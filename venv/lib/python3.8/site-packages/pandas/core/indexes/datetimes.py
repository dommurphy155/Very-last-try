...
class DatetimeIndex(DatetimeTimedeltaMixin):
    ...
    def __new__(
        ...
        if (
            ...
            # TODO(2.0): special casing can be removed once
    ...
    def _is_dates_only(self) -> bool:
        ...
        return self.tz is None and is_dates_only(self._values)  # type:
    ignore[arg-type]
    ...
    "DatetimeIndex")
            values = self._values.view("M8[ns]").copy()  # type:
    ignore[assignment]
    ...
    def _deprecate_mismatched_indexing(self, key, one_way: bool = False) ->
    None:
        # GH#36148
    ...
    def _maybe_cast_slice_bound(self, label, side: str, kind=lib.no_default):
        ...
        if isinstance(label, date) and not isinstance(label, datetime):
            # Pandas supports slicing with dates, treated as datetimes at
    midnight.
    ...
    kind=lib.no_default):
        ...
        # GH#33146 if start and end are combinations of str and None and Index
    is not
        # monotonic, we can not use Index.slice_indexer because it does not
    honor the
    ...
    def indexer_between_time(
        self, start_time, end_time, include_start: bool = True, include_end:
    bool = True
    ) -> npt.NDArray[np.intp]:
        """
        ...
        See Also
        --------
        indexer_at_time: Get index locations of values at particular time of
    day.
...
def date_range(
    ...
    **kwargs,
    ...
    satisfy `start <[=] x <[=] end`, where the first one and the last one are,
    resp.,
    the first and last time points in that range that fall on the boundary of
    ``freq``
    (if given as a frequency string) or that are valid for ``freq`` (if given
    as a:class:`pandas.tseries.offsets.DateOffset`). (If exactly one of ``start``,
    ``end``, or ``freq`` is *not* specified, this missing parameter can be
    computed
    ...
    freq: str or DateOffset, default 'D'
        Frequency strings can have multiples, e.g. '5H'. See:ref:`here <timeseries.offset_aliases>` for a list of
        frequency aliases.
    ...
    closed: {None, 'left', 'right'}, optional
        ...
        .. deprecated:: 1.4.0
           Argument `closed` has been deprecated to standardize boundary
    inputs.
