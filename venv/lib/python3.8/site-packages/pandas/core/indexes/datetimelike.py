...
class DatetimeIndexOpsMixin(NDArrayBackedExtensionIndex):
        ...
        if not isinstance(other, Index):
            ...
            if should_try:
                try:
                    ...
                    # e.g.
                    #  ValueError -> cannot parse str entry, or
    OutOfBoundsDatetime
                    #  TypeError  -> trying to convert IntervalIndex to
    DatetimeIndex
    ...
    def _format_with_header(
        self, header: list[str], na_rep: str = "NaT", date_format: str | None
    = None
    ) -> list[str]:
    ...
    def _partial_date_slice(
        ...
        if self.is_monotonic_increasing:
            ...
            if len(self) and (
                (t1 < self[0] and t2 < self[0]) or (t1 > self[-1] and t2 >
    self[-1])
            ):
                # we are out of range
                raise KeyError
...
class DatetimeTimedeltaMixin(DatetimeIndexOpsMixin):
    ...
    def is_type_compatible(self, kind: str) -> bool:
        warnings.warn(
            f"{type(self).__name__}.is_type_compatible is deprecated and will
    be "
    ...
    def _wrap_range_setop(self, other, res_i8):
        ...
        # because test_setops_preserve_freq fails with _validate_frequency
    raising.
    ...
    def _intersection(self, other: Index, sort=False) -> Index:
        """
        intersection specialized to the case with matching dtypes and both
    non-empty.
        """
        ...
        if self.freq is not None:
            if is_integer(loc):
                ...
                if is_list_like(loc):
                    ...
                    loc = lib.maybe_indices_to_slice(  # type:
    ignore[assignment]
    ...
    def _get_insert_freq(self, loc: int, item):
        ...
        if self.freq is not None:
            ...
            if self.size:
                if item is NaT:
                    pass
                elif (loc == 0 or loc == -len(self)) and item + self.freq ==
    self[0]:
                    freq = self.freq
