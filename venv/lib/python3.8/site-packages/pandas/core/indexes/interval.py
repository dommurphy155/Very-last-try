...
def _get_next_label(label):
    ...
    if is_datetime_or_timedelta_dtype(dtype) or is_datetime64tz_dtype(dtype):
        ...
        raise TypeError(f"cannot determine next label for type
    {repr(type(label))}")
...
@Appender(
    ...
    % {
        "klass": "IntervalIndex",
        "summary": "Immutable index of intervals that are closed on the same
    side.",
    ...
    Examples
    --------
    A new ``IntervalIndex`` is typically constructed using:func:`interval_range`:
    ...
    methods::meth:`IntervalIndex.from_arrays`,:meth:`IntervalIndex.from_breaks`, and:meth:`IntervalIndex.from_tuples`.
...
@inherit_names(["is_non_overlapping_monotonic", "closed"], IntervalArray,
    cache=True)
class IntervalIndex(ExtensionIndex):
    ...
    # --------------------------------------------------------------------
    # error: Return type "IntervalTree" of "_engine" incompatible with return
    type
    ...
    def _multiindex(self) -> MultiIndex:
        return MultiIndex.from_arrays([self.left, self.right], names=["left",
    "right"])
    ...
    def _maybe_convert_i8(self, key):
        ...
        -------
        scalar or list-like
            The original key if no conversion occurred, int if converted
    scalar,
    ...
    "left"):
        ...
        if isinstance(label, (IntervalMixin, IntervalIndex)):
            raise NotImplementedError("Interval objects are not currently
    supported")
    ...
    self._index_as_unique
        ...
        elif is_object_dtype(target.dtype) or not
    self._should_partial_index(target):
            ...
            target = self._maybe_convert_i8(target)
            indexer, missing =
    self._engine.get_indexer_non_unique(target.values)

        return ensure_platform_int(indexer), ensure_platform_int(missing)

    def _get_indexer_unique_sides(self, target: IntervalIndex) ->
    npt.NDArray[np.intp]:
        """
        _get_indexer specialized to the case where both of our sides are
    unique.
        """
        ...
        for i, key in enumerate(target):
            try:
                ...
                if isinstance(locs, slice):
                    ...
                    locs = np.arange(locs.start, locs.stop, locs.step,
    dtype="intp")
    ...
    def _convert_slice_indexer(self, key: slice, kind: str):
        if not (key.step is None or key.step == 1):
            ...
            msg = "label-based slicing with step!=1 is not supported for
    IntervalIndex"
            if kind == "loc":
    ...
    def _should_fallback_to_positional(self) -> bool:
        ...
        # error: Item "ExtensionDtype"/"dtype[Any]" of "Union[dtype[Any],
        # ExtensionDtype]" has no attribute "subtype"
        return self.dtype.subtype.kind in ["m", "M"]  # type:
    ignore[union-attr]
    ...
    def _format_native_types(
        ...
        return super()._format_native_types(na_rep=na_rep, quoting=quoting,
    **kwargs)
    ...
    def _from_join_target(self, result):
        raise NotImplementedError("IntervalIndex does not use libjoin
    fastpaths")
...
def interval_range(
    ...
    if not _is_valid_endpoint(start):
        raise ValueError(f"start must be numeric or datetime-like, got
    {start}")
    ...
    if freq is not None and not is_number(freq):
        try:
            ...
            raise ValueError(
                f"freq must be numeric or convertible to DateOffset, got
    {freq}"
            ) from err
    ...
    if is_number(endpoint):
        ...
        if all(is_integer(x) for x in com.not_none(start, end, freq)):
            ...
            # error: Argument 1 to "maybe_downcast_numeric" has incompatible
    type
        ...
        if isinstance(endpoint, Timestamp):
            breaks = date_range(start=start, end=end, periods=periods,
    freq=freq)
        else:
