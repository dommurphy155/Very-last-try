...
class NumericIndex(Index):
    ...
    NumericIndex is a special case of `Index` with purely numpy int/uint/float
    labels.
    ...
    Notes
    -----
    An NumericIndex instance can **only** contain numpy int64/32/16/8,
    uint64/32/16/8 or
    float64/32/16 dtype. In particular, ``NumericIndex`` *can not* hold Pandas
    numeric
    ...
    def _ensure_array(cls, data, dtype, copy: bool):
        ...
        if copy or not is_dtype_equal(data.dtype, dtype):
            # TODO: the try/except below is because it's difficult to predict
    the error
            # and/or error message from different combinations of data and
    dtype.
    ...
    def _validate_dtype(cls, dtype: Dtype | None) -> None:
        ...
        if not validation_func(dtype):
            raise ValueError(
                f"Incorrect `dtype` passed: expected {expected}, received
    {dtype}"
            )
    ...
    def _ensure_dtype(cls, dtype: Dtype | None) -> np.dtype | None:
        """
        Ensure int64 dtype for Int64Index etc. but allow int32 etc. for
    NumericIndex.
    ...
    def _convert_tolerance(self, tolerance, target):
        ...
        if not np.issubdtype(tolerance.dtype, np.number):
            if tolerance.ndim > 0:
                raise ValueError(
                    f"tolerance argument for {type(self).__name__} must
    contain "
                    "numeric elements if it is list type"
                )
            else:
                raise ValueError(
                    f"tolerance argument for {type(self).__name__} must be
    numeric "
    ...
    @classmethod
    def _assert_safe_casting(cls, data: np.ndarray, subarr: np.ndarray) ->
    None:
        """
        Ensure incoming data can be represented with matching signed-ness.

        Needed if the process of casting data from some accepted dtype to the
    internal
    ...
    def _format_native_types(
        self, *, na_rep="", float_format=None, decimal=".", quoting=None,
    **kwargs
...
_num_index_shared_docs[
    ...
    .. deprecated:: 1.4.0
        In pandas v2.0 %(klass)s will be removed and:class:`NumericIndex`
    used instead.
        %(klass)s will remain fully functional for the duration of pandas 1.x.

    The basic object storing axis labels for all pandas objects.
    %(klass)s is a special case of `Index` with purely %(ltype)s labels.
    %(extra)s.
...
class UInt64Index(IntegerIndex):
    ...
    _dtype_validation_metadata = (is_unsigned_integer_dtype, "unsigned
    integer")
