...
class Block(PandasObject):
    ...
    def is_bool(self) -> bool:
        """
        We can be bool if a) we are bool dtype or b) object dtype with bool
    objects.
        """
    ...
    def _standardize_fill_value(self, value):
        ...
        if self.dtype != _dtype_obj and is_valid_na_for_dtype(value,
    self.dtype):
    ...
    def __repr__(self) -> str:
        ...
        if self.ndim == 1:
            ...
            result = f"{name}: {self.mgr_locs.indexer}, {shape}, dtype:
    {self.dtype}"
    ...
    def getitem_block(self, slicer: slice | npt.NDArray[np.intp]) -> Block:
        ...
        # Invalid index type "Union[slice, ndarray[Any,
    ...
    def should_store(self, value: ArrayLike) -> bool:
        """
        Should we set self.values[indexer] = value inplace or do we need to
    cast?
    ...
    @final
    def _maybe_downcast(self, blocks: list[Block], downcast=None) ->
    list[Block]:
    ...
    @final
    def astype(
        self, dtype: DtypeObj, copy: bool = False, errors: IgnoreRaise =
    "raise"
    ) -> Block:
        """
    ...
    def to_native_types(self, na_rep="nan", quoting=None, **kwargs) -> Block:
        ...
        result = to_native_types(self.values, na_rep=na_rep, quoting=quoting,
    **kwargs)
    ...
    def replace_list(
        ...
        pairs = [
            (x, y) for x, y in zip(src_list, dest_list) if
    self._can_hold_element(x)
        ...
        # "Union[ExtensionArray, ndarray, bool]"; expected
    "Union[ExtensionArray,
        # ndarray]"
        masks = [extract_bool_array(x) for x in masks]  # type:
    ignore[arg-type]
    ...
    bool]";
                ...
                if convert and blk.is_object and not all(x is None for x in
    dest_list):
    ...
    def set_inplace(self, locs, values: ArrayLike, copy: bool = False) -> None:
        ...
        If copy=True, first copy the underlying values in place before
    modifying
        (for Copy-on-Write).
    ...
    coerce_to_target_dtype
        new_values = algos.take_nd(
            values, indexer, axis=axis, allow_fill=allow_fill,
    ...
    def _unstack(
        ...
        #  which the calling function needs in order to pass
        ...
        try:
            ...
            if self.dtype == _dtype_obj:
                ...
                if lib.is_list_like(vi):
                    ...
                    casted = setitem_datetimelike_compat(values, len(vi),
    casted)
    ...
    def fillna(
        self, value, limit: int | None = None, inplace: bool = False,
    downcast=None
    ) -> list[Block]:
        """
    ...
    def interpolate(
        ...
        **kwargs,
        ...
        if m is None and self.dtype.kind != "f":
            ...
            # bc we already checked that can_hold_na, we dont have int dtype
    here
            # test_interp_basic checks that we make a copy here
            return [self] if inplace else [self.copy()]

        if self.is_object and self.ndim == 2 and self.shape[0] != 1 and axis
    == 0:
    ...
    def shift(self, periods: int, axis: int = 0, fill_value: Any = None) ->
    list[Block]:
        ...
        #  NDFrame.shift.  If that ever changes, we can do a check for
    periods=0
    ...
    DataFrame
        ...
        result = quantile_compat(self.values, np.asarray(qs._values),
    interpolation)
...
class EABackedBlock(Block):
    ...
    positive
        ...
        #  tests for this.
        return super().fillna(value, limit=limit, inplace=inplace,
    downcast=downcast)
    ...
    def interpolate(
        self, method="pad", axis=0, inplace=False, limit=None,
    fill_value=None, **kwargs
    ):
        values = self.values
        if values.ndim == 2 and axis == 0:
            # NDArrayBackedExtensionArray.fillna assumes axis=1
            new_values = values.T.fillna(value=fill_value, method=method,
    limit=limit).T
        else:
            new_values = values.fillna(value=fill_value, method=method,
    limit=limit)
...
class ExtensionBlock(libinternals.Block, EABackedBlock):
    ...
    def _unwrap_setitem_indexer(self, indexer):
        ...
        if isinstance(indexer, tuple):
            ...
            #  Should never have length > 2.  Caller is responsible for
    checking.
            #  Length 1 is reached vis setitem_single_block and
    setitem_single_column
            #  each of which pass indexer=(pi,)
            if len(indexer) == 2:

                if all(isinstance(x, np.ndarray) and x.ndim == 2 for x in
    indexer):
                    # GH#44703 went through indexing.maybe_convert_ix
                    first, second = indexer
                    if not (
                        second.size == 1 and (second == 0).all() and
    first.shape[1] == 1
                    ):
                        raise NotImplementedError(
                            "This should not be reached. Please report a bug
    at "
                            "github.com/pandas-dev/pandas/"
                        )
                    indexer = first[:, 0]
                    ...
                    # reached via setitem_single_block passing the whole
    indexer
                    indexer = indexer[0]
    ...
    def getitem_block_index(self, slicer: slice) -> ExtensionBlock:
        ...
        """
        # GH#42787 in principle this is equivalent to values[..., slicer], but
    we don't
    ...
    verify_integrity=False
        ...
        blocks = [
            ...
            for i, (indices, place) in enumerate(zip(new_values,
    new_placement))
        ]
        return blocks, mask
...
class NDArrayBackedExtensionBlock(libinternals.NDArrayBackedBlock,
    EABackedBlock):
    """
...
def _catch_deprecated_value_error(err: Exception) -> None:
    """
    We catch ValueError for now, but only a specific one raised by
    DatetimeArray
...
def maybe_coerce_values(values: ArrayLike) -> ArrayLike:
    ...
    if isinstance(values, (DatetimeArray, TimedeltaArray)) and values.freq is
    not None:
...
def ensure_block_shape(values: ArrayLike, ndim: int = 1) -> ArrayLike:
    ...
    if values.ndim < ndim:
        if not is_1d_only_ea_dtype(values.dtype):
            ...
            values = cast("np.ndarray | DatetimeArray | TimedeltaArray",
    values)
...
def to_native_types(
    ...
    **kwargs,
    ...
    """convert to our native types format"""
    if isinstance(values, Categorical) and values.categories.dtype.kind in
    "Mm":
