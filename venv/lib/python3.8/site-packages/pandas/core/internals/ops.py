...
def operate_blockwise(
    ...
    res_blks: list[Block] = []
    for lvals, rvals, locs, left_ea, right_ea, rblk in _iter_block_pairs(left,
    right):
        ...
        #    assert len(nbs) == 1
        # else:
        #    assert res_values.shape == lvals.shape, (res_values.shape,
    lvals.shape)
    ...
    new_mgr = type(right)(tuple(res_blks), axes=right.axes,
