...
def arrays_to_mgr(
    ...
    *,
    ...
    if typ == "block":
        ...
        raise ValueError(f"'typ' needs to be one of {{'block', 'array'}}, got
    '{typ}'")
...
def rec_array_to_mgr(
    ...
    # error: Argument 1 to "reorder_arrays" has incompatible type
    "List[ndarray]";
...
def fill_masked_arrays(data: MaskedRecords, arr_columns: Index) ->
    list[np.ndarray]:
    """
    ...
    if typ == "block":
        if isinstance(mgr, BlockManager):
            ...
            if mgr.ndim == 2:
                ...
                new_mgr = SingleBlockManager.from_array(mgr.arrays[0],
    mgr.index)
    elif typ == "array":
...
def ndarray_to_mgr(
    ...
    if is_1d_only_ea_dtype(vdtype) or is_1d_only_ea_dtype(dtype):
        ...
        if isinstance(values, (np.ndarray, ExtensionArray)) and values.ndim >
    1:
    ...
    if dtype is None and is_object_dtype(values.dtype):
        ...
        if any(x is not y for x, y in zip(obj_columns, maybe_datetime)):
            dvals_list = [ensure_block_shape(dval, 2) for dval in
    maybe_datetime]
            block_values = [
...
def dict_to_mgr(
    ...
    *,
    ...
    nan_dtype)
        ...
        arrays = [arr if not isinstance(arr, Index) else arr._data for arr in
    arrays]
    ...
    if copy:
        if typ == "block":
            ...
            #  only EA arrays
            arrays = [x.copy() if isinstance(x, ExtensionArray) else x for x
    in arrays]
        else:
...
def _prep_ndarraylike(
    ...
    if isinstance(values, TimedeltaArray) or (
        ...
        # By retaining DTA/TDA instead of unpacking, we end up retaining
    non-nano
        pass
    ...
    if len(data) == 0:
        ...
        for val in data:
            if isinstance(val, ABCSeries):
                ...
                raise ValueError("Per-column arrays must each be
    1-dimensional")

        if not indexes and not raw_lengths:
            raise ValueError("If using all scalar values, you must pass an
    index")
        ...
        if have_raw_arrays:
            ...
            if have_dicts:
                raise ValueError(
                    "Mixing dicts with non-Series may lead to ambiguous
    ordering."
                )
    ...
    if not len(data):
        if isinstance(data, np.ndarray):
            if data.dtype.names is not None:
                ...
                if len(data) == 0:
                    # GH#42456 the indexing above results in list of 2D
    ndarrays
...
def _list_of_series_to_arrays(
    ...
    if columns is None:
        ...
        pass_data = [x for x in data if isinstance(x, (ABCSeries,
    ABCDataFrame))]
...
def _validate_or_indexify_columns(
    ...
    equal
            if len({len(col) for col in columns}) > 1:
                raise ValueError(
                    "Length of columns passed for MultiIndex columns is
    different"
                )
