...
class BaseArrayManager(DataManager):
    ...
    __slots__ = [
        "_axes",  # private attribute, because 'axes' has different order, see
    below
        "arrays",
    ]
    ...
    def make_empty(self: T, axes=None) -> T:
        """Return an empty ArrayManager with the items axis of len 0 (no
    columns)"""
        if axes is None:
    ...
    aligned
        ...
        # error: Argument 1 to "ArrayManager" has incompatible type
    "List[ndarray]";
        # expected "List[Union[ndarray, ExtensionArray]]"
        return type(self)(result_arrays, new_axes)  # type: ignore[arg-type]

    def apply_with_block(self: T, f, align_keys=None, swap_axis=True,
    **kwargs) -> T:
        ...
        for i, arr in enumerate(self.arrays):
            ...
            if isinstance(arr.dtype, np.dtype) and not isinstance(arr,
    np.ndarray):
            ...
            if self.ndim == 2:
                ...
                block = new_block(arr, placement=slice(0, len(self), 1),
    ndim=1)
            ...
            if self.ndim == 2 and arr.ndim == 2:
                ...
                assert len(arr) == 1
                # error: No overload variant of "__getitem__" of
    "ExtensionArray"
    ...
    def astype(self: T, dtype, copy: bool = False, errors: str = "raise") -> T:
        return self.apply(astype_array_safe, dtype=dtype, copy=copy,
    errors=errors)
    ...
    def get_bool_data(self: T, copy: bool = False) -> T:
        """
        Select columns that are bool-dtype and object-dtype columns that are
    all-bool.
    ...
    List[Any],
        ...
        values = np.empty(self.shape_proper[0], dtype=dtype)  # type:
    ignore[arg-type]
...
class ArrayManager(BaseArrayManager):
    ...
    def _verify_integrity(self) -> None:
        ...
        if not len(self.arrays) == n_columns:
            raise ValueError(
                "Number of passed arrays must equal the size of the column
    Index: "
    ...
    Index: "
            ...
            if not isinstance(arr, (np.ndarray, ExtensionArray)):
                raise ValueError(
                    "Passed arrays should be np.ndarray or ExtensionArray
    instances, "
    ...
    False
        ...
        if lib.is_integer(loc):
            ...
            # TODO can we avoid needing to unpack this here? That means
    converting
            ...
            # TODO we receive a datetime/timedelta64 ndarray from
    DataFrame._iset_item
    ...
    def idelete(self, indexer) -> ArrayManager:
        """
        Delete selected locations in-place (new block and array, same
    BlockManager)
        """
    ...
    -> T:
        """
        Apply grouped reduction function columnwise, returning a new
    ArrayManager.
    ...
    "List[ndarray]";
        # expected "List[Union[ndarray, ExtensionArray]]"
        return type(self)(result_arrays, [index, columns])  # type:
    ignore[arg-type]
    ...
    def reduce(
        ...
        """
        Apply reduction function column-wise, returning a single-row
    ArrayManager.
        ...
        for i, arr in enumerate(self.arrays):
            try:
                ...
                # TODO NaT doesn't preserve dtype, so we need to ensure to
    create
                ...
                if res is NaT and is_timedelta64_ns_dtype(arr.dtype):
                    result_arrays.append(np.array(["NaT"],
    ...
    "List[ndarray]";
        # expected "List[Union[ndarray, ExtensionArray]]"
        new_mgr = type(self)(result_arrays, [index, columns])  # type:
    ignore[arg-type]
    ...
    def operate_blockwise(self, other: ArrayManager, array_op) -> ArrayManager:
        ...
        result_arrays = [
            array_op(left, right) for left, right in zip(left_arrays,
    right_arrays)
        ]
        return type(self)(result_arrays, self._axes)
    ...
    def quantile(
        ...
        *,
        ...
        assert axis == 1
        new_arrs = [
            quantile_compat(x, np.asarray(qs._values), interpolation) for x in
    arrs
        ]
    ...
    def unstack(self, unstacker, fill_value) -> ArrayManager:
        ...
        if unstacker.mask.all():
            ...
            allow_fill = True
            # calculating the full mask once and passing it to take_1d is
    faster
    ...
    [index]
                    ...
                    new_arr = take_1d(arr, new_indexer2D[:, i],
    allow_fill=False)
...
class NullArrayProxy:
    ...
    def to_array(self, dtype: DtypeObj) -> ArrayLike:
        """
        Helper function to create the actual all-NA array from the
    NullArrayProxy
        object.
