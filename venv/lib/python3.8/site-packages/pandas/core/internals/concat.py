...
def _concatenate_array_managers(
    ...
    manager
        ...
        arrays = list(itertools.chain.from_iterable([mgr.arrays for mgr in
    mgrs]))
...
def concat_arrays(to_concat: list) -> ArrayLike:
    ...
    to_concat_no_proxy = [x for x in to_concat if not isinstance(x,
    NullArrayProxy)]
    ...
    elif all(x.kind in ["i", "u", "b"] and isinstance(x, np.dtype) for x in
    dtypes):
        # GH#42092
        target_dtype = np.find_common_type(list(dtypes), [])
    else:
        target_dtype = find_common_type([arr.dtype for arr in
    to_concat_no_proxy])
    ...
    # TODO decide on exact behaviour (we shouldn't do this only for empty
    result)
...
def concatenate_managers(
    ...
    concat_plans = [
        _get_mgr_concatenation_plan(mgr, indexers) for mgr, indexers in
    mgrs_indexers
    ]
    ...
    for placement, join_units in concat_plan:
        ...
        if len(join_units) == 1 and not join_units[0].indexers:
            ...
            values = _concatenate_join_units(join_units, concat_axis,
    copy=copy)
            fastpath = False
...
def _maybe_reindex_columns_na_proxy(
    axes: list[Index], mgrs_indexers: list[tuple[BlockManager, dict[int,
    np.ndarray]]]
    ...
    new_mgrs_indexers = []
    for mgr, indexers in mgrs_indexers:
        # We only reindex for axis=0 (i.e. columns), as this can be done
    cheaply
        if 0 in indexers:
...
def _get_mgr_concatenation_plan(mgr: BlockManager, indexers: dict[int,
    ...
    group=False):
        ...
        unit_no_ax0_reindexing = (
            ...
            (
                (blk.mgr_locs.is_slice_like and blk.mgr_locs.as_slice.step ==
    1)
                or
...
class JoinUnit:
    ...
    def _is_valid_na_for(self, dtype: DtypeObj) -> bool:
        """
        Check that we are all-NA of a type/dtype that is compatible with this
    dtype.
        Augments `self.is_na` with an additional check of the type of NA
    values.
        """
        if not self.is_na:
            return False
        ...
        if self.dtype == object:
            ...
            return all(is_valid_na_for_dtype(x, dtype) for x in
    values.ravel(order="K"))
        ...
        if na_value is NaT and not is_dtype_equal(self.dtype, dtype):
            ...
            # fill_values match but we should not cast self.block.values to
    dtype
        ...
        if upcasted_na is None and self.block.dtype.kind != "V":
            ...
            if self._is_valid_na_for(empty_dtype):
                ...
                if isinstance(empty_dtype, DatetimeTZDtype):
                    ...
                    if is_dtype_equal(blk_dtype, empty_dtype) and
    self.indexers:
                        # avoid creating new empty array if we already have an
    array
    ...
    dtype=empty_dtype)
                    ...
                    # NB: we should never get here with empty_dtype integer or
    bool;
...
def _concatenate_join_units(
    ...
    if len(to_concat) == 1:
        ...
        # concatting with at least one EA means we are concatting a single
    column
...
def _get_empty_dtype(join_units: Sequence[JoinUnit]) -> DtypeObj:
    ...
    if not len(dtypes):
        dtypes = [unit.dtype for unit in join_units if unit.block.dtype.kind
    != "V"]
...
def _is_uniform_join_units(join_units: list[JoinUnit]) -> bool:
    ...
    Int64+int64
        ...
        all(
            ...
            # GH#42092 we only want the dtype_equal check for non-numeric
    blocks
            ...
            if min_len == max_len:
                ...
                for i, (plc, unit) in enumerate(next_items):
                    ...
                    if len(plc) > min_len:
                        ...
                        next_items[i] = (plc[min_len:], _trim_join_unit(unit,
    min_len))
                    else:
