...
class BaseBlockManager(DataManager):
    ...
    def __init__(self, blocks, axes, refs=None, verify_integrity: bool = True)
    -> None:
        raise NotImplementedError
    ...
    def blknos(self) -> npt.NDArray[np.intp]:
        ...
        blknos[i] identifies the block from self.blocks that contains this
    column.
    ...
    def arrays(self) -> list[ArrayLike]:
        ...
        ArrayManager method (list of 1D arrays vs iterator of 2D ndarrays / 1D
    EAs).
    ...
    def setitem(self: T, indexer, value) -> T:
        ...
        if _using_copy_on_write() and not self._has_no_reference(0):
            # if being referenced -> perform Copy-on-Write and clear the
    reference
            # this method is only called if there is a single block ->
    hardcoded 0
            self = self.copy()
    ...
    def putmask(self, mask, new, align: bool = True):
        if _using_copy_on_write() and any(
            not self._has_no_reference_block(i) for i in
    range(len(self.blocks))
        ):
            # some reference -> copy full dataframe
            # TODO(CoW) this could be optimized to only copy the blocks that
    would
            # get modified
            self = self.copy()
    ...
    def shift(self: T, periods: int, axis: int, fill_value) -> T:
        ...
        return self.apply("shift", periods=periods, axis=axis,
    ...
    def fillna(self: T, value, limit, inplace: bool, downcast) -> T:
        ...
        if inplace:
            # TODO(CoW) can be optimized to only copy those blocks that have
    refs
            if _using_copy_on_write() and any(
                not self._has_no_reference_block(i) for i in
    range(len(self.blocks))
        ...
        return self.apply(
            "fillna", value=value, limit=limit, inplace=inplace,
    downcast=downcast
        )
    ...
    def _combine(
        self: T, blocks: list[Block], copy: bool = True, index: Index | None =
    None
    ) -> T:
        ...
        indexer = np.sort(np.concatenate([b.mgr_locs.as_array for b in
    blocks]))
    ...
    def consolidate(self: T) -> T:
        ...
        bm = type(self)(self.blocks, self.axes, self.refs,
        ...
        if self.is_single_block:
            ...
            if sl_type == "slice":
                ...
                if not allow_fill and only_slice:
                    ...
                    # We have
                    #  all(np.shares_memory(nb.values, blk.values) for nb in
    blocks)
            ...
            if blkno == -1:
                ...
                if not blk._can_consolidate and not blk._validate_ndim:
                    ...
                    for mgr_loc in mgr_locs:
                        ...
                        newblk.mgr_locs = BlockPlacement(slice(mgr_loc,
    mgr_loc + 1))
                    ...
                    if isinstance(taker, slice):
                        nb = blk.getitem_block_columns(taker,
    new_mgr_locs=mgr_locs)
                        ...
                        for i, ml in zip(taker, mgr_locs):
                            ...
                            nb = blk.getitem_block_columns(slc,
    new_mgr_locs=bp)
    ...
    def _make_na_block(
        self, placement: BlockPlacement, fill_value=None, use_na_proxy: bool =
    False
    ) -> Block:
    ...
    "Union[dtype,
        # ExtensionDtype]"; expected "Union[dtype, None, type, _SupportsDtype,
    str,
        # Tuple[Any, int], Tuple[Any, Union[int, Sequence[int]]], List[Any],
    _DtypeDict,
        # Tuple[Any, Any]]"
        block_values = np.empty(block_shape, dtype=dtype)  # type:
    ignore[arg-type]
...
class BlockManager(libinternals.BlockManager, BaseBlockManager):
    ...
    "
                ...
                if isinstance(block, DatetimeTZBlock) and block.values.ndim ==
    1:
    ...
    type
                    ...
                    # "DatetimeArray")
                    block.values = ensure_block_shape(  # type:
    ignore[assignment]
    ...
    def from_blocks(
        ...
        """
        Constructor for BlockManager and SingleBlockManager with same
    signature.
        """
    ...
    def fast_xs(self, loc: int) -> SingleBlockManager:
        ...
        if isinstance(dtype, ExtensionDtype) and not immutable_ea:
            ...
            # "None"
            result = np.empty(
                n, dtype=object if immutable_ea else dtype  # type:
    ignore[arg-type]
            )
        ...
        if immutable_ea:
            ...
            result = dtype.construct_array_type()._from_sequence(result,
    dtype=dtype)
    ...
    def iset(
        self, loc: int | slice | np.ndarray, value: ArrayLike, inplace: bool =
    False
    ):
        """
        ...
        unfit_val_locs = []
        removed_blknos = []
        for blkno_l, val_locs in libinternals.get_blkno_placements(blknos,
    group=True):
            blk = self.blocks[blkno_l]
            blk_locs = blklocs[val_locs.indexer]
            if inplace and blk.should_store(value):
                # Updating inplace -> check if we need to do Copy-on-Write
                if _using_copy_on_write() and not
    self._has_no_reference_block(blkno_l):
    ...
    copy=True)
                ...
                if len(val_locs) == len(blk.mgr_locs):
                    ...
                    blocks_tup = (
                        self.blocks[:blkno_l] + (nb,) + self.blocks[blkno_l +
    1:]
                    )
                    self.blocks = blocks_tup
                    self._blklocs[nb.mgr_locs.indexer] = np.arange(len(nb))
                    # blk.delete gives a copy, so we can remove a possible
    reference
                    self._clear_reference_block(blkno_l)
    ...
    len(removed_blknos))
            ...
            self.blocks = tuple(
                blk for i, blk in enumerate(self.blocks) if i not in
    set(removed_blknos)
            )
            if self.refs is not None:
                self.refs = [
        ...
        if unfit_val_locs:
            ...
            if value_is_extension_type:
                ...
                self._blknos[unfit_idxr] = np.arange(unfit_count) +
    len(self.blocks)
    ...
    def _iset_single(
        ...
        if inplace and blk.should_store(value):
            copy = False
            if _using_copy_on_write() and not
    self._has_no_reference_block(blkno):
    ...
    def column_setitem(
        self, loc: int, idx: int | slice | np.ndarray, value, inplace: bool =
    False
    ) -> None:
        """
        Set values ("setitem") into a single column (not setting the full
    column).

        This is a method on the BlockManager level, to avoid creating an
        intermediate Series at the DataFrame level (`s = df[loc]; s[idx] =
    value`)
        """
        ...
        # this manager is only created temporarily to mutate the values in
    place
    ...
    {value.T.shape}"
        ...
        if sum(not block.is_extension for block in self.blocks) > 100:
            warnings.warn(
                ...
                "of calling `frame.insert` many times, which has poor
    performance.  "
                "Consider joining all columns at once using pd.concat(axis=1) "
                "instead. To get a de-fragmented frame, use `newframe =
    frame.copy()`",
                PerformanceWarning,
                stacklevel=find_stack_level(),
            )
    ...
    def _insert_update_blklocs_and_blknos(self, loc) -> None:
        ...
        if loc == self.blklocs.shape[0]:
            ...
            self._blknos = np.append(self._blknos[::-1],
    len(self.blocks))[::-1]
        else:
    ...
    def idelete(self, indexer) -> BlockManager:
        ...
        # TODO this might not be needed (can a delete ever be done in chained
    manner?)
        parent = None if com.all_none(*new_refs) else self
        return type(self)(tuple(nbs), axes, new_refs, parent,
    verify_integrity=False)

    # ----------------------------------------------------------------
    # Block-wise Operation

    def grouped_reduce(self: T, func: Callable, ignore_failures: bool = False)
    -> T:
        """
        Apply grouped reduction function blockwise, returning a new
    BlockManager.
    ...
    def reduce(
        ...
        """
        Apply reduction function blockwise, returning a single-row
    BlockManager.
        ...
        if ignore_failures:
            if res_blocks:
                indexer = np.concatenate([blk.mgr_locs.as_array for blk in
    res_blocks])
    ...
    def unstack(self, unstacker, fill_value) -> BlockManager:
        ...
        if allow_fill:
            # calculating the full mask once and passing it to Block._unstack
    is
    ...
    def to_dict(self, copy: bool = True):
        ...
        return {dtype: self._combine(blocks, copy=copy) for dtype, blocks in
    bd.items()}
    ...
    def as_array(
        ...
        if self.is_single_block:
            ...
            if blk.is_extension:
                ...
                # error: Item "ndarray" of "Union[ndarray, ExtensionArray]"
    has no
                # attribute "to_numpy"
    ...
    def _consolidate_inplace(self) -> None:
        ...
        if not self.is_consolidated():
            if self.refs is None:
                ...
                self.blocks, self.refs = _consolidate_with_refs(self.blocks,
    self.refs)
            self._is_consolidated = True
            self._known_consolidated = True
            self._rebuild_blknos_and_blklocs()
...
class SingleBlockManager(BaseBlockManager, SingleDataManager):
    ...
    def to_2d_mgr(self, columns: Index) -> BlockManager:
        ...
        return BlockManager(
            [new_blk], axes=axes, refs=refs, parent=parent,
    verify_integrity=False
        )
    ...
    def _has_no_reference(self, i: int = 0) -> bool:
        ...
        Returns True if the column has no references.
        """
        return (self.refs is None or self.refs[0] is None) and
    ...
    def getitem_mgr(self, indexer: slice | npt.NDArray[np.bool_]) ->
    SingleBlockManager:
        ...
        # TODO(CoW) in theory only need to track reference if new_array is a
    view
        ref = weakref.ref(blk)
    ...
    def get_slice(self, slobj: slice, axis: int = 0) -> SingleBlockManager:
        ...
        # TODO this method is only used in groupby SeriesSplitter at the
    moment,
    ...
    def setitem_inplace(self, indexer, value) -> None:
        ...
        in place, not returning a new Manager (and Block), and thus never
    changing
        the dtype.
        """
        if _using_copy_on_write() and not self._has_no_reference(0):
...
def create_block_manager_from_blocks(
    ...
    #  set(x for blk in blocks for x in blk.mgr_locs) ==
    set(range(len(axes[0])))
...
def construction_error(
    ...
    return ValueError(f"Shape of passed values is {passed}, indices imply
    {implied}")
...
def _grouping_func(tup: tuple[int, ArrayLike]) -> tuple[int, bool, DtypeObj]:
    # compat for numpy<1.21, in which comparing a np.dtype with an
    ExtensionDtype
    # raises instead of returning False. Once earlier numpy versions are
    dropped,
...
def _form_blocks(arrays: list[ArrayLike], consolidate: bool) -> list[Block]:
    ...
    for (_, _, dtype), tup_block in grouper:
        ...
        if isinstance(dtype, np.dtype):
            ...
            blk = block_type(values, placement=BlockPlacement(placement),
    ndim=2)
            nbs.append(blk)
    ...
    placement=BlockPlacement(x[0]), ndim=2
                )
                for x in tup_block
            ]
            nbs.extend(dtype_blocks)
...
def _tuples_to_blocks_no_consolidate(tuples) -> list[Block]:
    ...
    return [
        new_block_2d(ensure_block_shape(x[1], ndim=2),
...
def _merge_blocks(
    ...
    List[Union[ndarray,
            ...
            new_values = np.vstack([b.values for b in blocks])  # type:
    ignore[misc]
        else:
