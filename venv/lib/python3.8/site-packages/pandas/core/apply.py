...
class Apply(metaclass=abc.ABCMeta):
    ...
    def transform(self) -> DataFrame | Series:
        ...
        # error: Argument 1 to "__get__" of "AxisProperty" has incompatible
    type
        # "Union[Series, DataFrame, GroupBy[Any], SeriesGroupBy,
        # DataFrameGroupBy, BaseWindow, Resampler]"; expected "Union[DataFrame,
        # Series]"
        if not isinstance(result, (ABCSeries, ABCDataFrame)) or not
    result.index.equals(
    ...
    def transform_dict_like(self, func):
        ...
        if len(failed_names) > 0:
            warnings.warn(
                f"{failed_names} did not transform successfully. If any error
    is "
    ...
    def transform_str_or_callable(self, func) -> DataFrame | Series:
        ...
        if isinstance(func, str):
            return self._try_aggregate_string_function(obj, func, *args,
    **kwargs)
    ...
    def agg_list_like(self) -> DataFrame | Series:
        ...
        if selected_obj.ndim == 1:
            for a in arg:
                colg = obj._gotitem(selected_obj.name, ndim=1,
    subset=selected_obj)
                try:
        ...
        # multiples
            ...
            for index, col in enumerate(selected_obj):
                colg = obj._gotitem(col, ndim=1, subset=selected_obj.iloc[:,
    index])
                try:
                    # Capture and suppress any warnings emitted by us in the
    call
    ...
    https://bugs.python.org/issue29672
                    ...
                    if len(record) > 0:
                        match =
    re.compile(depr_nuisance_columns_msg.format(".*"))
                    ...
                    if "Must produce aggregated value" in str(err):
                        ...
                        #  the selected_obj.ndim == 1 branch above with arg ==
    ["sum"]
        ...
        try:
            ...
            # Concat uses the first index to determine the final indexing
    order.
            # The union of a shorter first index with the other indices causes
            # the index sorting to be different from the order of the
    aggregating
            # functions. Reindex if this is the case.
            index_size = concatenated.index.size
            full_ordered_index = next(
                result.index for result in results if result.index.size ==
    index_size
            )
    ...
    def agg_dict_like(self) -> DataFrame | Series:
        ...
        if selected_obj.ndim == 1:
            ...
            results = {
                key: obj._gotitem(key, ndim=1).agg(how) for key, how in
    arg.items()
            }
        ...
        if all(is_ndframe):
            ...
            result = concat(
                {k: results[k] for k in keys_to_use}, axis=axis,
    keys=keys_to_use
            )
        elif any(is_ndframe):
    ...
    def apply_str(self) -> DataFrame | Series:
        ...
        if callable(func):
            ...
            if self.axis != 0 and (
                ...
                raise ValueError(f"Operation {f} does not support axis=1")
        return self._try_aggregate_string_function(obj, f, *self.args,
    **self.kwargs)
    ...
    def _try_aggregate_string_function(self, obj, arg: str, *args, **kwargs):
        ...
        if f is not None:
            ...
            assert len(args) == 0
            assert len([kwarg for kwarg in kwargs if kwarg not in ["axis"]])
    == 0
            return f
        ...
        raise AttributeError(
            f"'{arg}' is not a valid function for '{type(obj).__name__}'
    object"
        )
...
class FrameApply(NDFrameApply):
    ...
    def agg(self):
        ...
        if result is None:
            result = self.obj.apply(self.orig_f, axis, args=self.args,
    **self.kwargs)
    ...
    def apply_raw(self):
        ...
        result = np.apply_along_axis(wrap_function(self.f), self.axis,
    self.values)

        # TODO: mixed type case
        if result.ndim == 2:
            return self.obj._constructor(result, index=self.index,
    ...
    def wrap_results(self, results: ResType, res_index: Index) -> DataFrame |
    Series:
        from pandas import Series
...
class FrameColumnApply(FrameApply):
    ...
    def infer_to_same_shape(self, results: ResType, res_index: Index) ->
    DataFrame:
...
class SeriesApply(NDFrameApply):
    ...
    def apply_standard(self) -> DataFrame | Series:
        ...
        with np.errstate(all="ignore"):
            ...
            # row-wise access
            if is_extension_array_dtype(obj.dtype) and hasattr(obj._values,
    "map"):
        ...
        if len(mapped) and isinstance(mapped[0], ABCSeries):
            # GH#43986 Need to do list(mapped) in order to get treated as
    nested
...
) -> tuple[bool, AggFuncType | None, list[str] | None, npt.NDArray[np.intp] |
    None]:
    """
    This is the internal function to reconstruct func given if there is
    relabeling
    or not and also normalize the keyword to get new order of columns.

    If named aggregation is applied, `func` will be None, and kwargs contains
    the
    column and aggregation function information to be parsed;
    If named aggregation is not applied, `func` is either string (e.g. 'min')
    or
    Callable, or list of them (e.g. ['min', np.max]), or the dictionary of
    column name
    and str/Callable/list of them (e.g. {'A': 'min'}, or {'A': [np.min, lambda
    x: x]})
    ...
    **kwargs: dict, kwargs used in is_multi_agg_with_relabel and
    ...
    if not relabeling:
        if isinstance(func, list) and len(func) > len(set(func)):
            ...
            # GH 28426 will raise error if duplicated function names are used
    and
    ...
    "
            ...
            # nicer error message
            raise TypeError("Must provide 'func' or tuples of '(column,
    aggfunc).")
...
def normalize_keyword_aggregation(
    ...
    # GH 25719, due to aggspec will change the order of assigned columns in
    aggregation
    # uniquified_aggspec will store uniquified order list and will compare it
    with order
    # based on index
    aggspec_order = [
...
def relabel_result(
    ...
    ...       "C": [6, np.nan, np.nan], "B": [np.nan, 4, 2.5]})  # doctest:
    +SKIP
    ...
    reorder_mask = not isinstance(result, ABCSeries) and len(result.columns) >
    1
    for col, fun in func.items():
        s = result[col].dropna()

        # In the `_aggregate`, the callable names are obtained and used in
    `result`, and
        ...
        # accordingly so need to get the callable name if it is not parsed
    names, and
        # reorder the aggregated result for each column.
        # e.g. if df.agg(c1=("C2", sum), c2=("C2", lambda x: min(x))), correct
    order is
        # [sum, <lambda>], but in `result`, it will be [<lambda>, sum], and we
    need to
        # reorder so that aggregated values map to their functions regarding
    the order.

        # However there is only one column being used for aggregation, not
    need to
        # reorder since the index is not sorted, and keep as is in `funcs`,
    e.g.
        #         A
        # min   1.0
        # mean  1.5
        ...
        if reorder_mask:
            fun = [
                com.get_callable_name(f) if not isinstance(f, str) else f for
    f in fun
            ]
            col_idx_order = Index(s.index).get_indexer(fun)
            s = s[col_idx_order]

        # assign the new user-provided "named aggregation" as index names, and
    reindex
...
def validate_func_kwargs(
    ...
    for col_func in kwargs.values():
        if not (isinstance(col_func, str) or callable(col_func)):
            raise
    TypeError(tuple_given_message.format(type(col_func).__name__))
