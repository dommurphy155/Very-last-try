...
def preprocess_weights(obj: NDFrame, weights, axis: int) -> np.ndarray:
    ...
    Returns `weights` as an ndarray[np.float64], validated except for
    normalizing
    ...
    if isinstance(weights, str):
        if isinstance(obj, ABCDataFrame):
            ...
            raise ValueError(
                "Strings cannot be passed as weights when sampling from a
    Series."
            )
    ...
    if len(weights) != obj.shape[axis]:
        raise ValueError("Weights and axis to be sampled must be of same
    length")
...
def process_sampling_size(
    ...
    Returns None if `frac` should be used (variable sampling sizes), otherwise
    returns
    ...
    if n is None and frac is None:
        ...
        if frac < 0:
            raise ValueError(
                "A negative number of rows requested. Please provide `frac` >=
    0."
            )
