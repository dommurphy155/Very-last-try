    ...
    if isinstance(key, (ABCSeries, np.ndarray, ABCIndex)) or (
        ...
        if key.dtype == np.object_:
            ...
            if not lib.is_bool_array(key_array):
                na_msg = "Cannot mask with non-boolean array containing NA /
    NaN values"
                if lib.infer_dtype(key_array) == "boolean" and
    isna(key_array).any():
...
def cast_scalar_indexer(val, warn_float: bool = False):
    ...
    if lib.is_float(val) and val.is_integer():
        if warn_float:
            warnings.warn(
                "Indexing with a float is deprecated, and will raise an
    IndexError "
                "in pandas 2.0. You can manually convert to an integer key
    instead.",
                FutureWarning,
...
def asarray_tuplesafe(
    ...
    # ExtensionArray can only be returned when values is an Index, all other
    iterables
    # will return np.ndarray. Unfortunately "all other" cannot be encoded in a
    type
...
@overload
def asarray_tuplesafe(values: Iterable, dtype: NpDtype | None = ...) ->
    ArrayLike:
    ...


def asarray_tuplesafe(values: Iterable, dtype: NpDtype | None = None) ->
    ArrayLike:
    ...
    try:
        ...
        # Using try/except since it's more performant than checking
    is_list_like
        # over each element
        # error: Argument 1 to "construct_1d_object_array_from_listlike"
        # has incompatible type "Iterable[Any]"; expected "Sized"
        return construct_1d_object_array_from_listlike(values)  # type:
    ignore[arg-type]
...
def is_true_slices(line) -> list[bool]:
    """
    Find non-trivial slices in "line": return a list of booleans with same
    length.
    """
...
@overload
def random_state(
    state: int | ArrayLike | np.random.BitGenerator | np.random.RandomState |
    None,
...
def random_state(state: RandomState | None = None):
    ...
    Parameters
    ----------
    state: int, array-like, BitGenerator, Generator, np.random.RandomState,
    None.
        If receives an int, array-like, or BitGenerator, passes to
        np.random.RandomState() as seed.
        If receives an np.random RandomState or Generator, just returns that
    unchanged.
    ...
    Returns
    -------
    np.random.RandomState or np.random.Generator. If state is None, returns
    np.random
    ...
    if (
        ...
        # error: Argument 1 to "RandomState" has incompatible type
    "Optional[Union[int,
        # Union[ExtensionArray, ndarray[Any, Any]], Generator, RandomState]]";
    expected
        # "Union[None, Union[Union[_SupportsArray[dtype[Union[bool_,
    integer[Any]]]],
        # Sequence[_SupportsArray[dtype[Union[bool_, integer[Any]]]]],
        # Sequence[Sequence[_SupportsArray[dtype[Union[bool_,
    integer[Any]]]]]],
        ...
        # Sequence[Sequence[Union[bool, int]]],
    Sequence[Sequence[Sequence[Union[bool,
...
def convert_to_list_like(
    ...
    """
    Convert list-like or scalar input to list-like. List, numpy and pandas
    array-like
    inputs are returned unmodified whereas others are converted to list.
    """
    if isinstance(values, (list, np.ndarray, ABCIndex, ABCSeries,
    ABCExtensionArray)):
        return values
...
def fill_missing_names(names: Sequence[Hashable | None]) -> list[Hashable]:
    ...
    """
    return [f"level_{i}" if name is None else name for i, name in
    enumerate(names)]
...
def resolve_numeric_only(numeric_only: bool | None | lib.NoDefault) -> bool:
    ...
    if numeric_only is lib.no_default:
        # Methods that behave like numeric_only=True and only got the
    numeric_only
        # arg in 1.5.0 default to lib.no_default
        result = True
    elif numeric_only is None:
        # Methods that had the numeric_only arg prior to 1.5.0 and try all
    columns
...
def deprecate_numeric_only_default(
    ...
    deprecate_none: bool, default False
        Whether to also warn about the deprecation of specifying
    ``numeric_only=None``.
    """
    if name in ["all", "any"]:
        arg_name = "bool_only"
    ...
    msg += (
        f"Select only valid columns or specify the value of {arg_name} to
    silence "
        "this warning."
    )
