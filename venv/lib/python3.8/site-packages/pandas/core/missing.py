...
def mask_missing(arr: ArrayLike, values_to_mask) -> npt.NDArray[np.bool_]:
    ...
    values_to_mask = np.array(values_to_mask, dtype=dtype)  # type:
    ignore[arg-type]
...
def clean_fill_method(method: str | None, allow_nearest: bool = False):
    ...
    if method not in valid_methods:
        raise ValueError(f"Invalid fill method. Expecting {expecting}. Got
    {method}")
    return method
    ...
    **kwargs,
    ...
    if limit_area is not None:
        ...
        if limit_area not in valid_limit_areas:
            raise ValueError(
                f"Invalid limit_area: expecting one of {valid_limit_areas},
    got "
                f"{limit_area}."
            )
...
def _interpolate_scipy_wrapper(
    x, y, new_x, method, fill_value=None, bounds_error=False, order=None,
    **kwargs
):
    """
    ...
    if method in interp1d_methods:
        ...
        if isna(order) or (order <= 0):
            raise ValueError(
                f"order needs to be specified and greater than 0; got order:
    {order}"
            )
...
def _interpolate_with_limit_area(
    ...
    """
    Apply interpolation and limit_area logic to values along a to-be-specified
    axis.
...
def interpolate_2d(
    ...
    if values.ndim == 1:
        if axis != 0:  # pragma: no cover
            raise AssertionError("cannot interpolate on a ndim == 1 with axis
    != 0")
...
@_datetimelike_compat
def _pad_2d(values: np.ndarray, limit=None, mask: npt.NDArray[np.bool_] | None
    = None):
    mask = _fillna_prep(values, mask)
...
@_datetimelike_compat
def _backfill_2d(values, limit=None, mask: npt.NDArray[np.bool_] | None =
    None):
    mask = _fillna_prep(values, mask)
...
def _rolling_window(a: npt.NDArray[np.bool_], window: int) ->
    npt.NDArray[np.bool_]:
    """
