...
class BaseWindow(SelectionMixin):
    ...
    enforced
        ...
        if self.on is None:
            ...
            self._on = self.on
        elif isinstance(self.obj, ABCDataFrame) and self.on in
    self.obj.columns:
    ...
    def is_datetimelike(self) -> bool:
        warnings.warn(
            "is_datetimelike is deprecated and will be removed in a future
    version.",
            FutureWarning,
            stacklevel=find_stack_level(),
        )
    ...
    def _validate(self) -> None:
        ...
        if self.min_periods is not None:
            if not is_integer(self.min_periods):
                ...
                raise ValueError(
                    f"min_periods {self.min_periods} must be <= window
    {self.window}"
                )
    ...
    "
        ...
        elif len(start) != (num_vals + (self.step or 1) - 1) // (self.step or
    1):
            raise ValueError(
    ...
    def _slice_axis_for_step(self, index: Index, result: Sized | None = None)
    -> Index:
        """
    ...
    def _create_data(self, obj: NDFrameT, numeric_only: bool = False) ->
    NDFrameT:
        """
        Split data into blocks & return conformed data.
        """
        # filter out the on from the object
        if self.on is not None and not isinstance(self.on, Index) and obj.ndim
    == 2:
    ...
    everything
            # to float to calculate the complete row at once. We exclude all
    non-numeric
            # dtypes.
    ...
    def __repr__(self) -> str:
        ...
        attrs_list = (
            ...
            if getattr(self, attr_name, None) is not None and attr_name[0] !=
    "_"
        )
    ...
    def _get_window_indexer(self) -> BaseIndexer:
        """
        Return an indexer class that will compute the window start and end
    bounds
        """
        if isinstance(self.window, BaseIndexer):
            return self.window
    ...
    def _apply_series(
        self, homogeneous_func: Callable[..., ArrayLike], name: str | None =
    None
    ) -> Series:
        """
    ...
    def _apply_tablewise(
        ...
        """
        if self._selected_obj.ndim == 1:
            raise ValueError("method='table' not applicable for Series
    objects.")
        obj = self._create_data(self._selected_obj, numeric_only)
    ...
    def _apply_pairwise(
        ...
        pairwise: bool | None,
        func: Callable[[DataFrame | Series, DataFrame | Series], DataFrame |
    Series],
        numeric_only: bool,
    ) -> DataFrame | Series:
        """
    ...
    def aggregate(self, func, *args, **kwargs):
        result = ResamplerWindowApply(self, func, args=args,
    kwargs=kwargs).agg()
        if result is None:
...
class BaseWindowGroupby(BaseWindow):
    ...
    numeric_only)
        ...
        if other is not None and not all(
            len(group) == len(other) for group in
    self._grouper.indices.values()
            ...
            gb_pairs = (
                com.maybe_make_list(pair) for pair in
    self._grouper.indices.keys()
            )
            groupby_codes = []
            groupby_levels = []
    ...
    super()._apply_pairwise
        ...
        result_index = MultiIndex(
            result_levels, result_codes, names=result_names,
    verify_integrity=False
        )
        result.index = result_index
        return result
    ...
    NDFrameT:
        ...
        if not obj.empty:
            groupby_order =
    np.concatenate(list(self._grouper.indices.values())).astype(
                np.int64
            )
...
class Window(BaseWindow):
    ...
    other
        than None or 1 will produce a result with a different shape than the
    input.
    ...
    **step**
    ...
    Rolling sum with a window length of 2 observations, minimum of 1
    observation to
    ...
    **win_type**
    ...
    def _validate(self):
        ...
        signal = import_optional_dependency(
            "scipy.signal", extra="Scipy is required to generate window
    weight."
        )
        ...
        if self.method != "single":
            raise NotImplementedError("'single' is the only supported method
    type.")
    ...
    **kwargs)
    ...
    def std(self, ddof: int = 1, numeric_only: bool = False, *args, **kwargs):
        ...
        return zsqrt(
            self.var(ddof=ddof, name="std", numeric_only=numeric_only,
    **kwargs)
        )
...
class RollingAndExpandingMixin(BaseWindow):
    ...
    def apply(
        ...
        if maybe_use_numba(engine):
            if raw is False:
                raise ValueError("raw must be `True` when using the numba
    engine")
            numba_args = args
            if self.method == "single":
    ...
    **kwargs)
    ...
    def std(
        ...
        **kwargs,
        ...
        if maybe_use_numba(engine):
            if self.method == "table":
                raise NotImplementedError("std not supported with
    method='table'")
            else:
                from pandas.core._numba.kernels import sliding_var

                return zsqrt(self._numba_apply(sliding_var, engine_kwargs,
    ddof))
        window_func = window_aggregations.roll_var

        def zsqrt_func(values, begin, end, min_periods):
            return zsqrt(window_func(values, begin, end, min_periods,
    ddof=ddof))
    ...
    def var(
        ...
        **kwargs,
        ...
        if maybe_use_numba(engine):
            if self.method == "table":
                raise NotImplementedError("var not supported with
    method='table'")
            else:
                from pandas.core._numba.kernels import sliding_var
...
class Rolling(RollingAndExpandingMixin):
    ...
    def _validate(self):
        ...
        if (
            self.obj.empty
            or isinstance(self._on, (DatetimeIndex, TimedeltaIndex,
    PeriodIndex))
    ...
    def _validate_datetimelike_monotonic(self):
        ...
        if not (self._on.is_monotonic_increasing or
    ...
    @doc(
        ...
        dedent(
            """
        The below examples will show rolling mean calculations with window
    sizes of
    ...
    @doc(
        ...
        template_see_also,
        create_section_header("Notes"),
        "A minimum of three periods is required for the rolling
    calculation.\n",
        window_method="rolling",
    ...
    @doc(
        ...
        When `other` is not specified, the output will be self correlation
    (e.g.
        all 1's), except for:class:`~pandas.DataFrame` inputs with `pairwise`
        set to `True`.

        Function will return ``NaN`` for correlations of equal valued
    sequences;
        this is the result of a 0/0 division error.

        When `pairwise` is set to `False`, only matching columns between
    `self` and
        `other` will be used.
...
class RollingGroupby(BaseWindowGroupby, Rolling):
    ...
    def _validate_datetimelike_monotonic(self):
        ...
        for group_indices in self._grouper.indices.values():
            ...
            if not (
                group_on.is_monotonic_increasing or
    group_on.is_monotonic_decreasing
            ):
