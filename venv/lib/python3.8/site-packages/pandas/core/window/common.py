...
def flex_binary_moment(arg1, arg2, f, pairwise=False):
    ...
    range(len(arg2.columns))],
                    ...
                    if arg2.columns.nlevels > 1:
                        # mypy needs to know columns is a MultiIndex, Index
    doesn't
                        # have levels attribute
                        arg2.columns = cast(MultiIndex, arg2.columns)
                        # GH 21157: Equivalent to MultiIndex.from_product(
                        #  [result_index], <unique combinations of
    arg2.columns.levels>,
                        # )
                        # A normal MultiIndex.from_product will produce too
    many
                        # combinations.
    ...
    [result_index.name]
                        ...
                        new_order = [num_levels - 1] + list(range(num_levels -
    1))
...
def maybe_warn_args_and_kwargs(cls, kernel: str, args, kwargs) -> None:
    ...
    kernel: str
        Operation name.
    args: tuple or None
        args passed by user. Will be None if and only if kernel does not have
    args.
    kwargs: dict or None
        kwargs passed by user. Will be None if and only if kernel does not
    have kwargs.
    """
