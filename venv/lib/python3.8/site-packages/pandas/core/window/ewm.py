...
def _calculate_deltas(
    ...
    """
    Return the diff of the times divided by the half-life. These values are
    used in
...
class ExponentialMovingWindow(BaseWindow):
    ...
    ----------
    com: float, optional
        Specify decay in terms of center of mass:math:`\alpha = 1 / (1 + com)`, for:math:`com \geq 0`.

    span: float, optional
        Specify decay in terms of span:math:`\alpha = 2 / (span + 1)`, for:math:`span \geq 1`.

    halflife: float, str, timedelta, optional
        Specify decay in terms of half-life:math:`\alpha = 1 - \exp\left(-\ln(2) / halflife\right)`, for:math:`halflife > 0`.
    ...
    alpha: float, optional
        Specify smoothing factor:math:`\alpha` directly:math:`0 < \alpha \leq 1`.
    ...
    weights:math:`w_i = (1 - \alpha)^i`. For example, the EW moving average of
    the series
    ...
    ... + (1 -
            \alpha)^t x_0}{1 + (1 - \alpha) + (1 - \alpha)^2 + ... + (1 -
    \alpha)^t}
    ...
    calculating
          the final weighted average of [:math:`x_0`, None,:math:`x_2`] are:math:`(1-\alpha)^2` and:math:`1` if ``adjust=True``, and:math:`(1-\alpha)^2` and:math:`\alpha` if ``adjust=False``.
    ...
    **times**
    ...
    def __init__(
        ...
        *,
        ...
        if self.times is not None:
            if not self.adjust:
                raise NotImplementedError("times is not supported with
    adjust=False.")
            if isinstance(self.times, str):
                warnings.warn(
                    (
                        "Specifying times as a string column label is
    deprecated "
                        "and will be removed in a future version. Pass the
    column "
                        "into times instead."
                    ),
                    FutureWarning,
            ...
            if len(self.times) != len(obj):
                raise ValueError("times must be the same length as the
    object.")
            if not isinstance(self.halflife, (str, datetime.timedelta,
    np.timedelta64)):
                raise ValueError("halflife must be a timedelta convertible
    object")
    ...
    self.alpha)
            ...
            self._com = get_center_of_mass(
                # error: Argument 3 to "get_center_of_mass" has incompatible
    type
                # "Union[float, Any, None, timedelta64,
    ...
    engine is
              ``{{'nopython': True, 'nogil': False, 'parallel': False}}`` and
    will be
    ...
    def sum(
        ...
        **kwargs,
        ...
        if not self.adjust:
            raise NotImplementedError("sum is not implemented with
    adjust=False")
        if maybe_use_numba(engine):
            if self.method == "single":
                func = generate_numba_ewm_func
    ...
    @doc(
        ...
        template_see_also[:-1],
        window_method="ewm",
        aggregation_description="(exponential weighted moment) standard
    deviation",
        agg_method="std",
    )
    def std(self, bias: bool = False, numeric_only: bool = False, *args,
    **kwargs):
    ...
    def var(self, bias: bool = False, numeric_only: bool = False, *args,
    **kwargs):
    ...
    @doc(
        ...
        template_see_also[:-1],
        window_method="ewm",
        aggregation_description="(exponential weighted moment) sample
    covariance",
        agg_method="cov",
    )
    def cov(
...
class ExponentialMovingWindowGroupby(BaseWindowGroupby,
    ...
    _attributes = ExponentialMovingWindow._attributes +
    BaseWindowGroupby._attributes
    ...
    def __init__(self, obj, *args, _grouper=None, **kwargs) -> None:
        ...
        if not obj.empty and self.times is not None:
            ...
            groupby_order =
    np.concatenate(list(self._grouper.indices.values()))
    ...
    dtype=np.float64
        ...
        if update is not None:
            if self._mean.last_ewm is None:
                raise ValueError(
                    "Must call mean with update=None first before passing
    update"
                )
            result_from = 1
