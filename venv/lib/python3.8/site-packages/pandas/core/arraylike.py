...
def _maybe_fallback(ufunc: np.ufunc, method: str, *inputs: Any, **kwargs: Any):
    ...
    if n_alignable >= 2 and n_frames >= 1:
        # if there are 2 alignable inputs (Series or DataFrame), of which at
    least 1
        # is a DataFrame -> we would have had no alignment before -> warn that
    this
        # will align in the future

        # the first frame is what determines the output index/columns in
    pandas < 1.2
        first_frame = next(x for x in inputs if isinstance(x, DataFrame))

        # check if the objects are aligned or not
        non_aligned = sum(
            not _is_aligned(first_frame, x) for x in inputs if isinstance(x,
    NDFrame)
        )
    ...
    behaviour
        if non_aligned:
            warnings.warn(
                "Calling a ufunc on non-aligned DataFrames (or
    DataFrame/Series "
                "combination). Currently, the indices are ignored and the
    result "
                "takes the index/columns of the first DataFrame. In the future
    , "
                "the DataFrames/Series will be aligned before applying the
    ufunc.\n"
                "Convert one of the arguments to a NumPy array "
                "(eg 'ufunc(df1, np.asarray(df2)') to keep the current
    behaviour, "
                "or align manually (eg 'df1, df2 = df1.align(df2)') before
    passing to "
                "the ufunc to obtain the future behaviour and silence this
    warning.",
                FutureWarning,
...
def array_ufunc(self, ufunc: np.ufunc, method: str, *inputs: Any, **kwargs:
    Any):
    """
    ...
    **kwargs)
        ...
        if result.ndim != self.ndim:
            if method == "outer":
                if self.ndim == 2:
                    ...
                    warnings.warn(
                        msg.format(ufunc), FutureWarning,
    stacklevel=find_stack_level()
                    )
                    return result
                raise NotImplementedError
        ...
        if isinstance(result, BlockManager):
            ...
            result = self._constructor(result, **reconstruct_kwargs,
    copy=False)
        else:
    ...
    if "out" in kwargs:
        ...
        result = dispatch_ufunc_with_out(self, ufunc, method, *inputs,
    **kwargs)
...
def dispatch_ufunc_with_out(self, ufunc: np.ufunc, method: str, *inputs,
    **kwargs):
    """
...
def _assign_where(out, result, where) -> None:
    """
    Set a ufunc result into 'out', masking with a 'where' argument if
    necessary.
    """
    if where is None:
...
def dispatch_reduction_ufunc(self, ufunc: np.ufunc, method: str, *inputs,
    **kwargs):
    """
    ...
    if self.ndim > 1:
        ...
        if "axis" not in kwargs:
            ...
            #  so calls DataFrame.min (without ever getting here) with the
    np.min
            #  default of axis=None, which DataFrame.min catches and changes
    to axis=0.
