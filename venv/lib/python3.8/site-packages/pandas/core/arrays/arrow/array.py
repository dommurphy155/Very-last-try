...
if not pa_version_under1p01:
    ...
    ARROW_LOGICAL_FUNCS = {
        ...
        "rxor": NotImplemented if pa_version_under2p0 else lambda x, y:
    pc.xor(y, x),
    }
...
ArrowExtensionArrayT = TypeVar("ArrowExtensionArrayT",
    bound="ArrowExtensionArray")
...
class ArrowExtensionArray(OpsMixin, ExtensionArray):
    ...
    Some methods may either raise an exception or raise a
    ``PerformanceWarning`` if an
    associated compute function is not available based on the installed
    version of PyArrow.

    Please install the latest version of PyArrow to enable the best
    functionality and avoid
    ...
    def __init__(self, values: pa.Array | pa.ChunkedArray) -> None:
        if pa_version_under1p01:
            msg = "pyarrow>=1.0.0 is required for PyArrow backed
    ArrowExtensionArray."
    ...
    copy=False):
        ...
        if is_cls or isinstance(scalars, (pa.Array, pa.ChunkedArray)):
            ...
            return cls(
                pa.chunked_array(pa.array(scalars, type=pa_dtype,
    from_pandas=True))
            )
    ...
    def _from_sequence_of_strings(
        ...
        if pa_type is None:
            ...
            scalars =
    BooleanArray._from_sequence_of_strings(strings).to_numpy()
        elif (
    ...
    ArrowDtype[string]
                if self._dtype.name == "string" and self._dtype.storage ==
    "pyarrow":
                    pa_dtype = pa.string()
                else:
    ...
    def __invert__(self: ArrowExtensionArrayT) -> ArrowExtensionArrayT:
        if pa_version_under2p0:
            raise NotImplementedError("__invert__ not implement for pyarrow <
    2.0")
    ...
    def argsort(
        ...
        **kwargs,
        ...
        null_placement = {"last": "at_end", "first":
    "at_start"}.get(na_position, None)
    ...
    def _argmin_max(self, skipna: bool, method: str) -> int:
        if self._data.length() in (0, self._data.null_count) or (
            ...
            # For empty or all null, pyarrow returns -1 but pandas expects
    TypeError
            # For skipna=False and data w/ null, pandas expects
    NotImplementedError
    ...
    def isin(self, values) -> npt.NDArray[np.bool_]:
        ...
        # for an empty value_set pyarrow 3.0.0 segfaults and pyarrow 2.0.0
    returns True
        ...
        if pa_version_under3p0:
            # in pyarrow 2.0.0 skip_null is ignored but is a required keyword
    and raises
    ...
    def _values_for_factorize(self) -> tuple[np.ndarray, Any]:
        ...
        Notes
        -----
        The values returned by this method are also used in:func:`pandas.util.hash_pandas_object`.
        """
        if pa_version_under2p0:
    ...
    use_na_sentinel)
        if pa_version_under4p0:
            ...
            null_encoding = "mask" if resolved_na_sentinel is not None else
    "encode"
        ...
        if indices.dtype.kind == "f":
            indices[np.isnan(indices)] = (
                resolved_na_sentinel if resolved_na_sentinel is not None else
    -1
            )
        ...
        if encoded.num_chunks:
            ...
            if resolved_na_sentinel is None and pa_version_under4p0:
                ...
                if na_mask[na_index]:
                    na_code = 0 if na_index == 0 else indices[:na_index].max()
    + 1
    ...
    def _reduce(self, name: str, *, skipna: bool = True, **kwargs):
        ...
        **kwargs
        ...
        if name == "sem":
            ...
            pyarrow_meth = getattr(pc, pyarrow_name, None)  # type:
    ignore[assignment]
    ...
    def __setitem__(self, key: int | slice | np.ndarray, value: Any) -> None:
        ...
        self._data = self._set_via_chunk_iteration(indices=indices,
    value=value)
    ...
    def _quantile(
        self: ArrowExtensionArrayT, qs: npt.NDArray[np.float64],
    interpolation: str
    ) -> ArrowExtensionArrayT:
        """
    ...
    def _mode(self: ArrowExtensionArrayT, dropna: bool = True) ->
    ArrowExtensionArrayT:
        """
        ...
        """
        if pa_version_under6p0:
            raise NotImplementedError("mode only supported for pyarrow version
    >= 6.0")
    ...
    def _maybe_convert_setitem_value(self, value):
        ...
        # TODO: Make more robust like
    ArrowStringArray._maybe_convert_setitem_value
        return value
    ...
    def _set_via_chunk_iteration(
        ...
        for chunk in self._data.iterchunks():
            ...
            if len(indices) == 0 or stop <= indices[0]:
                ...
                new_data.append(self._replace_with_indices(chunk, c_ind,
    c_value))
