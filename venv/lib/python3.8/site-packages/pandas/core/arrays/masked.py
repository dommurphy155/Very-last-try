...
class BaseMaskedArray(OpsMixin, ExtensionArray):
    ...
    @overload
    def __getitem__(self: BaseMaskedArrayT, item: SequenceIndexer) ->
    BaseMaskedArrayT:
        ...
    ...
    def _validate_setitem_value(self, value):
        ...
        if kind == "b":
            ...
            if lib.is_integer(value) or (lib.is_float(value) and
    value.is_integer()):
                return value
    ...
    def to_numpy(
        ...
        By default converts to an object-dtype NumPy array. Specify the
    `dtype` and
        ...
        ...
        ValueError: cannot convert to bool numpy array in presence of missing
    values
    ...
    def _propagate_mask(
        ...
        if mask is None:
            mask = self._mask.copy()  # TODO: need test for BooleanArray
    needing a copy
    ...
    def _arith_method(self, other, op):
        ...
        if isinstance(other, BaseMaskedArray):
            ...
            if other.ndim > 1:
                raise NotImplementedError("can only perform ops with 1-d
    structures")
        ...
        if other is libmissing.NA:
            ...
            if self.dtype.kind in ["i", "u"] and op_name in ["floordiv",
    "mod"]:
    ...
    def take(
        ...
        *,
        ...
        # to avoid upcasting
        data_fill_value = self._internal_fill_value if isna(fill_value) else
    fill_value
        result = take(
            self._data,
            indexer,
    ...
    def isin(self, values) -> BooleanArray:  # type: ignore[override]
        ...
        # algorithms.isin will eventually convert values to an ndarray, so no
    extra
        ...
        if self._hasna:
            ...
            # For now, NA does not propagate so set result according to
    presence of NA,
    ...
    def factorize(
        ...
        resolved_na_sentinel = algos.resolve_na_sentinel(na_sentinel,
    use_na_sentinel)
        arr = self._data
        mask = self._mask

        # Pass non-None na_sentinel; recode and add NA to uniques if necessary
    below
        na_sentinel_arg = -1 if resolved_na_sentinel is None else
    resolved_na_sentinel
        codes, uniques = factorize_array(arr, na_sentinel=na_sentinel_arg,
    mask=mask)
        ...
        if self._hasna:
            ...
            if self.ndim == 2:
                ...
                if is_integer_dtype(self.dtype):
                    # We try to maintain int dtype if possible for not all-na
    case
                    # as well
    ...
    def prod(self, *, skipna=True, min_count=0, axis: int | None = 0,
    **kwargs):
    ...
    def any(self, *, skipna: bool = True, **kwargs):
        ...
        missing values are present, similar:ref:`Kleene logic
    <boolean.kleene>`
        ...
        **kwargs: any, default None
        ...
        np.putmask(values, self._mask, self._falsey_value)  # type:
    ignore[arg-type]
        result = values.any()
        if skipna:
            return result
    ...
    <boolean.kleene>`
        ...
        **kwargs: any, default None
        ...
        # bool, int, float, complex, str, bytes,
        # _NestedSequence[Union[bool, int, float, complex, str, bytes]]]"
        np.putmask(values, self._mask, self._truthy_value)  # type:
    ignore[arg-type]
        result = values.all()
