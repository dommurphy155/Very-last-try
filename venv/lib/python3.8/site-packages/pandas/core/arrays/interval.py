...
-----
See the `user guide
<https://pandas.pydata.org/pandas-docs/stable/user_guide/advanced.html#interval
    index>`__
for more.
...
@Appender(
    ...
    % {
        "klass": "IntervalArray",
        "summary": "Pandas array for interval data that are closed on the same
    side.",
        "versionadded": "0.24.0",
        "name": "",
        "extra_attributes": "",
    ...
    methods::meth:`IntervalArray.from_arrays`,:meth:`IntervalArray.from_breaks`, and:meth:`IntervalArray.from_tuples`.
    """
        ),
    }
)
class IntervalArray(IntervalMixin, ExtensionArray):
    ...
    def _simple_new(
        ...
        if type(left) != type(right):
            ...
            raise ValueError(msg)
        elif isinstance(left, ABCDatetimeIndex) and str(left.tz) !=
    str(right.tz):
            msg = (
        ...
        for d in data:
            if isna(d):
                ...
                try:
                    ...
                    msg = f"{name}.from_tuples requires tuples of length 2,
    got {d}"
    ...
    @overload
    def __getitem__(self: IntervalArrayT, key: SequenceIndexer) ->
    IntervalArrayT:
        ...
    ...
    copy=False).astype(dtype.subtype)
                ...
                msg = (
                    f"Cannot convert {self.dtype} to {dtype}; subtypes are
    incompatible"
                )
                raise TypeError(msg) from err
    ...
    def _validate_listlike(self, value):
        ...
        try:
            ...
            msg = f"'value' should be an interval type, got {type(value)}
    instead."
            raise TypeError(msg) from err
    ...
    IntervalArrayT:
        ...
        return type(self)._simple_new(
            left=self._left, right=self._right, closed=closed,
    verify_integrity=False
        )
    ...
    _interval_shared_docs[
        ...
        Return a boolean whether the %(klass)s is non-overlapping and
    monotonic.
            ...
            if type.equals(interval_type.storage_type):
                ...
                if not type.equals(interval_type):
                    raise TypeError(
                        ...
                        f"different 'subtype' ({self.dtype.subtype} vs
    {type.subtype}) "
                        f"and 'closed' ({self.closed} vs {type.closed})
    attributes"
                    )
            else:
                raise TypeError(
    ...
    @Appender(
        _interval_shared_docs["to_tuples"] % {"return_type": "ndarray",
    "examples": ""}
    )
    ...
    def insert(self: IntervalArrayT, loc: int, item: Interval) ->
    IntervalArrayT:
        """
    ...
    def contains(self, other):
        if isinstance(other, Interval):
            raise NotImplementedError("contains not implemented for two
    intervals")
    ...
    def _from_combined(self, combined: np.ndarray) -> IntervalArray:
        ...
        if needs_i8_conversion(dtype):
            # error: "Type[ndarray[Any, Any]]" has no attribute
    "_from_sequence"
            new_left = type(self._left)._from_sequence(  # type:
    ignore[attr-defined]
                nc[:, 0], dtype=dtype
            )
            # error: "Type[ndarray[Any, Any]]" has no attribute
    "_from_sequence"
            new_right = type(self._right)._from_sequence(  # type:
    ignore[attr-defined]
                nc[:, 1], dtype=dtype
            )
        else:
    ...
    argument
        ...
        nc = unique(
            self._combined.view("complex128")[:, 0]  # type:
...
def _maybe_convert_platform_interval(values) -> ArrayLike:
    ...
    elif not hasattr(values, "dtype") and not isinstance(values, (list, tuple,
    range)):
