...
class PeriodArray(dtl.DatelikeOps, libperiod.PeriodMixin):
    ...
    def _generate_range(cls, start, end, periods, freq, fields):
        ...
        if start is not None or end is not None:
            if field_count > 0:
                raise ValueError(
                    "Can either instantiate from fields or endpoints, but not
    both"
                )
    ...
    def _unbox_scalar(  # type: ignore[override]
        ...
        if value is NaT:
            # error: Item "Period" of "Union[Period, NaTType]" has no
    attribute "value"
            ...
            raise ValueError(f"'value' should be a Period. Got '{value}'
    instead.")
    ...
    def __arrow_array__(self, type=None):
        ...
        if type is not None:
            if pyarrow.types.is_integer(type):
                return pyarrow.array(self._ndarray, mask=self.isna(),
    type=type)
    ...
    different "
        ...
        storage_array = pyarrow.array(self._ndarray, mask=self.isna(),
    type="int64")
    ...
    def to_timestamp(self, freq=None, how: str = "start") -> DatetimeArray:
        ...
        if self.freq.name == "B":
            ...
            #  len(self) is too small for infer_freq to distinguish between
    them
    ...
    def asfreq(self, freq=None, how: str = "E") -> PeriodArray:
        ...
        Equivalent to applying:meth:`pandas.Period.asfreq` with the given
    arguments
        ...
        See Also
        --------
        {other}.asfreq: Convert each Period in a {other_name} to the given
    frequency.
        Period.asfreq: Convert a:class:`~pandas.Period` object to the given
    frequency.
    ...
    def _check_timedeltalike_freq_compat(self, other):
        ...
        try:
            delta = astype_overflowsafe(td, dtype=dtype, copy=False,
    round_ok=False)
        except ValueError as err:
...
def period_array(
    ...
    if is_float_dtype(arrdata) and len(arrdata) > 0:
        raise TypeError("PeriodIndex does not allow floating point in
    construction")

    if is_integer_dtype(arrdata.dtype):
        arr = arrdata.astype(np.int64, copy=False)
        # error: Argument 2 to "from_ordinals" has incompatible type
    "Union[str,
...
def validate_dtype_freq(
    ...
    if dtype is not None:
        ...
        if freq is None:
            ...
            raise IncompatibleFrequency("specified freq and dtype are
    different")
    # error: Incompatible return value type (got "Union[BaseOffset, Any,
    None]",
    # expected "BaseOffset")
...
def _get_ordinal_range(start, end, periods, freq, mult=1):
    ...
    if periods is not None:
        ...
        if start is None:
            data = np.arange(
                end.ordinal - periods + mult, end.ordinal + 1, mult,
    dtype=np.int64
            )
        else:
            data = np.arange(
...
def _range_from_fields(
    ...
    if quarter is not None:
        ...
        for y, mth, d, h, mn, s in zip(*arrays):
            ordinals.append(libperiod.period_ordinal(y, mth, d, h, mn, s, 0,
    0, base))
...
def _make_field_arrays(*fields) -> list[np.ndarray]:
    ...
    expected
    # "Union[Union[int, integer[Any]], Union[bool, bool_], ndarray,
    Sequence[Union[int,
