...
class SparseDtype(ExtensionDtype):
    ...
    def update_dtype(self, dtype) -> SparseDtype:
        ...
        if not isinstance(dtype, cls):
            if not isinstance(dtype, np.dtype):
                raise TypeError("sparse arrays of extension dtypes not
    supported")
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        fill_values = [x.fill_value for x in dtypes if isinstance(x,
    SparseDtype)]
        ...
        np_dtypes = [x.subtype if isinstance(x, SparseDtype) else x for x in
    dtypes]
        return SparseDtype(np.find_common_type(np_dtypes, []),
