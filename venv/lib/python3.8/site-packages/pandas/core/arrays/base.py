...
class ExtensionArray:
    ...
    * _from_sequence_of_strings
    ...
    @overload
    def __getitem__(self: ExtensionArrayT, item: SequenceIndexer) ->
    ExtensionArrayT:
        ...
    ...
    def __contains__(self, item: object) -> bool | np.bool_:
        ...
        """
        # GH37867
        # comparisons of any item to pd.NA always return pd.NA, so e.g. "a" in
    [pd.NA]
        # would raise a TypeError. The implementation below works around that.
        if is_scalar(item) and isna(item):
            if not self._can_hold_na:
                return False
            elif item is self.dtype.na_value or isinstance(item,
    self.dtype.type):
                return self._hasna
            else:
                return False
    ...
    def __eq__(self, other: Any) -> ArrayLike:  # type: ignore[override]
        ...
        # When `other` is one of Series, Index, or DataFrame, this method
    should
        # return NotImplemented (to ensure that those objects are responsible
    for
    ...
    def __init_subclass__(cls, **kwargs) -> None:
        ...
        if (
            ...
            # TimelikeOps uses old factorize args to ensure we don't break
    things
            and cls.__name__ not in ("TimelikeOps", "DatetimeArray",
    "TimedeltaArray")
        ):
            # See GH#46910 for details on the deprecation
            name = cls.__name__
            warnings.warn(
                f"The `na_sentinel` argument of `{name}.factorize` is
    deprecated. "
                f"In the future, pandas will use the `use_na_sentinel`
    argument "
                f"instead.  Add this argument to `{name}.factorize` to be
    compatible "
    ...
    `self.isna()`).
        This means that the corresponding entries in the returned array don't
    need to
    ...
    use_na_sentinel
        ...
        use_na_sentinel: bool, default True
            ...
            NaN values will be encoded as non-negative integers and will not
    drop the
    ...
    def repeat(
        self: ExtensionArrayT, repeats: int | Sequence[int], axis: int | None
    = None
    ) -> ExtensionArrayT:
    ...
    def take(
        ...
        *,
        ...
        extension array to object dtype. This uses the helper method:func:`pandas.api.extensions.take`.
    ...
    def view(self, dtype: Dtype | None = None) -> ArrayLike:
        ...
        """
        # NB:
        # - This must return a *new* object referencing the same data, not
    self.
    ...
    def ravel(self, order: Literal["C", "F", "A", "K"] | None = "C") ->
    ExtensionArray:
        """
    ...
    def _putmask(self, mask: npt.NDArray[np.bool_], value) -> None:
        ...
        Notes
        -----
        Unlike np.putmask, we do not repeat listlike values with mismatched
    length.
    ...
    def _empty(cls, shape: Shape, dtype: ExtensionDtype):
        ...
        if not isinstance(result, cls) or dtype != result.dtype:
            raise NotImplementedError(
                f"Default 'empty' implementation is invalid for
    dtype='{dtype}'"
            )
        return result
    ...
    def __array_ufunc__(self, ufunc: np.ufunc, method: str, *inputs, **kwargs):
        if any(
            isinstance(other, (ABCSeries, ABCIndex, ABCDataFrame)) for other
    in inputs
        ):
