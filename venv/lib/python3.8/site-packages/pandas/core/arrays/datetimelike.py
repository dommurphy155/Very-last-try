...
DatetimeLikeArrayT = TypeVar("DatetimeLikeArrayT",
...
class DatetimeLikeArrayMixin(OpsMixin, NDArrayBackedExtensionArray):
    ...
    def __init__(self, data, dtype: Dtype | None = None, freq=None,
    copy=False) -> None:
    ...
    def _check_compatible_with(
        ...
        * Timedelta has no verification
        ...
        other
        setitem: bool, default False
            For __setitem__ we may have stricter compatibility restrictions
    than
            for comparisons.
    ...
    lookup.
        ...
        if is_period:
            ...
            key = check_array_indexer(self, key)  # maybe ndarray[bool] ->
    slice
            freq = None
        ...
        if is_object_dtype(dtype):
            ...
            if is_unsigned_integer_dtype(dtype):
                ...
                if dtype != np.uint64:
                    # GH#45034
                    warnings.warn(
                        f"The behavior of .astype from {self.dtype} to {dtype}
    is "
                        "deprecated. In a future version, this astype will
    return "
                        "exactly the specified dtype instead of uint64, and
    will "
    ...
    def _concat_same_type(
        ...
        if is_period_dtype(dtype):
            ...
            if obj.freq is not None and all(x.freq == obj.freq for x in
    to_concat):
    ...
    def _validation_error_message(self, value, allow_listlike: bool = False)
    -> str:
        """
    ...
    def _unbox(
        ...
        """
        Unbox either a scalar with _unbox_scalar or an instance of our own
    type.
        """
    ...
    def inferred_freq(self) -> str | None:
        """
        Tries to return a string representing a frequency generated by
    infer_freq.
    ...
    def _validate_frequency(cls, index, freq, **kwargs):
        ...
        try:
            on_freq = cls._generate_range(
                start=index[0], end=None, periods=len(index), freq=freq,
    **kwargs
            )
    ...
    @classmethod
    def _generate_range(
        cls: type[DatetimeLikeArrayT], start, end, periods, freq, *args,
    **kwargs
    ) -> DatetimeLikeArrayT:
    ...
    def _cmp_method(self, other, op):
        ...
        if not is_period_dtype(self.dtype):
            ...
            if self._reso != other._reso:
                ...
                return compare_mismatched_resolutions(self._ndarray,
    other_arr, op)
    ...
    NaT.to_datetime64().astype(f"M8[{self._unit}]")
            # Preserve our resolution
            return DatetimeArray._simple_new(result, dtype=result.dtype)

        if self._reso != other._reso:
            raise NotImplementedError(
                "Addition between TimedeltaArray and Timestamp with
    mis-matched "
    ...
    def _sub_datetimelike_scalar(self, other: datetime | np.datetime64):
        if self.dtype.kind != "M":
            raise TypeError(f"cannot subtract a datelike from a
    {type(self).__name__}")
    ...
    def _sub_period(self, other: Period) -> npt.NDArray[np.object_]:
        if not is_period_dtype(self.dtype):
            raise TypeError(f"cannot subtract Period from a
    {type(self).__name__}")
    ...
    def _add_timedeltalike_scalar(self, other):
        ...
        if isna(other):
            ...
            new_values = np.empty(self.shape,
    dtype="i8").view(self._ndarray.dtype)
            new_values.fill(iNaT)
            return type(self)._simple_new(new_values, dtype=self.dtype)

        # PeriodArray overrides, so we only get here with DTA/TDA
        # error: "DatetimeLikeArrayMixin" has no attribute "_reso"
        inc = delta_to_nanoseconds(other, reso=self._reso)  # type:
    ignore[attr-defined]
    ...
    def _sub_period_array(self, other: PeriodArray) -> npt.NDArray[np.object_]:
        if not is_period_dtype(self.dtype):
            raise TypeError(
                f"cannot subtract {other.dtype}-dtype from
    {type(self).__name__}"
            )
    ...
    def __add__(self, other):
        ...
        elif is_datetime64_dtype(other_dtype) or
    is_datetime64tz_dtype(other_dtype):
    ...
    def __rsub__(self, other):
        ...
        if is_datetime64_any_dtype(other_dtype) and
    ...
    {type(other).__name__}"
            ...
            raise TypeError(f"cannot subtract {type(self).__name__} from
    {other.dtype}")
    ...
    def mean(self, *, skipna: bool = True, axis: int | None = 0):
        ...
        See Also
        --------
        numpy.ndarray.mean: Returns the average of array elements along a
    given axis.
    ...
    def median(self, *, axis: int | None = None, skipna: bool = True,
    **kwargs):
...
class DatelikeOps(DatetimeLikeArrayMixin):
    ...
    format
        doc (such as `"%%R"`, `"%%r"`) are not officially supported and should
    be
        preferably replaced with their supported equivalents (such as
    `"%%H:%%M"`,
        `"%%I:%%M:%%S %%p"`).
...
_round_doc = """
    ...
    freq: str or Offset
        ...
        frequency like 'S' (second) not 'ME' (month end). See:ref:`frequency aliases <timeseries.offset_aliases>` for
...
class TimelikeOps(DatetimeLikeArrayMixin):
    ...
    def __init__(self, values, dtype=None, freq=lib.no_default, copy: bool =
    False):
    ...
    (2022-07-01)
                    raise TypeError(
                        f"dtype={dtype} does not match data dtype
    {values.dtype}"
                    )
    ...
    # GH#46910 - Keep old signature to test we don't break things for EA
    library authors
...
def ensure_arraylike_for_datetimelike(data, copy: bool, cls_name: str):
    ...
    if isinstance(data, IntegerArray):
        ...
        # TODO: cases where we need to do another pass through
    maybe_convert_dtype,
