...
class DatetimeArray(dtl.TimelikeOps, dtl.DatelikeOps):
            ...
            if tz is not None and endpoint_tz is None:
                ...
                if not timezones.is_utc(tz):
                    ...
                    i8values = tzconversion.tz_localize_to_utc(
                        i8values, tz, ambiguous=ambiguous,
    ...
    def _check_compatible_with(self, other, setitem: bool = False):
        ...
        if setitem:
            ...
            if self.tz is not None and not timezones.tz_compare(self.tz,
    other.tz):
                # TODO(2.0): remove this check. GH#37605
                warnings.warn(
                    "Setitem-like behavior with mismatched timezones is
    deprecated "
    ...
    "
                ...
                raise ValueError(f"Timezones don't match. '{self.tz}' !=
    '{other.tz}'")
    ...
    copy=copy)
            ...
            warnings.warn(
                ...
                "and will raise in a future version. Pass 'datetime64[ns]'
    instead",
                FutureWarning,
                stacklevel=find_stack_level(),
            )
    ...
    def _assert_tzawareness_compat(self, other) -> None:
        ...
        if other is NaT:
            ...
            if other_tz is not None:
                raise TypeError(
                    "Cannot compare tz-naive and tz-aware datetime-like
    objects."
                )
    ...
    DatetimeArray:
        """
        Localize tz-naive Datetime Array/Index to tz-aware Datetime
    Array/Index.
...
default 'raise'
    ...
    def to_perioddelta(self, freq) -> TimedeltaArray:
        """
        Calculate deltas between self values and self converted to Periods at
    a freq.
    ...
    DatetimeIndex
        ...
        if self._ndarray.dtype != "M8[ns]":
            raise NotImplementedError("Only supported for nanosecond
    resolution.")
    ...
    "timedelta64")
        ...
        tda = TimedeltaArray._simple_new(self._ndarray.view(dtype),
    dtype=dtype)
...
def sequence_to_datetimes(data, require_iso8601: bool = False) ->
    DatetimeArray:
    """
    Parse/convert the passed data to either DatetimeArray or
    np.ndarray[object].
    """
...
def _sequence_to_dt64ns(
    ...
    *,
    ...
    allow_mixed: bool, default False
        Interpret integers as timestamps when datetime objects are also
    present.
...
def objects_to_datetime64ns(
    ...
    try:
        ...
        # Exception is raised when a part of date is greater than 32 bit
    signed int
        raise OutOfBoundsDatetime("Out of bounds nanosecond timestamp") from
    err
...
def maybe_convert_dtype(data, copy: bool, tz: tzinfo | None = None):
    ...
    if is_float_dtype(data.dtype):
        ...
        if (
            ...
            # GH#23675, GH#45573 deprecate to treat symmetrically with integer
    dtypes
            warnings.warn(
                "The behavior of DatetimeArray._from_sequence with a
    timezone-aware "
                "dtype and floating-dtype data is deprecated. In a future
    version, "
                "this data will be interpreted as nanosecond UTC timestamps "
                "instead of wall-times, matching the behavior with integer
    dtypes. "
                "To retain the old behavior, explicitly cast to
    'datetime64[ns]' "
                "before passing the data to pandas. To get the future
    behavior, "
                "first cast to 'int64'.",
                FutureWarning,
                stacklevel=find_stack_level(),
        ...
        raise TypeError(f"dtype {data.dtype} cannot be converted to
    datetime64[ns]")
    ...
    elif is_extension_array_dtype(data.dtype) and not
...
def _maybe_infer_tz(tz: tzinfo | None, inferred_tz: tzinfo | None) -> tzinfo |
    None:
    """
...
def _validate_dt64_dtype(dtype):
    ...
    allowed. "
        ...
        if (isinstance(dtype, np.dtype) and dtype != DT64NS_DTYPE) or not
    isinstance(
    ...
    if dtype is not None:
        ...
        if dtz is not None:
            if tz is not None and not timezones.tz_compare(tz, dtz):
                raise ValueError("cannot supply both a tz and a dtype with a
    tz")
            tz = dtz
...
def _infer_tz_from_endpoints(
    ...
    if tz is not None and inferred_tz is not None:
        if not timezones.tz_compare(inferred_tz, tz):
            raise AssertionError("Inferred time zone not equal to passed time
    zone")
