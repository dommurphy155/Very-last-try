...
class StringDtype(StorageExtensionDtype):
    ...
    def __init__(self, storage=None) -> None:
        ...
        if storage not in {"python", "pyarrow"}:
            raise ValueError(
                f"Storage must be 'python' or 'pyarrow'. Got {storage}
    instead."
            )
    ...
    ==============================================
            ...
            ==========================
    ==============================================
            ``'string'``               pd.options.mode.string_storage, default
    python
...
class StringArray(BaseStringArray, PandasArray):
    ...
    values: array-like
        ...
        .. warning::
           ...
           This may change without warning in the future. Use:meth:`pandas.array` with ``dtype="string"`` for a stable way of
    ...
    However, instantiating StringArrays directly with non-strings will raise
    an error.
    ...
    skipna=True):
            raise ValueError("StringArray requires a sequence of strings or
    pandas.NA")
    ...
    StringDtype().na_value
            result = lib.ensure_string_array(scalars, na_value=libmissing.NA,
    copy=copy)

        # Manually creating new array avoids the validation step in the
    __init__, so is
    ...
    def __setitem__(self, key, value):
        ...
        if scalar_value:
            if isna(value):
                ...
                raise ValueError(
                    f"Cannot set non-string value '{value}' into a
    StringArray."
                )
        else:
    ...
    def _cmp_method(self, other, op):
        ...
        if not lib.is_scalar(other):
            if len(other) != len(self):
                ...
                raise ValueError(
                    f"Lengths of operands do not match: {len(self)} !=
    {len(other)}"
                )
