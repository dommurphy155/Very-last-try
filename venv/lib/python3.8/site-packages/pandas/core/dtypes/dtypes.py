...
class CategoricalDtype(PandasExtensionDtype, ExtensionDtype):
    ...
    ordered: bool or None, default False
        ...
        None can be used to maintain the ordered value of existing
    categoricals when
        used in operations that combine categoricals, e.g. astype, and will
    resolve to
    ...
    def _from_values_or_dtype(
        ...
        if dtype is not None:
            ...
            if isinstance(dtype, str):
                ...
                raise ValueError(
                    "Cannot specify `categories` or `ordered` together with
    `dtype`."
                )
            elif not isinstance(dtype, CategoricalDtype):
                raise ValueError(f"Cannot not construct CategoricalDtype from
    {dtype}")
    ...
    def construct_from_string(cls, string: str_type) -> CategoricalDtype:
        ...
        ----------
        string: str
            Must be the string "category" in order to be successfully
    constructed.
        ...
        if string != cls.name:
            raise TypeError(f"Cannot construct a 'CategoricalDtype' from
    '{string}'")

        # need ordered=None to ensure that operations specifying
    dtype="category" don't
        # override the ordered value for existing categoricals
        return cls(ordered=None)

    def _finalize(self, categories, ordered: Ordered, fastpath: bool = False)
    -> None:
    ...
    def _hash_categories(self) -> int:
        ...
        if len(categories) and isinstance(categories[0], tuple):
            ...
            if categories.dtype == "O" and len({type(x) for x in categories})
    != 1:
        ...
        combined_hashed = combine_hash_arrays(iter(cat_array),
    ...
    def validate_categories(categories, fastpath: bool = False) -> Index:
        ...
        if not fastpath and not is_list_like(categories):
            raise TypeError(
                f"Parameter 'categories' must be list-like, was
    {repr(categories)}"
            )
    ...
    def update_dtype(self, dtype: str_type | CategoricalDtype) ->
    CategoricalDtype:
        """
    ...
    None
        new_categories = (
            dtype.categories if dtype.categories is not None else
    self.categories
        )
        new_ordered = dtype.ordered if dtype.ordered is not None else
    self.ordered
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        non_init_cats = [
            isinstance(x, CategoricalDtype) and x.categories is None for x in
    dtypes
        ]
        ...
        dtypes = [x.subtype if isinstance(x, SparseDtype) else x for x in
    dtypes]
        # extract the categories' dtype
        non_cat_dtypes = [
            x.categories.dtype if isinstance(x, CategoricalDtype) else x for x
    in dtypes
        ]
    ...
    None:
        ...
        if unit != "ns":
            ...
            if unit not in ["s", "ms", "us", "ns"]:
                raise ValueError("DatetimeTZDtype only supports s, ms, us, ns
    units")
...
class PeriodDtype(dtypes.PeriodDtypeBase, PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    "PeriodArray";
        ...
        if not results:
            return PeriodArray(np.array([], dtype="int64"), freq=self.freq,
    copy=False)
...
class IntervalDtype(PandasExtensionDtype):
    ...
    **This is not an actual numpy dtype**, but a duck type.
    ...
    def __new__(cls, subtype=None, closed: str_type | None = None):
        ...
        if closed is not None and closed not in {"right", "left", "both",
    "neither"}:
            raise ValueError("closed must be one of 'right', 'left', 'both',
    'neither'")
    ...
    def _can_hold_na(self) -> bool:
        ...
        if subtype is None:
            ...
            raise NotImplementedError(
                "_can_hold_na is not defined for partially-initialized
    IntervalDtype"
            )
    ...
    def construct_from_string(cls, string: str_type) -> IntervalDtype:
        ...
        if string.lower() == "interval" or cls._match.search(string) is not
    None:
            return cls(string)
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        common = find_common_type([cast("IntervalDtype", x).subtype for x in
    dtypes])
        if common == object:
            return np.dtype(object)
...
class PandasDtype(ExtensionDtype):
    ...
    def kind(self) -> str:
        """
        A character code (one of 'biufcmMOSUV') identifying the general kind
    of data.
        """
...
class BaseMaskedDtype(ExtensionDtype):
    ...
    def _get_common_dtype(self, dtypes: list[DtypeObj]) -> DtypeObj | None:
        ...
        new_dtype = find_common_type(
            [
                dtype.numpy_dtype if isinstance(dtype, BaseMaskedDtype) else
    dtype
                for dtype in dtypes
            ]
        )
