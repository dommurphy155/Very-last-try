...
def maybe_box_datetimelike(value: Scalar, dtype: Dtype | None = None) ->
    Scalar:
    """
...
def maybe_box_native(value: Scalar) -> Scalar:
    ...
    if is_float(value):
        ...
        # "Union[Union[str, int, float, bool], Union[Any, Timestamp,
    Timedelta, Any]]";
...
@overload
def maybe_downcast_to_dtype(result: np.ndarray, dtype: str | np.dtype) ->
    np.ndarray:
    ...


@overload
def maybe_downcast_to_dtype(result: ExtensionArray, dtype: str | np.dtype) ->
    ArrayLike:
    ...


def maybe_downcast_to_dtype(result: ArrayLike, dtype: str | np.dtype) ->
    ArrayLike:
    """
...
def maybe_downcast_numeric(
    ...
    """
    if not isinstance(dtype, np.dtype) or not isinstance(result.dtype,
    np.dtype):
    ...
    if is_bool_dtype(dtype) or is_integer_dtype(dtype):
        ...
        if np.allclose(new_result, result, equal_nan=True, rtol=0.0,
    atol=atol):
            return new_result
...
def ensure_dtype_can_hold_na(dtype: DtypeObj) -> DtypeObj:
    """
    If we have a dtype that cannot hold NA values, find the best match that
    can.
    """
    if isinstance(dtype, ExtensionDtype):
...
def maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    # for performance, we are using a cached version of the actual
    implementation
    # of the function in _maybe_promote. However, this doesn't always work (in
    case
    # of non-hashable arguments), so we fallback to the actual implementation
    if needed
    try:
        # error: Argument 3 to "__call__" of "_lru_cache_wrapper" has
    incompatible type
...
def _maybe_promote(dtype: np.dtype, fill_value=np.nan):
    ...
    if issubclass(dtype.type, np.datetime64):
        ...
        if isinstance(fill_value, date) and not isinstance(fill_value,
    datetime):
            ...
            try:
                ...
                warnings.warn(
                    "Using a `date` object for fill_value with
    `datetime64[ns]` "
                    "dtype is deprecated. In a future version, this will be
    cast "
                    "to object dtype. Pass `fill_value=Timestamp(date_obj)`
    instead.",
                    FutureWarning,
...
def maybe_upcast(
    ...
    # error: Incompatible return value type (got "Tuple[ndarray[Any,
    dtype[Any]],
    # Union[Union[str, int, float, bool] Union[Period, Timestamp, Timedelta,
    Any]]]",
    # expected "Tuple[NumpyArrayT, Union[Union[str, int, float, bool],
    Union[Period,
...
def invalidate_string_dtypes(dtype_set: set[DtypeObj]) -> None:
    ...
    """
    # error: Argument 1 to <set> has incompatible type "Type[generic]";
    expected
    # "Union[dtype[Any], ExtensionDtype, None]"
    # error: Argument 2 to <set> has incompatible type "Type[generic]";
    expected
    # "Union[dtype[Any], ExtensionDtype, None]"
...
def soft_convert_objects(
    ...
    if numeric and is_object_dtype(values.dtype):
        converted, _ = lib.maybe_convert_numeric(values, set(),
    coerce_numeric=True)
...
def convert_dtypes(
    ...
    if (
        convert_string or convert_integer or convert_boolean or
    convert_floating
        ...
        if convert_boolean:
            ...
            elif isinstance(inferred_dtype, str) and inferred_dtype ==
    "boolean":
                inferred_dtype = pandas_dtype("boolean")
    ...
    "ExtensionArray",
        ...
        if isna(v).all():
            ...
            if lib.infer_dtype(value, skipna=False) in ["mixed"]:
                ...
                # error: Incompatible types in assignment (expression has type
                # "ExtensionArray", variable has type "Union[ndarray,
    List[Any]]")
                value = try_datetime(v)  # type: ignore[assignment]
...
def maybe_cast_to_datetime(
    ...
    deprecated
                            if getattr(vdtype, "kind", None) == "M":
                                # GH#24559, GH#33401 deprecate behavior
    inconsistent
    ...
    dtype=dtype)",
            ...
            # we are already datetimelike and want to coerce to
    non-datetimelike;
            #  astype_nansafe will raise for anything other than object, then
    upcast.
...
def sanitize_to_nanoseconds(values: np.ndarray, copy: bool = False) ->
    np.ndarray:
    """
    Safely convert non-nanosecond datetime64 or timedelta64 values to
    nanosecond.
    """
    dtype = values.dtype
...
def find_result_type(left: ArrayLike, right: Any) -> DtypeObj:
    ...
    if (
        ...
        if lib.is_float(right) and right.is_integer() and left.dtype.kind !=
    "f":
            right = int(right)
...
def construct_2d_arraylike_from_scalar(
    ...
    try:
        ...
        raise TypeError(
            f"DataFrame constructor called with incompatible data and dtype:
    {err}"
        ) from err
...
def np_can_hold_element(dtype: np.dtype, element: Any) -> Any:
    """
    Raise if we cannot losslessly set this element into an ndarray with this
    dtype.
    ...
    if dtype.kind in ["i", "u"]:
        ...
        elif is_integer(element) or (is_float(element) and
    element.is_integer()):
        ...
        if tipo is not None:
            if tipo.kind not in ["i", "u"]:
                if isinstance(element, np.ndarray) and element.dtype.kind ==
    "f":
                    # If all can be losslessly cast to integers, then we can
    hold them
                    with np.errstate(invalid="ignore"):
                        # We check afterwards if cast was losslessly, so no
    need to show
                        # the warning
        ...
        if tipo is not None:
            ...
            if tipo.kind not in ["f", "i", "u"]:
                ...
                if isinstance(element, np.ndarray):
                    ...
                    casted = element.astype(dtype)
                    # TODO(np>=1.20): we can just use np.array_equal with
    equal_nan
        ...
        if lib.is_integer(element) or lib.is_complex(element) or
    lib.is_float(element):
            if np.isnan(element):
...
def _dtype_can_hold_range(rng: range, dtype: np.dtype) -> bool:
    """
    _maybe_infer_dtype_type infers to int64 (and float64 for very large
    endpoints),
...
class LossySetitemError(Exception):
    """
    Raised when trying to do a __setitem__ on an np.ndarray that is not
    lossless.
    """
