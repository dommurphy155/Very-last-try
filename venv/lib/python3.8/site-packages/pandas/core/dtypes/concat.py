...
def cast_to_common_type(arr: ArrayLike, dtype: DtypeObj) -> ArrayLike:
    ...
    _DTypeDict,
        ...
        return arr.to_dense().astype(dtype, copy=False)  # type:
    ignore[arg-type]
...
def concat_compat(to_concat, axis: int = 0, ea_compat_axis: bool = False):
    ...
    contains_datetime = any(
        isinstance(dtype, (np.dtype, DatetimeTZDtype)) and dtype.kind in ["m",
    "M"]
        for dtype in dtypes
    ) or any(isinstance(obj, ABCExtensionArray) and obj.ndim > 1 for obj in
    to_concat)
    ...
    if any_ea:
        ...
        if not single_dtype:
            ...
            target_dtype = common_dtype_categorical_compat(to_concat,
    target_dtype)
            to_concat = [cast_to_common_type(arr, target_dtype) for arr in
    to_concat]
        ...
        if len(kinds) != 1:
            ...
            if not len(kinds - {"i", "u", "f"}) or not len(kinds - {"b", "i",
    "u"}):
                # let numpy coerce
                pass
            else:
    ...
    if "b" in kinds and result.dtype.kind in ["i", "u", "f"]:
        # GH#39817
        warnings.warn(
            "Behavior when concatenating bool-dtype and numeric-dtype arrays
    is "
...
def union_categoricals(
    ...
    ordered = False
    if all(first._categories_match_up_to_permutation(other) for other in
    to_union[1:]):
        # identical categories - fastpath
        categories = first.categories
        ordered = first.ordered

        all_codes = [first._encode_with_my_categories(x)._codes for x in
    to_union]
        new_codes = np.concatenate(all_codes)

        if sort_categories and not ignore_order and ordered:
            raise TypeError("Cannot use sort_categories=True with ordered
    Categoricals")
        ...
        new_codes = [
            recode_for_categories(c.codes, c.categories, categories) for c in
    to_union
        ]
        ...
        if all(c.ordered for c in to_union):
            msg = "to union ordered Categoricals, all categories must be the
    same"
            raise TypeError(msg)
        else:
...
def _concat_datetime(to_concat, axis=0):
    ...
    if not single_dtype:
        ...
        return _concatenate_2d([x.astype(object) for x in to_concat],
    axis=axis)
