...
def astype_nansafe(
    ...
    if issubclass(dtype.type, str):
        ...
        raise TypeError(f"cannot astype a datetimelike from [{arr.dtype}] to
    [{dtype}]")
        ...
        raise TypeError(f"cannot astype a timedelta from [{arr.dtype}] to
    [{dtype}]")
        ...
        if is_datetime64_dtype(dtype):
            ...
            #  `np.asarray(to_timedelta(arr))`, but using a lower-level API
    that
...
def _astype_float_to_int_nansafe(
    ...
    """
    astype with a check preventing converting NaN to an meaningless integer
    value.
    """
    ...
    if dtype.kind == "u":
        # GH#45151
        if not (values >= 0).all():
            raise ValueError(f"Cannot losslessly cast from {values.dtype} to
    {dtype}")
...
def astype_array(values: ArrayLike, dtype: DtypeObj, copy: bool = False) ->
    ...
    if (
        ...
        msg = rf"cannot astype a datetimelike from [{values.dtype}] to
    [{dtype}]"
        raise TypeError(msg)
...
def astype_dt64_to_dt64tz(
    ...
    if via_utc:
        ...
        raise NotImplementedError("dtype_equal case should be handled
    elsewhere")
