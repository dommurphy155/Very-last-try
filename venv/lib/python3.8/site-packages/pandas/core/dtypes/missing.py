...
# handle unions
@overload
def isna(obj: NDFrameT | ArrayLike | Index | list) -> NDFrameT |
    npt.NDArray[np.bool_]:
    ...
...
def _isna_array(values: ArrayLike, inf_as_na: bool = False):
    ...
    if not isinstance(values, np.ndarray):
        ...
        if inf_as_na and is_categorical_dtype(dtype):
            ...
            # "Union[ndarray[Any, Any], ExtensionArraySupportsAnyAll]",
    variable has
...
def _isna_string_dtype(values: np.ndarray, inf_as_na: bool) ->
    npt.NDArray[np.bool_]:
...
# handle unions
@overload
def notna(obj: NDFrameT | ArrayLike | Index | list) -> NDFrameT |
    npt.NDArray[np.bool_]:
    ...
...
def _array_equivalent_object(left: np.ndarray, right: np.ndarray, strict_nan:
    bool):
    if not strict_nan:
...
def is_valid_na_for_dtype(obj, dtype: DtypeObj) -> bool:
    ...
    if not lib.is_scalar(obj) or not isna(obj):
        ...
        # Numeric
        return obj is not NaT and not isinstance(obj, (np.datetime64,
    np.timedelta64))
    elif dtype.kind == "b":
...
def isna_all(arr: ArrayLike) -> bool:
    ...
    if dtype.kind == "f" and isinstance(dtype, np.dtype):
        ...
        checker = lambda x: np.asarray(x.view("i8")) == iNaT  # type:
    ignore[assignment]
    ...
    return all(
        checker(arr[i: i + chunk_len]).all() for i in range(0, total_len,
    chunk_len)
    )
