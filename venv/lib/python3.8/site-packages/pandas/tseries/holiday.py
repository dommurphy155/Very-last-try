...
class Holiday:
    ...
    def __init__(
        ...
        ...     "Memorial Day", month=5, day=31,
    offset=pd.DateOffset(weekday=MO(-1))
        ... )
        >>> USMemorialDay
        Holiday: Memorial Day (month=5, day=31, offset=<DateOffset:
    weekday=MO(-1)>)

        >>> USLaborDay = pd.tseries.holiday.Holiday(
        ...     "Labor Day", month=9, day=1,
    offset=pd.DateOffset(weekday=MO(1))
        ... )
        >>> USLaborDay
        Holiday: Labor Day (month=9, day=1, offset=<DateOffset:
    weekday=MO(+1)>)
        ...
        Holiday: New Years Day (
            month=1, day=1, observance=<function nearest_workday at
    0x66545e9bc440>
        )
        ...
        self.end_date = Timestamp(end_date) if end_date is not None else
    end_date
    ...
    filter_start_date
        ...
        holiday_dates = holiday_dates[
            (holiday_dates >= filter_start_date) & (holiday_dates <=
    filter_end_date)
        ]
        if return_name:
...
class AbstractHolidayCalendar(metaclass=HolidayCalendarMetaClass):
    ...
    def holidays(self, start=None, end=None, return_name=False):
        ...
        if self.rules is None:
            raise Exception(
                f"Holiday Calendar {self.name} does not have any rules
    specified"
            )
        ...
        # get them again
        if self._cache is None or start < self._cache[0] or end >
    self._cache[1]:
            pre_holidays = [
...
EasterMonday = Holiday("Easter Monday", month=1, day=1, offset=[Easter(),
    Day(1)])
