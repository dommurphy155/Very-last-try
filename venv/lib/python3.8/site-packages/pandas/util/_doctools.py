...
class TablePlotter:
    ...
    def _get_cells(self, left, right, vertical) -> tuple[int, int]:
        ...
        if vertical:
            ...
            vcells = max(sum(self._shape(df)[0] for df in left),
    self._shape(right)[0])
            hcells = max(self._shape(df)[1] for df in left) +
    self._shape(right)[1]
        else:
            vcells = max([self._shape(df)[0] for df in left] +
    [self._shape(right)[0]])
            hcells = sum([self._shape(df)[1] for df in left] +
    [self._shape(right)[1]])
    ...
    True):
        ...
        if vertical:
            ...
            for i, (l, label) in enumerate(zip(left, labels)):
                ...
                self._make_table(ax, l, title=label, height=1.0 /
    max_left_rows)
            # right
    ...
    def _insert_index(self, data):
        ...
        if col_nlevels > 1:
            ...
            values = [
                data.columns._get_level_values(i)._values for i in range(1,
    col_nlevels)
            ]
...
if __name__ == "__main__":
    ...
    p.plot(
        [df1, df3], pd.concat([df1, df3], axis=1), labels=["df1", "df2"],
    vertical=False
    )
    plt.show()
