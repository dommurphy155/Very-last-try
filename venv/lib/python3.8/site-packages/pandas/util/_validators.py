...
def _check_for_default_values(fname, arg_val_dict, compat_args):
    ...
    for key in arg_val_dict:
        ...
        try:
            ...
            if (v1 is not None and v2 is None) or (v1 is None and v2 is not
    None):
                match = False
            else:
                match = v1 == v2
...
def _check_for_invalid_keys(fname, kwargs, compat_args):
    ...
    if diff:
        bad_arg = list(diff)[0]
        raise TypeError(f"{fname}() got an unexpected keyword argument
    '{bad_arg}'")
...
def validate_axis_style_args(
    ...
    ...                          'mapper', 'rename')
    {'columns': <built-in function id>, 'index': <method 'upper' of 'str'
    objects>}

    This emits a warning
    >>> validate_axis_style_args(df, (str.upper, id), {},
    ...                          'mapper', 'rename')
    {'index': <method 'upper' of 'str' objects>, 'columns': <built-in function
    id>}
    """
    ...
    if "axis" in kwargs and any(x in kwargs for x in
    data._AXIS_TO_AXIS_NUMBER):
    ...
    if arg_name in kwargs:
        if args:
            msg = f"{method_name} got multiple values for argument
    '{arg_name}'"
            raise TypeError(msg)
...
def validate_percentile(q: float | Iterable[float]) -> np.ndarray:
    ...
    This function checks if the given float or iterable of floats is a valid
    percentile
...
def validate_inclusive(inclusive: str | None) -> tuple[bool, bool]:
    """
    Check that the `inclusive` argument is among {"both", "neither", "left",
    "right"}.
...
def validate_insert_loc(loc: int, length: int) -> int:
    ...
    if not is_integer(loc):
        raise TypeError(f"loc must be an integer between -{length} and
    {length}")

    if loc < 0:
        loc += length
    if not 0 <= loc <= length:
        raise IndexError(f"loc must be an integer between -{length} and
    {length}")
    return loc
