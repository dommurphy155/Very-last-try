...
# TODO(pytest#7469): return type, _pytest.mark.structures.MarkDecorator is not
    public
...
def skip_if_installed(package: str):
    ...
    return pytest.mark.skipif(
        safe_import(package), reason=f"Skipping because {package} is
    installed."
    )
...
skip_if_mpl = pytest.mark.skipif(not _skip_if_no_mpl(), reason="matplotlib is
    present")
skip_if_32bit = pytest.mark.skipif(not IS64, reason="skipping for 32 bit")
skip_if_windows = pytest.mark.skipif(is_platform_windows(), reason="Running on
    Windows")
skip_if_not_us_locale = pytest.mark.skipif(
    _skip_if_not_us_locale(), reason=f"Specific locale is set
    {locale.getlocale()[0]}"
)
...
def async_mark():
    try:
        ...
        async_mark = pytest.mark.skip(reason="Missing dependency
    pytest-asyncio")
