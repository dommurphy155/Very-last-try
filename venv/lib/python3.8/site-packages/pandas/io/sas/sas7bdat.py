...
class SAS7BDATReader(ReaderBase, abc.Iterator):
    ...
    const.endianness_length)
        ...
        #  expected "Sized"
        if len(self._cached_page) != self.header_length:  # type:
    ignore[arg-type]
    ...
    def _read_bytes(self, offset: int, length: int):
        if self._cached_page is None:
            ...
            if len(buf) < length:
                self.close()
                msg = f"Unable to read {length:d} bytes from file position
    {offset:d}."
    ...
    def _read_and_convert_header_text(self, offset: int, length: int) -> str |
    bytes:
    ...
    def _parse_metadata(self) -> None:
        ...
        while not done:
            ...
            if len(self._cached_page) != self._page_length:
                raise ValueError("Failed to read a meta data page from the SAS
    file.")
    ...
    def _process_page_metadata(self) -> None:
        ...
        for i in range(self._current_page_subheaders_count):
            ...
            if pointer.compression == const.truncated_subheader_id:
                continue
            subheader_signature =
    self._read_subheader_signature(pointer.offset)
    ...
    def _get_subheader_index(self, signature: bytes, compression, ptype) ->
    int:
    ...
    def _process_subheader_pointers(
        ...
        total_offset = offset + subheader_pointer_length *
    subheader_pointer_index
        ...
        x = _SubheaderPointer(
            subheader_offset, subheader_length, subheader_compression,
    subheader_type
        )
    ...
    def _process_columntext_subheader(self, offset: int, length: int) -> None:
        ...
        if len(self.column_names_raw) == 1:
            ...
            if hasattr(self, "creator_proc"):
                self.creator_proc =
    self._convert_header_text(self.creator_proc)
    ...
    (int_len + 8)
        for i in range(column_attributes_vectors_count):
            col_data_offset = (
                offset + int_len + const.column_data_offset_offset + i *
    (int_len + 8)
            )
            col_data_len = (
                offset
    ...
    def _process_format_subheader(self, offset: int, length: int) -> None:
        ...
        text_subheader_format = (
            offset + const.column_format_text_subheader_index_offset + 3 *
    int_len
        )
        col_format_offset = offset + const.column_format_offset_offset + 3 *
    int_len
        col_format_len = offset + const.column_format_length_offset + 3 *
    int_len
        text_subheader_label = (
            offset + const.column_label_text_subheader_index_offset + 3 *
    int_len
        )
        col_label_offset = offset + const.column_label_offset_offset + 3 *
    int_len
