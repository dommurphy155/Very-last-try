...
def parse_date_time(date_col, time_col) -> npt.NDArray[np.object_]:
    ...
    warnings.warn(
        """
        Use pd.to_datetime(date_col + " " + time_col) instead to get a Pandas
    Series.
        Use pd.to_datetime(date_col + " " + time_col).to_pydatetime() instead
...
def parse_date_fields(year_col, month_col, day_col) -> npt.NDArray[np.object_]:
    ...
    warnings.warn(
        """
        Use pd.to_datetime({"year": year_col, "month": month_col, "day":
    day_col}) instead to get a Pandas Series.
        Use ser = pd.to_datetime({"year": year_col, "month": month_col, "day":
    day_col}) and
        np.array([s.to_pydatetime() for s in ser]) instead to get a Numpy
    array.
""",  # noqa: E501
        FutureWarning,
        stacklevel=find_stack_level(),
...
def parse_all_fields(
    ...
    warnings.warn(
        ...
        Use pd.to_datetime({"year": year_col, "month": month_col, "day":
    day_col,
        "hour": hour_col, "minute": minute_col, second": second_col}) instead
...
def generic_parser(parse_func, *cols) -> np.ndarray:
    """
    Use dateparser to parse columns with data information into a single
    datetime column.
    ...
    for i, n in enumerate(map(len, tail)):
        if n != N:
            raise AssertionError(
                f"All columns must have the same length: {N}; column {i} has
    length {n}"
            )
