...
class SeriesWriter(Writer):
    ...
    def _format_axes(self):
        if not self.obj.index.is_unique and self.orient == "index":
            raise ValueError(f"Series index must be unique for
    orient='{self.orient}'")
...
def read_json(
    ...
    nrows: int, optional
        The number of lines from the line-delimited jsonfile that has to be
    read.
        This can only be passed if `lines=True`.
    ...
    Notes
    -----
    Specific to ``orient='table'``, if a:class:`DataFrame` with a literal:class:`Index` name of `index` gets written with:func:`to_json`, the
    subsequent read operation will incorrectly set the:class:`Index` name to
    ``None``. This is because `index` is also used by:func:`DataFrame.to_json`
    to denote a missing:class:`Index` name, and the subsequent:func:`read_json` operation cannot distinguish between the two. The same
...
'
    ...
    variable
        # has type "Union[ExtensionDtype, str, dtype[Any], Type[str],
    Type[float],
...
class JsonReader(abc.Iterator, Generic[FrameSeriesStrT]):
    ...
    string
        ...
        if (
            ...
            and filepath_or_buffer.lower().endswith(
                (".json",) + tuple(f".json{c}" for c in
    _extension_to_compression)
            )
            and not file_exists(filepath_or_buffer)
        ):
            raise FileNotFoundError(f"File {filepath_or_buffer} does not
    exist")
    ...
    def _combine_lines(self, lines) -> str:
        ...
        """
        return (
            f'[{",".join([line for line in (line.strip() for line in lines) if
    line])}]'
        )
    ...
    @overload
    def read(self: JsonReader[Literal["frame", "series"]]) -> DataFrame |
    Series:
        ...
    ...
    @overload
    def __next__(self: JsonReader[Literal["frame", "series"]]) -> DataFrame |
    Series:
        ...
...
class Parser:
    ...
    def __init__(
        ...
        if date_unit is not None:
            ...
            if date_unit not in self._STAMP_UNITS:
                raise ValueError(f"date_unit must be one of
    {self._STAMP_UNITS}")
    ...
    def check_keys_split(self, decoded) -> None:
        ...
        if bad_keys:
            ...
            raise ValueError(f"JSON data had unexpected key(s):
    {bad_keys_joined}")
    ...
    def _try_convert_data(
        ...
        if use_dtypes:
            ...
            # Type[object]]]]", right operand type: "Literal[True]")
                ...
                # dtype to force
                dtype = (
                    self.dtype.get(name) if isinstance(self.dtype, dict) else
    self.dtype
                )
...
class FrameParser(Parser):
    ...
    def _try_convert_dates(self):
        ...
        self._process_converter(
            ...
            lambda col, c: (
                (self.keep_default_dates and is_ok(col)) or col in
    convert_dates
            ),
        )
