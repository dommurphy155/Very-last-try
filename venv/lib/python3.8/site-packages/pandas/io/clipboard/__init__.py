...
def init_xclip_clipboard():
    ...
    def copy_xclip(text, primary=False):
        ...
        with subprocess.Popen(
            ["xclip", "-selection", selection], stdin=subprocess.PIPE,
    close_fds=True
        ) as p:
            p.communicate(input=text.encode(ENCODING))
...
def init_xsel_clipboard():
    ...
    def copy_xsel(text, primary=False):
        ...
        with subprocess.Popen(
            ["xsel", selection_flag, "-i"], stdin=subprocess.PIPE,
    close_fds=True
        ) as p:
            p.communicate(input=text.encode(ENCODING))
    ...
    def paste_xsel(primary=False):
        ...
        if primary:
            selection_flag = PRIMARY_SELECTION
        with subprocess.Popen(
            ["xsel", selection_flag, "-o"], stdout=subprocess.PIPE,
    close_fds=True
        ) as p:
            stdout = p.communicate()[0]
        return stdout.decode(ENCODING)
...
def init_dev_clipboard_clipboard():
    def copy_dev_clipboard(text):
        ...
        if text == "":
            warnings.warn(
                "Pyperclip cannot copy a blank string to the clipboard on
    Cygwin. "
        ...
        with window() as hwnd:
            ...
            with clipboard(hwnd):
                ...
                if text:
                    ...
                    count = wcslen(text) + 1
                    handle = safeGlobalAlloc(GMEM_MOVEABLE, count *
    sizeof(c_wchar))
...
def init_wsl_clipboard():
    def copy_wsl(text):
        text = _stringifyText(text)  # Converts non-str values to str.
        with subprocess.Popen(["clip.exe"], stdin=subprocess.PIPE,
    close_fds=True) as p:
            p.communicate(input=text.encode(ENCODING))
...
def set_clipboard(clipboard):
    ...
    clipboard_types = {
        ...
        "qt": init_qt_clipboard,  # TODO - split this into 'qtpy', 'pyqt4',
    and 'pyqt5'
