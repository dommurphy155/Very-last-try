...
def _get_path_or_handle(
    ...
    FilePath | ReadBuffer[bytes] | WriteBuffer[bytes], IOHandles[bytes] |
    None, Any
]:
    ...
    if is_fsspec_url(path_or_handle) and fs is None:
        ...
        raise ValueError("storage_options passed with buffer, or non-supported
    URL")
    ...
    if (
        ...
        # use get_handle only when we are very certain that it is not a
    directory
        # fsspec resources can also point to directories
        # this branch is used for example when reading from non-fsspec URLs
        handles = get_handle(
            path_or_handle, mode, is_text=False,
...
class BaseImpl:
    ...
    def validate_dataframe(df: DataFrame) -> None:
        ...
        if isinstance(df.columns, MultiIndex):
            if not all(
                x.inferred_type in {"string", "empty"} for x in
    df.columns.levels
            ):
                raise ValueError(
                    """
        ...
        valid_names = all(
            isinstance(name, str) for name in df.index.names if name is not
    None
        )
        if not valid_names:
...
class PyArrowImpl(BaseImpl):
    def __init__(self) -> None:
        ...
        import pandas.core.arrays.arrow.extension_types  # pyright: ignore #
    noqa:F401
    ...
    def write(
        ...
        **kwargs,
        ...
        from_pandas_kwargs: dict[str, Any] = {"schema": kwargs.pop("schema",
    None)}
...
class FastParquetImpl(BaseImpl):
    ...
    mode.
            ...
            raise ValueError(
                "storage_options passed with file object or non-fsspec file
    path"
            )
    ...
    def read(
        self, path, columns=None, storage_options: StorageOptions = None,
    **kwargs
    ) -> DataFrame:
...
def to_parquet(
    ...
    **kwargs,
    ...
    path_or_buf: FilePath | WriteBuffer[bytes] = io.BytesIO() if path is None
    else path
