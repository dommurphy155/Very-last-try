...
def justify(texts: Iterable[str], max_len: int, mode: str = "right") ->
    list[str]:
    """
...
def _pprint_seq(
    ...
    r = [
        pprint_thing(next(s), _nest_lvl + 1, max_seq_items=max_seq_items,
    **kwds)
...
def format_object_summary(
    ...
    def _extend_line(
        s: str, line: str, value: str, display_width: int, next_line_prefix:
    str
    ) -> tuple[str, str]:
...
def _justify(
    ...
    head = [
        tuple(x.rjust(max_len) for x, max_len in zip(seq, max_length)) for seq
    in head
    ]
    tail = [
        tuple(x.rjust(max_len) for x, max_len in zip(seq, max_length)) for seq
