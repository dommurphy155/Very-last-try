    ...
    if caption:
        if isinstance(caption, str):
            ...
            try:
                ...
                msg = "caption must be either a string or a tuple of two
    strings"
...
class RowStringConverter(ABC):
    ...
    def get_strrow(self, row_num: int) -> str:
        ...
        is_multicol = (
            row_num < self.column_levels and self.fmt.header and
    self.multicolumn
        )
        ...
        is_cline_maybe_required = is_multirow and row_num < len(self.strrows)
    - 1
...
class GenericTableBuilder(TableBuilderAbstract):
    ...
    def _select_iterator(self, over: str) -> type[RowStringIterator]:
        ...
        if over == "header":
            ...
            msg = f"'over' must be either 'header' or 'body', but {over} was
    provided"
...
class RegularTableBuilder(GenericTableBuilder):
    ...
    >>> df = pd.DataFrame({"a": [1, 2], "b": ["b1", "b2"]})
    >>> formatter = fmt.DataFrameFormatter(df)
    >>> builder = RegularTableBuilder(formatter, caption='caption',
    label='lab',
    ...                               column_format='lrc')
    >>> table = builder.get_result()
    >>> print(table)
...
class LatexFormatter:
    ...
    def __init__(
        ...
        self.multicolumn_format = multicolumn_format
        self.multirow = multirow
        self.caption, self.short_caption =
    _split_into_full_short_caption(caption)
        self.label = label
        self.position = position
    ...
    def column_format(self, input_column_format: str | None) -> None:
        ...
        if input_column_format is None:
            self._column_format = (
                self._get_index_format() +
    self._get_column_format_based_on_dtypes()
            )
...
def _convert_to_bold(crow: Sequence[str], ilevels: int) -> list[str]:
    ...
    return [
        f"\\textbf{{{x}}}" if j < ilevels and x.strip() not in ["", "{}"] else
    x
