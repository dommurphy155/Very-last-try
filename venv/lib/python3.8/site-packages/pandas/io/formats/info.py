...
INFO_DOCSTRING = dedent(
    ...
    memory_usage: bool, str, optional
        ...
        at the cost of computational resources. See the:ref:`Frequently Asked Questions <df-memory-usage>` for more
        details.
...
class BaseInfo(ABC):
    ...
    def non_null_counts(self) -> Sequence[int]:
        """Sequence of non-null counts for all columns or column (if
    series)."""
...
class DataFrameInfoPrinter(InfoPrinterAbstract):
    ...
    def exceeds_info_cols(self) -> bool:
        """Check if number of columns to be summarized does not exceed
    maximum."""
    ...
    def _initialize_show_counts(self, show_counts: bool | None) -> bool:
        if show_counts is None:
            return bool(not self.exceeds_info_cols and not
    self.exceeds_info_rows)
        else:
            return show_counts

    def _create_table_builder(self) -> DataFrameTableBuilder:
        """
        Create instance of table builder based on verbosity and display
    settings.
        """
        if self.verbose:
        ...
        elif self.verbose is False:  # specifically set to False, not
    necessarily None
    ...
    def add_header_line(self) -> None:
        header_line = self.SPACING.join(
            [
                ...
                for header, col_width in zip(self.headers,
        ...
        for row in self.strrows:
            body_line = self.SPACING.join(
                [
                    ...
                    for col, gross_colwidth in zip(row,
    self.gross_column_widths)
                ]
            )
