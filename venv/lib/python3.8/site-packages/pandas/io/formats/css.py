...
def _side_expander(prop_fmt: str) -> Callable:
    """
    Wrapper to expand shorthand property into top, right, bottom, left
    properties
    ...
    Returns
    -------
        function: Return to call when a 'border(-{side}): {value}' string is
    encountered
    """

    def expand(self, prop, value: str) -> Generator[tuple[str, str], None,
    None]:
        """
        Expand shorthand property into side-specific property (top, right,
    bottom, left)
...
def _border_expander(side: str = "") -> Callable:
    """
    Wrapper to expand 'border' property into border color, style, and width
    properties
    ...
    standards?
        ...
        for token in tokens:
            ...
            elif any(ratio in token.lower() for ratio in
    self.BORDER_WIDTH_RATIOS):
                border_declarations[f"border{side}-width"] = token
            else:
                border_declarations[f"border{side}-color"] = token
            # TODO: Warn user if item entered more than once (e.g. "border:
    red green")
...
class CSSResolver:
    ...
    CSS_EXPANSIONS = {
        **{
            "-".join(["border", prop] if prop else ["border"]):
    _border_expander(prop)
    ...
    def parse(self, declarations_str: str) -> Iterator[tuple[str, str]]:
        ...
        for decl in declarations_str.split(";"):
            ...
            if sep:
                ...
                warnings.warn(
                    f"Ill-formatted attribute: expected a colon in
    {repr(decl)}",
                    CSSWarning,
                    stacklevel=find_stack_level(),
                )
