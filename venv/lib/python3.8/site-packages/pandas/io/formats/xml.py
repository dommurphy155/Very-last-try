...
class BaseXMLFormatter:
    ...
    def validate_columns(self) -> None:
        ...
        if self.attr_cols and not is_list_like(self.attr_cols):
            raise TypeError(
                f"{type(self.attr_cols).__name__} is not a valid type for
    attr_cols"
            )

        if self.elem_cols and not is_list_like(self.elem_cols):
            raise TypeError(
                f"{type(self.elem_cols).__name__} is not a valid type for
    elem_cols"
            )
    ...
    def handle_indexes(self) -> None:
        ...
        indexes: list[str] = [
            x for x in self.frame_dicts[first_key].keys() if x not in
    self.orig_cols
        ]
    ...
    def _build_elems(self, sub_element_cls, d: dict[str, Any], elem_row: Any)
    -> None:
        ...
        if self.namespaces:
            ...
            if self.prefix:
                try:
                    ...
                    raise KeyError(f"{self.prefix} is not included in
    namespaces")
            else:
...
class LxmlXMLFormatter(BaseXMLFormatter):
    ...
    def build_tree(self) -> bytes:
        ...
        self.root = Element(f"{self.prefix_uri}{self.root_name}",
