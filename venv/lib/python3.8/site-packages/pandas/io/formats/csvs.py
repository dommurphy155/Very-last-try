...
class CSVFormatter:
    ...
    def _initialize_index_label(self, index_label: IndexLabel | None) ->
    IndexLabel:
    ...
    def _initialize_columns(self, cols: Sequence[Hashable] | None) ->
    np.ndarray:
        # validate mi options
    ...
    def data_index(self) -> Index:
        ...
        if (
            ...
            data_index = Index(
                [x.strftime(self.date_format) if notna(x) else "" for x in
    data_index]
            )
    ...
    def write_cols(self) -> Sequence[Hashable]:
        if self._has_aliases:
            ...
            if len(self.header) != len(self.cols):
                raise ValueError(
                    f"Writing {len(self.cols)} cols but got {len(self.header)}
    aliases"
                )
            else:
                return self.header
    ...
    def _generate_multiindex_header_rows(self) -> Iterator[list[Hashable]]:
        ...
        for i in range(columns.nlevels):
            ...
            if self.index:
                ...
                if isinstance(self.index_label, list) and
    len(self.index_label) > 1:
    ...
    def _save_chunk(self, start_i: int, end_i: int) -> None:
        ...
        ix =
    self.data_index[slicer]._format_native_types(**self._number_format)
