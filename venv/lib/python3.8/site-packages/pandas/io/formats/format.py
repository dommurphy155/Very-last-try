...
class TextAdjustment:
    ...
    def justify(self, texts: Any, max_len: int, mode: str = "right") ->
    list[str]:
...
class EastAsianTextAdjustment(TextAdjustment):
    ...
    def len(self, text: str) -> int:
        ...
        return sum(
            self._EAW_MAP.get(east_asian_width(c), self.ambiguous_width) for c
    in text
        )
...
def get_dataframe_repr_params() -> dict[str, Any]:
    """Get the parameters used to repr(dataFrame) calls using
    DataFrame.to_string.
...
def get_series_repr_params() -> dict[str, Any]:
    ...
    ``repr(series)``. This is useful if you want to adjust the series repr
    output.
...
class DataFrameFormatter:
    ...
    def is_truncated(self) -> bool:
        return bool(self.is_truncated_horizontally or
    self.is_truncated_vertically)

    @property
    def is_truncated_horizontally(self) -> bool:
        return bool(self.max_cols_fitted and (len(self.columns) >
    self.max_cols_fitted))

    @property
    def is_truncated_vertically(self) -> bool:
        return bool(self.max_rows_fitted and (len(self.frame) >
    self.max_rows_fitted))

    @property
    def dimensions_info(self) -> str:
        return f"\n\n[{len(self.frame)} rows x {len(self.frame.columns)}
    columns]"
    ...
    def _initialize_formatters(
        ...
        if formatters is None:
            return {}
        elif len(self.frame.columns) == len(formatters) or
    isinstance(formatters, dict):
            return formatters
        else:
            raise ValueError(
    ...
    def _initialize_colspace(self, col_space: ColspaceArgType | None) ->
    ColspaceType:
        result: ColspaceType
    ...
    def _get_strcols_without_index(self) -> list[list[str]]:
        ...
        for i, c in enumerate(self.tr_frame):
            ...
            header_colwidth = max(
                int(self.col_space.get(c, 0)), *(self.adj.len(x) for x in
    cheader)
            )
            fmt_values = self.format_col(i)
            fmt_values = _make_fixed_width(
    ...
    def _get_formatted_column_labels(self, frame: DataFrame) ->
    list[list[str]]:
        ...
        if isinstance(columns, MultiIndex):
            ...
            restrict_formatting = any(level.is_floating for level in
    columns.levels)
            ...
            need_leadsp = dict(zip(fmt_columns, map(is_numeric_dtype, dtypes)))
            str_columns = [
                [" " + x if not self._get_formatter(i) and need_leadsp[x] else
    x]
    ...
    def _get_formatted_index(self, frame: DataFrame) -> list[str]:
        ...
        if isinstance(index, MultiIndex):
            ...
            fmt_index = [index.format(name=self.show_row_idx_names,
    formatter=fmt)]

        fmt_index = [
            tuple(
                _make_fixed_width(
                    list(x), justify="left", minimum=col_space.get("", 0),
    adj=self.adj
                )
            )
            for x in fmt_index
    ...
    def _get_column_name_list(self) -> list[Hashable]:
        ...
        if isinstance(columns, MultiIndex):
            names.extend("" if name is None else name for name in
    columns.names)
        else:
...
class FloatArrayFormatter(GenericArrayFormatter):
    ...
    def get_result_as_array(self) -> np.ndarray:
        ...
        def format_with_na_rep(values: ArrayLike, formatter: Callable, na_rep:
    str):
            mask = isna(values)
            formatted = np.array(
                [
...
def format_percentiles(
    ...
    prec = -np.floor(
        np.log10(np.min(np.ediff1d(unique_pcts, to_begin=to_begin,
    to_end=to_end)))
    ).astype(int)
    prec = max(1, prec)
...
def is_dates_only(values: np.ndarray | DatetimeArray | Index | DatetimeIndex)
    -> bool:
    ...
    even_days = np.logical_and(consider_values, values_int % ppd != 0).sum()
    == 0
    if even_days:
        return True
    return False
...
def _format_datetime64(x: NaTType | Timestamp, nat_rep: str = "NaT") -> str:
    ...
    # Timestamp.__str__ falls back to datetime.datetime.__str__ =
    isoformat(sep=' ')
...
def get_format_timedelta64(
    ...
    both = np.logical_and(consider_values, not_midnight)  # type:
    ignore[arg-type]
...
def _trim_zeros_complex(str_complexes: np.ndarray, decimal: str = ".") ->
    list[str]:
    """
