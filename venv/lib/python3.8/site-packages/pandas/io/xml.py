...
class _XMLFrameParser:
    ...
    Notes
    -----
    To subclass this class effectively you must override the following
    methods:`
        *:func:`parse_data`
        *:func:`_parse_nodes`
    ...
    def _parse_nodes(self, elems: list[Any]) -> list[dict[str, str | None]]:
        ...
        if self.elems_only and self.attrs_only:
            raise ValueError("Either element or attributes can be parsed not
    both.")
        ...
        dicts = [
            {k.split("}")[1] if "}" in k else k: v for k, v in d.items()} for
    d in dicts
        ]
    ...
    dicts]
        ...
        if self.names:
            dicts = [{nm: v for nm, v in zip(self.names, d.values())} for d in
    dicts]

        return dicts

    def _iterparse_nodes(self, iterparse: Callable) -> list[dict[str, str |
    None]]:
        """
        Iterparse xml nodes.

        This method will read in local disk, decompressed XML files for
    elements
        ...
        if not isinstance(self.iterparse, dict):
            raise TypeError(
                f"{type(self.iterparse).__name__} is not a valid type for
    iterparse"
            )
        ...
        if (
            ...
            raise ParserError(
                "iterparse is designed for large XML files that are fully
    extracted on "
                ...
                if self.names:
                    for col, nm in zip(self.iterparse[row_node], self.names):
                        ...
                        if col in elem.attrib:
                            if elem.attrib[col] not in row.values() and nm not
    in row:
            ...
            if event == "end":
                ...
                if hasattr(elem, "getprevious"):
                    while (
                        elem.getprevious() is not None and elem.getparent() is
    not None
                    ):
...
class _EtreeFrameParser(_XMLFrameParser):
    ...
    def parse_data(self) -> list[dict[str, str | None]]:
        ...
        if self.stylesheet is not None:
            raise ValueError(
                "To use stylesheet, you need lxml installed and selected as
    parser."
            )
        ...
        if self.iterparse is None:
            ...
            self._validate_path()
            elems = self.xml_doc.findall(self.xpath,
    ...
    def _validate_path(self) -> None:
        ...
        try:
            ...
            if elems is not None and elems.find("*") is None and elems.attrib
    is None:
                raise ValueError(msg)
    ...
    def _validate_names(self) -> None:
        ...
        if self.names:
            if self.iterparse:
                ...
                parent = self.xml_doc.find(self.xpath,
    namespaces=self.namespaces)
            ...
            if is_list_like(self.names):
                if len(self.names) < len(children):
                    raise ValueError(
                        "names does not match length of child elements in
    xpath."
                    )
            else:
                raise TypeError(
                    f"{type(self.names).__name__} is not a valid type for
    names"
                )
    ...
    **kwargs,
    ...
    if parser == "lxml":
        ...
        if lxml is not None:
            ...
            raise ImportError("lxml not found, please install or use the etree
    parser.")
...
def read_xml(
    ...
    path_or_buffer: str, path object, or file-like object
        ...
        object implementing a ``read()`` function. The string can be any valid
    XML
    ...
    parse_dates: bool or list of int or names or list of lists or dict,
    default False
        Identifiers to parse index or columns to datetime. The behavior is as
    follows:
    ...
    iterparse: dict, optional
        ...
        list of elements or attribute names that are descendants of the
    repeated
        element. Note: If this option is used, it will replace ``xpath``
    parsing
        and unlike xpath, descendants do not need to relate to each other but
    can
        exist any where in document under the repeating element. This memory-
        efficient method should be used for very large XML files (500MB, 1GB,
    or 5GB+).
        For example,::

            iterparse = {{"row_element": ["child_elem", "attr",
    "grandchild_elem"]}}
