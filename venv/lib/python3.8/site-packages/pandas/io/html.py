...
def _get_skiprows(skiprows: int | Sequence[int] | slice | None) -> int |
    Sequence[int]:
    """
    ...
    if isinstance(skiprows, slice):
        ...
        return 0
    raise TypeError(f"{type(skiprows).__name__} is not a valid type for
    skipping rows")


def _read(
    obj: bytes | FilePath | ReadBuffer[str] | ReadBuffer[bytes], encoding: str
    | None
) -> str | bytes:
    """
    ...
    if (
        ...
        # error: Argument 1 to "get_handle" has incompatible type "Union[str,
    bytes,
...
class _HtmlFrameParser:
    ...
    extract_links: {None, "all", "header", "body", "footer"}
        Table elements in the specified section(s) with <a> tags will have
    their
        href extracted.
        ...
        for tr in rows:
            ...
            for td in tds:
                ...
                while remainder and remainder[0][0] <= index:
                    ...
                    if prev_rowspan > 1:
                        next_remainder.append((prev_i, prev_text, prev_rowspan
    - 1))
                    index += 1

                # Append the text from this <td>, colspan times
                text = _remove_whitespace(self._text_getter(td))
                if self.extract_links == "all" or self.extract_links ==
    section:
...
class _BeautifulSoupHtml5LibFrameParser(_HtmlFrameParser):
    ...
    Notes
    -----
    Documentation strings for this class are in the base class:class:`pandas.io.html._HtmlFrameParser`.
    """
    ...
    def _parse_tables(self, doc, match, attrs):
        ...
        for table in tables:
            if self.displayed_only:
                for elem in
    table.find_all(style=re.compile(r"display:\s*none")):
                    elem.decompose()

            if table not in unique_tables and table.find(string=match) is not
    None:
                result.append(table)
            unique_tables.add(table)

        if not result:
            raise ValueError(f"No tables found matching pattern
    {repr(match.pattern)}")
        return result
...
class _LxmlFrameParser(_HtmlFrameParser):
    ...
    Notes
    -----
    Documentation strings for this class are in the base class:class:`_HtmlFrameParser`.
    """
    ...
    def _parse_tables(self, doc, match, kwargs):
        ...
        xpath_expr = f"//table//*[re:test(text(),
    {repr(pattern)})]/ancestor::table"
        ...
        if self.displayed_only:
            for table in tables:
                ...
                for elem in table.xpath(".//*[@style]"):
                    if "display:none" in elem.attrib.get("style",
    "").replace(" ", ""):
    ...
    if head:
        ...
        if header is None:
            if len(head) == 1:
                ...
                header = [i for i, row in enumerate(head) if any(text for text
    in row)]
...
def _parser_dispatch(flavor: str | None) -> type[_HtmlFrameParser]:
    ...
    if flavor not in valid_parsers:
        raise ValueError(
            f"{repr(flavor)} is not a valid flavor, valid flavors are
    {valid_parsers}"
        )
    ...
    if flavor in ("bs4", "html5lib"):
        ...
        if not _HAS_BS4:
            raise ImportError("BeautifulSoup4 (bs4) not found, please install
    it")
    ...
    **kwargs):
    ...
    for table in tables:
        try:
            ...
            # Cast MultiIndex header to an Index of tuples when extracting
    header
            # links and replace nan with None (therefore can't use
    mi.to_flat_index()).
            ...
            if extract_links in ("all", "header") and isinstance(
                ...
                df.columns = Index(
                    ((col[0], None if isna(col[1]) else col[1]) for col in
    df.columns),
                    tupleize_cols=False,
                )
...
def read_html(
    ...
    <https://html.spec.whatwg.org/multipage/dom.html#global-attributes>`__.::
        ...
        <https://html.spec.whatwg.org/multipage/tables.html>`__. It contains
    the
