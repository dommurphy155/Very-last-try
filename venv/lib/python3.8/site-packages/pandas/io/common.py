...
class IOHandles(Generic[AnyStr]):
    ...
    This is used to easily close created buffers and to handle corner cases
    when
...
def validate_header_arg(header: object) -> None:
    ...
    if is_list_like(header, allow_sets=False):
        ...
        if any(i < 0 for i in header):
            raise ValueError("cannot specify multi-index header with negative
    integers")
        return
    if is_bool(header):
        raise TypeError(
...
@overload
def stringify_path(filepath_or_buffer: FilePath, convert_file_like: bool =
    ...) -> str:
    ...
...
def stringify_path(
    ...
    opened a
        # file. This prevents opening the file a second time.
    infer_compression calls
...
def _get_filepath_or_buffer(
    ...
    compression_method = infer_compression(filepath_or_buffer,
    compression_method)

    # GH21227 internal compression is not used for non-binary handles.
    if compression_method and hasattr(filepath_or_buffer, "write") and "b" not
    in mode:
        warnings.warn(
            "compression has no effect when passing a non-binary object as
    input.",
            RuntimeWarning,
    ...
    (fsspec)
    # except when text mode is explicitly requested. The original mode is
    returned if
    # fsspec is not used.
    fsspec_mode = mode
    ...
    if isinstance(filepath_or_buffer, str) and is_url(filepath_or_buffer):
        ...
        req_info = urllib.request.Request(filepath_or_buffer,
    ...
    if is_fsspec_url(filepath_or_buffer):
        ...
        # two special-case s3-like protocols; these have special meaning in
    Hadoop,
    ...
    if not (
        hasattr(filepath_or_buffer, "read") or hasattr(filepath_or_buffer,
    "write")
    ):
...
def infer_compression(
    ...
    valid = ["infer", None] + sorted(_supported_compressions)  # type:
    ignore[operator]
    msg = (
...
def check_parent_directory(path: Path | str) -> None:
    ...
    if not parent.is_dir():
        raise OSError(rf"Cannot save file into a non-existent directory:
    '{parent}'")
...
def get_handle(
    ...
    *,
    ...
    is_text: bool, default True
        ...
        bytes. This is not the same as `"b" not in mode`. If a string content
    is
    ...
    if compression:
        ...
        # ZIP Compression
            ...
            # "Union[str, BaseBuffer]"; expected "Union[Union[str,
    PathLike[str]],
            # ReadBuffer[bytes], WriteBuffer[bytes]]"
            handle = _BytesZipFile(
                handle, ioargs.mode, **compression_args  # type:
    ignore[arg-type]
            )
            if handle.buffer.mode == "r":
                ...
                if len(zip_names) == 1:
                    ...
                    raise ValueError(f"Zero files found in ZIP file
    {path_or_buf}")
                else:
                    raise ValueError(
        ...
        # TAR Encoding
            ...
            if isinstance(handle, str):
                ...
                handle = _BytesTarFile(
                    fileobj=handle, **compression_args  # type:
    ignore[arg-type]
                )
            ...
            if "r" in handle.buffer.mode:
                ...
                if len(files) == 1:
                    ...
                    raise ValueError(f"Zero files found in TAR archive
    {path_or_buf}")
                else:
                    raise ValueError(
        ...
        # XZ Compression
            ...
            # BaseBuffer]"; expected "Optional[Union[Union[str, bytes,
    PathLike[str],
            # PathLike[bytes]], IO[bytes]]]"
            handle = get_lzma_file()(handle, ioargs.mode)  # type:
    ignore[arg-type]
    ...
    return IOHandles(
        ...
        # error: Argument "created_handles" to "IOHandles" has incompatible
    type
...
class _BytesTarFile(_BufferedWriter):
    ...
    def infer_filename(self) -> str | None:
        """
        If an explicit archive_name is not given, we still want the file
    inside the zip
        file not to be named something.tar, because that causes confusion
    (GH39465).
        """
...
class _BytesZipFile(_BufferedWriter):
    def __init__(
        ...
        **kwargs,
        ...
        self.buffer = zipfile.ZipFile(file, mode, **kwargs)  # type:
    ignore[arg-type]
    ...
    def infer_filename(self) -> str | None:
        ...
        If an explicit archive_name is not given, we still want the file
    inside the zip
        file not to be named something.zip, because that causes confusion
    (GH39465).
        """
...
class _IOWrapper:
    # TextIOWrapper is overly strict: it request that the buffer has seekable,
    readable,
    # and writable. If we have a read-only buffer, we shouldn't need writable
    and vice
    # versa. Some buffers, are seek/read/writ-able but they do not have the
    "-able"
    # methods, e.g., tempfile.SpooledTemporaryFile.
    # If a buffer does not have the above "-able" methods, we simple assume
    they are
    # seek/read/writ-able.
...
def _maybe_memory_map(
    ...
    try:
        ...
        wrapped = _IOWrapper(
            mmap.mmap(
                handle.fileno(), 0, access=mmap.ACCESS_READ  # type:
    ignore[arg-type]
            )
        )
    finally:
...
def _get_binary_io_classes() -> tuple[type, ...]:
    ...
    # have to use the `zstd.ZstdDecompressionReader` class for isinstance
    checks.
    # Unfortunately `zstd.ZstdDecompressionReader` isn't exposed by
    python-zstandard
