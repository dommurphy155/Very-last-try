...
class ParserBase:
    ...
    def __init__(self, kwds) -> None:
        ...
        self.parse_dates = _validate_parse_dates_arg(kwds.pop("parse_dates",
    False))
        ...
        if is_list_like(self.header, allow_sets=False):
            if kwds.get("usecols"):
                raise ValueError(
                    "cannot specify usecols when specifying a multi-index
    header"
                )
            if kwds.get("names"):
                raise ValueError(
        ...
        self.usecols, self.usecols_dtype =
    self._validate_usecols_arg(kwds["usecols"])

        # Fallback to error to pass a sketchy
    test(test_override_set_noconvert_columns)
        # Normally, this arg would get pre-processed earlier on
        self.on_bad_lines = kwds.get("on_bad_lines",
    self.BadLineHandleMethod.ERROR)

    def _validate_parse_dates_presence(self, columns: Sequence[Hashable]) ->
    Iterable:
        """
        ...
        if is_dict_like(self.parse_dates):
            ...
            cols_needed = itertools.chain.from_iterable(
                col if is_list_like(col) and not isinstance(col, tuple) else
    [col]
    ...
    self.index_col)
        ...
        if not all(len(header_iter) == field_count for header_iter in
    header[1:]):
            raise ParserError("Header rows must have an equal number of
    columns.")
        ...
        if len(ic):
            col_names = [
                r[ic[0]]
                if ((r[ic[0]] is not None) and r[ic[0]] not in
    self.unnamed_cols)
                else None
                for r in header
            ]
    ...
    @final
    def _maybe_dedup_names(self, names: Sequence[Hashable]) ->
    Sequence[Hashable]:
    ...
    def _get_complex_date_index(self, data, col_names):
        def _get_name(icol):
            ...
            if col_names is None:
                raise ValueError(f"Must supply column order to use {icol!s} as
    index")
    ...
    self.keep_default_na
            ...
            if self.index_names is not None:
                ...
                if isinstance(converters, dict):
                    index_converter = converters.get(self.index_names[i]) is
    not None
    ...
    def _convert_to_ndarrays(
        ...
        for c, values in dct.items():
            ...
            if c in self._parse_date_cols:
                ...
                mask = algorithms.isin(values, set(col_na_values) |
    col_na_fvalues)
                ...
                if cast_type is not None:
                    warnings.warn(
                        (
                            ...
                            f"for column {c} - only the converter will be
    used."
                        ),
                        ParserWarning,
    ...
    def _set_noconvert_dtype_columns(
        ...
        undergo such conversions. If usecols is specified, the positions of
    the columns
        ...
        Returns
        -------
        A set of integers containing the positions of the columns not to
    convert.
        """
        ...
        elif callable(self.usecols) or self.usecols_dtype not in ("empty",
    None):
    ...
    strings in isin
            na_values = np.array([val for val in na_values if not
    isinstance(val, str)])
    ...
    dtype=cast_type)
                ...
                raise NotImplementedError(
                    ...
                    "_from_sequence_of_strings in order to be used in parser
    methods"
                ) from err
    ...
    ) -> tuple[Sequence[Hashable] | Index, Mapping[Hashable, ArrayLike] |
    DataFrame]:
    ...
    def _check_data_length(
        ...
        if not self.index_col and len(columns) != len(data) and columns:
            ...
            empty_str_or_na = empty_str | isna(data[-1])  # type:
    ignore[operator]
    ...
    def _validate_usecols_names(self, usecols, names):
        ...
        if len(missing) > 0:
            raise ValueError(
                f"Usecols do not match columns, columns expected but not
    found: "
                f"{missing}"
            )
    ...
    def _clean_index_names(self, columns, index_col):
        ...
        # In case of no rows and multiindex columns we have to set index_names
    to
    ...
    def _get_empty_meta(
        ...
        if not is_dict_like(dtype):
            ...
            dtype_dict = defaultdict(
                lambda: object,
                {columns[k] if is_integer(k) else k: v for k, v in
    dtype.items()},
            )
        ...
        col_dict = {
            col_name: Series([], dtype=dtype_dict[col_name]) for col_name in
    columns
        }
...
def _make_date_converter(
    date_parser=None, dayfirst=False, infer_datetime_format=False,
    cache_dates=True
):
        ...
        for colspec in parse_spec:
            if is_scalar(colspec) or isinstance(colspec, tuple):
                ...
                if new_name in data_dict:
                    raise ValueError(f"New date column already in dict
    {new_name}")
        ...
        for new_name, colspec in parse_spec.items():
            ...
            # If original column can be converted to date we keep the
    converted values
...
def _is_potential_multi_index(
    ...
    return bool(
        ...
        and all(isinstance(c, tuple) for c in columns if c not in
    list(index_col))
    )
