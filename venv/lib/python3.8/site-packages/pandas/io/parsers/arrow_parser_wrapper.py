...
class ArrowParserWrapper(ParserBase):
    ...
    def _parse_kwds(self):
        ...
        if isinstance(na_values, dict):
            raise ValueError(
                "The pyarrow engine doesn't support passing a dict for
    na_values"
            )
    ...
    def _get_pyarrow_options(self):
        ...
        for pandas_name, pyarrow_name in mapping.items():
            if pandas_name in self.kwds and self.kwds.get(pandas_name) is not
    None:
        ...
        self.convert_options = {
            ...
            if option_value is not None
            and option_name
            in ("include_columns", "null_values", "true_values",
    "false_values")
        }
        self.read_options = {
    ...
    def _finalize_output(self, frame: DataFrame) -> DataFrame:
        ...
        if self.header is None:
            ...
            if len(self.names) != num_cols:
                # usecols is passed through to pyarrow, we only handle index
    col here
                # The only way self.names is not the same length as number of
    cols is
                # if we have int index_col. We should just pad the names(they
    will get
                # removed anyways) to expected length then.
                self.names = list(range(num_cols - len(self.names))) +
    self.names
