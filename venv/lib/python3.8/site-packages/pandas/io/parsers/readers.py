...
squeeze: bool, default False
    ...
    .. deprecated:: 1.4.0
        Append ``.squeeze("columns")`` to the call to ``{func_name}`` to
    squeeze
        the data.
prefix: str, optional
...
_deprecated_defaults: dict[str, _DeprecationConfig] = {
    "error_bad_lines": _DeprecationConfig(None, "Use on_bad_lines in the
    future."),
    "warn_bad_lines": _DeprecationConfig(None, "Use on_bad_lines in the
    future."),
...
def _validate_names(names: Sequence[Hashable] | None) -> None:
    ...
    if names is not None:
        ...
        if not (
            is_list_like(names, allow_sets=False) or isinstance(names,
    abc.KeysView)
        ):
...
def _read(
    ...
    if kwds.get("engine") == "pyarrow":
        if iterator:
            raise ValueError(
                "The 'iterator' option is not supported with the 'pyarrow'
    engine"
            )
...
def read_fwf(
    ...
    **kwds,
    ...
    filepath_or_buffer: str, path object, or file-like object
        ...
        object implementing a text ``read()`` function.The string could be a
    URL.
    ...
    **kwds: optional
    ...
    if names is not None:
        if len(names) != len(colspecs) and colspecs != "infer":
            ...
            if kwds.get("usecols") is None and len(names) + len_index !=
    len(colspecs):
                # If usecols is used colspec may be longer than names
                raise ValueError("Length of colspecs must match length of
    names")
...
class TextFileReader(abc.Iterator):
    ...
    def __init__(
        ...
        **kwds,
        ...
        if dialect is not None:
            if engine == "pyarrow":
                raise ValueError(
                    "The 'dialect' option is not supported with the 'pyarrow'
    engine"
                )
        ...
        for argname, default in parser_defaults.items():
            ...
            if (
                ...
                if (
                    ...
                    argname == "on_bad_lines" and kwds.get("warn_bad_lines")
    is not None
                ):
                ...
                # GH12935
                raise ValueError("Setting mangle_dupe_cols=False is not
    supported yet")
            else:
    ...
    _python_unsupported:
                        ...
                        raise ValueError(
                            f"The {repr(argname)} option is not supported with
    the "
        ...
        if quotechar is not None and isinstance(quotechar, (str, bytes)):
            if (
                ...
                fallback_reason = (
                    ...
                    f"and the '{engine}' engine does not support such
    quotechars"
                )
                engine = "python"
    ...
    stacklevel=find_stack_level())
        ...
        # previous behavior.
        result["squeeze"] = False if options["squeeze"] is None else
    options["squeeze"]
...
def _refine_defaults_read(
    ...
    """Validate/refine default values of input parameters of read_csv,
    read_table.
    ...
    if delimiter and (sep is not lib.no_default):
        raise ValueError("Specified a sep and a delimiter; you can only
    specify one.")
    ...
    if (
        ...
        raise ValueError("Specified named and prefix; you can only specify
    one.")
    ...
    engine "
            "which does not accept a line terminator. Hence it is not allowed
    to use "
    ...
    "
        ...
        if on_bad_lines == "error":
            ...
            if engine != "python":
                raise ValueError(
                    "on_bad_line can only be a callable function if
    engine='python'"
                )
