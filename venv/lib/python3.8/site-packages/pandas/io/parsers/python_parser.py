...
class PythonParser(ParserBase):
    ...
    def _make_reader(self, f: IO[str] | ReadCsvBuffer[str]) -> None:
        ...
        if sep is None or len(sep) == 1:
            if self.lineterminator:
                raise ValueError(
                    "Custom line terminators not supported in python parser
    (yet)"
                )
    ...
    "_reader",
        # variable has type "Union[IO[Any], RawIOBase, BufferedIOBase,
    TextIOBase,
    ...
    def read(
        ...
        Index | None, Sequence[Hashable] | MultiIndex, Mapping[Hashable,
    ArrayLike]
    ]:
        try:
        ...
        if not len(content):  # pragma: no cover
            ...
            conv_columns = self._maybe_make_multi_index_columns(columns,
    self.col_names)
    ...
    def _exclude_implicit_index(
        ...
        return {
            name: alldata[i + offset] for i, name in enumerate(names) if i <
    len_alldata
        }, names
            ...
            for level, hr in enumerate(header):
                try:
                    ...
                    if 0 < self.line_pos <= hr and (
                        ...
                        # If no rows we want to raise a different message and
    if
                        # we have mi columns, the last line is not part of the
    header
                        joi = list(map(str, header[:-1] if have_mi_columns
    else header))
                    ...
                    for i in col_loop_order:
                        ...
                        if cur_count > 0:
                            ...
                            if (
                                ...
                                self.dtype.update({col:
    self.dtype.get(old_col)})
                        this_columns[i] = col
    ...
    or ic == 0:
                ...
                unnamed_cols.update({this_columns[i] for i in
    this_unnamed_cols})
            ...
            if names is not None:
                ...
                len_first_data_row = 0 if first_line is None else
    len(first_line)

                if len(names) > len(columns[0]) and len(names) >
    len_first_data_row:
                    raise ValueError(
    ...
    columns")
            ...
            try:
                ...
                if not names:
                    raise EmptyDataError("No columns to parse from file") from
    err
    ...
    num_original_columns)
                    ...
                    if not callable(self.usecols) and len(names) !=
    len(self.usecols):
                        raise ValueError(
                ...
                for col in self.usecols:
                    if isinstance(col, str):
                        try:
                            ...
                            self._validate_usecols_names(self.usecols,
    usecols_key)
                    else:
                ...
                if missing_usecols:
                    warnings.warn(
                        "Defining usecols with out of bounds indices is
    deprecated "
    ...
    def _remove_empty_lines(self, lines: list[list[Scalar]]) ->
    list[list[Scalar]]:
        """
    ...
    def _check_thousands(self, lines: list[list[Scalar]]) ->
    list[list[Scalar]]:
        if self.thousands is None:
            return lines
        ...
        for line in lines:
            ...
            for i, x in enumerate(line):
                if (
                    not isinstance(x, str)
                    or search not in x
                    or (self._no_thousands_columns and i in
    ...
    "List[int]",
        ...
        # see gh-13320
        zipped_content = list(lib.to_object_array(content,
    min_width=col_len).T)
                ...
                try:
                    if rows is not None:
                        ...
                        for _ in range(rows + rows_to_skip):
                            # assert for mypy, data is Iterator[str] or None,
    would
                            # error in next
                            assert self.data is not None
    ...
    def _remove_skipped_rows(self, new_rows: list[list[Scalar]]) ->
    list[list[Scalar]]:
        if self.skiprows:
            return [
                row for i, row in enumerate(new_rows) if not self.skipfunc(i +
    self.pos)
            ]
        return new_rows
...
class FixedWidthReader(abc.Iterator):
    ...
    def get_rows(self, infer_nrows: int, skiprows: set[int] | None = None) ->
    list[str]:
        """
    ...
    def __next__(self) -> list[str]:
        ...
        return [line[fromm:to].strip(self.delimiter) for (fromm, to) in
    self.colspecs]
