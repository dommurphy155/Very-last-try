...
# TODO: Add typing. As of January 2020 it is not possible to type this
    function since
#  mypy doesn't understand that a Series and an int can be combined using
    mathematical
#  operations. (+, -).
def _stata_elapsed_date_to_datetime_vec(dates, fmt) -> Series:
    ...
    def convert_delta_safe(base, deltas, unit) -> Series:
        ...
        if unit == "d":
            ...
            if deltas.max() > MAX_MS_DELTA or deltas.min() < MIN_MS_DELTA:
                values = [
                    base + relativedelta(microseconds=(int(d) * 1000)) for d
    in deltas
                ]
                return Series(values, index=index)
        else:
    ...
    if bad_locs.any():
        has_bad_values = True
        # reset cache to avoid SettingWithCopy checks (we own the DataFrame
    and the
    ...
    microseconds
            ...
            if delta:
                ...
                def f(x: datetime.timedelta) -> float:
                    return US_PER_DAY * x.days + 1000000 * x.seconds +
    x.microseconds
...
def _cast_to_stata_types(data: DataFrame) -> DataFrame:
    ...
    for col in data:
        ...
        is_nullable_int = isinstance(data[col].dtype, (IntegerDtype,
    BooleanDtype))
        orig = data[col]
    ...
    the range "
        ...
        if is_nullable_int:
            if orig_missing.any():
                ...
                sentinel =
    StataMissingValue.BASE_MISSING_VALUES[data[col].dtype.name]
...
class StataParser:
    def __init__(self) -> None:
        ...
        self.DTYPE_MAP = dict(
            list(zip(range(1, 245), [np.dtype("a" + str(i)) for i in range(1,
    245)]))
            + [
...
class StataReader(StataParser, abc.Iterator):
    ...
    def _read_header(self) -> None:
        ...
        self.has_string_data = len([x for x in self.typlist if type(x) is
    int]) > 0
    ...
    def _read_new_header(self) -> None:
        ...
        if self.format_version not in [117, 118, 119]:
            raise
    ValueError(_version_error.format(version=self.format_version))
        self._set_encoding()
    ...
    def _get_dtypes(
        ...
        def f(typ: int) -> int | str:
            ...
            try:
                ...
                raise ValueError(f"cannot convert stata types [{typ}]") from
    err
        ...
        def g(typ: int) -> str | np.dtype:
            ...
            try:
                ...
                raise ValueError(f"cannot convert stata dtype [{typ}]") from
    err
    ...
    def _get_varlist(self) -> list[str]:
        ...
        return [self._decode(self.path_or_buf.read(b)) for _ in
    range(self.nvar)]
    ...
    def _get_variable_labels(self) -> list[str]:
        if self.format_version >= 118:
            vlblist = [
                self._decode(self.path_or_buf.read(321)) for _ in
    range(self.nvar)
            ]
        elif self.format_version > 105:
            vlblist = [
                self._decode(self.path_or_buf.read(81)) for _ in
    range(self.nvar)
            ]
        else:
            vlblist = [
                self._decode(self.path_or_buf.read(32)) for _ in
    range(self.nvar)
            ]
        return vlblist
    ...
    from err
        try:
            ...
            raise ValueError(f"cannot convert stata dtypes
    [{invalid_dtypes}]") from err

        if self.format_version > 108:
            self.varlist = [
                self._decode(self.path_or_buf.read(33)) for _ in
    range(self.nvar)
            ]
        else:
            self.varlist = [
                self._decode(self.path_or_buf.read(9)) for _ in
    range(self.nvar)
            ]
        self.srtlist = struct.unpack(
            self.byteorder + ("h" * (self.nvar + 1)),
    ...
    def _setup_dtype(self) -> np.dtype:
        ...
        for i, typ in enumerate(self.typlist):
            if typ in self.NUMPY_TYPE_MAP:
                ...
                dtypes.append(("s" + str(i), self.byteorder +
...
the string values returned are correct."""
    ...
    self.path_or_buf.read(4))[0]
            off = np.frombuffer(
                self.path_or_buf.read(4 * n), dtype=self.byteorder + "i4",
    count=n
            )
            val = np.frombuffer(
                self.path_or_buf.read(4 * n), dtype=self.byteorder + "i4",
    ...
    def read(
        ...
        if index_col is None:
            ...
            data.index = Index(rng)  # set attr instead of set_index to avoid
    copy
    ...
    def _do_convert_missing(self, data: DataFrame, convert_missing: bool) ->
    DataFrame:
    ...
    def _do_select_columns(self, data: DataFrame, columns: Sequence[str]) ->
    DataFrame:
...
"""
    ...
    def value_labels(self) -> dict[str, dict[float, str]]:
        """
        Return a nested dict associating each variable name to its value and
    label.
...
def _maybe_convert_to_int_keys(convert_dates: dict, varlist: list[Hashable])
    -> dict:
    new_dict = {}
    for key in convert_dates:
        ...
        if key in varlist:
            ...
            if not isinstance(key, int):
                raise ValueError("convert_dates key must be a column or an
    integer")
    ...
    if dtype.type is np.object_:
        ...
        if itemsize > max_str_len:
            if dta_version >= 117:
                ...
                raise
    ValueError(excessive_string_length_error.format(column.name))
...
class StataWriter(StataParser):
    ...
    def _set_formats_and_types(self, dtypes: Series) -> None:
        ...
        for col, dtype in dtypes.items():
            self.fmtlist.append(_dtype_to_default_stata_fmt(dtype,
    self.data[col]))
    ...
    def _prepare_pandas(self, data: DataFrame) -> None:
        ...
        for key in self._convert_dates:
            new_type =
    _convert_datetime_to_stata_type(self._convert_dates[key])
    ...
    def write_file(self) -> None:
        ...
        with get_handle(
            ...
            if self.handles.compression["method"] is not None:
                # ZipFile creates a file (with the same name) for each write
    call.
                # Write it first into a buffer and then write the buffer to
    the ZipFile.
                self._output_file, self.handles.handle = self.handles.handle,
    BytesIO()
            ...
            try:
                ...
                self.handles.close()
                if isinstance(self._fname, (str, os.PathLike)) and
    os.path.isfile(
                    self._fname
                ):
                    try:
                        ...
                        warnings.warn(
                            f"This save was not successful but {self._fname}
    could not "
    ...
    def _write_variable_labels(self) -> None:
        ...
        for col in self.data:
            if col in self._variable_labels:
                ...
                if len(label) > 80:
                    raise ValueError("Variable labels must be 80 characters or
    fewer")
...
def _dtype_to_stata_type_117(dtype: np.dtype, column: Series, force_strl:
    bool) -> int:
    """
...
class StataWriterUTF8(StataWriter117):
    ...
    Directly write a zip file
    >>> compression = {"method": "zip", "archive_name": "data_file.dta"}
    >>> writer = StataWriterUTF8('./data_file.zip', data,
    compression=compression)
    >>> writer.write_file()
