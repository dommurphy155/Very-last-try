...
def _ensure_term(where, scope_level: int):
    ...
    if isinstance(where, (list, tuple)):
        where = [
            Term(term, scope_level=level + 1) if maybe_expression(term) else
    term
            for term in where
            if term is not None
        ]
...
dropna_doc: Final = """: boolean
    drop ALL nan rows when appending to a table
"""
format_doc: Final = """: format
...
def read_hdf(
    ...
    **kwargs,
    ...
    path_or_buf: str, path object, pandas.HDFStore
        Any valid string path is acceptable. Only supports the local file
    system,
...
class HDFStore:
    ...
    **kwargs, the HDF5 file is loaded or created in-memory and will only be
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if complib is not None and complib not in tables.filters.all_complibs:
            raise ValueError(
                f"complib only supports {tables.filters.all_complibs}
    compression."
            )
    ...
    classname="Table")
        ...
        raise ValueError(
            f"`include` should be either 'pandas' or 'native' but is
    '{include}'"
        )
            ...
            if self._mode in ["a", "w"] and mode in ["r", "r+"]:
                ...
                if self.is_open:
                    raise PossibleDataLossError(
                        f"Re-opening the file [{self._path}] with mode
    [{self._mode}] "
    ...
    criteria.
        ...
        .. warning::
           ...
           Pandas uses PyTables for reading and writing HDF5 files, which
    allows
           serializing object-dtype data with pickle when using the "fixed"
    format.
           Loading pickled data received from untrusted sources can be unsafe.
        ...
        def func(_start, _stop, _where):
            return s.read(start=_start, stop=_stop, where=_where,
    columns=columns)
    ...
    format.
        ...
        for t, k in itertools.chain([(s, selector)], zip(tbls, keys)):
            ...
            if not t.is_table:
                raise TypeError(
                    f"object [{t.pathname}] is not a table, and cannot be used
    in all "
                    "select as multiple"
                )
            ...
            if nrows is None:
                ...
                raise ValueError("all tables must have exactly the same
    nrows!")
    ...
    def put(
        ...
        format: 'fixed(f)|table(t)', default is 'fixed'
            ...
            ``'fixed'``
                Fixed format.  Fast writing/reading. Not-appendable, nor
    searchable.
            ``'table'``
                Table format.  Write as a PyTables Table structure which may
    perform
    ...
    existing.
        data_columns: list of columns or True, default None
            List of columns to create as data columns, or True to use all
    columns.
            See `here
    ...
    def append(
        ...
        format: 'table' is the default
            Format to use when storing object in HDFStore.  Value can be one
    of:
    ...
    <https://pandas.pydata.org/pandas-docs/stable/user_guide/io.html#query-via-data-columns>`__.
        ...
        if columns is not None:
            raise TypeError(
                "columns is not a supported keyword in append, try
    data_columns"
            )
    ...
    is None"
        ...
        for k, v in d.items():
            ...
            filtered = (
                {key: value for (key, value) in min_itemsize.items() if key in
    v}
    ...
    **kwargs)
    ...
    def create_table_index(
        ...
        if not isinstance(s, Table):
            raise TypeError("cannot create table index on a Fixed format
    store")
        ...
        return [
            ...
            if (
                ...
                and (
                    ...
                    or (isinstance(g, _table_mod.table.Table) and g._v_name !=
    "table")
                )
            )
        ]

    def walk(self, where: str = "/") -> Iterator[tuple[str, list[str],
    list[str]]]:
        """
    ...
    new store.
        ...
        """
        new_store = HDFStore(
            file, mode=mode, complib=complib, complevel=complevel,
                ...
                for k in lkeys:
                    try:
                        ...
                        if s is not None:
                            ...
                            values.append(pprint_thing(s or "invalid_HDFStore
    node"))
                    except AssertionError:
                        # surface any assertion errors for e.g. debugging
                        raise
    ...
    errors=errors)
        if append:
            ...
            if not s.is_table or (s.is_table and format == "fixed" and
    s.is_exists):
        ...
        if not s.is_table and complib:
            raise ValueError("Compression not supported on Fixed format
    stores")
    ...
    def get_result(self, coordinates: bool = False):
        ...
        if self.chunksize is not None:
            if not isinstance(self.s, Table):
                raise TypeError("can only use an iterator or chunksize on a
    table")
...
class IndexCol:
    ...
    def __repr__(self) -> str:
        temp = tuple(
            map(pprint_thing, (self.name, self.cname, self.axis, self.pos,
    self.kind))
        )
        return ",".join(
            [
                f"{key}->{value}"
                for key, value in zip(["name", "cname", "axis", "pos",
    "kind"], temp)
            ]
        )
    ...
    def convert(
        ...
        if is_datetime64_dtype(values.dtype) or
    is_datetime64tz_dtype(values.dtype):
            ...
            factory = lambda x, **kwds: PeriodIndex(  # type:
    ignore[assignment]
                ordinal=x, **kwds
            )
...
class DataCol(IndexCol):
    ...
    def __repr__(self) -> str:
        temp = tuple(
            map(
                pprint_thing, (self.name, self.cname, self.dtype, self.kind,
    self.shape)
            )
        )
        return ",".join(
            [
                f"{key}->{value}"
                for key, value in zip(["name", "cname", "dtype", "kind",
    "shape"], temp)
            ]
        )
    ...
    def _get_atom(cls, values: ArrayLike) -> Col:
        ...
        """
        dtype = values.dtype
        # error: Item "ExtensionDtype" of "Union[ExtensionDtype, dtype[Any]]"
    has no
        # attribute "itemsize"
    ...
    def validate_attr(self, append) -> None:
        ...
        if append:
            ...
            if existing_fields is not None and existing_fields !=
    list(self.values):
    ...
    in table!")
            ...
            if existing_dtype is not None and existing_dtype != self.dtype:
                raise ValueError(
                    "appended items dtype do not match existing items dtype in
    table!"
                )
...
class Fixed:
    ...
    def version(self) -> tuple[int, int, int]:
        ...
        version = _ensure_decoded(getattr(self.group._v_attrs,
    "pandas_version", None))
        try:
    ...
    def pandas_type(self):
        return _ensure_decoded(getattr(self.group._v_attrs, "pandas_type",
    None))
...
class GenericFixed(Fixed):
    ...
    def validate_read(self, columns, where) -> None:
        ...
        if columns is not None:
            raise TypeError(
                ...
                "a Fixed format store. this store must be selected in its
    entirety"
            )
        if where is not None:
            raise TypeError(
                "cannot pass a where specification when reading "
                "from a Fixed format store. this store must be selected in its
    entirety"
            )
    ...
    def read_array(self, key: str, start: int | None = None, stop: int | None
    = None):
    ...
    def write_multi_index(self, key: str, index: MultiIndex) -> None:
        ...
        for i, (lev, level_codes, name) in enumerate(
            ...
            if is_extension_array_dtype(lev):
                raise NotImplementedError(
                    "Saving a MultiIndex with an extension dtype is not
    supported."
                )
    ...
    def write_array(
        ...
        if is_categorical_dtype(value.dtype):
            raise NotImplementedError(
                "Cannot store a category dtype in a HDF5 dataset that uses
    format="
    ...
    _tables().ObjectAtom())
            ...
            # error: Item "ExtensionArray" of "Union[Any, ExtensionArray]" has
    no
            # attribute "asi8"
...
class BlockManagerFixed(GenericFixed):
    ...
    def read(
        ...
        for i in range(self.nblocks):
            ...
            values = self.read_array(f"block{i}_values", start=_start,
    stop=_stop)
    ...
    def write(self, obj, **kwargs) -> None:  # type: ignore[override]
        ...
        for i, ax in enumerate(data.axes):
            if i == 0 and (not ax.is_unique):
                raise ValueError("Columns index has to be unique for fixed
    format")
...
class Table(Fixed):
    ...
    def validate_multiindex(
        ...
        try:
            ...
            raise ValueError(
                "duplicate names/columns in the multi-index when storing as a
    table"
            ) from err
    ...
    def queryables(self) -> dict[str, Any]:
        ...
        d3 = [
            (v.cname, v) for v in self.values_axes if v.name in
    set(self.data_columns)
        ]
    ...
    def get_attrs(self) -> None:
        ...
        self.values_axes = [a for a in self.indexables if not
    a.is_an_indexable]
    ...
    def validate_version(self, where=None) -> None:
        ...
        if where is not None:
            if self.is_old_version:
                ws = incompatibility_doc % ".".join([str(x) for x in
    self.version])
                warnings.warn(
                    ws,
                    IncompatibilityWarning,
    ...
    def indexables(self):
        ...
        def f(i, c):
            ...
            # TODO: figure out why these two versions of `meta` dont always
    match.
            #  meta = "category" if md is not None else None
        ...
        #  `c` below is a str.
        _indexables.extend([f(i, c) for i, c in
    enumerate(self.attrs.values_cols)])
    ...
    but "
                            "cannot be indexed when using table format. Either
    use "
                ...
                raise AttributeError(
                    ...
                    f"In order to read column {c} you must reload the
    dataframe \n"
                    f"into HDFStore and include {c} with the data_columns
    argument."
                )
    ...
    def _create_axes(
        ...
        validate: bool, default True
            Whether to validate the obj against an existing object already
    written.
        nan_rep:
    ...
    np.array(exist_axis)):
        ...
        blocks, blk_items = self._get_blocks_and_items(
            frame, table_exists, new_non_index_axes, self.values_axes,
    data_columns
        )
        ...
        for i, (blk, b_items) in enumerate(zip(blocks, blk_items)):
            ...
            if data_columns and len(b_items) == 1 and b_items[0] in
    data_columns:
                klass = DataIndexableCol
                name = b_items[0]
    ...
    non-str?
                    raise ValueError("cannot have non-object label
    DataIndexableCol")
    ...
    def _get_blocks_and_items(
        ...
        if len(data_columns):
            ...
            # error: Item "ArrayManager" of "Union[ArrayManager,
    BlockManager]" has no
            # attribute "blocks"
        ...
        if selection.filter is not None:
            for field, op, filt in selection.filter.format():
                ...
                def process_filter(field, filt):
                    ...
                    raise ValueError(f"cannot find the field [{field}] for
    filtering!")
...
class AppendableTable(Table):
    ...
    def write_data(self, chunksize: int | None, dropna: bool = False) -> None:
        """
        we form the data into a 2-d including indexes,values,mask write
    chunk-by-chunk
        """
    ...
    def delete(self, where=None, start: int | None = None, stop: int | None =
    None):
...
class AppendableFrameTable(AppendableTable):
    ...
    def read(
        ...
        inds = [i for i, ax in enumerate(self.axes) if ax is
    self.index_axes[0]]
        assert len(inds) == 1
        ind = inds[0]
        ...
        for i, a in enumerate(self.axes):
            ...
            if isinstance(values, np.ndarray):
                ...
                # Categorical
                df = DataFrame._from_arrays([values], columns=cols_,
    index=index_)
...
class AppendableMultiFrameTable(AppendableFrameTable):
    ...
    def read(
        ...
        df.index = df.index.set_names(
            [None if self._re_levels.search(name) else name for name in
    df.index.names]
        )
...
def _reindex_axis(obj: DataFrame, axis: int, labels: Index, other=None) ->
    DataFrame:
...
@overload
def _set_tz(values: np.ndarray | Index, tz: None, coerce: bool = False) ->
    np.ndarray:
    ...
...
def _convert_index(name: str, index: Index, encoding: str, errors: str) ->
    IndexCol:
    assert isinstance(name, str)

    index_name = index.name
    # error: Argument 1 to "_get_data_and_dtype_name" has incompatible type
    "Index";
    # expected "Union[ExtensionArray, ndarray]"
    converted, dtype_name = _get_data_and_dtype_name(index)  # type:
    ignore[arg-type]
    kind = _dtype_to_kind(dtype_name)
    ...
    if (
        ...
        # Includes Int64Index, RangeIndex, DatetimeIndex, TimedeltaIndex,
    PeriodIndex,
    ...
    if inferred_type == "date":
        ...
        return IndexCol(
            name, converted, "date", _tables().Time32Col(),
    index_name=index_name
        )
    elif inferred_type == "string":
...
def _unconvert_index(data, kind: str, encoding: str, errors: str) ->
    np.ndarray | Index:
...
def _maybe_convert_for_string_atom(
    ...
    if inferred_type != "string":
        ...
        for i in range(data.shape[0]):
            ...
            if inferred_type != "string":
                error_column_label = columns[i] if len(columns) > i else
    f"No.{i}"
                raise TypeError(
    ...
    if isinstance(min_itemsize, dict):
        min_itemsize = int(min_itemsize.get(name) or
    min_itemsize.get("values") or 0)
...
def _convert_string_array(data: np.ndarray, encoding: str, errors: str) ->
    np.ndarray:
    """
    Take a string-like that is object dtype and coerce to a fixed size string
    type.
...
def _maybe_convert(values: np.ndarray, val_kind: str, encoding: str, errors:
    str):
...
def _maybe_adjust_name(name: str, version: Sequence[int]) -> str:
    ...
    if isinstance(version, str) or len(version) < 3:
        raise ValueError("Version is incorrect, expected sequence of 3
    integers.")
            ...
            with suppress(ValueError):
                ...
                if inferred == "integer" or inferred == "boolean":
                    ...
                    if where.dtype == np.bool_:
                        ...
                        if (self.start is not None and (where <
    self.start).any()) or (
                            self.stop is not None and (where >=
    self.stop).any()
                        ):
                            raise ValueError(
    ...
    encoding=self.table.encoding)
            ...
            msg = dedent(
                ...
                The passed where expression: {where}
                            ...
                            all of the variable references must be a reference
    to
