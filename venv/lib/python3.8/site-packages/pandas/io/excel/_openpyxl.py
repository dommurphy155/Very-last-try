...
class OpenpyxlWriter(ExcelWriter):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ExcelWriter replaced "a" by "r+" to allow us to first read the excel
    file from
    ...
    @classmethod
    def _convert_to_style_kwargs(cls, style_dict: dict) -> dict[str,
    Serialisable]:
        """
    ...
    def _convert_to_border(cls, border_dict):
        ...
        _border_key_map = {"diagonalup": "diagonalUp", "diagonaldown":
    "diagonalDown"}
        ...
        if sheet_name in self.sheets and self._if_sheet_exists != "new":
            if "r+" in self._mode:
                if self._if_sheet_exists == "replace":
                    ...
                    raise ValueError(
                        f"'{self._if_sheet_exists}' is not valid for
    if_sheet_exists. "
                        "Valid options are 'error', 'new', 'replace' and
    'overlay'."
                    )
            else:
                wks = self.sheets[sheet_name]
...
class OpenpyxlReader(BaseExcelReader):
    ...
    def load_workbook(self, filepath_or_buffer: FilePath | ReadBuffer[bytes]):
        ...
        return load_workbook(
            filepath_or_buffer, read_only=True, data_only=True,
    keep_links=False
        )
    ...
    def get_sheet_data(
        ...
        for row_number, row in enumerate(sheet.rows):
            converted_row = [self._convert_cell(cell, convert_float) for cell
    in row]
