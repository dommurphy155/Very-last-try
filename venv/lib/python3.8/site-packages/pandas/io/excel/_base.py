...
thousands: str, default None
    ...
    format.
decimal: str, default '.'
    Character to recognize as decimal point for parsing string columns to
    numeric.
...
@deprecate_nonkeyword_arguments(allowed_args=["io", "sheet_name"],
    version="2.0")
...
class BaseExcelReader(metaclass=abc.ABCMeta):
    def __init__(
        ...
        if not isinstance(filepath_or_buffer, (ExcelFile,
    self._workbook_class)):
            self.handles = get_handle(
                filepath_or_buffer, "rb", storage_options=storage_options,
    is_text=False
            )
        ...
        if isinstance(self.handles.handle, self._workbook_class):
            ...
            raise ValueError(
                "Must explicitly set engine if not passing in buffer or path
    for io."
            )
    ...
    @abc.abstractmethod
    def get_sheet_data(self, sheet, convert_float: bool, rows: int | None =
    None):
        pass
    ...
    def raise_if_bad_sheet_by_index(self, index: int) -> None:
        ...
        if index >= n_sheets:
            raise ValueError(
                f"Worksheet index {index} is invalid, {n_sheets} worksheets
    found"
            )
    ...
    def _calc_rows(
        ...
        if is_list_like(skiprows):
            ...
            return self._check_skiprows_func(partial(f, skiprows), header_rows
    + nrows)
    ...
    def parse(
        ...
        **kwds,
        ...
        if convert_float is None:
            ...
            warnings.warn(
                "convert_float is deprecated and will be removed in a future
    version.",
                FutureWarning,
    ...
    nrows)
            ...
            if header is not None and is_list_like(header):
                ...
                for row in header:
                    ...
                    data[row], control_row = fill_mi_header(data[row],
    control_row)
            ...
            if is_list_header and not is_len_one_list_header and index_col is
    not None:
                ...
                # We have to handle mi without names. If any of the entries in
    the data
                ...
                if len(header) < len(data):
                    ...
                    has_index_names = all(x == "" or x is None for x in
    potential_data)
            ...
            if is_list_like(index_col):
                ...
                # GH34673: if MultiIndex names present and not defined in the
    header,
...
class ExcelWriter(metaclass=abc.ABCMeta):
    ...
    * `xlsxwriter <https://pypi.org/project/XlsxWriter/>`__ for xlsx files if
    xlsxwriter
    ...
    * `odswriter <https://pypi.org/project/odswriter/>`__ for ods files
    ...
    The writer should be used as a context manager. Otherwise, call `close()`
    to save
    ...
    if_sheet_exists: {{'error', 'new', 'replace', 'overlay'}}, default 'error'
        ...
        * overlay: Write contents to the existing sheet without removing the
    old
          contents.
    ...
    **kwargs: dict, optional
    ...
    ...         [datetime(1998, 5, 26, 23, 33, 4), datetime(2014, 2, 28, 13,
    5, 13)],
    ...     ],
    ...     index=["Date", "Datetime"],
    ...     columns=["X", "Y"],
    ...
    >>> with pd.ExcelWriter("path_to_file.xlsx", mode="a", engine="openpyxl")
    as writer:
    ...
    ...     df2.to_excel(writer, sheet_name="Sheet1", startcol=3)  # doctest:
    +SKIP
    ...
    silent=True)
                    ...
                    raise ValueError(f"No engine for filetype: '{ext}'") from
    err
    ...
    "
                        "This is the only engine in pandas that supports
    writing "
                        "in the xls format. Install openpyxl and write to an
    xlsx "
                        "file instead. You can set the option
    io.excel.xls.writer "
                        "to 'xlwt' to silence this warning. While this option
    is "
    ...
    def write_cells(
        ...
        return self._write_cells(cells, sheet_name, startrow, startcol,
    freeze_panes)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "b" not in mode:
            mode += "b"
        # use "a" for the user to append data to excel but internally use "r+"
    to let
    ...
    data to it
        ...
        if if_sheet_exists and "r+" not in mode:
            raise ValueError("if_sheet_exists is only valid in append mode
    (mode='a')")
    ...
    def if_sheet_exists(self) -> str:
        """
        How to behave when writing to a sheet that already exists in append
    mode.
        """
    ...
    def _get_sheet_name(self, sheet_name: str | None) -> str:
        ...
        if sheet_name is None:  # pragma: no cover
            raise ValueError("Must pass explicit sheet_name or set _cur_sheet
    property")
        return sheet_name
    ...
    cls._supported_extensions):
            raise ValueError(f"Invalid extension for engine '{cls.engine}':
    '{ext}'")
        else:
            return True
...
def inspect_excel_format(
    ...
    BadZipFile
        If resulting stream does not have an XLS signature and is not a valid
    zipfile.
    """
    ...
    with get_handle(
        ...
        with zipfile.ZipFile(stream) as zf:
            # Workaround for some third party files that use forward slashes
    and
            # lower case names.
            component_names = [
...
class ExcelFile:
    ...
    engine: str, default None
        ...
        .. versionchanged:: 1.2.0
           ...
           - If ``path_or_buffer`` is an OpenDocument format (.odf, .ods,
    .odt),
             then `odf <https://pypi.org/project/odfpy/>`_ will be used.
           - Otherwise if ``path_or_buffer`` is an xls format,
             ``xlrd`` will be used.
    ...
    installed,
           ...
           - Otherwise if ``xlrd >= 2.0`` is installed, a ``ValueError`` will
    be raised.
           - Otherwise ``xlrd`` will be used and a ``FutureWarning`` will be
    raised.
             This case will raise a ``ValueError`` in a future version of
    pandas.

           .. warning::

            Please do not report issues when using ``xlrd`` to read ``.xlsx``
    files.
    ...
    xlrd
            if ext and ext != "xls" and xlrd_version >= Version("2"):
                raise ValueError(
                    ...
                    f"only the xls format is supported. Install openpyxl
    instead."
                )
