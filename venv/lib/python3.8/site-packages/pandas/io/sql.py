...
def read_sql(
    ...
    The ``parse_dates`` argument calls ``pd.to_datetime`` on the provided
    columns.
    Custom argument values for applying ``pd.to_datetime`` on a column are
    specified
...
def to_sql(
    ...
    **engine_kwargs,
    ...
    method: {None, 'multi', callable}, optional
        ...
        - callable with signature ``(pd_table, conn, keys, data_iter) -> int |
    None``.
    ...
    **engine_kwargs
    ...
    Notes
    -----
    The returned rows affected is the sum of the ``rowcount`` attribute of
    ``sqlite3.Cursor``
    or SQLAlchemy connectable. The returned value may not reflect the exact
    number of written
...
def pandasSQL_builder(con, schema: str | None = None) -> SQLDatabase |
    SQLiteDatabase:
    """
...
class SQLTable(PandasObject):
    ...
    def create(self) -> None:
        if self.exists():
            if self.if_exists == "fail":
                ...
                raise ValueError(f"'{self.if_exists}' is not valid for
    if_exists")
        else:
            self._execute_create()
    ...
    from err
        ...
        # this just pre-allocates the list: None's will be replaced with
    ndarrays
    ...
    def _get_column_names_and_types(self, dtype_mapper):
        ...
        column_names_and_types += [
            (str(self.frame.columns[i]), dtype_mapper(self.frame.iloc[:, i]),
    False)
    ...
    def _sqlalchemy_type(self, col):
        ...
        if col_type == "timedelta64":
            ...
            if col.dtype.name.lower() in ("int8", "uint8", "int16"):
                ...
                raise ValueError("Unsigned 64 bit integer datatype is not
    supported")
            else:
                return BigInteger
    ...
    def insert_records(
        ...
        **engine_kwargs,
        ...
        try:
            ...
            msg = r"""(\(1054, "Unknown column 'inf(e0)?' in 'field
    list'"\))(?#
...
class SQLDatabase(PandasSQL):
    ...
    def read_query(
        ...
        parse_dates: list or dict, default: None
            ...
            - Dict of ``{column_name: arg dict}``, where the arg dict
              corresponds to the keyword arguments of:func:`pandas.to_datetime` Especially useful with databases
    ...
    def prep_table(
        ...
        """
        Prepares table in the database for data insertion. Creates it if
    needed, etc.
        """
        if dtype:
    ...
    incompatible
                ...
                dtype = {col_name: dtype for col_name in frame}  # type:
    ignore[misc]
            else:
                dtype = cast(dict, dtype)
            ...
            for col, my_type in dtype.items():
                if not isinstance(to_instance(my_type), TypeEngine):
                    raise ValueError(f"The type of {col} is not a SQLAlchemy
    type")
    ...
    def check_case_sensitive(
        ...
        if not name.isdigit() and not name.islower():
            ...
            with self.connectable.connect() as conn:
                ...
                table_names = insp.get_table_names(schema=schema or
    self.meta.schema)
    ...
    def get_table(self, table_name: str, schema: str | None = None) -> Table:
        ...
        tbl = Table(
            table_name, self.meta, autoload_with=self.connectable,
    schema=schema
        )
    ...
    def drop_table(self, table_name: str, schema: str | None = None) -> None:
        ...
        if self.has_table(table_name, schema):
            self.meta.reflect(bind=self.connectable, only=[table_name],
    schema=schema)
...
def _get_unicode_name(name):
    try:
        ...
        raise ValueError(f"Cannot convert identifier to UTF-8: '{name}'") from
    err
    return uname
...
class SQLiteTable(SQLTable):
    ...
    self._get_column_names_and_types(self._sql_type_name)
        ...
        create_tbl_stmts = [
            escape(cname) + " " + ctype for cname, ctype, _ in
    column_names_and_types
        ]
...
class SQLiteDatabase(PandasSQL):
    ...
    def has_table(self, name: str, schema: str | None = None) -> bool:
        ...
        wld = "?"
        query = f"SELECT name FROM sqlite_master WHERE type='table' AND
    name={wld};"
