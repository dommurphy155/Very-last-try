...
class TestReductions:
    ...
    def test_ops(self, opname, obj):
        ...
        if not isinstance(obj, PeriodIndex):
            ...
            expected = Period(ordinal=getattr(obj.asi8, opname)(),
    freq=obj.freq)
...
class TestIndexReductions:
    ...
    def test_timedelta_ops(self):
        ...
        s = Series(
            [Timestamp("20130101") + timedelta(seconds=i * i) for i in
    range(10)]
        )
        td = s.diff()
    ...
    ordered=False)
        msg = (
            ...
            r"you can use .as_ordered\(\) to change the Categorical to an
    ordered one\n"
        )
        with pytest.raises(TypeError, match=msg):
            ci.min()
...
class TestSeriesReductions:
    ...
    @pytest.mark.parametrize("dtype", ("m8[ns]", "m8[ns]", "M8[ns]", "M8[ns,
    UTC]"))
    ...
    def test_all_any_params(self):
        ...
        # Check level.
        s = Series([False, False, True, True, False, True], index=[0, 0, 1, 1,
    2, 2])
        with tm.assert_produces_warning(FutureWarning):
    ...
    @pytest.mark.parametrize("bool_agg_func", ["any", "all"])
    @pytest.mark.parametrize(
        "data", [[False, None], [None, False], [False, np.nan], [np.nan,
    False]]
    )
    ...
    @pytest.mark.parametrize("dtype", ["boolean", "Int64", "UInt64",
    "Float64"])
    @pytest.mark.parametrize("bool_agg_func", ["any", "all"])
    @pytest.mark.parametrize("skipna", [True, False])
    @pytest.mark.parametrize(
...
class TestCategoricalSeriesReductions:
    ...
    def test_min_max_skipna(self, function, skipna):
        cat = Series(
            Categorical(["a", "b", np.nan, "a"], categories=["b", "a"],
    ordered=True)
        )
        result = getattr(cat, function)(skipna=skipna)
...
class TestSeriesMode:
    ...
    @pytest.mark.parametrize(
        "dropna, expected",
        [(True, Series([], dtype=np.float64)), (False, Series([],
    dtype=np.float64))],
    )
    ...
    @pytest.mark.parametrize("dropna, expected", [(True, [1.0]), (False, [1,
    np.nan])])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                Categorical([np.nan, 3, 1], categories=[3, 2, 1],
    ordered=True),
            ),
        ],
    )
    def test_mode_category(self, dropna, expected1, expected2, expected3):
        ...
        s = Series(
            Categorical(
                [1, 1, 2, 3, 3, np.nan, np.nan], categories=[3, 2, 1],
    ordered=True
            )
        )
        result = s.mode(dropna)
