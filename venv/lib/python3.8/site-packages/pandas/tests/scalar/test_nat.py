...
@pytest.mark.parametrize("value", [None, np.nan, iNaT, float("nan"), NaT,
    "NaT", "nat"])
...
def test_missing_public_nat_methods(klass, expected):
    ...
    missing = [x for x in klass_names if x not in nat_names and not
    x.startswith("_")]
    missing.sort()
...
def _get_overlap_public_nat_methods(klass, as_tuple=False):
    ...
    overlap = [
        x
        for x in nat_names
        if x in klass_names and not x.startswith("_") and
...
def test_nat_arithmetic_scalar(op_name, value, val_type):
    ...
    if op_name in invalid_ops.get(val_type, set()):
        if (
            ...
            msg = rf"unsupported operand type\(s\) for \*: '{typs}' and
    '{typs}'"
        elif val_type == "str":
...
@pytest.mark.parametrize(
    "val,expected", [(np.nan, NaT), (NaT, np.nan), (np.timedelta64("NaT"),
    np.nan)]
)
...
@pytest.mark.parametrize("other", [np.timedelta64(0, "ns"),
...
@pytest.mark.parametrize("other_and_type", [("foo", "str"), (2, "int"), (2.0,
    "float")])
@pytest.mark.parametrize(
    "symbol_and_op",
...
def test_nat_comparisons_invalid(other_and_type, symbol_and_op):
    ...
    msg = f"'{symbol}' not supported between instances of 'NaTType' and
    '{other_type}'"
    with pytest.raises(TypeError, match=msg):
        op(NaT, other)

    msg = f"'{symbol}' not supported between instances of '{other_type}' and
    'NaTType'"
    with pytest.raises(TypeError, match=msg):
        op(other, NaT)
