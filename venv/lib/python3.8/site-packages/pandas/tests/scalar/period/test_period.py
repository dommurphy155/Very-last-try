...
class TestPeriodConstruction:
    ...
    def test_period_constructor_offsets(self):
        ...
        assert Period("2005", freq=offsets.YearEnd()) == Period("2005",
    freq="A")
        assert Period("2005", freq=offsets.MonthEnd()) == Period("2005",
    freq="M")
        assert Period("3/10/12", freq=offsets.BusinessDay()) == Period(
            "3/10/12", freq="B"
        )
        assert Period("3/10/12", freq=offsets.Day()) == Period("3/10/12",
    freq="D")
    ...
    Period(
        ...
        assert Period(year=2005, month=3, day=1, freq=offsets.Day(3)) ==
    expected
        assert Period(year=2005, month=3, day=1, freq="3D") == expected

        assert Period(year=2012, month=3, day=10, freq=offsets.BDay(3)) ==
    Period(
            year=2012, month=3, day=10, freq="3B"
        )

        assert Period(200701, freq=offsets.MonthEnd()) == Period(200701,
    freq="M")
    ...
    "])
    @pytest.mark.parametrize("hour", ["00:00:00", "00:00:01", "23:59:59",
    "12:00:59"])
    ...
    def test_period_constructor_nanosecond(self, day, hour, sec_float,
    expected):
        # GH 34621
...
class TestPeriodMethods:
    ...
    def test_hash(self):
        assert hash(Period("2011-01", freq="M")) == hash(Period("2011-01",
    freq="M"))

        assert hash(Period("2011-01-01", freq="D")) != hash(Period("2011-01",
    freq="M"))

        assert hash(Period("2011-01", freq="3M")) != hash(Period("2011-01",
    freq="2M"))

        assert hash(Period("2011-01", freq="M")) != hash(Period("2011-02",
    freq="M"))

    # --------------------------------------------------------------
    # to_timestamp

    @pytest.mark.parametrize("tzstr", ["Europe/Brussels", "Asia/Tokyo",
    "US/Pacific"])
    def test_to_timestamp_tz_arg(self, tzstr):
        ...
        exp = Timestamp(day=31, month=12, year=2005,
    ...
    @pytest.mark.parametrize(
        "tzstr",
        ["dateutil/Europe/Brussels", "dateutil/Asia/Tokyo",
    "dateutil/US/Pacific"],
    )
...
class TestPeriodProperties:
    ...
    @pytest.mark.parametrize("bound, offset", [(Timestamp.min, -1),
    (Timestamp.max, 1)])
    @pytest.mark.parametrize("period_property", ["start_time", "end_time"])
    def test_outer_bounds_start_and_end_time(self, bound, offset,
    period_property):
        # GH #13346
    ...
    (Timestamp.max, 1)])
    @pytest.mark.parametrize("period_property", ["start_time", "end_time"])
    def test_inner_bounds_start_and_end_time(self, bound, offset,
    period_property):
        # GH #13346
        period = TestPeriodProperties._period_constructor(bound, -offset)
    ...
    def test_properties_hourly(self):
        ...
        for h_date in [h_date1, h_date2]:
            ...
            assert (
                Period(freq="H", year=2012, month=2, day=1,
    ...
    minute=0)
        ...
        assert (
            Period(freq="D", year=2012, month=2, day=1, hour=0,
    ...
    def test_properties_secondly(self):
        ...
        assert (
            Period(
                freq="Min", year=2012, month=2, day=1, hour=0, minute=0,
    second=0
            ).days_in_month
            == 29
        )
...
class TestPeriodField:
    def test_get_period_field_array_raises_on_out_of_range(self):
        msg = "Buffer dtype mismatch, expected 'const int64_t' but got
    'double'"
        with pytest.raises(ValueError, match=msg):
...
class TestPeriodComparisons:
    ...
    def test_comparison_invalid_type(self):
        ...
        msg = f"not supported between instances of {int_or_per} and
    {int_or_per}"
...
class TestArithmetic:
    ...
    def test_sub_n_gt_1_offsets(self, offset, kwd_name, n, normalize):
        ...
        expected = Period(p2_d, freq=p2.freq.base) - Period(p1_d,
    freq=p1.freq.base)
    ...
    def test_add_offset(self):
        ...
        for freq in ["A", "2A", "3A"]:
            ...
            for o in [
                ...
                msg = "Input has different freq|Input cannot be converted to
    Period"
                with pytest.raises(IncompatibleFrequency, match=msg):
                    p + o
                with pytest.raises(IncompatibleFrequency, match=msg):
    ...
    def test_sub_offset(self):
        ...
        for freq in ["D", "2D", "3D"]:
            ...
            assert p - np.timedelta64(2, "D") == Period("2011-03-30",
    freq=freq)
            assert p - np.timedelta64(3600 * 24, "s") == Period("2011-03-31",
    freq=freq)
        ...
        for freq in ["H", "2H", "3H"]:
            ...
            assert p - np.timedelta64(3, "h") == Period("2011-04-01 06:00",
    freq=freq)
            assert p - np.timedelta64(3600, "s") == Period(
                "2011-04-01 08:00", freq=freq
            )
            assert p - timedelta(minutes=120) == Period("2011-04-01 07:00",
    freq=freq)
