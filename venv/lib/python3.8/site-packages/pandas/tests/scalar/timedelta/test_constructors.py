...
@pytest.mark.parametrize(
    "npdtype", [np.int64, np.int32, np.int16, np.float64, np.float32,
    np.float16]
)
...
def test_construction_out_of_bounds_td64(val, unit, name):
    ...
    assert td64.astype("m8[ns]").view("i8") < 0  # i.e. naive astype will be
    wrong
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (to_timedelta, "10s", "ms", (ValueError, "unit must not be
    specified")),
        (to_timedelta, ["1", 2, 3], "s", (ValueError, "unit must not be
    specified")),
    ],
)
