...
class TestAsUnit:
    ...
    def test_as_unit_overflows(self):
        ...
        us = 9223372800000000
        td = Timedelta._from_value_and_reso(us,
...
class TestTimedeltas:
    ...
    def test_conversion(self):
        ...
        for td in [Timedelta(10, unit="d"), Timedelta("1 days,
    10:11:12.012345")]:
            pydt = td.to_pytimedelta()
            assert td == Timedelta(pydt)
            assert td == pydt
            assert isinstance(pydt, timedelta) and not isinstance(pydt,
    Timedelta)
    ...
    def test_unit_parser(self, unit, np_unit, wrapper):
        ...
        # scalar
        expected = Timedelta(np.timedelta64(2,
    np_unit).astype("timedelta64[ns]"))
    ...
    def test_timedelta_conversions(self):
        assert Timedelta(timedelta(seconds=1)) == np.timedelta64(1,
    "s").astype(
            "m8[ns]"
        )
        assert Timedelta(timedelta(microseconds=1)) == np.timedelta64(1,
    "us").astype(
            "m8[ns]"
        )
        assert Timedelta(timedelta(days=1)) == np.timedelta64(1,
    "D").astype("m8[ns]")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("2S", Timedelta("1 days 02:34:56"), Timedelta("-1 days
    02:34:56")),
            ("5S", Timedelta("1 days 02:34:55"), Timedelta("-1 days
    02:34:55")),
            ("T", Timedelta("1 days 02:35:00"), Timedelta("-1 days 02:35:00")),
            ("12T", Timedelta("1 days 02:36:00"), Timedelta("-1 days
    02:36:00")),
    ...
    def test_round_implementation_bounds(self):
        ...
        msg = "|".join(
            ["Python int too large to convert to C long", "int too big to
    convert"]
        )
    ...
    def test_full_format_converters(self):
        ...
        assert Timedelta("06:00:01.0") == conv(np.timedelta64(6 * 3600 + 1,
    "s"))
        assert Timedelta("06:00:01.01") == conv(
            np.timedelta64(1000 * (6 * 3600 + 1) + 10, "ms")
        )

        assert Timedelta("- 1days, 00:00:01") == conv(-d1 + np.timedelta64(1,
    "s"))
    ...
    def test_implementation_limits(self):
        ...
        msg = "Cannot cast -9223372036854775809 from ns to 'ns' without
    overflow"
        with pytest.raises(OutOfBoundsTimedelta, match=msg):
            Timedelta(min_td.value - 2, "ns")

        msg = "Cannot cast 9223372036854775808 from ns to 'ns' without
    overflow"
        with pytest.raises(OutOfBoundsTimedelta, match=msg):
