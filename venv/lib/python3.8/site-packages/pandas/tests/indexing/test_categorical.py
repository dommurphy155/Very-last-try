...
def df():
    return DataFrame(
        ...
        index=CategoricalIndex(list("aabbca"), dtype=CDT(list("cab")),
    name="B"),
    )
...
class TestCategoricalIndex:
    ...
    def test_loc_setitem_expansion_label_unused_category(self, df2):
        ...
        df.loc["e"] = 20
        result = df.loc[["a", "b", "e"]]
        exp_index = CategoricalIndex(list("aaabbe"), categories=list("cabe"),
    name="B")
    ...
    def test_loc_listlike_dtypes_duplicated_categories_and_codes(self):
        ...
        res = df.loc[["a", "b"]]
        exp = DataFrame(
            {"A": [1, 3, 2], "B": [4, 6, 5]}, index=CategoricalIndex(["a",
    "a", "b"])
        )
        tm.assert_frame_equal(res, exp, check_index_type=True)
    ...
    def test_ix_categorical_index(self):
        # GH 12531
        df = DataFrame(np.random.randn(3, 3), index=list("ABC"),
    columns=list("XYZ"))
        cdf = df.copy()
    ...
    def test_ix_categorical_index_non_unique(self):
        ...
        # non-unique
        df = DataFrame(np.random.randn(3, 3), index=list("ABA"),
    columns=list("XYX"))
        cdf = df.copy()
        cdf.index = CategoricalIndex(df.index)
        cdf.columns = CategoricalIndex(df.columns)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [-1.5, -2.5, -3.5],
            # numpy int/uint
            *(np.array([1, 2, 3], dtype=dtype) for dtype in
    tm.ALL_INT_NUMPY_DTYPES),
            # numpy floats
            *(np.array([1.5, 2.5, 3.5], dtype=dtyp) for dtyp in
    tm.FLOAT_NUMPY_DTYPES),
            # numpy object
            np.array([1, "b", 3.5], dtype=object),
            # pandas scalars
            [Interval(1, 4), Interval(4, 6), Interval(6, 9)],
            [Timestamp(2019, 1, 1), Timestamp(2019, 2, 1), Timestamp(2019, 3,
    1)],
            [Timedelta(1, "d"), Timedelta(2, "d"), Timedelta(3, "D")],
            # pandas Integer arrays
            *(pd.array([1, 2, 3], dtype=dtype) for dtype in
