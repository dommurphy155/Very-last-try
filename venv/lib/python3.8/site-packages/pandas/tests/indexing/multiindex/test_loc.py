...
class TestMultiIndexLoc:
    def test_loc_setitem_frame_with_multiindex(self,
    ...
    def test_loc_getitem_general(self):
        ...
        index = MultiIndex.from_arrays([[4.0] * 3, [2012] * 3], names=["col",
    "year"])
    ...
    dtype=np.float64),
        ...
        # 0-dim array (scalar):
        scalar = np.int64(1)
        expected = Series(data=[0, 1, 2], index=["A", "B", "C"],
    dtype=np.float64)
        result = x.loc[scalar]
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize("key, pos", [([2, 4], [0, 1]), ([2], []), ([2,
    3], [])])
    ...
    def test_loc_multiindex_too_many_dims_raises(self):
        ...
        s = Series(
            range(8),
            index=MultiIndex.from_product([["a", "b"], ["c", "d"], ["e",
    "f"]]),
        )
...
def test_loc_getitem_duplicates_multiindex_empty_indexer(columns_indexer):
    # GH 8737
    # empty indexer
    multi_index = MultiIndex.from_product((["foo", "bar", "baz"], ["alpha",
    "beta"]))
...
def test_loc_getitem_duplicates_multiindex_non_scalar_type_object():
    ...
    df = DataFrame(
        ...
        columns=MultiIndex.from_tuples([("functs", "mean"), ("functs",
    "median")]),
        index=["function", "name"],
    )
...
def
    test_loc_getitem_int_raises_exception(frame_random_data_integer_multi_index):
    df = frame_random_data_integer_multi_index
...
def test_loc_datetime_mask_slicing():
    ...
    df = DataFrame(
        data=[[1, 2], [3, 4], [5, 6], [7, 6]], index=m_idx, columns=["C1",
    "C2"]
    )
    result = df.loc[(dt_idx[0], (df.index.get_level_values(1) >
    "2017-05-04")), "C1"]
    expected = Series(
        [3],
        name="C1",
...
def test_loc_with_mi_indexer():
    ...
    expected = DataFrame(
        [["a", 1], ["b", 1], ["c", 2]],
        index=MultiIndex.from_tuples([(0, 1), (1, 1), (1, 1)], names=["index",
    "date"]),
        columns=["author", "price"],
    )
    tm.assert_frame_equal(result, expected)
...
def
    test_getitem_loc_commutability(multiindex_year_month_day_dataframe_random_data):
...
def test_loc_with_nan():
    ...
    result = df.loc[["a"]]
    expected = DataFrame(
        {"col": [1]}, index=MultiIndex.from_tuples([("a", 1)], names=["ind1",
    "ind2"])
    )
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize("indexer", [[1, 2, 7, 6, 2, 3, 8, 7], [1, 2, 7, 6, 3,
    8]])
...
def test_mi_columns_loc_list_label_order():
    ...
    expected = DataFrame(
        np.zeros((5, 4)),
        columns=MultiIndex.from_tuples([("B", 1), ("B", 2), ("A", 1), ("A",
    2)]),
    )
    tm.assert_frame_equal(result, expected)
...
def test_loc_get_scalar_casting_to_float():
    # GH#41369
    df = DataFrame(
        {"a": 1.0, "b": 2}, index=MultiIndex.from_arrays([[3], [4]],
    names=["c", "d"])
    )
    result = df.loc[(3, 4), "b"]
    assert result == 2
