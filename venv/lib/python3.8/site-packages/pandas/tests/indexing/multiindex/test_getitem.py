...
@pytest.mark.parametrize(
    "access_method",
    [lambda s, x: s[:, x], lambda s, x: s.loc[:, x], lambda s, x: s.xs(x,
    level=1)],
)
@pytest.mark.parametrize(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda s: s.__getitem__(len(s)), KeyError, ""),  # match should
    include len(s)
    ...
    [
        (
            ...
            (
                DataFrame(
                    [[2, 3], [5, 6]],
                    columns=MultiIndex.from_tuples([("b", "bar"), ("b",
    np.nan)]),
                    dtype="int64",
                )
            ),
...
def dataframe_with_duplicate_index():
    ...
    data = [["a", "d", "e", "c", "f", "b"], [1, 4, 5, 3, 6, 2], [1, 4, 5, 3,
    6, 2]]
    index = ["h1", "h3", "h5"]
    columns = MultiIndex(
...
def test_frame_mi_empty_slice():
    # GH 15454
    df = DataFrame(0, index=range(2), columns=MultiIndex.from_product([[1],
    [2]]))
    result = df[[]]
    expected = DataFrame(
