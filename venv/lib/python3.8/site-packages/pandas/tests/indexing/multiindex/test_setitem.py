...
class TestMultiIndexSetItem:
    def check(self, target, indexers, value, compare_fn=assert_equal,
    expected=None):
    ...
    def test_setitem_multiindex3(self):
        ...
        idx = MultiIndex.from_product(
            [["A", "B", "C"], date_range("2015-01-01", "2015-04-01",
    freq="MS")]
        )
        ...
        subcols = MultiIndex.from_tuples(
            [("foo", Timestamp("2016-01-01")), ("foo",
    Timestamp("2016-02-01"))]
        )

        vals = DataFrame(np.random.random((2, 2)), index=subidx,
    columns=subcols)
        self.check(
            target=df,
    ...
    # TODO(ArrayManager) df.loc["bar"] *= 2 doesn't raise an error but results
    in
    ...
    def test_multiindex_setitem(self):
        ...
        msg = "cannot align on a multi-index with out specifying the join
    levels"
    ...
    def test_multiindex_assignment(self):
        ...
        df.loc[4, "d"] = arr
        tm.assert_series_equal(df.loc[4, "d"], Series(arr, index=[8, 10],
    name="d"))
    ...
    def test_groupby_example(self):
        ...
        NUM_ROWS = 100
        NUM_COLS = 10
        col_names = ["A" + num for num in map(str,
    ...
    def test_frame_setitem_multi_column(self):
        df = DataFrame(
            np.random.randn(10, 4), columns=[["a", "a", "b", "b"], [0, 1, 0,
    1]]
        )
    ...
    def test_loc_getitem_setitem_slice_integers(self, frame_or_series):
        index = MultiIndex(
            levels=[[0, 1, 2], [0, 2]], codes=[[0, 0, 1, 1, 2, 2], [0, 1, 0,
    1, 0, 1]]
        )

        obj = DataFrame(
            np.random.randn(len(index), 4), index=index, columns=["a", "b",
    "c", "d"]
        )
    ...
    def test_astype_assignment_with_dups(self):
        ...
        df = DataFrame(np.arange(3).reshape((1, 3)), columns=cols,
    dtype=object)
        index = df.index.copy()
...
class TestSetitemWithExpansionMultiIndex:
    ...
    def test_setitem_new_column_all_na(self):
        # GH#1534
        mix = MultiIndex.from_tuples([("1a", "2a"), ("1a", "2b"), ("1a",
    "2c")])
...
@td.skip_array_manager_invalid_test  # df["foo"] select multiple columns ->
    .values
# is not a view
