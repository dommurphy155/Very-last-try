...
class TestFancy:
    ...
    iterable"
        with pytest.raises(ValueError, match=msg):
            df.loc[df.index[2:5], "bar"] = np.array([2.33j, 1.23 + 0.1j, 2.2,
    1.0])
    ...
    def test_getitem_ndarray_3d(
        ...
        if frame_or_series is Series and indexer_sli in [tm.setitem, tm.iloc]:
            msgs.append(r"Wrong number of dimensions. values.ndim > ndim \[3 >
    1\]")
            if using_array_manager:
                msgs.append("Passed array should be 1-dimensional")
        if frame_or_series is Series or indexer_sli is tm.iloc:
            msgs.append(r"Buffer has wrong number of dimensions \(expected 1,
    got 3\)")
            if using_array_manager:
        ...
        if isinstance(index, pd.IntervalIndex) and indexer_sli is tm.iloc:
            msgs.append("Index data must be 1-dimensional")
        if isinstance(index, (pd.TimedeltaIndex, pd.DatetimeIndex,
    pd.PeriodIndex)):
    ...
    def test_setitem_ndarray_3d(self, index, frame_or_series, indexer_sli):
        ...
        if indexer_sli is tm.iloc:
            ...
            msg = "|".join(
                [
                    r"Buffer has wrong number of dimensions \(expected 1, got
    3\)",
    ...
    def test_dups_fancy_indexing_not_in_order(self):
        ...
        df = DataFrame(
            {"test": [5, 7, 9, 11], "test1": [4.0, 5, 6, 7], "other":
    list("abcd")},
            index=["A", "A", "B", "C"],
        )
        rows = ["C", "B"]
    ...
    def test_dups_fancy_indexing2(self):
        ...
        df = DataFrame(np.random.randn(5, 5), columns=["A", "B", "B", "B",
    "A"])
    ...
    def test_dups_fancy_indexing3(self):
        ...
        df = DataFrame(
            np.random.randn(9, 2), index=[1, 1, 1, 2, 2, 2, 3, 3, 3],
    columns=["a", "b"]
        )
    ...
    def test_indexing_mixed_frame_bug(self):
        ...
        # GH3492
        df = DataFrame(
            {"a": {1: "aaa", 2: "bbb", 3: "ccc"}, "b": {1: 111, 2: 222, 3:
    333}}
        )
    ...
    def test_multitype_list_index_access(self):
        # GH 10610
        df = DataFrame(np.random.random((10, 5)), columns=["a"] + [20, 21, 22,
    23])

        with pytest.raises(KeyError, match=re.escape("'[26, -8] not in
    index'")):
            df[[22, 26, -8]]
    ...
    def test_astype_assignment(self):
        ...
        df = df_orig.copy()
        with tm.assert_produces_warning(DeprecationWarning, match=msg):
            df.iloc[:, 0:2] = df.iloc[:, 0:2]._convert(datetime=True,
    numeric=True)
        expected = DataFrame(
            [[1, 2, "3", ".4", 5, 6.0, "foo"]], columns=list("ABCDEFG")
        )
...
class TestMisc:
    ...
    @pytest.mark.parametrize(
        "idx", [_mklbl("A", 20), np.arange(20) + 100, np.linspace(100, 150,
    20)]
    )
    ...
    def test_slice_with_zero_step_raises(self, index, indexer_sl,
    frame_or_series):
    ...
    def test_loc_setitem_indexing_assignment_dict_already_exists(self):
        ...
        df.loc[5] = rhs
        expected = DataFrame({"x": [1, 2, 9], "y": [2.0, 2.0, 99.0]},
    index=index)
        tm.assert_frame_equal(df, expected)
    ...
    def test_partial_boolean_frame_indexing(self):
        # GH 17170
        df = DataFrame(
            np.arange(9.0).reshape(3, 3), index=list("abc"),
    columns=list("ABC")
        )
...
class TestDataframeNoneCoercion:
    ...
    def test_coercion_with_setitem_and_dataframe(self, expected):
        ...
        start_dataframe = DataFrame({"foo": start_data})
        start_dataframe[start_dataframe["foo"] == start_dataframe["foo"][0]] =
    None
        ...
        start_dataframe = DataFrame(
            {
                "a": [1, 2, 3],
                "b": [1.0, 2.0, 3.0],
                "c": [datetime(2000, 1, 1), datetime(2000, 1, 2),
    datetime(2000, 1, 3)],
                "d": ["a", "b", "c"],
            }
        )
...
class TestDatetimelikeCoercion:
    ...
    @pytest.mark.parametrize("box", [list, np.array, pd.array, pd.Categorical,
    Index])
    @pytest.mark.parametrize(
...
def test_extension_array_cross_section():
    ...
    expected = Series(pd.array([1, 3], dtype="Int64"), index=["A", "B"],
    name="a")
    result = df.loc["a"]
    tm.assert_series_equal(result, expected)
...
def test_ser_list_indexer_exceeds_dimensions(indexer_li):
    # GH#13831
    # Make sure an exception is raised when a tuple exceeds the dimension of
    the series,
...
def test_scalar_setitem_with_nested_value(value):
    ...
    df = DataFrame({"A": [1, 2, 3], "B": np.array([1, "a", "b"],
    dtype=object)})
