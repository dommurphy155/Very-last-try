...
class TestFloatIndexers:
    ...
    @pytest.mark.parametrize("index_func", [tm.makeIntIndex,
    tm.makeRangeIndex])
    ...
    @pytest.mark.parametrize("idx", [slice(3.0, 4), slice(3, 4.0), slice(3.0,
    4.0)])
    ...
    indexer_sli):
        ...
        if indexer_sli is tm.iloc:
            msg = (
                ...
                rf"on {type(index).__name__} with these indexers \[(3|4)\.0\]
    of "
                "type float"
            )
        else:
    ...
    @pytest.mark.parametrize("idx", [slice(2, 4.0), slice(2.0, 4), slice(2.0,
    4.0)])
    ...
    def test_slice_integer_frame_getitem(self, index_func):
        ...
        for idx in [slice(0.0, 1), slice(0, 1.0), slice(0.0, 1.0)]:
            ...
            msg = (
                "cannot do slice indexing "
                rf"on {type(index).__name__} with these indexers \[(0|1)\.0\]
    of "
                "type float"
            )
            with pytest.raises(TypeError, match=msg):
    ...
    @pytest.mark.parametrize("index_func", [tm.makeIntIndex,
    tm.makeRangeIndex])
    def test_float_slice_getitem_with_integer_index_raises(self, idx,
    index_func):
