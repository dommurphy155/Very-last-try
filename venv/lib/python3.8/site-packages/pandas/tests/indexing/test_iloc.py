...
class TestiLocBaseIndependent:
    ...
    column_vals):
        # GH 25753
        df = DataFrame(
            np.random.randn(len(index), len(columns)), index=index,
    columns=columns
        )
    ...
    def test_iloc_getitem_array(self):
        ...
        expected = DataFrame([{"A": 1, "B": 2, "C": 3}, {"A": 100, "B": 200,
    "C": 300}])
        tm.assert_frame_equal(df.iloc[[0, 1]], expected)

        expected = DataFrame([{"B": 2, "C": 3}, {"B": 2000, "C": 3000}],
    index=[0, 2])
    ...
    def test_iloc_getitem_slice_dups(self):
        ...
        df2 = DataFrame(
            np.random.randint(0, 10, size=20).reshape(10, 2), columns=["A",
    "C"]
        )
    ...
    def test_iloc_setitem(self):
        df = DataFrame(
            np.random.randn(4, 4), index=np.arange(0, 8, 2),
    ...
    def test_iloc_setitem_list(self):
        ...
        df = DataFrame(
            np.arange(9).reshape((3, 3)), index=["A", "B", "C"], columns=["A",
    "B", "C"]
        )
        df.iloc[[0, 1], [1, 2]]
        df.iloc[[0, 1], [1, 2]] += 100
    ...
    def test_iloc_getitem_frame(self):
        df = DataFrame(
            np.random.randn(10, 4), index=range(0, 20, 2), columns=range(0, 8,
    2)
        )
    ...
    def test_iloc_getitem_labelled_frame(self):
        ...
        df = DataFrame(
            np.random.randn(10, 4), index=list("abcdefghij"),
    columns=list("ABCD")
        )
    ...
    def test_iloc_setitem_list_of_lists(self):
        ...
        df = DataFrame(
            {"A": np.arange(5, dtype="int64"), "B": np.arange(5, 10,
    dtype="int64")}
        )
        ...
        df = DataFrame(
            {"A": ["a", "b", "c", "d", "e"], "B": np.arange(5, 10,
    dtype="int64")}
        )
        df.iloc[2:4] = [["x", 11], ["y", 13]]
        expected = DataFrame({"A": ["a", "b", "x", "y", "e"], "B": [5, 6, 11,
    13, 9]})
        tm.assert_frame_equal(df, expected)

    @pytest.mark.parametrize("indexer", [[0], slice(None, 1, None),
    np.array([0])])
    ...
    def test_iloc_mask(self):
        ...
        mask = df.a % 2 == 0
        msg = "iLocation based boolean indexing cannot use an indexable as a
    mask"
        with pytest.raises(ValueError, match=msg):
            df.iloc[mask]
    ...
    available"
        ...
        expected = {
            ...
            ("index", ".loc"): "0b11",
            ("index", ".iloc"): (
                "iLocation based boolean indexing cannot use an indexable as a
    mask"
            ),
        ...
        with catch_warnings(record=True):
            ...
            for idx in [None, "index", "locs"]:
                ...
                for method in ["", ".loc", ".iloc"]:
                    ...
                    except (ValueError, IndexingError, NotImplementedError) as
    e:
                        answer = str(e)
                    ...
                    if r != answer:
                        raise AssertionError(
                            f"[{key}] does not match [{answer}], received
    [{r}]"
                        )
    ...
    def test_iloc_empty_list_indexer_is_ok(self):
        ...
        # horizontal empty
        tm.assert_frame_equal(
            df.iloc[[]], df.iloc[:0,:], check_index_type=True,
    check_column_type=True
        )
    ...
    dtype=dtype
        ...
        exp = Series([nulls_fixture, not_na, nulls_fixture, nulls_fixture],
    dtype=dtype)
        tm.assert_series_equal(ser, exp)

    def test_iloc_setitem_empty_frame_raises_with_3d_ndarray(self):
        idx = Index([])
        obj = DataFrame(np.random.randn(len(idx), len(idx)), index=idx,
    columns=idx)
    ...
    def test_iloc_getitem_read_only_values(self, indexer):
        ...
        tm.assert_frame_equal(indexer(rw_df)[[1, 2, 3]], indexer(ro_df)[[1, 2,
    3]])
    ...
    def test_iloc_assign_series_to_df_cell(self):
        ...
        expected = DataFrame({"a": [Series([1, 2, 3])]}, columns=["a"],
    index=[0])
        tm.assert_frame_equal(df, expected)
    ...
    def test_iloc_setitem_pure_position_based(self, indexer):
        ...
        expected = DataFrame({"a": [1, 2, 3], "b": [11, 12, 13], "c": [7, 8,
    9]})
    ...
    def test_iloc_setitem_2d_ndarray_into_ea_block(self):
        ...
        expected = DataFrame({"status": ["a", "a", "c"]},
    ...
    def test_iloc_setitem_multicolumn_to_datetime(self, using_array_manager):
        ...
        # GH#20511
        df = DataFrame({"A": ["2022-01-01", "2022-01-02"], "B": ["2021",
    "2022"]})
...
class TestILocErrors:
    ...
    def test_iloc_float_raises(self, series_with_simple_index,
    frame_or_series):
        # GH#4892
...
class TestILocSetItemDuplicateColumns:
    ...
    def test_iloc_setitem_series_duplicate_columns(self):
        df = DataFrame(
            np.arange(8, dtype=np.int64).reshape(2, 4), columns=["A", "B",
    "A", "B"]
        )
