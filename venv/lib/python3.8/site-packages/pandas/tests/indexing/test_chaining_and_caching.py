...
class TestCaching:
    ...
    def test_setitem_cache_updating(self, do_ref):
        ...
        df = DataFrame({"a": cont, "b": cont[3:] + cont[:3], "c":
    np.arange(7)})
    ...
    def test_setitem_cache_updating_slices(self, using_copy_on_write):
        ...
        out = DataFrame({"A": [0, 0, 0]}, index=date_range("5/7/2014",
    "5/9/2014"))
...
class TestChaining:
    ...
    def test_setting_with_copy_bug(self, using_copy_on_write):
        ...
        # operating on a copy
        df = DataFrame(
            {"a": list(range(4)), "b": list("ab.."), "c": ["a", "b", np.nan,
    "d"]}
        )
        df_original = df.copy()
        mask = pd.isna(df.c)
    ...
    def test_setting_with_copy_bug_no_warning(self):
        ...
        # GH 8730
        df1 = DataFrame({"x": Series(["a", "b", "c"]), "y": Series(["d", "e",
    "f"])})
        df2 = df1[["x"]]
