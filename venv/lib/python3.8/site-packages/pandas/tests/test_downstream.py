...
def test_dask_ufunc():
    # At the time of dask 2022.01.0, dask is still directly using
    __array_wrap__
    ...
    subprocess.check_call(
        [
            ...
            (
                ...
                "pickle.loads(pickle.dumps(pd.date_range('2021-01-01',
    periods=1)))"
            ),
        ]
    )
...
def test_geopandas():
    ...
    gdf = geopandas.GeoDataFrame(
        {"col": [1, 2, 3], "geometry": geopandas.points_from_xy([1, 2, 3], [1,
    2, 3])}
    )
...
@pytest.mark.filterwarnings("ignore:RangeIndex.* is
