...
def test_regression_allowlist_methods(raw_frame, op, level, axis, skipna,
    sort):
    # GH6944
    # GH 17537
...
def test_groupby_blocklist(df_letters):
    ...
    for bl in blocklist:
        for obj in (df, s):
            ...
            # e.g., query, eval
            not_defined = (
                f"(?:^'{type(gb).__name__}' object has no attribute
    {repr(bl)}$)"
            )
...
def test_groupby_selection_other_methods(df):
    ...
    tm.assert_frame_equal(g.apply(lambda x: x.sum()), g_exp.apply(lambda x:
    x.sum()))
...
how to fix this test.
    ...
    all_categorized = reduction_kernels | transformation_kernels |
    groupby_other_methods
    print(names)
...
They're still defined in one of the lists that live in
