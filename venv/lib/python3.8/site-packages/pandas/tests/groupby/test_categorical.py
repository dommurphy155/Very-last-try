    ...
    def f(a):
        if isinstance(a, (CategoricalIndex, Categorical)):
            ...
            a = Categorical.from_codes(
                np.arange(len(categories)), categories=categories,
    ordered=a.ordered
            )
        return a
...
def test_basic():  # TODO: split this test
    ...
    tm.assert_frame_equal(df.groupby(c, observed=False).transform(sum),
    df[["a"]])
...
def test_apply(ordered):
    ...
    #  is coming back as Series([0., 1., 0.], index=["missing", "dense",
    "values"])
...
def test_observed(observed):
    ...
    expected = DataFrame({"values": Series([1, 2, 3, 4],
    index=exp_index)}).sort_index()
    result = gb.sum()
    if not observed:
    ...
    expected = DataFrame({"ints": [1.5, 1.5], "val": [20.0, 30]},
    index=exp_index)
    if not observed:
...
def test_unstack_categorical():
    ...
    gcat = df.groupby(["artist", "medium"],
    observed=False)["a"].count().unstack()
...
def test_bins_unequal_len():
    ...
    with pytest.raises(ValueError, match="Grouper and axis must be same
    length"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Group a series with length equal to that of the grouper and index
    unequal to
        # that of the grouper.
        (Series(range(4)).rename(lambda idx: idx + 1), {"A": [2], "B": [0,
    1]}),
...
def test_categorical_series(series, data):
    # Group the given series by a series with categorical data type such that
    group A
    # takes indices 0 and 3 and group B indices 1 and 2, obtaining the values
    mapped in
    # the given data.
    ...
    df = DataFrame(
        {
            "A": [1, 2, 1, 1, 2],
            "B": [10, 16, 22, 28, 34],
            "C1": Categorical(list("abaab"), categories=list("bac"),
    ordered=False),
            "C2": Categorical(list("abaab"), categories=list("bac"),
    ordered=True),
        }
    )
    # single grouper
    exp_full = DataFrame(
        {
            "A": [2.0, 1.0, np.nan],
            "B": [25.0, 20.0, np.nan],
            "C1": Categorical(list("bac"), categories=list("bac"),
    ordered=False),
            "C2": Categorical(list("bac"), categories=list("bac"),
    ordered=True),
        }
    )
    for col in ["C1", "C2"]:
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            ...
            result2 = (
                df.groupby(by=col, as_index=True,
...
def test_sort2():
    ...
    expected_nosort = DataFrame(
        [[10, 10], [5, 30], [6, 40], [1, 60]], index=index, columns=["foo",
    "bar"]
    )
...
def test_empty_sum():
    ...
    df = DataFrame(
        {"A": Categorical(["a", "a", "b"], categories=["a", "b", "c"]), "B":
    [1, 2, 1]}
    )
...
def test_groupby_agg_observed_true_single_column(as_index, expected):
    # GH-23970
    df = DataFrame(
        {"a": Series([1, 1, 2], dtype="category"), "b": [1, 2, 2], "x": [1, 2,
    3]}
    )
...
def test_seriesgroupby_observed_true(df_cat, operation):
    # GH 24880
    lev_a = Index(["foo", "foo", "bar", "bar"], dtype=df_cat["A"].dtype,
    name="A")
    lev_b = Index(["one", "two", "one", "three"], dtype=df_cat["B"].dtype,
    name="B")
    ...
    [
        (
            ...
            MultiIndex.from_arrays(
                [
                    Index(["foo"] * 4 + ["bar"] * 4, dtype="category",
    name="A"),
                    Index(
                        ["one", "one", "two", "two", "one", "one", "three",
    "three"],
                        dtype="category",
                        name="B",
                    ),
...
def test_groupby_categorical_axis_1(code):
    # GH 13420
    df = DataFrame({"a": [1, 2, 3, 4], "b": [-1, -2, -3, -4], "c": [5, 6, 7,
    8]})
...
def test_groupby_cat_preserves_structure(observed, ordered):
    # GH 28787
    df = DataFrame(
        {"Name": Categorical(["Bob", "Greg"], ordered=ordered), "Item": [1,
    2]},
        columns=["Name", "Item"],
    )
    expected = df.copy()
...
def test_series_groupby_on_2_categoricals_unobserved_zeroes_or_nans(
    ...
    unobserved = [tuple("AC"), tuple("BC"), tuple("CA"), tuple("CB"),
    tuple("CC")]
    args = get_groupby_method_args(reduction_func, df)
    ...
    # If we expect unobserved values to be zero, we also expect the dtype to
    be int.
    # Except for .sum(). If the observed categories sum to dtype=float (i.e.
    their
    # sums have decimals), then the zeros for the missing categories should
    also be
    # floats.
...
def
    test_dataframe_groupby_on_2_categoricals_when_observed_is_true(reduction_func):
    # GH 23865
    # GH 27075
...
def test_series_groupby_categorical_aggregation_getitem():
    # GH 8870
    d = {"foo": [10, 8, 4, 1], "bar": [10, 20, 30, 40], "baz": ["d", "c", "d",
    "c"]}
    df = DataFrame(d)
...
def test_groupby_first_returned_categorical_instead_of_dataframe(func):
    ...
    df = DataFrame({"A": [1997], "B": Series(["b"],
...
def test_read_only_category_no_sort():
    ...
    df = DataFrame(
        {"a": [1, 3, 5, 7], "b": Categorical([1, 1, 2, 2],
...
def test_categorical_transform():
    ...
    expected["last_status"].astype(delivery_status_type)
...
def test_groupby_categorical_observed_nunique():
    ...
    expected = Series(
        [1, 1],
        index=MultiIndex.from_arrays(
            [CategoricalIndex([1, 2], name="a"), CategoricalIndex([1, 2],
    name="b")]
        ),
        name="c",
    )
...
def test_groupby_categorical_aggregate_functions():
    ...
    df = DataFrame(
        [[1, "small"], [1, "big"], [2, "small"]], columns=["grp",
    "description"]
