...
class TestSelection:
    ...
    def test_groupby_duplicated_column_errormsg(self):
        # GH7511
        df = DataFrame(
            columns=["A", "B", "A", "C"], data=[range(4), range(2, 6),
    range(0, 8, 2)]
        )
    ...
    def test_indices_grouped_by_tuple_with_lambda(self):
        # GH 36158
        df = DataFrame(
            {"Tuples": ((x, y) for x in [0, 1] for y in np.random.randint(3,
    5, 5))}
        )
...
class TestGrouping:
    ...
    def test_grouper_multilevel_freq(self):
        ...
        date_index = MultiIndex.from_product([dates, dates], names=["foo",
    "bar"])
        ...
        expected = (
            df.reset_index()
            .groupby([pd.Grouper(key="foo", freq="W"), pd.Grouper(key="bar",
    freq="W")])
            .sum()
        )
        ...
        result = df.groupby(
            [pd.Grouper(level="foo", freq="W"), pd.Grouper(level="bar",
    freq="W")]
        ).sum()
        tm.assert_frame_equal(result, expected)
    ...
    def test_grouper_creation_bug(self):
        ...
        df = DataFrame(
            {"A": [0, 0, 0, 1, 1, 1], "B": [1, 1, 2, 2, 3, 3], "C": [1, 2, 3,
    4, 5, 6]}
        )
        ...
        expected = Series(
            [28],
            index=pd.DatetimeIndex([Timestamp("2013-01-31")], freq="M",
    name="three"),
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_grouper_column_and_index(self):
        ...
        df_multi = DataFrame(
            {"A": np.arange(6), "B": ["one", "one", "two", "two", "one",
    "one"]},
            index=idx,
        )
        msg = "The default value of numeric_only"
    ...
    def test_groupby_levels_and_columns(self):
        # GH9344, GH9049
        idx_names = ["x", "y"]
        idx = MultiIndex.from_tuples([(1, 1), (1, 2), (3, 4), (5, 6)],
    names=idx_names)
    ...
    2]], int
        ...
        cat_columns = CategoricalIndex(columns, categories=categories,
    ordered=True)
    ...
    def test_grouping_error_on_multidim_input(self, df):
        msg = "Grouper for '<class 'pandas.core.frame.DataFrame'>' not
    1-dimensional"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_groupby_multiindex_tuple(self):
        ...
        df = DataFrame(
            ...
            columns=MultiIndex.from_arrays([["a", "b", "b", "c"], [1, 1, 2,
    2]]),
        )
        ...
        df3 = DataFrame(df.values, columns=[("a", "d"), ("b", "d"), ("b",
    "e"), "c"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "agg",
                Series(name=2, dtype=np.float64, index=Float64Index([],
    name=1)),
            ),
            (
                "apply",
    ...
    def test_groupby_level_index_value_all_na(self):
        ...
        expected = DataFrame(
            data=[],
            index=MultiIndex(
                levels=[Index(["x"], dtype="object"), Index([],
    dtype="float64")],
                codes=[[], []],
                names=["A", "B"],
            ),
    ...
    def test_groupby_multiindex_level_empty(self):
        ...
        df = DataFrame(
            [[123, "a", 1.0], [123, "b", 2.0]], columns=["id", "category",
    "value"]
        )
...
class TestGetGroup:
    ...
    def test_gb_key_len_equal_axis_len(self):
        ...
        df = DataFrame(
            [["foo", "bar", "B", 1], ["foo", "bar", "B", 2], ["foo", "baz",
    "C", 3]],
...
class TestIteration:
    ...
    def test_multi_iter_frame(self, three_group):
        ...
        k2 = np.array(["1", "2", "1", "2", "1", "2"])
        df = DataFrame(
            {"v1": np.random.randn(6), "v2": np.random.randn(6), "k1": k1,
    "k2": k2},
    ...
    def test_groupby_with_small_elem(self):
        ...
        df = DataFrame(
            {"event": ["start", "start", "start"], "change": [1234, 5678,
    9123]},
