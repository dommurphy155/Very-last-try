...
@pytest.fixture(
    ...
    ids=["np.int32", "np.int64", "np.float32", "np.float64", "Int64",
    "Float64"],
)
...
@pytest.mark.parametrize("keys", ["jim", ["jim", "joe"]])  # Single key  #
    Multi-key
...
class TestNumericOnly:
        ...
        df = DataFrame(
            {
                ...
                "datetimetz": date_range("20130101", periods=3,
    tz="US/Eastern"),
    ...
    def test_averages(self, df, method):
        ...
        with tm.assert_produces_warning(FutureWarning, match="Dropping
    invalid"):
    ...
    def _check(self, df, method, expected_columns, expected_columns_numeric):
        ...
        with tm.assert_produces_warning(
            warn, match="Dropping invalid columns",
    raise_on_extra_warnings=False
        ...
        if len(expected_columns) < len(gb._obj_with_exclusions.columns):
            warn = FutureWarning
        with tm.assert_produces_warning(warn, match="Dropping invalid
    columns"):
            result = getattr(gb, method)(numeric_only=False)
...
class TestGroupByNonCythonPaths:
    ...
    def test_mad(self, gb, gni):
        ...
        expected = DataFrame([[1, 0.0], [3, np.nan]], columns=["A", "B"],
    index=[0, 1])
        with tm.assert_produces_warning(
            FutureWarning, match="The 'mad' method is deprecated"
        ):
    ...
    def test_describe(self, df, gb, gni):
        ...
        expected_col = MultiIndex(
            levels=[["B"], ["count", "mean", "std", "min", "25%", "50%",
    "75%", "max"]],
...
def test_cython_api2():
    ...
    # cumsum (GH5614)
    df = DataFrame([[1, 2, np.nan], [1, np.nan, 9], [3, 4, 9]], columns=["A",
    "B", "C"])
    expected = DataFrame([[2, np.nan], [np.nan, 9], [4, 9]], columns=["B",
    "C"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("count", {"df": [{"a": 1, "b": 2}, {"a": 2, "b": 2}], "out_type":
    "int64"}),
    ],
)
...
def test_axis1_numeric_only(request, groupby_func, numeric_only):
    ...
    if groupby_func in ("corrwith", "skew"):
        msg = "GH#47723 groupby.corrwith and skew do not correctly implement
    axis=1"
    ...
    no_args = ("cumprod", "cumsum", "diff", "fillna", "pct_change", "rank",
    "shift")
    ...
    if numeric_only is not None and groupby_func in no_args:
        ...
        warn = FutureWarning if groupby_func == "skew" and not numeric_only
    else None
        with tm.assert_produces_warning(warn, match="Dropping of nuisance
    columns"):
            with pytest.raises(TypeError, match=msg):
                method(*args, **kwargs)
...
def test_groupby_cumprod():
    ...
    expected = df.groupby("key", group_keys=False)["value"].apply(lambda x:
    x.cumprod())
    expected.name = "value"
...
def test_nlargest_and_smallest_noop(data, groups, method):
    ...
    expected = Series(data, index=MultiIndex.from_arrays([groups, ser.index]),
    name="a")
    tm.assert_series_equal(result, expected)
...
def test_cummin(dtypes_for_minmax):
    ...
    # GH 15048
    base_df = DataFrame({"A": [1, 1, 1, 1, 2, 2, 2, 2], "B": [3, 4, 3, 2, 2,
    3, 2, 1]})
    ...
    tm.assert_frame_equal(result, expected)
    result = df.groupby("A", group_keys=False).B.apply(lambda x:
    x.cummin()).to_frame()
    tm.assert_frame_equal(result, expected)
    ...
    expected = (
        df.groupby("A", group_keys=False).B.apply(lambda x:
    x.cummin()).to_frame()
    )
    tm.assert_frame_equal(result, expected, check_exact=True)
    ...
    tm.assert_frame_equal(result, expected)
    expected = (
        base_df.groupby("A", group_keys=False).B.apply(lambda x:
    x.cummin()).to_frame()
    )
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize("method", ["cummin", "cummax"])
@pytest.mark.parametrize("dtype", ["UInt64", "Int64", "Float64", "float",
    "boolean"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Test with inf vals
        (
            [np.inf, 1, -np.inf, np.inf, 2, -3, -np.inf, 5, -3, -np.inf,
    -np.inf],
...
def test_groupby_sum_below_mincount_nullable_integer():
    ...
    df = DataFrame({"a": [0, 1, 2], "b": [0, 1, 2], "c": [0, 1, 2]},
    dtype="Int64")
    grouped = df.groupby("a")
    ...
    expected = DataFrame({"b": [pd.NA] * 3, "c": [pd.NA] * 3}, dtype="Int64",
    index=idx)
    tm.assert_frame_equal(result, expected)
...
def test_mean_on_timedelta():
    ...
    expected = Series(
        pd.to_timedelta([4, 5]), name="time", index=Index(["A", "B"],
    name="cat")
    )
    tm.assert_series_equal(result, expected)
...
def test_groupby_sum_timedelta_with_nat():
    ...
    df = DataFrame(
        {
            "a": [1, 1, 2, 2],
            "b": [pd.Timedelta("1d"), pd.Timedelta("2d"), pd.Timedelta("3d"),
    pd.NaT],
        }
    )
    ...
    expected = Series([td3, pd.NaT], dtype="m8[ns]", name="b",
...
def test_deprecate_numeric_only(
    ...
    # GH#46072
    # drops_nuisance: Whether the op drops nuisance columns even when
    numeric_only=False
    # has_arg: Whether the op has a numeric_only arg
    df = DataFrame({"a1": [1, 1], "a2": [2, 2], "a3": [5, 6], "b": 2 *
    [object]})

    args = get_groupby_method_args(kernel, df)
    kwargs = {} if numeric_only is lib.no_default else {"numeric_only":
    numeric_only}
    ...
    if has_arg and (
        ...
        if numeric_only is True or (not numeric_only_default and not
    drops_nuisance):
            warn = None
        else:
            warn = FutureWarning
        if numeric_only is lib.no_default and numeric_only_default:
            msg = f"The default value of numeric_only in
    DataFrameGroupBy.{kernel}"
        else:
        ...
        or (
            ...
            kernel in ("any", "all", "bfill", "ffill", "fillna", "nth",
    "nunique")
        ...
        assert numeric_only is not lib.no_default or numeric_only_default is
    False
...
def test_deprecate_numeric_only_series(dtype, groupby_func, request):
    ...
    # Test default behavior; kernels that fail may be enabled in the future
    but kernels
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"percentiles": [0.10, 0.20, 0.30], "include": None, "exclude":
    ["int"]},
        {"percentiles": [0.10, 0.20, 0.30], "include": ["int"], "exclude":
    None},
    ],
)
