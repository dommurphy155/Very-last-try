...
def test_groupby_sample_frac_gt_one_without_replacement_raises():
    df = DataFrame({"a": [1, 2], "b": [1, 2]})
    msg = "Replace has to be set to `True` when upsampling the population
    `frac` > 1."
...
def test_groupby_sample_with_weights(index, expected_index):
    ...
    result = df.groupby("a")["b"].sample(n=2, replace=True, weights=[1, 0, 1,
    0])
