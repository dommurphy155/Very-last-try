...
def test_apply_trivial():
    ...
    expected = pd.concat([df.iloc[1:], df.iloc[1:]], axis=1, keys=["float64",
    "object"])
...
def test_apply_trivial_fail():
    ...
    result = df.groupby([str(x) for x in df.dtypes], axis=1,
...
@pytest.mark.parametrize(
    ...
    [1, 2, 3]),
        ...
        (DataFrame({"a": [1, 2, 3, 1, 2, 3], "two": [4, 5, 6, 7, 8, 9]}), [1,
    2, 3]),
        (
            DataFrame(
                {
...
def test_group_apply_once_per_group2(capsys):
    ...
    expected = 2  # Number of times `apply` should call a function for the
    current test
...
def test_groupby_as_index_apply():
    ...
    res = df.groupby(0, as_index=False, group_keys=False).apply(lambda x:
    x).index
...
def test_apply_series_to_frame():
    def f(piece):
        with np.errstate(invalid="ignore"):
            logged = np.log(piece)
        return DataFrame(
            {"value": piece, "demeaned": piece - piece.mean(), "logged":
    logged}
        )
...
def test_apply_chunk_view(group_keys):
    ...
    result = df.groupby("key", group_keys=group_keys).apply(lambda x:
    x.iloc[:2])
...
def test_apply_corner(tsframe):
    result = tsframe.groupby(lambda x: x.year, group_keys=False).apply(lambda
    x: x * 2)
    expected = tsframe * 2
    tm.assert_frame_equal(result, expected)
...
def test_apply_with_duplicated_non_sorted_axis(test_series):
    # GH 30667
    df = DataFrame(
        [["x", "p"], ["x", "p"], ["x", "o"]], columns=["X", "Y"], index=[1, 2,
    2]
    )
    if test_series:
...
def test_apply_reindex_values():
    # GH: 26209
    # reindexing from a single column of a groupby object with duplicate
    indices caused
    # a ValueError (cannot reindex from duplicate axis) in 0.24.2, the problem
    was
    # solved in #30679
    values = [1, 2, 3, 4]
    indices = [1, 1, 2, 2]
    df = DataFrame({"group": ["Group1", "Group2"] * 2, "value": values},
    index=indices)
...
def test_apply_aggregating_timedelta_and_datetime():
    ...
    result = df.groupby("clientid").apply(
        lambda ddf: Series(
            {"clientid_age": ddf.time_delta_zero.min(), "date":
    ddf.datetime.min()}
        )
    )
    expected = DataFrame(
...
def test_apply_groupby_datetimeindex():
    ...
    df = DataFrame(
        data, columns=["Name", "Value"], index=pd.date_range("2020-09-01",
    "2020-09-05")
    )
...
def test_time_field_bug():
    ...
    dfg_conversion_expected = DataFrame({"b": datetime(2015, 1, 1), "c": 2},
    index=[1])
...
def test_gb_apply_list_of_unequal_len_arrays():
    ...
    df = DataFrame(
        {
            "group1": ["a", "a", "a", "b", "b", "b", "a", "a", "a", "b", "b",
    "b"],
            "group2": ["c", "c", "d", "d", "d", "e", "c", "c", "d", "d", "d",
    "e"],
...
def test_apply_series_return_dataframe_groups():
    ...
    expected = Series(
        ["17661101"], index=pd.DatetimeIndex(["2015-02-24"], name="day"),
    name="userId"
    )
    tm.assert_series_equal(result, expected)
...
def test_apply_multi_level_name(category):
    ...
    if category:
        ...
        expected_index = pd.CategoricalIndex([1, 2], categories=[1, 2, 3],
    name="B")
    else:
        expected_index = Index([1, 2], name="B")
    df = DataFrame(
        {"A": np.arange(10), "B": b, "C": list(range(10)), "D":
    list(range(10))}
    ).set_index(["A", "B"])
...
def test_apply_function_returns_non_pandas_non_scalar(function,
    expected_values):
    # GH 31441
...
def test_apply_function_returns_numpy_array():
    ...
    expected = Series(
        [[1.0, 2.0], [3.0], [np.nan]], index=Index(["a", "b", "none"],
    name="A")
    )
    tm.assert_series_equal(result, expected)


@pytest.mark.parametrize("function", [lambda gr: gr.index, lambda gr: gr.index
    + 1 - 1])
...
def test_apply_function_with_indexing_return_column():
    ...
    expected = DataFrame({"foo1": ["one", "three", "two"], "foo2": [3.0, 4.0,
    4.0]})
    tm.assert_frame_equal(result, expected)
...
def test_apply_with_timezones_aware():
    ...
    result1 = df1.groupby("x", group_keys=False).apply(lambda df: df[["x",
    "y"]].copy())
    result2 = df2.groupby("x", group_keys=False).apply(lambda df: df[["x",
    "y"]].copy())
...
def
    test_apply_is_unchanged_when_other_methods_are_called_first(reduction_func):
    # GH #34656
    # GH #34271
...
def test_apply_by_cols_equals_apply_by_rows_transposed():
    ...
    by_cols = df.groupby(axis=1, level=0).apply(lambda x: x.droplevel(axis=1,
    level=0))
...
def test_apply_dropna_with_indexed_same(dropna):
    ...
    result = df.groupby("group", dropna=dropna, group_keys=False).apply(lambda
    x: x)
...
@pytest.mark.parametrize(
    ...
    [
        [
            False,
            DataFrame(
                [[1, 1, 1], [2, 2, 1]], columns=Index(["a", "b", None],
    dtype=object)
            ),
        ],
        [
            True,
            Series(
                [1, 1], index=MultiIndex.from_tuples([(1, 1), (2, 2)],
    names=["a", "b"])
            ),
        ],
    ],
...
def test_apply_na(dropna):
    ...
    expected = dfgrp.apply(lambda x: x.sort_values("z",
...
def test_apply_empty_string_nan_coerce_bug():
    ...
    expected = DataFrame(
        [[1, "", pd.to_datetime(2, unit="s")], [2, "", pd.to_datetime(4,
    unit="s")]],
        columns=["a", "b", "c"],
...
@pytest.mark.parametrize("args, kwargs", [([True], {}), ([], {"numeric_only":
    True})])
...
def test_empty_df(method, op):
    ...
    expected = Series(
        [], name="b", dtype="float64", index=Index([], dtype="float64",
    name="a")
    )
