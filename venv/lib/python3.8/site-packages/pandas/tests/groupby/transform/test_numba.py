...
def test_correct_function_signature():
    ...
    with pytest.raises(NumbaUtilError, match="The first 2"):
        data.groupby("key")["data"].transform(incorrect_function,
    engine="numba")
...
def test_numba_vs_cython(jit, pandas_obj, nogil, parallel, nopython):
    ...
    result = grouped.transform(func, engine="numba",
...
def test_cache(jit, pandas_obj, nogil, parallel, nopython):
    ...
    result = grouped.transform(func_1, engine="numba",
    engine_kwargs=engine_kwargs)
    expected = grouped.transform(lambda x: x + 1, engine="cython")
    tm.assert_equal(result, expected)

    result = grouped.transform(func_2, engine="numba",
    engine_kwargs=engine_kwargs)
...
def test_index_data_correctly_passed():
    ...
    df = DataFrame({"group": ["A", "A", "B"], "v": [4, 5, 6]}, index=[-1, -2,
    -3])
...
def test_multiindex_multi_key_not_supported(nogil, parallel, nopython):
    ...
    engine_kwargs = {"nopython": nopython, "nogil": nogil, "parallel":
    parallel}
    with pytest.raises(NotImplementedError, match="More than 1 grouping
    labels"):
