...
def test_transform():
    ...
    df = DataFrame(
        np.arange(6, dtype="int64").reshape(3, 2), columns=["a", "b"],
    index=[0, 2, 1]
    )
    key = [0, 0, 1]
    expected = (
    ...
    result = df.groupby(key).transform(lambda x: x -
    x.mean()).groupby(key).mean()
    ...
    expected = people.groupby(key,
    group_keys=False).apply(demean).groupby(key).mean()
    tm.assert_frame_equal(result, expected)
...
def test_transform_fast():
    ...
    df = DataFrame({"id": np.arange(100000) / 3, "val":
...
def test_transform_axis_1(request, transformation_func):
    ...
    transformation_func =
    maybe_normalize_deprecated_kernels(transformation_func)
    ...
    with tm.assert_produces_warning(warn):
        result = df.groupby([0, 0, 1], axis=1).transform(transformation_func,
    *args)
...
def test_transform_axis_1_reducer(request, reduction_func):
    ...
    if reduction_func in (
        ...
        marker = pytest.mark.xfail(reason="transform incorrectly fails -
    GH#45986")
...
def test_transform_axis_ts(tsframe):
    ...
    c = len(base.columns)
    tso = DataFrame(
        np.random.randn(r, c), index=base.index, columns=base.columns,
    dtype="float64"
    )
    # monotonic
    ts = tso
...
def test_transform_datetime_to_timedelta():
    ...
    df = DataFrame({"A": Timestamp("20130101"), "B": np.arange(5)})
    expected = Series([Timestamp("20130101") - Timestamp("20130101")] * 5,
    name="A")
...
def test_transform_datetime_to_numeric():
    # GH 10972
    # convert dt to float
    df = DataFrame({"a": 1, "b": date_range("2015-01-01", periods=2,
    freq="D")})
...
def test_transform_casting():
    ...
    result = df[["ID3", "DATETIME"]].groupby("ID3").transform(lambda x:
    x.diff())
...
def test_series_fast_transform_date():
    # GH 13191
    df = DataFrame(
        {"grouping": [np.nan, 1, 1, 3], "d": date_range("2014-1-1",
    "2014-1-4")}
    )
...
def test_groupby_transform_with_int():
    ...
    with np.errstate(all="ignore"):
        with tm.assert_produces_warning(
            ...
            result = df.groupby("A").transform(lambda x: (x - x.mean()) /
    x.std())
    expected = DataFrame(
...
def test_groupby_transform_with_nan_group():
    ...
    expected = Series([1.0, 1.0, 2.0, 3.0, np.nan, 6.0, 6.0, 9.0, 9.0, 9.0],
    name="a")
    tm.assert_series_equal(result, expected)
...
def test_cython_transform_series(op, args, targop):
    ...
    for data in [s, s_missing]:
        ...
        tm.assert_series_equal(expected, data.groupby(labels).transform(op,
    *args))
        tm.assert_series_equal(expected, getattr(data.groupby(labels),
    op)(*args))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"key": ["b"] * 10, "value": np.nan}, Series([np.nan] * 10,
    name="value")),
        # When there is a single NaN
        (
            {"key": ["b"] * 10 + ["a"] * 2, "value": [3] * 3 + [np.nan] + [3]
    * 8},
            {
                ("cumprod", False): [3.0, 9.0, 27.0] + [np.nan] * 7 + [3.0,
    9.0],
                ("cumprod", True): [
                    3.0,
                    9.0,
        ...
        for gb_target in [
            ...
            for c in df:
                if (
                    ...
                    expected.name = c
                    tm.assert_series_equal(expected, gb[c].transform(op,
    *args))
...
def test_transform_with_non_scalar_group():
    ...
    df = DataFrame(
        np.random.randint(1, 10, (4, 12)), columns=cols, index=["A", "C", "G",
    "T"]
    )

    msg = "transform must return a scalar value for each group.*"
    with pytest.raises(ValueError, match=msg):
        df.groupby(axis=1, level=1).transform(lambda z: z.div(z.sum(axis=1),
    axis=0))
...
def test_transform_numeric_ret(cols, expected, agg_func):
    ...
    df = DataFrame(
        {"a": date_range("2018-01-01", periods=3), "b": range(3), "c":
    range(7, 10)}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("ffill", 1, [np.nan, np.nan, "val1", "val1", np.nan, "val2", "val2",
    np.nan]),
        (
            "bfill",
            None,
        ...
        ("bfill", 1, [np.nan, "val1", "val1", np.nan, "val2", "val2", np.nan,
    np.nan]),
    ],
)
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason="GH#23918 before method uses freq in vectorized
    approach"
            ),
        ),
    ],
...
def test_groupby_transform_rename():
    ...
    def demean_rename(x):
        ...
        result = result.rename(columns={c: f"{c}_demeaned" for c in
    result.columns})
...
def test_groupby_transform_timezone_column(func):
    ...
    expected = DataFrame([[ts, 1, ts]], columns=["end_time", "id",
    "max_end_time"])
    tm.assert_frame_equal(result, expected)
...
def test_categorical_and_not_categorical_key(observed):
    ...
    # DataFrame case
    result = df_with_categorical.groupby(["A", "C"],
    observed=observed).transform("sum")
...
def test_null_group_str_reducer(request, dropna, reduction_func):
    ...
    df = DataFrame({"A": [1, 1, np.nan, np.nan], "B": [1, 2, 2, 3]},
    index=index)
...
def test_null_group_str_transformer(request, dropna, transformation_func):
    ...
    if dropna:
        ...
        buffer.append(DataFrame([[np.nan]], index=[3], dtype=dtype,
    columns=["B"]))
    ...
    warn = FutureWarning if transformation_func in ("backfill", "pad") else
    None
...
@pytest.mark.filterwarnings("ignore:tshift is deprecated:FutureWarning")
def test_null_group_str_transformer_series(request, dropna,
    transformation_func):
    # GH 17093
    if transformation_func == "tshift":
        msg = "tshift requires timeseries"
...
def test_transform_aligns_depr(func, series, expected_values, keys,
    keys_in_index):
