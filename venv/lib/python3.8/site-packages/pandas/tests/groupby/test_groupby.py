...
def test_len():
    ...
    grouped = df.groupby([lambda x: x.year, lambda x: x.month, lambda x:
    x.day])
...
def test_with_na_groups(dtype):
    ...
    labels = Series(
        [np.nan, "foo", "bar", "bar", np.nan, np.nan, "bar", "bar", np.nan,
    "foo"],
        index=index,
    )
...
def test_multi_func(df):
    ...
    tm.assert_frame_equal(
        agged.loc[:, ["C", "D"]], expected.loc[:, ["C", "D"]],
    check_names=False
    )
...
def test_multi_key_multiple_functions(df):
    ...
    expected = DataFrame({"mean": grouped.agg(np.mean), "std":
    grouped.agg(np.std)})
...
def test_frame_multi_key_function_list():
    ...
    expected = pd.concat(
        [grouped["D"].agg(funcs), grouped["E"].agg(funcs),
    grouped["F"].agg(funcs)],
        keys=["D", "E", "F"],
        axis=1,
    )
...
def test_as_index_select_column():
    ...
    expected = Series(
        [2, 6, 6], name="B", index=MultiIndex.from_tuples([(0, 0), (0, 1), (1,
    2)])
    )
    tm.assert_series_equal(result, expected)
...
def test_groupby_as_index_agg(df):
    ...
    df = DataFrame(np.random.randint(0, 100, (50, 3)), columns=["jim", "joe",
    "jolie"])
...
def test_ops_not_as_index(reduction_func):
    ...
    with tm.assert_produces_warning(warn, match="The 'mad' method is
    deprecated"):
    ...
    if reduction_func != "size":
        # 32 bit compat -> groupby preserves dtype whereas reset_index casts
    to int64
...
def test_keep_nuisance_agg(df, agg_function):
    ...
    expected.loc["bar", "B"] = getattr(df.loc[df["A"] == "bar", "B"],
    agg_function)()
    expected.loc["foo", "B"] = getattr(df.loc[df["A"] == "foo", "B"],
    agg_function)()
    tm.assert_frame_equal(result, expected)
...
def test_omit_nuisance_agg(df, agg_function, numeric_only):
    # GH 38774, GH 38815
    if numeric_only is lib.no_default or (not numeric_only and agg_function !=
    "sum"):
    ...
    if agg_function in ("var", "std", "sem") and numeric_only is False:
        ...
        if numeric_only is lib.no_default:
            msg = (
                f"The default value of numeric_only in
    DataFrameGroupBy.{agg_function}"
            )
        else:
        ...
        if (
            ...
            # These methods drop non-numeric columns even when numeric_only is
    False
        ...
        if agg_function == "sum" and numeric_only is False:
            ...
            warn = None
        elif agg_function in ("sum", "std", "var", "sem") and numeric_only is
    not True:
            warn = FutureWarning
        else:
            warn = None
...
def test_groupby_level_nonmulti():
    # GH 1313, GH 13901
    s = Series([1, 2, 3, 10, 4, 5, 20, 6], Index([1, 2, 3, 1, 4, 5, 2, 6],
    name="foo"))
...
def test_groupby_series_indexed_differently():
    ...
    s2 = Series(
        [1.0, 1.0, 4.0, 5.0, 5.0, 7.0], index=Index(["a", "b", "d", "f", "g",
    "h"])
    )
...
def test_groupby_mixed_type_columns():
    ...
    expected = DataFrame([[1, 2]], columns=["B", 0], index=Index([0],
    name="A"))
...
def test_groupby_unit64_float_conversion():
    ...
    df = DataFrame({"first": [1], "second": [1], "value":
...
def test_groupby_keys_same_size_as_index():
    ...
    df = DataFrame([["A", 10], ["B", 15]], columns=["metric", "values"],
    index=index)
...
def test_groupby_nat_exclude():
    ...
    tm.assert_frame_equal(grouped.get_group(Timestamp("2013-01-01")),
    df.iloc[[1, 7]])
    tm.assert_frame_equal(grouped.get_group(Timestamp("2013-02-01")),
    df.iloc[[3, 5]])
...
def test_int32_overflow():
    ...
    A = np.arange(25000)
    df = DataFrame({"A": A, "B": B, "C": A, "D": B, "E":
    np.random.randn(25000)})
...
def test_dont_clobber_name_column():
    df = DataFrame(
        {"key": ["a", "a", "a", "b", "b", "b"], "name": ["foo", "bar", "baz"]
    * 2}
    )
...
def test_groupby_reindex_inside_function():
    ...
    df = DataFrame({"high": np.arange(periods), "low": np.arange(periods)},
    index=ind)
...
def test_groupby_multiindex_not_lexsorted():
    ...
    not_lexsorted_df = DataFrame(
        columns=["a", "b", "c", "d"], data=[[1, "b1", "c1", 3], [1, "b2",
    "c2", 4]]
    )
    ...
    for level in [0, 1, [0, 1]]:
        for sort in [False, True]:
            result = df.groupby(level=level, sort=sort,
...
@pytest.mark.parametrize(
    "group_column", ["int_groups", "string_groups", ["int_groups",
    "string_groups"]]
)
...
def test_pivot_table_values_key_error():
    ...
    df = DataFrame(
        {
            "eventDate": date_range(datetime.today(), periods=20,
    freq="M").tolist(),
            "thename": range(0, 20),
        }
    )
...
def test_empty_groupby(columns, keys, values, method, op, request,
    ...
    if (
        ...
        if using_array_manager and op == "mad":
            ...
            msg = "Regex pattern \"'Categorical' does not implement.*" +
    right_msg
        ...
        mark = pytest.mark.xfail(
            raises=ValueError, match="attempt to get arg(min|max) of an empty
    sequence"
        )
        request.node.add_marker(mark)
    elif (
        ...
        request.node.add_marker(mark)
    elif isinstance(values, Categorical) and len(keys) == 1 and op in ["sum",
    "prod"]:
        mark = pytest.mark.xfail(
        ...
        mark = pytest.mark.xfail(
            raises=TypeError,
            match=r"Cannot interpret 'datetime64\[ns, US/Eastern\]' as a data
    type",
        )
        request.node.add_marker(mark)
...
def test_tuple_correct_keyerror():
    ...
    df = DataFrame(1, index=range(3), columns=MultiIndex.from_product([[1, 2],
    [3, 4]]))
...
def test_groupby_groups_in_BaseGrouper():
    ...
    mi = MultiIndex.from_product([["A", "B"], ["C", "D"]], names=["alpha",
    "beta"])
...
def test_shift_bfill_ffill_tz(tz_naive_fixture, op, expected):
    ...
    expected = DataFrame(expected).assign(time=lambda x:
    x.time.dt.tz_localize(tz))
    tm.assert_frame_equal(result, expected)
...
def test_group_on_empty_multiindex(transformation_func, request):
    ...
    *args)
    expected = df.groupby(["col_1"]).transform(transformation_func,
    *args).iloc[:0]
    ...
    *args)
    ...
    expected = (
        df["col_3"].groupby(["col_1"]).transform(transformation_func,
    *args).iloc[:0]
    )
    if transformation_func in ("diff", "shift"):
        expected = expected.astype(int)
...
def test_groupby_crash_on_nunique(axis):
    ...
    df = DataFrame({("A", "B"): [1, 2], ("A", "C"): [1, 3], ("D", "B"): [0,
    0]})
...
def test_groupby_numerical_stability_sum_mean(func, values):
    ...
    expected = DataFrame({"a": values, "b": values}, index=Index([1, 2],
    name="group"))
    tm.assert_frame_equal(result, expected)
...
def test_groupby_numerical_stability_cumsum():
    ...
    exp_data = (
        [1e16] * 2 + [1e16 + 96, 1e16 + 98] + [5e15 + 97, 5e15 + 98] + [97.0,
    98.0]
    )
...
def
    test_groupby_mean_duplicate_index(rand_series_with_duplicate_datetimeindex):
...
def test_groupby_filtered_df_std():
    # GH 16174
    dicts = [
        {"filter_col": False, "groupby_col": True, "bool_col": True,
    "float_col": 10.5},
        {"filter_col": True, "groupby_col": True, "bool_col": True,
    "float_col": 20.5},
        {"filter_col": True, "groupby_col": True, "bool_col": True,
    "float_col": 30.5},
    ]
...
def test_groupby_none_column_name():
    ...
    expected = DataFrame({"b": [2, 5], "c": [9, 13]}, index=Index([1, 2],
    name=None))
    tm.assert_frame_equal(result, expected)
...
def test_single_element_list_grouping():
    # GH 42795
    df = DataFrame(
        {"a": [np.nan, 1], "b": [np.nan, 5], "c": [np.nan, 2]}, index=["x",
    "y"]
    )
    msg = (
