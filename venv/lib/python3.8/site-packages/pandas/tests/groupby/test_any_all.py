...
def test_groupby_bool_aggs(agg_func, skipna, vals):
    ...
    exp_df = DataFrame([exp] * 2, columns=["val"], index=Index(["a", "b"],
    name="key"))
...
def test_masked_mixed_types(dtype1, dtype2, exp_col1, exp_col2):
    ...
    data = [1.0, np.nan]
    df = DataFrame(
        {"col1": pd.array(data, dtype=dtype1), "col2": pd.array(data,
    dtype=dtype2)}
    )
...
@pytest.mark.parametrize("skipna", [True, False])
def test_masked_bool_aggs_skipna(bool_agg_func, dtype, skipna,
    frame_or_series):
    # GH#40585
