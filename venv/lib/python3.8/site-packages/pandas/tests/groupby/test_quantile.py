...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([np.nan, 4.0, np.nan, 2.0, np.nan], [np.nan, 4.0, np.nan, 2.0,
    np.nan]),
        # Timestamps
        (
...
def test_quantile(interpolation, a_vals, b_vals, q, request):
    ...
    df = DataFrame(
        {"key": ["a"] * len(a_vals) + ["b"] * len(b_vals), "val": a_vals +
    b_vals}
    )

    expected = DataFrame(
        [a_expected, b_expected], columns=["val"], index=Index(["a", "b"],
    name="key")
    )
...
def test_quantile_array2():
    ...
    df = DataFrame(
        np.random.RandomState(0).randint(0, 5, size=(10, 3)),
    columns=list("ABC")
    )
...
def test_quantile_array_no_sort():
    ...
    expected = DataFrame(
        {"A": [0.5, 1.0, 1.5, 1.0, 1.0, 1.0], "B": [3.5, 4.0, 4.5, 4.0, 4.0,
    4.0]},
...
def test_quantile_array_multiple_levels():
    df = DataFrame(
        {"A": [0, 1, 2], "B": [3, 4, 5], "c": ["a", "a", "a"], "d": ["a", "a",
    "b"]}
    )
    result = df.groupby(["c", "d"]).quantile([0.25, 0.75])
    index = pd.MultiIndex.from_tuples(
        [("a", "a", 0.25), ("a", "a", 0.75), ("a", "b", 0.25), ("a", "b",
    0.75)],
        names=["c", "d", None],
    )
    expected = DataFrame(
...
def test_groupby_quantile_with_arraylike_q_and_int_columns(frame_size,
    groupby, q):
    # GH30289
    nrow, ncol = frame_size
    df = DataFrame(np.array([ncol * [_ % 4] for _ in range(nrow)]),
    columns=range(ncol))
    ...
    expected_values = [
        [float(x)] * (ncol - len(groupby)) for x in range(min(nrow, 4)) for _
    in q
    ]
...
def test_quantile_raises():
    df = DataFrame([["foo", "a"], ["foo", "b"], ["foo", "c"]], columns=["key",
    "val"])

    with pytest.raises(TypeError, match="cannot be performed against 'object'
    dtypes"):
...
def test_groupby_quantile_skips_invalid_dtype(q, numeric_only):
    ...
    if numeric_only is lib.no_default or numeric_only:
        ...
        with pytest.raises(
            TypeError, match="'quantile' cannot be performed against 'object'
    dtypes!"
        ):
...
def test_groupby_quantile_NA_float(any_float_dtype):
    ...
    expected = pd.Series(
        [0.2] * 2,
        index=pd.MultiIndex.from_product((exp_index, [0.5, 0.75]), names=["x",
    None]),
        name="y",
    )
    tm.assert_series_equal(result, expected)
...
def test_groupby_quantile_NA_int(any_int_ea_dtype):
    ...
    result = df.groupby("x")["y"].quantile(0.5)
    expected = pd.Series(
        [3.5], dtype=float, index=Index([1], name="x",
    dtype=any_int_ea_dtype), name="y"
    )
    tm.assert_series_equal(expected, result)
...
def test_groupby_timedelta_quantile():
    # GH: 29485
    df = DataFrame(
        {"value": pd.to_timedelta(np.arange(4), unit="s"), "group": [1, 1, 2,
    2]}
    )
...
def test_columns_groupby_quantile():
    ...
    expected = DataFrame(
        ...
        columns=pd.MultiIndex.from_tuples(
            [("A", 0.8), ("A", 0.2), ("B", 0.8), ("B", 0.2)], names=["col",
    None]
        ),
    )
