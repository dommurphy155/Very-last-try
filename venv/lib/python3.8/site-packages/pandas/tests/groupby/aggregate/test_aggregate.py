    ...
    df = DataFrame(
        {
            ...
            "by1": ["red", "blue", 1, 2, np.nan, "big", 1, 2, "red", 1,
    np.nan, 12],
            "by2": [
                "wet",
                "dry",
...
def test_groupby_aggregation_multi_level_column():
    ...
    df = DataFrame(
        data=lst,
        columns=MultiIndex.from_tuples([("A", 0), ("A", 1), ("B", 0), ("B",
    1)]),
    )
...
@pytest.mark.parametrize(
    "groupbyfunc", [lambda x: x.weekday(), [lambda x: x.month, lambda x:
    x.weekday()]]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("var", [4.5] * 3, "Int64", {"i": "float64", "j": "float64", "k":
    "float64"}),
    ],
)
def test_multiindex_groupby_mixed_cols_axis1(func, expected, dtype,
    result_dtype_dict):
    # GH#43209
    df = DataFrame(
        [[1, 2, 3, 4, 5, 6]] * 3,
...
def test_agg_multiple_functions_same_name():
    ...
    expected_columns = MultiIndex.from_tuples([("A", "quantile"), ("A",
    "quantile")])
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    ...
    result = df.resample("3T").agg(
        {"A": ["ohlc", partial(np.quantile, q=0.9999), partial(np.quantile,
    q=0.1111)]}
    )
    expected_index = pd.date_range("1/1/2012", freq="3T", periods=6,
    name="dti")
    ...
    expected_values = np.hstack([df.resample("3T").A.ohlc(),
...
def test_groupby_agg_coercing_bools():
    # issue 14873
    dat = DataFrame({"a": [1, 1, 2, 2], "b": [0, 1, 2, 3], "c": [None, None,
    1, 1]})
    gp = dat.groupby("a")
...
def test_callable_result_dtype_frame(
    ...
    expected_index = pd.RangeIndex(0, 1) if method == "transform" else
    agg_index
    expected = DataFrame({"c": [df["c"].iloc[0]]},
    index=expected_index).astype(
        result_dtype
    )
    if method == "apply":
...
def test_ohlc_ea_dtypes(any_numeric_ea_dtype):
    ...
    expected = DataFrame(
        ...
        columns=MultiIndex.from_product([["b"], ["open", "high", "low",
    "close"]]),
...
@pytest.mark.parametrize("how", ["first", "last", "min", "max", "mean",
    "median"])
...
class TestNamedAggregationDataFrame:
    def test_agg_relabel(self):
        df = DataFrame(
            {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7,
    8]}
        )
        result = df.groupby("group").agg(a_max=("A", "max"), b_max=("B",
    "max"))
        expected = DataFrame(
            {"a_max": [1, 3], "b_max": [6, 8]},
    ...
    8]}
        ...
        expected = DataFrame({"my col": [1, 3]}, index=Index(["a", "b"],
    name="group"))
        tm.assert_frame_equal(result, expected)
    ...
    def test_duplicate_no_raises(self):
        ...
        expected = DataFrame({"a": [1, 3], "b": [1, 3]}, index=Index([0, 1],
    name="A"))
        ...
        test = DataFrame({"col1": ["a", "a", "b", "b", "b"], "col2": [1, 2, 3,
    4, 5]})
    ...
    def test_agg_namedtuple(self):
        ...
        result = df.groupby("A").agg(
            b=pd.NamedAgg("B", "sum"), c=pd.NamedAgg(column="B",
    aggfunc="count")
        )
    ...
    def test_mangled(self):
        ...
        result = df.groupby("A").agg(b=("B", lambda x: 0), c=("C", lambda x:
    1))
        expected = DataFrame({"b": [0, 0], "c": [1, 1]}, index=Index([0, 1],
    name="A"))
        tm.assert_frame_equal(result, expected)
...
def test_agg_relabel_multiindex_column(
    ...
    df.columns = MultiIndex.from_tuples([("x", "group"), ("y", "A"), ("y",
    "B")])
...
def test_agg_relabel_multiindex_duplicates():
    ...
    # GH28426, after this change, duplicates should also work if the
    relabelling is
    # different
    df = DataFrame(
        {"group": ["a", "a", "b", "b"], "A": [0, 1, 2, 3], "B": [5, 6, 7, 8]}
...
def test_grouby_agg_loses_results_with_as_index_false_relabel():
    ...
    df = DataFrame(
        {"key": ["x", "y", "z", "x", "y", "z"], "val": [1.0, 0.8, 2.0, 3.0,
    3.6, 0.75]}
    )
...
def test_grouby_agg_loses_results_with_as_index_false_relabel_multiindex():
    ...
    result = grouped.agg(min_val=pd.NamedAgg(column="val", aggfunc="min"))
    expected = DataFrame(
        {"key": ["x", "x", "y"], "key1": ["a", "c", "b"], "min_val": [1.0,
    0.75, 0.8]}
    )
    tm.assert_frame_equal(result, expected)
...
def test_aggregate_mixed_types():
    # GH 16916
    df = DataFrame(
        data=np.array([0] * 9).reshape(3, 3), columns=list("XYZ"),
    index=list("abc")
    )
...
def test_aggregate_udf_na_extension_type():
    ...
    expected = DataFrame({"A": pd.array([1, pd.NA], dtype="Int64")}, index=[1,
    2])
    tm.assert_frame_equal(result, expected)
...
class TestLambdaMangling:
    ...
    @pytest.mark.filterwarnings("ignore:Dropping invalid
    columns:FutureWarning")
    ...
    def test_agg_multiple_lambda(self):
        ...
        result2 = df.groupby(by="kind").agg(
            ...
            height_max=pd.NamedAgg(column="height", aggfunc="max"),
            weight_max=pd.NamedAgg(column="weight", aggfunc="max"),
            height_max_2=pd.NamedAgg(column="height", aggfunc=lambda x:
    np.max(x)),
            weight_min=pd.NamedAgg(column="weight", aggfunc=lambda x:
    np.min(x)),
        )
        tm.assert_frame_equal(result2, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ({"nr": "min", "cat_ord": "min"}, {"nr": [1, 5], "cat_ord": ["a",
    "c"]}),
...
def test_groupby_single_agg_cat_cols(grp_col_dict, exp_data):
    ...
    cat_index = pd.CategoricalIndex(
        ["a", "b"], categories=["a", "b"], ordered=False, name="cat",
    dtype="category"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"nr": "min", "cat_ord": ["min", "max"]}, [(1, "a", "b"), (5, "c",
    "d")]),
...
def test_groupby_combined_aggs_cat_cols(grp_col_dict, exp_data):
    ...
    expected_df = DataFrame(data=exp_data, columns=multi_index,
    index=cat_index)
    for col in expected_df.columns:
        if isinstance(col, tuple) and "cat_ord" in col:
            # ordered categorical should be preserved
            expected_df[col] =
    expected_df[col].astype(input_df["cat_ord"].dtype)
...
def test_groupby_index_object_dtype():
    # GH 40014
    df = DataFrame({"c0": ["x", "x", "x"], "c1": ["x", "x", "y"], "p": [0, 1,
    2]})
...
@pytest.mark.parametrize(
    "func, output", [("mean", [8 + 18j, 10 + 22j]), ("sum", [40 + 90j, 50 +
    110j])]
)
...
@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"b": "sum"}, {"b": "prod", "c":
    "median"}]
)
def test_multi_axis_1_raises(func):
    ...
    with pytest.raises(NotImplementedError, match="axis other than 0 is not
    supported"):
        gb.agg(func)
...
@pytest.mark.parametrize(
    "test, constant",
    [
        ([[20, "A"], [20, "B"], [10, "C"]], {0: [10, 20], 1: ["C", ["A",
    "B"]]}),
        ([[20, "A"], [20, "B"], [30, "C"]], {0: [20, 30], 1: [["A", "B"],
    "C"]}),
        ([["a", 1], ["a", 1], ["b", 2], ["b", 3]], {0: ["a", "b"], 1: [1, [2,
    3]]}),
        pytest.param(
            [["a", 1], ["a", 2], ["b", 3], ["b", 3]],
            {0: ["a", "b"], 1: [[1, 2], 3]},
