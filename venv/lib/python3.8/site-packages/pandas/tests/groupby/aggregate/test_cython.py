...
def test_cython_agg_nothing_to_agg():
    frame = DataFrame({"a": np.random.randint(0, 5, 50), "b": ["foo", "bar"] *
    25})

    with tm.assert_produces_warning(FutureWarning, match="This will raise a
    TypeError"):
...
def test__cython_agg_general(op, targop):
    ...
    result = df.groupby(labels)._cython_agg_general(op, alt=None,
    numeric_only=True)
...
@pytest.mark.parametrize(
    "data", [Timestamp("2016-10-14 21:00:44.557"), Timedelta("17088 days
    21:00:44.557")]
)
