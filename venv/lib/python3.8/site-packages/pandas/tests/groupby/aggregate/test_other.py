...
def test_agg_datetimes_mixed():
    ...
    data = [
        [
            row[0],
            (dt.datetime.strptime(row[1], "%Y-%m-%d").date() if row[1] else
    None),
            row[2],
        ]
        for row in data
...
def test_agg_dict_parameter_cast_result_dtypes():
    ...
    # count
    exp = Series([2, 2, 2, 2], index=Index(list("ABCD"), name="class"),
    name="time")
    tm.assert_series_equal(grouped.time.agg(len), exp)
    tm.assert_series_equal(grouped.time.size(), exp)

    exp = Series([0, 1, 1, 2], index=Index(list("ABCD"), name="class"),
    name="time")
...
def test_aggregate_float64_no_int64():
    # see gh-11199
    df = DataFrame({"a": [1, 2, 3, 4, 5], "b": [1, 2, 2, 4, 5], "c": [1, 2, 3,
    4, 5]})
    ...
    expected = DataFrame({"a": [1, 2.5, 4, 5], "c": [1, 2.5, 4, 5]}, index=[1,
    2, 4, 5])
    expected.index.name = "b"
...
def test_agg_nested_dicts():
    ...
    msg = r"nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        g.aggregate({"r1": {"C": ["mean", "sum"]}, "r2": {"D": ["mean",
    "sum"]}})
...
def test_agg_timezone_round_trip():
    ...
    df = DataFrame({"a": 1, "b": [ts + dt.timedelta(minutes=nn) for nn in
    range(10)]})
    ...
    dates = [
        pd.Timestamp(f"2016-01-0{i:d} 12:00:00", tz="US/Pacific") for i in
    range(1, 5)
    ]
...
def test_agg_structs_dataframe(structure, expected):
    df = DataFrame(
        {"A": [1, 1, 1, 3, 3, 3], "B": [1, 1, 1, 4, 4, 4], "C": [1, 1, 1, 3,
    4, 4]}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda x: tuple(x), Series([(1, 1, 1), (3, 4, 4)], index=[1, 3],
    name="C")),
        (lambda x: list(x), Series([[1, 1, 1], [3, 4, 4]], index=[1, 3],
    name="C")),
    ],
)
...
def test_agg_category_nansum(observed):
    categories = ["a", "b", "c"]
    df = DataFrame(
        {"A": pd.Categorical(["a", "a", "b"], categories=categories), "B": [1,
    2, 3]}
    )
    result = df.groupby("A", observed=observed).B.agg(np.nansum)
    expected = Series(
        [3, 3, 0],
        index=pd.CategoricalIndex(["a", "b", "c"], categories=categories,
    name="A"),
        name="B",
    )
    if observed:
...
def test_agg_list_like_func():
    # GH 18473
    df = DataFrame({"A": [str(x) for x in range(3)], "B": [str(x) for x in
    range(3)]})
...
def test_groupby_agg_err_catching(err_cls):
    ...
    data = make_data()[:5]
    df = DataFrame(
        {"id1": [0, 0, 0, 1, 1], "id2": [0, 1, 0, 1, 1], "decimals":
    DecimalArray(data)}
    )
    ...
    def weird_func(x):
        # weird function that raise something other than TypeError or
    IndexError
        #  in _python_agg_general
        if len(x) == 0:
            raise err_cls
