...
class TestGroupBy:
    def test_groupby_with_timegrouper(self):
        ...
        for df in [df_original, df_reordered]:
            ...
            expected = DataFrame(
                {"Quantity": 0},
                index=date_range(
                    "20130901", "20131205", freq="5D", name="Date",
    inclusive="left"
                ),
            )
    ...
    def test_timegrouper_with_reg_groups(self):
        ...
        for df in [df_original, df_sorted]:
            ...
            df = df.reset_index()
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", key="Date"),
    "Buyer"]).sum()
            tm.assert_frame_equal(result, expected)

            with pytest.raises(KeyError, match="'The grouper name foo is not
    found'"):
            ...
            df = df.set_index("Date")
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", level="Date"),
    "Buyer"]).sum()
            tm.assert_frame_equal(result, expected)
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                result = df.groupby([Grouper(freq="1M", level=0),
    "Buyer"]).sum()
            tm.assert_frame_equal(result, expected)
    ...
    "Buyer"]).sum()
            ...
            expected = DataFrame(
                ...
                columns=["Quantity"],
                index=DatetimeIndex(
                    [datetime(2013, 10, 31, 0, 0)], freq=offsets.MonthEnd(),
    name="Date"
                ),
            )
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
    ...
    def test_timegrouper_with_reg_groups_freq(self, freq):
        ...
        result1 = (
            df.sort_index().groupby([Grouper(freq=freq),
    "user_id"])["whole_cost"].sum()
        )
        tm.assert_series_equal(result1, expected)

        result2 = df.groupby([Grouper(freq=freq),
    "user_id"])["whole_cost"].sum()
    ...
    def test_timegrouper_get_group(self):
        ...
        g_list = [("Joe", "2013-09-30"), ("Carl", "2013-10-31"), ("Joe",
    "2013-12-31")]
    ...
    def test_timegrouper_apply_return_type_series(self):
        ...
        # Issue #11742
        df = DataFrame({"date": ["10/10/2000", "11/10/2000"], "value": [10,
    13]})
        df_dt = df.copy()
        ...
        result = df_dt.groupby(Grouper(freq="M",
    ...
    def test_groupby_groups_datetimeindex(self):
        ...
        df = DataFrame({"A": [5, 6, 7, 8, 9], "B": [1, 2, 3, 4, 5]},
    index=index)
        result = df.groupby(level="date").groups
        dates = ["2015-01-05", "2015-01-04", "2015-01-03", "2015-01-02",
    "2015-01-01"]
        expected = {
            Timestamp(date): DatetimeIndex([date], name="date") for date in
    dates
        }
    ...
    def test_groupby_groups_datetimeindex_tz(self):
        ...
        df["datetime"] = df["datetime"].apply(lambda d: Timestamp(d,
    tz="US/Pacific"))
        ...
        df = DataFrame(
            {"value1": np.arange(6, dtype="int64"), "value2": [1, 2, 3, 1, 2,
    3]},
            index=didx,
        )
    ...
    def test_frame_datetime64_handling_groupby(self):
        # it works!
        df = DataFrame(
            [(3, np.datetime64("2012-07-03")), (3,
    np.datetime64("2012-07-04"))],
            columns=["a", "date"],
        )
...
4,2000-01-01 16:50:00,America/New_York"""
        ...
        df = pd.read_csv(StringIO(data), header=None, names=["value", "date",
    "tz"])
    ...
    tz="America/Los_Angeles"),
        ...
        exp_values = Series(
            ["2000-01-28 16:47:00", "2000-01-29 16:48:00", "2000-01-31
    16:50:00"],
            index=[0, 1, 3],
            name="date",
        )
        ...
        df = DataFrame(
            {
                ...
                "time": date_range("01/01/2000 00:00", periods=60, freq="s",
    tz="UTC"),
            }
        )
    ...
    def test_datetime_count(self):
        df = DataFrame(
            {"a": [1, 2, 3] * 2, "dates": date_range("now", periods=6,
    freq="T")}
        )
    ...
    def test_scalar_call_versus_list_call(self):
        ...
        data_frame = {
            ...
            "time": Series(
                ["2017-08-09 13:32:23", "2017-08-11 23:23:15", "2017-08-11
    22:23:15"],
                dtype="datetime64[ns]",
            ),
            "value": [1, 2, 3],
    ...
    def test_grouper_period_index(self):
        ...
        expected = Series(
            range(0, periods), index=Index(range(1, periods + 1),
    name=index.name)
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_groupby_apply_timegrouper_with_nat_dict_returns(
        ...
        # GH#43500 case where gb.grouper.result_index and
    gb.grouper.group_keys_seq
        #  have different lengths that goes through the `isinstance(values[0],
    dict)`
        #  path
    ...
    def test_groupby_apply_timegrouper_with_nat_apply_squeeze(
        ...
        #  so use a huge freq so that all non-NaT dates will be grouped
    together
        tdg = Grouper(key="Date", freq="100Y")

        with tm.assert_produces_warning(FutureWarning, match="`squeeze`
    parameter"):
