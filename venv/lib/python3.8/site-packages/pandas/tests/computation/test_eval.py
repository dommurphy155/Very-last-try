...
class TestEval:
    ...
    def test_complex_cmp_ops(self, cmp1, cmp2, binop, lhs, rhs, engine,
    parser):
    ...
    def test_binary_arith_ops(self, arith1, lhs, rhs, engine, parser):
        ...
        try:
            ...
            if engine == "numexpr":
                ...
                # TODO: update testing code so that assert_almost_equal
    statement
    ...
    def test_and_logic_string_match(self):
        ...
        assert pd.eval(f"{event.str.match('hello').a and
    event.str.match('hello').a}")
...
def should_warn(*args):
    ...
    args))
    only_one_dt = reduce(
        operator.xor, map(lambda x: issubclass(x.dtype.type, np.datetime64),
    args)
    )
...
class TestAlignment:
    ...
    def test_basic_frame_alignment(
        ...
        with warnings.catch_warnings(record=True):
            ...
            df = tm.makeCustomDataframe(
                10, 10, data_gen_f=f, r_idx_type=lr_idx_type,
    c_idx_type=c_idx_type
            )
            df2 = tm.makeCustomDataframe(
                20, 10, data_gen_f=f, r_idx_type=rr_idx_type,
    c_idx_type=c_idx_type
            )
    ...
    def test_frame_comparison(self, engine, parser, r_idx_type, c_idx_type):
        ...
        df3 = DataFrame(np.random.randn(*df.shape), index=df.index,
    columns=df.columns)
    ...
    def test_medium_complex_frame_alignment(self, engine, parser, r1, c1, r2,
    c2):
    ...
    def test_basic_frame_series_alignment(
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always", RuntimeWarning)
            df = tm.makeCustomDataframe(
                10, 10, data_gen_f=f, r_idx_type=r_idx_type,
    c_idx_type=c_idx_type
            )
    ...
    @pytest.mark.parametrize("r2", index_types)
    @pytest.mark.parametrize("c2", index_types)
    def test_complex_series_frame_alignment(self, engine, parser, r1, c1, r2,
    c2):
        n = 3
        m1 = 5
        m2 = 2 * m1
        ...
        with warnings.catch_warnings(record=True):
            ...
            if should_warn(df2.index, ser.index, df.index):
                with tm.assert_produces_warning(RuntimeWarning):
                    res = pd.eval("df2 + ser + df", engine=engine,
    parser=parser)
            else:
...
class TestOperations:
    ...
    def test_truediv(self):
        ...
        # FutureWarning: The `truediv` parameter in pd.eval is deprecated and
    will be
    ...
    def test_multi_line_expression(self):
        ...
        msg = "Multi-line expressions are only valid if all expressions
    contain"
        with pytest.raises(ValueError, match=msg):
            df.eval(
                """
    ...
    def test_basic_period_index_boolean_expression(self):
        df = tm.makeCustomDataframe(2, 2, data_gen_f=f, c_idx_type="p",
    r_idx_type="i")
    ...
    def test_simple_in_ops(self, engine, parser):
        if parser != "python":
            ...
            res = pd.eval("(3,) not in [(3,), 2]", engine=engine,
    parser=parser)
            assert not res

            res = pd.eval("[(3,)] in [[(3,)], 2]", engine=engine,
    parser=parser)
            assert res
        else:
...
def test_numexpr_option_incompatible_op():
    ...
    with pd.option_context("compute.use_numexpr", False):
        df = DataFrame(
            {"A": [True, False, True, False, None, None], "B": [1, 2, 3, 4, 5,
    6]}
        )
...
def test_more_than_one_expression_raises(engine, parser):
    with pytest.raises(SyntaxError, match="only a single expression is
    allowed"):
...
def test_eval_no_support_column_name(request, column):
    ...
    df = DataFrame(np.random.randint(0, 100, size=(10, 2)), columns=[column,
    "col1"])
