...
class TestTSPlot(TestPlotBase):
    ...
    def test_get_datevalue(self):
        ...
        assert get_datevalue(Period(1987, "A"), "M") == Period("1987-12",
    "M").ordinal
    ...
    def test_line_plot_period_frame(self, freq):
        idx = date_range("12/31/1999", freq=freq, periods=100)
        df = DataFrame(np.random.randn(len(idx), 3), index=idx, columns=["A",
    "B", "C"])
    ...
    def test_plot_multiple_inferred_freq(self):
        dr = Index([datetime(2000, 1, 1), datetime(2000, 1, 6), datetime(2000,
    1, 11)])
        ser = Series(np.random.randn(len(dr)), index=dr)
        _check_plot_works(ser.plot)
    ...
    def test_axis_limits(self):
        def _test(ax):
            ...
            # string
            expected = (Period("1/1/2000", ax.freq), Period("4/1/2000",
    ax.freq))
    ...
    def test_mixed_freq_shared_ax(self):
        ...
        assert ax1.freq == "M"
        assert ax2.freq == "M"
        assert ax1.lines[0].get_xydata()[0, 0] == ax2.lines[0].get_xydata()[0,
    0]
    ...
    def test_from_resampling_area_line_mixed(self):
        idxh = date_range("1/1/1999", periods=52, freq="W")
        idxl = date_range("1/1/1999", periods=12, freq="M")
        high = DataFrame(np.random.rand(len(idxh), 3), index=idxh, columns=[0,
    1, 2])
        low = DataFrame(np.random.rand(len(idxl), 3), index=idxl, columns=[0,
    1, 2])
    ...
    def test_time_musec(self):
        t = datetime(1, 1, 1, 3, 30, 0)
        deltas = np.random.randint(1, 20, 3).cumsum()
        ts = np.array([(t + timedelta(microseconds=int(x))).time() for x in
    deltas])
        df = DataFrame(
            {"a": np.random.randn(len(ts)), "b": np.random.randn(len(ts))},
    index=ts
    ...
    def test_irregular_ts_shared_ax_xlim(self):
        ...
        assert left <= DatetimeConverter.convert(ts_irregular.index.min(), "",
    ax)
        assert right >= DatetimeConverter.convert(ts_irregular.index.max(),
    "", ax)
    ...
    def test_timedelta_plot(self):
        ...
        # test long period
        index = timedelta_range("1 day 2 hr 30 min 10 s", periods=10, freq="1
    d")
    ...
    def test_add_matplotlib_datetime64(self):
        ...
        s = Series(np.random.randn(10), index=date_range("1970-01-02",
    periods=10))
        ax = s.plot()
    ...
    def test_matplotlib_scatter_datetime64(self):
        ...
        df = DataFrame(np.random.RandomState(0).rand(10, 2), columns=["x",
    "y"])
