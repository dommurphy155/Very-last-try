...
class TestDataFramePlots(TestPlotBase):
    def test_stacked_boxplot_set_axis(self):
        ...
        assert [int(x.get_text()) for x in ax.get_xticklabels()] ==
    df.index.to_list()
    ...
    def test_figsize(self):
        df = DataFrame(np.random.rand(10, 5), columns=["A", "B", "C", "D",
    "E"])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"boxes": "r"}, {"boxes": "r"}),
            ("r", {"boxes": "r", "whiskers": "r", "medians": "r", "caps":
    "r"}),
        ],
    )
    ...
    def test_boxplot_group_xlabel_ylabel(self, vert):
        ...
        for subplot in ax:
            target_label = subplot.get_xlabel() if vert else
    subplot.get_ylabel()
...
class TestDataFrameGroupByPlots(TestPlotBase):
    ...
    def test_grouped_box_return_type(self, hist_df):
        ...
        self._check_box_return_type(result, "dict", expected_keys=["Male",
    "Female"])
        ...
        for t in ["dict", "axes", "both"]:
            ...
            self._check_box_return_type(returned, t, expected_keys=["A", "B",
    "C"])
    ...
    def test_grouped_box_layout(self, hist_df):
        ...
        with pytest.raises(ValueError, match=msg):
            df.boxplot(column=["weight", "height"], by=df.gender, layout=(1,
    1))
    ...
    return_type="dict"
        ...
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            box = _check_plot_works(
                df.groupby("category").boxplot, column="height",
    return_type="dict"
            )
        ...
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            box = _check_plot_works(
                df.groupby("classroom").boxplot, column="height",
    return_type="dict"
            )
        ...
        # GH 5897
        axes = df.boxplot(
            column=["height", "weight", "category"], by="gender",
    return_type="axes"
        )
        self._check_axes_shape(self.plt.gcf().axes, axes_num=3, layout=(2, 2))
        ...
        box = df.boxplot(
            column=["height", "weight", "category"], by="gender", layout=(-1,
    1)
        )
    ...
    def test_grouped_box_multiple_axes(self, hist_df):
        ...
        with tm.assert_produces_warning(UserWarning):
            ...
            df.groupby("category").boxplot(column="height",
    return_type="axes", ax=axes)
            self._check_axes_shape(self.plt.gcf().axes, axes_num=4, layout=(2,
    2))
        ...
        with tm.assert_produces_warning(UserWarning):
            returned = df.groupby("classroom").boxplot(
                column=["height", "weight", "category"], return_type="axes",
    ax=axes[1]
            )
        returned = np.array(list(returned.values))
        self._check_axes_shape(returned, axes_num=3, layout=(1, 3))
        ...
        msg = "The number of passed axes must be 3, the same as the output
    plot"
        with pytest.raises(ValueError, match=msg):
            fig, axes = self.plt.subplots(2, 3)
    ...
    def test_boxplot_multiindex_column(self):
        ...
        df = DataFrame(np.random.randn(3, 8), index=["A", "B", "C"],
    columns=index)
