...
class TestSeriesPlots(TestPlotBase):
    ...
    def test_hist_layout_with_by(self, hist_df):
        ...
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            axes = _check_plot_works(df.height.hist, by=df.gender, layout=(2,
    1))
        self._check_axes_shape(axes, axes_num=2, layout=(2, 1))

        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            axes = _check_plot_works(df.height.hist, by=df.gender, layout=(3,
    -1))
    ...
    layout=(-1, 4))
        ...
        with tm.assert_produces_warning(UserWarning, check_stacklevel=False):
            axes = _check_plot_works(df.height.hist, by=df.classroom,
    layout=(2, 2))
        self._check_axes_shape(axes, axes_num=3, layout=(2, 2))

        axes = df.height.hist(by=df.category, layout=(4, 2), figsize=(12, 7))
        self._check_axes_shape(axes, axes_num=4, layout=(4, 2), figsize=(12,
    7))
    ...
    @pytest.mark.parametrize(
        "by, expected_axes_num, expected_layout", [(None, 1, (1, 1)), ("b", 2,
    (1, 2))]
    )
    def test_hist_with_legend(self, by, expected_axes_num, expected_layout):
        ...
        self._check_axes_shape(axes, axes_num=expected_axes_num,
    ...
    def test_hist_with_legend_raises(self, by):
        ...
        with pytest.raises(ValueError, match="Cannot use both legend and
    label"):
...
class TestDataFramePlots(TestPlotBase):
        ...
        df = DataFrame(
            {
                ...
                "c": to_datetime(
                    np.random.randint(
                        1582800000000000000, 1583500000000000000, 10,
    dtype=np.int64
                    )
                ),
                "d": to_datetime(
    ...
    dtype=np.int64
        ...
        msg = "hist method requires numerical or datetime columns, nothing to
    plot."
        with pytest.raises(ValueError, match=msg):
            df_o.hist()
    ...
    def test_histtype_argument(self, histtype, expected):
        # GH23992 Verify functioning of histtype argument
        df = DataFrame(np.random.randint(1, 10, size=(100, 2)), columns=["a",
    "b"])
    ...
    def test_hist_df_with_nonnumerics(self):
        ...
        with tm.RNGContext(1):
            df = DataFrame(np.random.randn(10, 4), columns=["A", "B", "C",
    "D"])
        df["E"] = ["x", "y"] * 5
        _, ax = self.plt.subplots()
        ax = df.plot.hist(bins=5, ax=ax)
...
class TestDataFrameGroupByPlots(TestPlotBase):
    ...
    def test_grouped_hist_layout(self, hist_df):
        ...
        axes = df.hist(column="height", by=df.category, layout=(4, 2),
    figsize=(12, 8))
        self._check_axes_shape(axes, axes_num=4, layout=(4, 2), figsize=(12,
    8))
        tm.close()
    ...
    def test_grouped_hist_multiple_axes(self, hist_df):
        ...
        msg = "The number of passed axes must be 1, the same as the output
    plot"
        with pytest.raises(ValueError, match=msg):
