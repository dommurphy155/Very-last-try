...
class TestDataFramePlots(TestPlotBase):
    ...
    index=list(string.ascii_letters[:10]))
        ...
        _check_plot_works(df.plot, default_axes=True, subplots=True,
    title="blah")
    ...
    index=MultiIndex.from_tuples(tuples))
        ...
        df = DataFrame(np.random.randint(0, 10, (8, 2)), columns=columns,
    index=index)
    ...
    def test_nullable_int_plot(self):
        ...
        _check_plot_works(df[["C", "A"]].plot, x="C", y="A")  # nullable value
    on x-axis
    ...
    def test_plot_xy(self):
        ...
        self._check_axes_shape(ax, axes_num=1, layout=(1, 1), figsize=(16.0,
    8.0))
    ...
    def test_unsorted_index_lims(self):
        ...
        df = DataFrame({"y": [0.0, 1.0, 2.0, 3.0], "z": [91.0, 90.0, 93.0,
    92.0]})
        ax = df.plot(x="z", y="y")
        xmin, xmax = ax.get_xlim()
        lines = ax.get_lines()
    ...
    def test_line_area_stacked(self, kind):
        with tm.RNGContext(42):
            ...
            msg = (
                "When stacked is True, each column must be either all positive
    or "
    ...
    index=tm.makeDateIndex(k=4))
        ...
        for d in [df, tdf]:
            ...
            tm.assert_numpy_array_equal(ax.lines[1].get_ydata(), expected1 +
    expected2)
    ...
    def test_bar_nan(self):
        df = DataFrame({"A": [10, np.nan, 20], "B": [5, 10, 20], "C": [1, 2,
    3]})
        ax = df.plot.bar()
    ...
    def test_plot_scatter(self):
        ...
        msg = re.escape("scatter() missing 1 required positional argument:
    'y'")
    ...
    def test_scatterplot_color_by_categorical(self, ordered, categories):
        ...
        result_yticklabels = [i.get_text() for i in
    colorbar.ax.get_ymajorticklabels()]
        assert all(i == j for i, j in zip(result_yticklabels,
    ...
    def test_plot_scatter_with_categorical_data(self, x, y):
        ...
        df = DataFrame({"x": [1, 2, 3, 4], "y": pd.Categorical(["a", "b", "a",
    "c"])})
    ...
    def test_plot_scatter_with_c(self):
        ...
        axes = [df.plot.scatter(x="x", y="y", c="z"), df.plot.scatter(x=0,
    y=1, c=2)]
        for ax in axes:
            # default to Greys
    ...
    def test_plot_scatter_with_s(self):
        ...
        df = DataFrame(np.random.random((10, 3)) * 100, columns=["a", "b",
    "c"])
    ...
    def test_hist_weights(self, weights):
        ...
        patch_height_with_weights = [patch.get_height() for patch in
    ax1.patches]

        # original heights with no weights, and we manually multiply with
    example
        # weights, so after multiplication, they should be almost same
        expected_patch_height = [0.1 * patch.get_height() for patch in
    ax2.patches]
    ...
    def test_hist_df_coord(self):
        normal_df = DataFrame(
            {
                "A": np.repeat(np.array([1, 2, 3, 4, 5]), np.array([10, 9, 8,
    7, 6])),
                "B": np.repeat(np.array([1, 2, 3, 4, 5]), np.array([8, 8, 8,
    8, 8])),
                "C": np.repeat(np.array([1, 2, 3, 4, 5]), np.array([6, 7, 8,
    9, 10])),
            },
            columns=["A", "B", "C"],
        )
        ...
        nan_df = DataFrame(
            {
                "A": np.repeat(
                    np.array([np.nan, 1, 2, 3, 4, 5]), np.array([3, 10, 9, 8,
    7, 6])
                ),
                "B": np.repeat(
                    np.array([1, np.nan, 2, 3, 4, 5]), np.array([8, 3, 8, 8,
    8, 8])
                ),
                "C": np.repeat(
                    np.array([1, 2, 3, np.nan, 4, 5]), np.array([6, 7, 8, 3,
    9, 10])
                ),
            },
            columns=["A", "B", "C"],
    ...
    @pytest.mark.parametrize("x,y,colnames", [(0, 1, ["A", "B"]), (1, 0, [0,
    1])])
    ...
    def test_errorbar_plot_iterator(self):
        with warnings.catch_warnings():
            ...
            # yerr is iterator
            ax = _check_plot_works(df.plot, yerr=itertools.repeat(0.1,
    len(df)))
            self._check_has_errorbars(ax, xerr=0, yerr=2)
    ...
    def test_errorbar_asymmetrical(self):
        ...
        msg = re.escape(
            "Asymmetrical error bars should be provided with the shape (3, 2,
    5)"
        )
        with pytest.raises(ValueError, match=msg):
            df.plot(yerr=err.T)
    ...
    def test_errorbar_scatter(self):
        ...
        df_err = DataFrame(
            np.abs(np.random.randn(5, 2)) / 5, index=range(5), columns=["x",
    "y"]
        )
    ...
    yerr=df_err)
        ...
        def _check_errorbar_color(containers, expected, has_err="has_xerr"):
            lines = []
            errs = [c.lines for c in ax.containers if getattr(c, has_err,
    False)][0]
            for el in errs:
                if is_list_like(el):
                    lines.extend(el)
    ...
    def test_df_gridspec_patterns(self):
        ...
        ts = Series(np.random.randn(10), index=date_range("1/1/2000",
    periods=10))
    ...
    sharey=True)
        ...
        df = DataFrame(np.random.randn(10, 4), index=ts.index,
    columns=list("ABCD"))
    ...
    def test_xlim_plot_line_correctly_in_mixed_plot_type(self):
        # test if xlim is set correctly when ax contains multiple different
    kinds
        # of plots, GH 27686
        fig, ax = self.plt.subplots()
    ...
    def test_group_subplot_multiindex_notimplemented(self):
        df = DataFrame(np.eye(2), columns=MultiIndex.from_tuples([(0, 1), (1,
    2)]))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("a", "each entry should be a list/tuple"),  # iterable of
    non-iterable
            ((1,), "each entry should be a list/tuple"),  # iterable of
    non-iterable
            (("a",), "each entry should be a list/tuple"),  # iterable of
    strings
        ],
    ...
    def test_group_subplot_invalid_column_name(self):
        ...
        with pytest.raises(ValueError, match=r"Column label\(s\)
    \['bad_name'\]"):
    ...
    def test_group_subplot_invalid_kind(self, kind):
        ...
        df = DataFrame(d)
        with pytest.raises(
            ValueError, match="When subplots is an iterable, kind must be one
    of"
        ):
