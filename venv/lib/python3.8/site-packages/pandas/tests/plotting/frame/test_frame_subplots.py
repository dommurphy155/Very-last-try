...
class TestDataFramePlotsSubplots(TestPlotBase):
    ...
    def test_subplots_warnings(self):
        ...
        with tm.assert_produces_warning(None):
            ...
            df = DataFrame(
                np.random.randn(100, 4), index=date_range("1/1/2000",
    periods=100)
            )
            df.plot(subplots=True, layout=(3, 2))
    ...
    index=list(string.ascii_letters[:10]))
            ...
            returned = df.plot(
                subplots=True, ax=axes, layout=(2, 1), sharex=False,
    sharey=False
            )
    ...
    sharey=False
            ...
            returned = df.plot(
                subplots=True, ax=axes, layout=(-1, 2), sharex=False,
    sharey=False
            )
        self._check_axes_shape(returned, axes_num=4, layout=(2, 2))
        assert returned.shape == (4,)
    ...
    def test_subplots_sharex_axes_existing_axes(self):
        # GH 9158
        d = {"A": [1.0, 2.0, 3.0, 4.0], "B": [4.0, 3.0, 2.0, 1.0], "C": [5, 1,
    3, 4]}
    ...
    ylabel
        axes = df.plot(kind=kind, ylabel=new_label, xlabel=new_label,
    subplots=True)
    ...
    def _check_bar_alignment(
        ...
        for ax in axes:
            ...
            if kind == "bar" and (stacked is True or subplots is True):
                ...
                center = p.get_y() + p.get_height() * len(df.columns) *
    position
                edge = p.get_y()
            else:
                raise ValueError
