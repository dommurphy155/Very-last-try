...
class TestFrameLegend(TestPlotBase):
    ...
    def test_legend_false(self):
        ...
        ax = df.plot(legend=True, color={"a": "blue", "b": "green"},
    secondary_y="b")
    ...
    index=ind)
        df2 = DataFrame(np.random.randn(3, 3), columns=["d", "e", "f"],
    index=ind)
        df3 = DataFrame(np.random.randn(3, 3), columns=["g", "h", "i"],
    index=ind)
        ax = df.plot(legend=True, secondary_y="b")
        self._check_legend_labels(ax, labels=["a", "b (right)", "c"])
        ax = df2.plot(legend=False, ax=ax)
        ...
        self._check_legend_labels(ax, labels=["a", "b (right)", "c", "g", "h",
    "i"])
    ...
    def test_missing_marker_multi_plots_on_same_ax(self):
        # GH 18222
        df = DataFrame(data=[[1, 1, 1, 1], [2, 2, 4, 8]], columns=["x", "r",
    "g", "b"])
    ...
    def test_legend_name(self):
        multi = DataFrame(
            np.random.randn(4, 4),
            columns=[np.array(["a", "a", "b", "b"]), np.array(["x", "y", "x",
    "y"])],
        )
