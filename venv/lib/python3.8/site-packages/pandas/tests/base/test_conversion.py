...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            np.array([pd.Period("2000", freq="D"), pd.Period("2001",
    freq="D")]),
        ),
        (pd.array([0, np.nan], dtype="Int64"), np.array([0, pd.NA],
    dtype=object)),
        (
    ...
    dtype="M8[ns]"
        ...
        (
            pd.Categorical(date_range("2016-01-01", periods=2,
    tz="US/Pacific")),
            np.array(
                [
...
def test_to_numpy(arr, expected, index_or_series_or_array, request):
    ...
    if arr.dtype.name == "int64" and box is pd.array:
        mark = pytest.mark.xfail(reason="thing is Int64 and to_numpy() returns
    object")
        request.node.add_marker(mark)
...
@pytest.mark.parametrize(
    "arr", [np.array([1, 2, 3], dtype="int64"), np.array(["a", "b", "c"],
    dtype=object)]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [Timestamp("2000"), Timestamp("2000"), pd.NaT],
            [(0, Timestamp("2021")), (0, Timestamp("2022")), (1,
    Timestamp("2000"))],
            None,
            Timestamp("2000"),
            [np.datetime64("2000-01-01T00:00:00.000000000")] * 3,
