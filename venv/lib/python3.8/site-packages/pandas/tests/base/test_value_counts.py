...
def test_value_counts_null(null_obj, index_or_series_obj):
    ...
    if obj.duplicated().any():
        # TODO(GH#32514):
        #  Order of entries with the same count is inconsistent on CI
    (gh-32449)
        with tm.maybe_produces_warning(
            PerformanceWarning,
            pa_version_under7p0 and getattr(obj.dtype, "storage", "") ==
...
def test_value_counts_datetime64(index_or_series):
    ...
    df = pd.DataFrame(
        {
            "person_id": ["xxyyzz", "xxyyzz", "xxyyzz", "xxyyww", "foofoo",
    "foofoo"],
            "dt": pd.to_datetime(
                [
                    "2010-01-01",
    ...
    expected_s = pd.concat([Series([4], index=DatetimeIndex([pd.NaT])),
    expected_s])
    tm.assert_series_equal(result, expected_s)
