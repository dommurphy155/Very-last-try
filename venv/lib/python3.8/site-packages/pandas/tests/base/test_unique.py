...
def test_unique(index_or_series_obj):
    ...
    with tm.maybe_produces_warning(
        PerformanceWarning,
        pa_version_under2p0 and str(index_or_series_obj.dtype) ==
    "string[pyarrow]",
    ):
        result = obj.unique()
    ...
    elif isinstance(obj, pd.Index) and
    obj._is_backward_compat_public_numeric_index:
...
def test_unique_null(null_obj, index_or_series_obj):
    ...
    unique_values_not_null = [val for val in unique_values_raw if not
    pd.isnull(val)]
    unique_values = [null_obj] + unique_values_not_null

    if isinstance(obj, pd.Index) and
    obj._is_backward_compat_public_numeric_index:
        expected = NumericIndex(unique_values, dtype=obj.dtype)
        tm.assert_index_equal(result, expected, exact=True)
    elif isinstance(obj, pd.Index):
