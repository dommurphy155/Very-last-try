...
def test_rolling_apply_invalid_raw(bad_raw):
    with pytest.raises(ValueError, match="raw parameter must be `True` or
    `False`"):
...
def test_rolling_apply_out_of_bounds(engine_and_raw):
    ...
    result = vals.rolling(10, min_periods=1).apply(np.sum, engine=engine,
    raw=raw)
...
def test_rolling_apply(engine_and_raw, step):
    ...
    result = s.rolling(2, min_periods=0, step=step).apply(len, engine=engine,
    raw=raw)
    tm.assert_series_equal(result, expected)
...
def test_ragged_apply(engine_and_raw):
    ...
    f = lambda x: 1
    result = df.rolling(window="1s", min_periods=1).apply(f, engine=engine,
    raw=raw)
    expected = df.copy()
    expected["B"] = 1.0
    tm.assert_frame_equal(result, expected)

    result = df.rolling(window="2s", min_periods=1).apply(f, engine=engine,
    raw=raw)
    expected = df.copy()
    expected["B"] = 1.0
    tm.assert_frame_equal(result, expected)

    result = df.rolling(window="5s", min_periods=1).apply(f, engine=engine,
    raw=raw)
    expected = df.copy()
    expected["B"] = 1.0
    tm.assert_frame_equal(result, expected)
...
def test_invalid_engine():
    with pytest.raises(ValueError, match="engine must be either 'numba' or
    'cython'"):
        Series(range(1)).rolling(1).apply(lambda x: x, engine="foo")


def test_invalid_engine_kwargs_cython():
    with pytest.raises(ValueError, match="cython engine does not accept
    engine_kwargs"):
...
def test_invalid_raw_numba():
    with pytest.raises(
        ...
        Series(range(1)).rolling(1).apply(lambda x: x, raw=False,
    engine="numba")
...
def test_rolling_apply_args_kwargs(args_kwargs):
    ...
    result = df.rolling(1).apply(foo, args=args_kwargs[0],
...
def test_min_periods(raw, series, minp, step):
    result = series.rolling(len(series) + 1, min_periods=minp,
    step=step).apply(
        f, raw=raw
    )
