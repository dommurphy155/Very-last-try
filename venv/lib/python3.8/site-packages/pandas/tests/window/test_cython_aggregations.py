...
def _get_rolling_aggregations():
    ...
    named_roll_aggs = (
        ...
        + [
            (f"roll_var({ddof})", partial(window_aggregations.roll_var,
    ddof=ddof))
            for ddof in [0, 1]
        ]
        + [
...
def test_rolling_aggregation_boundary_consistency(rolling_aggregation):
    ...
    result = Series(rolling_aggregation(values, start[selarr], end[selarr],
    minp))
...
def test_rolling_aggregation_with_unused_elements(rolling_aggregation):
    ...
    assert np.isfinite(expected.values).all(), "Not all expected values are
    finite"
    tm.assert_equal(expected, result)
