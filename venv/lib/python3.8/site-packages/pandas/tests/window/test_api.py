...
def test_getitem(step):
    ...
    tm.assert_index_equal(r._selected_obj.columns, frame[::step].columns[[1,
    3]])
...
def test_skip_sum_object_raises(step):
    df = DataFrame({"A": range(5), "B": range(5, 10), "C": "foo"})
    r = df.rolling(window=3, step=step)
    msg = r"nuisance columns.*Dropped columns were Index\(\['C'\],
    dtype='object'\)"
...
def test_agg(step):
    ...
    with pytest.raises(SpecificationError, match=msg):
        r.aggregate(
            {"A": {"mean": "mean", "sum": "sum"}, "B": {"mean2": "mean",
    "sum2": "sum"}}
        )
...
def test_agg_nested_dicts():
    ...
    msg = "nested renamer is not supported"
    with pytest.raises(SpecificationError, match=msg):
        r.aggregate({"r1": {"A": ["mean", "sum"]}, "r2": {"B": ["mean",
    "sum"]}})
    ...
    with pytest.raises(SpecificationError, match=msg):
        r[["A", "B"]].agg({"A": {"ra": ["mean", "std"]}, "B": {"rb": ["mean",
    "std"]}})
...
def test_count_nonnumeric_types(step):
    ...
    dt_nat_col = [Timestamp("20170101"), Timestamp("20170203"),
    Timestamp(None)]
...
def test_centered_axis_validation(step):
    ...
    # ok
    Series(np.ones(10)).rolling(window=3, center=True, axis=0,
    step=step).mean()

    # bad axis
    msg = "No axis named 1 for object type Series"
    with pytest.raises(ValueError, match=msg):
        Series(np.ones(10)).rolling(window=3, center=True, axis=1,
    step=step).mean()
...
def test_args_kwargs_depr(roll_type, class_name, kernel, has_args, raises):
    ...
    if roll_type == "ewm" and kernel not in (
        ...
        with pytest.raises(AttributeError, match=f"has no attribute
    '{kernel}'"):
            getattr(r, kernel)
    else:
    ...
    match=error_msg):
            ...
            warn_msg = f"Passing additional args and kwargs to
    {class_name}.{kernel}"
            with tm.assert_produces_warning(FutureWarning, match=warn_msg):
                if raises:
                    with pytest.raises(UnsupportedFunctionCall,
