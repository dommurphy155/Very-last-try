...
def test_expanding_apply_consistency_sum_nans(request, all_data, min_periods,
    f):
    if f is np.sum:
        if not no_nans(all_data) and not (
            ...
            request.node.add_marker(
                pytest.mark.xfail(reason="np.sum has different behavior with
    NaNs")
            )
...
def test_moments_consistency_var(all_data, min_periods, ddof):
    ...
    if ddof == 0:
        ...
        mean_x2 = (all_data *
    all_data).expanding(min_periods=min_periods).mean()
...
def test_expanding_consistency_var_std_cov(all_data, min_periods, ddof):
    ...
    cov_x_x = all_data.expanding(min_periods=min_periods).cov(all_data,
    ddof=ddof)
...
def test_expanding_consistency_series_cov_corr(series_data, min_periods, ddof):
    var_x_plus_y = (
        (series_data +
    series_data).expanding(min_periods=min_periods).var(ddof=ddof)
    )
    ...
    cov_x_y = series_data.expanding(min_periods=min_periods).cov(series_data,
    ddof=ddof)
    ...
    if ddof == 0:
        ...
        mean_x_times_y = (
            (series_data *
    series_data).expanding(min_periods=min_periods).mean()
        )
...
def test_expanding_consistency_constant(consistent_data, min_periods):
    ...
    corr_x_x =
    consistent_data.expanding(min_periods=min_periods).corr(consistent_data)
