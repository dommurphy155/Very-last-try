        ...
        for i in range(len(s)):
            if s.iat[i] == s.iat[i]:
                if prev_i == -1:
                    ...
                    w.iat[i] = alpha * sum_wts / pow(1.0 - alpha, count -
    prev_i)
                sum_wts += w.iat[i]
                prev_i = count
                count += 1
...
def test_ewm_consistency_mean(all_data, adjust, ignore_na, min_periods):
    ...
    weights = create_mock_weights(all_data, com=com, adjust=adjust,
    ignore_na=ignore_na)
    expected = (
    ...
    expected[
        all_data.expanding().count() < (max(min_periods, 1) if min_periods
    else 1)
    ] = np.nan
    tm.assert_equal(result, expected.astype("float64"))


def test_ewm_consistency_consistent(consistent_data, adjust, ignore_na,
    min_periods):
    com = 3.0
...
def test_moments_consistency_var(all_data, adjust, ignore_na, min_periods,
    bias):
    com = 3.0
    ...
    if bias:
        ...
        mean_x2 = (
            (all_data * all_data)
            .ewm(com=com, min_periods=min_periods, adjust=adjust,
    ignore_na=ignore_na)
            .mean()
        )
...
def test_ewm_consistency_series_cov_corr(
    ...
    # std(y))
    with tm.assert_produces_warning(FutureWarning, match="Passing additional
    kwargs"):
        corr_x_y = series_data.ewm(
            com=com, min_periods=min_periods, adjust=adjust,
    ignore_na=ignore_na
