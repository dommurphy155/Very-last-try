...
def test_freq_window_not_implemented(window):
    ...
    with pytest.raises(
        NotImplementedError, match="step is not supported with frequency
    windows"
    ):
...
def test_closed_fixed(closed, arithmetic_win_operators):
    ...
    df_time = DataFrame({"A": [0, 1, 2, 3, 4]}, index=date_range("2020",
    periods=5))
...
def test_datetimelike_centered_selections(
    ...
    expected = DataFrame(
        {"A": [getattr(df_time["A"].iloc[s], func_name)() for s in
    window_selections]},
...
def test_even_number_window_alignment():
    ...
    s = Series(range(3), index=date_range(start="2020-01-01", freq="D",
    periods=3))
...
def test_closed_empty(closed, arithmetic_win_operators):
    # GH 26005
    func_name = arithmetic_win_operators
    ser = Series(data=np.arange(5), index=date_range("2000", periods=5,
    freq="2D"))
...
def test_closed_one_entry_groupby(func):
    ...
    exp_idx = MultiIndex.from_arrays(arrays=[[1, 1, 2], ser.index],
    names=("A", None))
...
def test_missing_minp_zero_variable():
    ...
    x = Series(
        [np.nan] * 4,
        index=DatetimeIndex(["2017-01-01", "2017-01-04", "2017-01-06",
    "2017-01-07"]),
    )
...
def test_multi_index_names():
    ...
    # GH 16789, 16825
    cols = MultiIndex.from_product([["A", "B"], ["C", "D", "E"]], names=["1",
    "2"])
...
def test_rolling_datetime(axis_frame, tz_naive_fixture):
    ...
    tz = tz_naive_fixture
    df = DataFrame(
        {i: [1] * 2 for i in date_range("2019-8-01", "2019-08-03", freq="D",
    tz=tz)}
    )
...
def test_rolling_window_as_string(center, expected_data):
    ...
    result = df.rolling(window="21D", min_periods=2, closed="left",
    center=center)[
        "metric"
    ].agg("max")
...
def test_iter_rolling_on_dataframe(expected, window):
    ...
    expected = [
        DataFrame(values, index=df.loc[index, "C"]) for (values, index) in
    expected
    ]
...
def test_iter_rolling_datetime(expected, expected_index, window):
    # GH 11704
    ser = Series(range(5), index=date_range(start="2020-01-01", periods=5,
    freq="D"))

    expected = [
        Series(values, index=idx) for (values, idx) in zip(expected,
    expected_index)
    ]
...
def test_rolling_positional_argument(grouping, _index, raw):
    ...
    result = df.groupby(**grouping).rolling(1).apply(scaled_sum, raw=raw,
    args=(2,))
    tm.assert_frame_equal(result, expected)
...
def test_rolling_numerical_accuracy_kahan_mean(add):
    ...
    result = (
        df.resample("1s").ffill().rolling("3s", closed="left",
    min_periods=3).mean()
    )
...
def test_rolling_numerical_accuracy_jump():
    # GH: 32761
    index = date_range(start="2020-01-01", end="2020-01-02",
    freq="60s").append(
...
def test_rolling_on_df_transposed():
    ...
    expected = DataFrame({"A": [1.0, np.nan], "B": [5.0, 5.0], "C": [11.0,
    13.0]})
...
@pytest.mark.parametrize(
    ...
    [
        (
            period_range(start="2020-01-01 08:00", end="2020-01-01 08:08",
    freq="T"),
            "2T",
        ),
        (
            period_range(start="2020-01-01 08:00", end="2020-01-01 12:00",
    freq="30T"),
            "1h",
        ),
    ],
...
@pytest.mark.xfail(is_platform_arm() and not is_platform_mac(), reason="GH
    38921")
@pytest.mark.parametrize(
...
@pytest.mark.parametrize("method", ["var", "sum", "mean", "skew", "kurt",
    "min", "max"])
def test_rolling_decreasing_indices(method):
    """
    Make sure that decreasing indices give the same results as increasing
    indices.

    GH 36933
    """
    df = DataFrame({"values": np.arange(-15, 10) ** 2})
    df_reverse = DataFrame({"values": df["values"][::-1]},
    index=df.index[::-1])
...
def test_rolling_decreasing_indices_centered(window, closed, expected,
    frame_or_series):
    """
    Ensure that a symmetrical inverted index return same result as
    non-inverted.
    """
    #  GH 43927
...
def test_rolling_non_monotonic(method, expected):
    ...
    output from 1.1.3 is assumed to be the expected output. Output of sum/mean
    has
...
def test_rolling_numeric_dtypes():
    # GH#41779
    df = DataFrame(np.arange(40).reshape(4, 10),
    columns=list("abcdefghij")).astype(
        {
            "a": "float16",
            "b": "float32",
            "c": "float64",
...
def test_rank(window, method, pct, ascending, test_data):
    ...
    if test_data == "default":
        ...
        ser = Series(
            data=np.random.choice([1.0, 0.25, 0.75, np.nan, np.inf, -np.inf],
    length)
        )
...
def test_invalid_quantile_value():
    ...
    with pytest.raises(ValueError, match=msg):
        s.rolling(len(data), min_periods=1).quantile(0.5,
...
def test_rolling_var_same_value_count_logic(values, window, min_periods,
    expected):
    # GH 42064.
