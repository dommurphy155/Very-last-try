...
@pytest.fixture(
    params=[
        ...
        DataFrame([[2.0, 4.0], [1.0, 2.0], [5.0, 2.0], [8.0, 1.0]],
    columns=[1, 0.0]),
...
def test_rolling_pairwise_cov_corr(func, frame):
    ...
    expected = getattr(frame[1].rolling(window=10, min_periods=5),
    func)(frame[5])
...
def test_flex_binary_frame(method, frame):
    ...
    exp = DataFrame(
        {k: getattr(frame[k].rolling(window=10), method)(frame2[k]) for k in
    frame}
    )
...
def test_rolling_functions_window_non_shrinkage_binary(f):
    ...
    df_expected = DataFrame(
        columns=Index(["A", "B"], name="foo"),
        index=MultiIndex.from_product([df.index, df.columns], names=["bar",
    "foo"]),
        dtype="float64",
    )
    df_result = f(df)
...
def test_moment_functions_zero_length_pairwise(f):
    ...
    df1 = DataFrame()
    df2 = DataFrame(columns=Index(["a"], name="foo"), index=Index([],
    name="bar"))
    ...
    df2_expected = DataFrame(
        index=MultiIndex.from_product([df2.index, df2.columns], names=["bar",
    "foo"]),
...
class TestPairwise:
    ...
    def test_pairwise_with_self(self, pairwise_frames, pairwise_target_frame,
    f):
    ...
    def test_no_pairwise_with_self(self, pairwise_frames,
    pairwise_target_frame, f):
    ...
    def test_no_pairwise_with_other(self, pairwise_frames,
    pairwise_other_frame, f):
        ...
        if result is not None:
            with warnings.catch_warnings(record=True):
                ...
                expected_index =
    pairwise_frames.index.union(pairwise_other_frame.index)
            ...
            with pytest.raises(ValueError, match="'arg1' columns are not
    unique"):
                f(pairwise_frames, pairwise_other_frame)
            with pytest.raises(ValueError, match="'arg2' columns are not
    unique"):
    ...
    def test_pairwise_with_series(self, pairwise_frames,
    pairwise_target_frame, f):
    ...
    def test_cov_mulittindex(self):
        ...
        index = range(3)
        df = DataFrame(np.arange(24).reshape(3, 8), index=index,
    columns=columns)

        result = df.ewm(alpha=0.1).cov()

        index = MultiIndex.from_product([range(3), list("ab"), list("xy"),
    list("AB")])
        columns = MultiIndex.from_product([list("ab"), list("xy"), list("AB")])
        expected = DataFrame(
            np.vstack(
    ...
    def test_multindex_columns_pairwise_func(self):
        # GH 21157
        columns = MultiIndex.from_arrays([["M", "N"], ["P", "Q"]], names=["a",
    "b"])
