...
def test_window_with_args(step):
    ...
    result = r.aggregate([lambda x: x.mean(std=10), lambda x:
    x.mean(std=0.01)])
...
def test_win_type_with_method_invalid():
    with pytest.raises(
        NotImplementedError, match="'single' is the only supported method
    type."
    ):
...
def test_win_type_not_implemented():
    ...
    with pytest.raises(NotImplementedError, match="BaseIndexer subclasses
    not"):
...
def test_cmov_window(step):
    ...
    vals = np.array([6.95, 15.21, 4.72, 9.12, 13.81, 13.49, 16.68, 9.48,
    10.63, 14.48])
    result = Series(vals).rolling(5, win_type="boxcar", center=True,
    step=step).mean()
    expected_values = [
        np.nan,
        np.nan,
...
def test_cmov_window_regular(win_types, step):
    ...
    rs = Series(vals).rolling(5, win_type=win_types, center=True,
    step=step).mean()
    tm.assert_series_equal(xp, rs)
...
def test_cmov_window_regular_missing_data(win_types, step):
    ...
    xp = Series(xps[win_types])[::step]
    rs = Series(vals).rolling(5, win_type=win_types, min_periods=3,
    step=step).mean()
    tm.assert_series_equal(xp, rs)
...
def test_rolling_center_axis_1():
    df = DataFrame(
        {"a": [1, 1, 0, 0, 0, 1], "b": [1, 0, 0, 1, 0, 0], "c": [1, 0, 0, 1,
    0, 1]}
    )
    ...
    expected = DataFrame(
        {"a": [np.nan] * 6, "b": [3.0, 1.0, 0.0, 2.0, 0.0, 2.0], "c": [np.nan]
    * 6}
    )
