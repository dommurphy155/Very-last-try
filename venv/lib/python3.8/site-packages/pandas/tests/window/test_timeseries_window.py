...
class TestRollingTS:
    ...
    def test_invalid_window_non_int(self, regular):
        ...
        # not a valid freq
        msg = "passed window foobar is not compatible with a datetimelike
    index"
    ...
    def test_frame_on(self):
        ...
        df = DataFrame(
            {"B": range(5), "C": date_range("20130101 09:00:00", periods=5,
    freq="3s")}
        )
    ...
    df.set_index("A").rolling("2s").B.sum().reset_index(drop=True)
        ...
        # original
        expected = (
            df.set_index("A").rolling("2s")[["B"]].sum().reset_index()[["B",
    "A"]]
        )
    ...
    def test_regular_min(self):
        ...
        df = DataFrame(
            {"A": date_range("20130101", periods=5, freq="s"), "B": [0.0, 1,
    2, 3, 4]}
        ).set_index("A")
        ...
        df = DataFrame(
            {"A": date_range("20130101", periods=5, freq="s"), "B": [5, 4, 3,
    4, 5]}
        ).set_index("A")
    ...
    def test_perf_min(self):
        ...
        dfp = DataFrame(
            {"B": np.random.randn(N)}, index=date_range("20130101", periods=N,
    freq="s")
        )
    ...
    def test_freqs_ops(self, freq, op, result_data):
        # GH 21096
        index = date_range(start="2018-1-1 01:00:00", freq=f"1{freq}",
    periods=10)
    ...
    def test_all2(self, arithmetic_win_operators):
        ...
        df = DataFrame(
            {"B": np.arange(50)}, index=date_range("20130101", periods=50,
    freq="H")
        )
        # in-range data
        ...
        expected = (
            df.groupby(df.index.day).apply(agg_by_day).reset_index(level=0,
    drop=True)
        )
    ...
    def test_rolling_on_multi_index_level(self):
        ...
        df = DataFrame(
            ...
            index=MultiIndex.from_product(
                [date_range("20190101", periods=3), range(2)], names=["date",
    "seq"]
            ),
        )
