...
def test_ewma_halflife_not_correct_type():
    ...
    with pytest.raises(ValueError, match=msg):
        Series(range(5)).ewm(halflife=1,
    times=np.arange(5).astype("datetime64[ns]"))


def test_ewma_halflife_without_times(halflife_with_times):
    msg = "halflife can only be a timedelta convertible argument if times is
    not None."
    with pytest.raises(ValueError, match=msg):
...
def test_ewma_with_times_equal_spacing(halflife_with_times, times,
    min_periods):
    halflife = halflife_with_times
    data = np.arange(10.0)
    data[::2] = np.nan
    df = DataFrame({"A": data, "time_col": date_range("2000", freq="D",
    periods=10)})
...
def test_ewm_with_nat_raises(halflife_with_times):
    ...
    with pytest.raises(ValueError, match="Cannot convert NaT values to
    integer"):
...
def test_times_string_col_deprecated():
    ...
    with tm.assert_produces_warning(FutureWarning, match="Specifying times"):
        result = df.ewm(halflife="1 day", min_periods=0,
    times="time_col").mean()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            True,
            False,
            [np.nan, (1.0 - (1.0 / (1.0 + 2.0))) ** 3, np.nan, np.nan, 1.0,
    np.nan],
        ),
        (
            Series([np.nan, 1.0, np.nan, np.nan, 101.0, np.nan]),
...
def test_ew_min_periods(min_periods, name):
    ...
    result = getattr(Series(dtype=object).ewm(com=50,
...
def test_numeric_only_series(arithmetic_win_operators, numeric_only, dtype):
    ...
    if numeric_only and dtype is object:
        msg = f"ExponentialMovingWindow.{kernel} does not implement
    numeric_only"
