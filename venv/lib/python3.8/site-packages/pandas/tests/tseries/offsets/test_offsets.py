...
class TestCommon(Base):
    ...
    normalize=False):
        ...
        if exp_warning is None and funcname == "_apply":
            ...
            with tm.assert_produces_warning(FutureWarning, match="apply is
    deprecated"):
    ...
    def test_add_dt64_ndarray_non_nano(self, offset_types, unit, request):
        ...
        if len(w):
            ...
            mark = pytest.mark.xfail(
                reason="Goes through object dtype in
    DatetimeArray._add_offset, "
...
class TestDateOffset(Base):
        ...
        if kwd == "millisecond":
            request.node.add_marker(
                pytest.mark.xfail(
                    ...
                    reason="Constructing DateOffset object with `millisecond`
    is not "
                    "yet supported.",
                )
            )
    ...
    def test_add(self, arithmatic_offset_type, expected):
        assert DateOffset(**{arithmatic_offset_type: 1}) + self.d ==
    Timestamp(expected)
        assert self.d + DateOffset(**{arithmatic_offset_type: 1}) ==
    Timestamp(expected)
    ...
    def test_sub(self, arithmatic_offset_type, expected):
        assert self.d - DateOffset(**{arithmatic_offset_type: 1}) ==
    Timestamp(expected)
        with pytest.raises(TypeError, match="Cannot subtract datetime from
    offset"):
    ...
    def test_mul_add(self, arithmatic_offset_type, n, expected):
        assert DateOffset(**{arithmatic_offset_type: 1}) * n + self.d ==
    Timestamp(
            expected
        )
        assert n * DateOffset(**{arithmatic_offset_type: 1}) + self.d ==
    Timestamp(
            expected
        )
        assert self.d + DateOffset(**{arithmatic_offset_type: 1}) * n ==
    Timestamp(
            expected
        )
        assert self.d + n * DateOffset(**{arithmatic_offset_type: 1}) ==
    Timestamp(
            expected
        )
...
class TestReprNames:
    def test_str_for_named_is_name(self):
        ...
        names += ["WOM-" + week + day for week in ("1", "2", "3", "4") for day
    in days]
        _offset_map.clear()
        for name in names:
            offset = _get_offset(name)
...
def test_valid_relativedelta_kwargs(kwd, request):
    if kwd == "millisecond":
        request.node.add_marker(
            pytest.mark.xfail(
                raises=NotImplementedError,
                reason="Constructing DateOffset object with `millisecond` is
    not "
                "yet supported.",
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DateOffset(nanoseconds=3), Timestamp("2022-01-01
    00:00:00.000000003")),
    ],
)
