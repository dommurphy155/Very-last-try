...
def test_repr(dtype):
    ...
    arr_name = "ArrowStringArray" if dtype.storage == "pyarrow" else
    "StringArray"
...
def test_astype_roundtrip(dtype, request):
    if dtype.storage == "pyarrow":
        ...
        mark = pytest.mark.xfail(
            reason="GH#36153 casting from StringArray to dt64 fails",
    raises=ValueError
        )
        request.node.add_marker(mark)
...
def test_add_sequence(dtype, request):
    if dtype.storage == "pyarrow":
        reason = "unsupported operand type(s) for +: 'ArrowStringArray' and
    'list'"
        mark = pytest.mark.xfail(raises=NotImplementedError, reason=reason)
        request.node.add_marker(mark)
...
def test_comparison_methods_scalar_not_string(comparison_op, dtype):
    ...
    result = getattr(a, op_name)(other)
    expected_data = {"__eq__": [False, None, False], "__ne__": [True, None,
    True]}[
        op_name
    ]
...
def test_constructor_raises(cls):
    if cls is pd.arrays.StringArray:
        ...
        msg = "Unsupported type '<class 'numpy.ndarray'>' for
    ArrowExtensionArray"
    ...
    if cls is pd.arrays.StringArray:
        # GH#45057 np.nan and None do NOT raise, as they are considered valid
    NAs
        #  for string dtype
...
@pytest.mark.parametrize("na", [np.nan, np.float64("nan"), float("nan"), None,
    pd.NA])
...
def test_astype_int(dtype):
    ...
    msg = r"int\(\) argument must be a string, a bytes-like object or a(
    real)? number"
...
def test_min_max_numpy(method, box, dtype, request):
    if dtype.storage == "pyarrow" and (pa_version_under6p0 or box is pd.array):
        if box is pd.array:
            reason = "'<=' not supported between instances of 'str' and
    'NoneType'"
        else:
            reason = "'ArrowStringArray' object has no attribute 'max'"
        mark = pytest.mark.xfail(raises=TypeError, reason=reason)
