...
class TestCategoricalAnalytics:
    ...
    @pytest.mark.parametrize(
        "values, categories",
        [(["a", "b", "c", np.nan], list("cba")), ([1, 2, 3, np.nan], [3, 2,
    1])],
    )
    ...
    def test_numpy_min_max_unsupported_kwargs_raises(self, method, kwarg):
        ...
        msg = (
            f"the '{kwarg}' parameter is not supported in the pandas
    implementation "
            f"of {method}"
        )
        if kwarg == "axis":
    ...
    def test_validate_inplace_raises(self, value):
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.assert_produces_warning(FutureWarning):
                # issue #37643 inplace kwarg deprecated
                cat.reorder_categories(["X", "Y", "Z"], ordered=True,
    inplace=value)

        with pytest.raises(ValueError, match=msg):
            with tm.assert_produces_warning(FutureWarning):
                # issue #37643 inplace kwarg deprecated
                cat.add_categories(new_categories=["D", "E", "F"],
    inplace=value)
