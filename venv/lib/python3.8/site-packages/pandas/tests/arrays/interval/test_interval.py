...
@pytest.fixture(
    params=[
        ...
        (timedelta_range("0 days", periods=3), timedelta_range("1 day",
    periods=3)),
...
class TestSetitem:
    def test_set_na(self, left_right_dtypes):
        ...
        if result.dtype.subtype.kind not in ["m", "M"]:
            msg = "'value' should be an interval type, got <.*NaTType'>
    instead."
...
def test_arrow_array():
    ...
    assert result.storage.field("left").equals(pa.array([1, 2, 3, 4],
    type="int64"))
    assert result.storage.field("right").equals(pa.array([2, 3, 4, 5],
    type="int64"))
...
def test_arrow_array_missing():
    ...
    expected = pa.StructArray.from_pandas(vals, mask=np.array([False, True,
    False]))
    assert result.storage.equals(expected)
...
def test_interval_index_subtype(timezone, inclusive_endpoints_fixture):
    ...
    dtype = f"interval[datetime64[ns, {timezone}],
