...
class TestNonNano:
    ...
    @pytest.mark.filterwarnings("ignore:Converting to
    PeriodArray.*:UserWarning")
...
class TestDatetimeArray:
    def test_astype_non_nano_tznaive(self):
        ...
        res = dta.astype("M8[s]")
        assert res.dtype == "M8[s]"
        assert isinstance(res, pd.core.arrays.DatetimeArray)  # used to be
    ndarray
    ...
    @pytest.mark.parametrize("dtype", ["datetime64[ns]", "datetime64[ns,
    UTC]"])
    @pytest.mark.parametrize(
        "other", ["datetime64[ns]", "datetime64[ns, UTC]", "datetime64[ns,
    CET]"]
    )
    ...
    @pytest.mark.parametrize("dtype", [int, np.int32, np.int64, "uint32",
    "uint64"])
    ...
    dtype=DatetimeTZDtype(tz="US/Central"))
        with pytest.raises(TypeError, match="Cannot compare tz-naive and
    tz-aware"):
