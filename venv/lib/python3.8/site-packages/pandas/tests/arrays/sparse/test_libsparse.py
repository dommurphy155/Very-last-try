...
CASES = [plain_case, delete_blocks, split_blocks, skip_block, no_intersect,
    one_empty]
IDS = [
    "plain_case",
    "delete_blocks",
...
class TestSparseIndexUnion:
    ...
    def test_index_make_union(self, xloc, xlen, yloc, ylen, eloc, elen):
        ...
        tm.assert_numpy_array_equal(bresult.blocs, np.array(eloc,
    dtype=np.int32))
        tm.assert_numpy_array_equal(bresult.blengths, np.array(elen,
    dtype=np.int32))
...
class TestSparseIndexIntersect:
    @td.skip_if_windows
    @pytest.mark.parametrize("xloc, xlen, yloc, ylen, eloc, elen", CASES,
    ids=IDS)
    ...
    @pytest.mark.parametrize(
        "case",
        [
            IntIndex(5, np.array([1, 2], dtype=np.int32)),  # type:
    ignore[arg-type]
            IntIndex(5, np.array([0, 2, 4], dtype=np.int32)),  # type:
    ignore[arg-type]
            IntIndex(0, np.array([], dtype=np.int32)),  # type:
    ignore[arg-type]
            IntIndex(5, np.array([], dtype=np.int32)),  # type:
    ignore[arg-type]
        ],
    )
...
class TestSparseIndexCommon:
    def test_int_internal(self):
        idx = make_sparse_index(4, np.array([2, 3], dtype=np.int32),
    kind="integer")
        assert isinstance(idx, IntIndex)
        assert idx.npoints == 2
        tm.assert_numpy_array_equal(idx.indices, np.array([2, 3],
    dtype=np.int32))

        idx = make_sparse_index(4, np.array([], dtype=np.int32),
    kind="integer")
        assert isinstance(idx, IntIndex)
        ...
        tm.assert_numpy_array_equal(idx.indices, np.array([0, 1, 2, 3],
    dtype=np.int32))
    ...
    dtype=np.int32))
        ...
        idx = make_sparse_index(4, np.array([0, 1, 2, 3], dtype=np.int32),
    kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 4
    ...
    dtype=np.int32))
        ...
        idx = make_sparse_index(4, np.array([0, 2, 3], dtype=np.int32),
    kind="block")
        assert isinstance(idx, BlockIndex)
        assert idx.npoints == 3
        tm.assert_numpy_array_equal(idx.blocs, np.array([0, 2],
    dtype=np.int32))
        tm.assert_numpy_array_equal(idx.blengths, np.array([1, 2],
    dtype=np.int32))
...
class TestBlockIndex:
    ...
    def test_make_block_boundary(self, i):
        idx = make_sparse_index(i, np.arange(0, i, 2, dtype=np.int32),
    kind="block")

        exp = np.arange(0, i, 2, dtype=np.int32)
        tm.assert_numpy_array_equal(idx.blocs, exp)
        tm.assert_numpy_array_equal(idx.blengths, np.ones(len(exp),
    dtype=np.int32))
    ...
    def test_to_int_index(self):
        ...
        tm.assert_numpy_array_equal(dense.indices, np.array(exp_inds,
    dtype=np.int32))
