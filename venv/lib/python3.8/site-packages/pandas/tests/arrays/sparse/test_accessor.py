...
class TestSeriesAccessor:
    ...
    @pytest.mark.parametrize("attr", ["npoints", "density", "fill_value",
    "sp_values"])
...
class TestFrameAccessor:
    ...
    def test_from_spmatrix(self, format, labels, dtype):
        ...
        result = pd.DataFrame.sparse.from_spmatrix(mat, index=labels,
    columns=labels)
    ...
    @pytest.mark.parametrize(
        "columns",
        [["a", "b"], pd.MultiIndex.from_product([["A"], ["a", "b"]]), ["a",
    "a"]],
    )
    @td.skip_if_no_scipy
    ...
    @pytest.mark.parametrize(
        "colnames", [("A", "B"), (1, 2), (1, pd.NA), (0.1, 0.2), ("x", "x"),
    (0, 0)]
    )
    @td.skip_if_no_scipy
    ...
    def test_series_from_coo(self, dtype, dense_index):
        ...
        expected = pd.Series(SparseArray(np.array([1, 1, 1], dtype=dtype)),
    index=index)
        if dense_index:
            expected =
    expected.reindex(pd.MultiIndex.from_product(index.levels))
