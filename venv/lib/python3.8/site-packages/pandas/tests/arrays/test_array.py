...
@pytest.mark.parametrize(
    ...
    PandasArray
        ...
        (
            ...
            DatetimeArray._from_sequence(np.array([1, 2],
    dtype="datetime64[ns]")),
        ),
        (
    ...
    dtype=np.int64))),
        ...
        (
            ["a", None],
            "string",
            pd.StringDtype().construct_array_type()._from_sequence(["a",
    None]),
        ),
        (
            ["a", None],
    ...
    None]),
        ...
        ([True, None], pd.BooleanDtype(), BooleanArray._from_sequence([True,
    None])),
        # Index
        (pd.Index([1, 2]), None, PandasArray(np.array([1, 2],
    dtype=np.int64))),
        ...
        # "3rd party" EAs work
        ([decimal.Decimal(0), decimal.Decimal(1)], "decimal", to_decimal([0,
    1])),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # interval
        ([pd.Interval(0, 1), pd.Interval(1, 2)], IntervalArray.from_breaks([0,
    1, 2])),
        # datetime
        (
...
def test_bounds_check():
    # GH21796
    with pytest.raises(
        TypeError, match=r"cannot safely cast non-equivalent int(32|64) to
    uint16"
    ):
...
class DecimalArray2(DecimalArray):
    ...
    def _from_sequence(cls, scalars, dtype=None, copy=False):
        if isinstance(scalars, (pd.Series, pd.Index)):
            raise TypeError("scalars should not be of type pd.Series or
    pd.Index")
