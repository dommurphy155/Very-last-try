...
def data():
    ...
    return pd.array(
        [True, False] * 4 + [np.nan] + [True, False] * 44 + [np.nan] + [True,
    False],
        dtype="boolean",
    )
...
def test_any_all(values, exp_any, exp_all, exp_any_noskip, exp_all_noskip):
    ...
    exp_any_noskip = pd.NA if exp_any_noskip is pd.NA else
    np.bool_(exp_any_noskip)
    exp_all_noskip = pd.NA if exp_all_noskip is pd.NA else
    np.bool_(exp_all_noskip)
