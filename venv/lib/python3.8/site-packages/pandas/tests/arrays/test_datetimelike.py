...
def period_index(freqstr):
    ...
    pi = pd.period_range(start=Timestamp("2000-01-01"), periods=100,
    freq=freqstr)
    return pi
...
def datetime_index(freqstr):
    ...
    dti = pd.date_range(start=Timestamp("2000-01-01"), periods=100,
    freq=freqstr)
    return dti
...
class SharedTests:
    ...
    @pytest.mark.parametrize("fill_value", [2, 2.0, Timestamp(2021, 1, 1,
    12).time])
    ...
    fill_value=str(arr1d[-1]))
        ...
        msg = f"value should be a '{arr1d._scalar_type.__name__}' or 'NaT'.
    Got"
        with pytest.raises(TypeError, match=msg):
    ...
    string_storage):
        if isinstance(arr1d, DatetimeArray):
            ...
            if tz is not None and ts1.tz.tzname(ts1) != ts2.tz.tzname(ts2):
                ...
                #  back we get pytz.UTC, and then consider them different
    timezones
                #  so incorrectly raise.
                mark = pytest.mark.xfail(
                    raises=TypeError, reason="timezone comparisons
    inconsistent"
                )
        ...
        arr_type = "StringArray" if string_storage == "python" else
    "ArrowStringArray"
    ...
    Australia/Melbourne
            ...
            with pytest.raises(ValueError, match=msg):
                ...
                with tm.assert_produces_warning(
                    ...
                    result = arr.take([-1, 1], allow_fill=True,
    fill_value=value)
...
def test_invalid_nat_setitem_array(arr, non_casting_nats):
    msg = (
        "value should be a '(Timestamp|Timedelta|Period)', 'NaT', or array of
    those. "
...
@pytest.mark.parametrize(
    "arg", [[1, 2], ["a", "b"], [Timestamp("2020-01-01", tz="Europe/London")]
    * 2]
)
