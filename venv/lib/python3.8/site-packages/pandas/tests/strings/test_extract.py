...
def test_extract_expand_False_mixed_object():
    ser = Series(
        ["aBAD_BAD", np.nan, "BAD_b_BAD", True, datetime.today(), "foo", None,
    1, 2.0]
    )
    ...
    expected = DataFrame([["BAD_", "BAD"], er, ["BAD_", "BAD"], er, er, er,
    er, er, er])
    tm.assert_frame_equal(result, expected)
    ...
    result = ser.str.extract(".*(BAD[_]+).*BAD", expand=False)
    expected = Series(
        ["BAD_", np.nan, "BAD_", np.nan, np.nan, np.nan, np.nan, np.nan,
    np.nan]
    )
...
def test_extract_expand_no_capture_groups_raises(index_or_series,
    any_string_dtype):
...
def test_extract_expand_single_capture_group(index_or_series,
    any_string_dtype):
...
def test_extract_expand_True_single_capture_group(index_or_series,
    any_string_dtype):
    # single group renames series/index properly
    s_or_idx = index_or_series(["A1", "A2"], dtype=any_string_dtype)
...
def test_extract_series(name, any_string_dtype):
    ...
    expected = DataFrame({"letter": ["A", "B", np.nan]},
    dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_extractall(any_string_dtype):
    ...
    # extractall should return a DataFrame with one row for each match,
    indexed by the
    ...
    # The index of the input Series should be used to construct the index of
    the output
    # DataFrame:
...
def test_extractall_column_names(pat, expected_names, any_string_dtype):
    ...
    expected = DataFrame(
        ...
        index=MultiIndex.from_tuples([(1, 0), (2, 0), (2, 1)], names=(None,
    "match")),
...
def test_extractall_single_group(any_string_dtype):
    ...
    expected = DataFrame(
        {"letter": ["a", "b", "d", "c"]}, index=expected_index,
    dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, expected)
...
def test_extractall_single_group_with_quantifier(any_string_dtype):
    # GH#13382
    # extractall(one un-named group with quantifier) returns DataFrame with
    one un-named
    # column.
...
def test_extractall_no_matches(data, names, any_string_dtype):
    ...
    expected = DataFrame(columns=[0, 1], index=expected_index,
    dtype=any_string_dtype)
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        columns=["first", "second"], index=expected_index,
    dtype=any_string_dtype
    )
    tm.assert_frame_equal(result, expected)
...
def test_extractall_stringindex(any_string_dtype):
    ...
    expected = DataFrame(
        {"digit": ["1", "2", "1"]},
        index=MultiIndex.from_tuples([(0, 0), (0, 1), (1, 0)], names=[None,
    "match"]),
        dtype=any_string_dtype,
    )
    tm.assert_frame_equal(result, expected)
...
def test_extractall_no_capture_groups_raises(any_string_dtype):
    # Does not make sense to use extractall with a regex that has no capture
    groups.
...
def test_extractall_same_as_extract_subject_index(any_string_dtype):
    ...
    s = Series(["a3", "b3", "c2"], index=mi, name="series_name",
    dtype=any_string_dtype)
