...
def test_contains_object_mixed():
    ...
    expected = Series(
        np.array(
            [False, np.nan, False, np.nan, np.nan, True, np.nan, np.nan,
    np.nan],
            dtype=np.object_,
        )
    )
...
def test_contains_na_kwarg_for_nullable_string_dtype(
    ...
    with tm.maybe_produces_warning(
        PerformanceWarning,
        nullable_string_dtype == "string[pyarrow]" and pa_version_under4p0 and
    regex,
    ):
...
def test_startswith(pat, dtype, null_value, na):
    ...
    xp = Series([False, np.nan, False, np.nan, np.nan, True, np.nan, np.nan,
    np.nan])
...
def test_endswith(pat, dtype, null_value, na):
    ...
    xp = Series([False, np.nan, False, np.nan, np.nan, False, np.nan, np.nan,
    np.nan])
    tm.assert_series_equal(rs, xp)
...
def test_replace_mixed_object():
    ser = Series(
        ["aBAD", np.nan, "bBAD", True, datetime.today(), "fooBAD", None, 1,
    2.0]
    )
    result = Series(ser).str.replace("BAD[_]*", "", regex=True)
    expected = Series(["a", np.nan, "b", np.nan, np.nan, "foo", np.nan,
    np.nan, np.nan])
    tm.assert_series_equal(result, expected)
...
def test_replace_unicode(any_string_dtype):
    ...
    with tm.maybe_produces_warning(
        ...
        result = ser.str.replace(r"(?<=\w),(?=\w)", ", ", flags=re.UNICODE,
    regex=True)
    tm.assert_series_equal(result, expected)
...
def test_replace_wrong_repl_type_raises(any_string_dtype, index_or_series,
    repl, data):
...
@pytest.mark.parametrize(
    "regex,expected", [(True, ["bao", "bao", np.nan]), (False, ["bao", "foo",
    np.nan])]
)
...
def test_replace_regex_single_character(regex, any_string_dtype):
    ...
    # The current behavior is to treat single character patterns as literal
    strings,
    ...
    if regex is None:
        msg = re.escape(
            "The default value of regex will change from True to False in a
    future "
            "version. In addition, single character regular expressions will
    *not* "
            "be treated as literal strings when regex=True."
        )
        pyarrow_warn = any_string_dtype == "string[pyarrow]" and
    pa_version_under4p0
        with tm.assert_produces_warning(
...
def test_find(any_string_dtype):
    ...
    tm.assert_series_equal(result, expected)
    expected = np.array([v.find("EF", 3) for v in np.array(ser)],
    dtype=np.int64)
    tm.assert_numpy_array_equal(np.array(result, dtype=np.int64), expected)
    ...
    expected = Series([4, 5, 7, 4, -1], dtype=expected_dtype)
    tm.assert_series_equal(result, expected)
    expected = np.array([v.rfind("EF", 3) for v in np.array(ser)],
    dtype=np.int64)
    tm.assert_numpy_array_equal(np.array(result, dtype=np.int64), expected)
    ...
    tm.assert_series_equal(result, expected)
    expected = np.array([v.find("EF", 3, 6) for v in np.array(ser)],
    dtype=np.int64)
    tm.assert_numpy_array_equal(np.array(result, dtype=np.int64), expected)
    ...
    expected = Series([4, 3, -1, 4, -1], dtype=expected_dtype)
    tm.assert_series_equal(result, expected)
    expected = np.array([v.rfind("EF", 3, 6) for v in np.array(ser)],
    dtype=np.int64)
    tm.assert_numpy_array_equal(np.array(result, dtype=np.int64), expected)
