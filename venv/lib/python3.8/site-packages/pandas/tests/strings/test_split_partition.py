...
def test_split_regex_explicit(any_string_dtype):
    ...
    with pytest.raises(
        ValueError,
        match="Cannot use a compiled regex as replacement pattern with
    regex=False",
    ):
...
def test_split_object_mixed(expand, method):
    mixed = Series(["a_b_c", np.nan, "d_e_f", True, datetime.today(), None, 1,
    2.0])
...
def test_posargs_deprecation(method):
    ...
    msg = (
        f"In a future version of pandas all arguments of
...
def test_split_blank_string(any_string_dtype):
    ...
    exp = DataFrame([[]], dtype=any_string_dtype)  # NOTE: this is NOT an
    empty df
...
def test_split_to_dataframe_with_index(any_string_dtype):
    s = Series(
        ["some_splits", "with_index"], index=["preserve", "me"],
    dtype=any_string_dtype
    )
    result = s.str.split("_", expand=True)
    exp = DataFrame(
...
def test_split_to_multiindex_expand_unequal_splits():
    idx = Index(["some_unequal_splits", "one_of_these_things_is_not", np.nan,
    None])
    result = idx.str.split("_", expand=True)
    exp = MultiIndex.from_tuples(
        [
...
def test_rsplit_to_multiindex_expand():
    ...
    result = idx.str.rsplit("_", expand=True)
    exp = MultiIndex.from_tuples([("some", "equal", "splits"), ("with", "no",
    "nans")])
    tm.assert_index_equal(result, exp)
    assert result.nlevels == 3
...
def test_get_mixed_object():
    ser = Series(["a_b_c", np.nan, "c_d_e", True, datetime.today(), None, 1,
    2.0])
    result = ser.str.split("_").str.get(1)
    expected = Series(["b", np.nan, "d", np.nan, np.nan, np.nan, np.nan,
    np.nan])
    tm.assert_series_equal(result, expected)
