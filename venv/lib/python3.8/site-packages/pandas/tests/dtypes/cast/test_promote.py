...
def _assert_match(result_fill_value, expected_fill_value):
    ...
    if hasattr(result_fill_value, "dtype"):
        ...
        assert result_fill_value.dtype.itemsize ==
...
def test_maybe_promote_datetime64_with_any(datetime64_dtype,
...
@pytest.mark.parametrize(
    ...
    ids=["pd.Timestamp", "np.datetime64", "datetime.datetime",
    "datetime.date"],
)
...
def test_maybe_promote_timedelta64_with_any(timedelta64_dtype,
    any_numpy_dtype_reduced):
...
def test_maybe_promote_any_with_timedelta64(any_numpy_dtype_reduced,
    fill_value):
    dtype = np.dtype(any_numpy_dtype_reduced)
...
def test_maybe_promote_any_numpy_dtype_with_na(any_numpy_dtype_reduced,
    ...
    if isinstance(fill_value, Decimal):
        # Subject to change, but ATM (When Decimal(NAN) is being added to
    nulls_fixture)
