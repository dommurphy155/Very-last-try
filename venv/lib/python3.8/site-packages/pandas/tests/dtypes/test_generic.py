...
class TestABCClasses:
    ...
    def test_abc_pairs_subclass_check(self, abctype1, abctype2, inst, _):
        ...
        if abctype1 == abctype2:
            ...
            with pytest.raises(
                TypeError, match=re.escape("issubclass() arg 1 must be a
    class")
            ):
    ...
    @pytest.mark.parametrize("abctype", [e for e in gt.__dict__ if
    e.startswith("ABC")])
    def test_abc_coverage(self, abctype):
        # GH 38588
        assert (
            abctype in (e for e, _ in self.abc_pairs) or abctype in
    self.abc_subclasses
        )
