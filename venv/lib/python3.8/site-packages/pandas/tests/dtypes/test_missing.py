...
class TestIsNA:
    ...
    def test_isna_old_datetimelike(self):
        ...
        objs = [dta, dta.tz_localize("US/Eastern"), dta - dta,
    dta.to_period("D")]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (np.float64(1), False),
            (np.array([1, 1 + 0j, np.nan, 3]), np.array([False, False, True,
    False])),
            (
...
def test_array_equivalent(dtype_equal):
    ...
    msg = "will be interpreted as nanosecond UTC timestamps instead of
    wall-times"
    ...
    assert not array_equivalent(DatetimeIndex([0, np.nan]), TimedeltaIndex([0,
    np.nan]))


@pytest.mark.parametrize(
    "val", [1, 1.1, 1 + 1j, True, "abc", [1, 2], (1, 2), {1, 2}, {"a": 1},
    None]
)
...
def test_array_equivalent_nested(strict_nan):
    ...
    right = np.array([np.array([50, 70, 90]), np.array([20, 30])],
    dtype=object)
...
def test_array_equivalent_nested2(strict_nan):
    ...
    left = np.array([np.array([np.array([50, 50, 50])], dtype=object)],
    dtype=object)
    right = np.array([50])
    assert not array_equivalent(left, right, strict_nan=strict_nan)
...
def test_array_equivalent_nested_mixed_list(strict_nan):
    ...
    left = np.array(
        [
            ...
            np.array([np.array([6]), np.array([7, 8]), np.array([9])],
    dtype=object),
        ],
        dtype=object,
    )
...
def test_array_equivalent_nested_dicts(strict_nan):
    left = np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)}],
    dtype=object)
    right = np.array(
