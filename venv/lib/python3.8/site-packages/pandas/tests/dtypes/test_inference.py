...
class MockNumpyLikeArray:
    ...
    (so that ``isinstance(abc.Iterable)`` returns ``True``) and has a:meth:`ndim` property, as pandas special-cases 0-dimensional arrays in some
    cases.
    ...
    a scalar (`is_scalar(np.array(1)) == False`), but it is not list-like
    either.
    """
...
@pytest.mark.parametrize("ll", [{}, {"A": 1}, Series([1]),
...
@pytest.mark.parametrize(
    "ll", [r"a", "x", r"asdf", re.compile("adsf"), r"\u2233\s*",
    re.compile(r"")]
)
...
class TestInference:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                True,
                BooleanArray(
                    np.array([True, False], dtype="bool"), np.array([False,
    True])
                ),
            ),
    ...
    def test_maybe_convert_nullable_boolean(self, convert_to_masked_nullable,
    exp):
        # GH 40687
    ...
    def test_convert_numeric_uint64(self):
        ...
        tm.assert_numpy_array_equal(lib.maybe_convert_numeric(arr, set())[0],
    exp)
    ...
    def test_convert_numeric_uint64_nan_values(
        ...
        expected = (
            np.array([np.nan, 2**63 + 1], dtype=float) if coerce else
    arr.copy()
        )
    ...
    def test_maybe_convert_objects_mixed_datetimes(self):
        ts = Timestamp("now")
        vals = [ts, ts.to_pydatetime(), ts.to_datetime64(), pd.NaT, np.nan,
    None]
    ...
    @pytest.mark.parametrize(
        "exp",
        [
            IntegerArray(np.array([2, 0], dtype="i8"), np.array([False,
    True])),
            IntegerArray(np.array([2, 0], dtype="int64"), np.array([False,
    True])),
        ],
    )
    def test_maybe_convert_objects_nullable_integer(self, exp):
        ...
        result = lib.maybe_convert_objects(arr,
    convert_to_nullable_integer=True)
    ...
    @pytest.mark.parametrize(
        "convert_to_masked_nullable, exp",
        [
            (True, IntegerArray(np.array([2, 0], dtype="i8"), np.array([False,
    True]))),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                True,
                FloatingArray(
                    np.array([2.0, 0.0], dtype="float64"), np.array([False,
    True])
                ),
            ),
    ...
    def test_mixed_dtypes_remain_object_array(self):
        # GH14956
        arr = np.array([datetime(2015, 1, 1, tzinfo=pytz.utc), 1],
    dtype=object)
...
class TestTypeInference:
    ...
    def test_length_zero(self, skipna):
        ...
        # GH 18004
        arr = np.array([np.array([], dtype=object), np.array([],
    dtype=object)])
    ...
    def test_floats(self):
        arr = np.array([1.0, 2.0, 3.0, np.float64(4), np.float32(5)],
    dtype="O")
        result = lib.infer_dtype(arr, skipna=True)
    ...
    dtype=object),
            np.array([np.timedelta64(1, "D"), np.datetime64("nat")],
    dtype=object),
        ],
    )
    ...
    def test_infer_dtype_period(self):
        # GH 13664
        arr = np.array([Period("2011-01", freq="D"), Period("2011-02",
    freq="D")])
    ...
    def test_infer_dtype_all_nan_nat_like(self):
        ...
        arr = np.array([pd.NaT, np.datetime64("nat"), np.timedelta64("nat"),
    np.nan])
        assert lib.infer_dtype(arr, skipna=False) == "mixed"
    ...
    @pytest.mark.parametrize("klass", [pd.array, Series])
    @pytest.mark.parametrize("skipna", [True, False])
    @pytest.mark.parametrize("data", [[True, False, True], [True, False,
    pd.NA]])
...
def test_nan_to_nat_conversions():
    ...
    df = DataFrame(
        {"A": np.asarray(range(10), dtype="float64"), "B":
    Timestamp("20010101")}
    )
    df.iloc[3:6,:] = np.nan
    result = df.loc[4, "B"]
