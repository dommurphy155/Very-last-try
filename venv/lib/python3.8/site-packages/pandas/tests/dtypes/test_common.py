...
@pytest.mark.parametrize("name1,dtype1", list(dtypes.items()), ids=lambda x:
    str(x))
@pytest.mark.parametrize("name2,dtype2", list(dtypes.items()), ids=lambda x:
    str(x))
...
@pytest.mark.parametrize("name,dtype", list(dtypes.items()), ids=lambda x:
    str(x))
...
def get_is_dtype_funcs():
    ...
    f.endswith("dtype"))]
    fnames.remove("is_string_or_object_np_dtype")  # fastpath requires
    np.dtype obj
    return [getattr(com, fname) for fname in fnames]


@pytest.mark.parametrize("func", get_is_dtype_funcs(), ids=lambda x:
    x.__name__)
...
def test_is_string_dtype_nullable(nullable_string_dtype):
    assert com.is_string_dtype(pd.array(["a", "b"],
...
@pytest.mark.parametrize(
    "dtype", [np.int64, np.array([1, 2], dtype=np.int64), "Int64",
    pd.Int64Dtype]
)
...
def test_type_comparison_with_signed_int_ea_dtype_and_signed_int_numpy_dtype(
    ...
    # GH#43038
    assert not pandas_dtype(any_signed_int_ea_dtype) ==
...
def test_is_timedelta64_ns_dtype():
    ...
    assert not com.is_timedelta64_ns_dtype(np.array([1, 2],
...
def test_is_datetime_or_timedelta_dtype():
    ...
    assert not com.is_datetime_or_timedelta_dtype(DatetimeTZDtype("ns",
    "US/Eastern"))

    assert com.is_datetime_or_timedelta_dtype(np.datetime64)
    assert com.is_datetime_or_timedelta_dtype(np.timedelta64)
    assert com.is_datetime_or_timedelta_dtype(np.array([],
    dtype=np.timedelta64))
    assert com.is_datetime_or_timedelta_dtype(np.array([],
    dtype=np.datetime64))
...
def test_is_numeric_v_string_like():
    ...
    assert not com.is_numeric_v_string_like(np.array(["foo"]),
    np.array(["foo"]))
...
@pytest.mark.filterwarnings("ignore:'is_extension_type' is
...
def test_is_extension_array_dtype(check_scipy):
    ...
    assert com.is_extension_array_dtype(pd.DatetimeIndex(["2000"],
    tz="US/Eastern"))
    ...
    if check_scipy:
        ...
        assert not com.is_extension_array_dtype(scipy.sparse.bsr_matrix([1, 2,
    3]))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("datetime64[ns, Europe/London]", DatetimeTZDtype("ns",
    "Europe/London")),
...
def test_get_dtype_fails(input_param, expected_error_message):
    ...
    expected_error_message += f"|Cannot interpret '{input_param}' as a data
    type"
