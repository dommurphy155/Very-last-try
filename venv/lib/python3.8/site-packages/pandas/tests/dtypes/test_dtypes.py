...
class Base:
    ...
    def test_numpy_informed(self, dtype):
        ...
        msg = "|".join(
            ["data type not understood", "Cannot interpret '.*' as a data
    type"]
        )
...
class TestCategoricalDtype(Base):
    ...
    def test_update_dtype(self):
        # GH 27338
        result = CategoricalDtype(["a"]).update_dtype(Categorical(["b"],
    ordered=True))
...
class TestDatetimeTZDtype(Base):
    ...
    def test_construction_from_string(self, dtype):
        result = DatetimeTZDtype.construct_from_string("datetime64[ns,
    US/Eastern]")
        assert is_dtype_equal(dtype, result)
    ...
    def test_equality(self, dtype):
        ...
        assert not is_dtype_equal(
            DatetimeTZDtype("ns", "US/Eastern"), DatetimeTZDtype("ns",
    "US/Pacific")
        )
...
class TestIntervalDtype(Base):
    ...
    @pytest.mark.parametrize(
        "subtype", ["interval[int64]", "Interval[int64]", "int64",
    np.dtype("int64")]
    )
    ...
    def test_equality(self, dtype):
        ...
        assert not is_dtype_equal(
            IntervalDtype("int64", "neither"), IntervalDtype("float64",
    "right")
        )
        assert not is_dtype_equal(
...
class TestCategoricalDtypeParametrized:
    ...
    @pytest.mark.parametrize("v1, v2", [([1, 2, 3], [1, 2, 3]), ([1, 2, 3],
    [3, 2, 1])])
    ...
    def test_from_categorical_dtype_categories(self):
        c1 = Categorical([1, 2], categories=[1, 2, 3], ordered=True)
        # override categories
        result = CategoricalDtype._from_categorical_dtype(c1, categories=[2,
    3])
    ...
    def test_update_dtype(self, ordered, new_categories, new_ordered):
        ...
        expected_ordered = new_ordered if new_ordered is not None else
    dtype.ordered
    ...
    @pytest.mark.parametrize("bad_dtype", ["foo", object, np.int64,
    PeriodDtype("Q")])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("interval[datetime64[ns], left]", IntervalDtype("datetime64[ns]",
    "left")),
