...
"""
    ...
    result = parser.read_csv(
        StringIO(data), sep=r"\s+", header=None, names=["a", "b"], dtype={"a":
    np.int32}
    )
...
@pytest.mark.parametrize(
    ...
    unsupported.
        ...
        (
            ...
            {"dtype": {"A": "datetime64", "B": "float64"}, "parse_dates":
    ["B"]},
        ),
        (
...
def test_unsupported_dtype(c_parser_only, match, kwargs):
    parser = c_parser_only
    df = DataFrame(
        np.random.rand(5, 2), columns=list("AB"), index=["1A", "1B", "1C",
    "1D", "1E"]
    )
...
def test_grow_boundary_at_cap(c_parser_only):
    ...
    def test_empty_header_read(count):
        with StringIO("," * count) as s:
            expected = DataFrame(columns=[f"Unnamed: {i}" for i in range(count
    + 1)])
...
def test_internal_null_byte(c_parser_only):
    ...
    names = ["a", "b", "c"]
    data = "1,2,3\n4,\x00,6\n7,8,9"
    expected = DataFrame([[1, 2.0, 3], [4, np.nan, 6], [7, 8, 9]],
    columns=names)
...
def test_unix_style_breaks(c_parser_only):
    ...
    with tm.ensure_clean() as path:
        with open(path, "w", newline="\n") as f:
            f.write("blah\n\ncol_1,col_2,col_3\n\n")
        result = parser.read_csv(path, skiprows=2, encoding="utf-8",
    engine="c")
