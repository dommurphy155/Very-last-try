    ...
    def test_mangle_dupe_cols_false(self):
        ...
        for engine in ("c", "python"):
            with tm.assert_produces_warning(
                FutureWarning, match="the 'mangle_dupe_cols' keyword is
    deprecated"
            ):
...
"""
    ...
    def test_python_engine(self, python_engine):
        from pandas.io.parsers.readers import _python_unsupported as
    py_unsupported
...
1,2,3,4,"""
    ...
    def test_python_engine_file_no_iter(self, python_engine):
        ...
        data = "a\n1"
        msg = "'NoNextBuffer' object is not iterable|argument 1 must be an
    iterator"
    ...
    def test_pyarrow_engine(self):
        from pandas.io.parsers.readers import _pyarrow_unsupported as
    pa_unsupported
    ...
    def test_on_bad_lines_callable_python_only(self, all_parsers):
        ...
        if all_parsers.engine != "python":
            msg = "on_bad_line can only be a callable function if
    engine='python'"
            with pytest.raises(ValueError, match=msg):
...
def test_close_file_handle_on_invalid_usecols(all_parsers):
    ...
    if parser.engine == "pyarrow":
        ...
        if is_ci_environment() and (is_platform_windows() or
    is_platform_mac()):
            # GH#45547 causes timeouts on windows/mac builds
            pytest.skip("GH#45547 causing timeouts on windows/mac builds
    2022-01-22")
