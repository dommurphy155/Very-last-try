...
def test_compressed_urls(salaries_table, mode, engine, compression_only):
    ...
    url_table = read_csv(url, sep="\t", compression=compression_only,
    engine=engine)
...
class TestS3:
    ...
    Body=buf)
        ...
        # If we don't clear this cache, we saw `GetObject operation:
    Forbidden`.
        # Presumably the s3fs instance is being cached, with the directory
    listing
