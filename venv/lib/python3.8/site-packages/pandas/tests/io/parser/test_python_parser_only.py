...
"""
    parser = python_parser_only
    result = parser.read_csv(StringIO(data), index_col=0, sep=None,
    comment="#")
    expected = DataFrame(
        [[1, 2, 3], [4, 5, 6], [7, 8, 9]],
        columns=["A", "B", "C"],
...
"""
    ...
    result = parser.read_csv(data, index_col=0, sep=None, skiprows=2,
    encoding=encoding)
    expected = DataFrame(
        [[1, 2, 3], [4, 5, 6], [7, 8, 9]],
        columns=["A", "B", "C"],
...
def test_single_line(python_parser_only):
    ...
    parser = python_parser_only
    result = parser.read_csv(StringIO("1,2"), names=["a", "b"], header=None,
    sep=None)
...
"""
    ...
    expected = DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]], columns=["A", "B",
    "C"])
    tm.assert_frame_equal(result, expected)
...
def test_none_delimiter(python_parser_only, capsys):
    ...
    result = parser.read_csv(StringIO(data), header=0, sep=None,
    on_bad_lines="warn")
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize("bad_line_func", [lambda x: ["2", "3"], lambda x:
    x[:2]])
...
@pytest.mark.parametrize("bad_line_func", [lambda x: ["foo", "bar"], lambda x:
    x[:2]])
@pytest.mark.parametrize("sep", [",", "111"])
def test_on_bad_lines_callable_iterator_true(python_parser_only,
    bad_line_func, sep):
    # GH 5686
...
"""
    ...
    result_iter = parser.read_csv(
        bad_sio, on_bad_lines=bad_line_func, chunksize=1, iterator=True,
    sep=sep
    )
    expecteds = [
        {"0": "hi", "1": "there"},
...
"""
    ...
    result = parser.read_csv_check_warnings(
        ParserWarning, "Length of header or names", bad_sio,
...
def
    test_header_int_do_not_infer_multiindex_names_on_different_line(python_parser_only):
    # GH#46569
    parser = python_parser_only
    data = StringIO("a\na,b\nc,d,e\nf,g,h")
    result = parser.read_csv_check_warnings(
        ParserWarning, "Length of header", data, engine="python",
    index_col=False
    )
