...
10,11,12"""
    ...
    expected = DataFrame([[2, 3], [5, 6], [8, 9], [11, 12]], columns=["b",
    "c"])
...
10,11,12"""
    parser = all_parsers
    names = ["foo", "bar"]
    result = parser.read_csv(StringIO(data), names=names, usecols=[1, 2],
    header=0)
...
10,11,12"""
    parser = all_parsers
    result = parser.read_csv(StringIO(data), names=names, header=None,
    usecols=usecols)
...
10,11,12"""
    ...
    expected = DataFrame([[1, 2], [4, 5], [7, 8], [10, 11]], columns=["a",
    "b"])
    tm.assert_frame_equal(result, expected)
...
10,11,12"""
    parser = all_parsers
    msg = "Number of passed names did not match number of header fields in the
    file"
...
def test_usecols_implicit_index_col(all_parsers):
    ...
    expected = DataFrame({"a": ["apple", "orange"], "b": ["bat", "cow"]},
    index=[4, 8])
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([0, 1], DataFrame(data=[[1000, 2000], [4000, 5000]], columns=["2",
    "0"])),
...
def test_raises_on_usecols_names_mismatch(all_parsers, usecols, kwargs,
    expected, msg):
    data = "a,b,c,d\n1,2,3,4\n5,6,7,8"
...
def test_usecols_subset_names_mismatch_orig_columns(all_parsers, usecols):
    ...
    result = parser.read_csv(StringIO(data), header=0, names=names,
    usecols=usecols)
...
1,2
    ...
    with tm.assert_produces_warning(
        ...
        result = parser.read_csv(StringIO(data), usecols=[0, 2], names=names,
    header=0)
