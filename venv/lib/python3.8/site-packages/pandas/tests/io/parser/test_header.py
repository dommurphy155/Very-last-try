...
def test_negative_multi_index_header(all_parsers, header):
    ...
    data = """1,2,3,4,5
        ...
        """
    with pytest.raises(
        ValueError, match="cannot specify multi-index header with negative
    integers"
    ):
...
@pytest.mark.parametrize(
    "columns", [None, (["", "Unnamed"]), (["Unnamed", ""]), (["Unnamed",
    "NotUnnamed"])]
)
...
def test_header_none_and_implicit_index_in_second_row(all_parsers):
    ...
    parser = all_parsers
    data = "x,1\ny,2,5\nz,3\n"
    with pytest.raises(ParserError, match="Expected 2 fields in line 2, saw
    3"):
