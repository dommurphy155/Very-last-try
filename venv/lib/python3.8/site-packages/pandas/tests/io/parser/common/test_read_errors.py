...
"""
    ...
    with parser.read_csv(
        StringIO(data), header=1, comment="#", iterator=True, chunksize=1,
    skiprows=[2]
    ) as reader:
        with pytest.raises(ParserError, match=msg):
            reader.read(nrows)
    ...
    if parser.engine == "c" or (parser.engine == "python" and PY311):
        if parser.engine == "python" and PY311:
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="In Python 3.11, this is read as an empty character
    not null"
                )
            )
...
def test_open_file(request, all_parsers):
    ...
    with tm.ensure_clean() as path:
        ...
        with warnings.catch_warnings(record=True) as record:
            with pytest.raises(csv.Error, match="Could not determine
    delimiter"):
...
def test_invalid_on_bad_line(all_parsers):
    parser = all_parsers
    data = "a\n1\n1,2,3\n4\n5,6,7"
    with pytest.raises(ValueError, match="Argument abc is invalid for
    on_bad_lines"):
...
def test_conflict_on_bad_line(all_parsers, error_bad_lines, warn_bad_lines):
    parser = all_parsers
    data = "a\n1\n1,2,3\n4\n5,6,7"
    kwds = {"error_bad_lines": error_bad_lines, "warn_bad_lines":
    warn_bad_lines}
    with pytest.raises(
        ValueError,
