...
def test_zip_error_invalid_zip(parser_and_data):
    ...
    with tm.ensure_clean() as path:
        with open(path, "rb") as f:
            with pytest.raises(zipfile.BadZipfile, match="File is not a zip
    file"):
...
def test_compression(request, parser_and_data, compression_only, buffer,
    filename):
    parser, data, expected = parser_and_data
    ...
    if filename and buffer:
        request.node.add_marker(
            pytest.mark.xfail(
                reason="Cannot deduce compression from buffer of compressed
    data."
            )
        )
...
@skip_pyarrow
def test_compression_utf_encoding(all_parsers, csv_dir_path, utf_value,
    encoding_fmt):
    ...
    result = parser.read_csv(path, encoding=encoding, compression="zip",
    sep="\t")
    expected = DataFrame(
        {
...
def test_ignore_compression_extension(all_parsers):
    ...
    with tm.ensure_clean("test.csv") as path_csv:
        with tm.ensure_clean("test.csv.zip") as path_zip:
            ...
            tm.assert_frame_equal(parser.read_csv(path_zip, compression=None),
    df)
