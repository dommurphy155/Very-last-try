...
KORD6,19990127, 23:00:00, 22:56:00, -0.5900, 1.7100, 4.6000, 0.0000,
    280.0000"""
    header =
...
def test_multi_index_naming(all_parsers, index_names):
    ...
    expected = DataFrame(
        {"col": [1, 2, 3, 4]}, index=MultiIndex.from_product([["a", "b"],
    ["c", "d"]])
    )
...
def test_multi_index_naming_not_all_at_beginning(all_parsers):
    ...
    expected = DataFrame(
        {"Unnamed: 2": ["c", "d", "c", "d"]},
        index=MultiIndex(
            levels=[["a", "b"], [1, 2, 3, 4]], codes=[[0, 0, 1, 1], [0, 1, 2,
    3]]
        ),
    )
    tm.assert_frame_equal(result, expected)
...
def test_no_multi_index_level_names_empty(all_parsers):
    ...
    expected = DataFrame(np.random.randn(3, 3), index=midx, columns=["x", "y",
    "z"])
...
I2,1,3
"""
    midx = MultiIndex.from_tuples([("A", "B"), ("A", "B.1")], names=["I11",
    "I12"])
    idx = Index(["I2"])
...
def test_multiindex_columns_no_data(all_parsers):
    ...
    result = parser.read_csv(StringIO("a0,a1,a2\nb0,b1,b2\n"), header=[0, 1])
    expected = DataFrame(
        [], columns=MultiIndex.from_arrays([["a0", "a1", "a2"], ["b0", "b1",
    "b2"]])
    )
    tm.assert_frame_equal(result, expected)
