...
4,5.5"""
    expected = DataFrame(
        [[1, "2.5"], [2, "3.5"], [3, "4.5"], [4, "5.5"]], columns=["one",
    "two"]
    )
...
4,5.5"""
    ...
    with pytest.raises(TypeError, match="data type [\"']foo[\"'] not
    understood"):
...
def decimal_number_check(request, parser, numeric_decimal, thousands,
    ...
    if thousands is None and value in ("1_,", "1_234,56", "1_234,56e0"):
        request.node.add_marker(
            pytest.mark.xfail(reason=f"thousands={thousands} and sep is in
    {value}")
        )
    df = parser.read_csv(
        StringIO(value),
...
"""
    ...
    parser = all_parsers
    result = parser.read_csv(StringIO(data), dtype=dtype)
    expected = DataFrame({"a": [1], "b": [2.0], "a.1": [3], "b.2": [4.0],
    "b.1": [5]})
    tm.assert_frame_equal(result, expected)
