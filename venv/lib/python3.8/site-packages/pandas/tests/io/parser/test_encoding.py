...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("\n1", {"names": ["a"], "skip_blank_lines": True}, DataFrame({"a":
    [1]})),
...
def test_utf8_bom(all_parsers, data, kwargs, expected):
    ...
    result = parser.read_csv(_encode_data_with_bom(data), encoding=utf8,
    **kwargs)
    tm.assert_frame_equal(result, expected)
...
def test_encoding_temp_file(all_parsers, utf_value, encoding_fmt,
    pass_encoding):
    # see gh-24130
    parser = all_parsers
    encoding = encoding_fmt.format(utf_value)
    ...
    with tm.ensure_clean(mode="w+", encoding=encoding, return_filelike=True)
    as f:
        f.write("foo\nbar")
        f.seek(0)

        result = parser.read_csv(f, encoding=encoding if pass_encoding else
    None)
        tm.assert_frame_equal(result, expected)
...
def test_parse_encoded_special_characters(encoding):
    ...
    expected = DataFrame(data=[["：foo", 0], ["bar", 1], ["baz", 2]],
    columns=["a", "b"])
    tm.assert_frame_equal(result, expected)


@skip_pyarrow
@pytest.mark.parametrize("encoding", ["utf-8", None, "utf-16", "cp1255",
    "latin-1"])
...
def test_readcsv_memmap_utf8(all_parsers):
    ...
    Test correct handling of UTF-8 chars when memory_map=True and encoding is
    UTF-8
    """
    lines = []
    line_length = 128
