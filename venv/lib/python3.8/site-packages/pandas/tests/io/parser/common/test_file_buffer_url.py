...
def test_path_path_lib(all_parsers):
    parser = all_parsers
    df = tm.makeDataFrame()
    result = tm.round_trip_pathlib(df.to_csv, lambda p: parser.read_csv(p,
    index_col=0))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            DataFrame([["4", 5, 6], [" ", None, None]], columns=["a", "b",
    "c"]),
            None,
        ),
        # EAT_CRNL
        (
            "a,b,c\n4,5,6\n\r",
            {"skip_blank_lines": False},
            DataFrame([[4, 5, 6], [None, None, None]], columns=["a", "b",
    "c"]),
            None,
        ),
        # ESCAPED_CHAR
...
def test_internal_eof_byte_to_file(all_parsers):
    ...
    parser = all_parsers
    data = b'c1,c2\r\n"test \x1a    test", test\r\n'
    expected = DataFrame([["test \x1a    test", " test"]], columns=["c1",
    "c2"])
...
def test_read_csv_file_handle(all_parsers, io_class, encoding):
    ...
    content = "a,b\n1,2"
    handle = io_class(content.encode("utf-8") if io_class == BytesIO else
    content)
...
def test_file_descriptor_leak(all_parsers):
    ...
    with tm.ensure_clean() as path:
        ...
        def test():
            with pytest.raises(EmptyDataError, match="No columns to parse from
    file"):
                parser.read_csv(path)
