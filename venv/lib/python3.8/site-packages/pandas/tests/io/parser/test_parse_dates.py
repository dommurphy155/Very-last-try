...
def test_read_csv_with_custom_date_parser(all_parsers):
    ...
    expected = DataFrame(
        {
            "e": [-98573.7297, -98573.7299, -98573.7300, -98573.7299,
    -98573.7302],
            "n": [871458.0640, 871458.0640, 871458.0642, 871458.0643,
    871458.0640],
...
def test_read_csv_with_custom_date_parser_parse_dates_false(all_parsers):
    ...
    time = Series([41047.00, 41048.00, 41049.00, 41050.00, 41051.00],
    name="time")
    expected = DataFrame(
...
05/31/2012,15:30:00.029,1306.25,8,E,0,,1306.25"""
    ...
    result = parser.read_csv(
        StringIO(data), parse_dates=[[0, 1]], header=None,
...
def test_parse_dates_column_list(all_parsers, parse_dates):
    ...
    result = parser.read_csv(
        StringIO(data), index_col=[0, 1], parse_dates=parse_dates,
    dayfirst=True
    )
    tm.assert_frame_equal(result, expected)
...
"""
    if "dayfirst" in kwargs:
        ...
        exp_index = Index(
            [datetime(2010, 1, 31), datetime(2010, 2, 1), datetime(2010, 2,
    2)],
            name="time",
        )
        expected = DataFrame(
...
def test_parse_tz_aware(all_parsers, request):
    ...
    if parser.engine == "pyarrow" and pa_version_under7p0:
        request.node.add_marker(pytest.mark.xfail(reason="Fails for pyarrow <
    7.0"))
    data = "Date,x\n2012-06-13T01:39:00Z,0.5"

    result = parser.read_csv(StringIO(data), index_col=0, parse_dates=True)
    expected = DataFrame(
        {"x": [0.5]}, index=Index([Timestamp("2012-06-13 01:39:00+00:00")],
    name="Date")
    )
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [({"nominal": [1, 2]}, "nominal"), ({"nominal": [1, 2]}, 0), ([[1, 2]],
    0)],
)
...
"""
    result = parser.read_csv(
        StringIO(data), index_col=["nominal", "ID"], parse_dates={"nominal":
    [1, 2]}
    )
...
def test_parse_date_time_multi_level_column_name(all_parsers, date_parser,
    warning):
    data = """\
D,T,A,B
date, time,a,b
...
"""
    ...
    expected = DataFrame(expected_data, columns=["date_time", ("A", "a"),
    ("B", "b")])
    tm.assert_frame_equal(result, expected)
...
""",
        ...
        (
            ...
            {"header": None, "parse_dates": {"actual": [1, 2], "nominal": [1,
    3]}},
            DataFrame(
                [
                    [
...
def test_parse_date_time(all_parsers, data, kwargs, expected, date_parser,
    warning):
    parser = all_parsers
    with tm.assert_produces_warning(warning, check_stacklevel=False):
...
@pytest.mark.parametrize(
    ...
    [
        (
            "a\n135217135789158401\n1352171357E+5",
            DataFrame({"a": [135217135789158401, 135217135700000]},
    dtype="float64"),
        ),
        (
            "a\n99999999999\n123456789012345\n1234E+0",
            DataFrame({"a": [99999999999, 123456789012345, 1234]},
    dtype="float64"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    "date_string",
    ["32/32/2019", "02/30/2019", "13/13/2019", "13/2019", "a3/11/2018",
    "10/11/2o17"],
)
...
def test_parse_multiple_delimited_dates_with_swap_warnings():
    ...
    with tm.assert_produces_warning(UserWarning, match=warning_msg) as record:
        pd.to_datetime(["01/01/2000", "31/05/2000", "31/05/2001",
    "01/02/2000"])
...
@pytest.mark.parametrize(
    "date_format",
    ["%d %m %Y", "%m %d %Y", "%m %Y", "%Y %m %d", "%y %m %d", "%Y%m%d",
    "%y%m%d"],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["date1", "time1", "temperature"], None, ["date", "time"], "date,
    time"),
        (
...
def test_missing_parse_dates_column_raises(
    ...
    with pytest.raises(ValueError, match=msg):
        parser.read_csv(
            content, sep=",", names=names, usecols=usecols,
    parse_dates=parse_dates
        )
...
1,2
2019-12-31,6"""
    result = parser.read_csv(StringIO(data), parse_dates=[("a", "1")],
    header=[0, 1])
    expected = DataFrame({("a", "1"): Timestamp("2019-12-31"), ("b", "2"):
    [6]})
    tm.assert_frame_equal(result, expected)
...
def test_date_parser_multiindex_columns_combine_cols(all_parsers, parse_spec,
    col_name):
    parser = all_parsers
    data = """a,b,c
1,2,3
...
def test_date_parser_usecols_thousands(all_parsers):
    ...
    expected = DataFrame({"B": [3, 4], "C": [Timestamp("20-09-2001 01:00:00")]
    * 2})
    tm.assert_frame_equal(result, expected)
...
def test_dayfirst_warnings():
    ...
    default\) was "
        r"specified. This may lead to inconsistently parsed dates! Specify a
    format "
        r"to ensure consistent parsing."
    )
    warning_msg_month_first = (
        "Parsing dates in MM/DD/YYYY format when dayfirst=True was "
        "specified. This may lead to inconsistently parsed dates! Specify a
    format "
    ...
    with tm.assert_produces_warning(UserWarning, match=warning_msg_day_first):
        res2 = read_csv(
            StringIO(input), parse_dates=["date"], dayfirst=False,
    index_col="date"
        ).index
    ...
    match=warning_msg_month_first):
        res5 = read_csv(
            StringIO(input), parse_dates=["date"], dayfirst=True,
    index_col="date"
        ).index
