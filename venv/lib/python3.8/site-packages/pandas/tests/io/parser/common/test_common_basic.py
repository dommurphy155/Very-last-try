...
0,1,20140102,1000,4"""
    ...
    cols = {
        "a": [0, 0],
        "c_d": [Timestamp("2014-01-01 09:00:00"), Timestamp("2014-01-02
    10:00:00")],
    }
...
"""
    parser = all_parsers
    expected = DataFrame({"A": ["a", "b", "c"], "B": [1, 3, 4], "C": [2, 4,
    5]})
    result = parser.read_csv(StringIO(data))
    tm.assert_frame_equal(result, expected)
...
3,3,4,5
"""
    result = parser.read_csv(StringIO(data), low_memory=True, index_col=0,
    nrows=0)
...
"bra tv
    board","http://www.ikea.com/se/sv/catalog/categories/departments/living_room/10475/?se%7cps%7cnonbranded%7cvardagsrum%7cgoogle%7ctv_bord"
"tv p\xc3\xa5
    hjul","http://www.ikea.com/se/sv/catalog/categories/departments/living_room/10475/?se%7cps%7cnonbranded%7cvardagsrum%7cgoogle%7ctv_bord"
"SLAGBORD, \\"Bergslagen\\", IKEA:s 1700-tals
    series","http://www.ikea.com/se/sv/catalog/categories/departments/living_room/10475/?se%7cps%7cnonbranded%7cvardagsrum%7cgoogle%7ctv_bord"'''  # noqa:E501
    ...
    assert result["SEARCH_TERM"][2] == 'SLAGBORD, "Bergslagen", IKEA:s
    1700-tals series'
...
def test_read_filepath_or_buffer(all_parsers):
    ...
    with pytest.raises(TypeError, match="Expected file path name or
    file-like"):
...
b\n"""
    ...
    result = parser.read_csv(
        StringIO(data), skipinitialspace=True,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (r"\s+", True, [[1.0, 2.0, 4.0], [5.0, np.nan, 10.0], [-70.0, 0.4,
    1.0]]),
        (
            ",",
            False,
...
5.,NaN,10.0
"""
    expected = DataFrame([[1, 2.0, 4.0], [5.0, np.nan, 10.0]], columns=["A",
    "B", "C"])
    result = parser.read_csv(StringIO(data))
    tm.assert_frame_equal(result, expected)
...
""",
        ...
        (
            "    a b c\n1 2 3 \n4 5  6\n 7 8 9",
            DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]], columns=["a", "b",
    "c"]),
        ),
    ],
)
...
def test_blank_lines_between_header_and_data_rows(all_parsers, nrows):
    ...
    csv = "\nheader\n\na,b\n\n\n1,2\n\n3,4"
    parser = all_parsers
    df = parser.read_csv(StringIO(csv), header=3, nrows=nrows,
...
def test_read_csv_names_not_accepting_sets(all_parsers):
    ...
    4,5,6\n"""
    parser = all_parsers
    with pytest.raises(ValueError, match="Names should be an ordered
    collection."):
...
1,2,3
    """
    msg = (
        r"Specified \\n as separator or delimiter. This forces the python
    engine "
        r"which does not accept a line terminator. Hence it is not allowed to
    use "
...
def test_encoding_surrogatepass(all_parsers):
    ...
    with tm.ensure_clean() as path:
        ...
        df = parser.read_csv(path, encoding_errors="surrogatepass",
    index_col=0)
        tm.assert_frame_equal(df, expected)
        with pytest.raises(UnicodeDecodeError, match="'utf-8' codec can't
    decode byte"):
