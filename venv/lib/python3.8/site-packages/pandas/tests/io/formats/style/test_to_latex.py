...
def test_column_format(styler):
    ...
    styler.set_table_styles([{"selector": "column_format", "props":
    ":r|r|cc"}])
...
def test_position_float_raises(styler):
    msg = "`position_float` should be one of 'raggedright', 'raggedleft',
    'centering',"
...
@pytest.mark.parametrize("column_format", [(None, ""), ("rcrl",
    "{tabular}{rcrl}")])
@pytest.mark.parametrize("position_float", [(None, ""), ("centering",
    "\\centering")])
...
def test_multi_options(df_ext):
    ...
    with option_context("styler.latex.multirow_align", "b"):
        assert "\\multirow[b]{2}{*}{A} & a & 0 & -0.61 & ab \\\\" in
    styler.to_latex()


def test_multiindex_columns_hidden():
    df = DataFrame([[1, 2, 3, 4]])
    df.columns = MultiIndex.from_tuples([("A", 1), ("A", 2), ("A", 3), ("B",
    1)])
    s = df.style
...
def test_comprehensive(df_ext, environment):
    ...
    stlr.set_table_styles(
        [
            ...
            {"selector": "rowcolors", "props": ":{3}{pink}{}"},  # custom
    command
        ]
    )
    stlr.highlight_max(axis=0,
    props="textbf:--rwrap;cellcolor:[rgb]{1,1,0.6}--rwrap")
    stlr.highlight_max(axis=None, props="Huge:--wrap;", subset=[("Z", "a"),
    ("Z", "b")])
...
\\toprule
 ...
 &  & a & b & c \\\\
\\midrule
\\multirow[c]{2}{*}{A} & a & 0 & \\textbf{\\cellcolor[rgb]{1,1,0.6}{-0.61}} &
    ab \\\\
 & b & 1 & -1.22 & cd \\\\
...
def test_parse_latex_table_styles(styler):
    styler.set_table_styles(
        [
            ...
            {"selector": "bar", "props": [("attr", "baz"), ("attr2",
    "ignored")]},
            {"selector": "label", "props": [("", "{fig§item}")]},
        ]
    )
    ...
    assert _parse_latex_table_styles(styler.table_styles, "label") ==
    "{fig:item}"
...
def test_parse_latex_table_wrapping(styler):
    ...
    assert _parse_latex_table_wrapping(styler.table_styles, styler.caption) is
    False
    assert _parse_latex_table_wrapping(styler.table_styles, "some caption") is
    True
    styler.set_table_styles(
        [
...
@pytest.mark.parametrize(
    "css, expected",
    [
        ([("color", "red")], [("color", "{red}")]),  # test color and input
    format types
        (
        ...
        ([("font-weight", "bold")], [("bfseries", "")]),  # test font-weight
    and types
        ([("font-weight", "bolder")], [("bfseries", "")]),
        ([("font-weight", "normal")], []),
        ([("background-color", "red")], [("cellcolor", "{red}--lwrap")]),
        (
            [("background-color", "#FF00FF")],  # test background-color
    command and wrap
            [("cellcolor", "[HTML]{FF00FF}--lwrap")],
        ),
        ([("font-style", "italic")], [("itshape", "")]),  # test font-style
    and types
        ([("font-style", "oblique")], [("slshape", "")]),
        ([("font-style", "normal")], []),
        ([("color", "red /*--dwrap*/")], [("color", "{red}--dwrap")]),  # css
    comments
        ([("background-color", "red /* --dwrap */")], [("cellcolor",
    "{red}--dwrap")]),
    ],
)
...
def test_longtable_comprehensive(styler):
    result = styler.to_latex(
        environment="longtable", hrules=True, label="fig:A", caption=("full",
    "short")
    )
    expected = dedent(
        """\
...
@pytest.mark.parametrize("label, lab_exp", [(None, ""), ("tab:A", "
    \\label{tab:A}")])
...
def test_apply_map_header_render_mi(df_ext, index, columns, siunitx):
    ...
    func = lambda v: "bfseries: --rwrap" if "A" in v or "Z" in v or "c" in v
    else None
...
def test_bool_options(styler, option):
    ...
    assert latex_false != latex_true  # options are reactive under
    to_latex(*no_args)
...
@pytest.mark.parametrize("clines", ["bad", "index", "skip-last", "all",
    "data"])
...
def test_concat_recursion():
    ...
    styler2 = DataFrame([[9],
    [2]]).style.hide([0]).highlight_min(color="green")
...
@pytest.mark.parametrize(
    "clines", [None, "all;data", "all;index", "skip-last;data",
    "skip-last;index"]
)
