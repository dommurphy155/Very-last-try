...
def styler_mi():
    ...
    return Styler(DataFrame(np.arange(16).reshape(4, 4), index=midx,
    columns=midx))
...
def test_colspan_w3():
    ...
    assert '<th class="col_heading level0 col0" colspan="2">l0</th>' in
    styler.to_html()
...
def test_rowspan_w3():
    ...
    styler = Styler(df, uuid="_", cell_ids=False)
    assert '<th class="row_heading level0 row0" rowspan="2">l0</th>' in
    styler.to_html()
    ...
    expected = dedent(
        ...
        <table id="T_abc">
          <thead>
            <tr>
              <th class="blank level0" >&nbsp;</th>
              <th id="T_abc_level0_col0" class="col_heading level0 col0"
    >A</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <th id="T_abc_level0_row0" class="row_heading level0 row0"
    >a</th>
              <td id="T_abc_row0_col0" class="data row0 col0" >2.610000</td>
            </tr>
            <tr>
              <th id="T_abc_level0_row1" class="row_heading level0 row1"
    >b</th>
...
def test_sticky_basic(styler, index, columns, index_name):
    ...
    assert (left_css.format("thead tr th:nth-child(1)", "3 !important") in
    res) is index
    assert (left_css.format("tbody tr th:nth-child(1)", "1") in res) is index

    # test column stickys including if name row
    assert (
        top_css.format("thead tr:nth-child(1) th", "0", "2", "  height:
    25px;\n") in res
    ) is (columns and index_name)
    assert (
        top_css.format("thead tr:nth-child(2) th", "25", "2", "  height:
    25px;\n")
        in res
    ) is (columns and index_name)
    assert (top_css.format("thead tr:nth-child(1) th", "0", "2", "") in res)
    is (
...
def test_sticky_mi(styler_mi, index, columns):
    ...
    assert (
        left_css.format("thead tr th:nth-child(2)", "75", "3 !important") in
    res
    ) is index
    ...
    columns
    assert (top_css.format("thead tr:nth-child(2) th", "25", "2") in res) is
    columns
...
def test_sticky_levels(styler_mi, index, columns, levels):
    styler_mi.index.names, styler_mi.columns.names = ["zero", "one"], ["zero",
    "one"]
    if index:
    ...
    assert (top_css.format("thead tr:nth-child(2) th", "0", "2") in res) is
    columns
...
def test_sparse_options(sparse_index, sparse_columns):
    ...
    default_html = styler.to_html()  # defaults under pd.options to (True ,
    True)

    with option_context(
        "styler.sparse.index", sparse_index, "styler.sparse.columns",
    sparse_columns
    ):
...
def test_applymap_header_cell_ids(styler, index, columns):
    ...
    assert (
        '<th id="T__level0_row0" class="row_heading level0 row0" >a</th>' in
    result
    ) is index
    assert (
        '<th id="T__level0_row1" class="row_heading level0 row1" >b</th>' in
    result
    ) is index
    ...
    assert (
        '<th id="T__level0_col0" class="col_heading level0 col0" >A</th>' in
    result
    ) is columns
...
def test_maximums(styler_mi, rows, cols):
    ...
    assert (">2</td>" in result) is not cols  # first trimmed horizontal
    element
    ...
    <table id="T_">
      <thead>
        <tr>
          ...
          <th id="T__level1_col0" class="col_heading level1 col0"
    colspan="2">c1</th>
      ...
      <tbody>
        <tr>
          <th id="T__level0_row0" class="row_heading level0 row0"
    rowspan="2">i0</th>
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("scheme full structure: http://www.web.com", True,
    "http://www.web.com"),
        ...
        ("subdirectories: www.web.com/directory", True,
...
def test_rendered_links(type, text, exp, found):
    ...
    assert (text in result) is not exp  # test conversion done when expected
    and not
...
def test_concat(styler):
    ...
    <tr>
      <th id="T_X_level0_{fp}row0" class="{fp}row_heading level0 {fp}row0"
    >mean</th>
...
def test_concat_recursion(styler):
    ...
    <tr>
      <th id="T_X_level0_{fp1}row0" class="{fp1}row_heading level0 {fp1}row0"
    >mean</th>
      <td id="T_X_{fp1}row0_col0" class="{fp1}data {fp1}row0 col0" >2.650</td>
    </tr>
    <tr>
      <th id="T_X_level0_{fp2}row0" class="{fp2}row_heading level0 {fp2}row0"
    >mean</th>
...
def test_concat_combined():
    def html_lines(foot_prefix: str):
        ...
        <tr>
          <th id="T_X_level0_{fp}row0" class="{fp}row_heading level0 {fp}row0"
    >a</th>
          <td id="T_X_{fp}row0_col0" class="{fp}data {fp}row0 col0"
    >2.610000</td>
        </tr>
        <tr>
          <th id="T_X_level0_{fp}row1" class="{fp}row_heading level0 {fp}row1"
    >b</th>
          <td id="T_X_{fp}row1_col0" class="{fp}data {fp}row1 col0"
    >2.690000</td>
        </tr>
        """
            ),
        ...
        dedent(
            ...
            <table id="T_X">
              <thead>
                <tr>
                  <th class="blank level0" >&nbsp;</th>
                  <th id="T_X_level0_col0" class="col_heading level0 col0"
    >A</th>
                </tr>
              </thead>
              <tbody>
