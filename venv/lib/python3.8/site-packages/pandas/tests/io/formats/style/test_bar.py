...
def bar_grad(a=None, b=None, c=None, d=None):
    ...
    return ret + [
        (
            "background",
            f"linear-gradient(90deg,{','.join([x for x in [a, b, c, d] if
    x])})",
        )
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("zero", [bar_from_to(50, 66.7), bar_from_to(50, 83.3),
    bar_from_to(50, 100)]),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("mid", [bar_from_to(66.66, 100), bar_from_to(33.33, 100),
    bar_to(100)]),
    ...
    [
        (
            "left",
            {
                ...
                "none": [[no_bar(), bar_to(33.33)], [bar_to(66.66),
    bar_to(100)]],
            },
        ),
        (
            "mid",
            {
                "index": [[bar_to(33.33), bar_to(50)], [bar_to(100),
    bar_to(100)]],
                "columns": [[bar_to(50), bar_to(100)], [bar_to(75),
    bar_to(100)]],
...
@pytest.mark.parametrize("nullify", [None, "vmin", "vmax"])  # test min/max
    separately
@pytest.mark.parametrize("align", ["left", "right", "zero", "mid"])
def test_vmin_vmax_clipping(df_pos, df_neg, df_mix, values, vmin, vmax,
    nullify, align):
    ...
    expected = clip_df.style.bar(align=align, color=["red",
    "green"])._compute().ctx
    assert result == expected
...
@pytest.mark.parametrize("nullify", [None, "vmin", "vmax"])  # test min/max
    separately
@pytest.mark.parametrize("align", ["left", "right", "zero", "mid"])
def test_vmin_vmax_widening(df_pos, df_neg, df_mix, values, vmin, vmax,
    nullify, align):
    ...
    expected = expand_df.style.bar(align=align, color=["red",
    "green"])._compute().ctx
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("zero", [bar_from_to(33.33, 50, "red"), bar_from_to(50, 100,
    "green")]),
    ],
)
...
def test_bar_align_height():
    ...
    bg_s = "linear-gradient(90deg, #d65f5f 100.0%, transparent 100.0%)
    no-repeat center"
    expected = {
        (0, 0): [("width", "10em")],
        (1, 0): [
