...
def test_info_default_verbose_selection(num_columns, max_info_columns,
    verbose):
...
def test_info_memory_usage():
    ...
    df = DataFrame(
        data=1, index=MultiIndex.from_product([["a"], range(1000)]),
    columns=["A"]
    )
    df.index.nbytes
...
def test_info_memory_usage_qualified():
    ...
    buf = StringIO()
    df = DataFrame(
        1, columns=list("ab"), index=MultiIndex.from_product([range(3),
    range(3)])
    )
    df.info(buf=buf)
    assert "+" not in buf.getvalue()
