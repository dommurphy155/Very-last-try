...
class TestToLatexLongtable:
    ...
    def test_to_latex_longtable_continued_on_next_page(self, df,
    expected_number):
        result = df.to_latex(index=False, longtable=True)
...
class TestToLatexCaptionLabel:
    ...
    def test_to_latex_caption_and_label(self, df_short, caption_table,
    label_table):
        # GH 25436
    ...
    def test_to_latex_longtable_caption_only(self, df_short,
    caption_longtable):
        # GH 25436
...
class TestToLatexEscape:
    ...
    def df_with_symbols(self):
        ...
        a = "a"
        b = "b"
        yield DataFrame({"co$e^x$": {a: "a", b: "b"}, "co^l1": {a: "a", b:
    "b"}})
...
class TestToLatexMultiindex:
    ...
    def test_to_latex_index_has_name_tabular(self):
        # GH 10660
        df = DataFrame({"a": [0, 0, 1, 1], "b": list("abab"), "c": [1, 2, 3,
    4]})
    ...
    def test_to_latex_multicolumnrow_with_multicol_format(self,
    multicolumn_frame):
    ...
    def test_to_latex_multiindex_names(self, name0, name1, axes):
        ...
        col_names = [n if (bool(n) and 1 in axes) else placeholder for n in
    names]
        observed = df.to_latex()
...
"""
    ...
    def test_to_latex_multiindex_multirow(self):
        ...
        mi = pd.MultiIndex.from_product(
            [[0.0, 1.0], [3.0, 2.0, 1.0], ["0", "1"]], names=["i", "val0",
    "val1"]
        )
...
class TestTableBuilder:
    ...
    def test_create_body_wrong_kwarg_raises(self, table_builder):
        with pytest.raises(ValueError, match="must be either 'header' or
    'body'"):
...
class TestRowStringConverter:
    ...
    def test_get_strrow_normal_without_escape(self, row_num, expected):
        df = DataFrame({r"Design": [1, 2, 3], r"ratio": [4, 5, 6], r"xy": [10,
    11, 12]})
    ...
    def test_get_strrow_normal_with_escape(self, row_num, expected):
        df = DataFrame(
            {r"Design #": [1, 2, 3], r"ratio, %": [4, 5, 6], r"x&y": [10, 11,
    12]}
        )
        row_string_converter = RowStringConverter(
            formatter=DataFrameFormatter(df, escape=True),
