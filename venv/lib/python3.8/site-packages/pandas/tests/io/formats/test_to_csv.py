...
"""
    ...
    def test_to_csv_na_rep(self):
        ...
        expected = tm.convert_rows_list_to_csv_str([",0", "0,a", "1,ZZZZZ",
    "2,c"])
        assert expected == csv
    ...
    def test_to_csv_date_format(self):
        # GH 10209
        df_sec = DataFrame({"A": pd.date_range("20130101", periods=5,
    freq="s")})
        df_day = DataFrame({"A": pd.date_range("20130101", periods=5,
    freq="d")})
    ...
    def test_to_csv_date_format_in_categorical(self):
        # GH#40754
        ser = pd.Series(pd.to_datetime(["2021-03-27", pd.NaT],
    format="%Y-%m-%d"))
    ...
    def test_to_csv_multi_index(self):
        ...
        df = DataFrame([1], columns=pd.MultiIndex.from_arrays([["foo"],
    ["bar"]]))
...
"""
    ...
    def test_to_csv_stdout_file(self, capsys):
        # GH 21561
        df = DataFrame([["foo", "bar"], ["baz", "qux"]], columns=["name_1",
    "name_2"])
...
"""
    ...
    @pytest.mark.parametrize("archive_name", ["test_to_csv.csv",
    "test_to_csv.zip"])
    def test_to_csv_zip_arguments(self, compression, archive_name):
        ...
        with tm.ensure_clean("to_csv_archive_name.zip") as path:
            df.to_csv(
                path, compression={"method": compression, "archive_name":
    archive_name}
            )
    ...
    def test_na_rep_truncated(self):
        ...
        expected = tm.convert_rows_list_to_csv_str([",0", "0,8", "1,9",
    "2,10", "3,11"])
        assert result == expected
    ...
    def test_to_csv_binary_handle(self, mode):
        """
        Binary file objects should work (if 'mode' contains a 'b') or even
    without
        it in most cases.
