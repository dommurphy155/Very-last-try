...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            [
                {"is_visible": True, "attributes": 'colspan="2"', "value":
    "c0"},
...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            [
                {"is_visible": True, "attributes": 'rowspan="2"', "value":
    "i0"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1000, 3, 750, None, None, 250, 3),  # dynamically reduce rows to 250,
    keep cols
        (4, 1000, 500, None, None, 4, 125),  # dynamically reduce cols to 125,
    keep rows
        (1000, 3, 750, 10, None, 10, 3),  # overwrite above dynamics with
    max_row
        (4, 1000, 500, None, 5, 4, 5),  # overwrite above dynamics with max_col
        (100, 100, 700, 50, 50, 25, 25),  # rows cols below given maxes so <
    700 elmts
    ],
)
...
def test_render_empty_mi():
    # GH 43305
    df = DataFrame(index=MultiIndex.from_product([["A"], [0, 1]], names=[None,
    "one"]))
    expected = dedent(
        """\
    >
...
def test_copy(comprehensive, render, deepcopy, mi_styler, mi_styler_comp):
    ...
    s2 = copy.deepcopy(styler) if deepcopy else copy.copy(styler)  # make copy
    and check
    ...
    if not deepcopy:  # check memory locations are equal for all included
    attributes
...
def test_clear(mi_styler_comp):
    # NOTE: if this test fails for new features then 'mi_styler_comp' should
    be updated
    # to ensure proper testing of the 'copy', 'clear', 'export' methods with
    new feature
    # GH 40675
...
def test_export(mi_styler_comp, mi_styler):
    ...
    for attr in exp_attrs:
        ...
        assert not (
            all(check) if (hasattr(check, "__iter__") and len(check) > 0) else
    check
        )
    ...
    for attr in exp_attrs:
        ...
        assert all(check) if (hasattr(check, "__iter__") and len(check) > 0)
    else check
...
def test_hide_raises(mi_styler):
    ...
    with pytest.raises(ValueError, match=msg):
        mi_styler.hide(axis="index", subset="something", level="something
    else")
...
def test_hide_index_level(mi_styler, level):
    mi_styler.index.names, mi_styler.columns.names = ["zero", "one"], ["zero",
    "one"]
...
def test_apply_map_header(method, axis):
    ...
    func = {
        "apply": lambda s: ["attr: val" if ("A" in v or "C" in v) else "" for
    v in s],
...
def test_apply_map_header_mi(mi_styler, method, axis):
    ...
    result = getattr(mi_styler, f"{method}_index")(func[method],
    axis=axis)._compute()
...
def test_apply_map_header_raises(mi_styler):
    # GH 41893
    with pytest.raises(ValueError, match="No axis named bad for object type
    DataFrame"):
...
class TestStyler:
    ...
    def test_update_ctx_flatten_multi_and_trailing_semi(self, styler):
        attrs = DataFrame({"A": ["color: red; foo: bar", "color:blue ; foo:
    baz;"]})
    ...
    def test_multiple_render(self, df):
        # GH 39396
        s = Styler(df, uuid_len=0).applymap(lambda x: "color: red;",
    subset=["A"])
    ...
    labels OK
        ...
        func = lambda s: Series(["color: red;", "color: blue;"], index=["Y",
    "X"])
        result = df.style.apply(func, axis=axis)._compute().ctx
    ...
    def test_apply_dataframe_return(self, index, columns):
        ...
        assert result[(1, 1)] == [("color", "red")]  # (Y,Y) styles always
    present
        assert (result[(0, 1)] == [("color", "red")]) is index  # (X,Y) only
    if index
        assert (result[(1, 0)] == [("color", "red")]) is columns  # (Y,X) only
    if cols
    ...
    def test_apply_subset(self, slice_, axis, df):
        ...
        result = df.style.apply(h, axis=axis, subset=slice_,
    ...
    def test_applymap_subset(self, slice_, df):
        result = df.style.applymap(lambda x: "color:baz;",
    ...
    def test_applymap_subset_multiindex_code(self):
        ...
        df = DataFrame(
            [[1, -1, 1, 1], [-1, 1, 1, 1]], index=["hello", "world"],
    columns=columns
        )
    ...
    def test_table_styles(self, df):
        style = [{"selector": "th", "props": [("foo", "bar")]}]  # default
    format
    ...
    def test_table_styles_dict_multiple_selectors(self, df):
        ...
        result = df.style.set_table_styles(
            {
                "B": [
                    {"selector": "th,td", "props": [("border-left", "2px solid
    black")]}
                ]
            }
        )._translate(True, True)["table_styles"]
        ...
        expected = [
            {"selector": "th.col1", "props": [("border-left", "2px solid
    black")]},
            {"selector": "td.col1", "props": [("border-left", "2px solid
    black")]},
        ]
    ...
    def test_apply_none(self):
        ...
        result = DataFrame([[1, 2], [3, 4]]).style.apply(f,
    ...
    def test_export(self, df, styler):
        ...
        style1 = styler
        style1.applymap(f).applymap(g, z="b").highlight_max()._compute()  # =
    render
    ...
    def test_bad_apply_shape(self):
        ...
        with pytest.raises(ValueError, match=msg.format("index")):
            df.style._apply(lambda x: Series(["a:v;", ""], index=["A", "C"]),
    axis=0)
        ...
        with pytest.raises(ValueError, match=msg.format("columns")):
            df.style._apply(lambda x: Series(["a:v;", ""], index=["X", "Z"]),
    axis=1)
    ...
    def test_mi_sparse_column_names(self, blank_value):
        df = DataFrame(
            ...
            columns=MultiIndex.from_arrays(
                [["C1", "C1", "C2", "C2"], [1, 0, 1, 0]], names=["colnam_0",
    "colnam_1"]
            ),
        )
    ...
    def test_hide_single_index(self, df):
        ...
        ctx4 = df.set_index("A").style.hide(axis="index")._translate(True,
    True)
    ...
    def test_hide_multiindex(self):
        ...
        assert len(ctx1["head"][0]) == 4  # two visible indexes and two data
    columns
        ...
        # check for blank header rows
        assert len(ctx2["head"][0]) == 3  # one hidden (col name) and two data
    columns
        assert not ctx2["head"][0][0]["is_visible"]
    ...
    def test_hide_columns_index_mult_levels(self):
        ...
        ctx = df.style.hide([("b", 1)], axis=1).hide(axis=0)._translate(True,
    True)
        ...
        for i in [0, 1, 2, 3]:
            assert "row1" in ctx["body"][0][i]["class"]  # row0 not included
    in body
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame(data=[["test-class", "unused"]], columns=["B", "C"],
    index=["a"]),
        ],
    )
    def test_set_data_classes(self, classes):
        # GH 36159
        df = DataFrame(data=[[0, 1], [2, 3]], columns=["A", "B"], index=["a",
    "b"])
        s = Styler(df, uuid_len=0,
    cell_ids=False).set_td_classes(classes).to_html()
        ...
        # GH 39317
        s = Styler(df, uuid_len=0,
    cell_ids=True).set_td_classes(classes).to_html()
        assert '<td id="T__row0_col0" class="data row0 col0" >0</td>' in s
        assert '<td id="T__row0_col1" class="data row0 col1 test-class"
    >1</td>' in s
    ...
    def test_set_data_classes_reindex(self):
        # GH 39317
        df = DataFrame(
            data=[[0, 1, 2], [3, 4, 5], [6, 7, 8]], columns=[0, 1, 2],
    index=[0, 1, 2]
        )
        classes = DataFrame(
            data=[["mi", "ma"], ["mu", "mo"]],
    ...
    def test_column_and_row_styling(self):
        ...
        df = DataFrame(data=[[0, 1], [1, 2]], columns=["A", "B"])
        s = Styler(df, uuid_len=0)
        s = s.set_table_styles({"A": [{"selector": "", "props": [("color",
    "blue")]}]})
        assert "#T_ .col0 {\n  color: blue;\n}" in s.to_html()
...
def test_hidden_index_names(mi_df):
    ...
    for i in range(4):
        assert ctx["body"][0][i]["is_visible"]  # 2 index levels + 2 data
    values visible
...
def test_hidden_column_names(mi_df):
    ...
    ctx = mi_styler._translate(True, True)
    assert len(ctx["head"]) == 1  # no index names and only one visible column
    headers
    assert ctx["head"][0][1]["display_value"] == "&nbsp;"
...
def test_get_level_lengths_mi_hidden():
    ...
    result = _get_level_lengths(
        ...
        sparsify=False,
        max_index=100,
        hidden_elements=[0, 1, 0, 1],  # hidden element can repeat if
    duplicated index
    )
    tm.assert_dict_equal(result, expected)
...
def test_row_trimming_hide_index_mi():
    ...
    assert {"value": 0, "attributes": 'rowspan="2"', "is_visible":
    True}.items() <= ctx[
        "body"
    ][0][0].items()
    assert {"value": 0, "attributes": "", "is_visible": False}.items() <=
    ctx["body"][
        1
    ][0].items()
    ...
    for r, val in enumerate(["2", "3", "..."]):
        assert ctx["body"][r][1]["display_value"] == val  # level 1 index
    headers
    for r, val in enumerate(["3", "4", "..."]):
...
def test_col_trimming_hide_columns():
    ...
    assert len(ctx["head"][0]) == 6  # blank, [0, 1 (hidden)], [2 ,3
    (visible)], + trim
    for c, vals in enumerate([(1, False), (2, True), (3, True), ("...",
    True)]):
        assert ctx["head"][0][c + 2]["value"] == vals[0]
        assert ctx["head"][0][c + 2]["is_visible"] == vals[1]

    assert len(ctx["body"][0]) == 6  # index + 2 hidden + 2 visible + trimming
    col
