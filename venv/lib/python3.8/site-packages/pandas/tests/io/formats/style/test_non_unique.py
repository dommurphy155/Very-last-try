...
def test_format_non_unique(df):
    ...
    for val in ["1.000000<", "4.000000<", "7.000000<", "2.000000<",
    "3.000000<"]:
        assert val in html
...
def test_apply_applymap_non_unique_raises(df, func):
    ...
    with pytest.raises(KeyError, match="`Styler.apply` and `.applymap` are
    not"):
...
def test_tooltips_non_unique_raises(styler):
    ...
    ttips = DataFrame([["1", "2"], ["3", "4"]], columns=["c", "d"],
    index=["a", "b"])
    styler.set_tooltips(ttips=ttips)  # OK

    # ttips has non-unique columns
    ttips = DataFrame([["1", "2"], ["3", "4"]], columns=["c", "c"],
    index=["a", "b"])
    with pytest.raises(KeyError, match="Tooltips render only if `ttips` has
    unique"):
...
def test_set_td_classes_non_unique_raises(styler):
    ...
    classes = DataFrame([["1", "2"], ["3", "4"]], columns=["c", "d"],
    index=["a", "b"])
    styler.set_td_classes(classes=classes)  # OK

    # classes has non-unique columns
    classes = DataFrame([["1", "2"], ["3", "4"]], columns=["c", "c"],
    index=["a", "b"])
    with pytest.raises(KeyError, match="Classes render only if `classes` has
    unique"):
