...
def test_display_format(styler):
    ...
    assert all([len(c["display_value"]) <= 3 for c in row[1:]] for row in
    ctx["body"])
...
def test_format_dict(styler):
    ctx = styler.format({"A": "{:0.1f}", "B": "{0:.2%}"})._translate(True,
    True)
...
def test_format_callable(styler):
    ctx = styler.format(lambda v: "neg" if v < 0 else "pos")._translate(True,
    True)
...
def test_format_with_na_rep():
    ...
    ctx = df.style.format("{:.2%}", na_rep="-", subset=["B"])._translate(True,
    True)
    assert ctx["body"][0][2]["display_value"] == "-"
    assert ctx["body"][1][2]["display_value"] == "120.00%"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "\\textbackslash \\textasciitilde \\space \\textasciicircum
    \\space "
            "\\textbackslash \\space ",
        ),
    ],
...
def test_format_escape_html(escape, exp):
    ...
    assert styler._translate(True, True)["head"][0][1]["display_value"] ==
    f"&{chars}&"
...
def test_format_escape_na_rep():
    ...
    ex = '<td id="T__row0_col0" class="data row0 col0"
    >X&&lt;&gt;&amp;&#34;>X</td>'
...
def test_format_with_precision(precision, expected):
    # Issue #13257
    df = DataFrame([[1.0, 2.0090, 3.2121, 4.566]], columns=[1.0, 2.0090,
    3.2121, 4.566])
    styler = Styler(df)
    ...
    for col, exp in enumerate(expected):
        ...
        assert ctx["head"][0][col + 1]["display_value"] == exp  # format_index
    test
...
def test_format_index_level(axis, level, expected):
    midx = MultiIndex.from_arrays([["_", "_"], ["_", "_"]], names=["zero",
    "one"])
...
def test_format_subset():
    ...
    ctx = df.style.format("{:0.1f}", subset=IndexSlice[0,:])._translate(True,
    True)
    assert ctx["body"][0][1]["display_value"] == expected
    assert ctx["body"][1][1]["display_value"] == raw_11

    ctx = df.style.format("{:0.1f}", subset=IndexSlice["a"])._translate(True,
    True)
    assert ctx["body"][0][1]["display_value"] == expected
    assert ctx["body"][0][2]["display_value"] == "0.123400"

    ctx = df.style.format("{:0.1f}", subset=IndexSlice[0,
...
def test_format_thousands(formatter, decimal, precision, func, col):
    ...
    result = getattr(styler, func)(  # testing float
        thousands="_", formatter=formatter, decimal=decimal,
    precision=precision
    )._translate(True, True)
    ...
    styler = DataFrame([[1 + 1000000.123456789j]], index=[1 +
    1000000.123456789j]).style
...
def test_format_decimal(formatter, thousands, precision, func, col):
    styler = DataFrame([[1000000.123456789]], index=[1000000.123456789]).style
    result = getattr(styler, func)(  # testing float
        decimal="_", formatter=formatter, thousands=thousands,
    precision=precision
    )._translate(True, True)
...
def test_format_options():
    ...
    with option_context("styler.format.escape", "latex"):
        ctx_with_op = df.style._translate(True, True)
        assert ctx_with_op["body"][1][3]["display_value"] ==
    "\\&\\textasciitilde "
