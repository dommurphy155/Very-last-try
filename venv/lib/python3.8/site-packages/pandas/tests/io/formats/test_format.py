...
def has_truncated_repr(df):
    return has_horizontally_truncated_repr(df) or
    has_vertically_truncated_repr(df)


def has_doubly_truncated_repr(df):
    return has_horizontally_truncated_repr(df) and
    has_vertically_truncated_repr(df)
...
class TestDataFrameFormatting:
    ...
    def test_show_null_counts_deprecation(self):
        ...
        # GH37999
        with pytest.raises(ValueError, match=r"null_counts used with
    show_counts.+"):
    ...
    def test_repr_non_interactive(self):
        ...
        with option_context(
            "mode.sim_interactive", False, "display.width", 0,
    ...
    def test_repr_max_columns_max_rows(self):
        ...
        if term_width < 10 or term_height < 10:
            pytest.skip(f"terminal size too small, {term_width} x
    {term_height}")
    ...
    "ええええええ"]},
        ...
        # all
        df = DataFrame(
            {"あああ": ["あああ", "い", "う", "えええええ"], "いいいいい": ["あ", "いいい", "う",
    "ええ"]},
    ...
    "ええええええ"]},
            ...
            # column name
            df = DataFrame(
                {"b": ["あ", "いいい", "う", "ええええええ"], "あああああ": [1, 222, 33333,
    4]},
                index=["a", "bb", "c", "ddd"],
            )
            expected = (
    ...
    3):
            ...
            # ambiguous unicode
            df = DataFrame(
                {"b": ["あ", "いいい", "¡¡", "ええええええ"], "あああああ": [1, 222, 33333,
    4]},
                index=["a", "bb", "c", "¡¡¡"],
            )
            expected = (
    ...
    def test_to_string_truncate_indices(self, index, h, w):
        with option_context("display.expand_frame_repr", False):
            ...
            with option_context("display.max_rows", 15, "display.max_columns",
    15):
                if h == 20 and w == 20:
    ...
    def test_datetimeindex_highprecision(self, start_date):
        ...
        df = DataFrame({"A": date_range(start=start_date, freq="D",
    periods=5)})
        result = str(df)
    ...
    - 1)))
            df = DataFrame(
                tm.rands_array(25, (10, max_cols - 1)), index=midx,
    columns=mcols
            )
            df.index.names = ["Level 0", "Level 1"]
            with option_context("display.expand_frame_repr", False):
    ...
    def test_wide_repr_wide_long_columns(self):
        with option_context("mode.sim_interactive", True):
            df = DataFrame({"a": ["a" * 30, "b" * 30], "b": ["c" * 70, "d" *
    80]})
    ...
    def test_to_string(self):
        ...
        assert (np.abs(recons["A"].dropna() - biggie["A"].dropna()) <
    0.1).all()
        ...
        biggie.to_string(columns=["B", "A"], formatters={"A": lambda x:
    f"{x:.1f}"})
    ...
    def test_to_string_float_formatting(self):
        ...
        df = DataFrame(
            {"x": [0, 0.25, 3456.000, 12e45, 1.64e6, 1.7e8, 1.253456, np.pi,
    -1e6]}
        )
...
"""
    ...
    def test_repr_html_wide_multiindex_cols(self):
        ...
        mcols = MultiIndex.from_product(
            [np.arange(max_cols // 2), ["foo", "bar"]], names=["first",
    "second"]
        )
    ...
    columns=mcols)
        ...
        mcols = MultiIndex.from_product(
            (np.arange(1 + (max_cols // 2)), ["foo", "bar"]), names=["first",
    "second"]
        )
        df = DataFrame(tm.rands_array(25, size=(10, len(mcols))),
    columns=mcols)
    ...
    def test_repr_html_long(self):
        with option_context("display.max_rows", 60):
            ...
            h = max_rows - 1
            df = DataFrame({"A": np.arange(1, 1 + h), "B": np.arange(41, 41 +
    h)})
            reg_repr = df._repr_html_()
            assert ".." not in reg_repr
    ...
    def test_fake_qtconsole_repr_html(self, float_frame):
        ...
        def get_ipython():
            return {"config": {"KernelApp": {"parent_appname":
    "ipython-qtconsole"}}}
...
class TestSeriesFormatting:
    ...
    def test_to_string(self):
        ...
        assert last_line == (f"Freq: B, Name: foo, Length: {len(cp)}, dtype:
    float64")
    ...
    object"
        ...
        # both
        s = Series(["あ", "いい", "ううう", "ええええ"], index=["ああ", "いいいい", "う",
    "えええ"])
        expected = (
            "ああ         あ\nいいいい      いい\nう        ううう\nえええ     ええええ\ndtype:
    object"
    ...
    "あああ"]
        ...
        with option_context("display.unicode.east_asian_width", True):
            ...
            # unicode index
            s = Series(["a", "bb", "CCC", "D"], index=["あ", "いい", "ううう",
    "ええええ"])
            expected = (
                "あ            a\nいい         bb\nううう      CCC\n"
                "ええええ      D\ndtype: object"
            ...
            # unicode values
            s = Series(["あ", "いい", "ううう", "ええええ"], index=["a", "bb", "c",
    "ddd"])
            expected = (
                "a            あ\nbb         いい\nc        ううう\n"
                "ddd    ええええ\ndtype: object"
    ...
    def test_consistent_format(self):
        ...
        with option_context("display.max_rows", 10, "display.show_dimensions",
    False):
            res = repr(s)
        exp = (
            "0      1.0000\n1      1.0000\n2      1.0000\n3      "
    ...
    def test_format_explicit(self):
        ...
        with option_context("display.max_rows", 4, "display.show_dimensions",
    False):
...
class TestGenericArrayFormatter:
    def test_1d_array(self):
        # GenericArrayFormatter is used on types for which there isn't a
    dedicated
    ...
    def test_2d_array(self):
        obj = fmt.GenericArrayFormatter(np.array([[True, False], [False,
    True]]))
        res = obj.get_result()
        assert len(res) == 2
...
class TestPeriodIndexFormat:
    def test_period_format_and_strftime_default(self):
        ...
        per = pd.period_range("2003-01-01 12:01:01.123456789", periods=2,
    freq="n")
        formatted = per.format()
    ...
    def test_period_custom(self):
        ...
        # 6 digits
        per = pd.period_range("2003-01-01 12:01:01.123456", periods=2,
    freq="u")
        formatted = per.format(date_format="%y %I:%M:%S (ms=%l us=%u ns=%n)")
    ...
    def test_period_tz(self):
        ...
        with tm.assert_produces_warning(UserWarning, match="will drop
    timezone"):
...
class TestDatetimeIndexFormat:
    ...
    def test_date_tz(self):
        ...
        formatted = pd.to_datetime([datetime(2013, 1, 1), NaT],
    utc=True).format()
        assert formatted[0] == "2013-01-01 00:00:00+00:00"
...
class TestDatetimeIndexUnicode:
    def test_dates(self):
        text = str(pd.to_datetime([datetime(2013, 1, 1), datetime(2014, 1,
    1)]))
    ...
    def test_mixed(self):
        text = str(
            pd.to_datetime(
                [datetime(2013, 1, 1), datetime(2014, 1, 1, 12),
    datetime(2014, 1, 1)]
            )
        )
...
def test_filepath_or_buffer_arg(
    ...
    if filepath_or_buffer_id not in ["string", "pathlike"] and encoding is not
    None:
        with pytest.raises(
            ValueError, match="buf is not a file name and encoding is
    specified."
        ):
