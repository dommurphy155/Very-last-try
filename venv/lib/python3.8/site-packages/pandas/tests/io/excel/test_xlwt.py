    ...
    with tm.ensure_clean(ext) as f:
        ...
        with tm.assert_produces_warning(FutureWarning, match=msg):
            with ExcelWriter(f, engine="xlwt", **kwargs) as writer:
                assert (
                    writer.book._Workbook__styles.style_compression ==
    style_compression
                )
...
def test_engine_kwargs(ext, style_compression):
    ...
    with tm.ensure_clean(ext) as f:
        with ExcelWriter(f, engine="xlwt", engine_kwargs=engine_kwargs) as
    writer:
            assert writer.book._Workbook__styles.style_compression ==
    style_compression
            # xlwt won't allow us to close without writing something
            DataFrame().to_excel(writer)
