...
class TestReaders:
    ...
    def test_usecols_list(self, request, read_ext, df_ref):
        if read_ext == ".xlsb":
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="Sheets containing datetimes not supported by
    pyxlsb"
                )
            )
        ...
        df1 = pd.read_excel(
            "test1" + read_ext, sheet_name="Sheet1", index_col=0, usecols=[0,
    2, 3]
        )
        df2 = pd.read_excel(
            "test1" + read_ext,
    ...
    @pytest.mark.parametrize(
        "usecols", [[0, 1, 3], [0, 3, 1], [1, 0, 3], [1, 3, 0], [3, 0, 1], [3,
    1, 0]]
    )
    ...
    def test_usecols_diff_positional_str_columns_order(self, read_ext,
    usecols, df_ref):
        expected = df_ref[["B", "D"]]
    ...
    def test_index_col_empty(self, read_ext):
        ...
        expected = DataFrame(
            columns=["D", "E", "F"],
            index=MultiIndex(levels=[[]] * 3, codes=[[]] * 3, names=["A", "B",
    "C"]),
        )
    ...
    def test_index_col_with_unnamed(self, read_ext, index_col):
        ...
        expected = DataFrame(
            [["i1", "a", "x"], ["i2", "b", "y"]], columns=["Unnamed: 0",
    "col1", "col2"]
        )
        if index_col:
    ...
    pyxlsb"
        ...
        with tm.assert_produces_warning(
            ...
            # raise_on_extra_warnings because xlrd raises a
    PendingDeprecationWarning
        ...
        actual = pd.read_excel(
            basename + read_ext, sheet_name="Sheet1", converters={"StrCol":
    str}
        )
        tm.assert_frame_equal(actual, expected)
    ...
    def test_reader_dtype(self, read_ext):
        ...
        actual = pd.read_excel(
            basename + read_ext, dtype={"a": "float64", "b": "float32", "c":
    str}
        )
    ...
    def test_read_excel_ods_nested_xml(self, engine, read_ext, basename,
    expected):
        # see gh-35802
        if engine != "odf":
    ...
    def test_read_excel_blank_with_header(self, read_ext):
        ...
        actual = pd.read_excel("blank_with_header" + read_ext,
    sheet_name="Sheet1")
        tm.assert_frame_equal(actual, expected)

    @pytest.mark.filterwarnings("ignore:Cell A4 is
    marked:UserWarning:openpyxl")
    ...
    pyxlsb"
        ...
        if engine is None and read_ext in (".xlsx", ".xlsm"):
            # GH 35029
            request.node.add_marker(
                pytest.mark.xfail(reason="Defaults to openpyxl, maybe not
    supported")
            )
    ...
    @pytest.mark.parametrize(
        "sheet_name",
        [3, [0, 3], [3, 0], "Sheet4", ["Sheet1", "Sheet4"], ["Sheet4",
    "Sheet1"]],
    )
    ...
    directory"
        match = r"(No such file or directory|没有那个文件或目录|File o directory non
    esistente)"
    ...
    pyxlsb"
        ...
        actual = pd.read_excel(mi_file, sheet_name="mi_index", index_col=[0,
    1])
    ...
    def test_read_excel_multiindex_header_only(self, read_ext):
        ...
        mi_file = "testmultiindex" + read_ext
        result = pd.read_excel(mi_file, sheet_name="index_col_none",
    header=[0, 1])
    ...
    name=None
        ...
        actual = pd.read_excel(filename, sheet_name="single_names",
    index_col=0)
        tm.assert_frame_equal(actual, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("testskiprows", "skiprows_list", None, None, lambda x: x == 0 or
    x == 2),
        ],
    )
    ...
    def test_no_header_with_list_index_col(self, read_ext):
        ...
        result = pd.read_excel(
            file_name, sheet_name="index_col_none", index_col=[0, 1],
    header=None
        )
    ...
    def test_ignore_chartsheets_by_str(self, request, engine, read_ext):
        ...
        if engine == "pyxlsb":
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="pyxlsb can't distinguish chartsheets from
    worksheets"
                )
            )
        with pytest.raises(ValueError, match="Worksheet named 'Chart1' not
    found"):
    ...
    worksheets"
        ...
        with pytest.raises(
            ValueError, match="Worksheet index 1 is invalid, 1 worksheets
    found"
        ):
...
class TestExcelFileRead:
    ...
    def test_excel_passes_na(self, read_ext):
        with pd.ExcelFile("test4" + read_ext) as excel:
            parsed = pd.read_excel(
                excel, sheet_name="Sheet1", keep_default_na=False,
    na_values=["apple"]
            )
        expected = DataFrame(
        ...
        with pd.ExcelFile("test4" + read_ext) as excel:
            parsed = pd.read_excel(
                excel, sheet_name="Sheet1", keep_default_na=True,
    na_values=["apple"]
            )
        expected = DataFrame(
    ...
    def test_header_with_index_col(self, filename):
        # GH 33476
        idx = Index(["Z"], name="I2")
        cols = MultiIndex.from_tuples([("A", "B"), ("A", "B.1")],
    names=["I11", "I12"])
