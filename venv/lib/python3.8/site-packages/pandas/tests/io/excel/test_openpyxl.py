...
def test_to_excel_styleconverter(ext):
    ...
    hstyle = {
        ...
        "borders": {"top": "thin", "right": "thin", "bottom": "thin", "left":
    "thin"},
        "alignment": {"horizontal": "center", "vertical": "top"},
        "fill": {"patternType": "solid", "fgColor": {"rgb": "006666FF",
    "tint": 0.3}},
...
def test_write_cells_merge_styled(ext):
    ...
    merge_cells = [
        ExcelCell(
            col=0, row=0, val="pandas", mergestart=1, mergeend=1,
    style=sty_merged
        )
    ]
...
def test_engine_kwargs_write(ext, iso_dates):
    ...
    with tm.ensure_clean(ext) as f:
        with ExcelWriter(f, engine="openpyxl", engine_kwargs=engine_kwargs) as
    writer:
            assert writer.book.iso_dates == iso_dates
            # ExcelWriter won't allow us to close without writing something
            DataFrame().to_excel(writer)
...
def test_engine_kwargs_append_invalid(ext):
    ...
    with tm.ensure_clean(ext) as f:
        ...
        with pytest.raises(
            TypeError,
            match=re.escape(
                "load_workbook() got an unexpected keyword argument
    'apple_banana'"
            ),
        ):
            with ExcelWriter(
                f, engine="openpyxl", mode="a", engine_kwargs={"apple_banana":
    "fruit"}
            ) as writer:
...
def test_engine_kwargs_append_data_only(ext, data_only, expected):
    ...
    with tm.ensure_clean(ext) as f:
        ...
        with ExcelWriter(
            f, engine="openpyxl", mode="a", engine_kwargs={"data_only":
    data_only}
        ) as writer:
...
def test_if_sheet_exists_append_modes(ext, if_sheet_exists, num_sheets,
    expected):
    # GH 40230
...
def test_append_overlay_startrow_startcol(ext, startrow, startcol, greeting,
    goodbye):
    df1 = DataFrame({"greeting": ["hello", "world"], "goodbye": ["goodbye",
    "people"]})
    df2 = DataFrame(["poop"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "error",
            "Sheet 'foo' already exists and if_sheet_exists is set to
    'error'.",
        ),
        (
            None,
...
def test_read_multiindex_header_no_index_names(datapath, ext):
    ...
    expected = DataFrame(
        ...
        columns=pd.MultiIndex.from_tuples(
            [("X", "Y", "A1"), ("X", "Y", "A2"), ("XX", "YY", "B1"), ("XX",
    "YY", "B2")]
        ),
        index=pd.MultiIndex.from_tuples([("A", "AA", "AAA"), ("A", "BB",
    "BBB")]),
    )
    tm.assert_frame_equal(result, expected)
