...
# jinja2 is currently required for Styler.__init__(). Technically
    Styler.to_excel
# could compute styles and render to excel without jinja2, since there is no
# 'template' file, but this needs the import error to delayed until render
    time.
...
def test_styler_to_excel_unstyled(engine):
    ...
    with tm.ensure_clean(".xlsx") as path:
        ...
        openpyxl = pytest.importorskip("openpyxl")  # test loading only with
    openpyxl
...
shared_style_params = [
    ...
    ("border-left: 2pt dotted red", ["border", "left", "style"],
    "mediumDashDotDot"),
...
def test_styler_to_excel_basic(engine, css, attrs, expected):
    ...
    openpyxl
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            ...
            u_cell, s_cell = wb["dataframe"].cell(2, 2), wb["styled"].cell(2,
    2)
        for attr in attrs:
...
def test_styler_to_excel_basic_indexes(engine, css, attrs, expected):
    ...
    openpyxl
        with contextlib.closing(openpyxl.load_workbook(path)) as wb:
            ...
            # test styled cell has expected styles
            ui_cell, si_cell = wb["null_styled"].cell(2, 1),
    wb["styled"].cell(2, 1)
            uc_cell, sc_cell = wb["null_styled"].cell(1, 2),
    wb["styled"].cell(1, 2)
        for attr in attrs:
            ui_cell, si_cell = getattr(ui_cell, attr, None), getattr(si_cell,
    attr)
            uc_cell, sc_cell = getattr(uc_cell, attr, None), getattr(sc_cell,
    attr)
...
# Note: Leaving behavior of "width"-type styles undefined; user should use
    border-width
# instead
excel_border_styles = [
    # "thin",
...
def test_styler_to_s3(s3_resource, s3so):
    ...
    while True:
        ...
        result = read_excel(
            f"s3://{mock_bucket_name}/{target_file}", index_col=0,
    storage_options=s3so
        )
