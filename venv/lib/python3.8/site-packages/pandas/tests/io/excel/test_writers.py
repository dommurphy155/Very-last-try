...
@pytest.mark.parametrize(
    ...
    td.skip_if_no("xlrd")]),
        pytest.param(".xlsm", marks=[td.skip_if_no("openpyxl"),
    td.skip_if_no("xlrd")]),
        pytest.param(".xls", marks=[td.skip_if_no("xlwt"),
    td.skip_if_no("xlrd")]),
        pytest.param(
...
class TestRoundTrip:
    @pytest.mark.parametrize(
        "header,expected",
        [(None, DataFrame([np.nan] * 4)), (0, DataFrame({"Unnamed: 0":
    [np.nan] * 3}))],
    )
    def test_read_one_empty_col_no_header(self, ext, header, expected):
        # xref gh-12292
        filename = "no_header"
        df = DataFrame([["", 1, 100], ["", 2, 200], ["", 3, 300], ["", 4,
    400]])
    ...
    def test_set_column_names_in_parameter(self, ext):
        ...
        refdf = DataFrame([[1, "foo"], [2, "bar"], [3, "baz"]], columns=["a",
    "b"])

        with tm.ensure_clean(ext) as pth:
            with ExcelWriter(pth) as writer:
                refdf.to_excel(writer, "Data_no_head", header=False,
    index=False)
            ...
            with ExcelFile(pth) as reader:
                xlsdf_no_head = pd.read_excel(
                    reader, sheet_name="Data_no_head", header=None,
    names=["A", "B"]
                )
    ...
    def test_read_excel_parse_dates(self, ext):
        ...
        df = DataFrame(
            {"col": [1, 2, 3], "date_strings": pd.date_range("2012-01-01",
    periods=3)}
        )
        df2 = df.copy()
        ...
        with tm.ensure_clean(ext) as pth:
            ...
            res = pd.read_excel(
                pth, parse_dates=["date_strings"], date_parser=date_parser,
    index_col=0
            )
            tm.assert_frame_equal(df, res)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "xlwt", ".xls", marks=[td.skip_if_no("xlwt"),
    td.skip_if_no("xlrd")]
        ),
        pytest.param(
            "xlsxwriter",
...
class TestExcelWriter:
    ...
    @pytest.mark.parametrize("np_type", [np.int8, np.int16, np.int32,
    np.int64])
    ...
    def test_float_types(self, np_type, path):
        ...
        with ExcelFile(path) as reader:
            recons = pd.read_excel(reader, sheet_name="test1",
    index_col=0).astype(
                np_type
            )
    ...
    def test_roundtrip_indexlabels(self, merge_cells, frame, path):
        ...
        df.to_excel(path, "test1", index_label=["test"],
    ...
    def test_excel_roundtrip_indexname(self, merge_cells, path):
        ...
        with ExcelFile(path) as xf:
            result = pd.read_excel(xf, sheet_name=xf.sheet_names[0],
    index_col=0)
    ...
    dtype=np.int64)
        expected = df.copy()
        intervals = pd.cut(
            df[0], 10, labels=["A", "B", "C", "D", "E", "F", "G", "H", "I",
    "J"]
        )
        df["new"] = intervals
    ...
    def test_to_excel_timedelta(self, path):
        ...
        df = DataFrame(
            np.random.randint(-10, 10, size=(20, 1)), columns=["A"],
    dtype=np.int64
        )
        expected = df.copy()
    ...
    def test_to_excel_multiindex_nan_label(self, merge_cells, path):
        df = DataFrame({"A": [None, 2, 3], "B": [10, 20, 30], "C":
    np.random.sample(3)})
        df = df.set_index(["A", "B"])
    ...
    (50, 2)])
        ...
        if not merge_cells:
            fm = frame.columns.format(sparsify=False, adjoin=False,
    names=False)
    ...
    def test_to_excel_empty_multiindex(self, path):
        ...
        df = DataFrame([], index=MultiIndex.from_tuples([], names=[0, 1]),
    columns=[2])
        df.to_excel(path, "test1")
    ...
    def test_to_excel_output_encoding(self, ext):
        ...
        with tm.ensure_clean("__tmp_to_excel_float_format__." + ext) as
    filename:
            df.to_excel(filename, sheet_name="TestSheet")
            result = pd.read_excel(filename, sheet_name="TestSheet",
    index_col=0)
            tm.assert_frame_equal(result, df)
    ...
    def test_to_excel_unicode_filename(self, ext):
        with tm.ensure_clean("\u0192u." + ext) as filename:
            ...
            df = DataFrame(
                [[0.123456, 0.234567, 0.567567], [12.32112, 123123.2,
    321321.2]],
                index=["A", "B"],
                columns=["X", "Y", "Z"],
            )
    ...
    def test_excel_010_hemstring(
        ...
        def roundtrip(data, header=True, parser_hdr=0, index=True):
            data.to_excel(path, header=header, merge_cells=merge_cells,
    index=index)
        ...
        df = tm.makeCustomDataframe(
            nrows, ncols, r_idx_nlevels=r_idx_nlevels,
    ...
    def test_duplicated_columns(self, path):
        # see gh-5235
        df = DataFrame([[1, 2, 3], [1, 2, 3], [1, 2, 3]], columns=["A", "B",
    "B"])
        df.to_excel(path, "test1")
        expected = DataFrame(
        ...
        df = DataFrame([[1, 2, 3, 4], [5, 6, 7, 8]], columns=["A", "B", "A",
    "B"])
        df.to_excel(path, "test1")
        ...
        with tm.assert_produces_warning(
            ...
            with pytest.raises(ValueError, match=msg):
                pd.read_excel(
                    path, sheet_name="test1", header=None,
    ...
    def test_write_subset_columns(self, path, to_excel_index,
    read_excel_index_col):
        # GH 31677
        write_frame = DataFrame({"A": [1, 1, 1], "B": [2, 2, 2], "C": [3, 3,
    3]})
    ...
    def test_comment_arg(self, path):
        ...
        df = DataFrame({"A": ["one", "#one", "one"], "B": ["two", "two",
    "#two"]})
    ...
    "#two"]})
        ...
        expected = DataFrame({"A": ["one", None, "one"], "B": ["two", None,
    None]})
    ...
    @pytest.mark.filterwarnings("ignore:Calling
    close():UserWarning:xlsxwriter")
    @pytest.mark.parametrize(
    ...
    def test_deprecated_book_setter(self, engine, ext):
        ...
        with tm.ensure_clean(ext) as path:
            with ExcelWriter(path) as writer:
                msg = "Setting the `book` attribute is not part of the public
    API"
                # Some engines raise if nothing is written
                DataFrame().to_excel(writer)
                book = writer.book
