    ...
    params=[
        pytest.param(
            ...
            marks=pytest.mark.skipif(
                not _HAVE_FASTPARQUET or get_option("mode.data_manager") ==
    "array",
...
@pytest.fixture(
    params=[
        ...
        datetime.datetime.strptime("2019-01-04T16:41:24+0200",
    "%Y-%m-%dT%H:%M:%S%z"),
        datetime.datetime.strptime("2019-01-04T16:41:24+0215",
    "%Y-%m-%dT%H:%M:%S%z"),
        datetime.datetime.strptime("2019-01-04T16:41:24-0200",
    "%Y-%m-%dT%H:%M:%S%z"),
        datetime.datetime.strptime("2019-01-04T16:41:24-0215",
...
def test_get_engine_auto_error_message():
    ...
    if not have_usable_pa and not have_usable_fp:
        ...
        if have_pa_bad_version:
            match = f"Pandas requires version .{pa_min_ver}. or newer of
    .pyarrow."
        ...
        if have_fp_bad_version:
            match = f"Pandas requires version .{fp_min_ver}. or newer of
    .fastparquet."
            with pytest.raises(ImportError, match=match):
                get_engine("auto")
        else:
...
class TestBasic(Base):
    ...
    columns=list("ABC"))
        ...
        for index in [index1, index2]:
            ...
            check_round_trip(df, engine)
            check_round_trip(
                df, engine, read_kwargs={"columns": ["A", "B"]},
    expected=df[["A", "B"]]
            )
    ...
    def test_read_empty_array(self, pa, dtype):
        ...
        check_round_trip(
            df, pa, read_kwargs={"use_nullable_dtypes": True},
    expected=expected
        )


@pytest.mark.filterwarnings("ignore:CategoricalBlock is
...
class TestParquetPyArrow(Base):
    ...
    columns=list("aaa")).copy()
        self.check_error_on_write(df, pa, ValueError, "Duplicate column names
    found")
    ...
    def test_s3_roundtrip_for_dir(
        ...
        # Previous behaviour was pyarrow partitioned columns become 'category'
    dtypes
        # These are added to back of dataframe on read. In new API category
    dtype is
    ...
    def test_write_with_schema(self, pa):
        ...
        check_round_trip(df, pa, write_kwargs={"schema": schema},
    expected=out_df)
    ...
    def test_pyarrow_backed_string_array(self, pa, string_storage):
        ...
        df = pd.DataFrame({"a": pd.Series(["a", None, "c"],
    ...
    available
        if not pa_version_under6p0:
            ...
            ver = "2.0"
        df = pd.DataFrame({"a": pd.date_range("2017-01-01", freq="1n",
    periods=10)})
    ...
    def test_timezone_aware_index(self, request, pa, timezone_aware_date_list):
        if (
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="temporary skip this test until it is properly
    resolved: "
        ...
        # pyarrow always creates fixed offset timezones using
    pytz.FixedOffset()
...
class TestParquetFastParquet(Base):
    ...
    def test_error_on_using_partition_cols_and_partition_on(self, fp, df_full):
        ...
        df = df_full
        msg = (
            "Cannot use both partition_on and partition_cols. Use
    partition_cols for "
            "partitioning data"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_close_file_handle_on_read_error(self):
        with tm.ensure_clean("test.parquet") as path:
            ...
            with pytest.raises(Exception, match=""):  # Not important which
    exception
                read_parquet(path, engine="fastparquet")
