...
def test_encoding_options(datapath):
    ...
    with contextlib.closing(SAS7BDATReader(fname, convert_header_text=False))
    as rdr:
        df3 = rdr.read()
...
def round_datetime_to_ms(ts):
    if isinstance(ts, datetime):
        return ts.replace(microsecond=int(round(ts.microsecond, -3) / 1000) *
    1000)
    elif isinstance(ts, str):
        _ts = dateutil.parser.parse(timestr=ts)
        return _ts.replace(microsecond=int(round(_ts.microsecond, -3) / 1000)
    * 1000)
    else:
        return ts
...
def test_max_sas_date(datapath):
    ...
    expected = pd.DataFrame(
        ...
        columns=["text", "dt_as_float", "dt_as_dt", "date_as_float",
    "date_as_date"],
    )
...
def test_max_sas_date_iterator(datapath):
    # GH 20927
    # when called as an iterator, only those chunks with a date >
    pd.Timestamp.max
    # are returned as datetime.datetime, if this happens that whole chunk is
    returned
    # as datetime.datetime
    col_order = ["text", "dt_as_float", "dt_as_dt", "date_as_float",
    "date_as_date"]
    fname = datapath("io", "sas", "data", "max_sas_date.sas7bdat")
    results = []
...
def test_exception_propagation_rdc_rle_decompress(datapath, monkeypatch,
    test_file):
...
def test_exception_propagation_rle_decompress(tmp_path, datapath):
    """Illegal control byte in RLE decompressor should raise the correct
    ValueError."""
