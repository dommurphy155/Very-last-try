...
@pytest.mark.parametrize(
    ...
    [
        pd.DataFrame(
            100 * [[0.123456, 0.234567, 0.567567], [12.32112, 123123.2,
    321321.2]],
            columns=["X", "Y", "Z"],
        ),
...
@pytest.mark.parametrize(
    ...
    [
        ("to_csv", {"index": False, "header": True}, pd.read_csv, {"squeeze":
    True}),
...
def test_compression_warning(compression_only):
    ...
    with tm.ensure_clean() as path:
        with icom.get_handle(path, "w", compression=compression_only) as
    handles:
...
def test_gzip_compression_level(obj, method):
    ...
    with tm.ensure_clean() as path:
        ...
        getattr(obj, method)(path, compression={"method": "gzip",
    "compresslevel": 1})
...
def test_bzip_compression_level(obj, method):
    ...
    """
    with tm.ensure_clean() as path:
        getattr(obj, method)(path, compression={"method": "bz2",
    "compresslevel": 1})
...
def test_ambiguous_archive_zip():
    with tm.ensure_clean(filename=".zip") as path:
        ...
        file.close()
        with pytest.raises(ValueError, match="Multiple files found in ZIP
    file"):
            pd.read_csv(path)
...
def test_ambiguous_archive_tar():
    with tm.ensure_clean_dir() as dir:
        ...
        with pytest.raises(ValueError, match="Multiple files found in TAR
    archive"):
            pd.read_csv(tarpath)
...
def test_tar_gz_to_different_filename():
    with tm.ensure_clean(filename=".foo") as file:
        pd.DataFrame(
            [["1", "2"]],
            columns=["foo", "bar"],
        ).to_csv(file, compression={"method": "tar", "mode": "w:gz"},
    index=False)
