...
def test_wrong_dtype(datapath, parser, iterparse):
    ...
    with pytest.raises(
        ValueError, match=('Unable to parse string "Everyday Italian" at
    position 0')
    ):
        read_xml(filename, dtype={"title": "Int64"}, parser=parser,
    iterparse=iterparse)
...
def test_both_dtype_converters(parser):
    ...
    with tm.assert_produces_warning(ParserWarning, match="Both a converter and
    dtype"):
        df_result = read_xml(
            xml_types,
            dtype={"degrees": "str"},
...
def test_wrong_converters_type(datapath, parser, iterparse):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(TypeError, match=("Type converters must be a dict or
    subclass")):
        read_xml(filename, converters={"year", str}, parser=parser,
    iterparse=iterparse)
...
def test_callable_func_converters(datapath, parser, iterparse):
    ...
    with pytest.raises(TypeError, match=("'float' object is not callable")):
        read_xml(
            filename, converters={"year": float()}, parser=parser,
    iterparse=iterparse
        )
...
<data>
  ...
  <row>
    ...
    df_iter = read_xml_iterparse(
        ...
        iterparse={"row": ["shape", "degrees", "sides", "year", "month",
    "day"]},
    )

    df_expected = DataFrame(
        {
            "date_end": to_datetime(["2020-12-31", "2021-12-31",
    "2022-12-31"]),
            "shape": ["square", "circle", "triangle"],
            "degrees": [360, 360, 180],
            "sides": [4.0, float("nan"), 3.0],
...
def test_wrong_parse_dates_type(datapath, parser, iterparse):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(
        TypeError, match=("Only booleans, lists, and dictionaries are
    accepted")
    ):
        read_xml(filename, parse_dates={"date"}, parser=parser,
    iterparse=iterparse)
