...
@pytest.fixture(params=[pytest.param("lxml", marks=td.skip_if_no("lxml")),
    "etree"])
def parser(request):
    return request.param
...
def test_parser_consistency_url(parser):
    ...
    with tm.ensure_clean(filename="cta.xml") as path:
        (read_xml(url, xpath=".//row/row", parser=parser).to_xml(path,
    index=False))
...
def test_file_io(datapath, parser, mode):
    ...
    df_io = read_xml(
        (BytesIO(xml_obj) if isinstance(xml_obj, bytes) else
    StringIO(xml_obj)),
        parser=parser,
    )
...
def test_prefix_namespace(parser):
    ...
    df_iter = read_xml_iterparse(
        xml_prefix_nmsp, parser=parser, iterparse={"row": ["shape", "degrees",
    "sides"]}
    )
...
def test_missing_prefix_definition_etree(datapath):
    ...
    with pytest.raises(SyntaxError, match=("you used an undeclared namespace
    prefix")):
...
def test_file_only_attrs(datapath, parser):
    ...
    df_iter = read_xml(filename, parser=parser, iterparse={"book":
    ["category"]})
...
<TrainSchedule>
    ...
    df_iter_lx = read_xml_iterparse(xml, iterparse={"station": ["Name",
    "coords"]})
...
<shapes>
  ...
  <shape type="3D">
    ...
    df_xpath = read_xml(
        xml, xpath=".//shape", parser=parser, names=["type_dim", "shape",
    "type_edge"]
    )
...
<shapes>
  ...
  <shape>
    ...
    df_xpath = read_xml(xml, xpath=".//shape", parser=parser, names=["name",
    "group"])
...
def test_ascii_encoding(datapath, parser):
    filename = datapath("io", "data", "xml", "baby_names.xml")
    with pytest.raises(UnicodeDecodeError, match=("'ascii' codec can't decode
    byte")):
...
def test_default_parser_no_lxml(datapath):
    ...
    with pytest.raises(
        ImportError, match=("lxml not found, please install or use the etree
    parser.")
    ):
        read_xml(filename)
...
def test_empty_stylesheet(val):
    ...
    with pytest.raises(
        XMLSyntaxError, match=("Document is empty|Start tag expected, '<' not
    found")
    ):
    ...
    with pytest.raises(
        ...
        with open(filename) as f:
            read_xml(
                f,
                parser=parser,
                iterparse={"book": ["category", "title", "year", "author",
    "price"]},
            )


@pytest.mark.network
@tm.network(url="https://www.w3schools.com/xml/books.xml",
    check_before_test=True)
...
def test_wrong_dict_type(datapath, parser):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(TypeError, match="list is not a valid type for
    iterparse"):
        read_xml(
            filename,
            parser=parser,
...
def test_wrong_dict_value(datapath, parser):
    filename = datapath("io", "data", "xml", "books.xml")
    with pytest.raises(
        TypeError, match="<class 'str'> is not a valid type for value in
    iterparse"
    ):
...
@td.skip_if_no("lxml")
@tm.network(
    url="https://www.w3schools.com/xml/cdcatalog_with_xsl.xml",
    check_before_test=True
)
