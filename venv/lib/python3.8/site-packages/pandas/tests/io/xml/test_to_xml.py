...
<books>
  ...
  <book>
    ...
    with tm.ensure_clean("test.xml") as path:
        df_file.to_xml(
            path, index=False, root_name="books", row_name="book",
    parser=parser
        )
        with open(path, "rb") as f:
            output = f.read().decode("utf-8").strip()
...
def test_index_false_with_offset_input_index(parser, offset_index):
    """
    Tests that the output does not contain the `<index>` field when the index
    of the
...
<data>
  ...
  <row index="2" shape="triangle" degrees="180" sides="3.0"/>
    ...
    output = geom_df.to_xml(attr_cols=["shape", "degrees", "sides"],
    parser=parser)
    output = equalize_decl(output)
...
sum_mass="2667.54" mean_mass="333.44"/>
    ...
    output = pvt.to_xml(attr_cols=list(pvt.reset_index().columns.values),
    parser=parser)
    output = equalize_decl(output)
...
sum="189.23" mean="94.61"/>
    ...
    output = agg.to_xml(attr_cols=list(agg.reset_index().columns.values),
    parser=parser)
    output = equalize_decl(output)
...
<data xmlns="http://example.com">
  ...
  <row>
    ...
    output = geom_df.to_xml(namespaces={"": "http://example.com"},
    parser=parser)
    output = equalize_decl(output)
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    with pytest.raises(XMLSyntaxError, match=("Opening and ending tag
    mismatch")):
        geom_df.to_xml(stylesheet=xsl)
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    string-length('triangle')
                                     ...
                                     shape,
                                     substring($delim, 1,
    string-length(name(degrees))
                                               - string-length(degrees) + 2),
                                     degrees,
                                     substring($delim, 1,
    string-length(name(sides))
