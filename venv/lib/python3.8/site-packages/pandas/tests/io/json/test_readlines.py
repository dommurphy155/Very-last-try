...
def test_read_datetime():
    ...
    expected = DataFrame(
        [[1, "2020-03-05", "hector"], [2, "2020-04-08T09:58:49+00:00",
    "hector"]],
        columns=["accounts", "date", "name"],
    )
    tm.assert_frame_equal(result, expected)
...
def test_read_jsonl_unicode_chars():
    ...
    expected = DataFrame([["foo\u201d", "bar"], ["foo", "bar"]], columns=["a",
    "b"])
    tm.assert_frame_equal(result, expected)
...
def test_to_jsonl_count_new_lines():
    # GH36888
    df = DataFrame([[1, 2], [1, 2]], columns=["a", "b"])
    actual_new_lines_count = df.to_json(orient="records",
    lines=True).count("\n")
...
def test_readjson_chunks(lines_json_df, chunksize):
    ...
    with read_json(StringIO(lines_json_df), lines=True, chunksize=chunksize)
    as reader:
...
def test_readjson_invalid_chunksize(lines_json_df, chunksize):
    ...
    with pytest.raises(ValueError, match=msg):
        with read_json(StringIO(lines_json_df), lines=True,
    chunksize=chunksize) as _:
            pass
...
def test_readjson_nrows_chunks(nrows, chunksize):
    ...
    jsonl = """{"a": 1, "b": 2}
        ...
        {"a": 5, "b": 6}
        {"a": 7, "b": 8}"""
    with read_json(jsonl, lines=True, nrows=nrows, chunksize=chunksize) as
    reader:
        chunked = pd.concat(reader)
    expected = DataFrame({"a": [1, 3, 5, 7], "b": [2, 4, 6, 8]}).iloc[:nrows]
