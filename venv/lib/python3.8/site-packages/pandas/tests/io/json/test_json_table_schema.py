...
class TestTableSchemaType:
    ...
    def test_as_json_table_type_int_data(self, int_type):
        int_data = [1, 2, 3]
        assert as_json_table_type(np.array(int_data, dtype=int_type).dtype) ==
    "integer"

    @pytest.mark.parametrize("float_type", [float, np.float16, np.float32,
    np.float64])
    ...
    @pytest.mark.parametrize("str_data", [pd.Series(["a", "b"]),
    pd.Index(["a", "b"])])
    ...
    @pytest.mark.parametrize("float_dtype", [float, np.float16, np.float32,
    np.float64])
    ...
    @pytest.mark.parametrize("str_dtype", [object])  # TODO(GH#14904) flesh
    out dtypes?
...
class TestTableOrient:
    def test_build_series(self):
        ...
        fields = [{"name": "id", "type": "integer"}, {"name": "a", "type":
    "integer"}]
    ...
    def test_date_format_raises(self, df_table):
        msg = (
            "Trying to write with `orient='table'` and `date_format='epoch'`.
    Table "
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"type": "datetime", "tz": "US/Hawaii"}, "datetime64[ns,
    US/Hawaii]"),
            ({"type": "any"}, "object"),
            (
                {
    ...
    @pytest.mark.parametrize(
        ...
        [
            pd.Index([], name="index"),
            pd.MultiIndex.from_arrays([["foo"], ["bar"]], names=("level_0",
    "level_1")),
            pd.MultiIndex.from_arrays([["foo"], ["bar"]], names=("foo",
    "level_1")),
        ],
    )
    ...
    def test_timestamp_in_columns(self):
        df = DataFrame(
            [[1, 2]], columns=[pd.Timestamp("2016"), pd.Timedelta(10,
    unit="s")]
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame(
                {"A": [1]},
                index=pd.MultiIndex.from_arrays([["a"], [1]], names=["A",
    "a"]),
            ),
        ],
    )
...
class TestTableOrientReader:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pd.MultiIndex.from_product(
                [
                    pd.date_range("2020-08-30", freq="d", periods=2,
    tz="US/Central"),
                    ["x", "y"],
                ],
            ),
    ...
    @pytest.mark.parametrize(
        "index_names",
        [[None, None], ["foo", "bar"], ["foo", None], [None, "foo"], ["index",
    "foo"]],
    )
    def test_multiindex(self, index_names):
        # GH 18912
        df = DataFrame(
            [["Arr", "alpha", [1, 2, 3, 4]], ["Bee", "Beta", [10, 20, 30,
    40]]],
            index=[["A", "B"], ["Null", "Eins"]],
