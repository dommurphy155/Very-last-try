...
@pytest.mark.filterwarnings("ignore:the 'numpy' keyword is
...
class TestPandasContainer:
    ...
    def test_frame_non_unique_index(self, orient):
        df = DataFrame([["a", "b"], ["c", "d"]], index=[1, 1], columns=["x",
    "y"])
        result = read_json(df.to_json(orient=orient), orient=orient)
        expected = df.copy()
    ...
    def test_frame_non_unique_columns(self, orient, data):
        ...
        if orient == "values":
            ...
            if expected.iloc[:, 0].dtype == "datetime64[ns]":
                # orient == "values" by default will write Timestamp objects
    out
                # in milliseconds; these are internally stored in nanosecond,
                # so divide to get where we need
                # TODO: a to_epoch method would also solve; see GH 14772
                expected.iloc[:, 0] = expected.iloc[:, 0].view(np.int64) //
    1000000
        elif orient == "split":
            expected = df
    ...
    def test_frame_non_unique_columns_raises(self, orient):
        df = DataFrame([["a", "b"], ["c", "d"]], index=[1, 2], columns=["x",
    "x"])
    ...
    def test_roundtrip_categorical(
        ...
        if orient in ("index", "columns"):
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=f"Can't have duplicate index values for orient
    '{orient}')"
                )
            )
        ...
        if numpy and orient in ("records", "values"):
            request.node.add_marker(
                pytest.mark.xfail(reason=f"Orient {orient} is broken with
    numpy=True")
            )
    ...
    numpy=numpy)
        ...
        expected.index = expected.index.astype(str)  # Categorical not
    preserved
    ...
    numpy=numpy)
        ...
        expected = df.copy()
        expected =
    expected.assign(**expected.select_dtypes("number").astype(np.int64))
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                "|".join(
                    [
                        r"Length of values \(3\) does not match length of
    index \(2\)",
                    ]
                ),
                "split",
    ...
    def test_frame_empty(self):
        ...
        tm.assert_frame_equal(
            read_json(df.to_json(), dtype=dict(df.dtypes)), df,
    ...
    def test_frame_mixedtype_orient(self):  # GH10289
        ...
        df = DataFrame(
            vals, index=list("abcd"), columns=["1st", "2nd", "3rd", "4th",
    "5th"]
        )
    ...
    def test_series_non_unique_index(self):
        ...
        tm.assert_series_equal(
            s, read_json(s.to_json(orient="split"), orient="split",
    typ="series")
        )
    ...
    @pytest.mark.parametrize("date_typ", [datetime.date, datetime.datetime,
    Timestamp])
    ...
    def test_convert_dates_infer(self, infer_word):
        ...
        expected = DataFrame(
            [[1, Timestamp("2002-11-08")], [2, pd.NaT]], columns=["id",
    infer_word]
        )
...
\\[right\\]: RangeIndex\\(start=0, stop=2, step=1\\)"""
    ...
    def test_round_trip_exception_(self, datapath):
        ...
        s = df.to_json()
        result = read_json(s)
        tm.assert_frame_equal(result.reindex(index=df.index,
    ...
    def test_timedelta(self):
        ...
        s = Series([timedelta(23), timedelta(seconds=5)], index=pd.Index([0,
    1]))
        assert s.dtype == "timedelta64[ns]"
        result = read_json(s.to_json(), typ="series").apply(converter)
        tm.assert_series_equal(result, s)
    ...
    def test_mixed_timedelta_datetime(self):
        frame = DataFrame({"a": [timedelta(23), Timestamp("20130101")]},
    dtype=object)
    ...
    def test_default_handler_indirect(self):
        ...
        def default(obj):
            if isinstance(obj, complex):
                return [("mathjs", "Complex"), ("re", obj.real), ("im",
    obj.imag)]
            return str(obj)
        ...
        df_list = [
            9,
            DataFrame(
                {"a": [1, "STR", complex(4, -5)], "b": [float("nan"), None,
    "N/A"]},
                columns=["a", "b"],
            ),
        ]
        ...
        assert dumps(df_list, default_handler=default, orient="values") ==
    expected

    def test_default_handler_numpy_unsupported_dtype(self):
        # GH12554 to_json raises 'Unhandled numpy dtype 15'
        df = DataFrame(
            {"a": [1, 2.3, complex(4, -5)], "b": [float("nan"), None,
    complex(1.2, 0)]},
            columns=["a", "b"],
        )
        expected = (
    ...
    def test_datetime_tz(self):
        ...
        df = DataFrame({"A": tz_range, "B": pd.date_range("20130101",
    periods=3)})
    ...
    def test_tz_range_is_naive(self):
        ...
        dfexp =
    '{"DT":{"0":"2013-01-01T05:00:00.000","1":"2013-01-02T05:00:00.000"}}'
    ...
    def test_to_jsonl(self):
        ...
        df = DataFrame([["foo\\", "bar"], ['foo"', "bar"]], columns=["a\\",
    "b"])
        result = df.to_json(orient="records", lines=True)
        expected =
    '{"a\\\\":"foo\\\\","b":"bar"}\n{"a\\\\":"foo\\"","b":"bar"}\n'
    ...
    @pytest.mark.parametrize(
        "index", [None, [1, 2], [1.0, 2.0], ["a", "b"], ["1", "2"], ["1.",
    "2."]]
    )
    ...
    def test_read_json_table_convert_axes_raises(self):
        # GH25433 GH25435
        df = DataFrame([[1, 2], [3, 4]], index=[1.0, 2.0], columns=["1.",
    "2."])
        dfjson = df.to_json(orient="table")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                DataFrame([[1, 2], [4, 5]], columns=["a",
    "b"]).rename_axis("foo"),
                {"columns": ["a", "b"], "data": [[1, 2], [4, 5]]},
            ),
            (
                DataFrame(
                    [[1, 2], [4, 5]], columns=["a", "b"], index=[["a", "b"],
    ["c", "d"]]
                ),
                {"columns": ["a", "b"], "data": [[1, 2], [4, 5]]},
            ),
    ...
    def test_read_timezone_information(self):
        ...
        expected = Series([88], index=DatetimeIndex(["2019-01-01 11:00:00"],
    tz="UTC"))
        tm.assert_series_equal(result, expected)
    ...
    def test_read_json_with_very_long_file_path(self, compression):
        ...
        with pytest.raises(
            ...
            # path too long for Windows is handled in file_exists() but raises
    in
...
{spaces}}}
    ...
    def test_emca_262_nan_inf_support(self):
        # GH 12213
        data = '["a", NaN, "NaN", Infinity, "Infinity", -Infinity,
    "-Infinity"]'
    ...
    def test_frame_int_overflow(self):
        # GH 30320
        encoded_json = json.dumps([{"col": "31900441201190696999"}, {"col":
    "Text"}])
    ...
    storage_options=s3so)
        ...
        while True:
            if target_file in (
                obj.key for obj in
    s3_resource.Bucket("pandas-test").objects.all()
            ):
                break
            time.sleep(0.1)
            timeout -= 0.1
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                DataFrame(
                    [[-0.28 + 0.34j, -1.08 - 0.39j], [0.41 - 0.34j, -0.78 -
    1.35j]]
                ),
    ...
    def test_json_uint64(self):
        ...
        df = DataFrame(data={"col1": [13342205958987758245,
