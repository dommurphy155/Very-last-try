...
@pytest.fixture(
    params=[None, "split", "records", "values", "index"]  # Column indexed by
    default.
)
def orient(request):
...
def get_int32_compat_dtype(numpy, orient):
    # See GH#32527
    dtype = np.int64
    if not ((numpy is None or orient == "index") or (numpy is True and orient
    is None)):
...
class TestUltraJSONTests:
    ...
    @pytest.mark.parametrize(
        "long_number", [-4342969734183514, -12345678901234.56789012,
    -528656961.4399388]
    )
    ...
    def test_encode_array_of_nested_arrays(self):
        ...
        tm.assert_numpy_array_equal(
            nested_input, ujson.decode(output, numpy=True,
    dtype=nested_input.dtype)
        )
    ...
    def test_invalid_double_precision(self, invalid_val):
        ...
        expected_exception = ValueError if isinstance(invalid_val, int) else
    TypeError
        msg = (
    ...
    @pytest.mark.parametrize(
        "test",
        [datetime.time(), datetime.time(1, 2, 3), datetime.time(10, 12, 15,
    343243)],
    )
    ...
    def test_decode_depth_too_big(self, too_big_char):
        with pytest.raises(ValueError, match="Reached object decoding depth
    limit"):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("[[[true", "Unexpected character found when decoding array value
    (2)"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "numeric_int_as_str", ["31337", "-31337"]  # Should work with
    negatives.
    )
    ...
    def test_encode_list_long_conversion(self):
        ...
        tm.assert_numpy_array_equal(
            np.array(long_input), ujson.decode(output, numpy=True,
    dtype=np.int64)
        )

    @pytest.mark.parametrize("long_input", [9223372036854775807,
    ...
    GH-42768
        ...
        assert ujson.decode(ujson.encode(test_object)) == {"a": 1, "b": 2,
    "d": 4}
...
class TestNumpyJSONTests:
    ...
    def test_array_basic(self):
        ...
        tm.assert_numpy_array_equal(np.array(ujson.decode(ujson.encode(arr))),
    arr)
        tm.assert_numpy_array_equal(ujson.decode(ujson.encode(arr),
    numpy=True), arr)
    ...
    def test_0d_array(self):
        # gh-18878
        msg = re.escape("array(1) (0d array) is not JSON serializable at the
    moment")
        with pytest.raises(TypeError, match=msg):
    ...
    real)? "
            ...
            (
                [["a"], 42],
                ValueError,
                r"Cannot decode multidimensional arrays with variable length
    elements "
                r"to numpy",
                {},
            ),
    ...
    def test_array_numpy_labelled(self):
        labelled_input = {"a": []}
        output = ujson.loads(ujson.dumps(labelled_input), numpy=True,
    labelled=True)
    ...
    labelled=True)
        ...
        input_dumps = '[{"a": 42, "b":31}, {"a": 24, "c": 99}, {"a": 2.4, "b":
    78}]'
        output = ujson.loads(input_dumps, numpy=True, labelled=True)
        expected_vals = np.array([42, 31, 24, 99, 2.4, 78],
    dtype=int).reshape((3, 2))
...
class TestPandasJSONTests:
    def test_dataframe(self, request, orient, numpy):
        if orient == "records" and numpy:
            request.node.add_marker(
                pytest.mark.xfail(reason=f"Not idiomatic pandas if
    orient={orient}")
            )
    ...
    **decode_kwargs)
    ...
    def test_dataframe_numpy_labelled(self, orient, request):
        if orient in ("split", "values"):
            request.node.add_marker(
                pytest.mark.xfail(reason=f"{orient} incompatible for
    labelled=True")
            )
        ...
        output = DataFrame(
            *ujson.decode(ujson.encode(df, **kwargs), numpy=True,
    labelled=True)
        )
    ...
    **decode_kwargs)
    ...
    def test_datetime_index(self):
        ...
        rng = DatetimeIndex(list(date_range("1/1/2000", periods=20)),
    freq=None)
    ...
    @pytest.mark.parametrize("extreme_num", [9223372036854775807,
    -9223372036854775808])
    ...
    def test_decode_floating_point(self, sign, float_number):
        float_number *= sign
        tm.assert_almost_equal(float_number, ujson.loads(str(float_number)),
    rtol=1e-15)
