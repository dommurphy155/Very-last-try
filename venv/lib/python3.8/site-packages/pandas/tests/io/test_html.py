...
class TestReadHtml:
    ...
    def test_banklist_url_positional_match(self):
        url =
    "https://www.fdic.gov/resources/resolutions/bank-failures/failed-bank-list/index.html"  # noqa E501
        # Passing match argument as positional should cause a FutureWarning.
        with tm.assert_produces_warning(FutureWarning):
            df1 = self.read_html(
                # lxml cannot find attrs leave out for now
                url,
                "First Federal Bank of Florida",  # attrs={"class":
    "dataTable"}
            )
        with tm.assert_produces_warning(FutureWarning):
            # lxml cannot find attrs leave out for now
    ...
    "https://www.fdic.gov/resources/resolutions/bank-failures/failed-bank-list/index.html"  # noqa E501
        df1 = self.read_html(
            # lxml cannot find attrs leave out for now
            url,
            match="First Federal Bank of Florida",  # attrs={"class":
    "dataTable"}
        )
        # lxml cannot find attrs leave out for now
        df2 = self.read_html(
    ...
    def test_banklist(self, banklist_data):
        df1 = self.read_html(banklist_data, match=".*Florida.*", attrs={"id":
    "table"})
        df2 = self.read_html(banklist_data, match="Metcalf Bank", attrs={"id":
    "table"})
    ...
    def test_skiprows_invalid(self, spam_data):
        with pytest.raises(TypeError, match=("is not a valid type for skipping
    rows")):
    ...
    def test_header_and_index_no_types(self, spam_data):
        df1 = self.read_html(spam_data, match=".*Water.*", header=1,
    index_col=0)
    ...
    def test_bad_url_protocol(self):
        with pytest.raises(URLError, match="urlopen error unknown url type:
    git"):
    ...
    def test_file_url(self, banklist_data):
        url = banklist_data
        dfs = self.read_html(
            file_path_to_url(os.path.abspath(url)), match="First",
    attrs={"id": "table"}
        )
    ...
    def test_invalid_table_attrs(self, banklist_data):
        ...
        with pytest.raises(ValueError, match="No tables found"):
            self.read_html(
                url, match="First Federal Bank of Florida", attrs={"id":
    "tasdfable"}
            )
    ...
    def test_tfoot_read(self):
        ...
        data2 =
    data_template.format(footer="<tr><td>footA</td><th>footB</th></tr>")
    ...
    def test_banklist_header(self, banklist_data, datapath):
        ...
        df = self.read_html(banklist_data, match="Metcalf", attrs={"id":
    "table"})[0]
    ...
    def test_gold_canyon(self, banklist_data):
        ...
        df = self.read_html(banklist_data, match="Gold Canyon", attrs={"id":
    "table"})[
            0
        ]
    ...
    def test_colspan_rowspan_copy_values(self):
        ...
        expected = DataFrame(
            data=[["A", "B", "B", "Z", "C"]], columns=["X", "X.1", "Y", "Z",
    "W"]
        )
    ...
    def test_colspan_rowspan_both_not_1(self):
        ...
        expected = DataFrame(
            data=[["A", "B", "B", "B", "D"]], columns=["A", "B", "B.1", "B.2",
    "C"]
        )
    ...
    def test_header_inferred_from_rows_with_only_th(self):
        ...
        columns = MultiIndex(levels=[["A", "B"], ["a", "b"]], codes=[[0, 1],
    [0, 1]])
    ...
    def test_wikipedia_states_multiindex(self, datapath):
        ...
        assert np.allclose(result.loc["Alaska", ("Total area[2]", "sq mi")],
    665384.04)
    ...
    header or "
            "header=int or list-like of ints to specify the row(s) making up
    the "
            "column names"
        )
    ...
    def test_multiple_header_rows(self):
        ...
        expected_df = DataFrame(
            data=[("Hillary", 68, "D"), ("Bernie", 74, "D"), ("Donald", 69,
    "R")]
        )
        expected_df.columns = [
    ...
    def test_fallback_success(self, datapath):
        ...
        self.read_html(banklist_data, match=".*Water.*", flavor=["lxml",
    "html5lib"])
    ...
    def test_parse_failure_unseekable(self):
        ...
        with pytest.raises(ValueError, match="passed a non-rewindable file
    object"):
            self.read_html(bad)
        ...
        gh_13141_data = """
          <table>
            <tr>
              ...
              <th>FTP</th>
              <th><a
    href="https://en.wiktionary.org/wiki/linkless">Linkless</a></th>
            </tr>
            <tr>
              <td><a href="https://en.wikipedia.org/">Wikipedia</a></td>
              <td>SURROUNDING <a href="ftp://ftp.us.debian.org/">Debian</a>
    TEXT</td>
              <td>Linkless</td>
            </tr>
            <tfoot>
              <tr>
                <td><a
    href="https://en.wikipedia.org/wiki/Page_footer">Footer</a></td>
                <td>
                  Multiple <a href="1">links:</a> <a href="2">Only first
    captured.</a>
                </td>
              </tr>
            </tfoot>
        ...
        gh_13141_expected = {
            ...
            "body_ignore": ["Wikipedia", "SURROUNDING Debian TEXT",
    "Linkless"],
            "body_extract": [
