...
SQL_STRINGS = {
    "read_parameters": {
        ...
        "postgresql": 'SELECT * FROM iris WHERE "Name"=%s AND
    "SepalLength"=%s',
    },
...
def create_and_load_iris(conn, iris_file: Path, dialect: str):
    ...
    with iris_file.open(newline=None) as csvfile:
        reader = csv.reader(csvfile)
        header = next(reader)
        params = [{key: value for key, value in zip(header, row)} for row in
    reader]
...
def create_and_load_types_sqlite3(conn: sqlite3.Connection, types_data:
    list[dict]):
    cur = conn.cursor()
...
class SQLiteMixIn(MixInBase):
    ...
    def _get_all_tables(self):
        c = self.conn.execute("SELECT name FROM sqlite_master WHERE
    type='table'")
...
class PandasSQLTest:
    ...
    def _to_sql_with_sql_engine(self, test_frame1, engine="auto",
    **engine_kwargs):
    ...
    def _roundtrip(self, test_frame1):
        ...
        result = self.pandasSQL.read_query("SELECT * FROM
    ...
    def _to_sql_save_index(self):
        df = DataFrame.from_records(
            [(1, 2.1, "line1"), (2, 1.5, "line2")], columns=["A", "B", "C"],
    index=["A"]
        )
...
class _TestSQLApi(PandasSQLTest):
    ...
    def test_to_sql_append(self, test_frame1):
        assert sql.to_sql(test_frame1, "test_frame4", self.conn,
    if_exists="fail") == 4

        # Add to table again
        assert (
            sql.to_sql(test_frame1, "test_frame4", self.conn,
    if_exists="append") == 4
        )
    ...
    def test_roundtrip(self, test_frame1):
        ...
        result = sql.read_sql_query("SELECT * FROM test_frame_roundtrip",
    con=self.conn)
    ...
    def test_timedelta(self):
        ...
        # see #6921
        df = to_timedelta(Series(["00:00:01", "00:00:03"],
    name="foo")).to_frame()
    ...
    def test_to_sql_index_label_multiindex(self):
        ...
        msg = "Length of 'index_label' should match number of levels, which is
    2"
        with pytest.raises(ValueError, match=msg):
            sql.to_sql(
                temp_frame,
    ...
    def test_multiindex_roundtrip(self):
        ...
        result = sql.read_sql_query(
            "SELECT * FROM test_multiindex_roundtrip", self.conn,
    index_col=["A", "B"]
        )
...
@pytest.mark.skipif(not SQLALCHEMY_INSTALLED, reason="SQLAlchemy not
    installed")
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_read_table_index_col(self, test_frame1):
        ...
        result = sql.read_sql_table("test_frame", self.conn, index_col=["A",
    "B"])
    ...
    def test_warning_case_insensitive_table_name(self, test_frame1):
        ...
        with tm.assert_produces_warning(
            UserWarning,
            match=(
                r"The provided table name 'TABLE1' is not found exactly as
    such in "
    ...
    def test_sqlalchemy_integer_overload_mapping(self, integer):
        ...
        db = sql.SQLDatabase(self.conn)
        with pytest.raises(
            ValueError, match="Unsigned 64 bit integer datatype is not
    supported"
        ):
    ...
    def test_query_by_text_obj(self):
        ...
        iris_df = sql.read_sql(name_text, self.conn, params={"name":
    "Iris-versicolor"})
    ...
    def test_query_by_select_obj(self):
        ...
        iris_df = sql.read_sql(name_select, self.conn, params={"name":
    "Iris-setosa"})
        all_names = set(iris_df["Name"])
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_read_table_columns(self):
        ...
        tm.equalContents(iris_frame.columns.values, ["SepalLength",
    "SepalLength"])
    ...
    def test_datetime_with_timezone(self, request):
        ...
        if not hasattr(df, "DateColWithTz"):
            request.node.add_marker(
                pytest.mark.xfail(reason="no column with datetime with time
    zone")
            )
    ...
    zone")
        ...
        df = concat(
            list(read_sql_query("select * from types", self.conn,
    chunksize=1)),
            ignore_index=True,
        )
        col = df.DateColWithTz
    ...
    tz="US/Pacific")}
        ...
        result = sql.read_sql_query("SELECT * FROM test_datetime_tz",
    self.conn)
    ...
    def test_datetime(self):
        df = DataFrame(
            {"A": date_range("2013-01-01 09:00:00", periods=3), "B":
    np.arange(3.0)}
        )
    ...
    def test_dtype(self):
        ...
        # GH9083
        assert df.to_sql("dtype_test3", self.conn, dtype={"B": String(10)}) ==
    2
        meta.reflect(bind=self.conn)
    ...
    def test_double_precision(self):
        ...
        assert np.round(df["f64"].iloc[0], 14) == np.round(res["f64"].iloc[0],
    14)
    ...
    def test_connectable_issue_example(self):
        ...
        def bar(connection, data):
            data.to_sql(name="test_foo_data", con=connection,
    if_exists="append")
        ...
        assert (
            DataFrame({"test_foo_data": [0, 1, 2]}).to_sql("test_foo_data",
    self.conn)
            == 3
        )
        main(self.conn)
    ...
    @pytest.mark.parametrize(
        "input",
        [{"foo": [np.inf]}, {"foo": [-np.inf]}, {"foo": [-np.inf], "infe0":
    ["bar"]}],
    )
    def test_to_sql_with_negative_npinf(self, input, request):
        ...
        if self.flavor == "mysql":
            # GH 36465
            # The input {"foo": [-np.inf], "infe0": ["bar"]} does not raise
    any error
...
class _TestSQLAlchemyConn(_EngineToConnMixin, _TestSQLAlchemy):
    @pytest.mark.xfail(reason="Nested transactions rollbacks don't work with
    Pandas")
    def test_transactions(self):
...
class _TestSQLiteAlchemy:
    ...
    def test_row_object_is_named_tuple(self):
        ...
        assert (
            df.to_sql("test_frame", con=self.conn, index=False,
    if_exists="replace")
            == 2
        )
...
class _TestPostgreSQLAlchemy:
    ...
    def test_schema_support(self):
        ...
        # mysql/sqlite)
        df = DataFrame({"col1": [1, 2], "col2": [0.1, 0.2], "col3": ["a",
    "n"]})
        ...
        assert (
            df.to_sql(
                "test_schema_public_explicit", self.conn, index=False,
    schema="public"
            )
            == 2
        )
        assert (
            df.to_sql("test_schema_other", self.conn, index=False,
    schema="other") == 2
        )
    ...
    schema="other")
        ...
        assert (
            df.to_sql("test_schema_other", self.conn, schema="other",
    index=False) == 2
        )
        df.to_sql(
            "test_schema_other",
        ...
        res = sql.read_sql_table("test_schema_other", self.conn,
    schema="other")
    ...
    if_exists="append")
            ...
            res1 = sql.read_sql_table("test_schema_other2", self.conn,
    schema="other")
...
class TestSQLiteFallback(SQLiteMixIn, PandasSQLTest):
    ...
    def test_datetime_time(self, tz_aware):
        ...
        if not tz_aware:
            ...
            tz_dt = date_range("2013-01-01 09:00:00", periods=2,
    tz="US/Pacific")
            tz_times = Series(tz_dt.to_pydatetime()).map(lambda dt:
    dt.timetz())
    ...
    def _get_index_columns(self, tbl_name):
        ...
        for ix_name in ixs.name:
            ix_info = sql.read_sql_query(f"PRAGMA index_info({ix_name})",
    self.conn)
    ...
    def test_dtype(self):
        ...
        assert self._get_sqlite_column_type("single_dtype_test", "A") ==
    "STRING"
        assert self._get_sqlite_column_type("single_dtype_test", "B") ==
    "STRING"
...
class TestXSQLite:
    ...
    def test_basic(self):
        ...
        assert sql.to_sql(frame, name="test_table", con=self.conn,
    index=False) == 30
        ...
        assert sql.to_sql(frame2, name="test_table2", con=self.conn,
    index=False) == 30
    ...
    def test_keyword_as_column_names(self):
        ...
        assert sql.to_sql(df, con=self.conn, name="testkeywords", index=False)
    == 5
    ...
    def test_onecolumn_of_integer(self):
        ...
        assert sql.to_sql(mono_df, con=self.conn, name="mono_df", index=False)
    == 2
        # computing the sum via sql
        con_x = self.conn
        the_sum = sum(my_c0[0] for my_c0 in con_x.execute("select * from
    mono_df"))
    ...
    def test_if_exists(self):
        ...
        df_if_exists_2 = DataFrame({"col1": [3, 4, 5], "col2": ["C", "D",
    "E"]})
        ...
        # test if_exists='fail'
        sql.to_sql(
            frame=df_if_exists_1, con=self.conn, name=table_name,
    if_exists="fail"
        )
    ...
    if_exists="fail"
        ...
        assert tquery(sql_select, con=self.conn) == [(3, "C"), (4, "D"), (5,
    "E")]
        self.drop_table(table_name)
