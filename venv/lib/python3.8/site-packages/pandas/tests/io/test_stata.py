...
class TestStata:
    ...
    def test_read_dta12(self, datapath):
        parsed_117 = self.read_dta(datapath("io", "data", "stata",
    "stata12_117.dta"))
        expected = DataFrame.from_records(
            [
                [1, "abc", "abcdefghi"],
    ...
    "stata14_118.dta"))
        ...
        with StataReader(datapath("io", "data", "stata", "stata14_118.dta"))
    as rdr:
            vl = rdr.variable_labels()
            vl_expected = {
                "Unicode_Cities_Strl": "Here are some strls with Ünicode
    chars",
                "Longs": "long data",
    ...
    def test_read_write_dta10(self, version):
        ...
        with tm.ensure_clean() as path:
            original.to_stata(path, convert_dates={"datetime": "tc"},
    version=version)
            written_and_read_again = self.read_dta(path)
    ...
    def test_encoding(self, version, datapath):
        ...
        raw = read_stata(datapath("io", "data", "stata",
    "stata1_encoding.dta"))
    ...
    datapath):
        ...
        expected["date_td"] = pd.to_datetime(expected["date_td"],
    errors="coerce")

        tm.assert_frame_equal(parsed_114, parsed)

        with tm.ensure_clean() as path:
            parsed_114.to_stata(path, convert_dates={"date_td": "td"},
    version=version)
            written_and_read_again = self.read_dta(path)
            tm.assert_frame_equal(written_and_read_again.set_index("index"),
    parsed_114)
    ...
    def test_timestamp_and_label(self, version):
        ...
        with tm.ensure_clean() as path:
            original.to_stata(
                path, time_stamp=time_stamp, data_label=data_label,
    version=version
            )
    ...
    def test_variable_labels(self, datapath):
        with StataReader(datapath("io", "data", "stata", "stata7_115.dta")) as
    rdr:
            sr_115 = rdr.variable_labels()
        with StataReader(datapath("io", "data", "stata", "stata7_117.dta")) as
    rdr:
    ...
    @pytest.mark.parametrize("file", ["stata8_113", "stata8_115",
    "stata8_117"])
    ...
    def test_big_dates(self, datapath):
        ...
        for i in range(len(yr)):
            ...
            for j in range(7):
                if j == 0:
                    row.append(datetime(yr[i], mo[i], dd[i], hr[i], mm[i],
    ss[i]))
                elif j == 6:
        ...
        expected[5][2] = expected[5][3] = expected[5][4] = datetime(1677, 10,
    1)
    ...
    def test_categorical_writing(self, version):
        ...
        expected["incompletely_labeled"] =
    expected["incompletely_labeled"].apply(str)
    ...
    def test_categorical_order(self, file, datapath):
        ...
        expected = [
            ...
            (True, "noorder", ["a", "b", "c", "d", "e"], np.array([2, 1, 4, 0,
    3])),
            (True, "floating", ["a", "b", "c", "d", "e"], np.arange(0, 5)),
            (True, "float_missing", ["a", "d", "e"], np.array([0, 1, 2, -1,
    -1])),
        ...
        for is_cat, col, labels, codes in expected:
            if is_cat:
                cols.append(
                    (col, pd.Categorical.from_codes(codes, labels,
    ordered=True))
                )
            else:
    ...
    def test_write_variable_labels(self, version, mixed_frame):
        ...
        with tm.ensure_clean() as path:
            mixed_frame.to_stata(path, variable_labels=variable_labels,
    version=version)
            with StataReader(path) as sr:
    ...
    "City"}
        ...
        with tm.ensure_clean() as path:
            with pytest.raises(
                ValueError, match="Variable labels must contain only
    characters"
            ):
                mixed_frame.to_stata(
                    path, variable_labels=variable_labels, version=version
    ...
    def test_write_variable_label_errors(self, mixed_frame):
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                mixed_frame.to_stata(path,
    variable_labels=variable_labels_utf8)
        ...
        with pytest.raises(ValueError, match=msg):
            with tm.ensure_clean() as path:
                mixed_frame.to_stata(path,
    variable_labels=variable_labels_long)
...
"""
    ...
    def test_out_of_range_double(self):
        ...
        df = DataFrame(
            {
                "ColumnOk": [0.0, np.finfo(np.double).eps,
    4.49423283715579e307],
                "ColumnTooBig": [0.0, np.finfo(np.double).eps,
    ...
    def test_out_of_range_float(self):
        ...
        with tm.ensure_clean() as path:
            ...
            original["ColumnTooBig"] =
    original["ColumnTooBig"].astype(np.float64)
    ...
    def test_all_none_exception(self, version):
        ...
        output["none"] = None
        with tm.ensure_clean() as path:
            with pytest.raises(ValueError, match="Column `none` cannot be
    exported"):
...
the string values returned are correct."""
    ...
    def test_stata_119(self, datapath):
        ...
        assert df.iloc[0, 0] == pd.Timestamp(datetime(2012, 12, 21, 21, 12,
    21))
    ...
    def test_writer_118_exceptions(self):
        ...
        with tm.ensure_clean() as path:
            with pytest.raises(ValueError, match="version must be either 118
    or 119."):
...
def test_chunked_categorical(version):
    df = DataFrame({"cats": Series(["a", "b", "a", "b", "c"],
    dtype="category")})
    df.index.name = "index"
    with tm.ensure_clean() as path:
        ...
        for i, block in enumerate(reader):
            ...
            tm.assert_series_equal(block.cats, df.cats.iloc[2 * i: 2 * (i +
    1)])
...
def test_precision_loss():
    ...
    with tm.ensure_clean() as path:
        with tm.assert_produces_warning(
            PossiblePrecisionLoss, match="Column converted from int64 to
    float64"
        ):
            df.to_stata(path, write_index=False)
        reread = read_stata(path)
...
def test_stata_compression(compression_only, read_infer, to_infer):
    ...
    with tm.ensure_clean(filename) as path:
        ...
        result = read_stata(path, compression=read_compression,
    index_col="index")
...
def test_non_categorical_value_labels():
    ...
    with tm.ensure_clean() as path:
        ...
        msg = "Can't create value labels for notY, it wasn't found in the
    dataset."
...
def test_non_categorical_value_label_convert_categoricals_error():
    ...
    value_labels = {
        "repeated_labels": {10: "Ten", 20: "More than ten", 40: "More than
    ten"}
    }
