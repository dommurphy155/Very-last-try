...
Look,a snake,🐍"""
    ...
    def test_read_non_existent(self, reader, module, error_class, fn_ext):
        ...
        msg2 = rf"\[Errno 2\] No such file or directory:
    '.+does_not_exist\.{fn_ext}'"
        ...
        msg7 = (
            rf"\[Errno 2\] File o directory non esistente:
    '.+does_not_exist\.{fn_ext}'"
        )
    ...
    PyTables
    def test_write_missing_parent_directory(self, method, module, error_class,
    fn_ext):
        pytest.importorskip(module)

        dummy_frame = pd.DataFrame({"a": [1, 2, 3], "b": [2, 3, 4], "c": [3,
    4, 5]})

        path = os.path.join(HERE, "data", "missing_folder", "does_not_exist."
    + fn_ext)

        with pytest.raises(
            error_class,
            match=r"Cannot save file into a non-existent directory:
    .*missing_folder",
        ):
    ...
    def test_read_expands_user_home_dir(
        ...
        monkeypatch.setattr(icom, "_expand_user", lambda x:
    os.path.join("foo", x))
    ...
    @pytest.mark.filterwarnings("ignore:In future versions
    `DataFrame.to_latex`")
    @pytest.mark.parametrize(
...
def test_explicit_encoding(io_class, mode, msg):
    ...
    # it is used. In the case of this test it leads to an error as
    intentionally the
