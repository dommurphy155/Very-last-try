...
def test_put_string_index(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        index = Index([f"I am a very long string index: {i}" for i in
    range(20)])
...
def test_column_multiindex(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        tm.assert_frame_equal(
            store["df"], expected, check_index_type=True,
    check_column_type=True
        )

        store.put("df1", df, format="table")
        tm.assert_frame_equal(
            store["df1"], expected, check_index_type=True,
    check_column_type=True
        )

        msg = re.escape("cannot use a multi-index on axis [1] with
    data_columns ['A']")
        with pytest.raises(ValueError, match=msg):
    ...
    # non_index_axes name
    df = DataFrame(np.arange(12).reshape(3, 4), columns=Index(list("ABCD"),
    name="foo"))
    expected = df.copy()
    if isinstance(expected.index, RangeIndex):
        expected.index = Int64Index(expected.index)
...
def test_store_multiindex(setup_path):
    ...
    index=make_index())
        ...
        store.append("s", s)
        xp = Series(np.zeros(12), index=make_index(["date", "level_1",
    "level_2"]))
        ...
        msg = "duplicate names/columns in the multi-index when storing as a
    table"
        with pytest.raises(ValueError, match=msg):
            store.append("df", df)
