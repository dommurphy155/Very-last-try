...
@pytest.mark.filterwarnings("ignore:object
    name:tables.exceptions.NaturalNameWarning")
def test_append(setup_path):
    ...
    dtype=np.uint32
            ...
            store.append("uints", uint_data, data_columns=["u08", "u16",
    "u32"])
            tm.assert_frame_equal(store["uints"], uint_data)
...
def test_append_with_different_block_ordering(setup_path):
    ...
    dtype="float64")
        ...
        msg = re.escape(
            "cannot match existing table structure for [int16] on appending
    data"
        )
...
def test_append_with_strings(setup_path):
    ...
    min_itemsize={"index": 4})
            ...
            store.put("ss4", df["B"], format="table", min_itemsize={"index":
    6})
            store.append("ss4", df2["B"])
            tm.assert_series_equal(store.select("ss4"), concat([df["B"],
    df2["B"]]))
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        def check_col(key, name, size):
            assert getattr(store.get_storer(key).table.description,
    name).itemsize, size
        ...
        # a min_itemsize that creates a data_column2
        _maybe_remove(store, "df")
        store.append("df", df, data_columns=["B"], min_itemsize={"values":
    200})
        check_col("df", "B", 200)
        ...
        df = DataFrame(["foo", "foo", "foo", "barh", "barh", "barh"],
    columns=["A"])
        _maybe_remove(store, "df")
        msg = re.escape(
            "min_itemsize has the key [foo] which is not an axis or
    data_column"
        )
        with pytest.raises(ValueError, match=msg):
...
def test_append_with_data_columns(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        _maybe_remove(store, "df")
        store.append("df", df_new, data_columns=["A", "B", "string",
    "string2"])
        result = store.select("df", "string='foo' and string2='foo' and A>0
    and B<0")
        expected = df_new[
            (df_new.string == "foo")
            & (df_new.string2 == "foo")
        ...
        expected = df_new[(df_new.string == "foo") & (df_new.string2 ==
    "cool")]
        tm.assert_frame_equal(result, expected)
    ...
    "datetime"]
        ...
        expected = df_dc[(df_dc.B > 0) & (df_dc.C > 0) & (df_dc.string ==
    "foo")]
        tm.assert_frame_equal(result, expected, check_freq=False)
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df_dc = DataFrame(np.random.randn(8, 3), index=index, columns=["A",
    "B", "C"])
        df_dc["string"] = "foo"
        df_dc.loc[df_dc.index[4:6], "string"] = np.nan
        df_dc.loc[df_dc.index[7:9], "string"] = "bar"
        ...
        # on-disk operations
        store.append("df_dc", df_dc, data_columns=["B", "C", "string",
    "string2"])
    ...
    df = DataFrame(
        {
            ...
            "B": [
                Timestamp("20130101") + timedelta(days=i, seconds=10) for i in
    range(10)
            ],
        }
    )
...
def test_append_to_multiple(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        with pytest.raises(ValueError, match=msg):
            store.append_to_multiple({"df1": None, "df2": None}, df,
    selector="df3")
    ...
    "
        ...
        # regular operation
        store.append_to_multiple({"df1": ["A", "B"], "df2": None}, df,
    selector="df1")
...
def test_append_to_multiple_dropna_false(setup_path):
    ...
    dropna=False
        ...
        assert not
    store.select("df1a").index.equals(store.select("df2a").index)
