...
def test_select_columns_in_where(setup_path):
    ...
    # With a DataFrame
    df = DataFrame(np.random.randn(10, 3), index=index, columns=["A", "B",
    "C"])
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        tm.assert_frame_equal(store.select("df", where="columns=['A']"),
    expected)
...
def test_select_dtypes(setup_path):
    ...
    noqa:E712
        for v in [True, "true", 1]:
            result = store.select("df", f"boolv == {v}", columns=["A",
    "boolv"])
            tm.assert_frame_equal(expected, result)
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        # floats w/o NaN
        df = DataFrame({"cols": range(11), "values": range(11)},
    dtype="float64")
...
def test_select_with_many_inputs(setup_path):
    ...
    users=['a','b','c']")
        ...
        result = store.select("df", "ts>=Timestamp('2012-02-01') and
    users=selector")
        expected = df[(df.ts >= Timestamp("2012-02-01")) &
...
def test_select_iterator(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        results = list(
            store.select_as_multiple(["df1", "df2"], selector="df1",
    chunksize=150)
        )
        result = concat(results)
        tm.assert_frame_equal(expected, result)
...
def test_select_iterator_non_complete_8014(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        results = list(store.select("df", where=where, chunksize=chunksize))
        result = concat(results)
        rexpected = expected[(expected.index >= beg_dt) & (expected.index <=
    end_dt)]
        tm.assert_frame_equal(rexpected, result)
    ...
    with ensure_clean_path(["params.hdf", "hist.hdf"]) as paths:
        ...
        hist = DataFrame(
            ...
            index=MultiIndex.from_tuples(
                [(i, j) for i in range(5) for j in range(5)], names=["l1",
    "l2"]
            ),
        )
...
def test_select_as_multiple(setup_path):
    ...
    selector="df1")
        ...
        with pytest.raises(KeyError, match="'No object named df4 in the
    file'"):
            store.select_as_multiple(
        ...
        tm.assert_frame_equal(result, expected)
        expected = store.select_as_multiple("df1", where=["A>0", "B>0"],
    selector="df1")
        tm.assert_frame_equal(result, expected)
...
def test_query_compare_column_type(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        for op in ["<", ">", "=="]:
            ...
            for v in [2.1, True, Timestamp("2014-01-01"), pd.Timedelta(1,
    "s")]:
                query = f"date {op} v"
