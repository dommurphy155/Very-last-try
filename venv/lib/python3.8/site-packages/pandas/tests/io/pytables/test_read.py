...
def test_read_missing_key_opened_store(setup_path):
    ...
    with ensure_clean_path(setup_path) as path:
        ...
        with HDFStore(path, "r") as store:
            ...
            with pytest.raises(KeyError, match="'No object named k2 in the
    file'"):
                read_hdf(store, "k2")
...
def test_read_column(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        msg = re.escape("select_column() got an unexpected keyword argument
    'where'")
...
@pytest.mark.skipif(is_platform_windows(), reason="native2 read fails oddly on
    windows")
...
def test_read_nokey_table(setup_path):
    # GH13231
    df = DataFrame({"i": range(5), "c": Series(list("abacd"),
    dtype="category")})
...
def test_read_py2_hdf_file_in_py3(datapath):
    ...
    expected = DataFrame(
        [1.0, 2, 3],
        index=pd.PeriodIndex(["2015-01-01", "2015-01-02", "2015-01-05"],
    freq="B"),
    )
