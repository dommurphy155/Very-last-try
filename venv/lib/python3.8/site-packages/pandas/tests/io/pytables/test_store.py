...
def test_to_hdf_with_min_itemsize(setup_path):
    ...
    with ensure_clean_path(setup_path) as path:
        ...
        tm.assert_series_equal(read_hdf(path, "ss4"), concat([df["B"],
    df2["B"]]))
...
def test_create_table_index(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        with catch_warnings(record=True):
            ...
            store.append("f2", df, index=["string"], data_columns=["string",
    "string2"])
...
def test_create_table_index_data_columns_argument(setup_path):
    ...
    \n"
                "into HDFStore and include string2 with the data_columns
    argument."
            )
            with pytest.raises(AttributeError, match=msg):
...
def test_calendar_roundtrip_issue(setup_path):
    ...
    s = Series(dts.weekday, dts).map(Series("Mon Tue Wed Thu Fri Sat
    Sun".split()))
...
def test_same_name_scoping(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(np.random.randn(20, 2), index=date_range("20130101",
    periods=20))
...
def test_coordinates(setup_path):
    ...
    with ensure_clean_store(setup_path) as store:
        ...
        df = DataFrame(
            np.random.randn(1000, 2), index=date_range("20000101",
    periods=1000)
        )
        store.append("df", df)
    ...
    with catch_warnings(record=True):
        ...
        def do_copy(f, new_f=None, keys=None, propindexes=True, **kwargs):
            try:
                ...
                tstore = store.copy(new_f, keys=keys, propindexes=propindexes,
    **kwargs)
...
def test_columns_multiindex_modified(setup_path):
    ...
    df = DataFrame(np.random.rand(4, 5), index=list("abcd"),
    columns=list("ABCDE"))
