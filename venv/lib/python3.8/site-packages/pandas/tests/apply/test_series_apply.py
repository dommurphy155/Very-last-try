...
def test_series_map_stringdtype(any_string_dtype):
    ...
    expected = Series(data=["rabbit", "dog", "cat", pd.NA],
    dtype=any_string_dtype)
...
@pytest.mark.parametrize("series", [["1-1", "1-1", np.NaN], ["1-1", "1-2",
    np.NaN]])
...
def test_apply_empty_integer_series_with_datetime_index():
    # GH 21245
    s = Series([], index=pd.date_range(start="2018-01-01", periods=0),
    dtype=int)
...
def test_transform_partial_failure(op, request):
    ...
    if op in ("ffill", "bfill", "pad", "backfill", "shift"):
        request.node.add_marker(
            pytest.mark.xfail(
                raises=AssertionError, reason=f"{op} is successful on any
    dtype"
            )
        )
...
def test_with_nested_series(datetime_series):
    ...
    result = datetime_series.apply(lambda x: Series([x, x**2], index=["x",
    "x^2"]))
    expected = DataFrame({"x": datetime_series, "x^2": datetime_series**2})
    tm.assert_frame_equal(result, expected)

    result = datetime_series.agg(lambda x: Series([x, x**2], index=["x",
    "x^2"]))
    tm.assert_frame_equal(result, expected)
...
def test_map(datetime_series):
    ...
    exp = Series(
        pd.Categorical([np.nan, "B", "C", "D"], categories=["B", "C", "D",
    "E"])
    )
    tm.assert_series_equal(a.map(b), exp)
...
def test_map_empty(request, index):
    if isinstance(index, MultiIndex):
        request.node.add_marker(
            pytest.mark.xfail(
                reason="Initializing a Series from a MultiIndex is not
    supported"
            )
        )
...
def test_apply_to_timedelta():
    ...
    with tm.assert_produces_warning(FutureWarning, match="Inferring
    timedelta64"):
...
def test_apply_listlike_reducer(string_series, ops, names, how):
    # GH 39140
    expected = Series({name: op(string_series) for name, op in zip(names,
    ops)})
...
def test_apply_dictlike_transformer(string_series, ops):
    # GH 39140
    with np.errstate(all="ignore"):
        expected = concat({name: op(string_series) for name, op in
    ops.items()})
        expected.name = string_series.name
        result = string_series.apply(ops)
        tm.assert_series_equal(result, expected)
...
def test_apply_retains_column_name():
    ...
    result = df.x.apply(lambda x: Series(range(x + 1), Index(range(x + 1),
    name="y")))
    expected = DataFrame(
