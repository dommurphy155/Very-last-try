...
def test_map_arg_is_dict_with_invalid_na_action_raises(input_na_action):
    ...
    s = Series([1, 2, 3])
    msg = f"na_action must either be 'ignore' or None, {input_na_action} was
    passed"
    with pytest.raises(ValueError, match=msg):
...
def test_map_categorical_na_action():
    values = Categorical(list("ABBABCD"), categories=list("DCBA"),
    ordered=True)
...
@pytest.mark.parametrize(
    "how, args", [("pct_change", ()), ("nsmallest", (1, ["a", "b"])), ("tail",
    1)]
)
...
def test_apply_modify_traceback():
    ...
    def transform2(row):
        if notna(row["C"]) and row["C"].startswith("shin") and row["A"] ==
    "foo":
            row["D"] = 7
        return row
...
@pytest.mark.parametrize(
    "op_wrapper", [lambda x: x, lambda x: [x], lambda x: {"A": x}, lambda x:
    {"A": [x]}]
)
