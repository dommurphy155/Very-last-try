...
def create_block(typestr, placement, item_shape=None, num_offset=0,
    ...
    shape)
        ...
        values = Categorical(["a", "a", "a", "a", "b", "b", "c", "c", "c",
    "d"])
...
def create_mgr(descr, item_shape=None):
    ...
    * to force multiple blocks with same dtype, use '-SUFFIX'::
    ...
    for blockstr, placement in block_placements.items():
        ...
        blocks.append(
            create_block(
                typestr, placement, item_shape=item_shape,
...
class TestBlockManager:
    ...
    def test_set(self):
        ...
        tm.assert_numpy_array_equal(mgr.iget(0).internal_values(),
    np.array([0] * 3))
        tm.assert_numpy_array_equal(
            mgr.iget(1).internal_values(), np.array(["bar"] * 3,
    dtype=np.object_)
        )
        tm.assert_numpy_array_equal(mgr.iget(2).internal_values(),
    np.array([2] * 3))
        tm.assert_numpy_array_equal(
            mgr.iget(3).internal_values(), np.array(["foo"] * 3,
    dtype=np.object_)
        )
    ...
    def test_copy(self, mgr):
        ...
        for blk, cp_blk in zip(mgr.blocks, cp.blocks):
            ...
            if isinstance(cpvals, DatetimeArray):
                assert (lbase is None and rbase is None) or (lbase is not
    rbase)
    ...
    def test_astype(self, t):
        ...
        # mixed
        mgr = create_mgr("a,b: object; c: bool; d: datetime; e: f4; f: f2; g:
    f8")
    ...
    def test_convert(self):
        ...
        mgr = create_mgr(
            "a,b,foo: object; f: i4; bool: bool; dt: datetime; i: i8; g: f8;
    h: f2"
        )
        mgr.iset(0, np.array(["1"] * N, dtype=np.object_))
        mgr.iset(1, np.array(["2."] * N, dtype=np.object_))
    ...
    def test_consolidate_ordering_issues(self, mgr):
        ...
        tm.assert_numpy_array_equal(
            cons.blocks[0].mgr_locs.as_array, np.arange(len(cons.items),
    dtype=np.intp)
        )
    ...
    def test_get_numeric_data(self, using_copy_on_write):
        ...
        tm.assert_index_equal(numeric.items, Index(["int", "float", "complex",
    "bool"]))
        tm.assert_almost_equal(
...
class TestIndexing:
    ...
    def test_get_slice(self, mgr):
        ...
        for ax in range(mgr.ndim):
            ...
            if mgr.ndim < 2:
                ...
                assert_slice_ok(mgr, ax, np.ones(mgr.shape[ax],
    dtype=np.bool_))
                assert_slice_ok(mgr, ax, np.zeros(mgr.shape[ax],
    dtype=np.bool_))
    ...
    def test_take(self, mgr):
        def assert_take_ok(mgr, axis, indexer):
            ...
            tm.assert_numpy_array_equal(
                np.take(mat, indexer, axis), _as_array(taken),
    check_dtype=False
            )
    ...
    fill_value=fill_value)
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_axis_is_ok(mgr, ax, mgr.axes[ax][[0, 0, 0]],
    fill_value)
            assert_reindex_axis_is_ok(mgr, ax, Index(["foo", "bar", "baz"]),
    fill_value)
            ...
            if mgr.shape[ax] >= 3:
                assert_reindex_axis_is_ok(mgr, ax, mgr.axes[ax][:-3],
    fill_value)
                assert_reindex_axis_is_ok(mgr, ax, mgr.axes[ax][-3::-1],
    fill_value)
                assert_reindex_axis_is_ok(
    ...
    fill_value=fill_value)
        ...
        for ax in range(mgr.ndim):
            ...
            assert_reindex_indexer_is_ok(
                mgr, ax, mgr.axes[ax][::-1], np.arange(mgr.shape[ax]),
    fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, mgr.axes[ax], np.arange(mgr.shape[ax])[::-1],
    fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, Index(["foo", "bar", "baz"]), np.array([0, 0, 0]),
    fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr, ax, Index(["foo", "bar", "baz"]), np.array([-1, 0, -1]),
    fill_value
            )
            assert_reindex_indexer_is_ok(
                mgr,
...
def test_block_shape():
    ...
    assert a._mgr.blocks[0].mgr_locs.indexer ==
    b._mgr.blocks[0].mgr_locs.indexer
...
def test_make_block_no_pandas_array(block_maker):
    ...
    if block_maker is make_block:
        ...
        result = block_maker(arr, slice(len(arr)), dtype=arr.dtype,
    ndim=arr.ndim)
        assert result.dtype.kind in ["i", "u"]
        assert result.is_extension is False
