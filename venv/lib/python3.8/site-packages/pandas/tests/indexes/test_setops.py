...
@pytest.mark.parametrize("names", [("foo", "foo", "foo"), ("foo", "bar",
    None)])
...
def test_dunder_inplace_setops_deprecated(index):
    ...
    with tm.assert_produces_warning(FutureWarning,
    raise_on_extra_warnings=is_pyarrow):
        index ^= index
...
class TestSetOps:
    ...
    @pytest.mark.parametrize(
        "method", ["intersection", "union", "difference",
    ...
    def test_symmetric_difference(self, index):
        ...
        if index[0] in index[1:] or index[-1] in index[:-1]:
            # index fixture has e.g. an index of bools that does not satisfy
    this,
...
class TestSetOpsUnsorted:
    ...
    @pytest.mark.parametrize("index", ["string"], indirect=True)
    @pytest.mark.parametrize("second_name,expected", [(None, None), ("name",
    "name")])
