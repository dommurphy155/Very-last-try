...
class TestCommon:
    ...
    def test_droplevel(self, index_flat):
        ...
        for level in [index.name, [index.name]]:
            if isinstance(index.name, tuple) and level is index.name:
                ...
                continue
            msg = (
                "Cannot remove 1 levels from an index with 1 levels: at least
    one "
                "level must be left."
            )
    ...
    def test_copy_name2(self, index_flat):
        ...
        with pytest.raises(ValueError, match="Length of new names must be 1,
    got 2"):
    ...
    def test_searchsorted_monotonic(self, index_flat, request):
        ...
        if isinstance(index, pd.IntervalIndex):
            mark = pytest.mark.xfail(
                reason="IntervalIndex.searchsorted does not support Interval
    arg",
    ...
    def test_drop_duplicates(self, index_flat, keep):
        ...
        tm.assert_numpy_array_equal(idx.duplicated(keep=keep),
    expected_duplicated)
    ...
    def test_drop_duplicates_no_duplicates(self, index_flat):
        ...
        tm.assert_numpy_array_equal(unique_idx.duplicated(),
    expected_duplicated)
    ...
    def test_has_duplicates(self, index_flat):
        ...
        if not len(index) or isinstance(index, RangeIndex):
            ...
            pytest.skip("Skip check for empty Index, MultiIndex, and
    RangeIndex")
    ...
    @pytest.mark.parametrize(
        "dtype",
        ["int64", "uint64", "float64", "category", "datetime64[ns]",
    "timedelta64[ns]"],
    )
    def test_astype_preserves_name(self, index, dtype):
        ...
        if (
            ...
            warn = FutureWarning
        elif index.dtype.kind == "c" and dtype in ["float64", "int64",
    "uint64"]:
...
def test_sort_values_invalid_na_position(index_with_missing, na_position):
    ...
    with pytest.raises(ValueError, match=f"invalid na_position:
    {na_position}"):
