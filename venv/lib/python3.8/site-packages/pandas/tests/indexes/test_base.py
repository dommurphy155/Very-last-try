...
class TestIndex(Base):
    ...
    @pytest.mark.parametrize("index", ["string", "int", "float"],
    indirect=True)
    ...
    @pytest.mark.parametrize(
        "mapper",
        [Series(["foo", 2.0, "baz"], index=[0, 2, -1]), {0: "foo", 2: 2.0, -1:
    "baz"}],
    )
    ...
    def test_format_missing(self, vals, nulls_fixture):
        ...
        formatted = index.format()
        null_repr = "NaN" if isinstance(nulls_fixture, float) else
    str(nulls_fixture)
    ...
    def test_drop_with_duplicates_in_index(self, index):
        ...
        if isinstance(index, IntervalIndex) and not IS64:
            pytest.skip("Cannot test IntervalIndex with int64 dtype on 32 bit
    platform")
    ...
    @pytest.mark.parametrize(
        "index,expected",
        [
            (Index(["qux", "baz", "foo", "bar"]), np.array([False, False,
    True, True])),
    ...
    def test_isin_level_kwarg_bad_label_raises(self, label, index):
        if isinstance(index, MultiIndex):
            ...
            msg = rf"Requested level \({label}\) does not match index name
    \(foo\)"
        with pytest.raises(KeyError, match=msg):
    ...
    @pytest.mark.parametrize(
        "index,expected", [(Index(list("abcd")), True), (Index(range(4)),
    False)]
    )
    ...
    def test_outer_join_sort(self):
        ...
        with tm.assert_produces_warning(RuntimeWarning):
            expected =
    right_index.astype(object).union(left_index.astype(object))
    ...
    @pytest.mark.parametrize("labels", [[], np.array([]), np.array([],
    dtype=np.int64)])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (MultiIndex.from_tuples([(1, 2), (4, 6)]), np.array([True,
    False])),
        ],
    )
    ...
    def test_contains_method_removed(self, index):
        ...
        if isinstance(index, IntervalIndex):
            ...
            msg = f"'{type(index).__name__}' object has no attribute
    'contains'"
    ...
    def test_sortlevel(self):
        ...
        with pytest.raises(Exception, match="ascending must be a single bool
    value or"):
            index.sortlevel(ascending="True")

        with pytest.raises(
            Exception, match="ascending must be a list of bool values of
    length 1"
        ):
...
class TestMixedIntIndex(Base):
    ...
    pd.NaT]),
            ...
            (
                PeriodIndex(["2012-02", "2012-04", "NaT", "2012-05"],
    freq="M"),
                PeriodIndex(["2012-02", "2012-04", "2012-05"], freq="M"),
            ),
        ],
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [["a", "a"], ["c", "d"]],
                ["L1", "L2"],
                MultiIndex([["a"], ["c", "d"]], [[0, 0], [0, 1]], names=["L1",
    "L2"]),
            ),
        ],
    )
...
def test_deprecated_fastpath():
    ...
    with pytest.raises(TypeError, match=msg):
        Int64Index(np.array([1, 2, 3], dtype="int64"), name="test",
    fastpath=True)
