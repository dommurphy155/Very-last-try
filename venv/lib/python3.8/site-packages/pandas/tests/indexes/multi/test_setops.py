...
def test_intersection_non_object(idx, sort):
    ...
    expected = MultiIndex(levels=idx.levels, codes=[[]] * idx.nlevels,
    names=None)
...
def test_intersection_with_missing_values_on_both_sides(nulls_fixture):
    # GH#38623
    mi1 = MultiIndex.from_arrays([[3, nulls_fixture, 4, nulls_fixture], [1, 2,
    4, 2]])
...
def test_union_duplicates(index, request):
    ...
    if index.dtype.kind == "c":
        mark = pytest.mark.xfail(
            reason="sort_values() call raises bc complex objects are not
    comparable"
        )
    ...
    mi2 = MultiIndex.from_arrays([[values[0]] + values, [1] * (len(values) +
    1)])
    result = mi1.union(mi2)
    expected = mi2.sort_values()
    if mi2.levels[0].dtype == np.uint64 and (mi2.get_level_values(0) <
    2**63).all():
        # GH#47294 - union uses lib.fast_zip, converting data to Python
    integers
        # and loses type information. Result is then unsigned only when values
    are
...
def test_intersection_lexsort_depth(levels1, levels2, codes1, codes2, names):
    ...
    with tm.assert_produces_warning(FutureWarning,
    match="MultiIndex.lexsort_depth"):
