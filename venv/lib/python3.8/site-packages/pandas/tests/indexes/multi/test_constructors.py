...
def test_constructor_single_level():
    result = MultiIndex(
        levels=[["foo", "bar", "baz", "qux"]], codes=[[0, 1, 2, 3]],
    names=["first"]
    )
...
def test_na_levels():
    ...
    expected = MultiIndex(
        levels=[[np.nan, "s", pd.NaT, 128, None]], codes=[[-1, -1, 1, -1, 3,
    -1]]
    )
    tm.assert_index_equal(result, expected)
...
def test_from_arrays(idx):
    ...
    # infer correctly
    result = MultiIndex.from_arrays([[pd.NaT, Timestamp("20130101")], ["a",
    "b"]])
...
@pytest.mark.parametrize(
    ...
    tz="US/Eastern"),
            date_range("2015-01-01 10:00", freq="H", periods=3,
    tz="Asia/Tokyo"),
        ),
        (
...
def test_from_arrays_respects_none_names():
    ...
    expected = MultiIndex(
        levels=[[1, 2, 3], ["a", "b", "c"]], codes=[[0, 1, 2], [0, 1, 2]],
    names=None
    )
...
@pytest.mark.parametrize(
    "invalid_input", [1, [1], [1, 2], [[1], 2], "a", ["a"], ["a", "b"],
    [["a"], "b"]]
)
def test_from_product_invalid_input(invalid_input):
    msg = r"Input must be a list / sequence of iterables|Input must be
    list-like"
    with pytest.raises(TypeError, match=msg):
...
@pytest.mark.parametrize("f", [lambda x: x, lambda x: Series(x), lambda x:
    x.values])
def test_from_product_index_series_categorical(ordered, f):
    ...
    expected = pd.CategoricalIndex(
        list("abcaab") + list("abcaab"), categories=list("bac"),
    ordered=ordered
    )
...
@pytest.mark.parametrize(
    "names_in,names_out", [(None, [("L1", "x"), ("L2", "y")]), (["x", "y"],
    ["x", "y"])]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["a", "b", "c"], "Length of names must match number of levels in
    MultiIndex"),
    ],
)
...
def test_raise_invalid_sortorder():
    # Test that the MultiIndex constructor raise when a incorrect sortorder is
    given
    # GH#28518
    ...
    with pytest.raises(ValueError, match=r".* sortorder 2 with lexsort_depth
    1.*"):
        MultiIndex(
    ...
    with pytest.raises(ValueError, match=r".* sortorder 1 with lexsort_depth
    0.*"):
        MultiIndex(
...
def test_datetimeindex():
    ...
    for d1, d2 in itertools.product([date1, date2, date3], [date1, date2,
    date3]):
...
def test_dtype_representation():
    # GH#46900
    pmidx = MultiIndex.from_arrays([[1], ["a"]], names=[("a", "b"), ("c",
    "d")])
    result = pmidx.dtypes
    expected = Series(
        ["int64", "object"], index=MultiIndex.from_tuples([("a", "b"), ("c",
    "d")])
    )
