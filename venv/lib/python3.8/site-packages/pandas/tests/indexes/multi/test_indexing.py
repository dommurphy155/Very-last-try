...
class TestSliceLocs:
    ...
    def test_slice_locs_not_sorted(self):
        index = MultiIndex(
            levels=[Index(np.arange(4)), Index(np.arange(4)),
    Index(np.arange(4))],
            codes=[
...
class TestGetIndexer:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ([[1, np.nan, 2], [3, 4, 5]], [(np.nan, 4)], np.array([1],
    dtype=np.intp)),
            ([[1, 2, 3], [np.nan, 4, 5]], [(1, np.nan)], np.array([0],
    dtype=np.intp)),
            (
                [[1, 2, 3], [np.nan, 4, 5]],
                [np.nan, 4, 5],
    ...
    def test_get_indexer_methods(self):
        ...
        # indexer and backfilling, and getting an indexer and padding all
    behave
        # correctly in the case where all of the target values fall in between
        # several levels in the MultiIndex into which they are getting an
    indexer
        #
        ...
        backfill_indexer = mult_idx_1.get_indexer(mult_idx_2,
    method="backfill")
    ...
    dtype=indexer_no_fill.dtype)
        ...
        indexer_backfilled = mult_idx_1.get_indexer(mult_idx_2,
    method="backfill")
    ...
    def test_get_indexer_crossing_levels(self):
        # https://github.com/pandas-dev/pandas/issues/29896
        # tests a corner case with get_indexer() with MultiIndexes where, when
    we
...
@pytest.mark.parametrize(
    "ind2",
    [[True, False, True, False, False], Index([True, False, True, False,
    False])],
)
...
class TestGetLoc:
    ...
    def test_get_loc_multiple_dtypes(self, dtype1, dtype2):
        # GH 18520
        levels = [np.array([0, 1]).astype(dtype1), np.array([0,
    1]).astype(dtype2)]
    ...
    def test_get_loc_cast_bool(self, dtype):
        # GH 19086: int is casted to bool, but not vice-versa (for object
    dtype)
        #  With bool dtype, we don't cast in either direction.
        levels = [Index([False, True], dtype=dtype), np.arange(2,
    dtype="int64")]
        idx = MultiIndex.from_product(levels)
    ...
    def test_get_loc_duplicates2(self):
        ...
        index = MultiIndex(
            ...
            codes=[[0, 0, 0, 1, 2, 2, 2, 2, 2, 2], [1, 3, 4, 6, 0, 2, 2, 3, 5,
    7]],
            names=["tag", "day"],
        )
...
def test_timestamp_multiindex_indexer():
    ...
    idx = MultiIndex.from_product(
        [
            date_range("2019-01-01T00:15:33", periods=100, freq="H",
    name="date"),
            ["x"],
            [3],
        ]
...
def test_slice_indexer_with_missing_value(index_arr, expected, start_idx,
    end_idx):
    # issue 19132
    idx = MultiIndex.from_arrays(index_arr)
