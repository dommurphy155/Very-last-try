...
def test_take_invalid_kwargs():
    vals = [["A", "B"], [pd.Timestamp("2011-01-01"),
...
def test_large_multiindex_error():
    ...
    df_below_1000000 = pd.DataFrame(
        1, index=MultiIndex.from_product([[1, 2], range(499999)]),
    columns=["dest"]
    )
    ...
    df_above_1000000 = pd.DataFrame(
        1, index=MultiIndex.from_product([[1, 2], range(500001)]),
    columns=["dest"]
    )
    with pytest.raises(KeyError, match=r"^\(-1, 0\)$"):
...
def test_rangeindex_fallback_coercion_bug():
    ...
    expected = pd.DataFrame(
        ...
        index=MultiIndex.from_product([range(10), range(10)], names=["fizz",
    "buzz"]),
    )
