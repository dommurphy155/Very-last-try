...
def test_get_dtypes():
    ...
    idx_multitype = MultiIndex.from_product(
        [[1, 2, 3], ["a", "b", "c"], pd.date_range("20200101", periods=2,
    tz="UTC")],
        names=["int", "string", "dt"],
    )
    expected = pd.Series(
...
def test_set_value_keeps_names():
    ...
    df = pd.DataFrame(
        np.random.randn(6, 4), columns=["one", "two", "three", "four"],
    index=idx
    )
    df = df.sort_index()
...
def test_set_levels_with_iterable():
    ...
    expected = MultiIndex.from_arrays([expected_sizes, colors], names=["size",
    "color"])
    tm.assert_index_equal(result, expected)
...
def test_set_levels_pos_args_deprecation():
    ...
    msg = (
        r"In a future version of pandas all arguments of MultiIndex.set_levels
    except "
...
def test_set_codes_pos_args_depreciation(idx):
    # https://github.com/pandas-dev/pandas/issues/41485
    msg = (
        r"In a future version of pandas all arguments of MultiIndex.set_codes
    except "
        r"for the argument 'codes' will be keyword-only"
    )
    with tm.assert_produces_warning(FutureWarning, match=msg):
        result = idx.set_codes([[0, 0, 1, 2, 3, 3], [0, 1, 0, 1, 0, 1]], [0,
    1])
    expected = MultiIndex.from_tuples(
        [
            ("foo", "one"),
