...
class TestFloatNumericIndex(NumericBase):
    ...
    def test_constructor_explicit(self, mixed_index, float_index):
        ...
        self.check_coerce(
            float_index, Index((np.arange(5) * 2.5), dtype=object),
    is_float_index=False
        )
        self.check_coerce(
            mixed_index, Index([1.5, 2, 3, 4, 5], dtype=object),
    is_float_index=False
        )
    ...
    def test_nan_multiple_containment(self):
        ...
        tm.assert_numpy_array_equal(idx.isin([2.0, np.pi]), np.array([False,
    False]))
        tm.assert_numpy_array_equal(idx.isin([np.nan]), np.array([False,
    True]))
        tm.assert_numpy_array_equal(idx.isin([1.0, np.nan]), np.array([True,
    True]))
        idx = index_cls([1.0, 2.0])
        tm.assert_numpy_array_equal(idx.isin([np.nan]), np.array([False,
    False]))
...
class NumericInt(NumericBase):
    ...
    def test_identical(self, simple_index, dtype):
        ...
        assert not
    index.astype(dtype=object).identical(index.astype(dtype=dtype))
...
class TestIntNumericIndex(NumericInt):
    ...
    @pytest.fixture(
        params=[range(0, 20, 2), range(19, -1, -1)], ids=["index_inc",
    "index_dec"]
    )
    def index(self, request, dtype):
        return self._index_cls(request.param, dtype=dtype)
    ...
    def test_constructor(self, dtype):
        ...
        msg = (
            rf"{index_cls.__name__}\(\.\.\.\) must be called with a collection
    of some "
            "kind, 5 was passed"
        )
        with pytest.raises(TypeError, match=msg):
...
@pytest.mark.parametrize(
    "box",
    [list, lambda x: np.array(x, dtype=object), lambda x: Index(x,
    dtype=object)],
)
