...
class TestGetItem:
    ...
    def test_getitem(self):
        ...
        for idx in [idx1]:
            ...
            result = idx[0:5]
            expected = period_range("2011-01-01", "2011-01-05", freq="D",
    name="idx")
            ...
            expected = PeriodIndex(
                ["2011-01-01", "2011-01-03", "2011-01-05", "2011-01-07",
    "2011-01-09"],
                freq="D",
                name="idx",
            )
            ...
            result = idx[-20:-5:3]
            expected = PeriodIndex(
                ["2011-01-12", "2011-01-15", "2011-01-18", "2011-01-21",
    "2011-01-24"],
                freq="D",
                name="idx",
            )
    ...
    def test_getitem_index(self):
        ...
        result = idx[[1, 3, 5]]
        exp = PeriodIndex(["2007-02", "2007-04", "2007-06"], freq="M",
    name="x")
        tm.assert_index_equal(result, exp)

        result = idx[[True, True, False, False, False, True, True, False,
    False, False]]
        exp = PeriodIndex(
...
class TestGetLoc:
    ...
    def test_get_loc3(self):
        ...
        assert idx.get_loc("2000-01-02T12", method="nearest", tolerance="1
    day") == 1
        assert (
            idx.get_loc("2000-01-02T12", method="nearest",
    tolerance=Timedelta("1D"))
            == 1
        )
        assert (
            idx.get_loc(
                "2000-01-02T12", method="nearest", tolerance=np.timedelta64(1,
    "D")
            )
            == 1
        )
    ...
    tolerance=timedelta(1)) == 1
        ...
        with pytest.raises(
            ValueError, match="list-like tolerance size must match target
    index size"
        ):
            idx.get_loc(
                "2000-01-10",
...
class TestGetIndexer:
    def test_get_indexer(self):
        ...
        tm.assert_numpy_array_equal(
            idx.get_indexer(target, "pad"), np.array([-1, 0, 1, 2],
    dtype=np.intp)
        )
        tm.assert_numpy_array_equal(
            idx.get_indexer(target, "backfill"), np.array([0, 1, 2, -1],
    dtype=np.intp)
        )
        tm.assert_numpy_array_equal(
            idx.get_indexer(target, "nearest"), np.array([0, 0, 1, 2],
    dtype=np.intp)
        )

        res = idx.get_indexer(target, "nearest", tolerance=Timedelta("1 day"))
        tm.assert_numpy_array_equal(res, np.array([0, 0, 1, -1],
    dtype=np.intp))
    ...
    def test_get_indexer_mismatched_dtype_different_length(self,
    non_comparable_idx):
    ...
    def test_get_indexer_mismatched_dtype_with_method(self,
    non_comparable_idx, method):
        dti = date_range("2016-01-01", periods=3)
        pi = dti.to_period("D")
    ...
    def test_get_indexer2(self):
        ...
        tm.assert_numpy_array_equal(
            idx.get_indexer(target, "backfill"), np.array([0, 1, 2],
    dtype=np.intp)
        )
        tm.assert_numpy_array_equal(
            idx.get_indexer(target, "nearest"), np.array([0, 1, 1],
    dtype=np.intp)
        )
        tm.assert_numpy_array_equal(
        ...
        tm.assert_numpy_array_equal(
            idx.get_indexer(
                target, "nearest", tolerance=[np.timedelta64(x) for x in
    tol_raw]
            ),
            np.array([0, -1, 1], dtype=np.intp),
        )
        ...
        with pytest.raises(
            libperiod.IncompatibleFrequency, match="Input has different
    freq=None from"
        ):
...
class TestTake:
    def test_take(self):
        ...
        for idx in [idx1]:
            ...
            expected = period_range("2011-01-01", "2011-01-03", freq="D",
    name="idx")
            tm.assert_index_equal(result, expected)
            assert result.freq == "D"
            assert result.freq == expected.freq
            ...
            expected = PeriodIndex(
                ["2011-01-01", "2011-01-03", "2011-01-05"], freq="D",
    name="idx"
            )
            tm.assert_index_equal(result, expected)
            assert result.freq == expected.freq
            ...
            expected = PeriodIndex(
                ["2011-01-08", "2011-01-05", "2011-01-02"], freq="D",
    name="idx"
            )
            tm.assert_index_equal(result, expected)
            assert result.freq == expected.freq
            ...
            expected = PeriodIndex(
                ["2011-01-04", "2011-01-03", "2011-01-06"], freq="D",
    name="idx"
            )
            tm.assert_index_equal(result, expected)
            assert result.freq == expected.freq
            ...
            result = idx.take([-3, 2, 5])
            expected = PeriodIndex(
                ["2011-01-29", "2011-01-03", "2011-01-06"], freq="D",
    name="idx"
            )
            tm.assert_index_equal(result, expected)
            assert result.freq == expected.freq
