...
class TestIntervalIndex:
    ...
    @pytest.mark.parametrize(
        "breaks",
        [date_range("2018-01-01", periods=5), timedelta_range("0 days",
    periods=5)],
    )
    ...
    def test_comparison(self):
        ...
        msg = "|".join(
            [
                ...
                r"Invalid comparison between dtype=interval\[int64, right\]
    and ",
            ]
        )
        with pytest.raises(TypeError, match=msg):
    ...
    def test_missing_values(self, closed):
        idx = Index(
            [np.nan, Interval(0, 1, closed=closed), Interval(1, 2,
    closed=closed)]
        )
    ...
    closed=closed)
        ...
        for other_closed in {"left", "right", "both", "neither"} - {closed}:
            ...
            expected =
    index1.astype(object).append(index_other_closed.astype(object))
            tm.assert_index_equal(result, expected)
    ...
    def test_is_overlapping(self, start, shift, na_value, closed):
        ...
        # non-overlapping
        tuples = [(start + n * shift, start + (n + 1) * shift) for n in (0, 2,
    4)]
        ...
        # overlapping
        tuples = [(start + n * shift, start + (n + 2) * shift) for n in
    range(3)]
        index = IntervalIndex.from_tuples(tuples, closed=closed)
        ...
        # common endpoints
        tuples = [(start + n * shift, start + (n + 1) * shift) for n in
    range(3)]
        index = IntervalIndex.from_tuples(tuples, closed=closed)
    ...
    @pytest.mark.parametrize("new_closed", ["left", "right", "both",
    "neither"])
...
def test_searchsorted_invalid_argument(arg):
    ...
    'pandas._libs.interval.Interval' and "
    with pytest.raises(TypeError, match=msg):
