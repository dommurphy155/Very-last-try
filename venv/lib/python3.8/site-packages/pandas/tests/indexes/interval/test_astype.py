...
class AstypeTests:
    ...
    def test_astype_category(self, index):
        ...
        expected = CategoricalIndex(index.values, categories=categories,
    ordered=True)
        tm.assert_index_equal(result, expected)
...
class TestIntSubtype(AstypeTests):
    ...
    indexes = [
        ...
        IntervalIndex.from_breaks(np.arange(100, dtype="uint64"),
    closed="left"),
    ]
    ...
    def test_subtype_conversion(self, index, subtype):
        ...
        expected = IntervalIndex.from_arrays(
            index.left.astype(subtype), index.right.astype(subtype),
    closed=index.closed
        )
        tm.assert_index_equal(result, expected)

    @pytest.mark.parametrize(
        "subtype_start, subtype_end", [("int64", "uint64"), ("uint64",
    "int64")]
    )
...
class TestFloatSubtype(AstypeTests):
    ...
    indexes = [
        ...
        IntervalIndex.from_arrays(
            [-1.5, np.nan, 0.0, 0.0, 1.5], [-0.5, np.nan, 1.0, 1.0, 3.0],
    closed="both"
        ),
    ]
    ...
    def test_subtype_integer_errors(self):
        ...
        dtype = IntervalDtype("uint64", "right")
        msg = re.escape(
            "Cannot convert interval[float64, right] to interval[uint64,
    right]; "
...
class TestDatetimelikeSubtype(AstypeTests):
    ...
    def test_subtype_integer(self, index, subtype):
        ...
        if index.isna().any() and subtype == "uint64":
            warn = FutureWarning
        msg = "In a future version, this astype will raise if the conversion
    overflows"
        ...
        expected = IntervalIndex.from_arrays(new_left, new_right,
    closed=index.closed)
        tm.assert_index_equal(result, expected)
