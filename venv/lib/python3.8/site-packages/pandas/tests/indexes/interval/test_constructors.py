    ...
    def test_constructor_pass_closed(self, constructor, breaks):
        ...
        for dtype in (iv_dtype, str(iv_dtype)):
            with tm.assert_produces_warning(warn):
                ...
                result = constructor(dtype=dtype, closed="left",
    **result_kwargs)
            assert result.dtype.closed == "left"

    @pytest.mark.filterwarnings("ignore:Passing keywords other:FutureWarning")
    @pytest.mark.parametrize("breaks", [[np.nan] * 2, [np.nan] * 4, [np.nan] *
    50])
    ...
    @pytest.mark.parametrize("cat_constructor", [Categorical,
    CategoricalIndex])
...
class TestFromArrays(ConstructorTests):
    ...
    @pytest.mark.parametrize(
        "left_subtype, right_subtype", [(np.int64, np.float64), (np.float64,
    np.int64)]
    )
...
class TestClassConstructors(ConstructorTests):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [Interval(0, 3, closed="neither"), Interval(2, 5,
    closed="neither")],
                "left",
            ),
            (
                [Interval(0, 3, closed="left"), Interval(2, 5,
    closed="right")],
                "neither",
            ),
