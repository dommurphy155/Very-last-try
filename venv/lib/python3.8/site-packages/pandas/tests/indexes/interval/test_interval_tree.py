...
def skipif_32bit(param):
    ...
    """
    marks = pytest.mark.skipif(not IS64, reason="GH 23440: int type mismatch
    on 32bit")
...
class TestIntervalTree:
    def test_get_indexer(self, tree):
        ...
        with pytest.raises(
            KeyError, match="'indexer does not intersect a unique set of
    intervals'"
        ):
    ...
    def test_get_indexer_overflow(self, dtype, target_value, target_dtype):
        left, right = np.array([0, 1], dtype=dtype), np.array([1, 2],
    dtype=dtype)
    ...
    def test_get_indexer_non_unique(self, tree):
        indexer, missing = tree.get_indexer_non_unique(np.array([1.0, 2.0,
    6.5]))
    ...
    target_dtype):
        left, right = np.array([0, 2], dtype=dtype), np.array([1, 3],
    dtype=dtype)
        tree = IntervalTree(left, right)
    ...
    @pytest.mark.parametrize(
        "leaf_size", [skipif_32bit(1), skipif_32bit(10), skipif_32bit(100),
    10000]
    )
    ...
    @pytest.mark.parametrize("order", (list(x) for x in
    ...
    def test_construction_overflow(self):
        # GH 25485
        left, right = np.arange(101, dtype="int64"), [np.iinfo(np.int64).max]
    * 101
        tree = IntervalTree(left, right)
