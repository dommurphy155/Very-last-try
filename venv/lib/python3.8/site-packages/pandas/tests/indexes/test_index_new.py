...
class TestIndexConstructorInference:
    ...
    @pytest.mark.parametrize(
        "vals", [[True, False, True], np.array([True, False, True],
    dtype=bool)]
    )
    ...
    def test_constructor_infer_nat_dt_like(
        ...
        if nulls_fixture is NA:
            ...
            mark = pytest.mark.xfail(reason="Broken with np.NaT ctor; see GH
    31884")
...
class TestDtypeEnforced:
    ...
    @pytest.mark.parametrize("dtype", [object, "float64", "uint64",
    "category"])
    ...
    @pytest.mark.parametrize("dtype", [object, "float64", "uint64",
    "category"])
    def test_constructor_categorical_values_mismatched_non_ea_dtype(self,
    dtype):
...
class TestIndexConstructorUnwrapping:
    ...
    def test_constructor_from_series_dt64(self, klass):
        stamps = [Timestamp("20110101"), Timestamp("20120101"),
    Timestamp("20130101")]
