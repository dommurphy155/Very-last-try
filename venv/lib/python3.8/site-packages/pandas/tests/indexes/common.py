...
class Base:
    ...
    def test_ensure_copied_data(self, index):
        ...
        if isinstance(index, PeriodIndex):
            ...
            tm.assert_numpy_array_equal(index.asi8, result.asi8,
    check_same="same")
    ...
    def test_repeat(self, simple_index):
        ...
        new_index_cls = Int64Index if isinstance(idx, RangeIndex) else
    idx._constructor
    ...
    def test_insert_out_of_bounds(self, index):
        ...
        if len(index) == 0:
            ...
            msg = "slice indices must be integers or None or have an __index__
    method"
    ...
    def test_equals(self, index):
        if isinstance(index, IntervalIndex):
            # IntervalIndex tested separately, the
    index.equals(index.astype(object))
            #  fails for IntervalIndex
            return

        is_ea_idx = type(index) is Index and not isinstance(index.dtype,
    np.dtype)
    ...
    def test_nulls(self, index):
        ...
        if len(index) == 0:
            ...
            tm.assert_numpy_array_equal(index.isna(), np.zeros(len(index),
    dtype=bool))
            tm.assert_numpy_array_equal(index.notna(), np.ones(len(index),
    dtype=bool))
        else:
            result = isna(index)
...
class NumericBase(Base):
    ...
    def test_invalid_dtype(self, invalid_dtype):
        # GH 29539
        dtype = invalid_dtype
        msg = rf"Incorrect `dtype` passed: expected \w+(?: \w+)?, received
    {dtype}"
        with pytest.raises(ValueError, match=msg):
