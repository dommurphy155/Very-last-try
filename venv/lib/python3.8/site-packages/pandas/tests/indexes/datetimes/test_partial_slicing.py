...
class TestSlicing:
    def test_string_index_series_name_converted(self):
        # GH#1644
        df = DataFrame(np.random.randn(10, 4), index=date_range("1/1/2000",
    periods=10))
    ...
    def test_partial_slicing_dataframe(self):
        ...
        for rnum, resolution in enumerate(resolutions[2:], 2):
            ...
            for fmt in formats[:rnum]:
                for element, theslice in [[0, slice(None, 1)], [1, slice(1,
    None)]]:
    ...
    def test_partial_slicing_with_multiindex_series(self):
        ...
        ser = DataFrame(
            np.random.rand(1000, 1000), index=date_range("2000-1-1",
    periods=1000)
        ).stack()
    ...
    def test_partial_slice_doesnt_require_monotonicity(self):
        ...
        with pytest.raises(KeyError, match=r"Timestamp\('2014-01-10
    00:00:00'\)"):
    ...
    def test_slice_reduce_to_series(self):
        # GH 27516
        df = DataFrame({"A": range(24)}, index=date_range("2000", periods=24,
    freq="M"))
        expected = Series(
