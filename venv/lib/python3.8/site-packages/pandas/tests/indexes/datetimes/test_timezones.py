...
class TestDatetimeIndexTimezones:
    ...
    def test_dti_tz_convert_hour_overflow_dst(self):
        ...
        ts = ["2008-05-12 09:50:00", "2008-12-12 09:50:35", "2009-05-12
    09:50:32"]
        ...
        ts = ["2008-05-12 13:50:00", "2008-12-12 14:50:35", "2009-05-12
    13:50:32"]
    ...
    09:50:32"]
        ...
        ts = ["2008-05-12 13:50:00", "2008-12-12 14:50:35", "2008-05-12
    13:50:32"]
        tt = DatetimeIndex(ts).tz_localize("UTC")
        ut = tt.tz_convert("US/Eastern")
        expected = Index([9, 9, 9])
    ...
    def test_dti_tz_convert_trans_pos_plus_1__bug(self, freq, n):
        ...
        idx = date_range(datetime(2011, 3, 26, 23), datetime(2011, 3, 27, 1),
    freq=freq)
    ...
    def test_dti_tz_convert_dst(self):
        for freq, n in [("H", 1), ("T", 60), ("S", 3600)]:
            ...
            idx = date_range(
                "2014-03-08 18:00", "2014-03-09 05:00", freq=freq,
    tz="US/Eastern"
            )
            idx = idx.tz_convert("UTC")
            expected = np.repeat(
            ...
            idx = date_range(
                "2014-11-01 18:00", "2014-11-02 05:00", freq=freq,
    tz="US/Eastern"
            )
            idx = idx.tz_convert("UTC")
            expected = np.repeat(
    ...
    tz="UTC")
        ...
        for idx, expected in [(idx1, exp1), (idx2, exp2), (idx3, exp3), (idx4,
    exp4)]:
    ...
    def test_dti_tz_localize_nonexistent_raise_coerce(self):
        ...
        test_times = ["2015-03-08 01:00-05:00", "NaT", "2015-03-08
    03:00-04:00"]
    ...
    time"):
        ...
        tm.assert_index_equal(expected, DatetimeIndex(times, tz=tz,
    ambiguous="infer"))
    ...
    freq=pd.offsets.Hour())
        with pytest.raises(pytz.NonExistentTimeError, match="2011-03-13
    02:30:00"):
            dr.tz_localize(tz)

        # after dst transition, it works
        dr = date_range(
            datetime(2011, 3, 13, 3, 30), periods=3, freq=pd.offsets.Hour(),
    tz=tz
        )
    ...
    time"):
        ...
        # UTC is OK
        dr = date_range(
            datetime(2011, 3, 13), periods=48, freq=pd.offsets.Minute(30),
    tz=pytz.utc
        )
    ...
    def test_dti_tz_localize(self, prefix):
        ...
        dti = date_range(start="1/1/2005", end="1/1/2005 0:00:30.256",
    freq="L")
        dti2 = dti.tz_localize(tzstr)

        dti_utc = date_range(
            start="1/1/2005 05:00", end="1/1/2005 5:00:30.256", freq="L",
    tz="utc"
        )
        ...
        dti = date_range(start="11/6/2011 1:59", end="11/6/2011 2:00",
    freq="L")
        with pytest.raises(pytz.AmbiguousTimeError, match="Cannot infer dst
    time"):
            dti.tz_localize(tzstr)

        dti = date_range(start="3/13/2011 1:59", end="3/13/2011 2:00",
    freq="L")
        with pytest.raises(pytz.NonExistentTimeError, match="2011-03-13
    02:00:00"):
            dti.tz_localize(tzstr)
    ...
    def test_dti_tz_localize_utc_conversion(self, tz):
        ...
        with pytest.raises(pytz.NonExistentTimeError, match="2012-03-11
    02:00:00"):
            rng.tz_localize(tz)
    ...
    time"):
            date_range(
                "2013-10-26 23:00", "2013-10-27 01:00", tz="Europe/London",
    freq="H"
            )

        times = date_range(
            "2013-10-26 23:00", "2013-10-27 01:00", freq="H", tz=tz,
    ambiguous="infer"
        )
    ...
    def test_dti_construction_nonexistent_endpoint(self, tz, option, expected):
        ...
        with pytest.raises(pytz.NonExistentTimeError, match="2019-03-10
    02:00:00"):
            date_range(
                "2019-03-10 00:00", "2019-03-10 02:00", tz="US/Pacific",
    freq="H"
            )

        times = date_range(
            "2019-03-10 00:00", "2019-03-10 02:00", freq="H", tz=tz,
    nonexistent=option
        )
    ...
    def test_dti_tz_localize_nonexistent(self, tz, method, exp):
        ...
        if method == "raise":
            with pytest.raises(pytz.NonExistentTimeError, match="2015-03-29
    02:00:00"):
    ...
    def test_dti_constructor_with_fixed_tz(self):
        ...
        rng3 = date_range("3/11/2012 05:00:00+07:00", "6/11/2012
    05:00:00+07:00")
    ...
    def test_dti_tz_constructors(self, tzstr):
        ...
        idx2 = date_range(start="2005-11-10 08:00:00", freq="H", periods=2,
    tz=tzstr)
    ...
    @pytest.mark.parametrize(
        "dtype",
        [None, "datetime64[ns, CET]", "datetime64[ns, EST]", "datetime64[ns,
    UTC]"],
    )
    ...
    def test_with_tz(self, tz):
        ...
        dr = bdate_range(
            datetime(2005, 1, 1, tzinfo=pytz.utc), datetime(2009, 1, 1,
    tzinfo=pytz.utc)
        )
    ...
    def test_field_access_localize(self, prefix):
        ...
        dr = date_range(
            "2011-10-02 00:00", freq="h", periods=10, tz=prefix +
    "America/Atikokan"
        )
    ...
    def test_dti_convert_tz_aware_datetime_datetime(self, tz):
        # GH#1581
        dates = [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1,
    3)]
    ...
    @pytest.mark.parametrize("setop", ["union", "intersection",
    ...
    def test_iteration_preserves_nanoseconds(self, tz):
        # GH 19603
        index = DatetimeIndex(
            ["2018-02-08 15:00:00.168456358", "2018-02-08
    15:00:00.168456359"], tz=tz
        )
