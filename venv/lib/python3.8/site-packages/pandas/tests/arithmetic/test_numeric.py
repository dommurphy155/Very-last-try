...
# List comprehension has incompatible type List[PandasObject]; expected
    List[RangeIndex]
#  See GH#29725
...
lefts.extend(
    [
        ...
        for dtype in ["i1", "i2", "i4", "i8", "u1", "u2", "u4", "u8", "f2",
    "f4", "f8"]
...
class TestNumericArraylikeArithmeticWithDatetimeLike:
    ...
    box_with_array):
        ...
        box = box_with_array
        index = numeric_idx
        expected = TimedeltaIndex([Timedelta(days=n) for n in
    range(len(index))])
    ...
    def test_add_sub_datetimedeltalike_invalid(
        ...
        msg = "|".join(
            [
                ...
                r"operand type\(s\) all returned NotImplemented from
    __array_ufunc__",
...
class TestDivisionByZero:
    ...
    def test_div_negative_zero(self, zero, numeric_idx, op):
        ...
        expected = Index([-np.inf, -np.inf, -np.inf, np.nan, np.inf],
    dtype=np.float64)
...
class TestAdditionSubtraction:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                Series([3.0, 4.0, np.nan, np.nan], index=list("ABCD"),
    name="x"),
            ),
            (
                Series([1, 2, 3], index=list("ABC"), name="x"),
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                pd.DataFrame({"x": [3.0, 4.0, np.nan, np.nan]},
    index=list("ABCD")),
            ),
            (
                pd.DataFrame({"x": [1, 2, 3]}, index=list("ABC")),
    ...
    def test_frame_operators_none_to_nan(self):
        ...
        tm.assert_frame_equal(df + df, pd.DataFrame({"a": ["aa", np.nan,
    "bb"]}))
...
class TestUFuncCompat:
    ...
    @pytest.mark.parametrize("holder", [Int64Index, UInt64Index, Float64Index,
    Series])
...
class TestNumericArithmeticUnsorted:
    ...
    @pytest.mark.parametrize("idx1", [RangeIndex(0, 10, 1), RangeIndex(0, 20,
    2)])
    @pytest.mark.parametrize("idx2", [RangeIndex(0, 10, 1), RangeIndex(0, 20,
    2)])
    ...
    @pytest.mark.parametrize("idx", [RangeIndex(0, 10, 1), RangeIndex(0, 20,
    2)])
