...
@pytest.fixture(
    params=[
        ...
        (
            date_range("20170101", periods=3, tz="US/Eastern").insert(3,
    pd.NaT),
            date_range("20170102", periods=3, tz="US/Eastern").insert(3,
    pd.NaT),
        ),
    ],
...
def interval_array(left_right_dtypes):
    """
    Fixture to generate an IntervalArray of various dtypes containing NA if
    possible
    """
...
class TestComparison:
    ...
    def elementwise_comparison(self, op, interval_array, other):
        """
        Helper that performs elementwise comparisons between `array` and
    `other`
        """
        ...
        if box is pd.DataFrame:
            if interval_array.dtype.subtype.kind not in "iuf":
                mark = pytest.mark.xfail(
                    reason="raises on DataFrame.transpose (would be fixed by
    EA2D)"
                )
    ...
    def test_compare_list_like_interval_mixed_closed(
        ...
        interval_array = IntervalArray.from_arrays(range(2), range(1, 3),
    closed=closed)
        other = interval_constructor(range(2), range(1, 3),
    closed=other_closed)
