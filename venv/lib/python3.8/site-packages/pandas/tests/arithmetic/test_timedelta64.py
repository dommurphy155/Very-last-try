...
class TestTimedelta64ArrayLikeComparisons:
    ...
    def test_compare_timedelta64_zerodim(self, box_with_array):
        ...
        box = box_with_array
        xbox = (
            box_with_array if box_with_array not in [pd.Index, pd.array] else
    np.ndarray
        )
...
class TestTimedelta64ArrayComparisons:
    ...
    @pytest.mark.parametrize(
        ...
        [
            TimedeltaIndex(
                ["2 day", "2 day", NaT, NaT, "1 day 00:00:02", "5 days
    00:00:03"]
            ),
            np.array(
                [
...
class TestTimedelta64ArithmeticUnsorted:
    ...
    def test_ufunc_coercions(self):
        ...
        idx = TimedeltaIndex(["2H", "4H", "6H", "8H", "10H"], freq="2H",
    name="x")

        for result in [idx * 2, np.multiply(idx, 2)]:
            assert isinstance(result, TimedeltaIndex)
            exp = TimedeltaIndex(["4H", "8H", "12H", "16H", "20H"], freq="4H",
    name="x")
        ...
        for result in [idx / 2, np.divide(idx, 2)]:
            assert isinstance(result, TimedeltaIndex)
            exp = TimedeltaIndex(["1H", "2H", "3H", "4H", "5H"], freq="H",
    name="x")
            tm.assert_index_equal(result, exp)
        ...
        idx = TimedeltaIndex(["-2H", "-1H", "0H", "1H", "2H"], freq="H",
    name="x")
        for result in [abs(idx), np.absolute(idx)]:
            assert isinstance(result, TimedeltaIndex)
            exp = TimedeltaIndex(["2H", "1H", "0H", "1H", "2H"], freq=None,
    name="x")
            tm.assert_index_equal(result, exp)
...
class TestTimedeltaArraylikeAddSubOps:
    ...
    def test_timedelta_ops_with_missing_values(self):
        ...
        msg = r"dtype datetime64\[ns\] cannot be converted to
    timedelta64\[ns\]"
        with pytest.raises(TypeError, match=msg):
    ...
    nat_series_dtype_timedelta
        ...
        tm.assert_series_equal(timedelta_series * 1.5, Series([NaT,
    Timedelta("1.5s")]))
        tm.assert_series_equal(1.5 * timedelta_series, Series([NaT,
    Timedelta("1.5s")]))
    ...
    nat_series_dtype_timedelta)
        ...
        # division
        tm.assert_series_equal(timedelta_series / 2, Series([NaT,
    Timedelta("0.5s")]))
        tm.assert_series_equal(timedelta_series / 2.0, Series([NaT,
    Timedelta("0.5s")]))
        tm.assert_series_equal(timedelta_series / np.nan,
    nat_series_dtype_timedelta)
    ...
    def test_td64arr_addsub_numeric_scalar_invalid(self, box_with_array,
    other):
    ...
    name=names[0])
        other = pd.Index([offsets.Hour(n=1), offsets.Minute(n=-2)],
    name=names[1])
    ...
    def test_td64arr_with_offset_series(self, names, box_with_array):
        ...
        tdi = TimedeltaIndex(["1 days 00:00:00", "3 days 04:00:00"],
    name=names[0])
        other = Series([offsets.Hour(n=1), offsets.Minute(n=-2)],
    name=names[1])
    ...
    def test_td64arr_add_sub_object_array(self, box_with_array):
        ...
        other = np.array([Timedelta(days=1), offsets.Day(2),
    Timestamp("2000-01-04")])
        ...
        expected = pd.Index([Timedelta(0), Timedelta(0),
...
class TestTimedeltaArraylikeMulDivOps:
    ...
    def test_td64arr_div_td64_scalar(self, m, unit, box_with_array):
        ...
        # reverse op
        expected = Series([Timedelta(np.timedelta64(m, unit)) / x for x in
    flat])
        expected = tm.box_expected(expected, xbox)
    ...
    def test_td64arr_floordiv_int(self, box_with_array):
        ...
        pattern = "floor_divide cannot use operands|Cannot divide int by
    Timedelta*"
    ...
    def test_td64arr_mod_tdscalar(self, box_with_array, three_days):
        ...
        warn = None
        if box_with_array is DataFrame and isinstance(three_days,
    pd.DateOffset):
    ...
    dtype="timedelta64[ns]")
        ...
        if box_with_array is DataFrame:
            expected = [tdser.iloc[0, n] / vector[n] for n in
    range(len(vector))]
        else:
