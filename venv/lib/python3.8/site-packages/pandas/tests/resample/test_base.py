...
@pytest.mark.parametrize(
    "_index_factory,_series_name,_index_start,_index_end", [DATE_RANGE,
    TIMEDELTA_RANGE]
)
...
def test_resample_nat_index_series(request, freq, series, resample_method):
    ...
    if freq == "M":
        request.node.add_marker(pytest.mark.xfail(reason="Don't know why this
    fails"))
    ...
    if resample_method == "ohlc":
        expected = DataFrame(
            [], index=ser.index[:0].copy(), columns=["open", "high", "low",
    "close"]
        )
...
def test_resample_empty_dtypes(index, dtype, resample_method):
    ...
    try:
        getattr(empty_series_dti.resample("d", group_keys=False),
    resample_method)()
    except DataError:
