...
PERIOD_RANGE = (period_range, "pi", datetime(2005, 1, 1), datetime(2005, 1,
    10))
...
def test_deprecating_on_loffset_and_base():
    ...
    # not checking the stacklevel for .groupby().resample() because it's
    complicated to
    # reconcile it with the stacklevel for Series.resample() and
    DataFrame.resample();
    # see GH #37603
    with tm.assert_produces_warning(FutureWarning):
...
def test_resample_loffset_arg_type(frame, create_index, arg):
    # GH 13218, 15002
    df = frame
    expected_means = [df.values[i: i + 2].mean() for i in range(0,
    len(df.values), 2)]
    expected_index = create_index(df.index[0], periods=len(df.index) / 2,
    freq="2D")
...
@pytest.mark.parametrize(
    "loffset", [timedelta(minutes=1), "1min", Minute(1), np.timedelta64(1,
    "m")]
)
...
def test_resample_loffset_upsample():
    ...
    idx = date_range("1/1/2000", periods=4, freq="5min")
    expected = Series([s[0], s[5], s[10], s[-1]], index=idx +
    timedelta(minutes=1))
...
def test_resample_loffset_count():
    ...
    expected_index = date_range(start_time, periods=10, freq="10S") +
    timedelta(
        seconds=1
    )
...
def test_resample_float_base():
    # GH25161
    dt = pd.to_datetime(
        ["2018-11-26 16:17:43.51", "2018-11-26 16:17:44.51", "2018-11-26
    16:17:45.51"]
    )
...
def test_resample_with_non_zero_base(start, end, start_freq, end_freq, base,
    offset):
    # GH 23882
