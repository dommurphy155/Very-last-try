...
_test_frame = DataFrame({"A": test_series, "B": test_series, "C":
    np.arange(len(dti))})
...
def test_resample_group_keys():
    ...
    g = df.resample("5D")
    expected = df.copy()
    with tm.assert_produces_warning(FutureWarning, match="Not prepending group
    keys"):
    ...
    expected.index = pd.MultiIndex.from_arrays(
        [pd.to_datetime(["2000-01-01", "2000-01-06"]).repeat(5),
    expected.index]
    )
...
def test_agg_misc():
    ...
    for t in cases:
        ...
        result = t.agg(
            A=NamedAgg("A", np.sum), B=NamedAgg("B", lambda x: np.std(x,
    ddof=1))
        )
        tm.assert_frame_equal(result, expected, check_like=True)
    ...
    expected.columns = pd.MultiIndex.from_tuples(
        [("result1", "A"), ("result1", "B"), ("result2", "A"), ("result2",
    "B")]
    )
...
@pytest.mark.parametrize(
    "func", [["min"], ["mean", "max"], {"A": "sum"}, {"A": "prod", "B":
    "median"}]
)
...
def test_agg_nested_dicts():
    ...
    for t in cases:
        with pytest.raises(pd.errors.SpecificationError, match=msg):
            t.aggregate({"r1": {"A": ["mean", "sum"]}, "r2": {"B": ["mean",
    "sum"]}})
...
def test_agg_with_datetime_index_list_agg_func(col_name):
    ...
    expected = DataFrame(
        [47.5, 143.5, 195.5],
        index=date_range(start="2017-01-01", freq="D", periods=3,
    tz="Europe/Berlin"),
...
def test_frame_downsample_method(method, numeric_only, expected_data):
    ...
    if numeric_only is lib.no_default and method not in (
        ...
        warn = FutureWarning
        msg = (
            f"default value of numeric_only in DataFrameGroupBy.{method} is
    deprecated"
        )
    elif method in ("prod", "mean", "median") and numeric_only is not True:
        warn = FutureWarning
        msg = f"Dropping invalid columns in DataFrameGroupBy.{method} is
    deprecated"
    else:
        warn = None
        msg = ""
...
def test_series_downsample_method(method, numeric_only, expected_data):
    ...
    if numeric_only and numeric_only is not lib.no_default:
        with tm.assert_produces_warning(
            ...
            with pytest.raises(NotImplementedError, match="not implement
    numeric_only"):
                func(numeric_only=numeric_only)
    elif method == "prod":
        with pytest.raises(TypeError, match="can't multiply sequence by
    non-int"):
            func(numeric_only=numeric_only)
    else:
