...
def test_resample_integerarray():
    ...
    ts = Series(
        range(9), index=date_range("1/1/2000", periods=9, freq="T"),
    dtype="Int64"
    )
...
def test_resample_how(series, downsample_method):
    ...
    expected.index = date_range("1/1/2000", periods=4, freq="5min",
    name="index")
...
@pytest.mark.parametrize("func", ["min", "max", "sum", "prod", "mean", "var",
    "std"])
def test_numpy_compat(func):
    # see gh-12811
    s = Series([1, 2, 3, 4, 5], index=date_range("20130101", periods=5,
    freq="s"))
    r = s.resample("2s")
...
11-08-2014,00:00:21.191,1"""
    ...
    expected = DataFrame(
        {"value": [4, 9, 4, 2]}, index=date_range("2014-11-08", freq="6s",
    periods=4)
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        {"value": [4, 10, 4, 1]}, index=date_range("2014-11-08", freq="7s",
    periods=4)
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        {"value": [11, 8]}, index=date_range("2014-11-08", freq="11s",
    periods=2)
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        {"value": [13, 6]}, index=date_range("2014-11-08", freq="13s",
    periods=2)
    )
    tm.assert_frame_equal(result, expected)
    ...
    expected = DataFrame(
        {"value": [16, 3]}, index=date_range("2014-11-08", freq="17s",
    periods=2)
    )
    tm.assert_frame_equal(result, expected)
...
def test_resample_upsampling_picked_but_not_correct():
    ...
    s = Series(
        np.arange(1.0, 6), index=[datetime(1975, 1, i, 12, 0) for i in
    range(1, 6)]
    )
    expected = Series(
...
def test_resample_dup_index():
    ...
    expected.columns = [Period(year=2000, quarter=i + 1, freq="Q") for i in
    range(4)]
    tm.assert_frame_equal(result, expected)
...
def test_resample_timestamp_to_period(simple_date_range_series, freq,
    expected_kwargs):
...
def test_resample_axis1():
    rng = date_range("1/1/2000", "2/29/2000")
    df = DataFrame(np.random.randn(3, len(rng)), columns=rng, index=["a", "b",
    "c"])
...
def test_resample_origin(kwargs):
    ...
    exp_rng = date_range("1999-12-31 23:57:00", "2000-01-01 01:57",
    freq="5min")
...
@pytest.mark.parametrize(
    "origin", ["invalid_value", "epch", "startday", "startt", "2000-30-30",
    object()]
)
...
@pytest.mark.parametrize("offset", ["invalid_value", "12dayys", "2000-30-30",
    object()])
def test_resample_bad_offset(offset):
    rng = date_range("2000-01-01 00:00:00", "2000-01-01 02:00", freq="s")
    ts = Series(np.random.randn(len(rng)), index=rng)
    msg = f"'offset' should be a Timedelta convertible type. Got '{offset}'
    instead."
    with pytest.raises(ValueError, match=msg):
...
def test_resample_origin_prime_freq():
    ...
    exp_rng = date_range("2000-10-01 23:14:00", "2000-10-02 00:22:00",
    freq="17min")
    ...
    exp_rng = date_range("2000-10-01 23:30:00", "2000-10-02 00:21:00",
    freq="17min")
    ...
    exp_rng = date_range("2000-10-01 23:18:00", "2000-10-02 00:26:00",
    freq="17min")
    resampled = ts.resample("17min", origin="epoch").mean()
    tm.assert_index_equal(resampled.index, exp_rng)

    exp_rng = date_range("2000-10-01 23:24:00", "2000-10-02 00:15:00",
    freq="17min")
...
def test_nanosecond_resample_error():
    ...
    exp_indx = date_range(start=pd.to_datetime(exp_start), periods=10,
    freq="100n")
...
def test_resample_anchored_intraday(simple_date_range_series):
    ...
    expected = df.shift(1, freq="D").resample("Q", kind="period",
    closed="left").mean()
...
def test_how_lambda_functions(simple_date_range_series):
    ...
    result = ts.resample("M").apply([lambda x: x.mean(), lambda x:
    x.std(ddof=1)])
...
def test_resample_timegrouper(dates):
    ...
    df = DataFrame({"A": dates, "B": np.arange(len(dates)), "C":
    np.arange(len(dates))})
    result = df.set_index("A").resample("M").count()
    expected = DataFrame(
...
def test_resample_dst_anchor():
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            [5],
            index=DatetimeIndex([datetime(2012, 11, 1)], tz="US/Eastern",
    freq="MS"),
        ),
    )
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            ...
            index=date_range("9/30/2013", "11/4/2013", freq="W-MON",
    tz="Europe/Paris"),
        ),
        "W-MON Frequency",
    )
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            ...
            index=date_range("9/1/2013", "11/1/2013", freq="MS",
    tz="Europe/Paris"),
        ),
        "MS Frequency",
    )
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            ...
            index=date_range("9/1/2013", "11/1/2013", freq="2MS",
    tz="Europe/Paris"),
        ),
        "2MS Frequency",
    )
    ...
    tm.assert_frame_equal(
        ...
        DataFrame(
            ...
            index=date_range("10/26/2013", "10/29/2013", freq="D",
    tz="Europe/Paris"),
        ),
        "D Frequency",
    )
...
def test_downsample_across_dst_weekly_2():
    ...
    expected = Series(
        index=date_range("2013-04-07", freq="W", periods=5,
    tz="Europe/London"),
        dtype=np.float64,
    )
    tm.assert_series_equal(result, expected)
...
def test_resample_equivalent_offsets(n1, freq1, n2, freq2, k):
    ...
    n1_ = n1 * k
    n2_ = n2 * k
    s = Series(0, index=date_range("19910905 13:00", "19911005 07:00",
    freq=freq1))
    s = s + range(len(s))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("19910905 06:00", "19920406 06:00", "H", "19910905 06:00", "19920406
    07:00"),
...
def test_resample_calendar_day_with_dst(
    ...
    # GH 35219
    ts = Series(1.0, date_range(first, last, freq=freq_in,
    tz="Europe/Amsterdam"))
...
def test_resample_aggregate_functions_min_count(func):
    ...
    expected = Series(
        [np.nan],
        index=DatetimeIndex(["2020-03-31"], dtype="datetime64[ns]",
    freq="Q-DEC"),
    )
    tm.assert_series_equal(result, expected)
