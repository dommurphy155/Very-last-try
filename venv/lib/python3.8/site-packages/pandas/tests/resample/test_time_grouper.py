...
test_series = Series(np.random.randn(1000), index=date_range("1/1/2000",
    periods=1000))
...
@pytest.mark.xfail(reason="if TimeGrouper is used included, 'nth' doesn't work
    yet")
...
def test_resample_entirely_nat_window(method, method_args, unit):
    ...
    expected = Series(
        [0.0, unit], index=pd.DatetimeIndex(["2017-01-01", "2017-01-03"],
    freq="2D")
    )
    tm.assert_series_equal(result, expected)
...
def test_aggregate_with_nat(func, fill_value):
    ...
    pad = DataFrame([[fill_value] * 4], index=[3], columns=["A", "B", "C",
    "D"])
