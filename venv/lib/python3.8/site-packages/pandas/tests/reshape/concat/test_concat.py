...
class TestConcatenate:
    ...
    def test_concat_copy(self, using_array_manager):
        ...
        for arr in result._mgr.arrays:
            if arr.dtype.kind == "f":
                ...
                assert any(np.shares_memory(arr, other) for other in
    df3._mgr.arrays)
    ...
    def test_concat_with_group_keys(self):
        ...
        exp_index2 = MultiIndex.from_arrays([[0, 0, 0, 1, 1, 1], [0, 1, 2, 0,
    1, 2]])
    ...
    def test_concat_keys_and_levels(self):
        ...
        result = concat(
            [df, df2, df, df2],
            keys=[("foo", "one"), ("foo", "two"), ("baz", "one"), ("baz",
    "two")],
            levels=levels,
            names=names,
        )
    ...
    def test_concat_keys_levels_no_overlap(self):
        ...
        with pytest.raises(ValueError, match=msg):
            concat([df, df], keys=["one", "two"], levels=[["foo", "bar",
    "baz"]])

        msg = "Key one not in level"
        with pytest.raises(ValueError, match=msg):
            concat([df, df2], keys=["one", "two"], levels=[["foo", "bar",
    "baz"]])
    ...
    def test_crossed_dtypes_weird_corner(self):
        ...
        df = DataFrame(np.random.randn(1, 3), index=["a"])
        df2 = DataFrame(np.random.randn(1, 4), index=["b"])
        result = concat([df, df2], keys=["one", "two"], names=["first",
    "second"])
    ...
    def test_concat_mixed_objs(self):
        ...
        expected = DataFrame(
            np.repeat(arr, 5).reshape(-1, 5), index=index, columns=[0, 0, 1,
    2, 3]
        )
        ...
        s1.name = "foo"
        expected = DataFrame(
            np.repeat(arr, 3).reshape(-1, 3), index=index, columns=["foo", 0,
    0]
        )
    ...
    "bar"]
        ...
        # axis 0
        expected = DataFrame(
            np.tile(arr, 3).reshape(-1, 1), index=index.tolist() * 3,
    columns=[0]
        )
    ...
    def test_concat_keys_with_none(self):
        ...
        result = concat({"a": None, "b": df0, "c": df0[:2], "d": df0[:1], "e":
    df0})
        ...
        expected = concat([df0, df0[:2], df0[:1], df0], keys=["b", "c", "d",
    "e"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_concat_iterables(self):
        ...
        tm.assert_frame_equal(concat(deque((df1, df2)), ignore_index=True),
    expected)
        ...
        tm.assert_frame_equal(concat(CustomIterator1(), ignore_index=True),
    expected)
        ...
        tm.assert_frame_equal(concat(CustomIterator2(), ignore_index=True),
    expected)

    def test_concat_order(self):
        # GH 17344, GH#47331
        dfs = [DataFrame(index=range(3), columns=["a", 1, None])]
        dfs += [DataFrame(index=range(3), columns=[None, 1, "a"]) for _ in
    range(100)]
    ...
    def test_concat_ordered_dict(self):
        ...
        result = concat({"First": Series(range(3)), "Another":
    Series(range(4))})
        tm.assert_series_equal(result, expected)
    ...
    def test_concat_duplicate_indices_raise(self):
        ...
        df1 = DataFrame(np.random.randn(5), index=[0, 1, 2, 3, 3],
    columns=["a"])
        df2 = DataFrame(np.random.randn(5), index=[0, 1, 2, 2, 4],
    columns=["b"])
...
def test_concat_null_object_with_dti():
    ...
    dti = pd.DatetimeIndex(
        ["2021-04-08 21:21:14+00:00"], dtype="datetime64[ns, UTC]", name="Time
    (UTC)"
    )
...
def test_concat_ignore_all_na_object_float(empty_dtype, df_dtype):
    ...
    if df_dtype == "int64":
        ...
        if empty_dtype == "object":
            ...
            df_dtype = "float64"
    expected = DataFrame({"foo": [None, 1, 2], "bar": [None, 1, 2]},
    dtype=df_dtype)
    tm.assert_frame_equal(result, expected)
...
def test_concat_ignore_empty_from_reindex():
    ...
    expected = df1 = DataFrame({"a": [1, 2], "b": [pd.Timestamp("2012-01-01"),
    pd.NaT]})
    tm.assert_frame_equal(result, expected)
