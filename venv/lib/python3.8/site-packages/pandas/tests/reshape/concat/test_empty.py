...
class TestEmptyConcat:
    def test_handle_empty_objects(self, sort):
        ...
        # GH3259
        df = DataFrame(
            {"A": range(10000)}, index=date_range("20130101", periods=10000,
    freq="s")
        )
        empty = DataFrame()
    ...
    def test_concat_empty_series_timelike(self, tz, values):
        ...
        expected = DataFrame(
            {
                0: Series([pd.NaT] * len(values),
    dtype="M8[ns]").dt.tz_localize(tz),
                1: values,
            }
        )
    ...
    def test_concat_empty_series_dtypes(self, left, right, expected):
        warn = None
        if (left is np.bool_ or right is np.bool_) and expected is not
    np.object_:
            warn = FutureWarning
    ...
    @pytest.mark.parametrize("dtype", ["float64", "int8", "uint8", "m8[ns]",
    "M8[ns]"])
    @pytest.mark.parametrize(
        "dtype2",
    ...
    def test_concat_empty_series_dtypes_triple(self):
        ...
        assert (
            concat(
                [Series(dtype="M8[ns]"), Series(dtype=np.bool_),
    ...
    def test_concat_empty_df_object_dtype(self):
        # GH 9149
        df_1 = DataFrame({"Row": [0, 1, 1], "EmptyCol": np.nan, "NumberCol":
    [1, 2, 3]})
