...
class TestDatetimeConcat:
    ...
    def test_concat_datetime_timezone(self):
        ...
        result = concat([df1.resample("H").mean(), df2.resample("H").mean()],
    sort=True)
        expected = DataFrame(
    ...
    def test_concat_NaT_series(self):
        ...
        x = Series(
            date_range("20151124 08:00", "20151124 09:00", freq="1h",
    tz="US/Eastern")
        )
    ...
    def test_concat_NaT_dataframes(self, tz):
        ...
        second = DataFrame(
            [[Timestamp("2015/01/01", tz=tz)], [Timestamp("2016/01/01",
    tz=tz)]],
            index=[2, 3],
        )
        expected = DataFrame(
    ...
    def test_concat_NaT_dataframes_all_NaT_axis_0(self, tz1, tz2, s):
        ...
        # tz-naive
        first = DataFrame([[pd.NaT], [pd.NaT]]).apply(lambda x:
    x.dt.tz_localize(tz1))
...
class TestTimezoneConcat:
    ...
    def test_concat_multiindex_with_tz(self):
        ...
        exp_idx1 = DatetimeIndex(
            ["2014-01-01", "2014-01-02", "2014-01-03"] * 2, tz="US/Pacific",
    name="dt"
        )
        exp_idx2 = Index(["A", "B", "C"] * 2, name="b")
        exp_idx = MultiIndex.from_arrays([exp_idx1, exp_idx2])
        expected = DataFrame(
            {"c": [1, 2, 3] * 2, "d": [4, 5, 6] * 2}, index=exp_idx,
    columns=["c", "d"]
        )
...
def test_concat_multiindex_datetime_nat():
    ...
    expected = DataFrame(
        {"a": [1.0, np.nan], "b": 2}, MultiIndex.from_tuples([(1, pd.NaT), (2,
    pd.NaT)])
    )
    tm.assert_frame_equal(result, expected)
