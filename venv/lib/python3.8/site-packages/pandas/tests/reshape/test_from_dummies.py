...
def test_error_wrong_data_type():
    ...
    with pytest.raises(
        TypeError,
        match=r"Expected 'data' to be a 'DataFrame'; Received 'data' of type:
    list",
    ):
...
def test_error_no_prefix_wrong_default_category_type():
    ...
    with pytest.raises(
        TypeError,
        match=(
            r"Expected 'default_category' to be of type 'None', 'Hashable', or
    'dict'; "
...
def
    test_error_with_prefix_default_category_wrong_type(dummies_with_unassigned):
    with pytest.raises(
        TypeError,
        match=(
            r"Expected 'default_category' to be of type 'None', 'Hashable', or
    'dict'; "
        ...
        from_dummies(dummies_with_unassigned, sep="_", default_category=["x",
    "y"])
...
def test_error_with_prefix_contains_nan(dummies_basic):
    ...
    with pytest.raises(
        ValueError, match=r"Dummy DataFrame contains NA value in column:
    'col2_c'"
    ):
        from_dummies(dummies_basic, sep="_")
...
def test_error_with_prefix_contains_non_dummies(dummies_basic):
    ...
    with pytest.raises(TypeError, match=r"Passed DataFrame contains non-dummy
    data"):
        from_dummies(dummies_basic, sep="_")
...
def test_no_prefix_string_cats_basic():
    dummies = DataFrame({"a": [1, 0, 0, 1], "b": [0, 1, 0, 0], "c": [0, 0, 1,
    0]})
    expected = DataFrame({"": ["a", "b", "c", "a"]})
    result = from_dummies(dummies)
    tm.assert_frame_equal(result, expected)
...
def test_no_prefix_string_cats_basic_mixed_bool_values():
    dummies = DataFrame(
        {"a": [1, 0, 0, 1], "b": [False, True, False, False], "c": [0, 0, 1,
    0]}
    )
    expected = DataFrame({"": ["a", "b", "c", "a"]})
    result = from_dummies(dummies)
...
def test_no_prefix_float_cats_basic():
    dummies = DataFrame(
        {1.0: [1, 0, 0, 0], 25.0: [0, 1, 0, 0], 2.5: [0, 0, 1, 0], 5.84: [0,
    0, 0, 1]}
    )
...
def test_with_prefix_contains_get_dummies_NaN_column():
    ...
    expected = DataFrame({"col1": ["a", "b", "NaN"], "col2": ["NaN", "a",
    "c"]})
    result = from_dummies(dummies, sep="_")
    tm.assert_frame_equal(result, expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            DataFrame({"col1": ["a", "b", [1.25, False]], "col2": [(1, 2),
    "a", "c"]}),
