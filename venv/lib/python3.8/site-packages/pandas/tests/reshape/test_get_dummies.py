...
class TestGetDummies:
    @pytest.fixture
    def df(self):
        return DataFrame({"A": ["a", "b", "a"], "B": ["b", "b", "c"], "C": [1,
    2, 3]})
    ...
    3, 3, 2]}
        ...
        result = get_dummies(s_df, columns=s_df.columns, sparse=sparse,
    dtype=dtype)
        if sparse:
            dtype_name = f"Sparse[{self.effective_dtype(dtype).name},
    {fill_value}]"
        else:
    ...
    def test_get_dummies_include_na(self, sparse, dtype):
        ...
        res_just_na = get_dummies([np.nan], dummy_na=True, sparse=sparse,
    dtype=dtype)
        exp_just_na = DataFrame(
            Series(1, index=[0]), columns=[np.nan],
    dtype=self.effective_dtype(dtype)
        )
    ...
    def test_get_dummies_unicode(self, sparse):
        ...
        exp = DataFrame(
            {"letter_e": [1, 0, 0], f"letter_{eacute}": [0, 1, 1]},
    dtype=np.uint8
        )
        if sparse:
    ...
    def test_dataframe_dummies_all_obj(self, df, sparse):
        ...
        expected = DataFrame(
            {"A_a": [1, 0, 1], "A_b": [0, 1, 0], "B_b": [1, 1, 0], "B_c": [0,
    0, 1]},
            dtype=np.uint8,
        )
        if sparse:
    ...
    def test_dataframe_dummies_prefix_bad_length(self, df, sparse):
        msg = re.escape(
            "Length of 'prefix' (1) did not match the length of the columns
    being "
            "encoded (2)"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_dataframe_dummies_prefix_sep_bad_length(self, df, sparse):
        msg = re.escape(
            "Length of 'prefix_sep' (1) did not match the length of the
    columns being "
            "encoded (2)"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    def test_dataframe_dummies_prefix_dict(self, sparse):
        ...
        df = DataFrame({"C": [1, 2, 3], "A": ["a", "b", "a"], "B": ["b", "b",
    "c"]})
        result = get_dummies(df, prefix=prefixes, sparse=sparse)
        ...
        if sparse:
            expected[columns] = expected[columns].astype(SparseDtype("uint8",
    0))
    ...
    sparse=sparse)
        exp_na = DataFrame({"b": [0, 1, 0], np.nan: [0, 0, 1]},
    dtype=np.uint8).reindex(
            ["b", np.nan], axis=1
        )
        if sparse:
    ...
    def test_dataframe_dummies_drop_first(self, df, sparse):
        ...
        expected = DataFrame({"A_b": [0, 1, 0], "B_c": [0, 0, 1]},
    dtype=np.uint8)
        if sparse:
            expected = expected.apply(SparseArray, fill_value=0)
        tm.assert_frame_equal(result, expected)

    def test_dataframe_dummies_drop_first_with_categorical(self, df, sparse,
    dtype):
        df["cat"] = Categorical(["x", "y", "y"])
        result = get_dummies(df, drop_first=True, sparse=sparse)
        expected = DataFrame(
            {"C": [1, 2, 3], "A_b": [0, 1, 0], "B_c": [0, 0, 1], "cat_y": [0,
    1, 1]}
        )
        cols = ["A_b", "B_c", "cat_y"]
    ...
    dtype=np.uint8)
        ...
        result = get_dummies(data)
        expected = DataFrame(
            [[1, 0], [0, 1], [1, 0]], columns=Categorical(["a", "b"]),
    dtype=np.uint8
        )
        tm.assert_frame_equal(result, expected)
