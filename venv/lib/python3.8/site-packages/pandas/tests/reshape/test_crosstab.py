...
class TestCrosstab:
    ...
    def test_crosstab_pass_values(self):
        ...
        table = crosstab(
            [a, b], c, values, aggfunc=np.sum, rownames=["foo", "bar"],
    colnames=["baz"]
        )
    ...
    dtype=object)
        b = np.array(["one", "one", "two", "one", "two", "two", "two"],
    dtype=object)
        c = np.array(
            ["dull", "dull", "dull", "dull", "dull", "shiny", "shiny"],
    dtype=object
        )
        res = crosstab(a, [b, c], rownames=["a"], colnames=["b", "c"],
    dropna=False)
        m = MultiIndex.from_tuples(
            [("one", "dull"), ("one", "shiny"), ("two", "dull"), ("two",
    "shiny")],
            names=["b", "c"],
        )
    ...
    def test_margin_dropna2(self):
        ...
        df = DataFrame(
            {"a": [1, np.nan, np.nan, np.nan, 2, np.nan], "b": [3, np.nan, 4,
    4, 4, 4]}
        )
        actual = crosstab(df.a, df.b, margins=True, dropna=True)
    ...
    def test_margin_dropna3(self):
        ...
        df = DataFrame(
            {"a": [1, np.nan, np.nan, np.nan, np.nan, 2], "b": [3, 3, 4, 4, 4,
    4]}
        )
        actual = crosstab(df.a, df.b, margins=True, dropna=True)
        expected = DataFrame([[1, 0, 1], [0, 1, 1], [1, 1, 2]])
    ...
    dropna=False
        ...
        actual = crosstab(
            [a, b], c, rownames=["a", "b"], colnames=["c"], margins=True,
    dropna=False
        )
        m = MultiIndex.from_arrays(
    ...
    def test_crosstab_normalize(self):
        # Issue 12578
        df = DataFrame(
            {"a": [1, 2, 2, 2, 2], "b": [3, 3, 4, 4, 4], "c": [1, 1, np.nan,
    1, 1]}
        )

        rindex = Index([1, 2], name="a")
        cindex = Index([3, 4], name="b")
        full_normal = DataFrame([[0.2, 0], [0.2, 0.6]], index=rindex,
    columns=cindex)
        row_normal = DataFrame([[1.0, 0], [0.25, 0.75]], index=rindex,
    columns=cindex)
        col_normal = DataFrame([[0.5, 0], [0.5, 1.0]], index=rindex,
    columns=cindex)

        # Check all normalize args
        tm.assert_frame_equal(crosstab(df.a, df.b, normalize="all"),
    full_normal)
        tm.assert_frame_equal(crosstab(df.a, df.b, normalize=True),
    full_normal)
        tm.assert_frame_equal(crosstab(df.a, df.b, normalize="index"),
    row_normal)
        tm.assert_frame_equal(crosstab(df.a, df.b, normalize="columns"),
    col_normal)
        ...
        tm.assert_frame_equal(
            crosstab(df.a, df.b, normalize=0), crosstab(df.a, df.b,
    normalize="index")
        )
    ...
    1, 1]}
        ...
        # Test arrays
        crosstab(
            [np.array([1, 1, 2, 2]), np.array([1, 2, 1, 2])], np.array([1, 2,
    1, 2])
        )
        ...
        df = DataFrame(
            {"a": [1, 2, 2, 2, 2], "b": [3, 3, 4, 4, 4], "c": [0, 4, np.nan,
    3, 3]}
        )
    ...
    def test_crosstab_with_empties(self, using_array_manager):
        ...
        for i in [True, "index", "columns"]:
            calculated = crosstab(df.a, df.b, values=df.c, aggfunc="count",
    normalize=i)
        ...
        df = DataFrame(
            {
                ...
                "MODEL": ["Sedan", "Sedan", "Electric", "Pickup", "Sedan",
    "Sedan"],
            }
        )
    ...
    def test_crosstab_with_numpy_size(self):
        ...
        expected_index = MultiIndex(
            ...
            codes=[[1, 1, 1, 2, 2, 2, 3, 3, 3, 0], [1, 2, 3, 1, 2, 3, 1, 2, 3,
    0]],
            names=["A", "B"],
        )
        expected_column = Index(["bar", "foo", "All"], dtype="object",
    name="C")
    ...
    def test_crosstab_duplicate_names(self):
        ...
        expected = crosstab(s3, [s1, s2_bar]).rename_axis(columns=mapper,
    axis=1)
    ...
    def test_crosstab_unsorted_order(self):
        ...
        e_columns = MultiIndex.from_tuples([(1, 4), (2, 6), (3, 5)],
    names=["b", "a"])
        expected = DataFrame(
    ...
    "two"],
        ...
        # normalize on index
        result = crosstab(
            [df.A, df.B], df.C, margins=True, margins_name="Sub-Total",
    normalize=0
        )
        expected = DataFrame(
            [[0.5, 0.5], [0.5, 0.5], [0.666667, 0.333333], [0, 1], [0.444444,
    0.555556]]
        )
