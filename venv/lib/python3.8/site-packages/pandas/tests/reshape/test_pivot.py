...
class TestPivotTable:
    ...
    observed=observed
        ...
        expected = self.data.groupby(index +
    [columns])["D"].agg(np.mean).unstack()
        tm.assert_frame_equal(table, expected)

    def test_pivot_table_categorical_observed_equal(self, observed):
        # issue #24923
        df = DataFrame(
            {"col1": list("abcde"), "col2": list("fghij"), "col3": [1, 2, 3,
    4, 5]}
        )

        expected = df.pivot_table(
            index="col1", values="col3", columns="col2", aggfunc=np.sum,
    fill_value=0
        )
    ...
    def test_pivot_table_nocols(self):
        df = DataFrame(
            {"rows": ["a", "b", "c"], "cols": ["x", "y", "z"], "values": [1,
    2, 3]}
        )
        # GH#50538
    ...
    def test_pivot_table_categorical(self):
        ...
        result = pivot_table(df, values="values", index=["A", "B"],
    dropna=True)
    ...
    dropna=dropna)
        ...
        expected_columns = expected_columns.astype(CDT(categories,
    ordered=False))
        ...
        if not dropna:
            ...
            expected =
    expected.reindex(columns=Categorical(categories)).astype("float")
    ...
    def test_pivot_with_interval_index(self, interval_values, dropna):
        ...
        result = df.pivot_table(index="A", values="B", dropna=dropna)
        expected = DataFrame({"B": 1}, index=Index(interval_values.unique(),
    name="A"))
        if not dropna:
    ...
    def test_pivot_with_interval_index_margins(self):
        ...
        expected = Series(
            [3, 7, 10],
            index=Index([pd.Interval(0, 1), pd.Interval(1, 2), "All"],
    name="C"),
            name="All",
            dtype=np.intp,
        )
    ...
    def test_pass_array(self):
        result = self.data.pivot_table("D", index=self.data.A,
    columns=self.data.C)
    ...
    def test_pivot_dtypes(self):
        ...
        z = pivot_table(
            f, values="v", index=["a"], columns=["i"], fill_value=0,
    aggfunc=np.sum
        )
        result = z.dtypes
        expected = Series([np.dtype("int64")] * 2, index=Index(list("ab"),
    name="i"))
        tm.assert_series_equal(result, expected)
    ...
    aggfunc=np.mean
        ...
        result = z.dtypes
        expected = Series([np.dtype("float64")] * 2, index=Index(list("ab"),
    name="i"))
        tm.assert_series_equal(result, expected)
    ...
    def test_pivot_no_values(self):
        ...
        idx = pd.DatetimeIndex(
            ["2011-01-01", "2011-02-01", "2011-01-02", "2011-01-01",
    "2011-01-02"]
        )
        ...
        exp = DataFrame([[2.5, 4.0], [2.0, np.nan]], index=[1, 2],
    columns=exp_columns)
        ...
        res = df.pivot_table(index=df.index.month, columns=Grouper(key="dt",
    freq="M"))
        exp_columns = MultiIndex.from_tuples([("A",
    pd.Timestamp("2011-01-31"))])
        ...
        exp = DataFrame(
            [3], index=pd.DatetimeIndex(["2011-12-31"], freq="A"),
    columns=exp_columns
        )
        tm.assert_frame_equal(res, exp)
    ...
    def test_pivot_multi_values(self):
        result = pivot_table(
            self.data, values=["D", "E"], index="A", columns=["B", "C"],
    fill_value=0
        )
        expected = pivot_table(
            self.data.drop(["F"], axis=1), index="A", columns=["B", "C"],
    fill_value=0
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_multi_functions(self):
        f = lambda func: pivot_table(
            self.data, values=["D", "E"], index=["A", "B"], columns="C",
    aggfunc=func
        )
    ...
    def test_pivot_index_with_nan(self, method):
        ...
        tm.assert_frame_equal(result, expected)
        tm.assert_frame_equal(df.pivot(index="b", columns="a", values="c"),
    expected.T)
    ...
    @pytest.mark.filterwarnings("ignore:Timestamp.freq is
    ...
    def test_pivot_with_tz(self, method):
        ...
        exp_col2 = pd.DatetimeIndex(
            ["2014/01/01 09:00", "2014/01/02 09:00"] * 2, name="dt2",
    tz="Asia/Tokyo"
        )
        ...
        expected = DataFrame(
            ...
            index=pd.DatetimeIndex(
                ["2013/01/01 09:00", "2013/01/02 09:00"], name="dt1",
    tz="US/Pacific"
            ),
            columns=exp_col,
        )
    ...
    tz="US/Pacific"
            ...
            columns=pd.DatetimeIndex(
                ["2014/01/01 09:00", "2014/01/02 09:00"], name="dt2",
    tz="Asia/Tokyo"
            ),
        )
    ...
    def test_pivot_periods(self, method):
        ...
        exp_col1 = Index(["data1", "data1", "data2", "data2"])
        exp_col2 = pd.PeriodIndex(["2013-01", "2013-02"] * 2, name="p2",
    freq="M")
        exp_col = MultiIndex.from_arrays([exp_col1, exp_col2])
        expected = DataFrame(
            [[0, 2, 0, 2], [1, 3, 1, 3]],
            index=pd.PeriodIndex(["2013-01-01", "2013-01-02"], name="p1",
    freq="D"),
            columns=exp_col,
        )
        if method:
        ...
        expected = DataFrame(
            ...
            index=pd.PeriodIndex(["2013-01-01", "2013-01-02"], name="p1",
    freq="D"),
            columns=pd.PeriodIndex(["2013-01", "2013-02"], name="p2",
    freq="M"),
        )
        if method:
    ...
    def test_pivot_periods_with_margins(self):
        ...
        expected = DataFrame(
            ...
            columns=Index([pd.Period("2019Q1"), pd.Period("2019Q2"), "All"],
    name="b"),
        )

        result = df.pivot_table(index="a", columns="b", values="x",
    margins=True)
    ...
    def test_pivot_columns_none_raise_error(self):
        # GH 30924
        df = DataFrame({"col1": ["a", "b", "c"], "col2": [1, 2, 3], "col3":
    [1, 2, 3]})
    ...
    @pytest.mark.xfail(
        reason="MultiIndexed unstack with tuple names fails with KeyError
    GH#19966"
    )
    @pytest.mark.parametrize("method", [True, False])
    ...
    def _check_output(
        self, result, values_col, index=["A", "B"], columns=["C"],
    margins_col="All"
    ):
    ...
    def test_margins(self):
        ...
        result = self.data.pivot_table(
            values="D", index=["A", "B"], columns="C", margins=True,
    aggfunc=np.mean
        )
    ...
    def test_margin_with_only_columns_defined(
        ...
        df = DataFrame(
            {
                "A": ["foo", "foo", "foo", "foo", "foo", "bar", "bar", "bar",
    "bar"],
    ...
    "two"],
        ...
        if aggfunc == "sum":
            ...
            msg = "The operation <function mean.*failed"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            result = df.pivot_table(columns=columns, margins=True,
    aggfunc=aggfunc)
        expected = DataFrame(values, index=Index(["D", "E"]),
    columns=expected_columns)
    ...
    def test_margins_dtype(self):
        ...
        df = self.data.copy()
        df[["D", "E", "F"]] = np.arange(len(df) * 3).reshape(len(df),
    3).astype("i8")
    ...
    def test_pivot_table_multiindex_only(self, cols):
        # GH 17038
        df2 = DataFrame({cols[0]: [1, 2, 3], cols[1]: [1, 2, 3], "v": [4, 5,
    6]})

        result = df2.pivot_table(values="v", columns=cols)
        expected = DataFrame(
            [[4, 5, 6]],
            columns=MultiIndex.from_tuples([(1, 1), (2, 2), (3, 3)],
    names=cols),
            index=Index(["v"]),
        )
    ...
    def test_pivot_table_retains_tz(self):
        ...
        df = DataFrame({"A": np.random.randn(3), "B": np.random.randn(3), "C":
    dti})
    ...
    def test_pivot_integer_columns(self):
        ...
        table2 = df2.pivot_table(values="4", index=["0", "1", "3"],
    columns=["2"])
    ...
    def test_margins_no_values_two_row_two_cols(self):
        ...
        # are multi-indexed
        self.data["D"] = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j",
    "k"]
    ...
    @pytest.mark.parametrize("margin_name", ["foo", "one", 666, None, ["a",
    "b"]])
    ...
    def test_pivot_timegrouper_double(self):
        ...
        expected = DataFrame(
            np.array(
                [3, np.nan, 6, np.nan, 1, np.nan, 9, np.nan, 9, np.nan,
    np.nan, 3]
            ).reshape(6, 2),
            index=idx,
            columns=["A", "B"],
        ...
        result = pivot_table(
            df,
            index=[Grouper(freq="M", key="Date"), Grouper(freq="M",
    key="PayDay")],
            columns=["Branch"],
            values="Quantity",
            aggfunc=np.sum,
        ...
        result = pivot_table(
            df,
            index=["Branch"],
            columns=[Grouper(freq="M", key="Date"), Grouper(freq="M",
    key="PayDay")],
            values="Quantity",
            aggfunc=np.sum,
        )
    ...
    def test_pivot_datetime_tz(self):
        ...
        exp_idx = pd.DatetimeIndex(
            ["2011-07-19 07:00:00", "2011-07-19 08:00:00", "2011-07-19
    09:00:00"],
            tz="US/Pacific",
            name="dt1",
        )
        ...
        exp_col = MultiIndex.from_arrays([exp_col1, exp_col2])
        expected = DataFrame([[0, 3], [1, 4], [2, 5]], index=exp_idx,
    columns=exp_col)
        result = pivot_table(df, index=["dt1"], columns=["label"],
    values=["value1"])
        tm.assert_frame_equal(result, expected)

        exp_col1 = Index(["sum", "sum", "sum", "sum", "mean", "mean", "mean",
    "mean"])
    ...
    def test_pivot_dtaccessor(self):
        ...
        result = pivot_table(
            df, index=df["dt2"].dt.month, columns=df["dt1"].dt.hour,
    values="value1"
        )
        ...
        expected = DataFrame(
            np.array([[0, 3, 1, 4, 2, 5]], dtype="int64"), index=[2013],
    columns=exp_col
        )
        tm.assert_frame_equal(result, expected)
        ...
        expected = DataFrame(
            np.array(
                [[0, 3, 1, np.nan, 2, np.nan], [np.nan, np.nan, np.nan, 4,
    np.nan, 5]]
            ),
            index=["X", "Y"],
            columns=exp_col,
    ...
    def test_pivot_table_margins_name_with_aggfunc_list(self):
        ...
        ix = Index(["bacon", "cheese", margins_name], dtype="object",
    name="item")
        tups = [
            ("mean", "cost", "M"),
            ("mean", "cost", "T"),
    ...
    def test_pivot_with_categorical(self, observed, ordered):
        ...
        df = DataFrame(
            {
                "In": Categorical(idx, categories=["low", "high"],
    ordered=ordered),
                "Col": Categorical(col, categories=["A", "B"],
    ordered=ordered),
                "Val": range(1, 6),
            }
        )
        ...
        expected_cols = pd.CategoricalIndex(["A", "B"], ordered=ordered,
    name="Col")

        expected = DataFrame(data=[[2.0, np.nan], [np.nan, 3.0]],
    columns=expected_cols)
        expected.index = Index(
            Categorical(["low", "high"], categories=["low", "high"],
    ordered=ordered),
            name="In",
        )
    ...
    def test_pivot_table_not_series(self):
        ...
        df = DataFrame({"col1": [3, 4, 5], "col2": ["C", "D", "E"], "col3":
    [1, 3, 9]})

        result = df.pivot_table("col1", index=["col3", "col2"], aggfunc=np.sum)
        m = MultiIndex.from_arrays([[1, 3, 9], ["C", "D", "E"]],
    names=["col3", "col2"])
        expected = DataFrame([3, 4, 5], index=m, columns=["col1"])

        tm.assert_frame_equal(result, expected)

        result = df.pivot_table("col1", index="col3", columns="col2",
    aggfunc=np.sum)
        expected = DataFrame(
        ...
        expected = DataFrame([3, 4, 5], index=Index([1, 3, 9], name="col3"),
    columns=m)
    ...
    def test_pivot_string_as_func(self):
        ...
        mi = MultiIndex(
            levels=[["C"], ["one", "two"]], codes=[[0, 0], [0, 1]],
    names=[None, "B"]
        )
        expected = DataFrame(
            {("C", "one"): {"bar": 15, "foo": 13}, ("C", "two"): {"bar": 7,
    "foo": 20}},
            columns=mi,
        ).rename_axis("A")
        tm.assert_frame_equal(result, expected)
    ...
    def test_pivot_number_of_levels_larger_than_int32(self, monkeypatch):
        ...
        with monkeypatch.context() as m:
            ...
            df = DataFrame(
                {"ind1": np.arange(2**16), "ind2": np.arange(2**16), "count":
    0}
            )

            msg = "The following operation may generate"
            with tm.assert_produces_warning(PerformanceWarning, match=msg):
                with pytest.raises(Exception, match="Don't compute final
    result."):
                    df.pivot_table(
                        index="ind1", columns="ind2", values="count",
    aggfunc="count"
                    )
    ...
    def test_pivot_table_aggfunc_dropna(self, dropna):
        ...
        result = pivot_table(
            df, columns="fruit", aggfunc=[ret_sum, ret_none, ret_one],
    dropna=dropna
        )
    ...
    def test_pivot_table_aggfunc_scalar_dropna(self, dropna):
        # GH 22159
        df = DataFrame(
            {"A": ["one", "two", "one"], "x": [3, np.nan, 2], "y": [1, np.nan,
    np.nan]}
        )
    ...
    def test_pivot_table_empty_aggfunc(self):
        ...
        result = df.pivot_table(index="A", columns="D", values="id",
    aggfunc=np.size)
    ...
    def test_pivot_table_no_column_raises(self):
        ...
        foo = DataFrame({"X": [0, 0, 1, 1], "Y": [0, 1, 0, 1], "Z": [10, 20,
    30, 40]})
    ...
    "two"],
        ...
        index = MultiIndex.from_tuples(
            [("bar", "large"), ("bar", "small"), ("foo", "large"), ("foo",
    "small")],
            names=["A", "C"],
        )
        vals = np.array(
    ...
    def test_pivot_table_sort_false(self):
        ...
        result = df.pivot_table(
            index=["a", "col"], columns="year", values="num", aggfunc="sum",
    sort=False
        )
        expected = DataFrame(
    ...
    def test_pivot_table_sort_false_with_multiple_values(self):
        ...
        result = df.pivot_table(
            index=["lastname", "firstname"], values=["height", "age"],
    sort=False
        )
        expected = DataFrame(
            [[173, 47], [182, 33]],
    ...
    def test_pivot_table_with_margins_and_numeric_columns(self):
        # GH 26568
        df = DataFrame([["a", "x", 1], ["a", "y", 2], ["b", "y", 3], ["b",
    "z", 4]])
        df.columns = [10, 20, 30]

        result = df.pivot_table(
            index=10, columns=20, values=30, aggfunc="sum", fill_value=0,
    margins=True
        )
    ...
    def test_pivot_ea_dtype_dropna(self, dropna):
        # GH#47477
        df = DataFrame({"x": "a", "y": "b", "age": Series([20, 40],
    dtype="Int64")})
        result = df.pivot_table(
    ...
    "two"],
        ...
        result = pivot_table(
            df, values="D", index=["A", "B"], columns=[(7, "seven")],
    aggfunc=np.sum
        )
        expected = DataFrame(
