...
def dfs_for_indicator():
    df1 = DataFrame({"col1": [0, 1], "col_conflict": [1, 2], "col_left": ["a",
    "b"]})
    df2 = DataFrame(
        {
            "col1": [1, 2, 3, 4, 5],
...
class TestMerge:
    def setup_method(self):
        ...
        self.left = DataFrame(
            {"key": ["a", "b", "c", "d", "e", "e", "a"], "v1":
    np.random.randn(7)}
        )
        self.right = DataFrame({"v2": np.random.randn(4)}, index=["d", "b",
    "c", "a"])
    ...
    def test_merge_non_string_columns(self):
        ...
        left = DataFrame(
            {0: [1, 0, 1, 0], 1: [0, 1, 0, 0], 2: [0, 0, 2, 0], 3: [1, 0, 0,
    3]}
        )
    ...
    np.random.randn(7)}
        ...
        right = DataFrame({"v2": np.random.randn(4)}, index=["d", "b", "c",
    "a"])
    ...
    sort=False
        ...
        merged2 = merge(
            right, left, right_on="key", left_index=True, how="right",
    sort=False
        )
    ...
    def test_merge_misspecified(self):
        ...
        with pytest.raises(ValueError, match=msg):
            merge(self.df, self.df2, left_on=["key1"], right_on=["key1",
    "key2"])
    ...
    def test_merge_different_column_key_names(self):
        left = DataFrame({"lkey": ["foo", "bar", "baz", "foo"], "value": [1,
    2, 3, 4]})
        right = DataFrame({"rkey": ["foo", "bar", "qux", "foo"], "value": [5,
    6, 7, 8]})
        ...
        exp = Series(["bar", "baz", "foo", "foo", "foo", "foo", np.nan],
    name="lkey")
        tm.assert_series_equal(merged["lkey"], exp)

        exp = Series(["bar", np.nan, "foo", "foo", "foo", "foo", "qux"],
    name="rkey")
    ...
    columns=["value", "key"]
        ...
        right = DataFrame({"key": [1, 1, 2, 3, 4, 5], "rvalue":
    list(range(6))})
    ...
    def test_merge_non_unique_indexes(self):
        ...
        df2 = DataFrame(
            {"y": ["c", "d", "e", "f", "g", "h"]}, index=[dt3, dt3, dt2, dt2,
    dt, dt]
        )
        _check_merge(df1, df2)
    ...
    def test_merge_left_empty_right_notempty(self):
        # GH 10824
        left = DataFrame(columns=["a", "b", "c"])
        right = DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]], columns=["x",
    "y", "z"])
    ...
    def test_merge_left_notempty_right_empty(self):
        # GH 10824
        left = DataFrame([[1, 2, 3], [4, 5, 6], [7, 8, 9]], columns=["a", "b",
    "c"])
        right = DataFrame(columns=["x", "y", "z"])
    ...
    def test_join_append_timedeltas(self, using_array_manager):
        ...
        expected = DataFrame(
            {
                "d": [datetime(2013, 11, 5, 5, 56), datetime(2013, 11, 5, 5,
    56)],
    ...
    def test_other_timedelta_unit(self, unit):
        ...
        exp = DataFrame(
            {"entity_id": [101, 102], "days": np.array(["nat", "nat"],
    dtype=dtype)},
            columns=["entity_id", "days"],
        )
        tm.assert_frame_equal(result, exp)
    ...
    def test_merge_datetime64tz_with_dst_transition(self):
        # GH 18885
        df1 = DataFrame(
            pd.date_range("2017-10-29 01:00", periods=4, freq="H",
    tz="Europe/Madrid"),
            columns=["date"],
        )
        df1["value"] = 1
    ...
    def test_merge_on_periods(self):
        left = DataFrame(
            {"key": pd.period_range("20151010", periods=2, freq="D"), "value":
    [1, 2]}
        )
        right = DataFrame(
            {
    ...
    def test_merge_period_values(self):
        left = DataFrame(
            {"key": [1, 2], "value": pd.period_range("20151010", periods=2,
    freq="D")}
        )
        right = DataFrame(
            {"key": [2, 3], "value": pd.period_range("20151011", periods=2,
    freq="D")}
        )
    ...
    indicator=True)
        ...
        df_badcolumn = DataFrame({"col1": [1, 2], "custom_column_name": [2,
    2]})
        ...
        with pytest.raises(ValueError, match=msg):
            df1.merge(
                df_badcolumn, on="col1", how="outer",
    indicator="custom_column_name"
            )
    ...
    def test_merge_indicator_multiple_columns(self):
        ...
        test5 = merge(df3, df4, on=["col1", "col2"], how="outer",
    indicator=True)
        tm.assert_frame_equal(test5, hand_coded_result)
        test5 = df3.merge(df4, on=["col1", "col2"], how="outer",
    indicator=True)
        tm.assert_frame_equal(test5, hand_coded_result)

    def test_validation(self):
        left = DataFrame(
            {"a": ["a", "b", "c", "d"], "b": ["cat", "dog", "weasel",
    "horse"]},
            index=range(4),
        )
    ...
    validate="one_to_one"
        ...
        # Dups on right
        right_w_dups = concat([right, DataFrame({"a": ["e"], "c": ["moo"]},
    index=[4])])
        merge(
            left,
            right_w_dups,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                DatetimeIndex(
                    ["2001-01-01", "2002-02-02", "2003-03-03", pd.NaT, pd.NaT,
    pd.NaT]
                ),
            ),
            (Float64Index([1, 2, 3]), Float64Index([1, 2, 3, None, None,
    None])),
    ...
    freq="D"),
                PeriodIndex(
                    ["2001-01-01", "2001-01-02", "2001-01-03", pd.NaT, pd.NaT,
    pd.NaT],
                    freq="D",
                ),
            ),
    ...
    def test_merge_preserves_row_order(self, how):
        ...
        right_df = DataFrame({"animal": ["quetzal", "pig"], "max_speed": [80,
    11]})
        result = left_df.merge(right_df, on=["animal", "max_speed"], how=how)
        if how == "right":
            expected = DataFrame({"animal": ["quetzal", "pig"], "max_speed":
    [80, 11]})
        else:
            expected = DataFrame({"animal": ["dog", "pig"], "max_speed": [40,
    11]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_merge_take_missing_values_from_index_of_other_dtype(self):
        ...
        right = DataFrame({"b": [1, 2, 3]}, index=CategoricalIndex(["a", "b",
    "c"]))
        result = left.merge(right, left_on="key", right_index=True,
    how="right")
        expected = DataFrame(
    ...
    def test_merge_readonly(self):
        ...
        data1 = DataFrame(
            np.arange(20).reshape((4, 5)) + 1, columns=["a", "b", "c", "d",
    "e"]
        )
        data2 = DataFrame(
...
class TestMergeDtypes:
    ...
    @pytest.mark.parametrize("d1", [np.int64, np.int32, np.int16, np.int8,
    np.uint8])
    @pytest.mark.parametrize("d2", [np.int64, np.float64, np.float32,
    np.float16])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ([1, 2, 3], [1.0, 2.0, 3.0], {"X": [1, 2, 3], "Y": [1.0, 2.0,
    3.0]}),
    ...
    def test_merge_key_dtype_cast(self):
        # GH 17044
        df1 = DataFrame({"key": [1.0, 2.0], "v1": [10, 20]}, columns=["key",
    "v1"])
    ...
    def test_merge_EA_dtype(self, any_numeric_ea_dtype, how, expected_data):
        ...
        expected = DataFrame(expected_data, columns=["id"],
    ...
    def test_merge_ea_with_string(self, join_type, string_dtype):
        ...
        df2_copy = df2.copy()
        merged = merge(left=df1, right=df2, on=[("lvl0", "lvl1-a")],
    how=join_type)
    ...
    def test_merge_empty(self, left_empty, how, exp):
        ...
        if exp == "left":
            expected = DataFrame({"A": [2, 1], "B": [3, 4], "C": [np.nan,
    np.nan]})
        elif exp == "right":
            ...
            expected = DataFrame(columns=["A_x", "B", "A_y", "C"],
    dtype="int64")
...
class TestMergeCategorical:
    def test_identical(self, left):
        ...
        expected = Series(
            [CategoricalDtype(categories=["foo", "bar"]), np.dtype("O"),
    np.dtype("O")],
            index=["X", "Y_x", "Y_y"],
        )
    ...
    def tests_merge_categorical_unordered_equal(self):
        ...
        df1 = DataFrame(
            {
                "Foo": Categorical(["A", "B", "C"], categories=["A", "B",
    "C"]),
                "Left": ["A0", "B0", "C0"],
            }
        )
        ...
        df2 = DataFrame(
            {
                "Foo": Categorical(["C", "B", "A"], categories=["C", "B",
    "A"]),
                "Right": ["C1", "B1", "A1"],
            }
        )
    ...
    def test_other_columns(self, left, right):
        ...
        assert
    left.X.values._categories_match_up_to_permutation(merged.X.values)
        assert
    right.Z.values._categories_match_up_to_permutation(merged.Z.values)
    ...
    def test_dtype_on_merged_different(self, change, join_type, left, right):
        ...
        # assert not
    left.X.values._categories_match_up_to_permutation(right.X.values)

        merged = merge(left, right, on="X", how=join_type)

        result = merged.dtypes.sort_index()
        expected = Series(
            [np.dtype("O"), np.dtype("O"), np.dtype("int64")], index=["X",
    "Y", "Z"]
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_merging_with_bool_or_int_cateorical_column(
        ...
        expected = DataFrame({"id": [2, 4], "cat": expected_categories, "num":
    [1, 9]})
        expected["cat"] = expected["cat"].astype(CDT(categories,
    ordered=ordered))
        tm.assert_frame_equal(expected, result)
...
class TestMergeOnIndexes:
    @pytest.mark.parametrize(
        "how, sort, expected",
        [
            ("inner", False, DataFrame({"a": [20, 10], "b": [200, 100]},
    index=[2, 1])),
            ("inner", True, DataFrame({"a": [10, 20], "b": [100, 200]},
    index=[1, 2])),
            (
                "left",
                False,
                DataFrame({"a": [20, 10, 0], "b": [200, 100, np.nan]},
    index=[2, 1, 0]),
            ),
            (
                "left",
                True,
                DataFrame({"a": [0, 10, 20], "b": [np.nan, 100, 200]},
    index=[0, 1, 2]),
            ),
            (
                "right",
                False,
                DataFrame(
                    {"a": [np.nan, 10, 20], "b": [300, 100, 200]}, index=[3,
    1, 2]
                ),
            ),
            (
                ...
                True,
                DataFrame(
                    {"a": [10, 20, np.nan], "b": [100, 200, 300]}, index=[1,
    2, 3]
                ),
            ),
            (
    ...
    def test_merge_on_indexes(self, left_df, right_df, how, sort, expected):
        result = merge(
            left_df, right_df, left_index=True, right_index=True, how=how,
    sort=sort
        )
        tm.assert_frame_equal(result, expected)
...
def test_merge_index_types(index):
    ...
    expected = DataFrame({"left_data": [1, 2], "right_data": [1.0, 2.0]},
    index=index)
    tm.assert_frame_equal(result, expected)
...
def test_merge_series(on, left_on, right_on, left_index, right_index, nm):
    ...
    a = DataFrame(
        {"A": [1, 2, 3, 4]},
        index=MultiIndex.from_product([["a", "b"], [0, 1]], names=["outer",
    "inner"]),
    )
    b = Series(
        [1, 2, 3, 4],
        index=MultiIndex.from_product([["a", "b"], [1, 2]], names=["outer",
    "inner"]),
        name=nm,
    )
    expected = DataFrame(
        {"A": [2, 4], "B": [1, 3]},
        index=MultiIndex.from_product([["a", "b"], [1]], names=["outer",
    "inner"]),
    )
    if nm is not None:
        result = merge(
...
@pytest.mark.parametrize(
    ...
    [
        (
            "right",
            DataFrame(
                {"A": [100, 200, 300], "B1": [60, 70, np.nan], "B2": [600,
    700, 800]}
            ),
        ),
        (
...
@pytest.mark.parametrize("suffixes", [{"left", "right"}, {"left": 0, "right":
    0}])
def test_merge_suffix_warns(suffixes):
    ...
    with tm.assert_produces_warning(FutureWarning):
        merge(a, b, left_index=True, right_index=True, suffixes={"left",
    "right"})
...
@pytest.mark.parametrize(
    ...
    [
        ("a", "a", ("a", "b", "c"), r"too many values to unpack \(expected
    2\)"),
        ("a", "a", tuple("a"), r"not enough values to unpack \(expected 2, got
    1\)"),
    ],
)
...
def test_merge_equal_cat_dtypes(cat_dtype, reverse):
    ...
    df1 = DataFrame(
        {"foo": Series(["a", "b", "c"]).astype(cat_dtypes["one"]), "left": [1,
    2, 3]}
    ).set_index("foo")
    ...
    df2 = DataFrame(
        {"foo": Series(data_foo).astype(cat_dtypes[cat_dtype]), "right":
    data_right}
    ).set_index("foo")
...
def test_merge_equal_cat_dtypes2():
    ...
    expected = DataFrame(
        {"left": [1, 2], "right": [3, 2], "foo": Series(["a",
    "b"]).astype(cat_dtype)}
    ).set_index("foo")
...
def test_merge_multiindex_columns():
    ...
    # The original cause of this issue come from a bug lexsort_depth and is
    tested in
    # test_lexsort_depth

    letters = ["a", "b", "c", "d"]
    numbers = ["1", "2", "3"]
    index = MultiIndex.from_product((letters, numbers), names=["outer",
    "inner"])
...
def test_categorical_non_unique_monotonic(n_categories):
    ...
    df1 = DataFrame(range(n_categories + 1), columns=["value"],
    index=left_index)
    df2 = DataFrame(
        [[6]],
        columns=["value"],
...
def test_merge_join_cols_error_reporting_duplicates(func, kwargs, err_msg):
    ...
    msg = rf'Can only pass argument "{err_msg[0]}" OR "{err_msg[1]}" not
    both\.'
    with pytest.raises(MergeError, match=msg):
...
def test_merge_result_empty_index_and_on():
    ...
    expected = DataFrame({"a": [], "b": []}, dtype=np.int64).set_index(["a",
    "b"])
...
def test_merge_suffixes_produce_dup_columns_warns():
    ...
    expected = DataFrame(
        [[1, 1, 2, 2], [2, 1, 2, 2], [3, 1, 2, 2]], columns=["a", "b_x",
    "b_x", "b_y"]
    )
    with tm.assert_produces_warning(FutureWarning):
        result = merge(left, right, on="a")
...
def test_merge_duplicate_columns_with_suffix_causing_another_duplicate():
    ...
    left = DataFrame([[1, 1, 1, 1], [2, 2, 2, 2]], columns=["a", "b", "b",
    "b_x"])
    right = DataFrame({"a": [1, 3], "b": 2})
    with tm.assert_produces_warning(FutureWarning):
        result = merge(left, right, on="a")
    expected = DataFrame([[1, 1, 1, 1, 2]], columns=["a", "b_x", "b_x", "b_x",
    "b_y"])
    tm.assert_frame_equal(result, expected)
...
def test_merge_string_float_column_result():
    ...
    expected = DataFrame(
        [[9, 10, 1, 2], [11, 12, 3, 4]], columns=pd.Index(["x", "y", "a",
    114.0])
    )
    tm.assert_frame_equal(result, expected)
