...
class TestAsOfMerge:
    ...
    def allow_exact_matches_and_tolerance(self, datapath):
        return self.read_data(datapath,
    ...
    7]})
        ...
        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val": [1, 3,
    7]}
        )
    ...
    7]})
        ...
        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val": [1, 6,
    np.nan]}
        )
    ...
    def test_on_and_index_left_on(self, trades, quotes):
        ...
        with pytest.raises(MergeError, match=msg):
            merge_asof(
                trades, quotes, left_on="price", left_index=True,
    right_index=True
            )
    ...
    def test_on_and_index_right_on(self, trades, quotes):
        ...
        with pytest.raises(MergeError, match=msg):
            merge_asof(
                trades, quotes, right_on="bid", left_index=True,
    right_index=True
            )
    ...
    def test_multiby(self):
        ...
        expected = pd.DataFrame(
            ...
            columns=["time", "ticker", "exch", "price", "quantity", "bid",
    "ask"],
        )
    ...
    def test_valid_join_keys(self, trades, quotes):
        ...
        with pytest.raises(MergeError, match=msg):
            merge_asof(trades, quotes, left_on="time", right_on="bid",
    by="ticker")

        with pytest.raises(MergeError, match="can only asof on a key for
    left"):
    ...
    def test_tolerance_forward(self):
        ...
        left = pd.DataFrame({"a": [1, 5, 10], "left_val": ["a", "b", "c"]})
        right = pd.DataFrame({"a": [1, 2, 3, 7, 11], "right_val": [1, 2, 3, 7,
    11]})

        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val": [1,
    np.nan, 11]}
        )
    ...
    def test_tolerance_tz(self):
        ...
        result = merge_asof(left, right, on="date", tolerance=Timedelta("1
    day"))
    ...
    def test_tolerance_float(self):
        # GH22981
        left = pd.DataFrame({"a": [1.1, 3.5, 10.9], "left_val": ["a", "b",
    "c"]})
        right = pd.DataFrame(
            {"a": [1.0, 2.5, 3.3, 7.5, 11.5], "right_val": [1.0, 2.5, 3.3,
    7.5, 11.5]}
        )
    ...
    11]})
        ...
        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val": [2, 7,
    11]}
        )
    ...
    11]})
        ...
        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val": [2, 3,
    11]}
        )
    ...
    def test_allow_exact_matches_and_tolerance2(self):
        # GH 13695
        df1 = pd.DataFrame(
            {"time": to_datetime(["2016-07-15 13:30:00.030"]), "username":
    ["bob"]}
        )
        df2 = pd.DataFrame(
            {
    ...
    def test_allow_exact_matches_and_tolerance_forward(self):
        ...
        left = pd.DataFrame({"a": [1, 5, 10], "left_val": ["a", "b", "c"]})
        right = pd.DataFrame({"a": [1, 3, 4, 6, 11], "right_val": [1, 3, 4, 6,
    11]})

        expected = pd.DataFrame(
            {"a": [1, 5, 10], "left_val": ["a", "b", "c"], "right_val":
    [np.nan, 6, 11]}
        )
    ...
    def test_allow_exact_matches_and_tolerance_nearest(self):
        ...
        left = pd.DataFrame({"a": [1, 5, 10], "left_val": ["a", "b", "c"]})
        right = pd.DataFrame({"a": [1, 3, 4, 6, 11], "right_val": [1, 3, 4, 7,
    11]})
    ...
    def test_on_specialized_type_by_int(self, any_real_numpy_dtype):
        ...
        df2 = pd.DataFrame(
            {"value": [0, 80, 120, 125], "key": [1, 2, 2, 3], "result":
    list("xyzw")},
        ...
        df2 = pd.DataFrame(
            {
                ...
                "mpv": [0.0001, 0.01, 0.05, 0.0001, 0.01, 0.1, 0.0001, 0.25,
    1.0],
            },
        ...
        expected = pd.DataFrame(
            {
                ...
                "mpv": [0.0001, 0.0001, 0.01, 0.25, 0.01, 0.01, 0.05, 0.1,
    0.25],
            },
    ...
    def test_merge_datatype_error_raises(self):
        msg = r"Incompatible merge dtype, .*, both sides must have numeric
    dtype"

        left = pd.DataFrame({"left_val": [1, 5, 10], "a": ["a", "b", "c"]})
        right = pd.DataFrame({"right_val": [1, 2, 3, 6, 7], "a": [1, 2, 3, 6,
    7]})
    ...
    @pytest.mark.parametrize(
        "func", [lambda x: x, lambda x: to_datetime(x)], ids=["numeric",
    "datetime"]
    )
    ...
    def test_by_nullable(self, any_numeric_ea_dtype):
        ...
        result = merge_asof(left, right, by=["by_col1", "by_col2"],
    on="on_col")
        expected = pd.DataFrame(
            {
                "by_col1": arr,
    ...
    def test_timedelta_tolerance_nearest(self):
        ...
        result = merge_asof(
            left, right, on="time", tolerance=Timedelta("1ms"),
    direction="nearest"
        )
    ...
    def test_int_type_tolerance(self, any_int_numpy_dtype):
        ...
        expected = pd.DataFrame(
            {"a": [0, 10, 20], "left_val": [1, 2, 3], "right_val": [np.nan,
    1.0, 2.0]}
        )
    ...
    def test_merge_index_column_tz(self):
        ...
        left = pd.DataFrame([0.9, 0.8, 0.7, 0.6], columns=["xyz"],
    index=index[1:])
    ...
    def test_left_index_right_index_tolerance(self):
        ...
        dr1 = pd.date_range(start="1/1/2020", end="1/20/2020", freq="2D") +
    Timedelta(
            seconds=0.4
        )
...
@pytest.mark.parametrize(
    "data",
    [["2019-06-01 00:09:12", "2019-06-01 00:10:29"], [1.0, "2019-06-01
    00:10:29"]],
)
...
def test_merge_asof_non_numerical_dtype_object():
    # GH#29130
    left = pd.DataFrame({"a": ["12", "13", "15"], "left_val1": ["a", "b",
    "c"]})
...
def test_merge_asof_index_behavior(kwargs):
    ...
    left = pd.DataFrame({"left": ["a", "b", "c"], "left_time": [1, 4, 10]},
    index=index)
...
def test_merge_asof_numeri_column_in_index():
    ...
    expected = pd.DataFrame({"a": [1, 2, 3], "b": [10, 11, 12], "c": [20, 21,
    22]})
    tm.assert_frame_equal(result, expected)
...
def test_merge_asof_numeri_column_in_multiindex():
    ...
    left = pd.DataFrame(
        {"b": [10, 11, 12]},
        index=pd.MultiIndex.from_arrays([[1, 2, 3], ["a", "b", "c"]],
    names=["a", "z"]),
    )
    right = pd.DataFrame(
        {"c": [20, 21, 22]},
        index=pd.MultiIndex.from_arrays([[1, 2, 3], ["x", "y", "z"]],
    names=["a", "y"]),
    )
...
def test_merge_asof_numeri_column_in_index_object_dtype():
    # GH#34488
    left = pd.DataFrame({"b": [10, 11, 12]}, index=Index(["1", "2", "3"],
    name="a"))
    right = pd.DataFrame({"c": [20, 21, 22]}, index=Index(["m", "n", "o"],
    name="a"))
...
def test_merge_asof_array_as_on():
    ...
    right = pd.DataFrame(
        {
            "a": [2, 6],
            "ts": [pd.Timestamp("2021/01/01 00:37"), pd.Timestamp("2021/01/01
    01:40")],
        }
    )
    ...
    expected = pd.DataFrame({"b": [4, 8, 7], "a": [np.nan, 2, 6], "ts":
    ts_merge})
    tm.assert_frame_equal(result, expected)
