...
def left():
    ...
    "snap"]
    key2 = ["two", "one", "three", "one", "two", "one", "two", "two", "three",
    "one"]
...
class TestMergeMulti:
    ...
    def test_left_join_multi_index(self, sort):
        ...
        def bind_cols(df):
            ...
            return f(df["1st"]) + f(df["3rd"]) * 1e2 + df["2nd"].fillna(0) *
    1e4
        ...
        def run_asserts(left, right, sort):
            ...
            if sort:
                tm.assert_frame_equal(res, res.sort_values(icols,
    kind="mergesort"))

            out = merge(left, right.reset_index(), on=icols, sort=sort,
    how="left")
        ...
        left = DataFrame(np.random.choice(lc, (5000, 2)), columns=["1st",
    "3rd"])
    ...
    def test_merge_multiple_cols_with_mixed_cols_index(self):
        ...
        s = Series(
            range(6),
            MultiIndex.from_product([["A", "B"], [1, 2, 3]], names=["lev1",
    "lev2"]),
            name="Amount",
        )
        df = DataFrame({"lev1": list("AAABBB"), "lev2": [1, 2, 3, 1, 2, 3],
    "col": 0})
    ...
    def test_compress_group_combinations(self):
        ...
        df = DataFrame({"key1": key1, "key2": key2, "value1":
    np.random.randn(20000)})

        df2 = DataFrame(
            {"key1": key1[::2], "key2": key2[::2], "value2":
    ...
    def test_merge_datetime_index(self, klass):
        # see gh-19038
        df = DataFrame(
            [1, 2, 3], ["2016-01-01", "2017-01-01", "2018-01-01"],
    columns=["a"]
        )
    ...
    def test_merge_datetime_multi_index_empty_df(self, merge_type):
        ...
        left = DataFrame(
            ...
            index=MultiIndex.from_tuples(
                [[Timestamp("1950-01-01"), "A"], [Timestamp("1950-01-02"),
    "B"]],
                names=["date", "panel"],
            ),
        )
    ...
    columns=["state"]
        ...
        if merge_type == "left":
            ...
            results_merge = right.merge(left, how="right", on=["date",
    "panel"])
    ...
    def test_join_multi_levels_merge_equivalence(self, portfolio, household,
    expected):
        portfolio = portfolio.copy()
        household = household.copy()
    ...
    def test_join_multi_levels_invalid(self, portfolio, household):
        ...
        with pytest.raises(ValueError, match="columns overlap but no suffix
    specified"):
...
class TestJoinMultiMulti:
    ...
    def test_join_multi_wrong_order(self):
        ...
        midx3 = MultiIndex.from_tuples([(4, 1), (3, 2), (3, 1)], names=["b",
    "a"])
