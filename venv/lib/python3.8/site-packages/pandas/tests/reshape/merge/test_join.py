...
class TestJoin:
    ...
    def test_join_on_fails_with_different_right_index(self):
        df = DataFrame(
            {"a": np.random.choice(["m", "f"], size=3), "b":
    np.random.randn(3)}
        )
        df2 = DataFrame(
            {"a": np.random.choice(["m", "f"], size=10), "b":
    np.random.randn(10)},
            index=tm.makeCustomIndex(10, 2),
        )
        msg = r'len\(left_on\) must equal the number of levels in the index of
    "right"'
        with pytest.raises(ValueError, match=msg):
    ...
    def test_join_multiindex(self):
        ...
        df1 = DataFrame(data=np.random.randn(6), index=index1, columns=["var
    X"])
        df2 = DataFrame(data=np.random.randn(6), index=index2, columns=["var
    Y"])
    ...
    "snap"]
        ...
        to_join = DataFrame(
            np.random.randn(10, 3), index=index, columns=["j_one", "j_two",
    "j_three"]
        )
    ...
    def test_join_float64_float32(self):
        ...
        a = DataFrame(np.random.randn(10, 2), columns=["a", "b"],
    dtype=np.float64)
    ...
    def test_join_many_non_unique_index(self):
        ...
        df3 = DataFrame({"a": [1, 1, 1], "b": [1, 1, 2], "e": [1000, 2000,
    3000]})
        idf1 = df1.set_index(["a", "b"])
        idf2 = df2.set_index(["a", "b"])
        idf3 = df3.set_index(["a", "b"])
    ...
    "three"],
        ...
        s = Series(
            np.repeat(np.arange(8), 2), index=np.repeat(np.arange(8), 2),
    name="TEST"
        )
    ...
    def test_join_sort(self):
        left = DataFrame({"key": ["foo", "bar", "baz", "foo"], "value": [1, 2,
    3, 4]})
        right = DataFrame({"value2": ["a", "b", "c"]}, index=["bar", "baz",
    "foo"])
    ...
    def test_mixed_type_join_with_suffix(self):
        # GH #916
        df = DataFrame(np.random.randn(20, 6), columns=["a", "b", "c", "d",
    "e", "f"])
        df.insert(0, "id", 0)
        df.insert(5, "dt", "foo")
    ...
    def test_join_many(self):
        ...
        df_list = [df[["a", "b"]][:-2], df[["c", "d"]][2:], df[["e",
    "f"]][1:9]]
    ...
    def test_join_dups(self):
        ...
        df = concat(
            [
                DataFrame(np.random.randn(10, 4), columns=["A", "A", "B",
    "B"]),
                DataFrame(
    ...
    columns=["A", "C"]
        ...
        expected.columns = ["x_x", "y_x", "x_y", "y_y", "x_x", "y_x", "x_y",
    "y_y"]
...
def _check_join(left, right, result, join_col, how="left", lsuffix="_x",
    rsuffix="_y"):
...
def _restrict_to_columns(group, columns, suffix):
    found = [
        c for c in group.columns if c in columns or c.replace(suffix, "") in
    columns
    ]
...
def test_join_inner_multiindex_deterministic_order():
    ...
    expected = DataFrame(
        {"e": [5], "f": [6]},
        index=MultiIndex.from_tuples([(2, 1, 4, 3)], names=("b", "a", "d",
    "c")),
    )
    tm.assert_frame_equal(result, expected)
...
def test_join_cross(input_col, output_cols):
    ...
    expected = DataFrame({output_cols[0]: [1, 1, 3, 3], output_cols[1]: [3, 4,
    3, 4]})
    tm.assert_frame_equal(result, expected)
...
def test_join_multiindex_one_level(join_type):
    ...
    right = DataFrame(data={"d": 4}, index=MultiIndex.from_tuples([(2,)],
    names=("b",)))
...
def test_join_empty(left_empty, how, exp):
    ...
    if exp == "left":
        ...
        expected = DataFrame(index=Index([]), columns=["B", "C"],
    dtype="int64")
        if how != "cross":
