...
@pytest.fixture(
    autouse=True, scope="module", params=[0, 1000000], ids=["numexpr",
    "python"]
)
...
class TestFrameComparisons:
    ...
    def test_df_float_none_comparison(self):
        df = DataFrame(np.random.randn(8, 3), index=range(8), columns=["A",
    "B", "C"])
...
class TestFrameFlexArithmetic:
    ...
    masked_arith_op
        ...
        arr = np.arange(_MIN_ELEMENTS + 100).reshape(_MIN_ELEMENTS // 100 + 1,
    -1) * 100
        df = DataFrame(arr)
        df["C"] = 1.0
    ...
    def test_frame_multiindex_operations(self):
        ...
        series = Series(
            [0.4],
            index=MultiIndex.from_product([["b"], ["a"]], names=["mod",
    "scen"]),
        )
    ...
    def test_frame_multiindex_operations_series_index_to_frame_index(self):
        ...
        df = DataFrame(
            {2010: [1], 2020: [3]},
            index=MultiIndex.from_product([["a"], ["b"]], names=["scen",
    "mod"]),
        )
    ...
    def test_frame_multiindex_operations_no_align(self):
        ...
        series = Series(
            [0.4],
            index=MultiIndex.from_product([["c"], ["a"]], names=["mod",
    "scen"]),
        )
...
class TestFrameArithmetic:
    ...
    @pytest.mark.parametrize(
        "values", [[1, 2], (1, 2), np.array([1, 2]), range(1, 3), deque([1,
    2])]
    )
    ...
    def test_arith_non_pandas_object(self):
        ...
        added = DataFrame((df.values.T + val1).T, index=df.index,
    columns=df.columns)
    ...
    all_arithmetic_operators):
        ...
        expected = DataFrame([[getattr(n, op)(num) for n in data]],
    columns=ind)
        tm.assert_frame_equal(result, expected)
    ...
    def test_frame_with_frame_reindex(self):
        ...
        expected = DataFrame(
            {"foo": [pd.Timedelta(0), pd.Timedelta(0)], "bar": [np.nan,
    np.nan]},
            columns=["bar", "foo"],
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_arithmetic_midx_cols_different_dtypes(self):
        ...
        midx2 = MultiIndex.from_arrays([Series([1, 2], dtype="Int8"),
    Series([3, 4])])
    ...
    def test_arithmetic_midx_cols_different_dtypes_different_order(self):
        # GH#49769
        midx = MultiIndex.from_arrays([Series([1, 2]), Series([3, 4])])
        midx2 = MultiIndex.from_arrays([Series([2, 1], dtype="Int8"),
    Series([4, 3])])
        left = DataFrame([[1, 2], [3, 4]], columns=midx)
        right = DataFrame([[1, 2], [3, 4]], columns=midx2)
        result = left - right
...
class TestFrameArithmeticUnsorted:
    ...
    def test_combine_series(self, float_frame, mixed_float_frame,
    mixed_int_frame):
        ...
        _check_mixed_int(
            added, dtype={"A": "int64", "B": "float64", "C": "int64", "D":
    "int64"}
        )
        added = mixed_int_frame + (100 * series).astype("int32")
        _check_mixed_int(
            added, dtype={"A": "int32", "B": "float64", "C": "int32", "D":
    "int64"}
        )
    ...
    def test_combineFunc(self, float_frame, mixed_float_frame):
        ...
        for c, s in result.items():
            tm.assert_numpy_array_equal(s.values, mixed_float_frame[c].values
    * 2)
        _check_mixed_float(result, dtype={"C": None})
    ...
    @pytest.mark.parametrize(
        "func",
        [operator.eq, operator.ne, operator.lt, operator.gt, operator.ge,
    operator.le],
    )
    def test_comparisons(self, simple_frame, float_frame, func):
        ...
        result = func(df1, df2)
        tm.assert_numpy_array_equal(result.values, func(df1.values,
    df2.values))
        ...
        tm.assert_numpy_array_equal(result3.values, func(float_frame.values,
    0))
    ...
    @pytest.mark.parametrize("val", [[1, 2], (1, 2), np.array([1, 2]),
    range(1, 3)])
    ...
    def test_alignment_non_pandas_index_columns(self):
        ...
        val = np.zeros((3, 3, 3))
        msg = re.escape(
            "Unable to coerce to Series/DataFrame, dimension must be <= 2: (3,
    3, 3)"
        )
        with pytest.raises(ValueError, match=msg):
            align(df, val, "index")
...
def test_arith_reindex_with_duplicates():
    ...
    result = df1 + df2
    expected = DataFrame([[np.nan, 0, 0]], columns=["first", "second",
    "second"])
    tm.assert_frame_equal(result, expected)


@pytest.mark.parametrize("to_add", [[Series([1, 1])], [Series([1, 1]),
    Series([1, 1])]])
...
def test_frame_op_subclass_nonclass_constructor():
    ...
    result = sdf * 2
    expected = SubclassedDataFrame("some_data", {"A": [2, 4, 6], "B": [8, 10,
    12]})
    tm.assert_frame_equal(result, expected)
