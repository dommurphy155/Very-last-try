...
class TestDataFrameEval:
    ...
    def test_eval_resolvers_as_list(self):
        ...
        dict1 = {"a": 1}
        dict2 = {"b": 2}
        assert df.eval("a + b", resolvers=[dict1, dict2]) == dict1["a"] +
    dict2["b"]
        assert pd.eval("a + b", resolvers=[dict1, dict2]) == dict1["a"] +
    dict2["b"]
...
class TestDataFrameQueryWithMultiIndex:
    def test_query_with_named_multiindex(self, parser, engine):
        ...
        ind = Series(
            df.index.get_level_values("color").values, index=index,
    name="color"
        )
    ...
    def test_query_with_partially_named_multiindex(self, parser, engine):
        ...
        ind = Series(
            df.index.get_level_values("rating").values, index=index,
    name="rating"
        )
        exp = df[ind == 1]
...
class TestDataFrameQueryNumExprPandas:
    ...
    def test_date_index_query_with_NaT_duplicates(self):
        ...
        res = df.query("dates1 < 20130101 < dates3", engine=engine,
    parser=parser)
        expec = df[(df.index.to_series() < "20130101") & ("20130101" <
    df.dates3)]
        tm.assert_frame_equal(res, expec)
    ...
    def test_date_query_with_non_date(self):
        ...
        n = 10
        df = DataFrame(
            {"dates": date_range("1/1/2012", periods=n), "nondate":
    np.arange(n)}
        )
    ...
    def test_query_scope(self):
        ...
        # no column named 'c'
        with pytest.raises(UndefinedVariableError, match="name 'c' is not
    defined"):
            df.query("@a > b > c", engine=engine, parser=parser)
    ...
    def test_query_doesnt_pickup_local(self):
        ...
        with pytest.raises(UndefinedVariableError, match="name 'sin' is not
    defined"):
    ...
    parser=parser)
        ...
        result = pd.eval(
            "df[df > 0 and df2 > 0 and df[df > 0] > 0]", engine=engine,
    parser=parser
        )
    ...
    def test_nested_raises_on_local_self_reference(self):
        ...
        with pytest.raises(UndefinedVariableError, match="name 'df' is not
    defined"):
    ...
    @pytest.mark.parametrize("op, f", [["==", operator.eq], ["!=",
    operator.ne]])
    ...
    def test_method_calls_in_query(self):
        ...
        df = DataFrame(
            {
                "a": np.where(np.random.rand(n) < 0.5, np.nan,
    np.random.randn(n)),
                "b": np.random.randn(n),
            }
        )
...
class TestDataFrameQueryNumExprPython(TestDataFrameQueryNumExprPandas):
    ...
    def test_date_query_no_attribute_access(self):
        ...
        df["dates3"] = date_range("1/1/2014", periods=5)
        res = df.query(
            "(dates1 < 20130101) & (20130101 < dates3)", engine=engine,
    parser=parser
        )
        expec = df[(df.dates1 < "20130101") & ("20130101" < df.dates3)]
        tm.assert_frame_equal(res, expec)
    ...
    def test_date_index_query(self):
        ...
        assert return_value is None
        res = df.query(
            "(index < 20130101) & (20130101 < dates3)", engine=engine,
    parser=parser
        )
        expec = df[(df.index < "20130101") & ("20130101" < df.dates3)]
        tm.assert_frame_equal(res, expec)
    ...
    parser=parser)
        ...
        expected = df[(df > 0) & (df2 > 0) & (df[df > 0] > 0)]
        result = pd.eval(
            "df[(df > 0) & (df2 > 0) & (df[df > 0] > 0)]", engine=engine,
    parser=parser
        )
        tm.assert_frame_equal(expected, result)
...
class TestDataFrameQueryBacktickQuoting:
    ...
    def test_call_non_named_expression(self, df):
        ...
        with pytest.raises(TypeError, match="Only named functions are
    supported"):
            df.eval("@funcs[0]()")
