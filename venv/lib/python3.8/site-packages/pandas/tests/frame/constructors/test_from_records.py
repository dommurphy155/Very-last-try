...
class TestFromRecords:
    ...
    def test_from_records_sequencelike(self):
        ...
        for dtype, b in blocks.items():
            ...
            dtypes.extend([(c, np.dtype(dtype).descr[0][1]) for c in
    b.columns])
    ...
    results.append(DataFrame.from_records(asdict).reindex(columns=df.columns))
        results.append(
            DataFrame.from_records(asdict,
    columns=columns).reindex(columns=df.columns)
        )
        results.append(
            DataFrame.from_records(asdict2,
    columns=columns).reindex(columns=df.columns)
        )
    ...
    def test_from_records_bad_index_column(self):
        ...
        msg = "|".join(
            [
                r"Length of values \(10\) does not match length of index
    \(1\)",
            ]
        )
    ...
    def test_from_records_len0_with_columns(self):
        # GH#2633
        result = DataFrame.from_records([], index="foo", columns=["foo",
    "bar"])
        expected = Index(["bar"])
    ...
    def test_frame_from_records_utc(self):
        rec = {"datum": 1.5, "begin_time": datetime(2006, 4, 27,
    tzinfo=pytz.utc)}
    ...
    def test_from_records_iterator(self):
        arr = np.array(
            [(1.0, 1.0, 2, 2), (3.0, 3.0, 4, 4), (5.0, 5.0, 6, 6), (7.0, 7.0,
    8, 8)],
            dtype=[
                ("x", np.float64),
                ("u", np.float32),
    ...
    def test_from_records_tuples_generator(self):
        ...
        columns = [
            [i[j] for i in tuple_generator(10)] for j in
    range(len(columns_names))
        ]
        data = {"Integer": columns[0], "String": columns[1], "Float":
    columns[2]}
    ...
    def test_from_records_lists_generator(self):
        ...
        columns_names = ["Integer", "String", "Float"]
        columns = [
            [i[j] for i in list_generator(10)] for j in
    range(len(columns_names))
        ]
        data = {"Integer": columns[0], "String": columns[1], "Float":
    columns[2]}
    ...
    def test_from_records_duplicates(self):
        result = DataFrame.from_records([(1, 2, 3), (4, 5, 6)], columns=["a",
    "b", "a"])
    ...
    def test_from_records_misc_brokenness(self):
        ...
        expected = Series(
            [np.dtype("datetime64[ns]"), np.dtype("object")], index=["date",
    "test"]
        )
        ...
        result = df2_obj.dtypes
        expected = Series(
            [np.dtype("datetime64[ns]"), np.dtype("int64")], index=["date",
    "test"]
        )
        tm.assert_series_equal(result, expected)
