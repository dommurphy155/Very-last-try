...
class TestDataFrameConstructors:
    ...
    def test_constructor_dict_with_tzaware_scalar(self):
        # GH#42505
        dt = Timestamp("2019-11-03
    01:00:00-0700").tz_convert("America/Los_Angeles")
    ...
    def test_array_of_dt64_nat_with_td64dtype_raises(self, frame_or_series):
        ...
        msg = "|".join(
            [
                ...
                "Invalid type for timedelta scalar: <class
    'numpy.datetime64'>",
            ]
        )
    ...
    def test_datetimelike_values_with_object_dtype(self, kind,
    frame_or_series):
    ...
    def test_emptylike_constructor(self, emptylike, expected_index,
    expected_columns):
    ...
    def test_constructor_cast_failure(self):
        msg = "either all columns will be cast to that dtype, or a TypeError
    will"
    ...
    def test_constructor_dtype_nocast_view_2d_array(
        ...
        if not using_array_manager and not using_copy_on_write:
            ...
            # INFO(ArrayManager) DataFrame(ndarray) doesn't necessarily
    preserve
    ...
    def test_constructor_mixed_dtypes(self, typ, ad):
        if typ == "int":
            ...
            arrays = [np.array(np.random.randint(10, size=10), dtype=d) for d
    in dtypes]
    ...
    def test_constructor_subclass_dict(self, dict_subclass):
        ...
        df = DataFrame(data)
        refdf = DataFrame({col: dict(val.items()) for col, val in
    data.items()})
    ...
    def test_constructor_dict_cast(self):
        # cast float tests
        test_data = {"A": {"1": 1, "2": 2}, "B": {"1": "1", "2": "2", "3":
    "3"}}
    ...
    def test_constructor_dict_of_generators(self):
        # GH 26349
        data = {"a": (i for i in (range(3))), "b": (i for i in
    reversed(range(3)))}
        result = DataFrame(data)
        expected = DataFrame({"a": [0, 1, 2], "b": [2, 1, 0]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_dict_multiindex(self):
        ...
        df = DataFrame(d)
        expected = DataFrame(
            [x[1] for x in _d], index=MultiIndex.from_tuples([x[0] for x in
    _d])
        ).T
        ...
        d["z"] = {"y": 123.0, ("i", "i"): 111, ("i", "j"): 111, ("j", "i"):
    111}
        _d.insert(0, ("z", d["z"]))
        expected = DataFrame(
            [x[1] for x in _d], index=Index([x[0] for x in _d],
    tupleize_cols=False)
        ).T
    ...
    def test_constructor_dict_datetime64_index(self):
        ...
        def create_data(constructor):
            return {i: {constructor(s): 2 * i} for i, s in
    enumerate(dates_as_str)}
    ...
    def test_constructor_period_dict(self):
        ...
        # list of periods
        df = DataFrame({"a": a.astype(object).tolist(), "b":
    b.astype(object).tolist()})
        assert df["a"].dtype == a.dtype
        assert df["b"].dtype == b.dtype
    ...
    def _check_basic_constructor(self, empty):
        ...
        if empty is np.ones:
            ...
            # i.e. ma.masked_all
            # Since we have NaNs, refuse to cast to int dtype, which would
    take NaN
            #  to meaningless integers.  This matches Series behavior. 
    GH#26919
    ...
    mask=[False] * 5
        ...
        expected = DataFrame(np.zeros((5, 2), dtype=int), columns=["date",
    "price"])
        tm.assert_frame_equal(result, expected)

        # GH#40363 check that the alternative suggested in the deprecation
        #  warning behaves as expected
        alt = DataFrame({name: data[name] for name in data.dtype.names},
    dtype=int)
    ...
    def test_constructor_mrecarray(self):
        ...
        assert_fr_equal = functools.partial(
            tm.assert_frame_equal, check_index_type=True,
    check_column_type=True
        ...
        for name, arr in arrays[:]:
            arrays.append(
                ("masked1_" + name, np.ma.masked_array(arr, mask=[False,
    True]))
            )
        arrays.append(("masked_all", np.ma.masked_all((2,))))
        arrays.append(("masked_none", np.ma.masked_array([1.0, 2.5],
    mask=False)))
        ...
        for comb in itertools.combinations(arrays, 3):
            ...
            # fill the comb
            comb = {k: (v.filled() if hasattr(v, "filled") else v) for k, v in
    comb}
    ...
    def test_constructor_scalar_inference(self):
        data = {"int": 1, "bool": True, "float": 3.0, "complex": 4j, "object":
    "foo"}
    ...
    def test_constructor_mixed_dict_and_Series(self):
        ...
        # this is OK though
        result = DataFrame({"A": ["a", "b"], "B": Series(["a", "b"],
    index=["a", "b"])})
    ...
    def test_constructor_list_of_dataclasses_with_varying_types(self):
        ...
        expected = DataFrame(
            {"x": [0, np.nan], "y": [3, 3], "x0": [np.nan, 1], "x1": [np.nan,
    3]}
        )
        result = DataFrame(data)
        tm.assert_frame_equal(result, expected)
    ...
    def test_constructor_list_of_dict_order(self):
        ...
        data = [
            ...
            {"Second": 6, "First": 3, "Fourth": 12, "Third": 9, "YYY": 14,
    "XXX": 13},
        ]
        expected = DataFrame(
            {
    ...
    def test_constructor_index_names(self, name_in1, name_in2, name_in3,
    name_out):
        # GH13475
        indices = [
        ...
        series = {
            c: Series([0, 1, 2], index=i) for i, c in zip(indices, ["x", "y",
    "z"])
        }
        result = DataFrame(series)
    ...
    columns=["A", "B"]
        ...
        with pytest.raises(ValueError, match=msg):
            DataFrame({"A": float_frame["A"], "B":
    list(float_frame["B"])[:-2]})
    ...
    nullable_string_dtype):
        # GH 34915
        expected = DataFrame(index=[], columns=["c1"],
    ...
    def test_constructor_with_datetimes6(self):
        ...
        expected = DataFrame({"a": i.to_series().reset_index(drop=True), "b":
    i_no_tz})
        tm.assert_frame_equal(df, expected)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame([[0, 1, 2, 3], [4, 5, 6, 7]], columns=[np.nan, 1.1, 2.2,
    np.nan]),
            DataFrame(
                [[0.0, 1, 2, 3.0], [4, 5, 6, 7]], columns=[np.nan, 1.1, 2.2,
    np.nan]
            ),
            DataFrame([[0.0, 1, 2, 3.0], [4, 5, 6, 7]], columns=[np.nan, 1, 2,
    2]),
        ],
    )
    ...
    def test_constructor_categorical_series(self):
        ...
        expected = Series(
            Categorical(values=[np.nan, np.nan, np.nan], categories=["a", "b",
    "c"])
        )
    ...
    def test_check_dtype_empty_numeric_column(self, dtype):
        ...
        # Empty DataFrame is generated via dictionary data with
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            PeriodArray(pd.period_range(start="1/1/2017", end="1/1/2018",
    freq="M")),
        ],
    )
    ...
    def test_datetime_date_tuple_columns_from_dict(self):
        ...
        v = date.today()
        tup = v, v
        result = DataFrame({tup: Series(range(3), index=range(3))},
    columns=[tup])
    ...
    "unknown"]))
        ...
        expected = DataFrame(
            np.array([[39, 6, 4, np.nan, np.nan], [152.0, 242.0, 150.0, 2.0,
    2.0]]),
    ...
    def test_with_mismatched_index_length_raises(self):
        ...
        msg = "Shape of passed values|Passed arrays should have the same
    length"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_dict_nocopy(
        ...
        def check_views(c_only: bool = False):
            ...
            #  df._mgr.arrays corresponds to df["c"], we just check that
    exactly
        ...
        # FIXME(GH#35417): until GH#35417, iloc.setitem into EA values does
    not preserve
        ...
        if copy:
            ...
            assert c[0] == c_orig[0]  # i.e. df.iloc[0, 2]=45 did *not* update
    c
    ...
    def test_nested_list_columns(self):
        ...
        expected = DataFrame(
            [[1, 2, 3], [4, 5, 6]],
            columns=MultiIndex.from_tuples([("A", "a"), ("A", "b"), ("A",
    "c")]),
        )
        tm.assert_frame_equal(result, expected)
...
class TestDataFrameConstructorIndexInference:
    def
    test_frame_from_dict_of_series_overlapping_monthly_period_indexes(self):
...
class TestDataFrameConstructorWithDatetimeTZ:
    ...
    def test_constructor_data_aware_dtype_naive(self, tz_aware_fixture, pydt):
        ...
        with tm.assert_produces_warning(FutureWarning):
            result = DataFrame(np.array([ts], dtype=object),
    dtype="datetime64[ns]")
        tm.assert_frame_equal(result, expected)

        with tm.assert_produces_warning(FutureWarning):
            result = DataFrame(ts, index=[0], columns=[0],
    dtype="datetime64[ns]")
        tm.assert_frame_equal(result, expected)
    ...
    def constructor(self, request, frame_or_series):
        ...
        if box is None:
            ...
            if frame_or_series is Series:
                ...
                return lambda x, **kwargs: frame_or_series({"A": x}, **extra,
    **kwargs)
        else:
            if frame_or_series is Series:
    ...
    **kwargs)
    ...
    def test_from_scalar_datetimelike_mismatched(self, constructor, cls):
        ...
        if cls is np.datetime64:
            msg = "|".join(
                [
                    r"dtype datetime64\[ns\] cannot be converted to
    timedelta64\[ns\]",
                    "Cannot cast",
                ]
            )
    ...
    def test_tzaware_data_tznaive_dtype(self, constructor):
        ...
        with tm.assert_produces_warning(FutureWarning, match="Data is
    timezone-aware"):
