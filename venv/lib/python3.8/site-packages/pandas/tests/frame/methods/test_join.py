...
@pytest.mark.parametrize(
    "how, sort, expected",
    [
        ("inner", False, DataFrame({"a": [20, 10], "b": [200, 100]}, index=[2,
    1])),
        ("inner", True, DataFrame({"a": [10, 20], "b": [100, 200]}, index=[1,
    2])),
        (
            "left",
            False,
            DataFrame({"a": [20, 10, 0], "b": [200, 100, np.nan]}, index=[2,
    1, 0]),
        ),
        (
            "left",
            True,
            DataFrame({"a": [0, 10, 20], "b": [np.nan, 100, 200]}, index=[0,
    1, 2]),
        ),
        (
            "right",
            False,
            DataFrame({"a": [np.nan, 10, 20], "b": [300, 100, 200]}, index=[3,
    1, 2]),
        ),
        (
            "right",
            True,
            DataFrame({"a": [10, 20, np.nan], "b": [100, 200, 300]}, index=[1,
    2, 3]),
        ),
        (
            "outer",
...
def test_join_period_index(frame_with_period_index):
    ...
    joined_values = np.concatenate([frame_with_period_index.values] * 2,
    axis=1)
    ...
    expected = DataFrame(
        data=joined_values, columns=joined_cols,
...
class TestDataFrameJoin:
    ...
    def test_join_multiindex_leftright(self):
        ...
        df2 = DataFrame([["a", 10], ["b", 20]], columns=["first",
    "value2"]).set_index(
            ["first"]
        )
        ...
        tm.assert_frame_equal(df2.join(df1, how="right"), exp[["value2",
    "value1"]])
        ...
        tm.assert_frame_equal(df2.join(df1, how="left"), exp[["value2",
    "value1"]])
    ...
    def test_join_multiindex_dates(self):
        ...
        df1_index = MultiIndex.from_tuples([(0, date)], names=["index_0",
    "date"])
        df1 = DataFrame({"col1": [0]}, index=df1_index)
        df2_index = MultiIndex.from_tuples([(0, date)], names=["index_0",
    "date"])
        df2 = DataFrame({"col2": [0]}, index=df2_index)
        df3_index = MultiIndex.from_tuples([(0, date)], names=["index_0",
    "date"])
        df3 = DataFrame({"col3": [0]}, index=df3_index)

        result = df1.join([df2, df3])

        expected_index = MultiIndex.from_tuples([(0, date)], names=["index_0",
    "date"])
        expected = DataFrame(
    ...
    def test_merge_join_different_levels(self):
        ...
        columns = ["a", "b", ("a", ""), ("c", "c1")]
        expected = DataFrame(columns=columns, data=[[1, 11, 0, 44], [0, 22, 1,
    33]])
