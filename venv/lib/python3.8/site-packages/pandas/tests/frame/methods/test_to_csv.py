...
class TestDataFrameToCSV:
    ...
    def test_to_csv_from_csv2(self, float_frame):
        ...
        with tm.ensure_clean("__tmp_to_csv_from_csv2__") as path:
            ...
            # duplicate index
            df = DataFrame(
                np.random.randn(3, 3), index=["a", "a", "b"], columns=["x",
    "y", "z"]
            )
            df.to_csv(path)
            ...
            midx = MultiIndex.from_tuples([("A", 1, 2), ("A", 1, 2), ("B", 1,
    2)])
            df = DataFrame(np.random.randn(3, 3), index=midx, columns=["x",
    "y", "z"])
            ...
            if cols is not None:
                ...
                for c in cols:
                    ...
                    if isinstance(obj_df, Series):
                        ...
                        tm.assert_frame_equal(obj_df, obj_rs,
    check_names=False)
    ...
    def _return_result_expected(
        ...
        type_map = {"i": "i", "f": "f", "s": "O", "u": "O", "dt": "O", "p":
    "O"}
        if r_dtype:
    ...
    @pytest.mark.parametrize(
        "nrows", [2, 10, 99, 100, 101, 102, 198, 199, 200, 201, 202, 249, 250,
    251]
    )
    ...
    @pytest.mark.parametrize(
        "r_idx_type, c_idx_type", [("i", "i"), ("s", "s"), ("s", "dt"), ("p",
    "p")]
    )
    ...
    @pytest.mark.slow
    @pytest.mark.parametrize(
        "nrows", [10, 98, 99, 100, 101, 102, 198, 199, 200, 201, 202, 249,
    250, 251]
    )
    @pytest.mark.parametrize("ncols", [1, 2, 3, 4])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [{"r_idx_nlevels": 2, "c_idx_nlevels": 2}, {"rnlvl": 2, "cnlvl":
    2}],
        ],
    )
    ...
    **func_params)
    ...
    def test_to_csv_mixed(self):
        ...
        df_float = DataFrame(
            np.random.randn(100, 5), dtype="float64",
    ...
    columns=create_cols("bool"))
        ...
        df_dt = DataFrame(
            Timestamp("20010101"), index=df_float.index,
    columns=create_cols("date")
        ...
        with tm.ensure_clean() as filename:
            df.to_csv(filename)
            rs = read_csv(
                filename, index_col=0, dtype=dtypes,
    ...
    def test_to_csv_dups_cols(self):
        ...
        df_dt = DataFrame(Timestamp("20010101"), index=df_float.index,
    columns=range(3))
        df = pd.concat(
    ...
    def test_to_csv_index_no_leading_comma(self):
        df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]}, index=["one", "two",
    "three"])
    ...
    "gb2312"),
            (
                DataFrame(5 * [[123, "Γειά σου", "Κόσμε"]], columns=["X", "Y",
    "Z"]),
                "cp737",
            ),
        ],
    ...
    def test_to_csv_compression(self, df, encoding, compression):
        ...
        with tm.ensure_clean() as filename:
            ...
            result = read_csv(
                filename, compression=compression, index_col=0,
    encoding=encoding
            )
            tm.assert_frame_equal(df, result)
            ...
            with tm.decompress_file(filename, compression) as fh:
                tm.assert_frame_equal(df, read_csv(fh, index_col=0,
    encoding=encoding))
    ...
    def test_to_csv_with_dst_transitions_with_pickle(self):
        # GH11619
        idx = date_range("2015-01-01", "2015-12-31", freq="H",
    tz="Europe/Paris")
    ...
    def test_multi_index_header(self):
        # see gh-5539
        columns = MultiIndex.from_tuples([("a", 1), ("a", 2), ("b", 1), ("b",
    2)])
