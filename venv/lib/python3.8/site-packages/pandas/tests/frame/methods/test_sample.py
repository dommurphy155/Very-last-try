...
class TestSample:
    ...
    def test_sample(self, test, obj):
        ...
        tm.assert_equal(
            obj.sample(n=4, random_state=seed), obj.sample(n=4,
    random_state=seed)
        )
    ...
    def test_sample_requires_positive_n_frac(self, obj):
        with pytest.raises(
            ValueError,
            match="A negative number of rows requested. Please provide `n` >=
    0",
        ):
            obj.sample(n=-3)
        with pytest.raises(
            ValueError,
            match="A negative number of rows requested. Please provide `frac`
    >= 0",
        ):
            obj.sample(frac=-0.3)
    ...
    def test_sample_requires_integer_n(self, obj):
        ...
        with pytest.raises(ValueError, match="Only integers accepted as `n`
    values"):
            obj.sample(n=3.2)
    ...
    def test_sample_invalid_weight_lengths(self, obj):
        ...
        with pytest.raises(ValueError, match="Fewer non-zero entries in p than
    size"):
    ...
    def test_sample_zero_weights(self, obj):
        ...
        zero_weights = [0] * 10
        with pytest.raises(ValueError, match="Invalid weights: weights sum to
    zero"):
    ...
    def test_sample_random_state(self, func_str, arg, frame_or_series):
        ...
        expected = obj.sample(n=3,
    random_state=com.random_state(eval(func_str)(arg)))
...
class TestSampleDataFrame:
    ...
    def test_sample(self):
        ...
        ser = Series(range(10))
        msg = "Strings cannot be passed as weights when sampling from a
    Series."
        with pytest.raises(ValueError, match=msg):
            ser.sample(n=3, weights="weight_column")

        msg = (
            "Strings can only be passed to weights when sampling from rows on
    a "
            "DataFrame"
        )
        with pytest.raises(ValueError, match=msg):
    ...
    df[["col2"]]
        ...
        weight = [0] * 10
        weight[5] = 0.5
        tm.assert_frame_equal(df.sample(n=1, axis="rows", weights=weight),
    df.iloc[5:6])
        tm.assert_frame_equal(
        ...
        ser = Series(range(10))
        with pytest.raises(ValueError, match="No axis named 1 for object type
    Series"):
    ...
    def test_sample_axis1(self):
        ...
        df = DataFrame(
            {"col1": range(10, 20), "col2": range(20, 30), "colString": ["a"]
    * 10}
        )
    ...
    def test_sample_aligns_weights_with_frame(self):
        ...
        df = DataFrame({"col1": [5, 6, 7], "col2": ["a", "b", "c"]}, index=[9,
    5, 3])
