...
class TestDataFrameCov:
    def test_cov(self, float_frame, float_string_frame):
        ...
        result = df.cov()
        expected = DataFrame(
            np.cov(df.values.T).reshape((1, 1)), index=df.columns,
    columns=df.columns
        )
        tm.assert_frame_equal(result, expected)
        df.loc[0] = np.nan
    ...
    def test_cov_numeric_only(self, numeric_only):
        ...
        if numeric_only:
            ...
            with pytest.raises(ValueError, match="could not convert string to
    float"):
...
class TestDataFrameCorr:
    ...
    def test_corr_int_and_boolean(self, meth):
        ...
        expected = DataFrame(np.ones((2, 2)), index=["a", "b"], columns=["a",
    "b"])
    ...
    def test_corr_invalid_method(self):
        ...
        msg = "method must be either 'pearson', 'spearman', 'kendall', or a
    callable, "
    ...
    @pytest.mark.parametrize(
        "other_column",
        [pd.array([1, 2, 3]), np.array([1.0, 2.0, 3.0]), np.array([1.0, 2.0,
    np.nan])],
    )
    ...
    method):
        ...
        expected = DataFrame(np.ones((2, 2)), columns=["a", "b"], index=["a",
    "b"])
        tm.assert_frame_equal(result, expected)
    ...
    def test_calc_corr_small_numbers(self):
        # GH: 37452
        df = DataFrame(
            {"A": [1.0e-20, 2.0e-20, 3.0e-20], "B": [1.0e-20, 2.0e-20,
    3.0e-20]}
        )
        result = df.corr()
...
class TestDataFrameCorrWith:
    def test_corrwith(self, datetime_frame):
        ...
        df2 = DataFrame(np.random.randn(4, 4), index=index[:4],
    columns=columns)
    ...
    def test_corrwith_index_intersection(self):
        ...
        df2 = DataFrame(np.random.random(size=(10, 3)), columns=["a", "b",
    "c"])
