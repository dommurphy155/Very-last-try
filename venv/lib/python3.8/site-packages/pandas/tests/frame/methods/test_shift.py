...
class TestDataFrameShift:
    ...
    def test_shift_mismatched_freq(self, frame_or_series):
        ts = frame_or_series(
            np.random.randn(5), index=date_range("1/1/2000", periods=5,
    freq="H")
        )
    ...
    def test_shift(self, datetime_frame):
        ...
        tm.assert_numpy_array_equal(unshifted_ser.dropna().values,
    ser.values[:-5])
    ...
    def test_shift_by_offset(self, datetime_frame, frame_or_series):
        ...
        if frame_or_series is DataFrame:
            tm.assert_series_equal(obj.xs(d), shifted.xs(shifted_d),
    check_names=False)
        else:
    ...
    @td.skip_array_manager_not_yet_implemented  # TODO(ArrayManager) axis=1
    support
    ...
    def test_shift_dt64values_int_fill_deprecated(self):
        ...
        expected = DataFrame({"A": [pd.Timestamp(0), pd.Timestamp(0)], "B":
    df2["A"]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_shift_axis1_categorical_columns(self):
        ...
        expected = DataFrame(
            {"a": [np.nan, np.nan], "b": [1, 3], "c": [2, 4]}, index=ci[:-1],
    columns=ci
        )
        tm.assert_frame_equal(result, expected)
