...
class TestDataFrameCombineFirst:
    def test_combine_first_mixed(self):
        ...
        exp = DataFrame({"A": list("abab"), "B": [0, 1, 0, 1]}, index=[0, 1,
    5, 6])
    ...
    np.nan, 6.0]}
        ...
        expected = DataFrame({"A": [1, 2, 3, 5, 3, 7.0], "B": [np.nan, 2, 3,
    4, 6, 8]})
    ...
    def test_combine_first_return_obj_type_with_bools(self):
        ...
        df2 = DataFrame([[-42.6, np.nan, True], [-5.0, 1.6, False]], index=[1,
    2])
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1,
    3)],
    ...
    3)],
            ...
            (
                ...
                [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1,
    3)],
                [datetime(2000, 1, 2), datetime(2000, 1, 2), datetime(2000, 1,
    3)],
            ),
            (
                [datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000, 1,
    ...
    def test_combine_first_align_nan(self):
        ...
        exp = DataFrame({"a": [np.nan, np.nan], "b": [4, 5]}, columns=["a",
    "b"])
        tm.assert_frame_equal(res, exp)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (pd.Interval(left=0, right=1), pd.Interval(left=2, right=3,
    closed="left")),
    ],
)
def test_combine_first_timestamp_bug(scalar1, scalar2, nulls_fixture):
    ...
    if is_dtype_equal(common_dtype, "object") or frame.dtypes["b"] ==
    other.dtypes["b"]:
        val = scalar1
    else:
        val = na_value
...
def test_combine_first_timestamp_bug_NaT():
    ...
    result = frame.combine_first(other)
    expected = DataFrame(
        [[pd.NaT, datetime(2020, 1, 1), datetime(2020, 1, 2)]], columns=["a",
    "b", "c"]
    )
...
def test_combine_first_with_nan_multiindex():
    ...
    mi1 = MultiIndex.from_arrays(
        [["b", "b", "c", "a", "b", np.nan], [1, 2, 3, 4, 5, 6]], names=["a",
    "b"]
    )
    ...
    expected = DataFrame(
        {
            ...
            "d": [1.0, 4.0, np.nan, 2.0, 5.0, np.nan, np.nan, 3.0, np.nan,
    6.0, np.nan],
        },
        index=mi_expected,
    )
...
def test_combine_first_duplicates_rows_for_nan_index_values():
    ...
    df1 = DataFrame(
        {"x": [9, 10, 11]},
        index=MultiIndex.from_arrays([[1, 2, 3], [np.nan, 5, 6]], names=["a",
    "b"]),
    )

    df2 = DataFrame(
        {"y": [12, 13, 14]},
        index=MultiIndex.from_arrays([[1, 2, 4], [np.nan, 5, 7]], names=["a",
    "b"]),
    )
