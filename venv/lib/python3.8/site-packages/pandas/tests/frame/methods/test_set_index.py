...
class TestSetIndex:
    ...
    def test_set_index_empty_dataframe(self):
        # GH#38419
        df1 = DataFrame(
            {"a": Series(dtype="datetime64[ns]"), "b": Series(dtype="int64"),
    "c": []}
        )
    ...
    def test_set_index_cast_datetimeindex(self):
        df = DataFrame(
            {
                "A": [datetime(2000, 1, 1) + timedelta(i) for i in
    range(1000)],
    ...
    def test_set_index_dst(self):
        di = date_range("2006-10-29 00:00:00", periods=3, freq="H",
    tz="US/Pacific")
    ...
    index=di).reset_index()
        ...
        exp_index = MultiIndex.from_arrays([di, [0, 1, 2]], names=["index",
    "a"])
    ...
    def test_set_index_cast(self):
        ...
        df = DataFrame(
            {"A": [1.1, 2.2, 3.3], "B": [5.0, 6.1, 7.2]}, index=[2010, 2011,
    2012]
        )
    ...
    @pytest.mark.parametrize("keys", ["A", "C", ["A", "B"], ("tuple", "as",
    "label")])
    @pytest.mark.parametrize("inplace", [True, False])
    @pytest.mark.parametrize("drop", [True, False])
    def test_set_index_drop_inplace(self, frame_of_index_cols, drop, inplace,
    keys):
    ...
    "label")])
    @pytest.mark.parametrize("drop", [True, False])
    def test_set_index_append_to_multiindex(self, frame_of_index_cols, drop,
    keys):
        # append to existing multiindex
        df = frame_of_index_cols.set_index(["D"], drop=drop, append=True)

        keys = keys if isinstance(keys, list) else [keys]
        expected = frame_of_index_cols.set_index(["D"] + keys, drop=drop,
    append=True)
    ...
    @pytest.mark.parametrize(
        "append, index_name", [(True, None), (True, "B"), (True, "test"),
    (False, None)]
    )
    @pytest.mark.parametrize("drop", [True, False])
    ...
    @pytest.mark.parametrize(
        "box", [Series, Index, np.array, list, lambda x:
    MultiIndex.from_arrays([x])]
    )
    @pytest.mark.parametrize(
        "append, index_name",
        [(True, None), (True, "A"), (True, "B"), (True, "test"), (False,
    None)],
    )
    @pytest.mark.parametrize("drop", [True, False])
    ...
    @pytest.mark.parametrize(
        "append, index_name", [(True, None), (True, "A"), (True, "test"),
    (False, None)]
    )
    @pytest.mark.parametrize("drop", [True, False])
    ...
    @pytest.mark.parametrize("drop", [True, False])
    def test_set_index_pass_multiindex(self, frame_of_index_cols, drop,
    append):
        df = frame_of_index_cols
...
class TestSetIndexInvalid:
    ...
    def test_set_index_raise_keys(self, frame_of_index_cols, drop, append):
        ...
        # also within a list
        with pytest.raises(KeyError, match=msg):
            df.set_index(["A", df["A"], tuple(df["A"])], drop=drop,
    append=append)

    @pytest.mark.parametrize("append", [True, False])
    @pytest.mark.parametrize("drop", [True, False])
    @pytest.mark.parametrize("box", [set], ids=["set"])
    def test_set_index_raise_on_type(self, frame_of_index_cols, box, drop,
    append):
        df = frame_of_index_cols
        ...
        with pytest.raises(TypeError, match=msg):
            df.set_index(["A", df["A"], box(df["A"])], drop=drop,
    append=append)
...
class TestSetIndexCustomLabelType:
    def test_set_index_custom_label_type(self):
        ...
        expected = DataFrame({thing1: [0, 1]}, index=Index([2, 3],
    name=thing2))
