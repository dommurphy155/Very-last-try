...
class TestRank:
    ...
    results = {
        "average": np.array([1.5, 5.5, 7.0, 3.5, np.nan, 3.5, 1.5, 8.0,
    np.nan, 5.5]),
    ...
    def test_rank2(self):
        ...
        df = DataFrame({"a": [1e-20, -5, 1e-20 + 1e-40, 10, 1e60, 1e80,
    1e-30]})
    ...
    match="numeric_only=None"):
        ...
        with tm.assert_produces_warning(FutureWarning, match="Dropping of
    nuisance"):
    ...
    @pytest.mark.parametrize(
        "method,exp",
        [
            ("dense", [[1.0, 1.0, 1.0], [1.0, 0.5, 2.0 / 3], [1.0, 0.5, 1.0 /
    3]]),
            (
                "min",
                [
            ...
            (
                "max",
                [[1.0, 1.0, 1.0], [1.0, 2.0 / 3, 2.0 / 3], [1.0, 2.0 / 3, 1.0
    / 3]],
            ),
            (
                "average",
                [[2.0 / 3, 1.0, 1.0], [2.0 / 3, 0.5, 2.0 / 3], [2.0 / 3, 0.5,
    1.0 / 3]],
            ),
            (
                "first",
    ...
    "uint8"),
            ...
            (
                [datetime(2001, 1, 1), datetime(2001, 1, 2), datetime(2001, 1,
    5)],
                "datetime64",
            ),
        ],
    ...
    @pytest.mark.parametrize(
        "data,expected",
        [
            ({"a": [1, 2, "a"], "b": [4, 5, 6]}, DataFrame({"b": [1.0, 2.0,
    3.0]})),
