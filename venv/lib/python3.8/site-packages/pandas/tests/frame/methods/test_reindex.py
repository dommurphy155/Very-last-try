...
class TestReindexSetIndex:
    ...
    def test_dti_set_index_reindex_freq_with_tz(self):
        # GH#11314 with tz
        index = date_range(
            datetime(2015, 10, 1), datetime(2015, 10, 1, 23), freq="H",
    tz="US/Eastern"
        )
        df = DataFrame(np.random.randn(24, 1), columns=["a"], index=index)
        new_index = date_range(
            datetime(2015, 10, 2), datetime(2015, 10, 2, 23), freq="H",
    tz="US/Eastern"
        )
    ...
    index=range(0)).astype(
            {"a": "datetime64[ns]", "b": np.int64, "c": np.float64, "d":
    np.float64}
        )
    ...
    @pytest.mark.parametrize(
        ...
        [["America/Chicago", 2013, 11, 3, 1], ["America/Santiago", 2021, 4, 3,
    23]],
    )
    def test_reindex_timestamp_with_fold(self, timezone, year, month, day,
    hour):
        # see gh-40817
        ...
        df = (
            DataFrame({"index": [transition_1, transition_2], "vals": ["a",
    "b"]})
            .set_index("index")
            .reindex(["1", "2"])
        )
        tm.assert_frame_equal(
            df,
            DataFrame({"index": ["1", "2"], "vals": [None,
    None]}).set_index("index"),
        )
...
class TestDataFrameSelectReindex:
    ...
    def test_reindex_date_fill_value(self):
        ...
        with tm.assert_produces_warning(FutureWarning):
            res = df.reindex(index=range(4), columns=["A", "B", "C"],
    fill_value=fv)

        expected = DataFrame(
            {"A": df["A"].tolist() + [ts], "B": df["B"].tolist() + [ts], "C":
    [ts] * 4}
        )
    ...
    def test_reindex_with_multi_index(self):
        ...
        new_multi_index = MultiIndex.from_product([[0], new_index],
    names=["a", "b"])
        ...
        reindexed_with_backfilling = df.reindex(new_multi_index,
    method="bfill")
    ...
    def test_reindex_methods(self, method, expected_values):
        ...
        switched_method = (
            "pad" if method == "backfill" else "backfill" if method == "pad"
    else method
        )
    ...
    def test_reindex_methods_nearest_special(self):
        ...
        actual = df.reindex(target, method="nearest", tolerance=[0.5, 0.01,
    0.4, 0.1])
        tm.assert_frame_equal(expected, actual)
    ...
    def test_reindex_frame_tz_ffill_bfill(self, frame_or_series, method,
    exp_values):
        # GH#38566
        obj = frame_or_series(
            [0, 1, 2, 3],
            index=date_range("2020-01-01 00:00:00", periods=4, freq="H",
    tz="UTC"),
        )
        new_index = date_range("2020-01-01 00:01:00", periods=4, freq="H",
    tz="UTC")
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                ["1st", "2nd", "3rd"],
                [2, 3, 4, 0, 1, 8, 9, 5, 6, 7, 10, 11, 12, 13, 14, 18, 19, 15,
    16, 17],
            ],
            [
                ["3rd", "2nd", "1st"],
                [0, 1, 2, 3, 4, 10, 11, 12, 5, 6, 7, 8, 9, 15, 16, 17, 18, 19,
    13, 14],
            ],
            [["2nd", "3rd"], [0, 1, 5, 6, 7, 10, 11, 12, 18, 19, 15, 16, 17]],
            [["3rd", "1st"], [0, 1, 2, 3, 4, 10, 11, 12, 8, 9, 15, 16, 17, 13,
    14]],
        ],
    )
    ...
    def test_non_monotonic_reindex_methods(self):
        ...
        df_rev = DataFrame(data, index=dr[[3, 4, 5] + [0, 1, 2]],
    columns=list("A"))
    ...
    def test_reindex_sparse(self):
        ...
        df = DataFrame(
            {"A": [0, 1], "B": pd.array([0, 1], dtype=pd.SparseDtype("int64",
    0))}
        )
        ...
        expected = DataFrame(
            {
                "A": [0.0, np.nan],
                "B": pd.array([0.0, np.nan], dtype=pd.SparseDtype("float64",
    0.0)),
            },
            index=[0, 2],
        )
    ...
    def test_reindex_axes(self):
        ...
        df = DataFrame(
            np.ones((3, 3)),
            index=[datetime(2012, 1, 1), datetime(2012, 1, 2), datetime(2012,
    1, 3)],
            columns=["a", "b", "c"],
        )
    ...
    def test_reindex_positional_warns(self):
        # https://github.com/pandas-dev/pandas/issues/12392
        df = DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]})
        expected = DataFrame({"A": [1.0, 2], "B": [4.0, 5], "C": [np.nan,
    np.nan]})
        with tm.assert_produces_warning(FutureWarning):
    ...
    def test_reindex_api_equivalence(self):
        ...
        res3 = df.reindex(labels=["b", "a"], axis=0).reindex(labels=["e",
    "d"], axis=1)
        for res in [res2, res3]:
            tm.assert_frame_equal(res1, res)
    ...
    def test_reindex_boolean(self):
        frame = DataFrame(
            np.ones((10, 2), dtype=bool), index=np.arange(0, 20, 2),
    columns=[0, 2]
        )
    ...
    def test_reindex_multi_categorical_time(self):
        ...
        expected = DataFrame({"a": [0, 1, 2, 3, 4, 5, 6, np.nan, 8]},
    index=midx)
        tm.assert_frame_equal(result, expected)
    ...
    def test_reindex_with_categoricalindex(self):
        df = DataFrame(
            ...
            index=CategoricalIndex(list("abc"), dtype=CDT(list("cabe")),
    name="B"),
        )
        ...
        expected = DataFrame({"A": [0, 1, np.nan], "B":
    Series(list("abe"))}).set_index(
            "B"
        )
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["a", "b"])
        expected = DataFrame({"A": [0, 1], "B":
    Series(list("ab"))}).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["e"])
        expected = DataFrame({"A": [np.nan], "B":
    Series(["e"])}).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)

        result = df.reindex(["d"])
        expected = DataFrame({"A": [np.nan], "B":
    Series(["d"])}).set_index("B")
        tm.assert_frame_equal(result, expected, check_index_type=True)
    ...
    ordered=True))}
        ...
        df2 = DataFrame(
            ...
            index=CategoricalIndex(list("aabbca"), dtype=CDT(list("cabe")),
    name="B"),
        )
    ...
    specified
        ...
        expected = DataFrame([[0, 7], [3, 4], [3, 4]], index=mi2,
    columns=["x", "y"])
        tm.assert_frame_equal(result, expected)
