...
class TestDataFrameDrop:
    ...
    def test_drop(self):
        ...
        tm.assert_frame_equal(simple.drop(["A", "B"], axis="columns"),
    simple[[]])
        tm.assert_frame_equal(simple.drop([0, 1, 3], axis=0), simple.loc[[2],:])
        tm.assert_frame_equal(simple.drop([0, 3], axis="index"),
    simple.loc[[1, 2],:])
        ...
        # GH 42881
        with pytest.raises(KeyError, match=r"\['C', 'D', 'F'\] not found in
    axis"):
        ...
        tm.assert_frame_equal(simple.drop("C", axis=1, errors="ignore"),
    simple)
        tm.assert_frame_equal(
        ...
        # non-unique - wheee!
        nu_df = DataFrame(
            list(zip(range(3), range(-3, 1), list("abc"))), columns=["a", "a",
    "b"]
        )
    ...
    def test_drop_multiindex_other_level_nan(self):
        ...
        expected = DataFrame(
            ...
            index=MultiIndex.from_tuples(
                [("one", 0.0, "b"), ("one", np.nan, "a")], names=["A", "B",
    "C"]
            ),
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_drop_tuple_with_non_unique_multiindex(self, indexer):
        ...
        expected = DataFrame(
            {"x": [2, 3]}, index=MultiIndex.from_tuples([("b", "a"), ("b",
    "a")])
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_drop_level_missing_label_multiindex(self):
        ...
        with pytest.raises(KeyError, match="labels \\[5\\] not found in
    level"):
            df.drop(5, level=0)
