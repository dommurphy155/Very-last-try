...
class TestResetIndex:
    def test_reset_index_empty_rangeindex(self):
        # GH#45230
        df = DataFrame(
            columns=["brand"], dtype=np.int64, index=RangeIndex(0, 0, 1,
    name="foo")
        )
    ...
    def test_reset_index(self, float_frame):
        ...
        tm.assert_index_equal(deleveled.index, Index(range(len(deleveled))),
    exact=True)
    ...
    def test_reset_index_level(self, levels):
        df = DataFrame([[1, 2, 3, 4], [5, 6, 7, 8]], columns=["A", "B", "C",
    "D"])
    ...
    def test_reset_index_multiindex_col(self):
        ...
        rs = df.reset_index()
        xp = DataFrame(
            full, columns=[["a", "b", "b", "c"], ["", "mean", "median",
    "mean"]]
        )
        tm.assert_frame_equal(rs, xp)
        ...
        xp = DataFrame(
            full, columns=[["a", "b", "b", "c"], ["a", "mean", "median",
    "mean"]]
        )
        tm.assert_frame_equal(rs, xp)
        ...
        xp = DataFrame(
            full, columns=[["blah", "b", "b", "c"], ["a", "mean", "median",
    "mean"]]
        )
        tm.assert_frame_equal(rs, xp)
        ...
        df = DataFrame(
            vals,
            MultiIndex.from_arrays([[0, 1, 2], ["x", "y", "z"]], names=["d",
    "a"]),
            columns=[["b", "b", "c"], ["mean", "median", "mean"]],
        )
        rs = df.reset_index("a")
    ...
    def test_reset_index_multiindex_nan(self):
        ...
        df = DataFrame({"A": ["a", "b", "c"], "B": [0, 1, 2], "C": [np.nan,
    1.1, 2.2]})
        rs = df.set_index(["A", "B"]).reset_index()
        tm.assert_frame_equal(rs, df)
    ...
    def test_reset_index_multiindex_columns(self, multiindex_df):
        ...
        expected = pd.concat([idx_col, multiindex_df[[("B", "b"), ("A",
    "")]]], axis=1)
        result = multiindex_df.set_index([("B", "b")],
    ...
    def test_reset_index_allow_duplicates_check(self, multiindex_df,
    allow_duplicates):
    ...
    def test_reset_index_datetime(self, tz_naive_fixture):
        ...
        expected["idx1"] = expected["idx1"].apply(lambda d: Timestamp(d,
    tz=tz))
    ...
    tz=tz))
        ...
        expected["level_1"] = expected["level_1"].apply(lambda d: Timestamp(d,
    tz=tz))
        result = df.reset_index()
        tm.assert_frame_equal(result, expected)
    ...
    def test_reset_index_period(self):
        ...
        df = DataFrame(
            np.arange(9, dtype="int64").reshape(-1, 1), index=idx,
    columns=["a"]
        )
        expected = DataFrame(
            {
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {"a": [pd.NaT, Timestamp("2020-01-01")], "b": [1, 2], "x":
    [11, 12]},
            ),
            (
                [(pd.NaT, 1), (pd.Timedelta(123, "d"), 2)],
                {"a": [pd.NaT, pd.Timedelta(123, "d")], "b": [1, 2], "x": [11,
    12]},
            ),
        ],
    )
    ...
    def test_rest_index_multiindex_categorical_with_missing_values(self,
    codes):
        # GH#24206
        ...
        expected = DataFrame(
            {
                "level_0": Categorical.from_codes(codes[0], categories=["A",
    "B"]),
                "level_1": Categorical.from_codes(codes[1], categories=["a",
    "b"]),
                "col": range(4),
            }
        )
...
def test_reset_index_dtypes_on_empty_frame_with_multiindex(array, dtype):
    ...
    expected = Series({"level_0": np.int64, "level_1": np.float64, "level_2":
    dtype})
...
def test_reset_index_multiindex_nat():
    ...
    expected = DataFrame(
        ...
        index=pd.DatetimeIndex(["2015-07-01", "2015-07-02", "NaT"],
    name="tstamp"),
    )
    tm.assert_frame_equal(result, expected)
...
def test_reset_index_interval_columns_object_cast():
    # GH 19136
    df = DataFrame(
        np.eye(2), index=Index([1, 2], name="Year"), columns=cut([1, 2], [0,
    1, 2])
    )
    result = df.reset_index()
    expected = DataFrame(
...
def test_reset_index_rename_multiindex(float_frame):
    ...
    tm.assert_series_equal(result["first"], expected["new_first"],
    check_names=False)
    tm.assert_series_equal(result["second"], expected["new_second"],
    check_names=False)
...
def test_errorreset_index_rename(float_frame):
    ...
    with pytest.raises(
        ...
        stacked_df.reset_index(names={"first": "new_first", "second":
    "new_second"})
