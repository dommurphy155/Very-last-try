...
class TestDataFrameSortValues:
    ...
    def test_sort_values_sparse_no_warning(self, dtype):
        ...
        ser = pd.Series(Categorical(["a", "b", "a"], categories=["a", "b",
    "c"]))
    ...
    columns=list("ABC")
        ...
        sorted_df = frame.sort_values(by=[1, 3], axis=1, ascending=[True,
    False])
        tm.assert_frame_equal(sorted_df, expected)
    ...
    def test_sort_values_inplace(self):
        frame = DataFrame(
            np.random.randn(4, 4), index=[1, 2, 3, 4], columns=["A", "B", "C",
    "D"]
        )
        ...
        sorted_df = frame.copy()
        return_value = sorted_df.sort_values(by="A", ascending=False,
    inplace=True)
        assert return_value is None
    ...
    def test_sort_values_nan(self):
        ...
        sorted_df = df.sort_values(["A", "B"], ascending=[1, 0],
    na_position="first")
        tm.assert_frame_equal(sorted_df, expected)
    ...
    def test_sort_values_stable_multicolumn_sort(
        ...
        df = DataFrame(
            {
                "A": [1, 2, np.nan, 1, 1, 1, 6, 8, 4, 8, 8, np.nan, np.nan, 8,
    8],
                "B": [9, np.nan, 5, 2, 2, 2, 5, 4, 5, 3, 4, np.nan, np.nan, 4,
    4],
            }
        )
        # All rows with NaN in col "B" only have unique values in "A",
    therefore,
    ...
    def test_sort_values_stable_categorial(self):
        # GH#16793
        df = DataFrame({"x": Categorical(np.repeat([1, 2, 3, 4], 5),
    ordered=True)})
        expected = df.copy()
    ...
    def test_sort_values_frame_column_inplace_sort_exception(self,
    float_frame):
        s = float_frame["A"]
    ...
    def test_sort_values_nat_values_in_int_column(self):
        ...
        df = DataFrame(
            {"int": int_values, "float": float_values}, columns=["int",
    "float"]
        )
        ...
        # columns:
        df = DataFrame(
            {"datetime": [Timestamp("2016-01-01"), NaT], "float":
    float_values},
            columns=["datetime", "float"],
        )

        df_reversed = DataFrame(
            {"datetime": [NaT, Timestamp("2016-01-01")], "float":
    float_values[::-1]},
            columns=["datetime", "float"],
            index=[1, 0],
        )
    ...
    def test_sort_nat(self):
        ...
        d1 = [Timestamp(x) for x in ["2016-01-01", "2015-01-01", np.nan,
    "2016-01-01"]]
        d2 = [
            Timestamp(x)
        ...
        d3 = [Timestamp(x) for x in ["2015-01-01", "2016-01-01", "2016-01-01",
    np.nan]]
        d4 = [
            Timestamp(x)
        ...
        df = DataFrame(
            {
                column_name: Categorical(
                    ["A", np.nan, "B", np.nan, "C"], categories=categories,
    ordered=True
                )
            }
        )
        ...
        expected = DataFrame(
            {
                column_name: Categorical(
                    list_of_nans + categories, categories=categories,
    ordered=True
                )
            },
        ...
        expected = DataFrame(
            {
                column_name: Categorical(
                    categories + list_of_nans, categories=categories,
    ordered=True
                )
            },
...
class TestDataFrameSortKey:  # test key sorting (issue 27237)
    ...
    def test_sort_values_key_string(self):
        ...
        expected = df.sort_values(1, key=lambda col: col.str.lower(),
    ascending=False)
        tm.assert_frame_equal(result, expected)
...
class TestSortValuesLevelAsStr:
    ...
    def test_sort_column_level_and_index_label(
        ...
        result = df_idx.T.sort_values(by=sort_names, ascending=ascending,
    axis=1)
