...
class TestDataFrameDiff:
    ...
    def test_diff_timedelta64_with_nat(self):
        ...
        expected = DataFrame({0: df[0], 1: [pd.NaT, pd.Timedelta(2),
    pd.Timedelta(2)]})
    ...
    @pytest.mark.parametrize("tz", [pytest.param(None,
    marks=pytest.mark.xfail), "UTC"])
    ...
    def test_diff_timedelta(self):
        ...
        df = DataFrame(
            {
                "time": [Timestamp("20130101 9:01"), Timestamp("20130101
    9:02")],
                "value": [1.0, 2.0],
            }
        )
    ...
    def test_diff_axis1_mixed_dtypes(self):
        ...
        df = DataFrame({"A": range(3), "B": 2 * np.arange(3,
    dtype=np.float64)})
        ...
        df = DataFrame(
            {"a": np.arange(3, dtype="float32"), "b": np.arange(3,
    dtype="float64")}
        )
        result = df.diff(axis=1)
                ...
                DataFrame(
                    {
                        ...
                        "b": [np.nan, 1, np.nan, np.nan, -2, 1, np.nan,
    np.nan],
    ...
    def test_diff_all_int_dtype(self, any_int_numpy_dtype):
        ...
        result = df.diff()
        expected_dtype = (
            "float32" if any_int_numpy_dtype in ("int8", "int16") else
    "float64"
        )
