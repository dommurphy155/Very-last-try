...
class TestDataFrameAlign:
    ...
    def test_align_float(self, float_frame):
        ...
        af, bf = float_frame.align(
            other.iloc[:, 0], join="inner", axis=1, method=None,
    fill_value=None
        )
    ...
    def
    test_multiindex_align_to_series_with_common_index_level_missing_in_left(self):
        #  GH-46001
        foo_index = Index([1, 2, 3], name="foo")
        bar_index = Index([1, 2], name="bar")

        series = Series(
            [1, 2, 3, 4], index=Index([1, 2, 3, 4], name="bar"),
    name="foo_series"
        )
        df = DataFrame(
            {"col": np.arange(6)},
    ...
    def
    test_multiindex_align_to_series_with_common_index_level_missing_in_right(self):
        #  GH-46001
        foo_index = Index([1, 2, 3], name="foo")
        bar_index = Index([1, 2, 3, 4], name="bar")

        series = Series([1, 2], index=Index([1, 2], name="bar"),
    name="foo_series")
        df = DataFrame(
            {"col": np.arange(12)},
            index=pd.MultiIndex.from_product([foo_index, bar_index]),
    ...
    def
    test_multiindex_align_to_series_with_common_index_level_missing_in_both(self):
        #  GH-46001
        foo_index = Index([1, 2, 3], name="foo")
        ...
        expected_r = Series([1, np.nan, 3] * 3, index=df.index,
    name="foo_series")
        result_l, result_r = df.align(series, axis=0)
    ...
    def
    test_multiindex_align_to_series_with_common_index_level_non_unique_cols(self):
        #  GH-46001
        foo_index = Index([1, 2, 3], name="foo")
        bar_index = Index([1, 2], name="bar")
    ...
    def _check_align(self, a, b, axis, fill_axis, how, method, limit=None):
        aa, ab = a.align(
            b, axis=axis, join=how, method=method, limit=limit,
    fill_axis=fill_axis
        )
    ...
    def _check_align_fill(self, frame, kind, meth, ax, fax):
        ...
        self._check_align(left, right, axis=ax, fill_axis=fax, how=kind,
    method=meth)
        self._check_align(
        ...
        # empty left
        self._check_align(empty, right, axis=ax, fill_axis=fax, how=kind,
    method=meth)
        self._check_align(
    ...
    limit=1
        ...
        # empty right
        self._check_align(left, empty, axis=ax, fill_axis=fax, how=kind,
    method=meth)
        self._check_align(
        ...
        # both empty
        self._check_align(empty, empty, axis=ax, fill_axis=fax, how=kind,
    method=meth)
        self._check_align(
