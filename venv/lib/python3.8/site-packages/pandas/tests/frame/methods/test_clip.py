...
class TestDataFrameClip:
    ...
    def test_inplace_clip(self, float_frame):
        ...
        return_value = frame_copy.clip(upper=median, lower=median,
    inplace=True)
    ...
    def test_clip_mixed_numeric(self):
        ...
        df = DataFrame([[1, 2, 3.4], [3, 4, 5.6]], columns=["foo", "bar",
    "baz"])
        expected = df.dtypes
    ...
    def test_clip_against_list_like(self, simple_frame, inplace, lower, axis,
    res):
        # GH#15390
        original = simple_frame.copy(deep=True)

        result = original.clip(lower=lower, upper=[5, 6, 7], axis=axis,
    inplace=inplace)
    ...
    float_frame)
        ...
        df = DataFrame({"col_0": [1, 2, 3], "col_1": [4, 5, 6], "col_2": [7,
    8, 9]})
        ...
        expected = DataFrame({"col_0": [9, -3, 0, 6, 5], "col_1": [2, -4, 6,
    8, 3]})
        tm.assert_frame_equal(result, expected)
    ...
    def test_clip_pos_args_deprecation(self):
        ...
        msg = (
            r"In a future version of pandas all arguments of DataFrame.clip
    except "
