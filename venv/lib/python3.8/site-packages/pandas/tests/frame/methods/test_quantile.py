...
@pytest.fixture(
    params=[["linear", "single"], ["nearest", "table"]], ids=lambda x:
    "-".join(x)
)
...
class TestDataFrameQuantile:
    ...
    def test_numeric_only_default_false_warning(
        ...
        with tm.assert_produces_warning(FutureWarning, match="numeric_only"):
            result = df.quantile(0.5, interpolation=interpolation,
    method=method)
    ...
    def test_quantile(
        ...
        df = datetime_frame
        result = df.quantile(
            0.1, axis=0, numeric_only=True, interpolation=interpolation,
    method=method
        )
        expected = Series(
        ...
        if interpolation == "linear":
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    using_array_manager, reason="Name set incorrectly for
    arraymanager"
                )
            )
        ...
        result = df.quantile(
            0.9, axis=1, numeric_only=True, interpolation=interpolation,
    method=method
        )
        expected = Series(
    ...
    def test_axis_numeric_only_true(self, interp_method, request,
    using_array_manager):
        ...
        result = df.quantile(
            0.5, axis=1, numeric_only=True, interpolation=interpolation,
    method=method
        )
    ...
    def test_quantile_date_range(self, interp_method, request,
    using_array_manager):
        # GH 2460
        interpolation, method = interp_method
        ...
        expected = Series(
            ["2016-01-02 00:00:00"], name=0.5, dtype="datetime64[ns,
    US/Pacific]"
        )
        if method == "table" and using_array_manager:
            request.node.add_marker(
    ...
    def test_quantile_axis_mixed(self, interp_method, request,
    using_array_manager):
    ...
    def test_quantile_axis_parameter(self, interp_method, request,
    using_array_manager):
        # GH 9543/9544
        interpolation, method = interp_method
        if method == "table" and using_array_manager:
    ...
    def test_quantile_interpolation(self):
        ...
        # float
        df = DataFrame({"A": [1.0, 2.0, 3.0], "B": [2.0, 3.0, 4.0]}, index=[1,
    2, 3])
        result = df.quantile(0.5, axis=1, interpolation="nearest")
    ...
    def test_quantile_multi_axis_1(self, interp_method, request,
    using_array_manager):
        interpolation, method = interp_method
        df = DataFrame([[1, 1, 1], [2, 2, 2], [3, 3, 3]], columns=["a", "b",
    "c"])
    ...
    def test_quantile_datetime(self):
        ...
        result = df.quantile([0.5], numeric_only=False)
        expected = DataFrame(
            [[Timestamp("2010-07-02 12:00:00"), 2.5]], index=[0.5],
    columns=["a", "b"]
        )
        tm.assert_frame_equal(result, expected)
        ...
        expected = Series(
            [Timestamp("2010-07-02 12:00:00"), Timestamp("2011-07-02
    12:00:00")],
            index=[0, 1],
            name=0.5,
        )
    ...
    def test_quantile_dt64_empty(self, dtype, interp_method):
        ...
        res = df.quantile(
            0.5, axis=1, numeric_only=False, interpolation=interpolation,
    method=method
        )
    ...
    def test_quantile_box(self, interp_method, request, using_array_manager):
        ...
        res = df.quantile(
            [0.5], numeric_only=False, interpolation=interpolation,
    method=method
        )
        exp = DataFrame(
            [
    ...
    def test_quantile_nan(self, interp_method, request, using_array_manager):
        ...
        exp = Series(
            [3.0, 2.5 if interpolation == "linear" else 3.0], index=["a",
    "b"], name=0.5
        )
        tm.assert_series_equal(res, exp)

        res = df.quantile([0.5, 0.75], interpolation=interpolation,
    method=method)
        exp = DataFrame(
            {
                "a": [3.0, 4.0],
    ...
    method=method)
        ...
        res = df.quantile([0.5, 0.75], interpolation=interpolation,
    method=method)
        exp = DataFrame({"a": [3.0, 4.0], "b": [np.nan, np.nan]}, index=[0.5,
    0.75])
        tm.assert_frame_equal(res, exp)
    ...
    method=method
        ...
        exp = DataFrame(
            [[Timestamp("2012-01-02"), pd.NaT]], index=[0.5], columns=["a",
    "b"]
        )
        tm.assert_frame_equal(res, exp)
    ...
    def test_quantile_empty_no_rows_dt64(self, interp_method):
        ...
        exp = Series(
            [pd.NaT, pd.NaT], index=["a", "b"], dtype="datetime64[ns]",
    name=0.5
        )
        tm.assert_series_equal(res, exp)
    ...
    def test_quantile_empty_no_columns(self, interp_method):
        ...
        result = df.quantile(
            [0.5], numeric_only=True, interpolation=interpolation,
    method=method
        )
    ...
    def test_quantile_item_cache(self, using_array_manager, interp_method):
        ...
        df.quantile(numeric_only=False, interpolation=interpolation,
    method=method)
        ser.values[0] = 99
    ...
    def test_table_invalid_interpolation(self):
        with pytest.raises(ValueError, match="Invalid interpolation: foo"):
            DataFrame(range(1)).quantile(0.5, method="table",
    interpolation="foo")
...
class TestQuantileExtensionDtype:
    ...
    @pytest.fixture(
        params=[
            pytest.param(
                ...
                marks=pytest.mark.xfail(reason="raises when trying to add
    Intervals"),
            ),
    ...
    def test_quantile_ea(self, request, obj, index):
        ...
        if np_version_under1p21 and index.dtype == "timedelta64[ns]":
            msg = "failed on Numpy 1.20.3; TypeError: data type 'Int64' not
    understood"
        ...
        expected = Series(
            [index[4], index[0], index[-1]], dtype=exp_dtype, index=qs,
    name="A"
        )
    ...
    def test_quantile_ea_with_na(self, obj, index):
        ...
        # expected here assumes len(index) == 9
        expected = Series(
            [index[4], index[1], index[-2]], dtype=index.dtype, index=qs,
    name="A"
        )
        expected = type(obj)(expected)
        tm.assert_equal(result, expected)
    ...
    @pytest.mark.filterwarnings("ignore:Using .astype to
    convert:FutureWarning")
    def test_quantile_ea_all_na(self, request, obj, index):
        obj.iloc[:] = index._na_value

        # TODO(ArrayManager): this casting should be unnecessary after
    GH#39763 is fixed
