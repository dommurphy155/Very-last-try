...
class TestDataFramePctChange:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (-1, "ffill", None, [np.nan, np.nan, -0.5, -0.5, -0.6, 0, 0,
    np.nan]),
            (-1, "ffill", 1, [np.nan, np.nan, -0.5, -0.5, -0.6, 0, np.nan,
    np.nan]),
            (-1, "bfill", None, [0, 0, -0.5, -0.5, -0.6, np.nan, np.nan,
    np.nan]),
            (-1, "bfill", 1, [np.nan, 0, -0.5, -0.5, -0.6, np.nan, np.nan,
    np.nan]),
        ],
    )
    @pytest.mark.parametrize("klass", [DataFrame, Series])
    def test_pct_change_with_nas(self, periods, fill_method, limit, exp,
    klass):
        vals = [np.nan, np.nan, 1, 2, 4, 10, np.nan, np.nan]
        obj = klass(vals)

        res = obj.pct_change(periods=periods, fill_method=fill_method,
    limit=limit)
    ...
    columns=datetime_frame.columns)
        rs_freq = empty_ts.pct_change(freq=freq, fill_method=fill_method,
    limit=limit)
        rs_periods = empty_ts.pct_change(periods, fill_method=fill_method,
    limit=limit)
        tm.assert_frame_equal(rs_freq, rs_periods)
