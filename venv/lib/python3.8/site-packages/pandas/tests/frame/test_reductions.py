...
def assert_stat_op_calc(
    ...
    if check_dates:
        expected_warning = FutureWarning if opname in ["mean", "median"] else
    None
    ...
    deprecated"):
        ...
        tm.assert_series_equal(
            result0, frame.apply(wrapper), check_dtype=check_dtype, rtol=rtol,
    atol=atol
        )
        tm.assert_series_equal(
            result1,
...
class TestDataFrameAnalytics:
    ...
    def test_stat_op_api_float_string_frame(self, float_string_frame, axis,
    opname):
        warn = FutureWarning if opname == "mad" else None
        with tm.assert_produces_warning(
            warn, match="The 'mad' method is deprecated",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            DataFrame({0: [np.nan, 2], 1: [np.nan, 3], 2: [np.nan, 4]},
    dtype=object),
        ],
    )
    ...
    @pytest.mark.parametrize("op", ["mean", "std", "var", "skew", "kurt",
    "sem"])
    ...
    def test_mode_empty_df(self):
        ...
        result = df.mode()
        expected = DataFrame([], columns=["a", "b"], index=Index([],
    dtype=int))
        tm.assert_frame_equal(result, expected)
    ...
    def test_operators_timedelta64(self):
        ...
        # excludes numeric
        with tm.assert_produces_warning(FutureWarning, match="Select only
    valid"):
        ...
        expected = Series(
            [timedelta(seconds=5 * 60 + 5), timedelta(days=-1)], index=["A",
    "B"]
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_sum_prod_nanops(self, method, unit, numeric_only):
        idx = ["a", "b", "c"]
        df = DataFrame({"a": [unit, unit], "b": [unit, np.nan], "c": [np.nan,
    np.nan]})
        # The default
        ...
        result = getattr(df.iloc[1:], method)(numeric_only=numeric_only,
    min_count=1)
    ...
    def test_sum_nanops_dtype_min_count(self, float_type, kwargs,
    expected_result):
        # GH#46947
        df = DataFrame({"a": [1.0, 2.3, 4.4], "b": [2.2, 3, np.nan]},
    dtype=float_type)
    ...
    def test_prod_nanops_dtype_min_count(self, float_type, kwargs,
    expected_result):
        # GH#46947
        df = DataFrame(
    ...
    def test_sum_mixed_datetime(self):
        # GH#30886
        df = DataFrame({"A": date_range("2000", periods=4), "B": [1, 2, 3,
    4]}).reindex(
            [2, 3, 4]
        )
        with tm.assert_produces_warning(FutureWarning, match="Select only
    ...
    def test_mean_datetimelike_numeric_only_false(self):
        ...
        with pytest.raises(TypeError, match="mean is not implemented for
    Period"):
    ...
    def test_idxmax_mixed_dtype(self):
        ...
        df = DataFrame({1: [0, 2, 1], 2: range(3)[::-1], 3:
    dti.copy(deep=True)})
    ...
    def test_any_all_mixed_float(self, opname, axis, bool_only,
    float_string_frame):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(np.all, {"A": Series([0, 1], dtype="M8[ns, UTC]")},
    False),
            pytest.param(np.any, {"A": Series([0, 1], dtype="M8[ns]")}, True),
            pytest.param(np.any, {"A": Series([0, 1], dtype="M8[ns, UTC]")},
    True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="M8[ns]")}, True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="M8[ns, UTC]")},
    True),
            pytest.param(np.any, {"A": Series([1, 2], dtype="M8[ns]")}, True),
            pytest.param(np.any, {"A": Series([1, 2], dtype="M8[ns, UTC]")},
    True),
            pytest.param(np.all, {"A": Series([0, 1], dtype="m8[ns]")}, False),
            pytest.param(np.any, {"A": Series([0, 1], dtype="m8[ns]")}, True),
            pytest.param(np.all, {"A": Series([1, 2], dtype="m8[ns]")}, True),
...
class TestDataFrameReductions:
    ...
    def test_min_max_dt64_with_NaT_skipna_false(self, request,
    tz_naive_fixture):
        # GH#36907
        tz = tz_naive_fixture
    ...
    def test_min_max_dt64_api_consistency_with_NaT(self):
        # Calling the following sum functions returned an error for dataframes
    but
        # returned NaT for series. These tests check that the API is
    consistent in
    ...
    def test_min_max_dt64_api_consistency_empty_df(self):
        # check DataFrame/Series api consistency when calling min/max on an
    empty
        # DataFrame/Series.
        df = DataFrame({"x": []})
        expected_float_series = Series([], dtype=float)
        # check axis 0
        assert np.isnan(df.min(axis=0).x) ==
    np.isnan(expected_float_series.min())
        assert np.isnan(df.max(axis=0).x) ==
    ...
    @pytest.mark.parametrize(
        "initial",
        ["2018-10-08 13:36:45+00:00", "2018-10-08 13:36:45+03:00"],  # Non-UTC
    timezone
    )
    ...
    def test_frame_any_all_with_level(self):
        ...
        with tm.assert_produces_warning(FutureWarning, match="Using the
    level"):
    ...
    def test_reductions_skipna_none_raises(
        ...
        obj = frame_or_series([1, 2, 3])
        msg = 'For argument "skipna" expected type bool, received type
    NoneType.'
        with pytest.raises(ValueError, match=msg):
...
class TestNuisanceColumns:
    ...
    @pytest.mark.parametrize("method", ["min", "max"])
    def test_min_max_categorical_dtype_non_ordered_nuisance_column(self,
    method):
...
def test_mad_nullable_integer_all_na(any_signed_int_ea_dtype):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        df2.iloc[:, 1] = pd.NA  # FIXME(GH#44199): this doesn't operate
    in-place
        df2.iloc[:, 1] = pd.array([pd.NA] * len(df2),
...
def test_groupby_regular_arithmetic_equivalent(meth):
    # GH#40660
    df = DataFrame(
        {"a": [pd.Timedelta(hours=6), pd.Timedelta(hours=7)], "b": [12.1,
    13.3]}
    )
    expected = df.copy()
...
@pytest.mark.parametrize("method", ["min", "max", "mean", "median", "skew",
    "kurt"])
def test_reduction_axis_none_deprecation(method):
    # GH#21597 deprecate axis=None defaulting to axis=0 so that we can change
    it
...
def test_numeric_only_deprecation(kernel):
    ...
    if kernel in ("idxmax", "idxmin"):
        # kernels that default to numeric_only=False and fail on nuisance
    columns
