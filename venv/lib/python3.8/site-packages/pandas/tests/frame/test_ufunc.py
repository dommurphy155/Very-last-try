...
def test_unary_unary(dtype):
    ...
    df = pd.DataFrame(values, columns=["A", "B"], index=["a",
    "b"]).astype(dtype=dtype)
...
def test_unary_binary(request, dtype):
    ...
    if is_extension_array_dtype(dtype) or isinstance(dtype, dict):
        request.node.add_marker(
            pytest.mark.xfail(
                reason="Extension / mixed with multiple outputs not
    implemented."
            )
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            partial(np.negative, where=np.array([[False, True], [True,
    False]])),
            None,
            [0, -2, -3, 0],
        ),
...
def test_binary_input_aligns_columns(request, dtype_a, dtype_b):
    if (
        ...
        request.node.add_marker(
            pytest.mark.xfail(
                reason="Extension / mixed with multiple inputs not
    implemented."
            )
        )
...
def test_binary_input_aligns_index(request, dtype):
    ...
    df1 = pd.DataFrame({"A": [1, 2], "B": [3, 4]}, index=["a",
    "b"]).astype(dtype)
    df2 = pd.DataFrame({"A": [1, 2], "B": [3, 4]}, index=["a",
    "c"]).astype(dtype)
    with tm.assert_produces_warning(FutureWarning):
        result = np.heaviside(df1, df2)
    # Expected future behaviour:
    ...
    # expected = pd.DataFrame(expected, index=["a", "b", "c"], columns=["A",
    "B"])
    expected = pd.DataFrame(
...
@pytest.mark.filterwarnings("ignore:Calling a ufunc on
    non-aligned:FutureWarning")
...
def test_unary_accumulate_axis():
    ...
    expected = pd.DataFrame({"a": [1.0, 3.0, 3.0, 4.0], "b": [0.1, 4.0, 4.0,
    4.0]})
    tm.assert_frame_equal(result, expected)
    ...
    expected = pd.DataFrame({"a": [1.0, 3.0, 2.0, 4.0], "b": [1.0, 4.0, 3.0,
    4.0]})
    tm.assert_frame_equal(result, expected)
...
def test_alignment_deprecation_many_inputs(request):
    ...
    expected = pd.DataFrame([[3.0, 12.0], [6.0, 15.0], [9.0, 18.0]],
    columns=["a", "b"])
    tm.assert_frame_equal(result, expected)
