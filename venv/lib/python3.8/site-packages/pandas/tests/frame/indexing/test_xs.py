...
class TestXS:
    ...
    def test_xs_corner(self):
        ...
        xs = df.xs(0)
        exp = Series([1.0, "foo", 2.0, "bar", 3.0], index=list("ABCDE"),
    name=0)
    ...
    def test_xs_keep_level(self):
        ...
        with tm.assert_produces_warning(FutureWarning):
            result = df.xs([2008, "sat"], level=["year", "day"],
    drop_level=False)
        tm.assert_frame_equal(result, expected)
    ...
    def test_xs_view(self, using_array_manager, using_copy_on_write):
        ...
        dm = DataFrame(np.arange(20.0).reshape(4, 5), index=range(4),
    columns=range(5))
        df_orig = dm.copy()
...
class TestXSWithMultiIndex:
    ...
    @pytest.mark.parametrize("key, level", [("one", "second"), (["one"],
    ["second"])])
    ...
    def test_xs_missing_values_in_index(self):
        ...
        df = DataFrame(acc, columns=["a1", "a2", "cnt"]).set_index(["a1",
    "a2"])
        expected = DataFrame(
            {"cnt": [24, 26, 25, 26]},
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("a", "lvl0", lambda x: x[:, 0:2], Index(["bar", "foo"],
    name="lvl1")),
    ...
    def test_xs_IndexSlice_argument_not_implemented(self, klass):
        ...
        index = MultiIndex(
            levels=[[("foo", "bar", 0), ("foo", "baz", 0), ("foo", "qux", 0)],
    [0, 1]],
    ...
    using_copy_on_write):
        ...
        assert np.shares_memory(result.iloc[:, 0]._values, df.iloc[:,
    0]._values)
    ...
    def test_xs_list_indexer_droplevel_false(self):
        # GH#41760
        mi = MultiIndex.from_tuples([("x", "m", "a"), ("x", "n", "b"), ("y",
    "o", "c")])
