...
_slice_msg = "slice indices must be integers or None or have an __index__
    method"
...
class TestDataFrameIndexing:
    ...
    datetime_frame):
        ...
        for df in [
            ...
            bif = df[df > 0]
            bifw = DataFrame(
                {c: np.where(data[c] > 0, data[c], np.nan) for c in
    data.columns},
                index=data.index,
    ...
    def test_setitem_corner(self, float_frame):
        # corner case
        df = DataFrame({"B": [1.0, 2.0, 3.0], "C": ["a", "b", "c"]},
    index=np.arange(3))
        del df["B"]
        df["B"] = [1.0, 2.0, 3.0]
        assert "B" in df
    ...
    def test_setitem_None(self, float_frame):
        ...
        tm.assert_series_equal(float_frame[None], float_frame["A"],
    check_names=False)
        repr(float_frame)
    ...
    @td.skip_array_manager_invalid_test  # already covered in
    test_iloc_col_slice_view
    ...
    def test_setitem_frame_mixed(self, float_string_frame):
        ...
        f.loc[key] = piece
        tm.assert_almost_equal(f.loc[f.index[0:2], ["A", "B"]].values,
    piece.values)
    ...
    def test_setitem_frame_mixed_key_unaligned(self, float_string_frame):
        ...
        f.loc[key] = piece
        piece["B"] = np.nan
        tm.assert_almost_equal(f.loc[f.index[-2:], ["A", "B"]].values,
    piece.values)
    ...
    def test_getitem_setitem_ix_duplicates(self):
        # #1201
        df = DataFrame(np.random.randn(5, 3), index=["foo", "foo", "bar",
    "baz", "bar"])
    ...
    DataFrame"
        if using_copy_on_write:
            ...
            # TODO(ArrayManager) verify it is expected that the original
    didn't change
    ...
    def test_loc_duplicates(self):
        ...
        trange = trange.insert(loc=5, item=Timestamp(year=2017, month=1,
    day=5))
    ...
    def test_setitem_with_unaligned_tz_aware_datetime_column(self):
        ...
        column = Series(date_range("2015-01-01", periods=3, tz="utc"),
    name="dates")
    ...
    def test_loc_setitem_datetimelike_with_inference(self):
        ...
        df.loc[df.index[:3], "C"] = np.array([3 * one_hour] * 3,
    dtype="m8[ns]")
    ...
    dtype="m8[ns]")
        ...
        result = df.dtypes
        expected = Series(
            [np.dtype("timedelta64[ns]")] * 6 + [np.dtype("datetime64[ns]")] *
    2,
            index=list("ABCDEFGH"),
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_getitem_boolean_indexing_mixed(self):
        df = DataFrame(
            {
                0: {35: np.nan, 40: np.nan, 43: np.nan, 49: np.nan, 50:
    np.nan},
                1: {
                    35: np.nan,
                ...
                3: {35: np.nan, 40: np.nan, 43: np.nan, 49: np.nan, 50:
    np.nan},
                4: {
    ...
    def test_type_error_multiindex(self):
        ...
        dg = DataFrame(
            [[1, 1, 2, 2], [3, 3, 4, 4]], columns=mi, index=Index([0, 1],
    name="i")
        )
        ...
        columns = MultiIndex(
            levels=[["x", "y"], [0, 1]], codes=[[0, 1], [0, 0]], names=[None,
    "c"]
        )
    ...
    def test_setting_mismatched_na_into_nullable_fails(
        ...
        msg = "|".join(
            [
                r"timedelta64\[ns\] cannot be converted to
    (Floating|Integer)Dtype",
                r"datetime64\[ns\] cannot be converted to
    (Floating|Integer)Dtype",
    ...
    def test_loc_expand_empty_frame_keep_midx_names(self):
        # GH#46317
        df = DataFrame(
            columns=["d"], index=MultiIndex.from_tuples([], names=["a", "b",
    "c"])
        )
        df.loc[(1, 2, 3)] = "foo"
        expected = DataFrame(
    ...
    def test_loc_internals_not_updated_correctly(self):
        ...
        df = DataFrame(
            ...
            index=MultiIndex.from_arrays([[1, 2], [1, 2]], names=["idx1",
    "idx2"]),
        )
        idx = [(1, 1)]
...
class TestDataFrameIndexingUInt64:
    def test_setitem(self, uint64_frame):
        ...
        df2.iloc[1, 2] = pd.NaT
        result = df2["B"]
        tm.assert_series_equal(notna(result), Series([True, False, True],
    name="B"))
        tm.assert_series_equal(
            df2.dtypes,
            Series(
...
msg1 = r"Cannot setitem on a Categorical with a new category( \(.*\))?, set
    the"
...
class TestLocILocDataFrameCategorical:
    ...
    categories=["a", "b"])
        ...
        exp_single_row = DataFrame({"cats": cats1, "values": values1},
    index=idx1)
    ...
    categories=["a", "b"])
        ...
        exp_multi_row = DataFrame({"cats": cats2, "values": values2},
    index=idx2)
    ...
    categories=["a", "b"])
        ...
        exp_parts_cats_col = DataFrame({"cats": cats3, "values": values3},
    index=idx3)
    ...
    def test_loc_iloc_setitem_list_of_lists(self, orig, exp_multi_row,
    indexer):
    ...
    def test_getitem_preserve_object_index_with_dates(self, indexer):
        # https://github.com/pandas-dev/pandas/pull/42950 - when selecting a
    column
