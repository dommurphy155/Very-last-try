...
class TestGetitemListLike:
    ...
    def test_getitem_dupe_cols(self):
        ...
        msg = "\"None of [Index(['baf'], dtype='object')] are in the
    [columns]\""
    ...
    freq=DateOffset(days=1)),
        ...
        expected = DataFrame(
            [0, 1, 2],
            index=DatetimeIndex(
                ["2022-01-01 00:00:00", "2022-01-02 02:00:00", "2022-01-03
    04:00:00"],
                dtype="datetime64[ns]",
...
class TestGetitemBooleanMask:
    ...
    def df_dup_cols(self):
        dups = ["A", "A", "C", "D"]
        df = DataFrame(np.arange(12).reshape(3, 4), columns=dups,
    dtype="float64")
        return df
    ...
    def test_getitem_returns_view_when_column_is_unique_in_df(
        ...
        if using_copy_on_write:
            ...
            expected = DataFrame([[1, 2, 100], [4, 5, 100]], columns=["a",
    "a", "b"])
        tm.assert_frame_equal(df, expected)

    def test_getitem_frozenset_unique_in_column(self):
        # GH#41062
        df = DataFrame([[1, 2, 3, 4]], columns=[frozenset(["KEY"]), "B", "C",
    "C"])
...
class TestGetitemDeprecatedIndexers:
    ...
    def test_getitem_dict_and_set_deprecated(self, key):
        # GH#42825
        df = DataFrame(
            [[1, 2], [3, 4]], columns=MultiIndex.from_tuples([("a", 1), ("b",
    2)])
        )
        with tm.assert_produces_warning(FutureWarning):
            df[key]
