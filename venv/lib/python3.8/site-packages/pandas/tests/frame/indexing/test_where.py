...
def where_frame(request, float_string_frame, mixed_float_frame,
    mixed_int_frame):
...
def _safe_add(df):
    ...
    def is_ok(s):
        return (
            issubclass(s.dtype.type, (np.integer, np.floating)) and s.dtype !=
    "uint8"
        )

    return DataFrame(dict((c, s + 1) if is_ok(s) else (c, s) for c, s in
    df.items()))
...
class TestDataFrameIndexingWhere:
    def test_where_get(self, where_frame, float_string_frame):
        def _check_get(df, cond, check_dtypes=True):
            ...
            for k, v in rs.items():
                exp = Series(np.where(cond[k], df[k], other1[k]),
    index=v.index)
    ...
    def test_where_alignment(self, where_frame, float_string_frame):
        ...
        cond = df > 0
        check_dtypes = all(not issubclass(s.type, np.integer) for s in
    df.dtypes)
    ...
    def test_where_datetime(self, using_array_manager):
        ...
        msg = "'>' not supported between instances of 'float' and
    'datetime.datetime'"
        with pytest.raises(TypeError, match=msg):
            df > stamp
    ...
    def test_where_align(self):
        ...
        return_value = df.where(pd.notna(df), df.mean(), inplace=True,
    axis="columns")
        assert return_value is None
        tm.assert_frame_equal(df, expected)
    ...
    def test_where_complex(self):
        # GH 6345
        expected = DataFrame([[1 + 1j, 2], [np.nan, 4 + 1j]], columns=["a",
    "b"])
    ...
    def test_where_callable(self):
        ...
        tm.assert_frame_equal(result, exp)
        tm.assert_frame_equal(result, (df + 2).where((df + 2) > 8, (df + 2) +
    10))
    ...
    def test_where_ea_other(self):
        ...
        expected = DataFrame({"A": [1, pd.NA, 3], "B": [4, pd.NA,
    6]}).astype(object)
        tm.assert_frame_equal(result, expected)
...
def test_where_int_downcasting_deprecated():
    ...
    expected = DataFrame({0: arr[:, 0], 1: np.array([2**17] * 3,
    dtype=np.int32)})
...
def test_where_bool_comparison():
    # GH 10336
    df_mask = DataFrame(
        {"AAA": [True] * 4, "BBB": [False] * 4, "CCC": [True, False, True,
    False]}
    )
...
def test_where_duplicate_axes_mixed_dtypes():
    # GH 25399, verify manually masking is not affected anymore by dtype of
    column for
    # duplicate axes.
    ...
    d = result.where(mask)  # used to fail with "cannot reindex from a
    duplicate axis"
