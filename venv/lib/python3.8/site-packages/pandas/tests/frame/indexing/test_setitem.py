...
class TestDataFrameSetItem:
    def test_setitem_str_subclass(self):
        ...
        df["b"] = 2
        df[mystring("c")] = 3
        expected = DataFrame({"a": [1], "b": [2], mystring("c"): [3]},
    index=index)
    ...
    def test_setitem_error_msmgs(self):
        ...
        df = DataFrame(np.random.randint(0, 2, (4, 4)), columns=["a", "b",
    "c", "d"])

        msg = "Cannot set a DataFrame with multiple columns to the single
    column gr"
        with pytest.raises(ValueError, match=msg):
    ...
    def test_setitem_timestamp_empty_columns(self):
        ...
        expected = DataFrame(
            [[Timestamp("20130101", tz="UTC")]] * 3, index=[0, 1, 2],
    columns=["now"]
        )
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_extension_types(self, obj, dtype):
        # GH: 34832
        expected = DataFrame({"idx": [1, 2, 3], "obj": Series([obj] * 3,
    dtype=dtype)})
    ...
    def test_setitem_dt64_ndarray_with_NaT_and_diff_time_units(self):
        ...
        result["new"] = data_ns
        expected = DataFrame({0: [1, None], "new": [1, None]},
    dtype="datetime64[ns]")
        tm.assert_frame_equal(result, expected)
        ...
        result["new"] = data_s
        expected = DataFrame({0: [1, None], "new": [1e9, None]},
    dtype="datetime64[ns]")
        tm.assert_frame_equal(result, expected)
    ...
    def test_setitem_frame_duplicate_columns_size_mismatch(self):
        ...
        cols = ["A", "B", "C"] * 2
        df = DataFrame(index=range(3), columns=cols)
        with pytest.raises(ValueError, match="Columns must be same length as
    key"):
            df[["A"]] = (0, 3, 5)
    ...
    def test_setitem_multi_index(self):
        ...
        it = ["jim", "joe", "jolie"], ["first", "last"], ["left", "center",
    "right"]
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                DataFrame([[7, 8, 9], [7, 8, 9], [7, 8, 9]], columns=["A",
    "B", "C"]),
            ),
            (
                ["B", "C", "D"],
    ...
    def test_setitem_aligning_dict_with_index(self, vals):
        ...
        df.loc[:, "e"] = {1: 5}
        expected = DataFrame(
            {"a": [200, 100], "b": [3, 4], **vals, "c": [5, 6], "e": [np.nan,
    5]}
        )
        tm.assert_frame_equal(df, expected)
...
class TestDataFrameSetItemWithExpansion:
    ...
    def test_setitem_string_column_numpy_dtype_raising(self):
        ...
        df["0 - Name"] = [5, 6]
        expected = DataFrame([[1, 2, 5], [3, 4, 6]], columns=[0, 1, "0 -
    Name"])
        tm.assert_frame_equal(df, expected)
    ...
    def test_setitem_with_expansion_categorical_dtype(self):
        # assignment
        df = DataFrame(
            {"value": np.array(np.random.randint(0, 10000, 100),
    dtype="int32")}
        )
        labels = Categorical([f"{i} - {i + 499}" for i in range(0, 10000,
    500)])
...
class TestDataFrameSetItemSlicing:
    ...
    def test_setitem_slice_indexer_broadcasting_rhs(self, n, box, indexer):
        ...
        expected = DataFrame([[1, 3, 5]] + [[10, 11, 12]] * n, columns=["a",
    "b", "c"])
        tm.assert_frame_equal(df, expected)
    ...
    @pytest.mark.parametrize("box", [Series, np.array, list, pd.array])
    @pytest.mark.parametrize("n", [1, 2, 3])
    def test_setitem_slice_broadcasting_rhs_mixed_dtypes(self, n, box,
    indexer):
        # GH#40440
        df = DataFrame(
            [[1, 3, 5], ["x", "y", "z"]] + [[2, 4, 6]] * n, columns=["a", "b",
    "c"]
        )
        indexer(df)[1:] = box([10, 11, 12])
        expected = DataFrame(
...
class TestDataFrameSetItemBooleanMask:
    ...
    @pytest.mark.parametrize(
        "mask_type",
        [lambda df: df > np.abs(df) / 2, lambda df: (df > np.abs(df) /
    2).values],
        ids=["dataframe", "array"],
    )
    ...
    @pytest.mark.xfail(reason="Currently empty indexers are treated as all
    False")
    ...
    def test_setitem_loc_empty_indexer_raises_with_non_empty_value(self, box):
        ...
        if box == Series:
            ...
            indexer = box([])
        msg = "Must have equal len keys and value when setting with an
    iterable"
        with pytest.raises(ValueError, match=msg):
            df.loc[indexer, ["b"]] = [1]
    ...
    @pytest.mark.parametrize("kwargs", [{}, {"index": [1]}, {"columns":
    ["A"]}])
    ...
    dtype=float
        ...
        df2 = DataFrame(index=idx, columns=cols, data=np.ones((len(idx),
    len(cols))))

        expected = DataFrame(
            index=idx,
            columns=cols,
            data=np.array([[0.0, 0.5, 1.0], [1.5, 2.0, -1], [-1, -1, -1]],
    dtype=float),
        )
...
class TestDataFrameSetitemCopyViewSemantics:
    ...
    def test_setitem_partial_column_inplace(
        ...
        df = DataFrame(
            {"x": [1.1, 2.1, 3.1, 4.1], "y": [5.1, 6.1, 7.1, 8.1]}, index=[0,
    1, 2, 3]
        )
    ...
    @pytest.mark.parametrize("value", [1.0, np.array([[1.0], [1.0]]), [[1.0],
    [1.0]]])
    ...
    def test_setitem_column_update_inplace(self, using_copy_on_write):
        ...
        df = DataFrame({col: np.zeros(len(labels)) for col in labels},
    index=labels)
