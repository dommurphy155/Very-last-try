...
class TestTimeConversionFormats:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                "%d/%m/%Y",
                [Timestamp("20000101"), Timestamp("20000201"),
    Timestamp("20000301")],
            ],
            [
                "%m/%d/%Y",
                [Timestamp("20000101"), Timestamp("20000102"),
    Timestamp("20000103")],
            ],
        ],
    )
    ...
    cache=cache)
        expected = Series(
            [datetime(2012, 12, 31), datetime(2014, 12, 31), datetime(9999,
    12, 31)],
            dtype=object,
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_to_datetime_format_YYYYMMDD_coercion(self, cache):
        ...
        # GH 7930
        ser = Series([20121231, 20141231, 99991231])
        result = to_datetime(ser, format="%Y%m%d", errors="coerce",
    cache=cache)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (["201010", pd.NA], "%y%m%d", DatetimeIndex(["2020-10-10",
    "NaT"])),
            (["201010", pd.NA], "%d%m%y", DatetimeIndex(["2010-10-20",
    "NaT"])),
            (["201010", pd.NA], None, DatetimeIndex(["2010-10-20", "NaT"])),
            ([None, np.nan, pd.NA], None, DatetimeIndex(["NaT", "NaT",
    "NaT"])),
            ([None, np.nan, pd.NA], "%Y%m%d", DatetimeIndex(["NaT", "NaT",
    "NaT"])),
        ],
    )
    ...
    def test_to_datetime_format_integer(self, cache):
        ...
        expected = Series([Timestamp(x[:4] + "-" + x[4:]) for x in
    ser.apply(str)])
    ...
    > 8
            ...
            [20129930, 20129930],
            # short integer date partially parsed to datetime(2012,9,9),
    length < 8
            [2012993, 2012993],
    ...
    @pytest.mark.parametrize(
        "value, format, dt",
        [
            ["01/10/2010 15:20", "%m/%d/%Y %H:%M", Timestamp("2010-01-10
    15:20")],
            ["01/10/2010 05:43", "%m/%d/%Y %I:%M", Timestamp("2010-01-10
    05:43")],
            [
                "01/10/2010 13:56:01",
                "%m/%d/%Y %H:%M:%S",
            ...
            pytest.param(
                ...
                marks=pytest.mark.xfail(
                    ...
                    reason="fail on a CI build with
    ...
    def test_to_datetime_with_non_exact(self, cache):
        ...
        # exact kw
        ser = Series(
            ["19MAY11", "foobar19MAY11", "19MAY11:00:00:00", "19MAY11
    00:00:00Z"]
        )
        result = to_datetime(ser, format="%d%b%y", exact=False, cache=cache)
        expected = to_datetime(
            ser.str.extract(r"(\d+\w+\d+)", expand=False), format="%d%b%y",
    cache=cache
        )
        tm.assert_series_equal(result, expected)
...
class TestToDatetime:
    ...
    def test_to_datetime_now(self):
        ...
        with tm.set_timezone("US/Eastern"):
            ...
            with tm.assert_produces_warning(
                ...
                # checking stacklevel is tricky because we go through cython
    code
                # GH#18705
    ...
    def test_to_datetime_today(self, tz):
        ...
        with tm.set_timezone(tz):
            nptoday =
    np.datetime64("today").astype("datetime64[ns]").astype(np.int64)
    ...
    def test_to_datetime_utc_true(self, cache, init_constructor,
    end_constructor):
        ...
        result = to_datetime(
            init_constructor(data), format="%Y%m%d %H%M%S", utc=True,
    cache=cache
        )
    ...
    def test_to_datetime_utc_true_scalar(self, cache, scalar, expected):
        ...
        result = to_datetime(scalar, format="%Y%m%d %H%M%S", utc=True,
    cache=cache)
        assert result == expected
    ...
    dtype):
        ...
        result = to_datetime(Series([date], dtype=dtype), utc=True,
    cache=cache)
        tm.assert_series_equal(result, expected)
    ...
    def test_to_datetime_tz_psycopg2(self, request, cache):
        ...
        request.node.add_marker(
            pytest.mark.xfail(
                ...
                reason="psycopg2.tz is deprecated (and appears dropped) in
    2.9",
            )
        )
        ...
        expected = DatetimeIndex(["2000-01-01 13:00:00"],
    dtype="datetime64[ns, UTC]")
        tm.assert_index_equal(result, expected)
    ...
    def test_datetime_bool_arrays_mixed(self, cache):
        ...
        tm.assert_index_equal(
            ...
            DatetimeIndex(
                [to_datetime(0, cache=cache), NaT, NaT, to_datetime(0,
    cache=cache)]
            ),
        )
    ...
    def test_datetime_invalid_scalar(self, value, format, infer):
        ...
        with pytest.raises(ValueError, match=msg):
            to_datetime(
                value, errors="raise", format=format,
    ...
    def test_datetime_outofbounds_scalar(self, value, format, infer):
        ...
        if format is not None:
            msg = "is a bad directive in format|Out of bounds .* present at
    position 0"
            with pytest.raises(ValueError, match=msg):
                to_datetime(
                    value, errors="raise", format=format,
    ...
    @pytest.mark.parametrize("values", [["a"], ["00:01:99"], ["a", "b",
    "99:00:00"]])
    @pytest.mark.parametrize("infer", [True, False])
    @pytest.mark.parametrize("format", [None, "H%:M%:S%"])
    def test_datetime_invalid_index(self, values, format, infer):
        ...
        with pytest.raises(ValueError, match=msg):
            to_datetime(
                values, errors="raise", format=format,
    infer_datetime_format=infer
            )

    @pytest.mark.parametrize("utc", [True, None])
    @pytest.mark.parametrize("format", ["%Y%m%d %H:%M:%S", None])
    @pytest.mark.parametrize("constructor", [list, tuple, np.array, Index,
    deque])
    ...
    @pytest.mark.parametrize(
        ...
        (
            (
                Series([NaT] * 20 + [None] * 20, dtype="object"),  # type:
    ignore[list-item] # noqa: E501
                Series([NaT] * 40, dtype="datetime64[ns]"),
            ),
            (
                Series([NaT] * 60 + [None] * 60, dtype="object"),  # type:
    ignore[list-item] # noqa: E501
            ...
            (Series([np.NaN] * 20), Series([NaT] * 20,
    dtype="datetime64[ns]")),
            (Series([np.NaN] * 60), Series([NaT] * 60,
    dtype="datetime64[ns]")),
        ),
    )
    ...
    def test_iso_8601_strings_with_different_offsets(self):
        # GH 17697, 11736
        ts_strings = ["2015-11-18 15:30:00+05:30", "2015-11-18
    16:30:00+06:30", NaT]
    ...
    @pytest.mark.parametrize("dt_str", ["00010101", "13000101", "30000101",
    "99990101"])
...
class TestToDatetimeUnit:
    ...
    def test_unit_array_mixed_nans(self, cache):
        ...
        expected = DatetimeIndex(
            ["NaT", "1970-01-02", "1970-01-02", "NaT", "NaT", "NaT", "NaT",
    "NaT"]
        )
        tm.assert_index_equal(result, expected)
    ...
    def test_unit_with_numeric(self, cache, errors, dtype):
        ...
        expected = DatetimeIndex(["2015-06-19 05:33:20", "2015-05-27
    22:33:20"])
    ...
    def test_to_datetime_unit(self, dtype):
        ...
        expected = Series(
            [Timestamp("2013-06-09 02:42:28") + timedelta(seconds=t) for t in
    range(20)]
        )
        tm.assert_series_equal(result, expected)
...
class TestToDatetimeDataFrame:
    ...
    def test_dataframe_field_aliases_column_subset(self, df, cache, unit):
        # unit mappings
        result = to_datetime(df[list(unit.keys())].rename(columns=unit),
    cache=cache)
        expected = Series(
    ...
    def test_dataframe_coerce(self, cache):
        # passing coerce
        df2 = DataFrame({"year": [2015, 2016], "month": [2, 20], "day": [4,
    5]})
    ...
    def test_dataframe_extra_keys_raisesm(self, df, cache):
        # extra columns
        msg = r"extra keys have been passed to the datetime assemblage:
    \[foo\]"
        with pytest.raises(ValueError, match=msg):
            df2 = df.copy()
            df2["foo"] = 1
    ...
    5]})
        ...
        df2 = DataFrame(
            {"year": [2015, 2016], "month": [2, 20], "day": [4, 5], "hour":
    [4, 5]}
        )
    ...
    def test_dataframe_float(self, cache):
        # float
        df = DataFrame({"year": [2000, 2001], "month": [1.5, 1], "day": [1,
    1]})
...
class TestToDatetimeMisc:
    ...
    @pytest.mark.xfail(reason="fails to enforce dayfirst=True, which would
    raise")
    def test_to_datetime_respects_dayfirst(self, cache):
        ...
        with pytest.raises(ValueError, match=msg):
            # if dayfirst is respected, then this would parse as month=13,
    which
            #  would raise
            with tm.assert_produces_warning(UserWarning, match="Provide
    format"):
    ...
    def test_to_datetime_with_space_in_series(self, cache):
        ...
        expected_coerce = Series([datetime(2006, 10, 18), datetime(2008, 10,
    18), NaT])
    ...
    def test_to_datetime_with_apply_with_empty_str(self, cache):
        ...
        expected = to_datetime(td, format="%b %y", errors="coerce",
    cache=cache)

        result = td.apply(
            lambda x: to_datetime(x, format="%b %y", errors="coerce",
    cache=cache)
        )
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize("result", [Timestamp("2012"),
    to_datetime("2012")])
    ...
    def test_string_na_nat_conversion_with_name(self, cache):
        idx = ["a", "b", "c", "d", "e"]
        series = Series(
            ["1/1/2000", np.nan, "1/3/2000", np.nan, "1/5/2000"], index=idx,
    name="foo"
        )
        dseries = Series(
            [
    ...
    def test_dti_constructor_numpy_timeunits(self, cache, dtype):
        # GH 9114
        base = to_datetime(["2000-01-01T00:00", "2000-01-02T00:00", "NaT"],
    cache=cache)
    ...
    def test_dayfirst(self, cache):
        ...
        expected = DatetimeIndex(
            [datetime(2014, 2, 10), datetime(2014, 2, 11), datetime(2014, 2,
    12)]
        )
    ...
    def test_dayfirst_warnings_valid_input(self):
        ...
        warning_msg_day_first = (
            r"Parsing dates in DD/MM/YYYY format when dayfirst=False \(the
    default\) "
            "was specified. This may lead to inconsistently parsed dates!
    Specify a "
...
class TestGuessDatetimeFormat:
    ...
    def test_guess_datetime_format_for_array(self, test_array):
        ...
        assert tools._guess_datetime_format_for_array(test_array) ==
    expected_format
...
class TestToDatetimeInferFormat:
    @pytest.mark.parametrize(
        "test_format", ["%m-%d-%Y", "%m/%d/%Y %H:%M:%S.%f",
    "%Y-%m-%dT%H:%M:%S.%f"]
    )
    def test_to_datetime_infer_datetime_format_consistent_format(
        ...
        with_format = to_datetime(s_as_dt_strings, format=test_format,
    cache=cache)
    ...
    08:07:13").tz_localize(pytz.FixedOffset(offset))]
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_to_datetime_iso8601_noleading_0s(self, cache, format):
        ...
        tm.assert_series_equal(to_datetime(ser, format=format, cache=cache),
    expected)
...
class TestDaysInMonth:
    ...
    def test_day_not_in_month_coerce(self, cache, arg, format):
        assert isna(to_datetime(arg, errors="coerce", format=format,
    cache=cache))
    ...
    def test_day_not_in_month_raise_value(self, cache, arg):
        msg = f'time data "{arg}" at position 0 doesn\'t match format
    specified'
        with pytest.raises(ValueError, match=msg):
    ...
    def test_day_not_in_month_ignore(self, cache, expected, format):
        result = to_datetime(expected, errors="ignore", format=format,
    cache=cache)
        assert result == expected
...
class TestDatetimeParsingWrappers:
    ...
    def test_parsers(self, date_str, expected, cache):
        ...
        result4 = to_datetime(
            np.array([date_str], dtype=object), yearfirst=yearfirst,
    cache=cache
        )
    ...
    @pytest.mark.parametrize(
        "date_str, exp_def",
        [["10:15", datetime(1, 1, 1, 10, 15)], ["9:05", datetime(1, 1, 1, 9,
    5)]],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "Timestamp('2013-01-01 05:45:00+0545',
    tz='pytz.FixedOffset(345)')",
            ),
            (
                "2013-01-01 05:30+0530",
                pytz.FixedOffset(330),
                "Timestamp('2013-01-01 05:30:00+0530',
...
class TestOrigin:
    ...
    def test_epoch(self, units, epochs, epoch_1960, units_from_epochs):
        ...
        expected = Series(
            [pd.Timedelta(x, unit=units) + epoch_1960 for x in
    units_from_epochs]
        )

        result = Series(to_datetime(units_from_epochs, unit=units,
    origin=epochs))
        tm.assert_series_equal(result, expected)
    ...
    def test_invalid_origins(self, origin, exc, units, units_from_epochs):
        ...
        msg = f"origin {origin} (is Out of Bounds|cannot be converted to a
    Timestamp)"
    ...
    def test_invalid_origins_tzinfo(self):
        ...
        with pytest.raises(ValueError, match="must be tz-naive"):
            to_datetime(1, unit="D", origin=datetime(2000, 1, 1,
    tzinfo=pytz.utc))

    def test_incorrect_value_exception(self):
        # GH47495
        with pytest.raises(
            ValueError, match="Unknown string format: yesterday present at
    position 1"
        ):
...
class TestShouldCache:
    ...
    def test_should_cache(self, listlike, do_caching):
        assert (
            tools.should_cache(listlike, check_count=len(listlike),
    unique_share=0.7)
            == do_caching
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (0.5, 11, r"check_count must be in next bounds: \[0;
    len\(arg\)\]"),
...
def test_empty_string_datetime_coerce__unit():
    ...
    expected = DatetimeIndex(["1970-01-01 00:00:01", "NaT"],
    dtype="datetime64[ns]")
...
def test_to_datetime_monotonic_increasing_index(cache):
    ...
    times = times.to_frame(index=False, name="DT").sample(n=cstart,
    random_state=1)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [datetime(1724, 12, 20, 20, 20, 20, tzinfo=timezone.utc),
    datetime(2022, 1, 1)],
    ],
