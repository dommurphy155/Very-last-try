...
@pytest.mark.parametrize(
    "errors,exp_data", [("ignore", [1, -3.14, "apple"]), ("coerce", [1, -3.14,
    np.nan])]
)
...
def test_numeric_df_columns(columns):
    ...
    df = DataFrame(
        {
            "a": [1.2, decimal.Decimal(3.14), decimal.Decimal("infinity"),
    "0.1"],
            "b": [1.0, 2.0, 3.0, 4.0],
        }
    )

    expected = DataFrame({"a": [1.2, 3.14, np.inf, 0.1], "b": [1.0, 2.0, 3.0,
    4.0]})
...
def test_really_large_scalar(large_val, signed, transform, errors):
    ...
    if val_is_string and errors in (None, "raise"):
        ...
        expected = float(val) if (errors == "coerce" and val_is_string) else
    val
        tm.assert_almost_equal(to_numeric(val, **kwargs), expected)


def test_really_large_in_arr(large_val, signed, transform, multiple_elts,
    errors):
    # see gh-24910
    kwargs = {"errors": errors} if errors is not None else {}
    val = -large_val if signed else large_val
...
def test_really_large_in_arr_consistent(large_val, signed, multiple_elts,
    errors):
    # see gh-24910
    #
...
def test_period(request, transform_assert_equal):
    ...
    if not isinstance(inp, Index):
        request.node.add_marker(
            pytest.mark.xfail(reason="Missing PeriodDtype support in
    to_numeric")
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("uint32", "unsigned", [iinfo(np.uint16).min, iinfo(np.uint16).max +
    1]),
        ("uint64", "unsigned", [iinfo(np.uint32).min, iinfo(np.uint32).max +
    1]),
    ],
)
...
def test_downcast_float64_to_float32():
    ...
    series = Series([16777217.0, np.finfo(np.float64).max, np.nan],
    dtype=np.float64)
...
def test_to_numeric_from_nullable_string(values, nullable_string_dtype,
    expected):
