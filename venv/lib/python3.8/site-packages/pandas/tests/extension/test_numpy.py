...
def _assert_attr_equal(attr: str, left, right, obj: str = "Attributes"):
    ...
    if attr == "dtype":
        ...
        if isinstance(lattr, PandasDtype) and not isinstance(rattr,
    PandasDtype):
            left = left.astype(lattr.numpy_dtype)
        elif isinstance(rattr, PandasDtype) and not isinstance(lattr,
    PandasDtype):
...
class TestGroupby(BaseNumPyTests, base.BaseGroupbyTests):
    def test_groupby_extension_apply(self, data_for_grouping,
    groupby_apply_op):
...
class TestMethods(BaseNumPyTests, base.BaseMethodsTests):
    ...
    def test_insert(self, data, request):
        if data.dtype.numpy_dtype == object:
            mark = pytest.mark.xfail(reason="Dimension mismatch in
    np.concatenate")
            request.node.add_marker(mark)
...
class TestReshaping(BaseNumPyTests, base.BaseReshapingTests):
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                False,
                marks=pytest.mark.xfail(reason="PandasArray inconsistently
    extracted"),
            ),
        ],
    )
...
class TestSetitem(BaseNumPyTests, base.BaseSetitemTests):
    ...
    @pytest.mark.parametrize(
        ...
        ids=["list-False", "list-True", "integer-array-False",
    "integer-array-True"],
    )
    def test_setitem_integer_with_missing_raises(self, data, idx,
    box_in_series):
        super().test_setitem_integer_with_missing_raises(data, idx,
    box_in_series)
    ...
    def test_setitem_with_expansion_dataframe_column(self, data, full_indexer):
        ...
        # base class method has expected = df; PandasArray behaves oddly
    because
