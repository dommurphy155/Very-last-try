...
@pytest.mark.xfail(
    reason=(
        "DecimalArray constructor raises bc _from_sequence wants Decimals, not
    ints."
...
def test_combine_from_sequence_raises(monkeypatch):
    ...
    monkeypatch.setattr(DecimalDtype, "construct_array_type",
...
def test_groupby_agg():
    ...
    tm.assert_series_equal(result, expected, check_names=False)
    result = df["decimals"].groupby([df["id1"], df["id2"]]).agg(lambda x:
    x.iloc[0])
    tm.assert_series_equal(result, expected, check_names=False)

    # multiple columns
    expected = pd.DataFrame({"id2": [0, 1], "decimals": to_decimal([data[0],
    data[3]])})
...
def test_groupby_agg_ea_method(monkeypatch):
    ...
    monkeypatch.setattr(DecimalArray, "my_sum", DecimalArray__my_sum,
    raising=False)

    data = make_data()[:5]
    df = pd.DataFrame({"id": [0, 0, 0, 1, 1], "decimals": DecimalArray(data)})
    expected = pd.Series(to_decimal([data[0] + data[1] + data[2], data[3] +
    data[4]]))

    result = df.groupby("id")["decimals"].agg(lambda x: x.values.my_sum())
    tm.assert_series_equal(result, expected, check_names=False)
    s = pd.Series(DecimalArray(data))
    result = s.groupby(np.array([0, 0, 0, 1, 1])).agg(lambda x:
    x.values.my_sum())
    tm.assert_series_equal(result, expected, check_names=False)
...
def test_indexing_no_materialize(monkeypatch):
    ...
    monkeypatch.setattr(DecimalArray, "__array__", DecimalArray__array__,
    raising=False)
