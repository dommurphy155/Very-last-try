...
class BaseArithmeticOpsTests(BaseOpsUtil):
    ...
    * divmod_exc = TypeError
    ...
    def test_arith_series_with_array(self, data, all_arithmetic_operators):
        ...
        ser = pd.Series(data)
        self.check_opname(
            ser, op_name, pd.Series([ser.iloc[0]] * len(ser)),
...
class BaseComparisonOpsTests(BaseOpsUtil):
    ...
    @pytest.mark.parametrize("box", [pd.Series, pd.DataFrame])
    def test_direct_arith_with_ndframe_returns_not_implemented(self, data,
    box):
        # EAs should return NotImplemented for ops with Series/DataFrame
        # Pandas takes care of unboxing the series and calling the EA's op.
        other = pd.Series(data)
        ...
        if hasattr(data, "__eq__"):
            ...
            raise pytest.skip(f"{type(data).__name__} does not implement
    __eq__")
...
class BaseUnaryOpsTests(BaseOpsUtil):
    ...
    def test_unary_ufunc_dunder_equivalence(self, data, ufunc):
        ...
        attr = {np.positive: "__pos__", np.negative: "__neg__", np.abs:
    "__abs__"}[
            ufunc
        ]
