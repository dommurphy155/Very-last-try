...
class BaseMethodsTests(BaseExtensionTests):
    ...
    def test_value_counts_with_normalize(self, data):
        ...
        if not isinstance(data, pd.Categorical):
            expected = pd.Series([1 / len(values)] * len(values),
    index=result.index)
        else:
    ...
    def test_argmin_argmax_all_na(self, method, data, na_value):
        ...
        err_msg = "attempt to get"
        data_na = type(data)._from_sequence([na_value, na_value],
    dtype=data.dtype)
        with pytest.raises(ValueError, match=err_msg):
    ...
    def test_sort_values_frame(self, data_for_sorting, ascending):
        ...
        expected = pd.DataFrame(
            {"A": [1, 1, 2], "B": data_for_sorting.take([2, 0, 1])}, index=[2,
    0, 1]
        )
    ...
    deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            codes, uniques = pd.factorize(data_for_grouping,
    ...
    deprecated"
        with tm.assert_produces_warning(FutureWarning, match=msg):
            ...
            codes_2, uniques_2 =
    data_for_grouping.factorize(na_sentinel=na_sentinel)
    ...
    @pytest.mark.parametrize(
        "periods, indices",
        [(-2, [2, 3, 4, -1, -1]), (0, [0, 1, 2, 3, 4]), (2, [-1, -1, 0, 1,
    2])],
    )
    ...
    def test_equals(self, data, na_value, as_series, box):
        data2 = type(data)._from_sequence([data[0]] * len(data),
    dtype=data.dtype)
        data_na = type(data)._from_sequence([na_value] * len(data),
    dtype=data.dtype)
