...
def data(dtype):
    ...
    99.5]
        ...
        data = (
            [datetime(2020, 1, 1, 1, 1, 1, 1), datetime(1999, 1, 1, 1, 1, 1,
    1)] * 4
            + [None]
...
class TestConstructors(base.BaseConstructorsTests):
            ...
            if pa_version_under2p0:
                ...
                request.node.add_marker(
                    pytest.mark.xfail(
                        ...
                        reason=f"pyarrow.type_for_alias cannot infer
    {pa_dtype}",
                    )
                )
        ...
        if pa_version_under6p0:
            request.node.add_marker(
                pytest.mark.xfail(
                    ...
                    reason="month_day_nano_interval not implemented by
    pyarrow.",
                )
            )
    ...
    def test_from_sequence_of_strings_pa_array(self, data, request):
        ...
        if pa_version_under3p0:
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    reason="Iterating over ChunkedArray[bool] returns PyArrow
    scalars.",
                )
            )
            ...
            if pa_version_under7p0:
                request.node.add_marker(
                    pytest.mark.xfail(
                        raises=pa.ArrowNotImplementedError,
                        reason=f"pyarrow doesn't support string cast from
    {pa_dtype}",
                    )
                )
    ...
    {pa_dtype}",
        ...
        result = type(data)._from_sequence_of_strings(pa_array,
    dtype=data.dtype)
        tm.assert_extension_array_equal(result, data)
...
class TestGetitemTests(base.BaseGetitemTests):
    ...
    def test_take_series(self, request, data):
        ...
        if pa_version_under3p0 and tz not in (None, "UTC") and unit in
    bad_units:
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=(
    ...
    def test_loc_iloc_frame_single_dtype(self, request, using_array_manager,
    data):
        tz = getattr(data.dtype.pyarrow_dtype, "tz", None)
        unit = getattr(data.dtype.pyarrow_dtype, "unit", None)
        bad_units = ["ns"]
...
class TestBaseNumericReduce(base.BaseNumericReduceTests):
    def check_reduce(self, ser, op_name, skipna):
        ...
        if pa.types.is_boolean(pa_dtype):
            ...
            pytest.skip(
                "pandas boolean data with NA does not fully support all
    reductions"
            )
    ...
    request):
        ...
        if all_numeric_reductions in {"skew", "kurt"}:
            ...
            all_numeric_reductions in {"median", "var", "std", "prod", "max",
    "min"}
            ...
            and skipna is False
            and pa_version_under6p0
            and (pa.types.is_integer(pa_dtype) or
    pa.types.is_floating(pa_dtype))
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    ...
                    reason=(
                        f"{all_numeric_reductions} with skip_nulls={skipna}
    did not "
                        f"return NA for {pa_dtype} with
    pyarrow={pa.__version__}"
            ...
            and (pa.types.is_temporal(pa_dtype) and not
...
class TestBaseGroupby(base.BaseGroupbyTests):
    def test_groupby_agg_extension(self, data_for_grouping, request):
        ...
        if pa_version_under2p0 and tz not in (None, "UTC"):
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=f"Not supported by pyarrow < 2.0 with timestamp
    type {tz}."
                )
            )
...
class TestBaseMissing(base.BaseMissingTests):
    @pytest.mark.filterwarnings("ignore:Falling
    back:pandas.errors.PerformanceWarning")
...
class TestBaseReshaping(base.BaseReshapingTests):
    ...
    @pytest.mark.xfail(reason="GH 45419: pyarrow.ChunkedArray does not support
    views")
...
class TestBaseSetitem(base.BaseSetitemTests):
    ...
    request):
        ...
        tz = getattr(data.dtype.pyarrow_dtype, "tz", None)
        if pa_version_under2p0 and tz not in (None, "UTC") and not
    is_null_slice:
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=f"Not supported by pyarrow < 2.0 with timestamp
    type {tz}"
        ...
        super().test_setitem_with_expansion_dataframe_column(data,
    full_indexer)
    ...
    def test_EA_types(self, engine, data, request):
        ...
        if pa.types.is_boolean(pa_dtype):
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=NotImplementedError,
                    reason=f"Parameterized types with tz={pa_dtype.tz} not
    supported.",
                )
            )
...
class TestBaseMethods(base.BaseMethodsTests):
        ...
        if pa.types.is_unsigned_integer(pa_dtype) and periods == 1:
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=pa.ArrowInvalid,
                    reason=(
                        f"diff with {pa_dtype} and periods={periods} will
    overflow"
                    ),
                )
            )
    ...
    def test_argreduce_series(
        ...
        elif not pa_version_under6p0 and pa.types.is_duration(pa_dtype) and
    skipna:
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=pa.ArrowNotImplementedError,
    ...
    request):
        pa_dtype = data_for_sorting.dtype.pyarrow_dtype
        if pa.types.is_duration(pa_dtype) and not ascending and not
    pa_version_under2p0:
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=pa.ArrowNotImplementedError,
    ...
    def test_factorize(self, data_for_grouping, na_sentinel, request):
        ...
        if pa.types.is_duration(pa_dtype):
            request.node.add_marker(
                pytest.mark.xfail(
                    raises=pa.ArrowNotImplementedError,
                    reason=f"dictionary_encode has no pyarrow kernel for
    {pa_dtype}",
                )
            )
        elif pa.types.is_boolean(pa_dtype):
...
class TestBaseArithmeticOps(base.BaseArithmeticOpsTests):
    ...
    def _patch_combine(self, obj, other, op):
        ...
        pa_array =
    pa.array(expected_data._values).cast(original_dtype.pyarrow_dtype)
    ...
    def test_arith_series_with_scalar(
        ...
        if (
            ...
            and (pa.types.is_floating(pa_dtype) or
    pa.types.is_integer(pa_dtype))
            ...
            request.node.add_marker(
                pytest.mark.xfail(
                    reason=(
                        f"GH 29997: 1**pandas.NA == 1 while 1**pyarrow.NA ==
    NULL "
                        f"for {pa_dtype}"
                    )
                )
    ...
    pa.types.is_integer(pa_dtype))
        ...
        if all_arithmetic_operators == "__floordiv__" and
    pa.types.is_integer(pa_dtype):
            # BaseOpsUtil._combine always returns int64, while
    ArrowExtensionArray does
            # not upcast
            monkeypatch.setattr(TestBaseArithmeticOps, "_combine",
    self._patch_combine)
    ...
    def test_arith_frame_with_scalar(
        ...
        elif not (pa.types.is_floating(pa_dtype) or
    pa.types.is_integer(pa_dtype)):
    ...
    pa.types.is_integer(pa_dtype))
        ...
        ser = pd.Series(data)
        # pd.Series([ser.iloc[0]] * len(ser)) may not return
    ArrowExtensionArray
        # since ser.iloc[0] is a python scalar
        other = pd.Series(pd.array([ser.iloc[0]] * len(ser), dtype=data.dtype))
        if pa.types.is_floating(pa_dtype) or (
            pa.types.is_integer(pa_dtype) and all_arithmetic_operators !=
    "__truediv__"
        ):
            monkeypatch.setattr(TestBaseArithmeticOps, "_combine",
    self._patch_combine)
...
class TestBaseComparisonOps(base.BaseComparisonOpsTests):
    ...
    def test_invalid_other_comp(self, data, comparison_op):
        # GH 48833
        with pytest.raises(
            NotImplementedError, match=".* not implemented for <class
    'object'>"
        ):
