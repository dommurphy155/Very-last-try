...
class DateArray(ExtensionArray):
    ...
    def __len__(self) -> int:
        return len(self._year)  # all 3 arrays are enforced to have the same
    length

    def __getitem__(self, item: PositionalIndexer):
        if isinstance(item, int):
            return dt.date(self._year[item], self._month[item],
    self._day[item])
        else:
    ...
    def copy(self) -> "DateArray":
        return DateArray((self._year.copy(), self._month.copy(),
    self._day.copy()))

    def isna(self) -> np.ndarray:
        return np.logical_and(
            np.logical_and(
                self._year == dt.date.min.year, self._month ==
    dt.date.min.month
            ),
    ...
    @classmethod
    def _from_sequence(cls, scalars, *, dtype: Optional[Dtype] = None,
    copy=False):
