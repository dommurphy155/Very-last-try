...
def test_frame_equal_row_order_mismatch(check_like, obj_fixture):
    ...
    if not check_like:  # Do not ignore row-column orderings.
        ...
        with pytest.raises(AssertionError, match=msg):
            tm.assert_frame_equal(df1, df2, check_like=check_like,
    obj=obj_fixture)
    else:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DataFrame({"A": [1, 2, 3], "B": [4, 5, 6]}), DataFrame({"A": [1, 2,
    3]})),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        # Index
        (
            DataFrame.from_records({"a": [1, 2], "c": ["l1", "l2"]},
    index=["a"]),
            DataFrame.from_records({"a": [1.0, 2.0], "c": ["l1", "l2"]},
    index=["a"]),
        ...
        (
            DataFrame.from_records(
                {"a": [1, 2], "b": [2.1, 1.5], "c": ["l1", "l2"]}, index=["a",
    "b"]
            ),
            DataFrame.from_records(
...
{obj}\\.iloc\\[:, 1\\] \\(column name="B"\\) values are different
    \\(33\\.33333 %\\)
\\[index\\]: \\[0, 1, 2\\]
\\[left\\]:  \\[4, 5, 6\\]
\\[right\\]: \\[4, 5, 7\\]"""
    ...
    with pytest.raises(AssertionError, match=msg):
        tm.assert_frame_equal(df1, df2, by_blocks=by_blocks_fixture,
    obj=obj_fixture)
...
{obj}\\.iloc\\[:, 1\\] \\(column name="E"\\) values are different
    \\(33\\.33333 %\\)
\\[index\\]: \\[0, 1, 2\\]
\\[left\\]:  \\[é, è, ë\\]
\\[right\\]: \\[é, è, e̊\\]""",
...
def test_assert_frame_equal_columns_mixed_dtype():
    # GH#39168
    df = DataFrame([[0, 1, 2]], columns=["foo", "bar", 42], index=[1, "test",
    2])
...
def test_frame_equal_mixed_dtypes(frame_or_series, any_numeric_ea_dtype,
    indexer):
    ...
    msg = r'(Series|DataFrame.iloc\[:, 0\] \(column name="0"\) classes) are
    different'
    with pytest.raises(AssertionError, match=msg):
