...
def test_hash_tuples():
    ...
    msg = "|".join(["object is not iterable", "zip argument #1 must support
    iteration"])
    with pytest.raises(TypeError, match=msg):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        MultiIndex.from_product(
            [range(5), ["foo", "bar", "baz"], pd.date_range("20130101",
    periods=2)]
        ),
...
def test_categorical_with_nan_consistency():
    c = pd.Categorical.from_codes(
        [-1, 0, 1, 2, 3, 4], categories=pd.date_range("2012-01-01", periods=5,
    name="B")
    )
...
def test_hash_collisions():
    ...
    tm.assert_numpy_array_equal(result, np.concatenate([expected1, expected2],
    axis=0))
...
@pytest.mark.parametrize(
    "data, result_data",
    [
        [[tuple("1"), tuple("2")], [10345501319357378243,
    8331063931016360761]],
...
def test_hash_object_none_key():
    ...
    expected = Series([4578374827886788867, 17338122309987883691],
    dtype="uint64")
    tm.assert_series_equal(result, expected)
