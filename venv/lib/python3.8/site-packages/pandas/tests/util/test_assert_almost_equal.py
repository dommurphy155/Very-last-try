...
def _assert_not_almost_equal(a, b, **kwargs):
    ...
    **kwargs
    ...
    try:
        tm.assert_almost_equal(a, b, **kwargs)
        msg = f"{a} and {b} were approximately equal when they shouldn't have
    been"
        pytest.fail(msg=msg)
...
@pytest.mark.parametrize("a,b", [(1.1, 1.11), (0.1, 0.101), (0.000011,
    0.001012)])
...
def test_assert_not_almost_equal_complex_numbers(a, b, rtol):
    ...
    _assert_not_almost_equal_both(np.complex128(a), np.complex128(b),
    rtol=rtol)


@pytest.mark.parametrize("a,b", [(0, 0), (0, 0.0), (0, np.float64(0)),
    (0.00000001, 0)])
...
def test_assert_almost_equal_dict_like_object(val):
    ...
    func = (
        _assert_almost_equal_both if val == dict_val else
...
@pytest.mark.parametrize(
    ...
    [
        (np.inf, np.inf),
        (np.inf, float("inf")),
        (np.array([np.inf, np.nan, -np.inf]), np.array([np.inf, np.nan,
    -np.inf])),
        (
...
@pytest.mark.parametrize(
    "a,b,klass1,klass2",
    [(np.array([1]), 1, "ndarray", "int"), (1, np.array([1]), "int",
    "ndarray")],
)
...
\\[right\\]: \\[1\\.0, nan, 3\\.0\\]"""
    ...
    with pytest.raises(AssertionError, match=msg):
        tm.assert_almost_equal(np.array([np.nan, 2, 3]), np.array([1, np.nan,
    3]))
...
\\[right\\]: \\[\\[1, 3\\], \\[3, 4\\], \\[5, 6\\]\\]"""
    ...
    with pytest.raises(AssertionError, match=msg):
        tm.assert_almost_equal(
            np.array([[1, 2], [3, 4], [5, 6]]), np.array([[1, 3], [3, 4], [5,
    6]])
        )
...
\\[right\\]: \\[\\[1, 3\\], \\[3, 4\\]\\]"""
    ...
    with pytest.raises(AssertionError, match=msg):
        tm.assert_almost_equal(np.array([[1, 2], [3, 4]]), np.array([[1, 3],
    [3, 4]]))
...
\\[right\\]: \\(3L*,\\)"""
    with pytest.raises(AssertionError, match=msg):
        tm.assert_almost_equal(np.array([1, 2]), np.array([3, 4, 5]),
    obj="Index")
...
\\[right\\]: \\[á, à, å\\]"""
    ...
    with pytest.raises(AssertionError, match=msg):
        tm.assert_almost_equal(np.array(["á", "à", "ä"]), np.array(["á", "à",
    "å"]))
...
subarr[:] = [np.array([None, "b"], dtype=object), np.array(["c", "d"],
    dtype=object)]
...
NESTED_CASES = [
    ...
    (
        np.array(
            [
                np.array([np.array([1, 2, 3]), np.array([4, 5])],
    dtype=object),
                np.array(
    ...
    # dicts
    (
        np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)}],
    dtype=object),
        np.array([{"f1": 1, "f2": np.array(["a", "b"], dtype=object)}],
    dtype=object),
    ),
