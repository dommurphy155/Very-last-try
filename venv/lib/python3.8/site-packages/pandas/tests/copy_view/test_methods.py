...
def test_copy_shallow(using_copy_on_write):
    ...
    if using_copy_on_write:
        ...
        assert not np.shares_memory(get_array(df_copy, "a"), get_array(df,
    "a"))
...
def test_reset_index(using_copy_on_write):
    ...
    # resulting dataframe
    df = DataFrame(
        {"a": [1, 2, 3], "b": [4, 5, 6], "c": [0.1, 0.2, 0.3]}, index=[10, 11,
    12]
    )
    df_orig = df.copy()
    df2 = df.reset_index()
...
def test_rename_columns(using_copy_on_write):
    ...
    if using_copy_on_write:
        assert np.shares_memory(get_array(df2, "C"), get_array(df, "c"))
    expected = DataFrame({"A": [0, 2, 3], "B": [4, 5, 6], "C": [0.1, 0.2,
    0.3]})
...
def test_chained_methods(request, method, idx, using_copy_on_write):
    ...
    df2_is_view = not using_copy_on_write and request.node.callspec.id ==
    "shallow-copy"
