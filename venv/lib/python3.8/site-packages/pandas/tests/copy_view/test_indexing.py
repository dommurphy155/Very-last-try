...
def test_subset_row_slice(using_copy_on_write):
    ...
    expected = DataFrame({"a": [0, 3], "b": [5, 6], "c": [0.2, 0.3]},
    index=range(1, 3))
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
...
def test_subset_column_slice(using_copy_on_write, using_array_manager, dtype):
    ...
    expected = DataFrame({"b": [0, 5, 6], "c": np.array([7, 8, 9],
    dtype=dtype)})
    tm.assert_frame_equal(subset, expected)
    # original parent dataframe is not modified (also not for BlockManager
    case,
...
def test_subset_loc_rows_columns(
    ...
    tm.assert_frame_equal(subset, expected)
    # a few corner cases _do_ actually modify the parent (with both row and
    column
    ...
    if (
        ...
        and (using_array_manager or (dtype == "int64" and not
    using_copy_on_write))
    ):
        df_orig.iloc[1, 1] = 0
    tm.assert_frame_equal(df, df_orig)
...
def test_subset_set_with_row_indexer(indexer_si, indexer, using_copy_on_write):
    ...
    df = DataFrame({"a": [1, 2, 3, 4], "b": [4, 5, 6, 7], "c": [0.1, 0.2, 0.3,
    0.4]})
    df_orig = df.copy()
    subset = df[1:4]
    ...
    expected = DataFrame(
        {"a": [0, 0, 4], "b": [0, 0, 7], "c": [0.0, 0.0, 0.4]}, index=range(1,
    4)
    )
    tm.assert_frame_equal(subset, expected)
    if using_copy_on_write:
...
def test_subset_set_column_with_loc(using_copy_on_write, using_array_manager,
    dtype):
    ...
    if using_copy_on_write:
        ...
        with pd.option_context("chained_assignment", "warn"):
            # The (i)loc[:, col] inplace deprecation gets triggered here,
    ignore those
...
def test_subset_set_column_with_loc2(using_copy_on_write, using_array_manager):
    # Case: setting a single column with loc on a viewing subset
    # -> subset.loc[:, col] = value
    # separate test for case of DataFrame of a single column -> takes a
    separate
    # code path
...
def test_subset_set_columns(using_copy_on_write, dtype):
    ...
    expected = DataFrame({"a": [0, 0], "b": [5, 6], "c": [0, 0]},
    index=range(1, 3))
    tm.assert_frame_equal(subset, expected)
    tm.assert_frame_equal(df, df_orig)
...
def test_subset_set_with_column_indexer(
    ...
    subset._mgr._verify_integrity()
    expected = DataFrame({"a": [0, 0], "b": [0.0, 0.0], "c": [5, 6]},
    index=range(1, 3))
...
def test_series_subset_set_with_indexer(indexer_si, indexer,
    using_copy_on_write):
...
def test_dataframe_add_column_from_series():
    ...
    s[0] = 0
    expected = DataFrame({"a": [1, 2, 3], "b": [0.1, 0.2, 0.3], "new": [10,
    11, 12]})
