...
class TestSeriesFlexArithmetic:
    ...
    def test_flex_method_subclass_metadata_preservation(self,
    ...
    pairings = [(Series.div, operator.truediv, 1), (Series.rdiv, ops.rtruediv,
    1)]
...
class TestSeriesArithmetic:
    ...
    def test_add_na_handling(self):
        ...
        ser = Series(
            [Decimal("1.3"), Decimal("2.3")], index=[date(2012, 1, 1),
    date(2012, 1, 2)]
        )
...
class TestSeriesComparison:
    ...
    @pytest.mark.parametrize(
        "names", [(None, None, None), ("foo", "bar", None), ("baz", "baz",
    "baz")]
    )
    def test_ser_cmp_result_names(self, names, comparison_op):
        # datetime64 dtype
        op = comparison_op
        dti = date_range("1949-06-07 03:00:00", freq="H", periods=5,
    name=names[0])
        ...
        dti = pd.DatetimeIndex(dti, freq="infer")  # freq not preserved by
    tz_localize
        ser = Series(dti).rename(names[1])
        result = op(ser, dti)
        assert result.name == names[2]
    ...
    def test_comp_ops_df_compat(self, left, right, frame_or_series):
        # GH 1134
        msg = f"Can only compare identically-labeled
...
class TestTimeSeriesArithmetic:
    ...
    def test_datetime_understood(self):
        ...
        expected = Series(pd.to_datetime(["2011-12-26", "2011-12-27",
    "2011-12-28"]))
        tm.assert_series_equal(result, expected)
...
class TestNamePreservation:
    @pytest.mark.parametrize("box", [list, tuple, np.array, Index, Series,
    pd.array])
    @pytest.mark.parametrize("flex", [True, False])
    def test_series_ops_name_retention(self, flex, box, names,
...
class TestInplaceOperations:
    ...
    def test_series_inplace_ops(self, dtype1, dtype2, dtype_expected,
    dtype_mul):
        # GH 37910
...
def test_none_comparison(request, series_with_simple_index):
    ...
    if is_datetime64_dtype(series.dtype) or
    is_datetime64tz_dtype(series.dtype):
...
def test_series_varied_multiindex_alignment():
    ...
    s2 = Series(
        ...
        index=pd.MultiIndex.from_product([list("xy"), [1, 2]], names=["xy",
    "num"]),
    )
