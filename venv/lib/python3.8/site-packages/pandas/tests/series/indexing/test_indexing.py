...
def test_basic_getitem_dt64tz_values():
    ...
    # with tz for values
    ser = Series(
        date_range("2011-01-01", periods=3, tz="US/Eastern"), index=["a", "b",
    "c"]
    )
...
@pytest.mark.filterwarnings("ignore:.*append method is
...
def test_timedelta_assignment():
    ...
    tm.assert_series_equal(s, Series([np.nan, Timedelta("1 days")],
    index=["A", "B"]))
...
def test_multilevel_preserve_name(lexsorted_two_level_string_multiindex,
    indexer_sl):
...
def test_slice_with_negative_step(index):
    ...
    SLC[13:8:-1])
            tm.assert_indexing_slices_equivalent(ser, SLC[key:key2:-1],
    SLC[0:0:-1])
