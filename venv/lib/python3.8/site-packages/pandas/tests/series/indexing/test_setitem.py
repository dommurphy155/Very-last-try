...
class TestSetitemBooleanMask:
    ...
    def
    test_setitem_with_bool_mask_and_values_matching_n_trues_in_length(self):
        # GH#30567
        ser = Series([None] * 10)
        mask = [False] * 3 + [True] * 5 + [False] * 2
        ser[mask] = range(5)
        result = ser
        expected = Series([None] * 3 + list(range(5)) + [None] *
    2).astype("object")
        tm.assert_series_equal(result, expected)
    ...
    def test_setitem_mask_smallint_no_upcast(self):
        # like test_setitem_mask_smallint_upcast, but while we can't hold
    'alt',
...
class TestSetitemWithExpansion:
    ...
    def test_setitem_with_expansion_type_promotion(self):
        ...
        ser["b"] = 3.0
        ser["c"] = "foo"
        expected = Series([Timestamp("2016-01-01"), 3.0, "foo"], index=["a",
    "b", "c"])
        tm.assert_series_equal(ser, expected)
    ...
    def test_setitem_enlarge_with_na(self, na, target_na, dtype, target_dtype,
    indexer):
        # GH#32346
...
def test_setitem_categorical_assigning_ops():
    ...
    ser.index = ["x", "y"]
    ser["y"] = "a"
    exp = Series(Categorical(["b", "a"], categories=["a", "b"]), index=["x",
    "y"])
    tm.assert_series_equal(ser, exp)
...
class SetitemCastingEquivalents:
    """
    Check each of several methods that _should_ be equivalent to `obj[key] =
    val`
...
class TestSetitemNADatetimeLikeDtype(SetitemCastingEquivalents):
    ...
    @pytest.fixture(
        params=["m8[ns]", "M8[ns]", "datetime64[ns, UTC]", "datetime64[ns,
    US/Central]"]
    )
    def dtype(self, request):
        return request.param
    ...
    def is_inplace(self, val, obj):
        ...
        return val is NaT or val is None or val is np.nan or obj.dtype ==
    val.dtype
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Series([datetime(2000, 1, 1), datetime(2000, 1, 2), datetime(2000,
    1, 3)]),
...
@pytest.mark.parametrize(
    ...
    [
        np.array([2.0, 3.0]),
        np.array([2.5, 3.5]),
        np.array([2**65, 2**65 + 1], dtype=np.float64),  # all ints, but can't
    cast
    ],
)
...
@pytest.mark.parametrize(
    "val,exp_dtype",
    [(1, np.complex128), (1.1, np.complex128), (1 + 1j, np.complex128), (True,
    object)],
)
...
@pytest.mark.parametrize(
    "val,exp_dtype",
    [(1, np.int64), (1.1, np.float64), (1 + 1j, np.complex128), (True,
    object)],
)
...
@pytest.mark.parametrize(
    "val,exp_dtype",
    [(1, np.float64), (1.1, np.float64), (1 + 1j, np.complex128), (True,
    object)],
)
...
class TestCoercionFloat32(CoercionTest):
    ...
    def test_slice_key(self, obj, key, expected, val, indexer_sli, is_inplace):
        super().test_slice_key(obj, key, expected, val, indexer_sli,
    is_inplace)
...
@pytest.mark.parametrize(
    "val,exp_dtype",
    [
        (Timestamp("2012-01-01", tz="US/Eastern"), "datetime64[ns,
    US/Eastern]"),
...
class TestCoercionDatetime64TZ(CoercionTest):
    ...
    def assert_warns(self, request):
        ...
        if exp_dtype == object and isinstance(val, Timestamp) and val.tz is
    not None:
            with tm.assert_produces_warning(FutureWarning, match="mismatched
    timezone"):
                yield
...
def test_setitem_bool_indexer_dont_broadcast_length1_values(size, mask, item,
    box):
    # GH#44265
