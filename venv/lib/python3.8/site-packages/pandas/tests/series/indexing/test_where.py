...
def test_where_unsafe():
    ...
    msg = "cannot set using a list-like indexer with a different length than
    the value"
...
def test_where_datetimetz():
    # GH 15701
    timestamps = ["2016-12-31 12:00:04+00:00", "2016-12-31
    12:00:04.010000+00:00"]
    ser = Series([Timestamp(t) for t in timestamps], dtype="datetime64[ns,
    UTC]")
...
def test_where_categorical(frame_or_series):
    ...
    exp = frame_or_series(
        pd.Categorical(["A", "A", "B", "B", np.nan], categories=["A", "B",
    "C"]),
        dtype="category",
    )
