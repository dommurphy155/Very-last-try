...
class TestSeriesMisc:
    ...
    def test_index_tab_completion(self, index):
        ...
        for i, x in enumerate(s.index.unique(level=0)):
            if i < 100:
                assert not isinstance(x, str) or not x.isidentifier() or x in
    dir_s
            else:
    ...
    def test_numeric_only(self, kernel, has_numeric_only, dtype):
        ...
        if not has_numeric_only:
            ...
            if kernel == "rank":
                msg = "Calling Series.rank with numeric_only=True and dtype
    object"
                with tm.assert_produces_warning(FutureWarning, match=msg):
                    method(*args, numeric_only=True)
            else:
                warn_msg = (
                    f"Calling Series.{kernel} with numeric_only=True and dtype
    object"
                )
