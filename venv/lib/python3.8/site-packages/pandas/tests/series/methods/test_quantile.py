...
class TestSeriesQuantile:
    ...
    def test_quantile_multi(self, datetime_series):
        ...
        expected = Series(
            [Timestamp("2000-01-10 19:12:00"), Timestamp("2000-01-10
    19:12:00")],
            index=[0.2, 0.2],
            name="xxx",
        )
    ...
    def test_quantile_nan(self):
        ...
        for s in cases:
            ...
            tm.assert_series_equal(res, Series([np.nan, np.nan], index=[0.2,
    0.3]))
    ...
    @pytest.mark.parametrize(
        "values, dtype",
        [([0, 0, 0, 1, 2, 3], "Sparse[int]"), ([0.0, None, 1.0, 2.0],
    "Sparse[float]")],
    )
    ...
    def test_quantile_all_na(self, any_int_ea_dtype):
        ...
        expected = Series([pd.NA, pd.NA], dtype=any_int_ea_dtype, index=[0.1,
    0.5])
        tm.assert_series_equal(result, expected)
