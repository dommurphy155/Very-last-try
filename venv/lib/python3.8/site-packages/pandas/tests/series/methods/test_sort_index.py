...
class TestSeriesSortIndex:
    ...
    def test_sort_index_intervals(self):
        s = Series(
            [np.nan, 1, 2, 3], IntervalIndex.from_arrays([0, 1, 2, 3], [1, 2,
    3, 4])
        )
        ...
        expected = Series(
            [3, 2, 1, np.nan], IntervalIndex.from_arrays([3, 2, 1, 0], [4, 3,
    2, 1])
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_sort_index_ignore_index(
        self, inplace, original_list, sorted_list, ascending, ignore_index,
    output_index
    ):
        # GH 30114
...
class TestSeriesSortIndexKey:
    ...
    def test_sort_index_multiindex_key_multi_level(self):
        ...
        result = s.sort_index(level=["A", "C"], key=lambda x: x)  # nothing
    happens
        tm.assert_series_equal(backwards, result)
    ...
    def test_sort_index_key_int(self):
        series = Series(np.arange(6, dtype="int64"), index=np.arange(6,
    dtype="int64"))
    ...
    def test_sort_index_kind_neg_key(self, sort_kind):
        ...
        index_sorted_series = series.sort_index(kind=sort_kind, key=lambda x:
    -x)
        tm.assert_series_equal(expected_series, index_sorted_series)
    ...
    def test_sort_values_key_type(self):
        s = Series([1, 2, 3], DatetimeIndex(["2008-10-24", "2008-11-23",
    "2007-12-22"]))
