...
class TestSeriesDatetimeValues:
    ...
    def test_dt_namespace_accessor_datetime64tz(self):
        ...
        expected = Series(ser._values.tz_convert("CET"), index=ser.index,
    name="xxx")
        tm.assert_series_equal(result, expected)
    ...
    def test_dt_namespace_accessor_index_and_values(self):
        ...
        exp = Series(
            np.array([2014, 2014, 2014], dtype="int64"), index=index,
    name="xxx"
        )
        tm.assert_series_equal(ser.dt.year, exp)

        exp = Series(np.array([2, 2, 2], dtype="int64"), index=index,
    name="xxx")
        tm.assert_series_equal(ser.dt.month, exp)

        exp = Series(np.array([0, 1, 2], dtype="int64"), index=index,
    name="xxx")
    ...
    def test_dt_accessor_limited_display_api(self):
        ...
        results = get_dir(ser)
        tm.assert_almost_equal(results, sorted(set(ok_for_dt +
    ok_for_dt_methods)))

        # tzaware
        ser = Series(date_range("2015-01-01", "2016-01-01", freq="T"),
    name="xxx")
    ...
    def test_dt_accessor_not_writeable(self, using_copy_on_write):
        ...
        with pd.option_context("chained_assignment", "raise"):
            if using_copy_on_write:
                # TODO(CoW) it would be nice to keep a warning/error for this
    case
                ser.dt.hour[0] = 5
            else:
    ...
    def test_dt_round(self, method, dates):
        ...
        ser = Series(
            pd.to_datetime(
                ["2012-01-01 13:00:00", "2012-01-01 12:01:00", "2012-01-01
    08:00:00"]
            ),
            name="xxx",
        )
    ...
    def test_dt_round_tz_ambiguous(self, method):
        ...
        # bool-array
        result = getattr(df1.date.dt, method)("H", ambiguous=[True, False,
    False])
        tm.assert_series_equal(result, expected)
    ...
    def test_dt_round_tz_nonexistent(self, method, ts_str, freq):
        ...
        expected = Series([pd.Timestamp("2018-03-11 03:00:00",
    tz="America/Chicago")])
        tm.assert_series_equal(result, expected)
        ...
        with pytest.raises(pytz.NonExistentTimeError, match="2018-03-11
    02:00:00"):
    ...
    def test_dt_accessor_no_new_attributes(self):
        ...
        with pytest.raises(AttributeError, match="You cannot add any new
    attribute"):
            ser.dt.xlabel = "a"

    # error: Unsupported operand types for + ("List[None]" and "List[str]")
    @pytest.mark.parametrize(
        "time_locale", [None] + (tm.get_locales() or [])  # type:
    ignore[operator]
    )
    def test_dt_accessor_datetime_name_accessors(self, time_locale):
        ...
        ser = Series(date_range(freq="D", start=datetime(1998, 1, 1),
    periods=365))
        english_days = [
            "Monday",
            "Tuesday",
    ...
    "2013/01/05"]
        ...
        ser = Series(period_range("2015-02-03 11:22:33.4567", periods=5,
    freq="s"))
        result = ser.dt.strftime("%Y/%m/%d %H-%M-%S")
        expected = Series(
            [
    ...
    def test_strftime_dt64_microsecond_resolution(self):
        ser = Series([datetime(2013, 1, 1, 2, 32, 59), datetime(2013, 1, 2,
    14, 32, 1)])
        result = ser.dt.strftime("%Y-%m-%d %H:%M:%S")
    ...
    @pytest.mark.parametrize(
        "data", [DatetimeIndex([pd.NaT]), PeriodIndex([pd.NaT],
    dtype="period[D]")]
    )
    ...
    "day"]:
        ...
        result = ser.dt.time
        expected = Series([time(0), time(0), np.nan, time(0), time(0)],
    dtype="object")
        tm.assert_series_equal(result, expected)
    ...
    @pytest.mark.parametrize(
        "ser", [Series(np.arange(5)), Series(list("abcde")),
    ...
    def test_date_tz(self):
        ...
        ser = Series(rng)
        expected = Series([date(2014, 4, 4), date(2014, 7, 18), date(2015, 11,
    22)])
    ...
    def test_dt_timetz_accessor(self, tz_naive_fixture):
        ...
        ser = Series(dtindex)
        expected = Series(
            [time(23, 56, tzinfo=tz), time(21, 24, tzinfo=tz), time(22, 14,
    tzinfo=tz)]
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [["2010-01-01", pd.NaT], [[2009, 53, 5], [np.NaN, np.NaN,
    np.NaN]]],
            # see GH#36032
...
def test_normalize_pre_epoch_dates():
    # GH: 36294
    ser = pd.to_datetime(Series(["1969-01-01 09:00:00", "2016-01-01
    09:00:00"]))
