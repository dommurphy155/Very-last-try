...
def test_object_series_ok():
    ...
    tm.assert_series_equal(np.add(ser, Dummy(1)), pd.Series(np.add(ser,
    Dummy(1))))
...
@pytest.fixture(
    params=[
        ...
        pd.IntervalIndex([pd.Interval(0, 1), pd.Interval(2, 3), pd.Interval(1,
    2)]),
    ],
...
class TestNumpyReductions:
    ...
    def test_multiply(self, values_for_np_reduce, box_with_array, request):
        ...
        if isinstance(values, pd.core.arrays.SparseArray) and box is not
    pd.Index:
