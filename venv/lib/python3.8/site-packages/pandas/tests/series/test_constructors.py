...
class TestSeriesConstructors:
    @pytest.mark.parametrize(
        ...
        [
            # NOTE: some overlap with test_constructor_empty but that test
    does not
            # test for None or an empty generator.
            # test_constructor_pass_none tests None but only with the index
    also
            # passed.
            (lambda: Series(), True),
            ...
            (lambda: Series(data=[]), False),  # creates a RangeIndex
            (lambda: Series(data=(_ for _ in [])), False),  # creates a
    RangeIndex
        ],
    )
    ...
    def test_constructor_index_ndim_gt_1_raises(self):
        ...
        with pytest.raises(ValueError, match="Index data must be
    1-dimensional"):
    ...
    @pytest.mark.parametrize(
        "dtype",
        ["f8", "i8", "M8[ns]", "m8[ns]", "category", "object", "datetime64[ns,
    UTC]"],
    )
    ...
    def test_constructor_categorical_with_coercion2(self):
        ...
        x["person_name"] = Categorical(x.person_name)  # doing this breaks
    transform
    ...
    def test_constructor_categorical_dtype(self):
        ...
        expected = Series(
            ["a", "a"], index=[0, 1], dtype=CategoricalDtype(["a", "b"],
    ordered=True)
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_categorical_sideeffects_free(self):
        ...
        with tm.assert_produces_warning(FutureWarning, match="Use
    rename_categories"):
    ...
    def test_constructor_signed_int_overflow_deprecation(self):
        ...
        with tm.assert_produces_warning(warns, match=msg,
    ...
    def test_constructor_invalid_coerce_ints_with_float_nan(self,
    any_int_numpy_dtype):
        # GH 22585
    ...
    def test_constructor_datelike_coercion2(self):
        ...
        df = DataFrame({"wing1": wing1, "wing2": wing2, "mat": mat},
    index=belly)
    ...
    @pytest.mark.parametrize("interval_constructor", [IntervalIndex,
    IntervalArray])
    def test_construction_interval(self, interval_constructor):
        # construction from interval & array of intervals
        intervals = interval_constructor.from_breaks(np.arange(3),
    closed="right")
    ...
    def test_constructor_interval_mixed_closed(self, data_constructor):
        ...
        data = [Interval(0, 1, closed="both"), Interval(0, 2,
    closed="neither")]
        result = Series(data_constructor(data))
        assert result.dtype == object
    ...
    def test_constructor_dict_tuple_indexer(self):
        ...
        result = Series(data)
        expected = Series(
            -1.0, index=MultiIndex(levels=[[1], [1], [np.nan]], codes=[[0],
    [0], [-1]])
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_dict_of_tuples(self):
        ...
        expected = Series([3, 6], index=MultiIndex.from_tuples([(1, 2), (None,
    5)]))
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_dtype_timedelta64(self):
        ...
        td = Series([timedelta(days=1), timedelta(days=2), np.timedelta64(1,
    "s")])
    ...
    def test_convert_non_ns(self):
        ...
        arr = np.array(
            ["2013-01-01 00:00:01", "2013-01-01 00:00:02", "2013-01-01
    00:00:03"],
            dtype="datetime64[s]",
        )
        ser = Series(arr)
    ...
    def test_constructor_data_aware_dtype_naive(self, tz_aware_fixture, pydt):
        ...
        with tm.assert_produces_warning(FutureWarning):
            result = Series(np.array([ts], dtype=object),
    dtype="datetime64[ns]")
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_dtype_timedelta_alternative_construct(self):
        ...
        expected = Series(pd.to_timedelta([1000000, 200000, 3000000],
    unit="ns"))
        tm.assert_series_equal(result, expected)
    ...
    dtype="timedelta64[ns]").astype(
            "int64"
        )
        expected = Series([1000000, 200000, 3000000],
    dtype="timedelta64[s]").astype(
            "int64"
        )
        tm.assert_series_equal(result, expected)
    ...
    def test_constructor_mismatched_null_nullable_dtype(
        ...
        msg = "|".join(
            [
                ...
                "or a (real )?number",
                r"Cannot cast array data from dtype\('O'\) to
    dtype\('float64'\) "
